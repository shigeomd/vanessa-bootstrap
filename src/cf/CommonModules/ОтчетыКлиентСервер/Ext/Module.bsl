...
Функция КомпоновщикНастроек(Форма) Экспорт
	Если Форма.ИмяФормы = "ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.НастройкиОтчета" Тогда
...
Функция ПутьККомпоновщикуНастроек(Форма) Экспорт
	Если Форма.ИмяФормы = "ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.НастройкиОтчета" Тогда
...
Функция СкопироватьРекурсивно(Узел, ЧтоКопировать, КудаВставить, Индекс, Соответствие) Экспорт
	...
	Если ПараметрыКопирования.ИсключаяСвойства <> "*" Тогда
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ЧтоКопировать, , ПараметрыКопирования.ИсключаяСвойства);
	КонецЕсли;
	
	Если ПараметрыКопирования.КопируетсяДеревоФормы Тогда
		
		Соответствие.Вставить(ЧтоКопировать, НоваяСтрока);
		
		КоллекцияВложенных = ЧтоКопировать.ПолучитьЭлементы();
		Если КоллекцияВложенных.Количество() > 0 Тогда
			НоваяКоллекцияВложенных = НоваяСтрока.ПолучитьЭлементы();
	...
	Иначе
		...
		СтарыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(ЧтоКопировать);
		НовыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(НоваяСтрока);
		Соответствие.Вставить(СтарыйИдентификатор, НовыйИдентификатор);
		
		Если ПараметрыКопирования.ЕстьНастройки Тогда
			НоваяСтрока = НоваяСтрока.Настройки;
		...
		Если ПараметрыКопирования.ЕстьЭлементы Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьВыбор Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока.Выбор, ЧтоКопировать.Выбор, , "ДоступныеПоляВыбора, Элементы");
			КоллекцияВложенных = ЧтоКопировать.Выбор.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.Выбор.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьОтбор Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока.Отбор, ЧтоКопировать.Отбор, , "ДоступныеПоляОтбора, Элементы");
			КоллекцияВложенных = ЧтоКопировать.Отбор.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.Отбор.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
		...
		Если ПараметрыКопирования.ЕстьПараметрыВывода Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.ПараметрыВывода.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				УзелВложенных = НоваяСтрока.ПараметрыВывода;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
		...
		Если ПараметрыКопирования.ЕстьПараметрыДанных Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.ПараметрыДанных.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.ПараметрыДанных.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьПользовательскиеПоля Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.ПользовательскиеПоля.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.ПользовательскиеПоля.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьПоляГруппировки Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.ПоляГруппировки.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.ПоляГруппировки.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Новый Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьПорядок Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока.Порядок, ЧтоКопировать.Порядок, , "ДоступныеПоляПорядка, Элементы");
			КоллекцияВложенных = ЧтоКопировать.Порядок.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.Порядок.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьСтруктура Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока.Структура, ЧтоКопировать.Структура);
			КоллекцияВложенных = ЧтоКопировать.Структура;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.Структура;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьУсловноеОформление Тогда
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока.УсловноеОформление, ЧтоКопировать.УсловноеОформление, , "ДоступныеПоляОтбора, ДоступныеПоляПолей, Элементы");
			КоллекцияВложенных = ЧтоКопировать.УсловноеОформление.Элементы;
			Если КоллекцияВложенных.Количество() > 0 Тогда
				НоваяКоллекцияВложенных = НоваяСтрока.УсловноеОформление.Элементы;
				Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
					СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяКоллекцияВложенных, Неопределено, Соответствие);
				КонецЦикла;
			КонецЕсли;
		...
		Если ПараметрыКопирования.ЕстьКолонкиИСтроки Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.Колонки;
			НоваяКоллекцияВложенных = НоваяСтрока.Колонки;
			...
			КоллекцияВложенных = ЧтоКопировать.Строки;
			НоваяКоллекцияВложенных = НоваяСтрока.Строки;
			СтарыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(КоллекцияВложенных);
			НовыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(НоваяКоллекцияВложенных);
			Соответствие.Вставить(СтарыйИдентификатор, НовыйИдентификатор);
			Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
		...
		Если ПараметрыКопирования.ЕстьСерииИТочки Тогда
			...
			КоллекцияВложенных = ЧтоКопировать.Серии;
			НоваяКоллекцияВложенных = НоваяСтрока.Серии;
			СтарыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(КоллекцияВложенных);
			НовыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(НоваяКоллекцияВложенных);
			Соответствие.Вставить(СтарыйИдентификатор, НовыйИдентификатор);
			Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
			...
			КоллекцияВложенных = ЧтоКопировать.Точки;
			НоваяКоллекцияВложенных = НоваяСтрока.Точки;
			СтарыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(КоллекцияВложенных);
			НовыйИдентификатор = Узел.ПолучитьИдентификаторПоОбъекту(НоваяКоллекцияВложенных);
			Соответствие.Вставить(СтарыйИдентификатор, НовыйИдентификатор);
			Для Каждого ПодчиненнаяСтрока Из КоллекцияВложенных Цикл
		...
		Если ПараметрыКопирования.ЕстьЗначенияВложенныхПараметров Тогда
			...
			Для Каждого ПодчиненнаяСтрока Из ЧтоКопировать.ЗначенияВложенныхПараметров Цикл
				СкопироватьРекурсивно(Узел, ПодчиненнаяСтрока, НоваяСтрока.ЗначенияВложенныхПараметров, Неопределено, Соответствие);
			КонецЦикла;
		КонецЕсли;
		
		Если ПараметрыКопирования.ЕстьПоляИОформление Тогда
			Для Каждого ОформляемоеПоле Из ЧтоКопировать.Поля.Элементы Цикл
				ЗаполнитьЗначенияСвойств(НоваяСтрока.Поля.Элементы.Добавить(), ОформляемоеПоле);
			КонецЦикла;
			Для Каждого Источник Из ЧтоКопировать.Оформление.Элементы Цикл
				Приемник = НоваяСтрока.Оформление.НайтиЗначениеПараметра(Источник.Параметр);
...
Процедура ДополнитьСписок(СписокПриемник, Значения, КонтролироватьТип) Экспорт
	Если Значения = Неопределено Тогда
		Возврат;
	КонецЕсли;
	СписокИсточник = ЗначенияСписком(Значения);
	Если КонтролироватьТип Тогда
		ОписаниеТипов = СписокПриемник.ТипЗначения;
	КонецЕсли;
	Для Каждого ЭлементСписка Из СписокИсточник Цикл
		Значение = ЭлементСписка.Значение;
		Тип = ТипЗнч(Значение);
		Если КонтролироватьТип И Не ОписаниеТипов.СодержитТип(Тип) Тогда
			Продолжить;
		КонецЕсли;
		Если Тип = Тип("Число") Или Тип = Тип("Строка") Тогда
		...
		Иначе
			Представление = Неопределено;
		КонецЕсли;
		ДобавитьУникальноеЗначениеВСписок(СписокПриемник, Значение, Представление, ЭлементСписка.Пометка);
	КонецЦикла;
КонецПроцедуры
