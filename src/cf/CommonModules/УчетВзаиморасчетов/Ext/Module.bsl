...
Функция ОчиститьСуммыСпециальныхРежимовЗачетАвансов(ТаблицаРеквизиты, ТаблицаВзаиморасчетов, Отказ) Экспорт
	...
	Если ТаблицаРеквизиты = Неопределено 
		...
		ИЛИ ТаблицаВзаиморасчетов = Неопределено Тогда
...
Функция ПустаяТаблицаВзаиморасчетовПогашениеЗадолженности(УчитыватьЗадолженностьУСН, ВидДокумента = Неопределено) Экспорт
...
// ОПЛАТА ПЛАТЕЖНОЙ КАРТОЙ ДЛЯ УСН и ИП
...
Процедура СформироватьДвиженияПоПрочимРасчетам(ТаблицаОстатковПоПрочимРасчетам, Движения, Отказ) Экспорт
	...
	Если ТаблицаОстатковПоПрочимРасчетам = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьОстаткиАвансов(ТаблицаДокумента, Реквизиты, Отказ)
	...
	СубсчетаУСН = БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.УСНВспомогательныеСчета);
	Для каждого СубсчетУСН Из СубсчетаУСН Цикл
		Если БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(СубсчетУСН)
			И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоДоговорам(СубсчетУСН) Тогда
...
Процедура ДобавитьСтрокиПогашенияЗадолженности(ТаблицаВзаиморасчетов, НераспределеннаяСумма, НераспределеннаяСуммаБУ, КратностьВзаиморасчетов, ОстаткиЗадолженности, СтрокаПлатежа, Реквизиты, Отказ)
	...
	Если БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоДокументамРасчетов(Отбор.СчетРасчетов) Тогда
...
//	Строка
//
Функция СформироватьНадписьПорядокУчетаРасчетов(СчетРасчетов, СчетАвансов, СпособЗачетаАвансов) Экспорт
...
Процедура УстановитьПорядокУчетаРасчетов(Форма) Экспорт
	...
	Форма.ПорядокУчетаРасчетов = СформироватьНадписьПорядокУчетаРасчетов(
		Объект.СчетУчетаРасчетовСКонтрагентом, Объект.СчетУчетаРасчетовПоАвансам, Объект.СпособЗачетаАвансов);
		
	Если Найти(Форма.ПорядокУчетаРасчетов, "<...>") = 0 Тогда
	...
	Для каждого СтрокаВзаиморасчетов из Параметры.Взаиморасчеты Цикл
		...
		Если ОтражатьВНалоговомУчете Тогда
			...
			Если СвойстваКорСчета.НалоговыйУчет Тогда
				...
				Проводка["СуммаПР" + ДтКтКорСчета] = Проводка.Сумма - КорСуммаНУ;
				Проводка["СуммаВР" + ДтКтКорСчета] = 0;
			КонецЕсли;
