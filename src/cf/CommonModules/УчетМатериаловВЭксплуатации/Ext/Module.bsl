...
Процедура СформироватьДвиженияПередачаСпецодеждыСпецоснасткиВЭксплуатацию(ТаблицаМатериалы, ТаблицаСписанныеМатериалы, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаМатериалы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьТаблицуПогашениеСтоимостиСпецодеждыСпецоснастки(ТаблицаМатериалы = Неопределено, Реквизиты, Отказ)
	...
	Запрос.УстановитьПараметр("СчетаУчетаСпецодеждыЗабалансовый",   
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СпецодеждаВЭксплуатацииВспомогательный));
	Запрос.УстановитьПараметр("СчетаУчетаСпецоснасткиЗабалансовый", 
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СпецоснасткаВЭксплуатацииВспомогательный));
	Запрос.УстановитьПараметр("СчетаУчетаСпецодежды",              
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СпецодеждаВЭксплуатации));
	Запрос.УстановитьПараметр("СчетаУчетаСпецоснастки",            
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СпецоснасткаВЭксплуатации));
	...
	Иначе
		...
		Запрос.УстановитьПараметр("СписокПартийМатериаловВЭксплуатации",
			ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(ТаблицаМатериалы.ВыгрузитьКолонку("ПартияМатериаловВЭксплуатации")));
	...
	Для Каждого СтрокаТаблицы Из СтрокиПогашениеСтоимостиДляРаспределения Цикл
		...
		Если СтрокаТаблицы.СпособПогашенияСтоимости = Перечисления.СпособыПогашенияСтоимости.Линейный Тогда
			...
			СуммаПогашенияВР = СтрокаТаблицы.ПервоначальнаяСтоимостьВРКонечныйОстаток / СтрокаТаблицы.СрокПолезногоИспользования;
		...
		ИначеЕсли СтрокаТаблицы.СпособПогашенияСтоимости = Перечисления.СпособыПогашенияСтоимости.ПропорциональноОбъемуПродукцииРабот Тогда
			...
			СуммаПогашенияВР = СтрокаТаблицы.ПервоначальнаяСтоимостьВРКонечныйОстаток * СтрокаТаблицы.Выработка / СтрокаТаблицы.ОбъемПродукцииРабот;
			
		КонецЕсли;
		
		// Сумма погашения рассчитывается пропорционально количеству материалов,
		// по которым погашается стоимость.
		СуммаПогашения = ?(СтрокаТаблицы.КоличествоОстаток > 0,
						   СуммаПогашения / СтрокаТаблицы.КоличествоОстаток * СтрокаТаблицы.Количество,
						   0);
						   
		СуммаПогашенияНУ = ?(СтрокаТаблицы.КоличествоОстаток > 0,
						     СуммаПогашенияНУ / СтрокаТаблицы.КоличествоОстаток * СтрокаТаблицы.Количество,
						     0);
						   
		СуммаПогашенияПР =  ?(СтрокаТаблицы.КоличествоОстаток > 0,
							  СуммаПогашенияПР / СтрокаТаблицы.КоличествоОстаток * СтрокаТаблицы.Количество,
							  0);
		
		СуммаПогашенияВР =  ?(СтрокаТаблицы.КоличествоОстаток > 0,
							  СуммаПогашенияВР / СтрокаТаблицы.КоличествоОстаток * СтрокаТаблицы.Количество,
							  0);
							  
		СуммаПогашения = Окр(СуммаПогашения, 2);
		СуммаПогашенияНУ = Окр(СуммаПогашенияНУ, 2);
		СуммаПогашенияПР = Окр(СуммаПогашенияПР, 2);
		СуммаПогашенияВР = Окр(СуммаПогашенияВР, 2);
							  
		Погрешность = СуммаПогашенияНУ + СуммаПогашенияПР + СуммаПогашенияВР - СуммаПогашения;
							  
		Если Погрешность <> 0 Тогда 
          // Сумма погрешностей всех приемников заведомо равна 0, 
          // поэтому можно спрятать сумму погрешности в ПР, НУ или ВР (туда, где будет не заметно)
          Если СтрокаТаблицы.ПервоначальнаяСтоимостьПРКонечныйОстаток <> 0 Тогда // Проверяем суммы источника, а не результата оценки, чтобы у разных приемников погрешность была списана на одинаковую сумму
              СуммаПогашенияПР = СуммаПогашенияПР - Погрешность;
          ИначеЕсли СтрокаТаблицы.ПервоначальнаяСтоимостьНУКонечныйОстаток <> 0 Тогда
              СуммаПогашенияНУ = СуммаПогашенияНУ - Погрешность;
          ИначеЕсли СтрокаТаблицы.ПервоначальнаяСтоимостьВРКонечныйОстаток <> 0 Тогда // Маловероятно, чтобы 0 распределился в ненулевые суммы, но вдруг. 
              СуммаПогашенияВР = СуммаПогашенияВР - Погрешность;
          КонецЕсли;
		...
		СтрокаТаблицы.СуммаВР = Макс(0, Мин(СтрокаТаблицы.СтоимостьВРКонечныйОстаток, СуммаПогашенияВР));
...
Функция ПодготовитьТаблицуНачислениеПогашенияСтоимостиСпецодеждыСпецоснастки(ТаблицаМатериалы, Реквизиты, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаМатериалы Цикл
		...
		Если СтрокаТаблицы.СпособОтраженияРасходов.Способы.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ru='В способе отражения расходов по погашению стоимости для материала ""%1"" нет записей.
				|Рекомендуется проверить данные в документе передачи в эксплуатацию.'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки,
				СтрокаТаблицы.Номенклатура);
...
Функция ПодготовитьТаблицуСписанныйИзЭксплуатацииИнвентарь(ТаблицаМатериалы, Реквизиты, Отказ)
	...
	Запрос.УстановитьПараметр("СчетаУчетаИнвентаряЗабалансовый", 
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежностиВЭксплуатации));
...
Процедура СформироватьПроводкиПогашениеСтоимостиСпецодеждыСпецоснастки(ТаблицаПогашенияСтоимости, Реквизиты, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы из ТаблицаПогашенияСтоимости Цикл
		...
		Проводка.Содержание  = "Погашение стоимости " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаТаблицы.СчетПередачи);
		
		Проводка.СчетДт = СтрокаТаблицы.СчетРасходов;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаТаблицы.СубконтоРасходов1);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СтрокаТаблицы.СубконтоРасходов2);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаТаблицы.СубконтоРасходов3);
...
Процедура СформироватьПроводкиПоступлениеСпецодеждыСпецоснасткиВЭксплуатациюМЦ(ТаблицаПогашенияСтоимости, Реквизиты, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы из ТаблицаПогашенияСтоимости Цикл
		...
		Проводка.Период      = Реквизиты.Период;
		Проводка.Организация = Реквизиты.Организация;
		Проводка.Содержание  = "Передача " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаТаблицы.СчетПередачи) + " в эксплуатацию";
