...
Функция ЗаполнитьРеквизитыПриИзменении(МенеджерДокумента, КЗаполнению, Контекст, ИмяТабличнойЧасти = "", ДанныеСтроки = Неопределено, ВозвращатьТолькоИзмененные = Истина) Экспорт
	...
	ЗаполнитьКоллекцию(Коллекция, Правила.ПравилаЗаполнения, ДанныеЗаполнения, Отбор);
...
Процедура ЗаполнитьСтроки(Строки, ИмяТабличнойЧасти, Контекст, МенеджерОбъекта, ВключаяЗаполненные = Ложь) Экспорт
	...
	ЗаполнитьКоллекцию(Строки, Правила.ПравилаЗаполнения, ДанныеЗаполнения, Отбор);
...
Функция ПроверитьЗаполнение(Объект, Отказ, ПроверяемыеРеквизиты, РеквизитыЗаСсылками = Неопределено) Экспорт
	...
	Если Не ЗаполнитьСчетаУчетаПередЗаписью Тогда
		ДанныеЗаполнения = ДанныеЗаполненияИзДокумента(МенеджерОбъекта, Объект, Правила);
		ПланыВыполнения  = ПланыВыполненияПравил(Правила.ПравилаЗаполнения, ДанныеЗаполнения, "ПроверкаЗаполнения", Объект);
	КонецЕсли;
	...
	Для Каждого Правило Из Правила.ПравилаЗаполнения Цикл
		...
		ТребуетПроверкиКорректности = ТребуетПроверкиКорректности(Правило);
		...
		Если ПустаяСтрока(Правило.ТабличнаяЧасть) Тогда
			...
			ДополнитьНастройкиЭлемента(НастройкиЭлемента, 0, ПланВыполнения.Настройки);
			ПроверитьЗначение(Объект[Правило.ПолноеИмяРеквизита], Сообщения, Правило, ТребуетПроверкиКорректности, Объект, ПланВыполнения, НастройкиЭлемента);
		...
		Иначе
			...
			Для ИндексЭлемента = 0 По Коллекция.Количество() - 1 Цикл
				...
				ПроверитьЗначение(Элемент[Правило.Реквизит], Сообщения, Правило, ТребуетПроверкиКорректности, Элемент, ПланВыполнения, НастройкиЭлемента, Элемент.НомерСтроки);
...
Функция МетодЗаполнения(Правило)
	...
	Если РесурсНастройкиЗапасы(Роль) <> Неопределено Тогда
		Возврат "Запасы";
...
Функция ИсточникиДанныхПравила(Правило)
	...
	Если МетодЗаполнения = "Запасы" Или МетодЗаполнения = "Затраты" Или МетодЗаполнения = "СубконтоЗатрат" Тогда
...
Функция НастройкиДляЗаполнения(Правило)
	...
	ИначеЕсли МетодЗаполнения = "Затраты" Тогда
...
Функция НастройкиДляПроверкиЗаполнения(Правило)
	...
	Если МетодЗаполнения = "Расчеты" Тогда
...
Функция ПроверитьУсловиеПоФункциональнымОпциям(Условие) // очередность, сложность - 0
	...
	ИначеЕсли Условие = "ТоварыВЦенахПродажи" Тогда
...
Процедура ЗаполнитьРеквизит(Параметры)
	...
	ИначеЕсли МетодЗаполнения = "НематериальныеАктивы" Тогда
		...
		Параметры.Значение = ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов;
...
Функция ТребуетПроверкиКорректности(Правило)
	...
	Если ПроверятьСоответствиеСчетаВалютеДоговора(Правило) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ОшибкаЗаполнения(Элемент, Правило, ПланВыполнения, НастройкиЭлемента)
	
	Значение = Элемент[Правило.Реквизит];
	
	Если Не ЗначениеЗаполнено(Значение) Тогда
		Возврат "";
	КонецЕсли;
	
	ТребованиеБалансовостиСчета = ТребованиеБалансовостиСчетаЗапасов(Правило.Роль);
	...
	Ошибка = ОшибкаЗаполненияВыручки(Элемент, Правило);
	Если Не ПустаяСтрока(Ошибка) Тогда
		Возврат Ошибка;
	КонецЕсли;
	
	Ошибка = ОшибкаСоответствияСчетаВалютеДоговора(Элемент, Правило, ПланВыполнения, НастройкиЭлемента);
	Если Не ПустаяСтрока(Ошибка) Тогда
		Возврат Ошибка;
	КонецЕсли;
	
	Ошибка = ОшибкаЗаполненияСпособУчетаНДС(Элемент, Правило, ПланВыполнения, НастройкиЭлемента);
	Если Не ПустаяСтрока(Ошибка) Тогда
		Возврат Ошибка;
	КонецЕсли;
...
Функция РесурсНастройкиЗапасы(Роль)
	...
	Если Роль      = "Запасы"
		...
		Или Роль   = "ВозвратнаяТараНаСкладе" Тогда
...
Процедура ЗаполнитьЗапасы(Параметры)
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		...
		Если Роль = "ЗапасыОтгруженные" И ЗначениеЗаполнено(СчетВладелец) Тогда
			Параметры.Значение = ПланыСчетов.Хозрасчетный.СчетОтгруженныхЗапасов(СчетВладелец);
		ИначеЕсли Роль = "ЗапасыКромеЗабалансовых" Тогда
...
Функция ОшибкаЗаполненияСпособУчетаНДС(Элемент, Правило, ПланВыполнения, НастройкиЭлемента)
	
	Если Не ПроверятьСпособУчетаНДС(Правило) Тогда
		Возврат "";
	КонецЕсли;
	
	ПредъявленСчетФактура = ЗначениеРеквизитаЭлементаКоллекции(
		Элемент,
		Правило.ИсточникиДанных.РеквизитыОбъекта,
		"ПредъявленСчетФактура");
		
	Если ПредъявленСчетФактура = Неопределено Тогда
		ПредъявленСчетФактура = ПланВыполнения.ДанныеЗаполнения.ПредъявленСчетФактура;
	КонецЕсли;
...
Функция АналитикаЗатратОпределяетсяНоменклатуройУслуг(Роль)
	Возврат (Роль = "Затраты" Или Роль = "РасходыНаПродажу");
КонецФункции
...
Процедура ЗаполнитьЗатраты(Параметры)
	...
	Иначе
		Параметры.Значение = ДанныеНастройкиЭлементаКоллекции("ОсновнойСчетЗатрат", Параметры);
	КонецЕсли;
	...
	Если Роль = "Выручка" Тогда
		...
		Если ЗначениеЗаполнено(СчетВладелец) Тогда
			Для Каждого СчетЗапасов Из УчетДоходовРасходов.ЗапасыПродажаКоторыхПрочиеДоходы() Цикл
				Если БухгалтерскийУчетВызовСервераПовтИсп.СчетВИерархии(СчетВладелец, СчетЗапасов) Тогда
...
Функция ОшибкаЗаполненияВыручки(Элемент, Правило)
	
	Если Не ПроверятьСоответствиеВидовДеятельности(Правило) Тогда
		Возврат "";
	КонецЕсли;
	
	Выручка = Элемент[Правило.Реквизит];
	Расходы = Элемент[СвойствоВладельцаПравила(Правило, "Реквизит")];
...
Функция РесурсНастройкиРасчеты(Роль)
	...
	Если Роль = "РасчетыСПоставщиком" Тогда
...
Функция ПроверятьСоответствиеСчетаВалютеДоговора(Правило)
	...
	МетодЗаполнения = МетодЗаполнения(Правило);
	Возврат (МетодЗаполнения = "Расчеты");
	
КонецФункции

Функция ОшибкаСоответствияСчетаВалютеДоговора(Элемент, Правило, ПланВыполнения, НастройкиЭлемента)
	
	Если Не ПроверятьСоответствиеСчетаВалютеДоговора(Правило) Тогда
		Возврат "";
	КонецЕсли;
	
	УсловияРасчетов = ДанныеНастройкиЭлементаКоллекции("УсловияРасчетов", ПланВыполнения, НастройкиЭлемента);
	Если УсловияРасчетов = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	ВалютныйДоговор = (УсловияРасчетов.ВалютаВзаиморасчетов <> ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
	ВалютныйСчет    = Элемент[Правило.Реквизит].Валютный;
...
Процедура ЗаполнитьДеньги(Параметры)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВалютныйУчет") Тогда
		...
		ОплатаВВалюте = 
			ЗначениеЗаполнено(ВалютаДокумента) 
			И ВалютаДокумента <> ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
...
Процедура ЗаполнитьСубконтоНалогов(Параметры)
	...
	Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет Тогда
		...
		ТекущееЗначение = ЗначениеРеквизитаЭлементаКоллекции(Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "ВидНалоговогоОбязательства");
...
Функция ПланыВыполненияПравил(ПравилаЗаполнения, ДанныеЗаполнения, Режим, ЗаполняемыйОбъект = Неопределено, Отбор = Неопределено)
	...
	// План выполнения, в частноести, включает
	// - настройки
	// - исходные данные
	...
	Для Каждого Правило Из ПравилаЗаполнения Цикл
		...
		ПланВыполнения.ИндексПравила          = ПравилаЗаполнения.Индекс(Правило);
		ПланВыполнения.СоответствуетКонтексту = Истина; // Ниже можем пересмотреть
		...
		Иначе
			...
			Иначе
				...
				Если Коллекция.Количество() = 0 Тогда
					ПланВыполнения.СоответствуетКонтексту = Ложь;
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		ПланВыполнения.СоответствуетОтбору = ПравилоСоответствуетОтбору(Правило, Отбор, Коллекция);
		...
		Если ДляЗаполнения Или ТребуетПроверкиКорректности(Правило) Тогда
	...
	КонецЦикла; // По ПравилаЗаполнения
	
	Возврат ПланыВыполнения;
	
КонецФункции

Функция ПравилоСоответствуетОтбору(Правило, Отбор, Коллекция = Неопределено)
	...
	Если Отбор.Свойство("НеЗаполненныеРеквизиты") И Отбор.НеЗаполненныеРеквизиты 
		...
		И Не ТребуетПроверкиКорректности(Правило) Тогда
...
Функция РазбитьНаКластеры(ПредметыНастройки, ОптимальныйСоставКластера)
	...
	Если Кластеры.АтрибутыКлюча.Количество() > 0 Тогда
		...
		Кластеры.АтрибутыКлючаСтрокой = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Кластеры.АтрибутыКлюча);
...
Функция ИдентификаторНастройки(ИмяНастройки, ИсточникиДанных, ОдинаковаяДляВсехЭлементовКоллекции, ИмяТабличнойЧасти)
...
Процедура ЗаполнитьКоллекцию(Коллекция, ПравилаЗаполнения, ДанныеЗаполнения, Отбор = Неопределено)
	
	ТолькоНезаполненные = Отбор <> Неопределено И Отбор.Свойство("НеЗаполненныеРеквизиты") И Отбор.НеЗаполненныеРеквизиты;
	ПланыВыполнения     = ПланыВыполненияПравил(ПравилаЗаполнения, ДанныеЗаполнения, "ЗаполнениеКоллекции", Коллекция, Отбор);
	...
	Для ИндексЭлемента = 0 По Коллекция.Количество() - 1 Цикл
		...
		Для Каждого ПланВыполнения Из ПланыВыполнения Цикл
			...
			Правило = ПравилаЗаполнения[ПланВыполнения.ИндексПравила];
			...
			Если ТолькоНезаполненные И ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
				Если Не ТребуетПроверкиКорректности(Правило) Тогда
					Продолжить;
				КонецЕсли;
				Ошибка = ОшибкаЗаполнения(Элемент, Правило, ПланВыполнения, НастройкиЭлемента);
			...
			Если ТолькоНезаполненные Тогда
				...
				Для Каждого ПодчиненноеПравило Из ПодчиненныеПравила(Правило) Цикл
					ИндексПодчиненногоПравила = ПравилаЗаполнения.Индекс(ПодчиненноеПравило);
...
Процедура ЗаполнитьВсеКоллекции(Объект, Правила, ДанныеЗаполнения, Отбор)
	...
	Для Каждого ИмяКоллекции Из ИменаКоллекций Цикл
		...
		Если ПустаяСтрока(ИмяКоллекции) Тогда
			...
			ЗаполнитьКоллекцию(Коллекция, Правила.ПравилаЗаполнения, ДанныеЗаполнения, Отбор);
		Иначе
			ЗаполнитьКоллекцию(Объект[ИмяКоллекции], Правила.ПравилаЗаполнения, ДанныеЗаполнения, Отбор);
		КонецЕсли;
...
Процедура ПроверитьЗначение(Значение, Сообщения, Правило, ТребуетПроверкиКорректности, Элемент, ПланВыполнения, НастройкиЭлемента, НомерСтроки = 0)
				
	Если Не ЗначениеЗаполнено(Значение) Тогда
		
		Сообщение = Сообщения.Добавить();
		Сообщение.ПолноеИмяРеквизита = Правило.ПолноеИмяРеквизита;
