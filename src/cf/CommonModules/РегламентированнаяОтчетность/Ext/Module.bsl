...
Функция	ПолучитьКартуМногострочныхСекцийЛиста(ТаблДок, Верх, Низ)
	...
	Для Каждого Обл Из ТаблДок.Области Цикл
		Если НЕ Тип(Обл) = Тип("РисунокТабличногоДокумента") И Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки И Обл.Верх >= Верх И Обл.Низ <= Низ И Найти(ВРЕГ(Обл.Имя), "МНОГОСТРОЧНАЯЧАСТЬ") <> 0 Тогда
...
Процедура ПолучитьСведенияОПоказателяхОтчета(Форма, ПараметрыОтчета = Неопределено) Экспорт
	...
	//АвтозаполняемыеПоказателиОтчета = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОПоказателяхОтчета(Сред(Лев(Форма.ИмяФормы, Найти(Форма.ИмяФормы, ".Форма.") - 1), 7), Сред(Форма.ИмяФормы, Найти(Форма.ИмяФормы, ".Форма.") + 7), ПараметрыОтчета);
...
Процедура ЗаменитьСсылкиРазделенныйСпрУдалитьРеглОтчетыНаНеРазделенныйСпрРеглОтчеты() Экспорт
	...
	Пока ВыборкаУдалитьЭлектронныеПредставленияРегламентированныхОтчетов.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаУдалитьЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета) Тогда
			
			Если ТипЗнч(ВыборкаУдалитьЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета) = Тип("СправочникСсылка.УдалитьРегламентированныеОтчеты") Тогда
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаУдалитьЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета.ИсточникОтчета);
			Иначе
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаУдалитьЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета.Источник);
			КонецЕсли;
	...
	Пока ВыборкаЦиклыОбмена.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаЦиклыОбмена.УдалитьВидОтчета) Тогда
			
			Если ТипЗнч(ВыборкаЦиклыОбмена.УдалитьВидОтчета) = Тип("СправочникСсылка.УдалитьРегламентированныеОтчеты") Тогда
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаЦиклыОбмена.УдалитьВидОтчета.ИсточникОтчета);
			Иначе
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаЦиклыОбмена.УдалитьВидОтчета.Источник);
			КонецЕсли;
	...
	Пока ВыборкаЭлектронныеПредставленияРегламентированныхОтчетов.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета) Тогда
			
			Если ТипЗнч(ВыборкаЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета) = Тип("СправочникСсылка.УдалитьРегламентированныеОтчеты") Тогда
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета.ИсточникОтчета);
			Иначе
				РеглОтчет = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", ВыборкаЭлектронныеПредставленияРегламентированныхОтчетов.УдалитьВидОтчета.Источник);
			КонецЕсли;
...
Процедура ЗаполнитьПересозданныеНеразделенныеРегистры() Экспорт
	
	// Копирование ИндексУчетныхЗаписейДокументооборота.
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	УдалитьИндексУчетныхЗаписейДокументооборота.КодОбластиДанных КАК КодОбластиДанных,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.УникальныйИдентификаторУчетнойЗаписи КАК УникальныйИдентификаторУчетнойЗаписи,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.НаименованиеУчетнойЗаписи КАК НаименованиеУчетнойЗаписи,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.СерверSMTP КАК СерверSMTP,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ПортSMTP КАК ПортSMTP,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ТребуетсяSMTPАутентификация КАК ТребуетсяSMTPАутентификация,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ИмяПользователяSMTP КАК ИмяПользователяSMTP,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ПарольSMTP КАК ПарольSMTP,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.СерверPOP3 КАК СерверPOP3,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ПортPOP3 КАК ПортPOP3,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ИмяПользователяPOP3 КАК ИмяПользователяPOP3,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ПарольPOP3 КАК ПарольPOP3,
		|	УдалитьИндексУчетныхЗаписейДокументооборота.ТребуетсяОтправка КАК ТребуетсяОтправка
		|ИЗ
		|	РегистрСведений.УдалитьИндексУчетныхЗаписейДокументооборота КАК УдалитьИндексУчетныхЗаписейДокументооборота";
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			...
			Запись.ОбластьДанныхВспомогательныеДанные = Выборка.КодОбластиДанных;
			СписокПолей = "УникальныйИдентификаторУчетнойЗаписи,НаименованиеУчетнойЗаписи,СерверSMTP,ПортSMTP,ТребуетсяSMTPАутентификация,ИмяПользователяSMTP,ПарольSMTP,СерверPOP3,ПортPOP3,ИмяПользователяPOP3,ПарольPOP3,ТребуетсяОтправка";
			ЗаполнитьЗначенияСвойств(Запись, Выборка, СписокПолей);
	...
	// Копирование ОтправкиОтчетностиКОбработке.
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УдалитьОтправкиОтчетностиКОбработке.ОбластьДанных КАК ОбластьДанных,
		|	УдалитьОтправкиОтчетностиКОбработке.Идентификатор КАК Идентификатор,
		|	УдалитьОтправкиОтчетностиКОбработке.Статус КАК Статус,
		|	УдалитьОтправкиОтчетностиКОбработке.ТребуетАудита КАК ТребуетАудита
		|ИЗ
		|	РегистрСведений.УдалитьОтправкиОтчетностиКОбработке КАК УдалитьОтправкиОтчетностиКОбработке";
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			...
			СписокПолей = "Идентификатор,Статус,ТребуетАудита";
			ЗаполнитьЗначенияСвойств(Запись, Выборка, СписокПолей);
...
Функция ФорматВыгрузкиСтатистическойОтчетностиВЭлектронномВиде(Организация) Экспорт
	...
	СтруктураФорматовВыгрузки = ХранилищеНастроекДанныхФорм.Загрузить(
		"Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.НастройкиФорматаВыгрузкиОтчетовСтатистики",
		...
		Для каждого КлючИЗначение Из КонвертируемыеПоказатели Цикл
			...
			Если ТипЗнч(ЗначениеПоказателя) = Тип("Число") Тогда
				...
				Если НаправлениеИзКонтейнера И Лев(СтрТипДанных, 1) = "N" Тогда
					...
					ПозицияРазделителя = Макс(Найти(СтрФормат, ","), Найти(СтрФормат, "."));
...
Функция ЧислоПоФормату(Значение, СтрокаФормата, УдалитьНезначащиеНулиПослеРазделителя = Ложь) Экспорт 
	...
	Если  УдалитьНезначащиеНулиПослеРазделителя Тогда
		...
		ПозРазд = Найти(ЗначениеПоФормату, Разд);
...
Процедура УстановитьЦветФорматДоступностьЯчеек(Форма) Экспорт 
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(Форма.ИмяФормы, Найти(Форма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(СтруктураРеквизитовФормы.мСоставПоказателей);
	...
	НезаполняемыеПоля = ИнтерфейсыВзаимодействияБРО.ПоляКоторыеНеВсегдаТребуетсяЗаполнять(
		Сред(Лев(Форма.ИмяФормы, Найти(Форма.ИмяФормы, ".Форма.") - 1), 7),
...
Функция ИзвлечьСтруктуруXML(ИмяФормы, ИмяМакета) Экспорт 
	...
	Если Метаданные.Отчеты.Найти(ИмяФормы) = Неопределено Тогда 
		Макет = Отчеты[Сред(Лев(ИмяФормы, Найти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
	Иначе
...
Процедура ОтсечьНезаполненныеНеобязательныеУзлыСтатистики(Узел) Экспорт 
	...
	Если ТипЗнч(Узел) <> Тип("ДеревоЗначений") Тогда
		...
		ИначеЕсли (Найти(Узел.Обязательность, "М") <> 0 ИЛИ Найти(Узел.Обязательность, "M") <> 0) // учтем оба варианта: кириллицу и латиницу
			И УзелПуст(Узел)
			И ?(Найти(Узел.Обязательность, "О") <> 0 ИЛИ Найти(Узел.Обязательность, "O") <> 0, ИмеютсяАналогичныеСоседниеУзлыКлючРаздел(Узел), Истина) Тогда
			УдалитьУзел(Узел);
		КонецЕсли;
	КонецЕсли;
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	Иначе
		Если Найти(ИсточникОтчета, "Статистика") > 0 Тогда
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСГС;
		ИначеЕсли Найти(ИсточникОтчета, "Алко") > 0 Тогда
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСРАР;
		ИначеЕсли Найти(ИсточникОтчета, "ОкрСреда") > 0
		ИЛИ Найти(ИсточникОтчета, "ОтходыСубъектовМСП") > 0 
		ИЛИ Найти(ИсточникОтчета, "Форма2ТПОтходы") > 0 Тогда
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.РПН;
		ИначеЕсли Найти(ИсточникОтчета, "4ФСС") > 0 Тогда
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС;
		ИначеЕсли Найти(ИсточникОтчета, "РВ3") > 0 ИЛИ Найти(ИсточникОтчета, "РСВ") > 0 Тогда
...
Процедура РегламентированныйОтчетОтправляетсяВКонтролирующиеОрганы(ДокСсылка, ОбъектОтправляется) Экспорт
	...
	РеглОтчетыНеОтправляютсяВКонтролирующиеОрганы.Добавить("РегламентированныйОтчетРозничныеЦеныНаТабак");
...
Функция ПолучитьМассивФормСтатистикиТолькоДляОрганизации()
	...
	Результат.Добавить("РегламентированныйОтчетСведенияОДолеДоходовОтОбразовательнойИлиМедицинскойДеятельности");
...
Процедура УстановитьОтборВФормеВыбораОтчета(Форма) Экспорт
	...
	Если Найти(ИмяОтчета, "ОсновнаяФорма") = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИмяОтчета = Сред(ИмяОтчета, СтрДлина("Отчет.") + 1);
	ИмяОтчета = Лев(ИмяОтчета, Найти(ИмяОтчета, ".") - 1);
...
Функция ОтчетыОценкиРисков(Организация, ДатаНачала, ДатаОкончания)
	...
	Запрос.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	Запрос.УстановитьПараметр("ВидыОтчетов", ВидыОтчетовОценкиРисков());
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		УсловияПоиска = Новый Структура("ИсточникОтчета, ВыбраннаяФорма, ПериодДокумента, КодНалоговогоОргана");
		...
		Иначе
			...
			Если СтрокаТаблицы.Вид > Выборка.Вид Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтрокаТаблицы.Вид = Выборка.Вид
			   И СтрокаТаблицы.МоментВремени.Сравнить(Выборка.МоментВремени) >= 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ВставитьПоказатели_НДПИ(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Пока Раздел1.Свойство("П000100001003" + Постфикс) Цикл
		ЗначениеПоказателя = ЗначениеПоказателя
		                   + Раздел1["П000100001003" + Постфикс];
...
Процедура ВставитьПоказатели_3НДФЛ(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.ЛистВ Цикл
		ДанныеЛиста = Лист.Данные;
		ЗначениеПоказателя = ЗначениеПоказателя
		                   + ПоказательОтчета(ДанныеЛиста, "П000090001101");
	КонецЦикла;
	...
	Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.ЛистВ Цикл
		ДанныеЛиста = Лист.Данные;
		ЗначениеПоказателя = ЗначениеПоказателя
		                   + ПоказательОтчета(ДанныеЛиста, "П000090001004");
	КонецЦикла;
...
Функция КомментарийКПроверкеВыгрузки(СсылкаНаСтрокуСообщения)
	...
	Возврат "GUID " + ГУИД + ". " + Наименование + ". " + Вид + ". " + Период + ". " + ТекстСообщенияОбОшибке;
	
КонецФункции

Функция КомментарийКВыгрузкеРеглОтчета(СсылкаНаОбъект)
	
	Возврат "GUID " + СсылкаНаОбъект.УникальныйИдентификатор();
...
Функция КомментарийКСозданиюРеглОтчета(СсылкаНаОбъект)
	...
	Возврат "GUID " + ГУИД + ". " + Наименование + ". " + Вид + ". " + Период;
