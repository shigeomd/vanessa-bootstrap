	...
	Если НЕ УпрощенныйУчетНДС Тогда
		...
		Иначе
			...
			СформироватьДвиженияВыбытиеТоваров(
				СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
			СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеОтгруженныхТоваров(
				СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Движения, Отказ);
...
Процедура СформироватьДвиженияВыбытиеТоваров(СписанныеПартииНДС, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ) Экспорт
	...
	Если РаздельныйУчетНДС Тогда
		...
		СписанныеПартииНДС.Индексы.Добавить("СтавкаНДСДокумента");
		СписанныеПартииСтавка0 = СписанныеПартииНДС.Скопировать(
			Новый Структура("СтавкаНДСДокумента", Перечисления.СтавкиНДС.НДС0));
...
Процедура СформироватьДвиженияОприходованиеТоваров(ТаблицаТовары, ТаблицаНомераГТД, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ УпрощенныйУчетНДС Тогда
		...
		Иначе
			...
			Параметры.Товары.Индексы.Добавить("ВидЦенности");
			ОсновныеСредства = Параметры.Товары.Скопировать(Новый Структура("ВидЦенности", Перечисления.ВидыЦенностей.ОС));
			СформироватьДвиженияНДСПоПриобретеннымЦенностямОприходованиеТоваров(ОсновныеСредства, Реквизиты, Движения, Отказ);
...
Процедура СформироватьДвиженияПеремещениеТоваров(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, СписанныеПартииНДС, Движения, Отказ) Экспорт
	...
	Если Параметры.СписанныеТоварыБухУчет.Количество() = 0 
	...
	Если НЕ УпрощенныйУчетНДС Тогда
		...
		Если РаздельныйУчетНДС Тогда
			...
			Иначе
				...
				КоличествоСтрокТаблицы = Параметры.СписанныеТоварыБухУчет.Количество();
				
				Для ИндексСтроки = 0 по КоличествоСтрокТаблицы - 1 Цикл
					
					ТаблицаСписанныеТоварыБухУчет[ИндексСтроки].СуммаСписания = Параметры.СписанныеТоварыБухУчет[ИндексСтроки].СуммаСписания;
					ТаблицаСписанныеТоварыБухУчет[ИндексСтроки].СуммаСписанияНУ = Параметры.СписанныеТоварыБухУчет[ИндексСтроки].СуммаСписанияНУ;
					ТаблицаСписанныеТоварыБухУчет[ИндексСтроки].СуммаКорСписанияНУ = Параметры.СписанныеТоварыБухУчет[ИндексСтроки].СуммаКорСписанияНУ;
		...
		Иначе
			...
			Если НЕ ОтправительНТТПоПродажнойСтоимости Тогда
				...
				СформироватьДвиженияВыбытиеТоваров(
					СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
				Если НЕ ПолучательНТТПоПродажнойСтоимости Тогда
					СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеНаДругойСклад(
						СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
				КонецЕсли;
...
Функция ПодготовитьПараметрыСписаниеТоваровНедостача(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
	...
	Параметры.Товары.Колонки.Добавить("НовыйСпособУчетаНДС", 
		Новый ОписаниеТипов("ПеречислениеСсылка.СпособыУчетаНДС"));
		
	Параметры.Товары.ЗаполнитьЗначения(Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости, "НовыйСпособУчетаНДС");
...
Процедура СформироватьДвиженияКомплектацияТоваров(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ УпрощенныйУчетНДС Тогда
		...
		Иначе
			...
			СформироватьДвиженияВыбытиеТоваров(
				СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
			СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеКомплекта(
...
Процедура СформироватьДвиженияРазукомплектацияТоваров(ТаблицаКомплектующие, ТаблицаСписанныеТоварыБухУчет, ТаблицаТоварыСГТД, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ УпрощенныйУчетНДС Тогда
		...
		Иначе
			...
			СформироватьДвиженияВыбытиеТоваров(
				СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
			СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеКомплектующих(
				Параметры.Комплектующие, СписанныеПартииНДС, Реквизиты, Движения, Отказ);
...
Функция ПодготовитьПараметрыРазукомплектацияТоваров(ТаблицаКомплектующие, ТаблицаСписанныеТоварыБухУчет, ТаблицаТоварыСГТД, ТаблицаРеквизиты)
	...
	+ "Содержание"	          // <Строка,150>
	;
	Параметры.Вставить("ТоварыСГТД", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		ТаблицаТоварыСГТД, СписокОбязательныхКолонок));
...
Процедура СформироватьДвиженияПередачаМатериаловВЭксплуатацию(ТаблицаМатериалы, ТаблицаСписанныеМатериалыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.Материалы.Количество() = 0
		ИЛИ Параметры.СписанныеМатериалыБухУчет.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ УпрощенныйУчетНДС
		И РаздельныйУчетНДС Тогда
...
Функция ПолучитьТаблицуСписанныеПартииНДС(Товары, СписанныеТоварыБухУчет, Реквизиты, Отказ) Экспорт
	...
	Для каждого СтрокаДокумента Из Товары Цикл
		...
		Для каждого СписаннаяПартияБухУчет Из СписанныеТоварыПоСтрокеБухУчет Цикл
			...
			ОстаткиПартийНДСПоСтроке.Сортировать("ДатаПартии,Партия,ДатаСФ,СчетФактура", Новый СравнениеЗначений);
			...
			Для каждого ОстатокПартии Из ОстаткиПартийНДСПоСтроке Цикл
				...
				Если Реквизиты.НДСВСтоимостиТоваров = Перечисления.ДействиеНДСВСтоимостиТоваров.ИсключитьИзСтоимости 
					И ЕстьДополнительныеКолонки Тогда
					
					СписаннаяПартияБухУчет.СуммаСписания = СписаннаяПартияБухУчет.СуммаСписания - НоваяСтрока.НДС;
					СписаннаяПартияБухУчет.СуммаСписанияНУ = СписаннаяПартияБухУчет.СуммаСписанияНУ - НоваяСтрока.НДС;
					СписаннаяПартияБухУчет.СуммаКорСписанияНУ = СписаннаяПартияБухУчет.СуммаКорСписанияНУ - НоваяСтрока.НДС;
...
Функция ПолучитьОстаткиПартийНДС(СписанныеТовары, Реквизиты)
	...
	Для каждого СтрокаТаблицы Из СчетаУчета Цикл
		СтрокаТаблицы.ВестиСуммовойУчетПоСкладам = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(
...
Процедура ОпределитьВключениеИсключениеНДСИзСтоимости(СписанныеПартииНДС, НДСВСтоимостиТоваров, Реквизиты = Неопределено)
	...
	// Для документа "Возврат товаров поставщику" индивидуальный порядок определения необходимости включения НДС в стоимость
	ВозвратТоваровПоставщикуНДСВключенВСтоимость = Ложь; 
	Если Реквизиты <> Неопределено Тогда
		Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
			ВозвратТоваровПоставщикуНДСВключенВСтоимость = Реквизиты.НДСВключенВСтоимость;
		КонецЕсли;	
	КонецЕсли;	
	...
	Для каждого СтрокаТаблицы Из СписанныеПартииНДС Цикл
		...
		Иначе
			...
			Если СтрокаТаблицы.НДСБылВключенВСтоимость
				...
				И НЕ ВозвратТоваровПоставщикуНДСВключенВСтоимость Тогда
				// Исключаем Из стоимости партии НДС, ранее включенный в стоимость,
...
Процедура СформироватьДвиженияПринятиеНаУчетНМА(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Параметры = ПодготовитьПараметрыПринятиеНаУчетНМА(ТаблицаРеквизиты);
	Если Параметры.Реквизиты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("Организация",            Реквизиты.Организация);
	Запрос.УстановитьПараметр("Объект",                 Реквизиты.НематериальныйАктив);
	Запрос.УстановитьПараметр("КонецПериода",           Новый МоментВремени(Реквизиты.Период, Реквизиты.Регистратор));
	Запрос.УстановитьПараметр("Состояние",              Реквизиты.СостояниеНМА);
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ИспользоватьУправляемыеБлокировки() Тогда
		...
		ПараметрыБлокировки = Новый Структура("ТипТаблицы, ИмяТаблицы",
			"РегистрНакопления", "НДСпоОСиНМА");
		...
		ЗначенияБлокировки.Вставить("Состояние",   Реквизиты.СостояниеНМА);

		ОписанияИсточника = Новый Структура;
		ОписанияИсточника.Вставить("НематериальныйАктив", "НематериальныйАктив");
	...
	ТаблицаНДСпоОСиНМА.ЗагрузитьКолонку(ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
		Окр(Реквизиты.КоэффициентРаспределения * ТаблицаНДСпоОСиНМА.Итог("СуммаCНДС"), 2),
		ТаблицаНДСпоОСиНМА.ВыгрузитьКолонку("СуммаCНДС")), "СуммаCНДС");

	Если ТаблицаНДСпоОСиНМА.Итог("СуммаБезНДС") <> 0 Тогда
		ТаблицаНДСпоОСиНМА.ЗагрузитьКолонку(ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
			Окр(Реквизиты.КоэффициентРаспределения * ТаблицаНДСпоОСиНМА.Итог("СуммаБезНДС"), 2),
			ТаблицаНДСпоОСиНМА.ВыгрузитьКолонку("СуммаБезНДС")), "СуммаБезНДС");
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из ТаблицаНДСпоОСиНМА Цикл
		СтрокаТаблицы.НДС = СтрокаТаблицы.СуммаCНДС - СтрокаТаблицы.СуммаБезНДС;
		
		Запись = Движения.НДСпоОСиНМА.ДобавитьРасход();
		ЗаполнитьЗначенияСвойств(Запись, СтрокаТаблицы);
		
		Запись.Событие     = Перечисления.СобытияПоНДСПокупки.ОСВведеноВЭксплуатацию;
		Запись.ДатаСобытия = Реквизиты.Период;
		Запись.Период      = Реквизиты.Период;
		
	КонецЦикла;
	
	Движения.НДСпоОСиНМА.Записывать = Истина;

КонецПроцедуры

Функция ПодготовитьПараметрыПринятиеНаУчетНМА(ТаблицаРеквизиты)
...
Функция ПодготовитьТаблицуПредъявленногоНДС(ТаблицаРеквизиты, Отказ) Экспорт
	...
	ВерсияУчетаНДС    = УчетНДСКлиентСервер.Версия(Реквизиты.Период);
	УпрощенныйУчетНДС = ВерсияУчетаНДС = 1 И УчетнаяПолитика.УпрощенныйУчетНДС(Реквизиты.Организация, Реквизиты.Период);
	НДСПредъявленКВычету = (УпрощенныйУчетНДС ИЛИ ВерсияУчетаНДС = 2)
		И Реквизиты.НДСПредъявленКВычету;
	Если НЕ НДСПредъявленКВычету Тогда
		Возврат ПустаяТаблица;
	КонецЕсли;	
	...
	Запрос.Текст =  
	"ВЫБРАТЬ
	|	СчетФактураПолученныйДокументыОснования.ДокументОснование КАК Регистратор,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(СчетФактураПолученныйДокументыОснования.ДокументОснование КАК Документ.АвансовыйОтчет) ССЫЛКА Документ.АвансовыйОтчет
	|			ТОГДА СчетФактураПолученныйДокументыОснования.Ссылка
	|		КОГДА ВЫРАЗИТЬ(СчетФактураПолученныйДокументыОснования.ДокументОснование КАК Документ.ОтчетКомиссионераОПродажах) ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|				И СчетФактураПолученныйДокументыОснования.Ссылка.ВозвратЧерезКомиссионера
	|			ТОГДА СчетФактураПолученныйДокументыОснования.Ссылка
	...
	|	И НДСПредъявленный.Регистратор В
	|			(ВЫБРАТЬ
	|				ДокументыОснования.Регистратор
	|			ИЗ
	|				ДокументыОснования)
	...
	|	НДСПредъявленныйОстатки.СуммаБезНДС,
	|	НДСПредъявленныйОстатки.НДС,
	|	ВЫБОР
	...
	|ГДЕ
	|	(НДСПредъявленныйОстатки.СуммаБезНДС <> 0
	|			ИЛИ НДСПредъявленныйОстатки.НДС <> 0)";
...
Процедура РаспределитьОплатыПоТаблице(ТаблицаНДС, ТаблицаОплат, ОтбиратьПоПараметрам = Ложь, ДопКолонкиСумм = Неопределено)
	...
	Для каждого СтрокаНДС Из ТаблицаНДС Цикл
		...
		Если СтрокаНДС.ВидЦенности = Перечисления.ВидыЦенностей.НалоговыйАгентАренда
			ИЛИ СтрокаНДС.ВидЦенности = Перечисления.ВидыЦенностей.НалоговыйАгентИностранцы
			ИЛИ СтрокаНДС.ВидЦенности = Перечисления.ВидыЦенностей.НалоговыйАгентРеализацияИмущества Тогда
...
Процедура СформироватьДвиженияНДСНачисленныйРеализацияТоваровУслуг(ТоварыУслуги, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаВыручки Из ТоварыУслуги Цикл
		...
		ИначеЕсли БухгалтерскийУчетВызовСервераПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаВыручки.СчетДоходов) Тогда
...
Процедура СформироватьДвиженияНДСНачисленныйВозвратТоваровОтПокупателя(ДанныеДвижений, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из ДанныеДвижений Цикл
		...
		ИначеЕсли БухгалтерскийУчетВызовСервераПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаТаблицы.СчетДоходов) Тогда
...
Процедура СформироватьДвиженияНДСНачисленныйСуммовыеРазницы(ТаблицаСуммовыеРазницы, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.СуммовыеРазницы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеТоваров(Товары, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		КонецЕсли;	
		Если НЕ БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(СтрокаТаблицы.СчетУчета) Тогда
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеНаДругойСклад(СписанныеПартииНДС, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из СписанныеПартииНДС Цикл
		...
		ВедетсяСкладскойУчет = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(СчетУчетаПолучатель);
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеИзНТТ(Товары, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Если НЕ БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(Запись.СчетУчета) Тогда
			Запись.Склад = Неопределено;
		Иначе
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеКомплекта(СписанныеПартииНДС, Реквизиты, Движения, Отказ) Экспорт
	...
	// Склад записывается в движения, только если на счете есть аналитика по складам и ведется учет по сумме
	ВедетсяСкладскойУчет = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(Реквизиты.СчетУчетаКомплектов);
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямПоступлениеКомплектующих(Комплектующие, СписанныеПартииНДС, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Комплектующие Цикл
		...
		// Склад записывается в движения, только если на счете есть аналитика по складам и ведется учет по сумме
		ВедетсяСкладскойУчет = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(СтрокаТаблицы.СчетУчета);
		Если ВедетсяСкладскойУчет Тогда
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямВозвратТоваровПокупателем(ТаблицаВозвращаемыеТовары, Реквизиты, Движения, Отказ) Экспорт
	...
	ТаблицаВозвращаемыеТовары.Свернуть("Номенклатура,СчетУчета", "Количество");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВозвращаемыеТовары Цикл
		...
		СтруктураПоиска = Новый Структура("Номенклатура,СчетУчета", СтрокаТаблицы.Номенклатура, СтрокаТаблицы.СчетУчета);
...
Процедура СформироватьПроводкиПоступлениеГТД(НомераГТД, Реквизиты, Движения, Отказ) Экспорт
	...
	Если НомераГТД.Количество() = 0 
		ИЛИ УчетНДС.КонтрагентРезидентТаможенногоСоюза(Реквизиты.Контрагент) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьПроводкиСписаниеГТД(Товары, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СтранаПроисхождения)
			ИЛИ СтрокаТаблицы.СтранаПроисхождения = Справочники.СтраныМира.РОССИЯ
			ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицы.НомерГТД)
	...
	ВыборкаПоНомерамГТД = Результат.Выбрать();
	Пока ВыборкаПоНомерамГТД.Следующий() Цикл

		Если КонтролироватьОстаток Тогда
			КоличествоВПроводке = Мин(ВыборкаПоНомерамГТД.Количество, ВыборкаПоНомерамГТД.КоличествоОстаток);
		Иначе
			КоличествоВПроводке = ВыборкаПоНомерамГТД.Количество;
		КонецЕсли; 
		...
		Если КоличествоВПроводке > 0 Тогда
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт,
				"Номенклатура", ВыборкаПоНомерамГТД.Номенклатура);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт,
				"НомераГТД", ВыборкаПоНомерамГТД.НомерГТД);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт,
				"СтраныПроисхождения", ВыборкаПоНомерамГТД.СтранаПроисхождения);
	...
	Если ДостаточноДанных Тогда
		...
		Если ВключитьВСтоимость Тогда
			...
			Если НЕ СуммаКВключениюВСтоимость = 0 Тогда
				...
				Если Реквизиты.ПартионныйУчет И ЕстьПартияВСтрокеСписания И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(СтрокаСписанияПроводка.СчетДт)
...
Функция ПодготовитьТаблицуКорректировкаНДСПоПартиямЗапасов(Реквизиты)
	...
	ВестиСкладскойУчетБУ = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(ПланыСчетов.Хозрасчетный.Товары);
...
Функция ПолучитьСчетаУчетаРаспределяемыхРасходов()
	...
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОсновноеПроизводство) Цикл // 20
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства) Цикл // 23
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОбщепроизводственныеРасходы) Цикл // 25
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОбщехозяйственныеРасходы) Цикл // 26
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОбслуживающиеПроизводства) Цикл // 29
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ИздержкиОбращения) Цикл // 44.01
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
	Для каждого Счет Из БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КоммерческиеРасходы) Цикл // 44.02
		МассивСчетов.Добавить(Счет);
	КонецЦикла;
...
Функция ПолучитьТаблицуСчетФактураВыданный(ДокументСсылка, Ошибка)
	...
	Если ДокументСсылка.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда
	...
	НоваяСтрока.ВидЦенности = ?(
		ДокументСсылка.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАванс,
			Перечисления.ВидыЦенностей.АвансыПолученные,
			Перечисления.ВидыЦенностей.СуммыСвязанныеСРасчетамиПоОплате);
	НоваяСтрока.СчетФактура  = ДокументСсылка.ДокументОснование;
	НоваяСтрока.Сумма        = ДокументСсылка.Сумма - ДокументСсылка.СуммаНДС;
	НоваяСтрока.СтавкаНДС    = ДокументСсылка.СтавкаНДС;
	НоваяСтрока.НДС          = ДокументСсылка.СуммаНДС;
...
Функция ПолучитьТаблицуПриходныйКассовыйОрдер(ДокументОбъект, Ошибка)
	...
	Если НЕ ДокументОбъект.ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить() Тогда
		...
		СуммаДокументаВВалютеРеглУчета = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(ТаблицаВыручки.Итог("СуммаВал"),
...
//
Процедура ОпределитьИсправленныеСчетаФактуры(СписанныеПартии, Реквизиты) Экспорт
	...
	СписокСчетовФактур	= ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(СписанныеПартии.ВыгрузитьКолонку("СчетФактура"), Истина);
...
Функция ОпределитьДатуИНомерСФ(ЗаписьКниги, Секция = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗаписьКниги.СчетФактураДокумент) Тогда
		...
		Иначе
			ДатаНомер = "" + Формат(ЗаписьКниги.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy") + ", №" + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗаписьКниги.НомерСчетаФактуры, Истина, Ложь);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		ДатаНомер = "" + Формат(ЗаписьКниги.СчетФактура.Дата, "ДФ=dd.MM.yyyy") + ", №" + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактура.Номер, Истина, Ложь);
	...
	Иначе
		...
		Если Документ <> Неопределено Тогда
			ДатаНомер = "" + Формат(Документ.Дата, "ДФ=dd.MM.yyyy") + ", №" + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Ложь);
...
Процедура СформироватьДополнительныеЛистыПродажи(СписокОрганизаций, ПараметрыОтчета, СтандартнаяФорма, СписокСообщений)
	...
	СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(ПараметрыОтчета.Организация, ТекущаяДата(), СписокПоказателей);
...
Функция ОпределитьДатуИНомерСФПродажи(ЗаписьКниги, Секция = Неопределено, ПараметрыОтчета)
	...
	Если ЗначениеЗаполнено(ЗаписьКниги.СчетФактураДокумент) Тогда
		...
		Иначе
			...
			ДатаНомер = "" + Формат(ЗаписьКниги.ДатаСчетаФактуры, "ДФ=dd.MM.yyyy") + ", №" + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ПараметрыОбъекта.Номер, Истина, Ложь);
		КонецЕсли; 
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
	...
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		ДатаНомер = "" + Формат(ЗаписьКниги.СчетФактура.Дата, "ДФ=dd.MM.yyyy") + ", №" 
			+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактура.Номер, Истина, Ложь);
	ИначеЕсли ТипЗнч(ЗаписьКниги.СчетФактура) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
		ДатаНомер = "" + Формат(ЗаписьКниги.СчетФактура.Дата, "ДФ=dd.MM.yyyy") + ", №" 
			+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ЗаписьКниги.СчетФактура.Номер, Истина, Ложь);
	Иначе
		...
		Если Документ <> Неопределено Тогда
			ДатаНомер = "" + Формат(Документ.Дата, "ДФ=dd.MM.yyyy") + ", №" 
				+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Ложь);
...
Функция ВыводСтроки(ПараметрыОтчета, Документ, Секция)
	...
	Пока ДокументыОснования.Следующий() Цикл
		...
		Если Не ПредставлениеТипа = Неопределено Тогда
			Если ДокументыОснования.ДокументОснование <> Неопределено Тогда
				Номер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДокументыОснования.ДокументОснование.Номер, Истина, Ложь);
			Иначе
				Номер = "";
			КонецЕсли;
	...
	Если Документ.ОпределитьПараметрыСчетаФактуры Тогда
		...
		ПараметрыСФ = УчетНДСПереопределяемый.ПолучитьПараметрыСчетаФактуры(Документ.СчетФактура);
	...
	Секция.Параметры.ДатаНомер = Формат(Документ.Дата, "ДФ=dd.MM.yyyy") + ", № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер, Истина, Ложь);
...
Процедура ВыводСтрокиПолученный(ПараметрыОтчета, СчетФактура, Секция)
	...
	Если СчетФактура.ОпределитьПараметрыСчетаФактуры Тогда
		ПараметрыСФ = УчетНДСПереопределяемый.ПолучитьПараметрыСчетаФактуры(СчетФактура.СчетФактура);
		Секция.Параметры.Сумма = ?(ПараметрыСФ.СуммаДокумента = 0, 
			"", Формат(ПараметрыСФ.СуммаДокумента, "ЧЦ=19; ЧДЦ=2") + " " + Строка(ПараметрыСФ.ВалютаДокумента));
		ПредставлениеОснования = "";
		...
		Пока Документ.Следующий() Цикл
			...
			Если Не ПредставлениеТипа = Неопределено Тогда
				ПредставлениеОснования = ПредставлениеОснования + ПредставлениеТипа 
					+ " № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.ДокументОснование.Номер, Истина, Ложь)
	...
	Иначе
		...
		Пока Документ.Следующий() Цикл
			...
			Если Не ПредставлениеТипа = Неопределено Тогда
				ПредставлениеОснования = ПредставлениеОснования + ПредставлениеТипа
					+ " № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.ДокументОснованиеНомер, Истина, Ложь)
					+ " от " + Формат(Документ.ДокументОснованиеДата, "ДФ=dd.MM.yyyy") + " г.";
			Иначе	
				ПредставлениеОснования = ПредставлениеОснования + Строка(Документ.ДокументОснование);
			КонецЕсли; 
...
Функция ВыводСтрокиКорректировочногоСФ(ПараметрыОтчета, Документ, Секция)
	...
	НомерСФ = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.НомерДокументаОснования, Истина, Ложь);
