...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.ВычетыПоДоходамНДФЛ) И
		...
		Обработчик.ОбщиеДанные = 
		ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.ВычетыПоДоходамНДФЛ) ИЛИ
		ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.РазмерВычетовНДФЛ) ИЛИ
		ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.ТарифыСтраховыхВзносов);
	КонецЕсли;
	...
	Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ВидыТарифовСтраховыхВзносов);
	КонецЕсли;
	...
	Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.СтатусыНалогоплательщиковПоНДФЛ);
	КонецЕсли;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.МинимальнаяОплатаТрудаРФ) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.0.18.7";
		Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ИндексироватьМРОТ2013";
		Обработчик.ОбщиеДанные = Истина;
	КонецЕсли;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.МинимальнаяОплатаТрудаРФ) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.0.18.7";
		Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ИндексироватьМРОТ2014";
		Обработчик.ОбщиеДанные = Истина;
	КонецЕсли;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.0.18.18";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОчиститьПодсказкиКонтактнойИнформации";
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.МинимальнаяОплатаТрудаРФ) Тогда
		...
		Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ИндексироватьМРОТ2015";
		Обработчик.ОбщиеДанные = Истина;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КонфигурацииЗарплатаКадры") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("КонфигурацииЗарплатаКадры");
...
Процедура СоздатьВалютуУчетаЗаработнойПлаты() Экспорт
	...
	Если СтрокаТаблицы = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдена валюта с кодом'" + " "+ Код);
	КонецЕсли;
...
Процедура ИндексироватьМРОТ2014() Экспорт
...
Процедура ИндексироватьМРОТ2015() Экспорт
...
Процедура ОчиститьПодсказкиКонтактнойИнформации() Экспорт
	
	МассивВидов = Новый Массив;
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ФаксОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.EmailОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ПочтовыйАдресОрганизации);
	МассивВидов.Добавить(Справочники.ВидыКонтактнойИнформации.ДругаяИнформацияОрганизации);
	
	Подсказки = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивВидов, "Подсказка");
	Для каждого ВидКИ Из Подсказки Цикл
		
		Если НЕ ПустаяСтрока(ВидКИ.Значение) Тогда
			
			ВидКИОбъект = ВидКИ.Ключ.ПолучитьОбъект();
			ВидКИОбъект.Подсказка = "";
