...
#Область СлужебныйПрограммныйИнтерфейсКонтактнаяИнформация
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат) Экспорт
	...
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'БСП %1'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки()
	);
...
#Область ТекстЗапроса
		...
		|	И АдресныеОбъекты.Уровень В(&Уровни) И ( АдресныеОбъекты.Уровень = 7 ИЛИ АдресныеОбъекты.Уровень = 6)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И (СубъектРФ.Уровень ЕСТЬ NULL 
		|			ИЛИ СубъектРФ.Уровень В (&Уровни))
		|	И (Округ.Уровень ЕСТЬ NULL 
		|			ИЛИ Округ.Уровень В (&Уровни))
		|	И (Район.Уровень ЕСТЬ NULL 
		|			ИЛИ Район.Уровень В (&Уровни))
		|	И (Город.Уровень ЕСТЬ NULL 
		|			ИЛИ Город.Уровень В (&Уровни))
		|	И (ВнутригородскойРайон.Уровень ЕСТЬ NULL 
		|			ИЛИ ВнутригородскойРайон.Уровень В (&Уровни))
		|	И (НаселенныйПункт.Уровень ЕСТЬ NULL 
		|			ИЛИ НаселенныйПункт.Уровень В (&Уровни))
		|	И (Улица.Уровень ЕСТЬ NULL 
		|			ИЛИ Улица.Уровень В (&Уровни))
		|	И (Дополнительный.Уровень ЕСТЬ NULL 
		|			ИЛИ Дополнительный.Уровень В (&Уровни))
		|	И (Подчиненный.Уровень ЕСТЬ NULL 
		|			ИЛИ Подчиненный.Уровень В (&Уровни))
		|
		|УПОРЯДОЧИТЬ ПО
		|	Наименование
		|ИТОГИ
...
Функция ВариантыАвтоподбора(Текст, Родитель, Уровни, ДополнительныеПараметры) Экспорт 
	...
	Если ПустаяСтрока(Источник) Тогда
		// Локальное, всегда корректное.
		Если УровниЗапроса.Найти(7) = Неопределено Тогда
			СписокАвтоподбораЧастиНаселеногоПункта(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры);
		Иначе
			СписокАвтоподбораЧастиУлицы(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры);
		КонецЕсли;
...
// Заполнение данных для автоподбора из загруженных данных для населеного пункта
//
Процедура СписокАвтоподбораЧастиНаселеногоПункта(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры) Экспорт
	...
	ТекстЗапросаПодходящиеОбъекты = "ВЫБРАТЬ ПЕРВЫЕ 200
	...
	Данные = Результат.Данные;
	СведенияОЗагруженныхРегионах = АдресныйКлассификаторПовтИсп.СведенияОЗагруженныхРегионах();
	Для Каждого Строка Из Запрос.Выполнить().Выгрузить() Цикл
		СтрокаРезультата = Данные.Добавить();
		
		ЗаполнитьЗначенияСвойств(СтрокаРезультата, Строка, "Идентификатор");
		...
		ДобавитьВПредставлениеЭлементАдреса(Представление, Строка.СубъектРФНаименование, Строка.СубъектРФСокращение);
		СтрокаРезультата.Представление = Представление;
		СтрокаРезультата.РегионЗагружен = СведенияОЗагруженныхРегионах[Строка.КодСубъектаРФ];
		
	КонецЦикла;
	
КонецПроцедуры

// Заполнение данных для автоподбора из загруженных данных для улиц
//
Процедура СписокАвтоподбораЧастиУлицы(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры) Экспорт
	...
	Запрос.Текст = 
		...
		|	АдресныеОбъекты.Идентификатор КАК Идентификатор,
		|	АдресныеОбъекты.Сокращение
		|ПОМЕСТИТЬ НайденныеАдресныеОбъекты
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК РодительскийОбъект
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъекты
		|		ПО РодительскийОбъект.КодСубъектаРФ = АдресныеОбъекты.КодСубъектаРФ
		|			И РодительскийОбъект.КодОкруга = АдресныеОбъекты.КодОкруга
		|			И РодительскийОбъект.КодРайона = АдресныеОбъекты.КодРайона
		|			И РодительскийОбъект.КодГорода = АдресныеОбъекты.КодГорода
		|			И РодительскийОбъект.КодВнутригородскогоРайона = АдресныеОбъекты.КодВнутригородскогоРайона
		|			И РодительскийОбъект.КодНаселенногоПункта = АдресныеОбъекты.КодНаселенногоПункта
		|ГДЕ
		|	РодительскийОбъект.Идентификатор = &Идентификатор
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 20
		|	НайденныеАдресныеОбъекты.Наименование + "" "" + НайденныеАдресныеОбъекты.Сокращение КАК Представление,
		|	НайденныеАдресныеОбъекты.Идентификатор,
		|	ИСТИНА КАК РегионЗагружен
		|ИЗ
		|	НайденныеАдресныеОбъекты КАК НайденныеАдресныеОбъекты
		|ГДЕ
		|	(НайденныеАдресныеОбъекты.Наименование ПОДОБНО &НачалоФразы
		|			ИЛИ НайденныеАдресныеОбъекты.Наименование ПОДОБНО &НачалоСлова)";
	
	Запрос.УстановитьПараметр("Идентификатор", Новый УникальныйИдентификатор(Родитель));
	Запрос.УстановитьПараметр("НачалоФразы", ТекстПоиска + "%");
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Родитель) Тогда
		...
		ТекстЗапросаПодходящиеОбъекты = 
#Область ТекстЗапроса
			"// Родитель указан явно
			|ВЫБРАТЬ ПЕРВЫЕ " + ОтбиратьЗаписейДляАнализа + "
			|	АдресныйОбъект.Уровень,
			|	АдресныйОбъект.КодСубъектаРФ,
			|	АдресныйОбъект.КодОкруга,
			|	АдресныйОбъект.КодРайона,
...
#КонецОбласти
	...
	Для Каждого Строка Из Запрос.Выполнить().Выгрузить() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаРезультата, Строка, "Идентификатор, РегионЗагружен");
...
// Возвращает вспомогательные данные - коды ОКАТО, ОКТМО и т.п.
//
// Параметры:
//     Идентификатор - УникальныйИдентификатор - Идентификатор адресного объекта, ориентира или вспомогательных данных.
// 
// Возвращаемое значение:
//     Структура -  найденные варианты. Содержит поля:
//       * Отказ                        - Булево - Поставщик не доступен.
...
//       * КраткоеПредставлениеОшибки   - Строка - Описание ошибки, если поставщик недоступен. Неопределено, если Отказ
//                                                 = Ложь.
//       * Данные                       - Структура - Поля данных:
//           ** OKATO            - Число, Неопределено, если результат не найден.
//           ** ОКТМО            - Число, Неопределено, если результат не найден.
//           ** КодИФНСФЛ        - Число, Неопределено, если результат не найден.
//           ** КодИФНСЮЛ        - Число, Неопределено, если результат не найден.
//           ** КодУчасткаИФНСФЛ - Число, Неопределено, если результат не найден.
//           ** КодУчасткаИФНСЮЛ - Число, Неопределено, если результат не найден.
//
Функция ДополнительныеАдресныеСведения(Идентификатор) Экспорт
	
	Результат = Новый Структура("Данные", СтруктураДополнительныхАдресныхСведений() );
	СтруктураОписанияОшибкиПоставщика(Результат);
	
	Источник = ИсточникДанныхАдресногоКлассификатора();
	Если ПустаяСтрока(Источник) Тогда
		// Локальное, всегда корректное.
		ЗаполнитьДополнительныеАдресныеСведенияВнутр(Результат, Идентификатор);
		
	ИначеЕсли Источник = "Сервис1С" Тогда
		// Веб сервис 1С, может быть на обслуживании.
		Попытка
			ЗаполнитьДополнительныеАдресныеСведенияСервис1С(Результат, Идентификатор);
		Исключение
			СтруктураОписанияОшибкиПоставщика(Результат, ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации( СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , , Результат.ПодробноеПредставлениеОшибки);
		КонецПопытки;
...
// Заполнение дополнительных данных по данным сервиса 1С.
//
Процедура ЗаполнитьДополнительныеАдресныеСведенияСервис1С(Результат, Идентификатор)
	
	Сервис = АдресныйКлассификаторПовтИсп.СервисКлассификатора1С();
	
	КодЯзыка = ТекущийКодЛокализации();
	ДанныеСервиса = Сервис.GetExtraInfo(Строка(Идентификатор), КодЯзыка, Метаданные.Имя);
	
	Данные = Результат.Данные;
	Данные.OKATO            = ДанныеСервиса.OKATO;
	Данные.ОКТМО            = ДанныеСервиса.OKTMO;
	Данные.КодИФНСФЛ        = ДанныеСервиса.IFNSFL;
	Данные.КодИФНСЮЛ        = ДанныеСервиса.IFNSUL;
	Данные.КодУчасткаИФНСФЛ = ДанныеСервиса.TERRIFNSFL;
	Данные.КодУчасткаИФНСЮЛ = ДанныеСервиса.TERRIFNSUL;
...
//       * КраткоеПредставлениеОшибки   - Строка - Описание ошибки, если поставщик недоступен. Неопределено, если Отказ
//                                                 = Ложь.
//       * Данные                       - ОбъектXDTO - Адрес, см (http://www.v8.1c.ru/ssl/contactinfo) АдресРФ.
//
...
//
Процедура УстановитьИдентификаторыЧастейАдреса(ЧастиАдреса) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Регион.Идентификатор КАК ИдентификаторРегион,
		|	Округ.Идентификатор КАК ИдентификаторОкруг,
		|	Район.Идентификатор КАК ИдентификаторРайон,
		|	Город.Идентификатор КАК ИдентификаторГород,
		|	ВнутригРайон.Идентификатор КАК ИдентификаторВнутригРайон,
		|	НаселенныйПункт.Идентификатор КАК ИдентификаторНаселенныйПункт,
		|	Улица.Идентификатор КАК ИдентификаторУлица,
		|	ДополнительныйЭлемент.Идентификатор КАК ИдентификаторДополнительныйЭлемент,
		|	ПодчиненныйЭлемент.Идентификатор КАК ИдентификаторПодчиненныйЭлемент
		|ИЗ
		|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект
		...
		|			И (Город.КодДополнительногоЭлемента = 0)
		|			И (Город.КодПодчиненногоЭлемента = 0)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК ВнутригРайон
		|		ПО (ВнутригРайон.Уровень = 5)
		|			И (ВнутригРайон.КодСубъектаРФ = АдресныйОбъект.КодСубъектаРФ)
		|			И (ВнутригРайон.КодОкруга = АдресныйОбъект.КодОкруга)
		|			И (ВнутригРайон.КодРайона = АдресныйОбъект.КодРайона)
		|			И (ВнутригРайон.КодГорода = АдресныйОбъект.КодГорода)
		|			И (ВнутригРайон.КодВнутригородскогоРайона = АдресныйОбъект.КодВнутригородскогоРайона)
		|			И (ВнутригРайон.КодНаселенногоПункта = 0)
		|			И (ВнутригРайон.КодУлицы = 0)
		|			И (ВнутригРайон.КодДополнительногоЭлемента = 0)
		|			И (ВнутригРайон.КодПодчиненногоЭлемента = 0)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК НаселенныйПункт
		|		ПО (НаселенныйПункт.Уровень = 6)
		|			И (НаселенныйПункт.КодСубъектаРФ = АдресныйОбъект.КодСубъектаРФ)
		|			И (НаселенныйПункт.КодОкруга = АдресныйОбъект.КодОкруга)
		...
		|			И (Улица.КодДополнительногоЭлемента = 0)
		|			И (Улица.КодПодчиненногоЭлемента = 0)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК ДополнительныйЭлемент
		|		ПО (ДополнительныйЭлемент.Уровень = 90)
		|			И (ДополнительныйЭлемент.КодСубъектаРФ = АдресныйОбъект.КодСубъектаРФ)
		|			И (ДополнительныйЭлемент.КодОкруга = АдресныйОбъект.КодОкруга)
		|			И (ДополнительныйЭлемент.КодРайона = АдресныйОбъект.КодРайона)
		|			И (ДополнительныйЭлемент.КодГорода = АдресныйОбъект.КодГорода)
		|			И (ДополнительныйЭлемент.КодВнутригородскогоРайона = АдресныйОбъект.КодВнутригородскогоРайона)
		|			И (ДополнительныйЭлемент.КодНаселенногоПункта = АдресныйОбъект.КодНаселенногоПункта)
		|			И (ДополнительныйЭлемент.КодУлицы = АдресныйОбъект.КодУлицы)
		|			И (ДополнительныйЭлемент.КодДополнительногоЭлемента = АдресныйОбъект.КодДополнительногоЭлемента)
		|			И (ДополнительныйЭлемент.КодПодчиненногоЭлемента = 0)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК ПодчиненныйЭлемент
		|		ПО (ПодчиненныйЭлемент.Уровень = 91)
		|			И (ПодчиненныйЭлемент.КодСубъектаРФ = АдресныйОбъект.КодСубъектаРФ)
		|			И (ПодчиненныйЭлемент.КодОкруга = АдресныйОбъект.КодОкруга)
		|			И (ПодчиненныйЭлемент.КодРайона = АдресныйОбъект.КодРайона)
		|			И (ПодчиненныйЭлемент.КодГорода = АдресныйОбъект.КодГорода)
		|			И (ПодчиненныйЭлемент.КодВнутригородскогоРайона = АдресныйОбъект.КодВнутригородскогоРайона)
		|			И (ПодчиненныйЭлемент.КодНаселенногоПункта = АдресныйОбъект.КодНаселенногоПункта)
		|			И (ПодчиненныйЭлемент.КодУлицы = АдресныйОбъект.КодУлицы)
		|			И (ПодчиненныйЭлемент.КодДополнительногоЭлемента = АдресныйОбъект.КодДополнительногоЭлемента)
		|			И (ПодчиненныйЭлемент.КодПодчиненногоЭлемента = АдресныйОбъект.КодПодчиненногоЭлемента) ";
		
		Где = "";
		Разделитель = " ГДЕ ";
		Для каждого Часть Из ЧастиАдреса Цикл
			Значение = Часть.Значение;
			Если ЗначениеЗаполнено(Значение.Представление) Тогда
				Где = Где + Разделитель + Часть.Ключ +".Наименование = """ + Значение.Наименование + """ И " + Часть.Ключ + ".Сокращение = """ + Значение.Сокращение + """";
				Разделитель = " И ";
			КонецЕсли;
		КонецЦикла;
		...
		Если РезультатЗапроса.Следующий() Тогда
			Для каждого Часть Из ЧастиАдреса Цикл
				Если ЗначениеЗаполнено(Часть.Значение.Представление) Тогда
					Часть.Значение.Идентификатор = РезультатЗапроса["Идентификатор" + Часть.Ключ];
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура УстановитьИдентификаторыНаселенногоПункта(ЧастиАдреса, ИдентификаторНаселенногоПункта) Экспорт
	...
	Запрос.Текст = 
		...
		Для каждого Часть Из ЧастиАдреса Цикл
			Значение = Часть.Значение;
			Если ЗначениеЗаполнено(Значение.Представление) И Часть.Значение.Уровень < 7 Тогда
				Где = Где + Разделитель + Часть.Ключ +".Наименование = """ + Значение.Наименование + """ И " + Часть.Ключ + ".Сокращение = """ + Значение.Сокращение + """";
				Разделитель = " И ";
			КонецЕсли;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(ИдентификаторНаселенногоПункта) Тогда
			Запрос.Текст = Запрос.Текст + Разделитель + " АдресныйОбъект.Идентификатор = &Идентификатор";
			Запрос.УстановитьПараметр("Идентификатор", ИдентификаторНаселенногоПункта);
		КонецЕсли;
		
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		НижнийУровеньСИдентификатором = 0;
		Если РезультатЗапроса.Следующий() Тогда
			Для каждого Часть Из ЧастиАдреса Цикл
				Если ЗначениеЗаполнено(Часть.Значение.Представление)  И Часть.Значение.Уровень < 7 Тогда
					Часть.Значение.Идентификатор = РезультатЗапроса["Идентификатор" + Часть.Ключ];
					Если ЗначениеЗаполнено(Часть.Значение.Идентификатор) И НижнийУровеньСИдентификатором < Часть.Значение.Уровень Тогда
						ИдентификаторНаселенногоПункта = Часть.Значение.Идентификатор;
						НижнийУровеньСИдентификатором = Часть.Значение.Уровень;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Для Каждого ПроверяемыйАдрес Из Адреса Цикл
		...
		Если ТипЗнч(АдресXDTO) = ТипСтрока Тогда
			...
			Иначе 
				...
				Результат.КраткоеПредставлениеОшибки = НСтр("ru = 'Адрес не может быть проверен, так как не распознан.'");
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Регион ""%1"" не существует'"), 
				Адрес.СубъектРФ
			));
...
#КонецОбласти
	...
	ПроверятьОкруг                = Не ПустаяСтрока(Округ.Наименование)                И Уровни.Найти(2) <> Неопределено;
	ПроверятьРайон                = Не ПустаяСтрока(Район.Наименование)                И Уровни.Найти(3) <> Неопределено;
	ПроверятьГород                = Не ПустаяСтрока(Город.Наименование)                И Уровни.Найти(4) <> Неопределено;
	ПроверятьВнутригородскойРайон = Не ПустаяСтрока(ВнутригородскойРайон.Наименование) И Уровни.Найти(5) <> Неопределено;
	ПроверятьНаселенныйПункт      = Не ПустаяСтрока(НаселенныйПункт.Наименование)      И Уровни.Найти(6) <> Неопределено;
	ПроверятьУлицу                = Не ПустаяСтрока(Улица.Наименование)                И Уровни.Найти(7) <> Неопределено;
	ПроверятьДополнительный       = Не ПустаяСтрока(Дополнительный.Наименование)       И Уровни.Найти(90) <> Неопределено;
	ПроверятьПодчиненный          = Не ПустаяСтрока(Подчиненный.Наименование)          И Уровни.Найти(91) <> Неопределено;
...
#КонецОбласти
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Субъект РФ (регион) ""%1"" не найден в адресном классификаторе'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Округ ""%1"" отсутствует в адресном классификаторе'"), 
				Округ.Наименование + " " + Округ.Сокращение
			));
		КонецЕсли;
		
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'"), 
				Район.Наименование + " " + Район.Сокращение
			));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Город ""%1"" отсутствует в адресном классификаторе'"), 
				Город.Наименование + " " + Город.Сокращение
			));
		КонецЕсли;
		
			
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Внутригородской район ""%1"" отсутствует в адресном классификаторе'"), 
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Населенный пункт ""%1"" отсутствует в адресном классификаторе'"), 
				НаселенныйПункт.Наименование + " " + НаселенныйПункт.Сокращение
			));
		КонецЕсли;
		
		Если ПроверятьУлицу И Не Запись.УлицаНайден Тогда
			...
			Если ИсторияОбъекта.Количество() > 0 Тогда 
				Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
					ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Улица ""%1"" была переименована в ""%2""'"), 
					СтрокаОбъект.Значение, СтрокаОбъект.Представление
					));
				КонецЦикла;
			Иначе
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Улица ""%1"" отсутствует в адресном классификаторе'"), 
					Улица.Наименование + " " + Улица.Сокращение
				));
			КонецЕсли;
		КонецЕсли;
			
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Дополнительный элемент ""%1""отсутствует в адресном классификаторе'"), 
				Дополнительный.Наименование + " " + Дополнительный.Сокращение
			));
		КонецЕсли;
			
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подчиненный элемент ""%1"" отсутствует в адресном классификаторе'"), 
			...
			Если ЗначениеЗаполнено(Запись.Строения) Тогда
				...
				Если Не ПустаяСтрока(Описание) Тогда
					...
					Если Не ПустаяСтрока(Описание) Тогда
						...
						Если ОписаниеЗданийФИАССодержитДанные(Описание, ЗданияИПомещения) Тогда
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '""%1"" отсутствует в адресном классификаторе'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)
			));
		КонецЕсли;
	...
	Если НЕ ИндексКорректный И ЗначениеЗаполнено(Индекс) Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу'"), Индекс));
	КонецЕсли;
...
Функция ИдентификаторАдресногоОбъектаПоЧастямАдреса(Адрес, ИскатьПоУлице = Ложь) Экспорт
	...
	КодСубъектаРФ = 0;
	ИдентификаторСубъектаРФ = Неопределено;
	КодОкруга = 0;
	КодРайона = 0;
	КодГорода = 0;
	...
	Если ЗначениеЗаполнено(Адрес.СубъектРФ)  Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	АдресныйОбъект.Идентификатор КАК ИдентификаторАдресныйОбъект,
			|	АдресныйОбъект.КодСубъектаРФ
			|ИЗ
			|	РегистрСведений.АдресныеОбъекты КАК АдресныйОбъект
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 1
			|	И АдресныйОбъект.КодОкруга = 0
			|	И АдресныйОбъект.КодРайона = 0
			|	И АдресныйОбъект.КодГорода = 0
			|	И АдресныйОбъект.КодВнутригородскогоРайона = 0
			|	И АдресныйОбъект.КодНаселенногоПункта = 0
			...
			|	И АдресныйОбъект.КодДополнительногоЭлемента = 0
			|	И АдресныйОбъект.КодПодчиненногоЭлемента = 0
			|	И АдресныйОбъект.Сокращение = &Сокращение";
		
		Запрос.УстановитьПараметр("Наименование", СубъектРФ.Наименование);
		Запрос.УстановитьПараметр("Сокращение", СубъектРФ.Сокращение);
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодСубъектаРФ = РезультатЗапроса.КодСубъектаРФ;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Адрес.Округ) Тогда
		Округ = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(Адрес.Округ);
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 2
			|	И АдресныйОбъект.КодСубъектаРФ = &КодСубъектаРФ
			|	И АдресныйОбъект.КодРайона = 0
			|	И АдресныйОбъект.КодГорода = 0
			|	И АдресныйОбъект.КодВнутригородскогоРайона = 0
			|	И АдресныйОбъект.КодНаселенногоПункта = 0
		...
		Запрос.УстановитьПараметр("Наименование", Округ.Наименование);
		Запрос.УстановитьПараметр("Сокращение", Округ.Сокращение);
		
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодОкруга = РезультатЗапроса.КодОкруга;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;

	Если Адрес.СвРайМО <> Неопределено И ЗначениеЗаполнено(Адрес.СвРайМО.Район) Тогда
		
		Район = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(Адрес.СвРайМО.Район);
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 3
			|	И АдресныйОбъект.КодСубъектаРФ = &КодСубъектаРФ
			|	И АдресныйОбъект.КодОкруга = &КодОкруга
			|	И АдресныйОбъект.КодГорода = 0
			|	И АдресныйОбъект.КодВнутригородскогоРайона = 0
			|	И АдресныйОбъект.КодНаселенногоПункта = 0
			|	И АдресныйОбъект.КодУлицы = 0
		...
		Запрос.УстановитьПараметр("КодСубъектаРФ", КодСубъектаРФ);
		Запрос.УстановитьПараметр("КодОкруга", КодОкруга);
		Запрос.УстановитьПараметр("Наименование", Район.Наименование);
		Запрос.УстановитьПараметр("Сокращение", Район.Сокращение);
		
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодРайона = РезультатЗапроса.КодРайона;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Адрес.Город) Тогда
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 4
			|	И АдресныйОбъект.КодСубъектаРФ = &КодСубъектаРФ
			|	И АдресныйОбъект.КодОкруга = &КодОкруга
			|	И АдресныйОбъект.КодРайона = &КодРайона
			|	И АдресныйОбъект.КодВнутригородскогоРайона = 0
			|	И АдресныйОбъект.КодНаселенногоПункта = 0
			|	И АдресныйОбъект.КодУлицы = 0
			|	И АдресныйОбъект.КодДополнительногоЭлемента = 0
		...
		Запрос.УстановитьПараметр("КодСубъектаРФ", КодСубъектаРФ);
		Запрос.УстановитьПараметр("Наименование", Город.Наименование);
		Запрос.УстановитьПараметр("Сокращение", Город.Сокращение);
		
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодГорода = РезультатЗапроса.КодГорода;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Адрес.ВнутригРайон) Тогда
		ВнутригРайон = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(Адрес.ВнутригРайон);
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 5
			|	И АдресныйОбъект.КодСубъектаРФ = &КодСубъектаРФ
			|	И АдресныйОбъект.КодОкруга = &КодОкруга
			|	И АдресныйОбъект.КодРайона = &КодРайона
			|	И АдресныйОбъект.КодГорода = &КодГорода
			|	И АдресныйОбъект.КодНаселенногоПункта = 0
			|	И АдресныйОбъект.КодУлицы = 0
			|	И АдресныйОбъект.КодДополнительногоЭлемента = 0
			|	И АдресныйОбъект.КодПодчиненногоЭлемента = 0
		...
		Запрос.УстановитьПараметр("Наименование", ВнутригРайон.Наименование);
		Запрос.УстановитьПараметр("Сокращение", ВнутригРайон.Сокращение);
		
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодВнутригородскогоРайона = РезультатЗапроса.КодВнутригородскогоРайона;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;

	Если ЗначениеЗаполнено(Адрес.НаселПункт) Тогда
		НаселПункт = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(Адрес.НаселПункт);
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	АдресныйОбъект.Наименование = &Наименование
			|	И АдресныйОбъект.Уровень = 6
			|	И АдресныйОбъект.КодСубъектаРФ = &КодСубъектаРФ
			|	И АдресныйОбъект.КодОкруга = &КодОкруга
			|	И АдресныйОбъект.КодРайона = &КодРайона
			|	И АдресныйОбъект.КодГорода = &КодГорода
			|	И АдресныйОбъект.КодВнутригородскогоРайона = &КодВнутригородскогоРайона
			|	И АдресныйОбъект.КодУлицы = 0
			|	И АдресныйОбъект.КодДополнительногоЭлемента = 0
			|	И АдресныйОбъект.КодПодчиненногоЭлемента = 0
			|	И АдресныйОбъект.Сокращение = &Сокращение";
		...
		Запрос.УстановитьПараметр("КодРайона", КодРайона);
		Запрос.УстановитьПараметр("КодГорода", КодГорода);
		Запрос.УстановитьПараметр("КодВнутригородскогоРайона", КодВнутригородскогоРайона);
		Запрос.УстановитьПараметр("Наименование", НаселПункт.Наименование);
		Запрос.УстановитьПараметр("Сокращение", НаселПункт.Сокращение);
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодНаселенногоПункта = РезультатЗапроса.КодНаселенногоПункта;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;
	
	// Улица
	Если ЗначениеЗаполнено(Адрес.Улица) И ИскатьПоУлице Тогда
		Улица = АдресныйКлассификаторКлиентСервер.НаименованиеИСокращение(Адрес.Улица);
		...
		Запрос.Текст = 
			...
			|	И АдресныйОбъект.КодГорода = &КодГорода
			|	И АдресныйОбъект.КодВнутригородскогоРайона = &КодВнутригородскогоРайона
			|	И АдресныйОбъект.КодНаселенногоПункта = &КодНаселенногоПункта
			|	И АдресныйОбъект.КодДополнительногоЭлемента = 0
			|	И АдресныйОбъект.КодПодчиненногоЭлемента = 0
			|	И АдресныйОбъект.Сокращение = &Сокращение";
		...
		Запрос.УстановитьПараметр("Наименование", Улица.Наименование);
		Запрос.УстановитьПараметр("Сокращение", Улица.Сокращение);
		РезультатЗапроса = Запрос.Выполнить().Выбрать();
		
		Если РезультатЗапроса.Следующий() Тогда
			КодУлицы = РезультатЗапроса.КодУлицы;
			ИдентификаторНаселенногоПункта =  РезультатЗапроса.ИдентификаторАдресныйОбъект;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ИдентификаторНаселенногоПункта;
	
КонецФункции

// Определяет индекс адреса по частям адреса.
//
Функция ИндексАдресаПоЧастямАдреса(Адрес, Идентификатор = Неопределено) Экспорт
	...
	Если ПустаяСтрока(Источник) Тогда
		...
		Если Идентификатор <> Неопределено Тогда
			...
			Если РезультатЗапроса.Следующий() Тогда
				Если РезультатЗапроса.ПочтовыйИндекс = 0 Тогда
					...
					Для Каждого ДополнительныйЭлемент Из Адрес.ПолучитьСписок("ДопАдрЭл") Цикл
						...
						Если ТипАдресногоЭлемента = "" Тогда
							...
							Продолжить;
						ИначеЕсли ТипАдресногоЭлемента = ТипАдрЭлПочтовыйИндекс Тогда
							Индекс = ТипЧисло.ПривестиЗначение(ДополнительныйЭлемент.Значение);
							Продолжить;
						КонецЕсли;
					КонецЦикла;
					...
					КонецЦикла;
				Иначе
					Индекс = РезультатЗапроса.ПочтовыйИндекс;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Источник = "Сервис1С" Тогда
		...
		Если НЕ Результат.Отказ Тогда
			Если Результат.Данные[0].Варианты.Количество() > 0 Тогда
				Индекс = Результат.Данные[0].Варианты[0].Индекс;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеЗданийФИАССодержитДанные(Описание, ЗданияИПомещения)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "H" Тогда
			...
			Если  НомерВладения = НомерДомаИлиВладенияЗдания
				...
				И НомерСтроения = НомерСтроенияЗдания
			Тогда
				// Точное попадания
				Возврат Истина;
			КонецЕсли;
		...
		Если ВидИнтервала = 2 Тогда
			Если НомерЧетный Тогда
				...
				Если  (Интервал.Начало <= НомерЧислом Или Интервал.Начало = 0)
					И (Интервал.Конец >= НомерЧислом Или Интервал.Конец = 0)
				Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ТипЗнч(ИнформацияОбОшибке.Причина) = Тип("ИнформацияОбОшибке") Тогда
		Если ИнформацияОбОшибке.Причина.Причина <> Неопределено Тогда
			...
			ПозицияНачало = Найти(ОписаниеОшибкиДляПоиска , "<FAULTSTRING>");
			Если ПозицияНачало > 0 Тогда
				ПозицияОкончание = Найти(ОписаниеОшибкиДляПоиска , "</FAULTSTRING>");
...
Процедура ОбработатьОбщепринятыеСокращения(ДоступныеУровни)
	...
	Для каждого НераспознаннаяЧастьАдреса Из НераспознанныеСтроки Цикл
		Если (ВРЕГ(НераспознаннаяЧастьАдреса.Наименование) = "МОСКВА" ИЛИ
			 ВРЕГ(НераспознаннаяЧастьАдреса.Наименование) = "САНКТ-ПЕТЕРБУРГ")
...
Процедура УстановитьУровниАдресаПоЧастямАдреса(ЧастиАдреса, Уровни) Экспорт
	...
	Для каждого Элемент Из СписокПолей Цикл
		ТекстЗапросаВыборки = ТекстЗапросаВыборки + Разделитель + " " + Элемент.Ключ +".Наименование + "" "" + 
		| " + Элемент.Ключ + ".Сокращение КАК "+ Элемент.Ключ + ", " + Элемент.Ключ +".Уровень КАК " + Элемент.Ключ + "Уровень, " + 
		 Элемент.Ключ + "." + Элемент.Значение + " КАК " + Элемент.Значение;
		Разделитель = ", ";
	КонецЦикла;
...
//
Процедура УстановитьУровниАдресаПоЧастямАдресаДляНаселенногоПункта(Уровни, ЧастиАдреса) Экспорт
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		Возврат; // В веб-клиенте работа с адресным классификатором не поддерживается.
	КонецЕсли;
	
	// 2. Необходимость обновления - выводим всегда.
	ПоследнееОбновление = ОписаниеПоследнейЗагрузки();
	Подсказка           = ПоследнееОбновление.Представление;
	Если ПоследнееОбновление.НеобходимоОбновление Тогда
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		ДлительныеОперации.СообщитьПрогресс( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...'"), 
...
Функция СлужебныеАдресныеСведенияДляОбработкиКЛАДР()
	...
	Результат = 
		...
		// Строения
		СтрокаСлужебныхАдресныхСведений("STRSTAT", 1, Неопределено, "Строение",
		СтрокаСлужебныхАдресныхСведений("STRSTAT", 2, Неопределено, "Сооружение",
		СтрокаСлужебныхАдресныхСведений("STRSTAT", 3, Неопределено, "Литера",
		СтрокаСлужебныхАдресныхСведений("STRSTAT", 3, Неопределено, "Литер",
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка поиска типа диапазона для %1'"), Описание
		);
	КонецЕсли;
...
//
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено) Экспорт
...
//
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ) Экспорт
...
//
Функция АдресФайлаОписанияДоступныхВерсий() Экспорт
...
//
Функция ДоступныеВерсииАдресныхСведений() Экспорт
	...
	Для Каждого ЗаписьРегиона Из ПоследняяПубликация.ПолучитьСписок("Region") Цикл
		...
		СтрокаРегиона.ДоступноОбновление = ТекущийСубъект = Неопределено	// Новый регион
			...
			Или (                                                       	// Обновленные данные существующего субъекта.
				    ТекущийСубъект.КодСубъектаРФ <> ЗаписьРегиона.REGIONCODE
				Или ТекущийСубъект.Представление <> ЗаписьРегиона.FORMALNAME
				Или ТекущийСубъект.Сокращение    <> ЗаписьРегиона.SHORTNAME
				Или ТекущийСубъект.Индекс        <> ЗаписьРегиона.POSTALCODE
			);
	КонецЦикла;
...
//                                                         период актуальности.
//
Функция ОписаниеПоследнейЗагрузки() Экспорт
	...
	Иначе
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Адресный классификатор был загружен %1 назад.'"), ОбщегоНазначения.ИнтервалВремениСтрокой(КонецПериода, НачалоПериода)
		);
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В файле ""%1"" не найден список записей в типе  %2'"), Результат.ПолноеИмя, Результат.ТипXDTO
		);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В файле ""%1"" не найден корневой узел %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла
		);
	КонецЕсли;
...
Функция АдресИнтернетаВключаяПорт(Адрес)
	...
	ПозицияПараметра = Найти(ИмяФайла, "?");
