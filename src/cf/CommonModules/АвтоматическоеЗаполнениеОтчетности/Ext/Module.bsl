...
Процедура ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2015Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Иначе
		...
		СтруктураПолейПриложения2 = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ДанныеПриложения2);
		СтруктураПолейДопСтрокПриложения2 = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ДанныеДопСтрокПриложения2);
		...
		|			ТОГДА """"
		|		ИНАЧЕ ЕСТЬNULL(Доходы.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года, """")
		|	КОНЕЦ КАК КодВычета,
		...
		|	Вычеты.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года КАК П000020005101,
		...
		|СГРУППИРОВАТЬ ПО
		|	Вычеты.НомерСправки,
		|	Вычеты.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСправки,
		|	П000020005101";
		...
		Пока Справки.Следующий() Цикл
			...
			ДанныеПриложения2 = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПолейПриложения2);
			ДанныеДопСтрокПриложения2 = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(СтруктураПолейДопСтрокПриложения2);
			...
			ДанныеПриложения2.П000020000401 = СоответствиеКодовФНССтатусамНДФЛ.Получить(Справки.СтатусНалогоплательщика);
			
			СтруктураАдреса = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПустаяСтруктураАдреса);
