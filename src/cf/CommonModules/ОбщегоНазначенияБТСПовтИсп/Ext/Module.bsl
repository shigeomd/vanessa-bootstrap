...
//
// Вовзращаемое значение: ОписаниеТипов.
//
...
// Проверяет, что используется версия платформы 8.3.5.
//
// Возвращаемое значение: Булево.
//
Функция ИспользуетсяПлатформа8_3_5() Экспорт
	
	Информация = Новый СистемнаяИнформация();
	
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Информация.ВерсияПриложения, "8.3.5.1") >= 0;
	
КонецФункции

// Проверяет, доступны ли в текущей конфигурации механизмы платформы,
// доступные только при использовании платформы версии 8.3.5 с режимом совместимости
// "Не использовать".
//
// Возвращаемое значение: Истина, если доступны.
//
Функция ДоступныМеханизмыРежимаСовместимости8_3_5() Экспорт
	
	Возврат ДоступныМеханизмыРежимаСовместимостиПлатформы("8.3.5.1");
	
КонецФункции

// Проверяет, доступны ли в текущей конфигурации механизмы платформы,
// доступные только при использовании платформы версии 8.3.3 с режимом совместимости
// "Не использовать".
//
// Возвращаемое значение: Истина, если доступны.
//
Функция ДоступныМеханизмыРежимаСовместимости8_3_3() Экспорт
	
	Возврат ДоступныМеханизмыРежимаСовместимостиПлатформы("8.3.3.1");
	
КонецФункции

// Возвраащет ссылки точек маршрута бизнес-процессов.
//
// Возвращаемое значение: ФиксированноеСоответствие:
//                        * Ключ - Тип - тип ТочкаМаршрутаБизнесПроцессаСсылка,
//                        * Значенеи - Строка - имя бизнес-процесса.
//
...
Функция ДоступныМеханизмыРежимаСовместимостиПлатформы(Знач НомерВерсии)
	
	ВерсияРежимаСовместимости = ОбщегоНазначенияБТСПовтИсп.ВерсииРежимовСовместимостиПлатформы().Получить(Метаданные.РежимСовместимости);
	
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияРежимаСовместимости, НомерВерсии) >= 0;
	
КонецФункции

Функция ВерсииРежимовСовместимостиПлатформы() Экспорт
	
	Информация = Новый СистемнаяИнформация();
	ВерсияПлатформы = Информация.ВерсияПриложения;
	
	Результат = Новый Соответствие();
	
	Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_1"], "8.1.1.1");
	
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.2.13.1") >= 0 Тогда
		
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.2.16.1") >= 0 Тогда
			
			Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_2_13"], "8.2.13.1");
			
			Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.1.1") >= 0 Тогда
				
				Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_2_16"], "8.2.16.1");
				
				Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.2.1") >= 0 Тогда
					
					Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_3_1"], "8.3.1.1");
					
					Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.3.1") >= 0 Тогда
						
						Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_3_2"], "8.3.2.1");
						
						Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.4.1") >= 0 Тогда
							
							Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_3_3"], "8.3.3.1");
							
							Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.5.1") >= 0 Тогда
								
								Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_3_4"], "8.3.4.1");
								
								Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.6.1") >= 0 Тогда
									
									Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["Версия8_3_5"], "8.3.5.1");
									
								Иначе
									
									Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.3.5.1");
									
								КонецЕсли;
								
							Иначе
								
								Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.3.4.1");
								
							КонецЕсли;
							
						Иначе
							
							Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.3.3.1");
							
						КонецЕсли;
						
					Иначе
						
						Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.3.2.1");
						
					КонецЕсли;
					
				Иначе
					
					Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.3.1.1");
					
				КонецЕсли;
				
			Иначе
				
				Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.2.16.1");
				
			КонецЕсли;
			
		Иначе
			
			Результат.Вставить(Метаданные.СвойстваОбъектов.РежимСовместимости["НеИспользовать"], "8.2.13.1");
	...
	ИначеЕсли (Класс = КлассыМетаданных().Справочники
		...
		Если (Класс = КлассыМетаданных().ПланыСчетов ИЛИ Класс = КлассыМетаданных().ПланыВидовРасчета) Тогда
			...
			Для Каждого СтандартнаяТабличнаяЧасть Из ОбъектМетаданных.СтандартныеТабличныеЧасти Цикл
				Для Каждого СтандартныйРеквизи Из СтандартнаяТабличнаяЧасть.СтандартныеРеквизиты Цикл
					ЗаполнитьМодельПоТипамЗависимостейОбъектаМетаданных(ЗависимостиОбъекта, СтандартныйРеквизит.Тип);
				КонецЦикла;
			КонецЦикла;
