...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Если РеквизитыСХранилищемЗначений = Неопределено Тогда
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхХранилищЗначений.ПередВыгрузкойОбъекта()!'"),
...
Функция ВыгрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Если Значение = Неопределено ИЛИ
		(ОбщегоНазначенияБТС.ЭтоПримитивныйТип(ТипЗнч(Значение)) И Не ЗначениеЗаполнено(Значение)) Тогда
		Возврат Ложь;
	Иначе
...
Процедура ПередЗагрузкойКонстанты(Контейнер, Объект, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Тип владельца {%1}%2 не должен использоваться для объекта метаданных %3!'"),
...
Процедура ЗагрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение ТехнологияСервисаИнтеграцияСБСП.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неожиданный тип размещения данных хранилища значений в контейнере выгрузки: {%1}%2!'"),
