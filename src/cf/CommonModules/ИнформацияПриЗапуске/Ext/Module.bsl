...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	Событие = "СтандартныеПодсистемы.БазоваяФункциональность\ПриДобавленииПараметровРаботыКлиентскойЛогикиСтандартныхПодсистемПриЗапуске";
	СерверныеОбработчики[Событие].Добавить(СерверныйМодуль);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия      = "*";
	Обработчик.Процедура   = "ИнформацияПриЗапуске.ОбновитьКэшПервогоПоказа";
...
Процедура ОбновитьКэшПервогоПоказа() Экспорт
	
	Кэш = КэшДляПервогоПоказа();
	
	НаборЗаписей = РегистрыСведений.ПакетыИнформацииПриЗапуске.СоздатьНаборЗаписей();
	
	НомерПакета = 0;
	Для Каждого КлючИЗначение Из Кэш.ПодготовленныеПакеты Цикл
		НомерПакета = НомерПакета + 1;
		ИмяПакета = "ПодготовленныйПакет" + Формат(НомерПакета, "ЧН=; ЧГ=");
		
		Запись = НаборЗаписей.Добавить();
		Запись.Номер  = НомерПакета;
		Запись.Состав = Новый ХранилищеЗначения(КлючИЗначение.Значение);
		Кэш.ПодготовленныеПакеты.Вставить(КлючИЗначение.Ключ, НомерПакета);
	КонецЦикла;
...
// Глобальные настройки подсистемы.
Функция КэшДляПервогоПоказа() Экспорт
	Кэш = Новый Структура;
	
	// Чтение макета "Описатель" (наполнение таблицы "ПакетыСтраниц").
	ТабличныйДокумент = Обработки.ИнформацияПриЗапуске.ПолучитьМакет("Описатель");
	...
	ИменаМакетовСтраницСМинимальнымПриоритетом = Новый Массив;
	
	КонфигурацияБазовая       = СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации();
	КонфигурацияМоделиСервиса = ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ПолучитьФункциональнуюОпцию("РаботаВАвтономномРежиме");
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если МинимальныйПриоритет > ПакетСтраниц.Приоритет Тогда
			...
			ИменаМакетовСтраницСМинимальнымПриоритетом = Новый Массив;
			ИменаМакетовСтраницСМинимальнымПриоритетом.Добавить(ПакетСтраниц.ИмяМакета);
		ИначеЕсли МинимальныйПриоритет = ПакетСтраниц.Приоритет
			И ИменаМакетовСтраницСМинимальнымПриоритетом.Найти(ПакетСтраниц.ИмяМакета) = Неопределено Тогда
			ИменаМакетовСтраницСМинимальнымПриоритетом.Добавить(ПакетСтраниц.ИмяМакета);
		КонецЕсли;
	КонецЦикла;
	
	ПодготовленныеПакеты = Новый Соответствие;
	Для Каждого ИмяМакета Из ИменаМакетовСтраницСМинимальнымПриоритетом Цикл
		ПодготовленныеПакеты.Вставить(ИмяМакета, ИзвлечьФайлыПакета(ИмяМакета));
	КонецЦикла;
...
Функция ИзвлечьФайлыПакета(ИмяМакета) Экспорт
	...
	Попытка
		ДвоичныеДанные = Обработки.ИнформацияПриЗапуске.ПолучитьМакет(ИмяМакета);
