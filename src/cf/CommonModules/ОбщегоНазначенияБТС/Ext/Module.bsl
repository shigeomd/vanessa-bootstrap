...
Функция ЭтоНаборЗаписей(Знач ОбъектМетаданных) Экспорт
	...
	Возврат ЭтоРегистрСведений(ОбъектМетаданных)
		...
		Или ЭтоРегистрБухгалетрии(ОбъектМетаданных) 
...
Функция ЭтоНаборЗаписейПоддерживающийИтоги(Знач ОбъектМетаданных) Экспорт
	...
	ИначеЕсли ЭтоРегистрБухгалетрии(ОбъектМетаданных) Тогда
...
//                         * Интерактивное - Булево, флаг того, что право ограничивает возможнгость
//                                    выполнения интерактивных операций,
//                         * Чтение - Булево, флаг того, что право предоставлет или подразумевает
...
// Проверяет, содержит ли конфигурция БСП.
//
// Возвращаемое значение: Булево.
//
...
// Проверяет, поддерживаются ли в текущей конфигурации программнмые события БСП.
//
// Возвращаемое значение: Булево.
//
...
Функция СвойстваОбъектаМоделиКонфигурации(Знач Модель, Знач ОбъектМетаданных) Экспорт
	...
	Иначе
		...
		Имя = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолноеИмя, ".").Получить(1);
	КонецЕсли;
