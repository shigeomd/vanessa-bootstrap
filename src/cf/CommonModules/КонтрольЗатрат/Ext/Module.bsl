...
Процедура ВыявитьУчетныеОшибкиРасчетСебестоимости(Ошибки, Период, Организация) Экспорт
	...
	ПравилаРаспределенияРасходов.ПроверитьНастройку(Ошибки, Период, Организация, ЗаполненаАналитикаРасходов);
...
Процедура ДобавитьПереченьАналитикиВПараметрыСообщенияОбОшибке(ПараметрыСообщения, НеЗаполненаАналитика) Экспорт
	...
	Иначе
		...
		КонецЦикла;
		СтрокаРеквизиты = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(
			НаименованияНеЗаполненнойАналитики, 
			", ");
		СтрокаРеквизитыВинительныйПадеж = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(
			НаименованияНеЗаполненнойАналитикиВинительныйПадеж,
			", ");
...
Процедура ПроверитьНастроеноИспользованиеСчетовПроизводства(Ошибки, Период, Организация)
	...
	Ошибка.Рекомендация = НСтр("ru = 'Если организация не выпускает продукцию, не выполняет работы и не оказывает услуги, то счет 20 использовать не следует. 
		|Проверьте настройки на закладке ""Затраты"" учетной политики.'");
	Ошибка.Ссылка       = БухгалтерскийУчетКлиентСерверПереопределяемый.ПолучитьНавигационнуюСсылкуНаУчетнуюПолитикуОрганизации();
...
Функция ПроверитьЗаполненаАналитикаНаСчетахРасходов(Ошибки, Период, Организация)
	...
	Пока ВыборкаОшибок.Следующий() Цикл
		...
		Если ОдинНаборАналитики Тогда
			...
			КонецЦикла;
			ПараметрыТекста.Вставить("СписокСчетов", СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивСчетов,", "));
		...
		Иначе
			...
			КонецЦикла;
			ПараметрыТекста.Вставить("СписокПроблем", СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СписокПроблем, Символы.ПС));
...
Процедура ПроверитьКорректноОтраженыДоходыРасходыЕНВД(Ошибки, Период, Организация)
	...
	Пока ВыборкаВидыДеятельности.Следующий() Цикл
		...
		Пока ВыборкаНоменклатурныеГруппы.Следующий() Цикл
			...
			ПараметрыОписанияОшибки.Вставить("СчетДоходов",          СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СчетаДоходов, ", "));
...
Процедура ОписатьНевернаяОценкаНезавершенногоПроизводства(Ошибки, ДетальныеДанные, Период, Организация)
	...
	ЧтоНадоЗнать.Добавить(НСтр("ru = 'Для услуг, отраженных документами ""Реализация (акт, накладная)"" с учетом выручки остаток распределяется между наименованиями услуг, оказание которых отражено документами ""Реализация (акт, накладная)"" с такой же номенклатурной группой.'"));
	...
	Для Каждого СтрокаДанных Из ДетальныеДанные Цикл
		...
		// Вывод строки
		ДанныеДляВывода.Вставить("Продукция", СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СтрокаДанных.Продукция, ", "));
...
Функция ПрочитатьНастройкуРаспределенияРасходов(КлючЗаписи)
	
	Настройка = РегистрыСведений.МетодыРаспределенияКосвенныхРасходовОрганизаций.СоздатьМенеджерЗаписи();
	ЗаполнитьЗначенияСвойств(Настройка, КлючЗаписи);
	Настройка.Прочитать();
	Возврат Настройка;
...
Процедура ОписатьРасходыНеУдалосьРаспределить(Ошибки, ВсеДетальныеДанные, Период, Организация)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоискПодозрительныхЗатрат.ИндексСчет,
	|	ПоискПодозрительныхЗатрат.ИндексПравилоРаспределения,
	|	ПоискПодозрительныхЗатрат.ВидЗатрат КАК ВидЗатрат,
	|	ПоискПодозрительныхЗатрат.Подразделение КАК Подразделение
	...
	ШаблонДетальныхДанных = СкопироватьКолонки(РезультатЗапроса, "Счет, Подразделение, СтатьяЗатрат, КорСчет, Сумма");
	ШаблонДетальныхДанных.Колонки.Добавить("РасшифровкаСумма", Новый ОписаниеТипов("Структура"));
	...
	СчетаОбщехозяйственныхРасходов = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетаОбщехозяйственныхРасходов);
	...
	СчетаОбщепроизводственныхРасходов = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетаОбщепроизводственныхРасходов);
	
	ШаблонДетальныхДанных = СкопироватьКолонки(ВсеДетальныеДанные, "Счет, КорСчет, Подразделение, СтатьяЗатрат, Сумма");
	ШаблонДетальныхДанных.Колонки.Добавить("РасшифровкаСумма", Новый ОписаниеТипов("Структура"));
	...
	Для Каждого ГруппаОшибокСчет Из ВсеДетальныеДанные.Строки Цикл
		...
		Для Каждого ГруппаОшибокПравилоРаспределения Из ГруппаОшибокСчет.Строки Цикл
			...
			ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.Выручка Тогда
				...
				Настройка = ПрочитатьНастройкуРаспределенияРасходов(ГруппаОшибокПравилоРаспределения.Настройка); // Таких мало, поэтому допустимо читать данные в цикле
			...
			Иначе // База основана на стоимости затрат
				...
				ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.МатериальныеЗатраты Тогда
					...
					ПараметрыЧтоНадоЗнать.Вставить("КакОтражаютсяЗатратыОбразующиеБазу", НСтр("ru = '""Требование-накладная"", ""Отчет производства за смену"" (закладка ""Материалы""), ""Поступление (акт, накладная)"" (закладка ""Услуги"")'"));
				ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ПрямыеЗатраты Тогда
				...
				ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ОтдельныеСтатьиПрямыхЗатрат Тогда
					...
					СписокСтатейЗатратСтрокой = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СписокСтатейЗатрат, Символы.ПС);
			...
			Если МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.Выручка Тогда
				...
				ВыводСообщенийОбОшибках.ДобавитьСекцию(Секция, "Ссылка", БухгалтерскийУчетКлиентСерверПереопределяемый.ПолучитьНавигационнуюСсылкуНаМетодыРаспределенияКосвенныхРасходовОрганизации());
			КонецЕсли;
			...
			ВыводСообщенийОбОшибках.ДобавитьСекцию(Секция, "Ссылка", ПолучитьНавигационнуюСсылку(ГруппаОшибокПравилоРаспределения.Настройка));
			...
			Если ЭтоСчетОбщехозяйственныхРасходов Тогда
				...
				ВыводСообщенийОбОшибках.ДобавитьСекцию(Секция, "Ссылка", БухгалтерскийУчетКлиентСерверПереопределяемый.ПолучитьНавигационнуюСсылкуНаУчетнуюПолитикуОрганизации());
			КонецЕсли;
...
Функция ВидыДокументовОтраженияЗатрат(Организация)
	...
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ВидыДокументовОтраженияЗатрат, Символы.ПС);
