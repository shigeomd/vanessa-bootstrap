...
//
Функция ПолучитьСведенияОСпискеНоменклатуры(СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	Если Реализация И 
		...
		(ПрименяетсяОсобыйПорядокНалогообложения ИЛИ
		//Организация не плательщик НДС, но реализует комиссионные товары
		ОсуществляетсяРеализацияТоваровУслугКомитентов И (НЕ ПлательщикНДС ИЛИ ПрименяетсяОсвобождениеОтУплатыНДС)) Тогда
	...
	Если ЗначениеЗаполнено(ТипЦенПлановойСебестоимости) И ОпределятьЦены Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(СпособЗаполненияПлановойСебестоимости) И ОпределятьЦены Тогда 
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		Цена = 0;
		Если ОпределятьЦены И (ЗначениеЗаполнено(ТипЦен) ИЛИ ЗначениеЗаполнено(СпособЗаполненияЦены)) Тогда
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, Период = '20200101', Знач БанковскийСчет = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Организации.ЮридическоеФизическоеЛицо = &ФизическоеЛицо
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ТелефонРабочийФизическиеЛица)
...
Функция СчетаВыручкиЕНВД() Экспорт
	...
	Запрос.УстановитьПараметр("Счета90_01", 
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.Выручка));
