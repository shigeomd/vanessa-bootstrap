...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Редактирование", Метаданные.Справочники.ДополнительныеОтчетыИОбработки)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("ДополнительныеОтчетыИОбработки") Тогда
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.Ошибки = 
				СтрЗаменить(НСтр("ru = 'При подключении дополнительного отчета ""%1"" возникла ошибка:'"), "%1", Строка(Ссылка))
				+ Символы.ПС
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если (РегистрационныеДанные.Вид = ВидДополнительныйОтчет) ИЛИ (РегистрационныеДанные.Вид = ВидОтчет) Тогда
		...
		КонецЕсли;
		ПараметрыРегистрации.Свойство("ОпределитьНастройкиФормы", Объект.ТеснаяИнтеграцияСФормойОтчета);
	КонецЕсли;
