	...
	Если СтруктураПараметров.ПроверитьИНН Тогда
		...
		Если СтруктураВозврата.ОшибокПоИННнет Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.ИННВведенКорректно = Ложь;
				СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'Неизвестен вид контрагента. Укажите вид контрагента'"),,ЦветВыделенияНекорректногоЗначение);
			...
			Если СтруктураВозврата.ОшибокПоИННнет Тогда
				...
				Если  ЭтоЮрЛицо И ДлинаИНН <> 10 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'ИНН юридического лица должен состоять из 10 цифр'"),,ЦветВыделенияНекорректногоЗначение);
				...
				ИначеЕсли НЕ ЭтоЮрЛицо  И ДлинаИНН <> 12 Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'ИНН физического лица должен состоять из 12 цифр'"),,ЦветВыделенияНекорректногоЗначение);
				...
				ИначеЕсли Лев(ИНН, 2) = "00" Тогда
					...
					СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'Первые две цифры ИНН не могут быть ""00""'"),,ЦветВыделенияНекорректногоЗначение);
				...
				Если СтруктураВозврата.ОшибокПоИННнет Тогда
					...
					Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'ИНН должен включать только цифры'"),,ЦветВыделенияНекорректногоЗначение);
					...
					Если СтруктураВозврата.ОшибокПоИННнет Тогда
						...
						Если ЭтоЮрЛицо Тогда
							...
							Если КонтрольныйРазряд <> Число(Сред(ИНН, 10, 1)) или КонтрольнаяСумма = 0 Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'ИНН юридического лица введен некорректно'"),,ЦветВыделенияНекорректногоЗначение);
						...
						Иначе
							...
							Если КонтрольныйРазряд11 <> Число(Сред(ИНН,11,1)) ИЛИ КонтрольныйРазряд12 <> Число(Сред(ИНН,12,1)) или (КонтрольнаяСумма11 = 0 и КонтрольнаяСумма12 = 0) Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоИНН = Новый ФорматированнаяСтрока(НСтр("ru = 'ИНН физического лица введен некорректно'"),,ЦветВыделенияНекорректногоЗначение);
	...
	Если СтруктураПараметров.ПроверитьКПП Тогда
		...
		Если СтруктураПараметров.СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
			...
			Если ЭтоЮрЛицо = Неопределено Тогда
				...
				СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ru = 'Неизвестен вид контрагента. Укажите вид контрагента'"),,ЦветВыделенияНекорректногоЗначение);
			...
			Если СтруктураВозврата.ОшибокПоКППнет Тогда
				...
				Если СтруктураВозврата.ОшибокПоКППнет Тогда
					...
					Если ДлинаКПП <> 9 Тогда
						...
						СтруктураВозврата.НадписьПоясненияНекорректногоКПП  = Новый ФорматированнаяСтрока(НСтр("ru = 'КПП должен содержать 9 цифр'"),,ЦветВыделенияНекорректногоЗначение);
					...
					Если СтруктураВозврата.ОшибокПоКППнет Тогда
						...
						Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КПП) Тогда
							...
							СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ru = 'КПП должен включать только цифры'"),,ЦветВыделенияНекорректногоЗначение);
						...
						Если СтруктураВозврата.ОшибокПоКППнет Тогда
							...
							Если СтруктураПараметров.ОбособленноеПодразделение и ПризнакГоловногоПодразделения Тогда 
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ru = 'Введен КПП юридического лица. Измените вид контрагента'"),,ЦветВыделенияНекорректногоЗначение);
							...
							ИначеЕсли НЕ СтруктураПараметров.ОбособленноеПодразделение И ПризнакОбособленногоПодразделения Тогда  
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ru = 'Введен КПП обособленного подразделения. Измените вид контрагента'"),,ЦветВыделенияНекорректногоЗначение);
							...
							ИначеЕсли НЕ ПризнакГоловногоПодразделения и НЕ ПризнакОбособленногоПодразделения Тогда
								...
								СтруктураВозврата.НадписьПоясненияНекорректногоКПП = Новый ФорматированнаяСтрока(НСтр("ru = 'КПП не соответствует формату'"),,ЦветВыделенияНекорректногоЗначение);
