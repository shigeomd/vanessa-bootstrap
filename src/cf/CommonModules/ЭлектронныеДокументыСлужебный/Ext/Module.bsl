...
Процедура ПроверитьКонтрагентовЭДРегламентноеЗадание() Экспорт
	...
	|ИЗ
	|	РегистрСведений.СостоянияКонтрагентовЭД КАК СостоянияКонтрагентов
	|ГДЕ
	|	СостоянияКонтрагентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияКонтрагентаЭД.ПустаяСсылка)";
...
Процедура ПроверитьНепроверенныхКонтрагентов(Параметры = Неопределено) Экспорт
	...
	|	Контрагенты.КПП,
	|	&Дата КАК Дата,
	|	ЗНАЧЕНИЕ(Перечисление.СостоянияКонтрагентаЭД.ПустаяСсылка) КАК Состояние
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияКонтрагентовЭД КАК СостоянияКонтрагентов
	|		ПО Контрагенты.Ссылка = СостоянияКонтрагентов.Контрагент
	|ГДЕ
	|	(СостоянияКонтрагентов.Состояние ЕСТЬ NULL 
	|			ИЛИ СостоянияКонтрагентов.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияКонтрагентаЭД.ПустаяСсылка))
	|	И Контрагенты.ИНН <> """"
...
Процедура ПроверитьКонтрагентов(ДанныеКонтрагентов, АдресХранилища = Неопределено, ЭтоПроверкаСправочникаКонтрагентов = Истина)
	...
	Отбор.Вставить("Состояние", Перечисления.СостоянияКонтрагентаЭД.ПустаяСсылка());
...
Процедура СохранитьРезультатыПроверкиВРегистр(ДанныеКонтрагентов)
	...
	Для каждого ДанныеКонтрагента Из ДанныеКонтрагентов Цикл
		...
		Попытка
			...
			СтруктураКлюча = Новый Структура("Контрагент", Контрагент);
			Ключ = РегистрыСведений.СостоянияКонтрагентовЭД.СоздатьКлючЗаписи(СтруктураКлюча);
			ЗаблокироватьДанныеДляРедактирования(Ключ);
			
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СостоянияКонтрагентовЭД");
			ЭлементБлокировки.УстановитьЗначение("Контрагент", Контрагент);
			Блокировка.Заблокировать();
			
			// Записываем данные в регистр
			НаборЗаписей = РегистрыСведений.СостоянияКонтрагентовЭД.СоздатьНаборЗаписей();
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Проверка подключений к сервису 1С-ЭДО.Запись результатов проверки контрагентов в регистр'"), 
...
Функция ШаблонТаблицыДанныеКонтрагента() Экспорт
	...
	ДанныеКонтрагентов.Колонки.Добавить("Состояние", 	Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияКонтрагентаЭД"));
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор) Экспорт
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		ИмяВременногоФайлаЗапросаСостояний = ЭлектронныеДокументыСлужебный.ТекущееИмяВременногоФайла("xml");
		...
		Исключение
			...
			Если ЗначениеЗаполнено(Результат) Тогда
				...
				ШаблонСообщения = НСтр("ru = 'При проверке состояния контаргентов на сервисе 1С-ЭДО возникла ошибка:'");
		...
		Если ДеревоXML <> Неопределено Тогда
			...
			Для каждого Строка Из ДанныеКонтрагентовДляПроверкиСервисом Цикл
				Если ЗначениеЗаполнено(ДеревоXML.Строки.Найти(Строка.ИНН, "Значение", Истина))
					...
					Строка.Состояние = Перечисления.СостоянияКонтрагентаЭД.Подключен;
				КонецЕсли;
...
Функция ОпределитьПредставлениеЭД(ВидЭД, СтруктураПараметров) Экспорт
	...
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ПлатежноеТребование Тогда
		ТекстВидЭД = НСтр("ru = 'Платежное требование'");
	...
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ИзвещениеОСостоянииЭД Тогда
		ТекстВидЭД = НСтр("ru = 'Извещение о состоянии электронного документа'");
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗапросОСостоянииЭД Тогда
		ТекстВидЭД = НСтр("ru = 'Запрос о состоянии электронного документа'");
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗапросЗонд Тогда
		ТекстВидЭД = НСтр("ru = 'Тестовый запрос'");
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.ЗапросНаОтзывЭД Тогда
		ТекстВидЭД = НСтр("ru = 'Запрос на отзыв электронного документа'");
	КонецЕсли;
...
Процедура ОбработатьОтветБанка(ОтветБанка, ЭД, НовыйЭД = Неопределено, ЕстьОшибка = Неопределено) Экспорт
	...
	Иначе
		...
		НаименованиеФайла = СтрЗаменить(НаименованиеЗапроса, Нстр("ru = 'Запрос выписки'"), Нстр("ru = 'Выписка банка за период'"));
...
Процедура ОтправитьЗапросСостоянияЭДВБанк(СтруктураПараметров, АдресХранилища) Экспорт
	...
	НазваниеЭД = СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(НСтр("ru = 'Запрос состояния ЭД'"));
...
Процедура ОтправитьРегистрационныйПакет1СЭДО(Знач Файл, ПараметрыАутентификации, ЕстьОшибки) Экспорт
	...
	Исключение
		...
		Если ЗначениеЗаполнено(Результат) Тогда
			...
			ШаблонСообщения = НСтр("ru = 'При отправке заявления о регистрации: %1 на сервисе 1С-ЭДО возникла ошибка:'");
