...
Процедура ОчиститьВариантыЗаполнения(Форма, Знач Раздел = Неопределено, УстановитьЦветВРазделе = Истина, Знач ИмяСекции = "") Экспорт
	...
	Для Каждого СтрокаУровня1 Из Форма.мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если Форма.мСтруктураВариантыЗаполнения.Свойство(НазваниеСтраницы, МассивВариантовЗаполненияСтраниц) Тогда
			...
			#Иначе
				Если Найти(СокрЛП(ИмяСекции), СокрЛП(НазваниеСтраницы)) > 0 Тогда
...
Процедура УдалитьСлужебныеСимоволыИзПечатнойФормы(СписокПечатаемыхЛистов)
	...
	Для Каждого Лист Из СписокПечатаемыхЛистов Цикл 
		...
		Для Каждого Обл Из ТабДок.Области Цикл 
			Если Найти(Обл.Имя, "УдалитьСтроку") = 1 И Не Обл.СодержитЗначение Тогда 
				Обл.Текст = "";
			КонецЕсли;
			Если Найти(Обл.Имя, "ДобавитьСтроку") = 1 И Не Обл.СодержитЗначение Тогда 
				Обл.Текст = "";
...
Процедура ПроставитьСведенияОПредставителеОрганизации(Форма, ФлагТолькоФЛ, СхемаПолученияКодИФНС = "0") Экспорт
	...
	Если НЕ ((ПустаяСтрока(СтруктураПредставителя.ПредставительЮр)) И (ПустаяСтрока(СтруктураПредставителя.ПредставительФЛ)) И (ПустаяСтрока(СтруктураПредставителя.ДокументПредставителя))) Тогда
		...
		Если (НЕ Форма.СтруктураРеквизитовФормы.мПредставительФизЛицо) И (ФлагТолькоФЛ) Тогда
			...
			//Если НЕ ПустаяСтрока(ФИОУпПред) Тогда
				...
				//Форма.СтруктураРеквизитовФормы.мПредставительФизЛицо = Истина;
				//Форма.СтруктураРеквизитовФормы.мФамилияПредставителя = СтрПолучитьСтроку(ФИОУпПред, 1);
				//Форма.СтруктураРеквизитовФормы.мИмяПредставителя = СтрПолучитьСтроку(ФИОУпПред, 2);
				//Форма.СтруктураРеквизитовФормы.мОтчествоПредставителя = СтрПолучитьСтроку(ФИОУпПред, 3);

				//Титульный.Области.ОргУп.Значение = ФИОУпПредПолностью;
				//Титульный.Области.ДокУпПред.Значение = "";
                				
			//	Сообщение = Новый СообщениеПользователю;

			//	Сообщение.Текст = НСтр("ru='Необходимо указать данные по представителю организации в справочнике ""Регистрации в налоговом органе"".'");
	...
	Иначе
		...
		//Если НЕ ПустаяСтрока(ФИОУпПред) Тогда
			//Форма.СтруктураРеквизитовФормы.мПредставительФизЛицо = Истина;
			//Титульный.Области.ОргУп.Значение = ФИОУпПред;
			//ФИОУпПред = СтрЗаменить(ФИОУпПред, " ", Символы.ПС);
...
Процедура СообщитьОбОшибке(Знач ТекстСообщения, Отказ = Ложь, Заголовок = "", Знач Статус = Неопределено, ВызыватьИсключение = Истина) Экспорт
	...
	#Если Сервер Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Контрольная запись.БРО'"), УровеньЖурналаРегистрации.Ошибка, , , Заголовок + Символы.ПС + ТекстСообщения);
...
Процедура ВыборФормыРегламентированногоОтчетаПоУмолчанию(Форма) Экспорт
	...
	#Если НаСервере Тогда
	     ...
	     Если Найти(Форма.ИмяФормы,".ОсновнаяФорма") > 0 Тогда
