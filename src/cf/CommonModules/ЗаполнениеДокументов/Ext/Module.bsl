...
Функция ВидыДеятельностиСовпадают(СчетДоходов, СчетРасходов) Экспорт
	...
	Иначе
		...
		Если СчетДоходовБУЕНВД = Неопределено Тогда
			СчетДоходовБУЕНВД = БухгалтерскийУчетВызовСервераПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СчетДоходовБУВСтроке);
		...
		Если СчетРасходовБУЕНВД = Неопределено Тогда
			СчетРасходовБУЕНВД = БухгалтерскийУчетВызовСервераПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СчетРасходовБУВСтроке);
...
Процедура Заполнить(ДокументОбъект, ДанныеЗаполнения = Неопределено, ЗаполнятьСчетаУчета = Ложь) Экспорт
	...
	Если ЕстьРеквизитОрганизация И НЕ ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОсновнаяОрганизация) И НЕ Справочники.Организации.ИспользуетсяНесколькоОрганизаций() Тогда
			ВызватьИсключение НСтр("ru = 'Не заполнены реквизиты организации'");
		КонецЕсли;
	...
	НДСНеВыделять = Ложь;
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("НДСНеВыделять", МетаданныеДокумента) Тогда
		НДСНеВыделять = ЕстьРеквизитОрганизация И УчетнаяПолитика.ПрименяетсяУСНДоходы(ДокументОбъект.Организация, ДокументОбъект.Дата);
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаСтавкаНДС) И НЕ ЗначениеЗаполнено(ДокументОбъект[ИмяРеквизитаСтавкаНДС]) Тогда
		...
		Если НЕ ПлательщикНДС И (ДокументЯвляетсяРеализацией ИЛИ НДСНеВыделять) Тогда
