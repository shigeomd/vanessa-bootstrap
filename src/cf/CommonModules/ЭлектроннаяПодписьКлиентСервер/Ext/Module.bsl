...
Функция ПредставлениеСертификата(Сертификат, Отчество = Ложь, СрокДействия = Истина) Экспорт
	...
	Возврат ПредставлениеСубъекта(Сертификат, Отчество) + ", "
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'до %1'"), Формат(Сертификат.ДатаОкончания, "ДФ=MM.yyyy"));
...
Функция ПредставлениеИздателя(Сертификат) Экспорт
	...
	Если ЗначениеЗаполнено(Издатель.ОбщееИмя)
	   ...
	   И Найти(Издатель.ОбщееИмя, Издатель.Организация) = 0 Тогда
		...
		Если Свойства.ОбщееИмя <> Свойства.Организация
		   И Не (Субъект.Свойство("T") И Свойства.ОбщееИмя = ПодготовитьСтроку(Субъект.T)) Тогда
			...
			Массив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
				Свойства.ОбщееИмя, " ", Истина, Истина);
			
			Если Массив.Количество() < 4 Тогда
				Если Массив.Количество() > 0 Тогда
					Свойства.Фамилия = Массив[0];
				КонецЕсли;
				Если Массив.Количество() > 1 Тогда
					Свойства.Имя = Массив[1];
				КонецЕсли;
				Если Массив.Количество() > 2 Тогда
					Свойства.Отчество = Массив[2];
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьОписаниеДанныхСертификата(Таблица, Сертификат) Экспорт
	...
	Строка.Значение = Формат(СтруктураСертификата.ДействителенДо, "ДФ=dd.MM.yyyy");
...
Функция ЭтоИННФизЛица(ИНН)
	...
	Для НомерСимвола = 1 По 12 Цикл
		Если Найти("0123456789", Сред(ИНН,НомерСимвола,1)) = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
