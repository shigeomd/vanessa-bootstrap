...
//
//                                * ВключатьБизнесЛогику  - Булево, флаг необходимость включения бизнес логики при
//                                                          записи объектов.
//                                                          Значение по умолчанию - Истина.
//
//                                * ЗаменаПарыВТранзакции - Булево - при значении Истина транзакция охватывает все
//                                                                   производимые замены для одной пары ссылок.
//                                                          Ложь - каждая замена ссылки в одном объекте выполняется в
//                                                                 отдельной транзакции.
//                                                          Значение по умолчанию - Истина.
//
//                                * ПривилегированнаяЗапись - Булево - При значении Истина запись происходит в
//                                                                     привилегированном режиме, иначе - с текущими
//                                                                     правами.
...
//                             "ОшибкаЗаписи"      - не смогли записать объект, или метод ВозможностьЗаменыЭлементов
//                                                   вернул отказ.
...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	Если Параметры <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
			"ОбщегоНазначения.ЗаменитьСсылки", "Параметры", Параметры, Тип("Структура"));
		...
		Если Параметры.Свойство("СпособУдаления", ЗначениеПараметра) Тогда
			...
			Иначе
				ТекстОшибки = НСтр("ru = 'Недопустимое значение параметра %1 в %2. 
					|Ожидалось: %3 или %4; передано значение: %5 (тип %6).'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
					"Параметры.СпособУдаления",
					"ОбщегоНазначения.ЗаменитьСсылки",
					"Непосредственно",
					"Пометка",
					?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, "Неопределено"),
		...
		Если Параметры.Свойство("ВключатьБизнесЛогику", ЗначениеПараметра) Тогда
			ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
				"ОбщегоНазначения.ЗаменитьСсылки", "Параметры.КонтрольПриЗаписи", ЗначениеПараметра, Тип("Булево"));
			ПараметрыЗамены.КонтрольПриЗаписи = ЗначениеПараметра;
		КонецЕсли;
		
		Если Параметры.Свойство("ЗаменаПарыВТранзакции", ЗначениеПараметра) Тогда
			ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
				"ОбщегоНазначения.ЗаменитьСсылки", "Параметры.ЗаменаПарыВТранзакции", ЗначениеПараметра, Тип("Булево"));
			ДлиннаяТранзакция = ЗначениеПараметра;
		КонецЕсли;
		
		Если Параметры.Свойство("ПривилегированнаяЗапись", ЗначениеПараметра) Тогда
			ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
				"ОбщегоНазначения.ЗаменитьСсылки", "Параметры.ПривилегированнаяЗапись", ЗначениеПараметра, Тип("Булево"));
			ПараметрыЗамены.ПривилегированнаяЗапись = ЗначениеПараметра;
		КонецЕсли;
	...
	Если ПараметрыЗамены.КонтрольПриЗаписи И ПодсистемаСуществует("СтандартныеПодсистемы.ПоискИУдалениеДублей") Тогда
		...
		Для Каждого КлючЗначение Из Ошибки Цикл
			Что = КлючЗначение.Ключ;
			НаЧто = ПарыЗамен[Что];
			ТекстОшибки = КлючЗначение.Значение;
			Причина = ОписаниеОшибкиЗамены("ОшибкаЗаписи", НаЧто, ПредметСтрокой(НаЧто), ТекстОшибки);
			ДобавитьРезультатЗамены(РезультатЗамены, Что, Причина);
			
			Индекс = СписокСсылок.Найти(Что);
			Если Индекс <> Неопределено Тогда
				СписокСсылок.Удалить(Индекс); // пропускаем проблемный элемент
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
// Устарела. Следует использовать ПоискИУдалениеДублей.НайтиДублиЭлемента.
//
Функция НайтиДублиЭлемента(Знач ОбластьПоиска, Знач ЭталонныйОбъект, Знач ДополнительныеПараметры) Экспорт
	
	Если ПодсистемаСуществует("СтандартныеПодсистемы.ПоискИУдалениеДублей") Тогда
		МодульПоискИУдалениеДублей = ОбщийМодуль("ПоискИУдалениеДублей");
		Возврат МодульПоискИУдалениеДублей.НайтиДублиЭлемента(ОбластьПоиска, ЭталонныйОбъект, ДополнительныеПараметры);
	КонецЕсли;
...
// Устарела. Следует использовать ОбщегоНазначенияКлиентСервер.ИмяCOMСоединителя.
//
Функция ИмяCOMСоединителя() Экспорт
	
	Возврат ОбщегоНазначенияКлиентСервер.ИмяCOMСоединителя();
...
// Устарела. Следует использовать ОбщегоНазначенияКлиентСервер.УстановитьВнешнееСоединение.
//
Функция УстановитьВнешнееСоединение(Параметры, СтрокаСообщенияОбОшибке = "", ОшибкаПодключенияКомпоненты = Ложь) Экспорт
	
	Возврат ОбщегоНазначенияКлиентСервер.УстановитьВнешнееСоединение(Параметры, СтрокаСообщенияОбОшибке, ОшибкаПодключенияКомпоненты);
	
КонецФункции

// Устарела. Следует использовать ОбщегоНазначенияКлиентСервер.УстановитьВнешнееСоединениеСБазой.
//
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	
	Возврат ОбщегоНазначенияКлиентСервер.УстановитьВнешнееСоединениеСБазой(Параметры);
...
Функция ИнтервалВремениСтрокой(ВремяНачала, ВремяОкончания = Неопределено) Экспорт
	...
	ИначеЕсли ВеличинаИнтервалаВДнях >= 1 Тогда
		...
		ОписаниеИнтервала = СтрЗаменить(
			ПрописьЧислаИПредмета,
			ПрописьЧисла,
			Формат(ВеличинаИнтервалаВДнях, "ЧДЦ=0") + " ");
	Иначе
