...
Функция ПолучитьПроверочныйКод(Знач ПредметПроверки, ПовторнаяОтправка = Ложь, Тип = "phone") Экспорт
	...
	Исключение
		Если Тип = "phone" Тогда
			Событие = НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Проверка мобильного телефона'");
		Иначе
			Событие = НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Проверка электронной почты'");
		КонецЕсли;
...
Функция ПроверитьНомерТелефона(НомерТелефона, ПроверочныйКод) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Проверка мобильного телефона'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПроверитьАдресЭлектроннойПочты(АдресЭлектроннойПочты, ПроверочныйКод) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Проверка электронной почты'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПолучитьВременныйПароль(Идентификатор, ПовторнаяОтправка = Ложь, Тип = "phone") Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Получение временного пароля'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПолучитьСеансовыйКлюч(Идентификатор, ВременныйПароль) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Получение сеансового ключа'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция Авторизован(Идентификатор) Экспорт
	...
	Если ЗначениеЗаполнено(СеансовыйКлюч) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Авторизация'"), 
				УровеньЖурналаРегистрации.Ошибка,,,
				КомментарийПоИсключению(
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПолучитьСвойстваСертификата(Сертификат) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Получение свойств сертификата'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция Подписать(Идентификатор, ИсходныеДанные, ВключатьИсходныеДанные) Экспорт
    ...
    Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Подписание'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
Функция ПроверитьПодпись(Сертификат, Подпись, ИсходныеДанные) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Проверка подписи'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Новый Структура("Сертификат,Подпись,ИсходныеДанные", Сертификат, Подпись, ИсходныеДанные)));
		ВызватьСтандартноеИсключение();
	КонецПопытки;
...
Функция Зашифровать(Идентификатор, СертификатыПолучателей, ИсходныеДанные) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Шифрование'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
...
//
Функция Расшифровать(Идентификатор, ЗашифрованныеДанные) Экспорт
	
	Попытка
		КриптосервисПрокси = КриптосервисПрокси();

		Возврат КриптосервисПрокси.Decrypt(СеансовыйКлюч(Идентификатор), ЗашифрованныеДанные);
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Дешифрование'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Новый Структура("Идентификатор,ЗашифрованныеДанные", Идентификатор, ЗашифрованныеДанные)));
		ВызватьСтандартноеИсключение();
	КонецПопытки;
...
Функция ПодписатьИЗашифровать(Идентификатор, СертификатыПолучателей, ИсходныеДанные) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Подписание и шифрование'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Новый Структура("Идентификатор,СертификатыПолучателей,ИсходныеДанные", Идентификатор, СертификатыПолучателей, ИсходныеДанные)));
...
Функция ПолучитьСпособыДоставкиПаролей(Идентификатор) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Получение способов доставки паролей'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Новый Структура("Идентификатор", Идентификатор)));
...
Функция СообщениеСПаролемДоставлено(Идентификатор) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Электронная подпись в модели сервиса.Криптосервис.Получение статуса SMS'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			КомментарийПоИсключению(
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				Новый Структура("Идентификатор", Идентификатор)));
...
Функция ПредставлениеСпискаOID(СписокOID)
	...
	Если Лев(СокрЛП(СтандартныеПодсистемыСервер.ВерсияБиблиотеки()) + ".", 4) = "2.1." Тогда
		Возврат ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СтроковыеФункцииКлиентСервер").ПолучитьСтрокуИзМассиваПодстрок(МассивЗначений, ",");
	Иначе
		Возврат ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СтроковыеФункцииКлиентСервер").СтрокаИзМассиваПодстрок(МассивЗначений, ",");
	КонецЕсли;
