...
Функция ВыгрузитьРегламентированныйОтчет(Отчет, ПроверкаБлокировкиФормы, ТекстВыгрузки = Неопределено, ИмяФайлаВыгрузки = Неопределено, КодировкаТекстаВыгрузки = Неопределено, ТипФайлаВыгрузки = Неопределено, парамДокументы = Неопределено, УникальныйИдентификатор, ВозвращатьИменаИТекстыФайловВыгрузкиВВидеМассивов = Ложь) Экспорт
	...
	Если ТипЗнч(Отчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если ТипЗнч(ДанныеВыгрузки) = Тип("Массив") Тогда
			Для Каждого ЭлементВыгрузки Из ДанныеВыгрузки Цикл
				Если ЭлементВыгрузки.ТипФайлаВыгрузки = "НДС2015Разделы1_7" Тогда
