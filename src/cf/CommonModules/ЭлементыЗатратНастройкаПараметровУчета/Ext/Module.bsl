Функция РазрезыУчетаРасходовНаПланеСчетов()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетаРасходов", Элементызатрат.ПредопределенныеСчетаРасходов());
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Процедура ДобавитьОписаниеРазрезовУчета(ОписаниеРазрезовУчета, ШаблонОписанияСчетов, ШаблонОписанияВидовСубконто, ШаблонОписанияПараметров, ШаблонОписанияИсключений) Экспорт
	...
	// Описание параметров см. в ОбщегоНазначенияБПВызовСервера.ПолучитьСоответствиеСубконтоПараметрамУчета()
	
	ОписаниеСчетов = ШаблонОписанияСчетов.СкопироватьКолонки();
	Для Каждого СчетРасходов Из ЭлементыЗатрат.ПредопределенныеСчетаРасходов() Цикл
	...
	Для Каждого ИмяПараметра Из ЭлементыЗатратНастройкаПараметровУчетаКлиентСервер.ПараметрыУчета() Цикл
		...
		НоваяСтрока.Вид            = ЭлементыЗатратНастройкаПараметровУчетаКлиентСервер.ВидСубконто(ИмяПараметра);
...
Процедура ПрочитатьЗначенияПараметровУчета(ПараметрыУчета) Экспорт
	...
	Для Каждого ИмяПараметра Из ЭлементыЗатратНастройкаПараметровУчетаКлиентСервер.ПараметрыУчета() Цикл
		...
		ВидСубконто = ЭлементыЗатратНастройкаПараметровУчетаКлиентСервер.ВидСубконто(ИмяПараметра);
...
Функция РасходыУчитываютсяПоЭлементамЗатрат(НачалоПериода, КонецПериода, Организация) Экспорт
	...
	РазрезыУчетаРасходовНаПланеСчетов = РазрезыУчетаРасходовНаПланеСчетов();
	Если РазрезыУчетаРасходовНаПланеСчетов.Найти(ЭлементыЗатратНастройкаПараметровУчетаКлиентСервер.ВидСубконтоЭлементыЗатрат()) = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	|	УчетнаяПолитика.УчитыватьРасходыПоЭлементамЗатрат КАК УчитыватьРасходыПоЭлементамЗатрат
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&НачалоПериода, Организация = &Организация) КАК УчетнаяПолитика
	|ГДЕ
