	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент:
			           |Некорректное значение параметра ДанныеСертификата ""%1"".'"), Строка(Контекст.ДанныеСертификата));
...
Процедура ПроверитьПодпись(Оповещение, ИсходныеДанные, Подпись, МенеджерКриптографии = Неопределено) Экспорт
	...
	Если МенеджерКриптографии = Неопределено Тогда
		СоздатьМенеджерКриптографии(Новый ОписаниеОповещения(
			...
			"ПроверкаПодписи", Не Контекст.ПроверятьНаСервере);
	Иначе
...
Процедура ПроверитьПодписьНаКлиенте(Контекст)
	...
	Контекст.МенеджерКриптографии.НачатьПроверкуПодписи(Новый ОписаниеОповещения(
		...
		Контекст.ИсходныеДанные, Подпись);
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	ПроверитьСертификат(Контекст.Оповещение, Сертификат, МенеджерКриптографии);
...
//
Процедура ПроверитьСертификат(Оповещение, Сертификат, МенеджерКриптографии = Неопределено) Экспорт
...
Процедура ПроверитьСертификатПослеВыгрузкиСертификата(Сертификат, Контекст) Экспорт
	...
	Если ЭлектроннаяПодписьСлужебныйВызовСервера.ПроверитьСертификат(АдресСертификата,
			Контекст.ОписаниеОшибкиНаСервере) Тогда
...
Процедура ПроверитьСертификатНаКлиенте(Контекст)
	...
	Если Контекст.МенеджерКриптографии = Неопределено Тогда
		СоздатьМенеджерКриптографии(Новый ОписаниеОповещения(
			...
			"ПроверкаСертификата", Контекст.ОписаниеОшибкиНаСервере = Неопределено);
	Иначе
...
Процедура ПроверитьСертификатПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Контекст.МенеджерКриптографии.НачатьПроверкуСертификата(Новый ОписаниеОповещения(
		...
		СертификатКриптографии, РежимыПроверкиСертификата);
...
// Продолжение процедуры СоздатьМенеджерКриптографии.
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст) Экспорт
	...
	Иначе
		Если ЗначениеЗаполнено(Контекст.Программа) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Программа %1 не доступна или не установлена на компьютере.'"), Контекст.Программа);
		Иначе
...
Процедура СохранитьДанныеВместеСПодписьюПослеЦикла(Контекст)
	...
	Если Контекст.ПолучаемыеФайлы.Количество() > 0 Тогда
		...
		НачатьЗапросРазрешенияПользователя(Новый ОписаниеОповещения(
			"СохранитьДанныеВместеСПодписьюПослеПолученияРазрешений", ЭтотОбъект, Контекст), Вызовы);
	Иначе
...
// Только для внутреннего использования.
Процедура ПолучитьДанныеИзОписанияДанных(Оповещение, Форма, ОписаниеДанных, ИсточникДанных, ДляСтороныКлиента)
