...
Функция НадписьРеквизитыПлатежейВБюджет(ДокументОбъект) Экспорт
	...
	Если ПлатежиВБюджетКлиентСервер.РеквизитЗаполнен(ДокументОбъект.КодБК) Тогда
		КБК = ДокументОбъект.КодБК;
	Иначе
		КБК = НСтр("ru = '<КБК не указан>'");
	КонецЕсли;
	
	Если ПлатежиВБюджетКлиентСервер.РеквизитЗаполнен(ДокументОбъект.КодОКАТО) Тогда
		КодТерритории = ДокументОбъект.КодОКАТО;
	Иначе
		КодТерритории = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '<%1 не указан>'"),
			ПлатежиВБюджетКлиентСервер.ЗаголовокКодаТерритории(ДокументОбъект.Дата));
	КонецЕсли;
	
	Если ПлатежиВБюджетКлиентСервер.РеквизитЗаполнен(ДокументОбъект.СтатусСоставителя) Тогда
		СтатусСоставителя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Статус: %1'"),
			ДокументОбъект.СтатусСоставителя);
	Иначе
	...
	Возврат КБК + "; "
		+ КодТерритории + "; "
		+ ДокументОбъект.ПоказательОснования + "; "
		+ ДокументОбъект.ПоказательПериода + "; "
		+ ДокументОбъект.ПоказательНомера + "; "
		+ ДокументОбъект.ПоказательДаты + "; "
		+ ?(ДокументОбъект.Дата < НачалоДействияПриказа126н(), ДокументОбъект.ПоказательТипа + "; ", "") // С 01.01.2015 не заполняется
		+ СтатусСоставителя + "; "
		+ ДокументОбъект.ИдентификаторПлатежа;
...
Функция ДопустимыеЗначенияРеквизита(ИмяРеквизита, Реквизиты, Контекст)
	...
	ИначеЕсли ИмяРеквизита = "СтатусПлательщика" Тогда
		...
		Возврат ДопустимыеСтатусыПлательщика(Реквизиты, Контекст.ФизическоеЛицо, Контекст.НалоговыйАгент, Контекст.Период);
...
Функция ЭтоСчетОрганаФедеральногоКазначейства(Знач НомерСчета)
	...
	Возврат УчетДенежныхСредствКлиентСервер.БалансовыйСчет(НомерСчета) = "40101"; // п.1 Приложения 1 к 107н
...
Функция ЭтоСчетИсполнителяГосуслуг(Знач НомерСчета, Период)
	...
	БалансовыйСчет       = УчетДенежныхСредствКлиентСервер.БалансовыйСчет(НомерСчета);
	ПризнакЛицевогоСчета = Сред(НомерСчета, 14, 1);
	
	// п.2 Приложения 4 к 107н,
	// Указание ЦБ РФ от 24 декабря 2010 г. N 2547-У
	// БалансовыйСчет = "40302" не включен в перечень, т.к. на бюджетополучателей по данному счету не распространяется приказ 107н.
	Возврат (БалансовыйСчет = "40501" И  ПризнакЛицевогоСчета = "2")
...
Функция ПроверитьУИН(ИдентификаторПлатежа, СтандартнаяПроверкаЗаполнения = Истина) Экспорт
	
	СтандартнаяПроверкаЗаполнения = Ложь; // Идентификатор заполняется при наличии. Поэтому незаполненный идентификатор считаем валидным
	
	Если НЕ РеквизитЗаполнен(ИдентификаторПлатежа) Тогда
		Возврат "";
	КонецЕсли;
	
	Если СтрДлина(ИдентификаторПлатежа) <> ДлинаУИН() Тогда
		
		ШаблонТекста = НСтр("ru = 'Уникальный идентификатор начисления должен состоять из %1 символов, а не из %2'");
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста, ДлинаУИН(), СтрДлина(ИдентификаторПлатежа));
	...
	Если НедопустимыеСимволы.Количество() > 0 Тогда
		...
		НедопустимыеСимволы = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(НедопустимыеСимволы, ",");
...
Функция ДопустимыеСтатусыПлательщика(Реквизиты, ФизическоеЛицо = Ложь, НалоговыйАгент = Неопределено, Период = Неопределено)
	...
	Иначе
		...
		Если ФизическоеЛицо Тогда
			Статусы.Добавить("24"); // физическое лицо, уплачивающее страховые взносы и иные платежи
		КонецЕсли;
...
Функция ЭтоПлатежПоИсполнительномуЛисту(КодОснования) Экспорт
	
	Возврат КодОснования = "АР" // Применяется для налоговых платежей
...
Функция ПроверитьЗаполнение(Реквизиты, Контекст) Экспорт
	...
	// 22 - УИН
	Ошибка = ПроверитьУИН(Реквизиты.ИдентификаторПлатежа, СтандартнаяПроверкаЗаполнения);
	...
	ДобавитьОписаниеРезультатаПроверки(Результат, Ошибка, "ИдентификаторПлатежа", СтандартнаяПроверкаЗаполнения);
