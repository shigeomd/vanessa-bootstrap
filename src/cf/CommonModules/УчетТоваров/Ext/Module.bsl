...
Функция ПодготовитьТаблицыСписанныеТоварыСУчетомВозврата(ТаблицыТовары, ТаблицаРеквизиты, ТаблицаВозвраты, Отказ) Экспорт
	...
	РассчитыватьСтоимость = БухгалтерскийУчетПереопределяемый.ПроводитьПоСписаниюМПЗ(
	...
	Для Каждого КлючИЗначение Из ТаблицыТовары Цикл
		...
		Для Каждого СтрокаТаблицыТовары Из ТаблицаТовары Цикл
			...
			ИначеЕсли Не РассчитыватьСтоимость Тогда
				
				// Стоимость рассчитывать не будем
...
Процедура СписатьОстаткиТоваров(ТаблицаСписаниеПоОстаткам, ТаблицаОстатки, ТаблицаСписанныеТовары, СпособОценкиМПЗПоСредней, КонтролироватьОстаток, Реквизиты, Отказ)
	...
	Для каждого СтрокаДокумента Из ТаблицаСписаниеПоОстаткам Цикл
		...
		Если ТаблицаОстатки.Количество() <> 0 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДокумента.ДокументОприходования) Тогда
				Если (НЕ СпособОценкиМПЗПоСредней ИЛИ БухгалтерскийУчетВызовСервераПовтИсп.КомиссионныйТовар(СтрокаДокумента.СчетУчета))
					И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(СтрокаДокумента.СчетУчета) Тогда
			...
			Для каждого СтрокаПартии Из ПартииОстатков Цикл
				...
				Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(СтрокаДокумента.СчетУчета) Тогда
		...
		Если КоличествоОсталосьПогасить > 0
			И (КонтролироватьОстаток ИЛИ БухгалтерскийУчетВызовСервераПовтИсп.КомиссионныйТовар(СтрокаДокумента.СчетУчета)) Тогда
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		...
		Если БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяРазукомплектацией(Реквизиты.Регистратор) Тогда
...
Процедура ВывестиСообщениеОбОшибке(Количество, КоличествоОсталосьПогасить, Регистратор, ИмяСписка = "", СинонимСписка = "", НомерСтроки = "", Отказ)
	...
	ТекстОшибки = НСтр("ru='Указанное количество превышает остаток.
...
Функция ПодготовитьТаблицуЗакупленныхТоварыКомитентов(ТаблицаТоварыКомитентов, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.ЗакупленныеТовары.Количество() = 0 Тогда
...
Функция ПодготовитьТаблицуСписанныеТоварыНТТ(ТаблицаТоварыНТТ, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.ТаблицаТоварыНТТ.Количество() = 0 Тогда
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТоварыНТТ Цикл
		...
		СписанныйТоварНТТ.СуммаСписания    = Сумма - СуммаНаценки;
		СписанныйТоварНТТ.СуммаСписанияНУ  = Сумма - СуммаНаценкиНУ;
...
Процедура СформироватьДвиженияСписаниеТоваров(ТаблицаСписанныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	НазваниеОбъектаВСодержании	= Найти(Реквизиты.Содержание, НСтр("ru = ' товаров'"));
	...
	Для каждого СтрокаПартии Из Параметры.ТаблицаСписанныеТовары Цикл
		...
		Если НазваниеОбъектаВСодержании Тогда
			Проводка.Содержание  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Реквизиты.Содержание,
				БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаПартии.СчетУчета));
		Иначе
...
Процедура СформироватьДвиженияПеремещениеТоваров(ТаблицаСписанныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаПартии Из Параметры.ТаблицаСписанныеТовары Цикл
		...
		Если Проводка.СчетДт = Проводка.СчетКт Тогда
			...
			Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(Проводка.СчетДт)
				И НЕ БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(Проводка.СчетДт)  Тогда
...
Процедура СформироватьДвиженияВозвратаПоЗакупленнымТоварамКомитента(ЗакупленныеТоварыКомитента, ОстаткиЗакупленныеТоварыКомитентов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТовары Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			Если РаспределитьСумма = 0 И РаспределитьКоличество = 0 Тогда
				Прервать;
			КонецЕсли;
			Если Строка.СуммаОстаток <= 0 И Строка.КоличествоОстаток <= 0 Тогда
				Продолжить;
			КонецЕсли;
		...
		Если РаспределитьСумма > 0 ИЛИ РаспределитьКоличество > 0 Тогда
...
Функция ПодготовитьТаблицуВозвращенныеСписанныеТовары(ТаблицаТовары, ТаблицаСчетаУчетаТоваров, ТаблицаСписокНоменклатуры, ТаблицаРеквизиты, Отказ) Экспорт
	...
	ВозвратПрошлогоМесяца = Месяц(Реквизиты.Период) <> Месяц(Реквизиты.ДатаДокументаРеализации) ИЛИ Год(Реквизиты.Период) <> Год(Реквизиты.ДатаДокументаРеализации);
	
	СтруктураСчетовДляЗапроса = РазделитьСчетаУчетаПоПартионномуУчету(Параметры.СчетаУчетаТоваров);
...
Функция РазделитьСчетаУчетаПоПартионномуУчету(ТаблицаСчетаУчетаТоваров)
	...
	Для каждого СтрокаСчета Из ТаблицаСчетаУчетаТоваров Цикл
		Если БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(СтрокаСчета.СчетУчета) Тогда
...
Процедура СформироватьДвиженияКорректировкаПоступлениеТоваров(ТаблицаВозвращенныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаВозвращенныеТовары.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияПоступлениеТары(ТаблицаТара, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТаблицаТара.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияКорректировкаСтоимостиТары(ТаблицаТара, ТаблицаСписанныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.ТаблицаТара.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТара Цикл
		...
		Для каждого СтрокаПартии Из СтрокиТаблицыСписаннаяТара Цикл
			...
			Если РазницаЦен > 0 Тогда
				...
				Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ПрочиеДоходы;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт, "ПрочиеДоходыИРасходы", 
...
Процедура СформироватьДвиженияКорректировкаСтоимостиТоваров(ТаблицаТовары, ТаблицаСписанныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Параметры = ПодготовитьПараметрыКорректировкаСтоимостиТоваров(ТаблицаТовары, ТаблицаСписанныеТовары, ТаблицаРеквизиты);
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТовары Цикл
		...
		РазницаВСтоимости    = СтрокаТаблицы.Сумма - СписанныеПартииТовара.Итог("СуммаСписания");
		РазницаВСтоимостиВал = СтрокаТаблицы.СуммаВзаиморасчетов - СписанныеПартииТовара.Итог("СуммаСписанияВал");
		РазницаВСтоимостиНУ = ?(ПлательщикНалогаНаПрибыль И СвойстваСчетаУчета.НалоговыйУчет,
			СтрокаТаблицы.Сумма - СписанныеПартииТовара.Итог("СуммаСписанияНУ"), 0);
		...
		Если РазницаВСтоимости > 0 ИЛИ
			...
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ПрочиеДоходы;
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,Проводка.СубконтоКт, "ПрочиеДоходыИРасходы", 
													ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.РазницыСтоимостиВозвратаИФактическойСтоимостиТоваров"));
...
Функция ПодготовитьПараметрыКорректировкаСтоимостиТоваров(ТаблицаТовары, ТаблицаСписанныеТовары, ТаблицаРеквизиты)
...
Процедура СформироватьДвиженияПоступлениеЗакупленныхТоваровУслугКомитентов(ТаблицаТоварыУслуги, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.ТоварыУслуги.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияОприходованиеТоваров(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ВедетсяУчетПоПартиям    = СпособОценкиМПЗ <> Перечисления.СпособыОценки.ПоСредней;
	ОтражатьВНалоговомУчете = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Реквизиты.Организация, Реквизиты.Период);
	СтавкаНалогаНаПрибыль   = ?(НЕ ОтражатьВНалоговомУчете, 0, НалоговыйУчет.ПолучитьСтавкуНалогаНаПрибыль(
		Новый Структура("Организация,Дата", Реквизиты.Организация, Реквизиты.Период)));
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТовары Цикл
		...
		Если ОтражатьВНалоговомУчете
			...
			СуммаНУ = СтрокаТаблицы.Сумма * ?(Реквизиты.Период < '20100101', СтавкаНалогаНаПрибыль, 1);

			Если СвойстваСчетаДт.НалоговыйУчет Тогда
				Проводка.СуммаНУДт = СуммаНУ;
				Проводка.СуммаПРДт = СтрокаТаблицы.Сумма - СуммаНУ;
				Проводка.СуммаВРДт = 0;
			КонецЕсли;
...
Процедура СформироватьДвиженияНачислениеТорговойНаценкиПеремещениеТоваров(ТаблицаТовары, ТаблицаСписанныеТовары,
	...
	Если Параметры.Реквизиты.Количество() = 0 
		ИЛИ Параметры.ТаблицаТовары.Количество() = 0 
		ИЛИ ТаблицаСписанныеТовары.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияСписаниеТорговойНаценкиАТТ(ТаблицаСписанныеТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если НЕ (Реквизиты.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин И УчетВПродажныхЦенах) Тогда // Не АТТ в ценах продажи
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьТаблицуСписанияТорговойНаценкиАТТ(ТаблицаСписанныеТовары, ТаблицаРеквизиты) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0 
		ИЛИ Параметры.ТаблицаСписанияТорговойНаценкиАТТ.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если Параметры.ТаблицаСписанияТорговойНаценкиАТТ.Количество() = 0 
		ИЛИ (НЕ (Реквизиты.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин И УчетВПродажныхЦенах)) Тогда
...
Процедура СформироватьДвиженияКорректировкиПоступленияТоваров(РеквизитыДокумента, ТаблицаПоТоварам, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0 
		ИЛИ ТаблицаПоТоварам = Неопределено
		ИЛИ ТаблицаПоТоварам.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТовары Из ТаблицаПоТоварам Цикл
		...
		Если Коэффициент1 <> 0 Тогда
			...
			Если СтрКоличество <> 0 ИЛИ СтрСуммаБУ <> 0 ИЛИ СтрНДС <> 0 Тогда
				...
				Проводка.Период       = Реквизиты.Период;
				Проводка.Организация  = Реквизиты.Организация;
				Проводка.Содержание  = "Поступление " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаТовары.СчетУчета)
				...
				Если Реквизиты.ВестиПартионныйУчет ИЛИ БухгалтерскийУчетВызовСервераПовтИсп.КомиссионныйТовар(СтрокаТовары.СчетУчета) Тогда
			...
			Если Реквизиты.РаздельныйУчетНДСНаСчете19 Тогда
				...
				Если НЕ НТТ И (СтрКоличество <> 0 ИЛИ СтрСуммаБУ <> 0 ИЛИ СтрНДС <> 0) Тогда
					...
					Для Ном = 1 По СвойстваСчетаДт.КоличествоСубконто Цикл
						...
						ИначеЕсли СвойстваСчетаДт["ВидСубконто" + Ном] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Партии Тогда
							Если Реквизиты.ВестиПартионныйУчет 
							 ИЛИ БухгалтерскийУчетВызовСервераПовтИсп.КомиссионныйТовар(СтрокаТовары.СчетУчета) Тогда
		...
		Если СвойстваСчетаУчета.Забалансовый Тогда
			Если ЗначениеЗаполнено(СтрокаТовары.Комитент) Тогда
				Содержание   = "Поступление " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаТовары.СчетУчета)
						+ " по вх.д." + Реквизиты.НомерВходящегоДокумента + " от " + Формат(Реквизиты.ДатаВходящегоДокумента, "ДЛФ=Д");
		...
		Если Коэффициент2 <> 0 И Коэффициент1 <> 1 Тогда
			...
			Если Реквизиты.РаздельныйУчетНДСНаСчете19 Тогда
				...
				Если СтрСуммаНДС0 <> 0 Тогда
					...
					НоваяСтрока.СчетФактура  = 
						?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение,
						Реквизиты.Регистратор,
						УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументПоступления(Реквизиты.ДокументОснование));
		...
		Если ((ЗначениеЗаполнено(СтрокаТовары.СтранаПроисхождения)
			И НЕ СтрокаТовары.СтранаПроисхождения = Справочники.СтраныМира.РОССИЯ
			И ЗначениеЗаполнено(СтрокаТовары.НомерГТД)) И МассивСчетовДляГТД.Найти(СтрокаТовары.СчетУчета) <> Неопределено)
			ИЛИ ((ЗначениеЗаполнено(СтрокаТовары.СтранаПроисхожденияДоИзменения)
			И НЕ СтрокаТовары.СтранаПроисхожденияДоИзменения = Справочники.СтраныМира.РОССИЯ
			И ЗначениеЗаполнено(СтрокаТовары.НомерГТДДоИзменения)) И МассивСчетовДляГТД.Найти(СтрокаТовары.СчетУчета) <> Неопределено) Тогда
			
			Если СтрокаТовары.НомерГТД = СтрокаТовары.НомерГТДДоИзменения 
				И СтрокаТовары.Количество <> СтрокаТовары.КоличествоДоИзменения Тогда
			...
			Иначе
				Если (ЗначениеЗаполнено(СтрокаТовары.СтранаПроисхожденияДоИзменения)
					И НЕ СтрокаТовары.СтранаПроисхожденияДоИзменения = Справочники.СтраныМира.РОССИЯ
					И ЗначениеЗаполнено(СтрокаТовары.НомерГТДДоИзменения)) И МассивСчетовДляГТД.Найти(СтрокаТовары.СчетУчета) <> Неопределено
					И СтрокаТовары.КоличествоДоИзменения <> 0 Тогда
...
Процедура СформироватьПроводкуКорректировкиПоступленияТоваровУслугКомитентаПоЗакупке(Реквизиты, СтрокаТаблицы, Движения, Отказ, Содержание) 
	...
	Если СвойстваСчетаДт.Валютный Тогда
		...
		Иначе
			...
			СписатьПоСтрСуммаВалКомитента 	= РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(СписатьПоСтрСуммаБУ, 
...
Процедура СформироватьДвиженияКорректировкиПоступленияУслуг(РеквизитыДокумента, ТаблицаПоУслугам, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0 
		ИЛИ ТаблицаПоУслугам = Неопределено
		ИЛИ ТаблицаПоУслугам.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаУслуги Из ТаблицаПоУслугам Цикл
		...
		Если Год(Реквизиты.ДокументПоступленияДата) < Год(Реквизиты.Период) Тогда
			...
			Если СтрСуммаБУ < 0 Тогда // Доходы
				...
				Проводка.Содержание  = "Поступление " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаУслуги.СчетЗатрат)
					+ " по вх.д." + Реквизиты.НомерВходящегоДокумента + " от " + Формат(Реквизиты.ДатаВходящегоДокумента, "ДЛФ=Д");
		...
		Если Реквизиты.РаздельныйУчетНДСНаСчете19 Тогда
			...
			Если СтрокаУслуги.НовыйСпособУчетаНДС = Перечисления.СпособыУчетаНДС.Распределяется И СтрНДС <> 0 Тогда
...
Процедура СформироватьДвиженияКорректировкиПоступленияАгентскихУслуг(РеквизитыДокумента, ТаблицаПоАгентскимУслугам, Движения, Отказ) Экспорт
	...
	Параметры = ПолучитьРеквизитыКорректировкиПоступленияУслуг(РеквизитыДокумента);
	Если Параметры.Реквизиты.Количество() = 0 
		ИЛИ ТаблицаПоАгентскимУслугам = Неопределено
		ИЛИ ТаблицаПоАгентскимУслугам.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвиженияКорректировкаПоступленияПоЗакупленнымТоварамКомитента(СписанныеТоварыКомитента, ОстаткиЗакупленныеТоварыКомитентов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаТовары Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			Если (Строка.СуммаОстаток = 0 И РаспределитьСумма < 0) И (Строка.КоличествоОстаток = 0 И РаспределитьКоличество<0) Тогда
				Продолжить;
			КонецЕсли;
			Если РаспределитьСумма = 0 И РаспределитьКоличество = 0 Тогда
				Прервать;
...
Процедура ДвиженияРегистровПодсистемыНДСБУ(Реквизиты, СтрокаТаблицы, ТаблицаКоэффициентов, СуммаНДССписанного, СуммаНДССписанногоВал, СуммаНДС0, Движения, Отказ)
	...
	Если НДС <> 0 Тогда
		...
		НалоговыйАгентЗаСчетСобственныхСредств = СтрокаТаблицы.ВидЦенности = Перечисления.ВидыЦенностей.НалоговыйАгентИностранцы;
		...
		Проводка.Содержание  = "Поступление " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(
			?(СтрокаТаблицы.Услуга, СтрокаТаблицы.СчетЗатрат, СтрокаТаблицы.СчетУчета))
			+ " по вх.д." + Реквизиты.НомерВходящегоДокумента + " от " + Формат(Реквизиты.ДатаВходящегоДокумента, "ДЛФ=Д");
		
		Проводка.СчетДт = СтрокаТаблицы.СчетУчетаНДС;
		
		СубконтоСФПолученные = 
			?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение,
			Реквизиты.Регистратор,
			УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументПоступления(Реквизиты.ДокументОснование));
...
// для отражения расходов БУ и НУ (Поступление (акт, накладная), Авансовый отчет, ГТД импорт, Поступление доп.расходов
	...
	Если ТранзакцияАктивна() И ОбщегоНазначенияБПВызовСервераПовтИсп.ИспользоватьУправляемыеБлокировки() Тогда
		...
		Для каждого СчетУчета Из МассивСчетовУчета Цикл
			...
			Если СуммовойУчетПоСкладам Тогда
				Если БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(СчетУчета) Тогда
...
Процедура СортировкаСчетовУчета(СчетаУчета,
	...
	Для каждого Счет Из СчетаУчета Цикл
		...
		Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(Счет)
			И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(Счет) Тогда
			СчетаУчетаНПС.Добавить(Счет);

		ИначеЕсли БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(Счет)
			И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(Счет) Тогда
			СчетаУчетаНПК.Добавить(Счет);

		ИначеЕсли БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(Счет)
			И БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(Счет) Тогда
		...
		ИначеЕсли БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(Счет) Тогда
			...
			Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(Счет) Тогда
...
Функция ПодготовитьТаблицуТорговойНаценкиАТТ(СтруктураПараметров)
	...
	ПартионныйУчет = БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяПартионныйУчет(ПланыСчетов.Хозрасчетный.ТоварыВРозничнойТорговлеВПродажныхЦенахАТТ);
	...
	Запрос.УстановитьПараметр("СчетаСебестоимостьПродаж",
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СебестоимостьПродаж));
	...
	Если НЕ ПартионныйУчет Тогда // Исключение третьего субконто (партии) из запроса
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ХозрасчетныйОстаткиТН.Субконто3 КАК Партия,", "ЗНАЧЕНИЕ(Документ.ДокументРасчетовСКонтрагентом.ПустаяСсылка) КАК Партия,");
...
Функция СведенияОПодразделении(Подразделение, ИНН)
	...
	ИначеЕсли ТипЗнч(Подразделение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
		...
		Данные = Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций);
		Результат = Подразделение.КонтактнаяИнформация.НайтиСтроки(Данные);
		
		Если Результат.Количество() > 0 Тогда
			Значение = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(Результат[0].ЗначенияПолей, Справочники.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций);
			ЗаполнитьЗначенияСвойств(СведенияОбОП, Значение);
			СведенияОбОП.ПредставлениеАдреса = Результат[0].Представление;
		КонецЕсли;
...
Процедура ЗаполнитьРегламентированныйОтчетАлкоПриложение12ФормаОтчета2012Кв3(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПоПодразделениям") Тогда
		ЗаполнитьРегламентированныйОтчетАлкоПриложение12ФормаОтчета2012Кв3ПоПодразделениям(ПараметрыОтчета, Контейнер);
	Иначе
		ЗаполнитьРегламентированныйОтчетАлкоПриложение12ФормаОтчета2012Кв3ПоОрганизации(ПараметрыОтчета, Контейнер);	
	КонецЕсли;
...
Процедура ЗаполнитьРегламентированныйОтчетАлкоПриложение12ФормаОтчета2012Кв3ПоПодразделениям(ПараметрыОтчета, Контейнер) Экспорт
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		Если СтрокаЗапроса.П000010000112 > 0 
			И (СтрокаЗапроса.КорСчет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты) 
			ИЛИ СтрокаЗапроса.КорСчет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.Товары)) Тогда
				СсылкаКорПодразделение = ?(ЗначениеЗаполнено(СтрокаЗапроса.КорПодразделение), ПолучитьОбособленноеПодразделение(СтрокаЗапроса.КорПодразделение), СтрокаЗапроса.Организация);
				Если СтрокаЗапроса.СсылкаПодразделение <> СсылкаКорПодразделение Тогда
					СтрокаЗапроса.П000010000112ПоПодразделениям = СтрокаЗапроса.П000010000112;
				КонецЕсли;
				СтрокаЗапроса.П000010000112 = 0;
		КонецЕсли;
		
		Если СтрокаЗапроса.П000010000115 > 0 
			И (СтрокаЗапроса.КорСчет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты) 
			ИЛИ СтрокаЗапроса.КорСчет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.Товары)) Тогда
				СсылкаКорПодразделение = ?(ЗначениеЗаполнено(СтрокаЗапроса.КорПодразделение), ПолучитьОбособленноеПодразделение(СтрокаЗапроса.КорПодразделение), СтрокаЗапроса.Организация);
				Если СтрокаЗапроса.СсылкаПодразделение <> СсылкаКорПодразделение Тогда
