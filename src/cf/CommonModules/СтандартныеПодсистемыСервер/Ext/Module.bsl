...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	// Инициализация всех остальных параметров сеанса (к моменту вызова служебных событий параметры работы программы уже
	// обновлены).
	Обработчики = Новый Соответствие;
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		...
		Результат.Пароль = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("АутентификацияНаПользовательскомСайте", "Пароль", "");
		 Возврат ?(Результат.Логин <> "" И Результат.Пароль <> "", Результат, Неопределено);
	КонецЕсли;
...
Процедура СохранитьПараметрыАутентификацииНаСайте(СохраняемыеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей"), 
		НСтр("ru = 'Недопустимый вызов процедуры.'"), "АдресныйКлассификаторСлужебный.СохранитьПараметрыАутентификацииНаСайте");
...
// Для внешних вызовов следует использовать процедуру ОбновитьВсеПараметрыРаботыПрограммы.
//
...
//
Процедура УстановитьКлючНазначенияФормы(Форма, Ключ, КлючПоложения = "") Экспорт
	
	УстановитьКлючНазначенияИспользованияФормы(Форма, Ключ);
	УстановитьКлючСохраненияПоложенияОкнаФормы(Форма, ?(КлючПоложения = "", Ключ, КлючПоложения));
		...
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			Исключение
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
					...
					МодульОбменДаннымиСервер.ПриОшибкеПроверкиИдентификаторовОбъектовМетаданныхВПодчиненномУзлеРИБ(ВыполнитьЗагрузку);
				КонецЕсли;
				ВызватьИсключение;
			КонецПопытки;
			...
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				...
				Иначе
					// Настройка в подчиненном узле ИБ при первом запуске.
					ИмяСобытия = НСтр("ru = 'Идентификаторы объектов метаданных.Требуется выполнить критичные изменения'",
						ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				КонецЕсли;
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
					...
					МодульОбменДаннымиСервер.ПриОшибкеПроверкиИдентификаторовОбъектовМетаданныхВПодчиненномУзлеРИБ(ВыполнитьЗагрузку, Истина);
				КонецЕсли;
				...
				Иначе
					// Настройка в подчиненном узле ИБ при первом запуске.
					ТекстОшибки =
						НСтр("ru = 'В главном узле не обновлен справочник ""Идентификаторы объектов метаданных"":
...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = "8.3.5.1336";
	...
	Если ЗначениеЗаполнено(НедоступныйРежим) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается.
			           ...
			           |версии не ниже 8.3.5 или ""Не использовать"".'"),
			НедоступныйРежим);
	КонецЕсли;
...
Процедура УстановитьКлючНазначенияИспользованияФормы(Форма, Ключ)
...
// Для процедуры УстановитьКлючНазначенияФормы.
Процедура УстановитьКлючСохраненияПоложенияОкнаФормы(Форма, Ключ)
...
Процедура ПроверитьОбновитьПараметрыРаботыПрограммы(ЕстьИзменения, ТолькоПроверка, БезИзменений, ВФоне)
	...
	Если НЕ БезИзменений.Свойство("БазоваяФункциональностьИдентификаторыОбъектовМетаданных") Тогда
		...
		Иначе
			...
			Иначе
				Справочники.ИдентификаторыОбъектовМетаданных.ВыполнитьОбновлениеДанных(ЕстьТекущиеИзменения, Ложь);
			КонецЕсли;
			Если ЕстьТекущиеИзменения Тогда
				ЕстьИзменения = Истина;
				Если ТолькоПроверка Тогда
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	МодульТекущиеДелаСлужебный = ОбщегоНазначения.ОбщийМодуль("ТекущиеДелаСлужебный");
	Если МодульТекущиеДелаСлужебный.ДелоОтключено("РекомендацияПоПовышениюСкоростиРаботы") Тогда
		Возврат;
	КонецЕсли;
...
Функция ТребуетсяПоказРекомендацииПоОбъемуОперативнойПамяти()
	...
	ОперативнаяПамять = СтандартныеПодсистемыСервер.ПараметрыКлиентаНаСервере().Получить("ОперативнаяПамять");
