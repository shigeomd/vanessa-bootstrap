...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.Файлы)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("РедактируемыеФайлы") Тогда
		Возврат;
	КонецЕсли;
	МодульТекущиеДелаСервер = ОбщегоНазначения.ОбщийМодуль("ТекущиеДелаСервер");
	
	КоличествоЗанятыхФайлов = КоличествоЗанятыхФайлов(Пользователи.ТекущийПользователь());
	...
	Если Разделы = Неопределено Тогда
		Возврат; // Интерфейс по работе с файлами не вынесен в командный интерфейс пользователя.
	КонецЕсли;
