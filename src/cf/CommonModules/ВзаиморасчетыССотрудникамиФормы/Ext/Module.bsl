...
Процедура ВедомостьПриПолученииДанныхСтрокиСостава(Форма, СтрокаСостава) Экспорт
	...
	ИначеЕсли ПериодыВзаиморасчетов.Количество() = 1 Тогда
		...
		РасшифровкаПериодов = НСтр("ru = 'за '") + НРег(ЗарплатаКадрыКЛиентСервер.ПолучитьПредставлениеМесяца(ПериодыВзаиморасчетов[0].ПериодВзаиморасчетов));
	Иначе
		// Периодов несколько, есть отличающиеся от периода ведомости
		РасшифровкаПериодов = НСтр("ru = 'в т.ч. за '");
		...
		Для Индекс = 0 По ПериодыВзаиморасчетов.Количество()-1 Цикл
			...
			Если Индекс = 3 Тогда
				РасшифровкаПериодов = РасшифровкаПериодов + "...";
				Прервать
			ИначеЕсли Индекс > 0 Тогда
				РасшифровкаПериодов = РасшифровкаПериодов + ", ";
			КонецЕсли;

			РасшифровкаПериодов = РасшифровкаПериодов + НРег(ЗарплатаКадрыКЛиентСервер.ПолучитьПредставлениеМесяца(ПериодыВзаиморасчетов[Индекс].ПериодВзаиморасчетов));
...
Функция ПредставлениеОплаты(СписокОплат)  
	...
	ИначеЕсли СписокОплат.Количество() = 1 Тогда	
		...
		ПредставлениеДокумента = 
			...
			Сред(СписокОплат[0].Представление, 2, Найти(СписокОплат[0].Представление, " - ") - 2);
