...
Функция УсловияЗапросаПроверкиНеобходимостиЗаполненияПодчиненныхПодразделений(Запрос, ИсточникДанных) Экспорт
	...
	Если НЕ ВыполняетсяЗаписьОрганизации Тогда
		СписокУсловий.Добавить("&СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов И НЕ ПодразделенияОрганизаций.СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов");
		Запрос.УстановитьПараметр("СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов", ИсточникДанных.СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов);
	КонецЕсли;
...
Процедура ЗаполнитьПодчиненноеПодразделение(ПодразделениеОбъект, ИсточникДанных) Экспорт
	...
	Если НЕ ВыполняетсяЗаписьОрганизации Тогда
		ПодразделениеОбъект.СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов = ИсточникДанных.СоответствуетСудамЗарегистрированнымВРоссийскомМеждународномРеестреСудов;
	КонецЕсли;
...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(ИсточникиДанных) Экспорт
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПачкаДокументовАДВ_1",				"Дата", 			"КадровыйУчет", "Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПачкаДокументовАДВ_2",				"Дата", 			"КадровыйУчет", "Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПачкаДокументовАДВ_3",				"Дата", 			"КадровыйУчет", "Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПачкаДокументовДСВ_1",				"Дата", 			"КадровыйУчет", "Организация");
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ПачкаДокументовСЗВ_К",				"Дата", 			"КадровыйУчет", "Организация");
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.СправкиНДФЛДляПередачиВНалоговыйОрган", 						"НалоговыйПериодДата",	"НДФЛ", "Организация");
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "РегистрНакопления.ИмущественныеВычетыНДФЛ", 							"Период", 				"НДФЛ", "ГоловнаяОрганизация");
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "РегистрСведений.ТерриториальныеУсловияПФР", 							"Период", 					"СтраховыеВзносы", "СтруктурнаяЕдиница.ГоловнаяОрганизация");
	...
	// Депоненты
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.УчетДепонированнойЗарплаты") Тогда
		МодульУчетДепонированнойЗарплаты = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("УчетДепонированнойЗарплаты");
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ВнешниеХозяйственныеОперации.ВзаиморасчетыССотрудникамиВХО") Тогда
		МодульВзаиморасчеты = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ВзаиморасчетыССотрудникамиВХО");
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.БухучетХозрасчетныхОрганизаций") Тогда
		МодульБухучетХозрасчетныхОрганизаций = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("БухучетХозрасчетныхОрганизаций");
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РегламентированнаяОтчетность") Тогда
...
Процедура ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	НовыеСведения.Объект			= "Справочник.ФизическиеЛица";
	НовыеСведения.ПоляРегистрации	= "Ссылка,Наименование,ИНН,СтраховойНомерПФР";
	НовыеСведения.ПоляДоступа		= "ДатаРождения,Пол,МестоРождения";
	...
	НовыеСведения.Объект			= "Документ.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников";
	НовыеСведения.ПоляРегистрации	= "Сотрудники.ФизическоеЛицо,Сотрудники.ЭмбоссированныйТекст1,Сотрудники.ЭмбоссированныйТекст2,Сотрудники.ЭмбоссированныйТекст3";
	НовыеСведения.ПоляДоступа		= "Сотрудники.ТелефонДомашний,Сотрудники.ТелефонДомашнийПредставление,Сотрудники.ТелефонРабочий,Сотрудники.ТелефонРабочийПредставление,Сотрудники.ЯвляетсяРезидентом,Сотрудники.Гражданство";
	НовыеСведения.ОбластьДанных		= "ЛичныеДанные";
...
Процедура ЗаполнитьПоказателиРегламентированногоОтчета(ПоказателиОтчета, ИДОтчета, ИДРедакцииОтчета) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет4ФСС" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2012Кв1" 
			Или ИДРедакцииОтчета = "ФормаОтчета2012Кв3" Тогда
...
Процедура УстановитьНачальныеЗначения(ИменаОбъектовМетаданных) Экспорт
	...
	Для каждого ОбрабатываемоеИмя Из ОбрабатываемыеИменаОбъектов Цикл
		...
		// РасчетЗарплаты
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.МинимальнаяОплатаТрудаРФ") Тогда
			РегистрыСведений.МинимальнаяОплатаТрудаРФ.НачальноеЗаполнение();
		КонецЕсли;
		
		// УчетНДФЛ
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.РазмерВычетовНДФЛ") Тогда
		...
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.ТарифыСтраховыхВзносов") Тогда
			РегистрыСведений.ТарифыСтраховыхВзносов.НачальноеЗаполнение();
		КонецЕсли;
		
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.ПредельнаяВеличинаБазыСтраховыхВзносов") Тогда
		...
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.МаксимальныйРазмерЕжемесячнойСтраховойВыплаты") Тогда
			РегистрыСведений.МаксимальныйРазмерЕжемесячнойСтраховойВыплаты.НачальноеЗаполнение();
		КонецЕсли;
...
Функция ОписанияРегистровСодержащихРегистрацииВНалоговомОргане() Экспорт
	...
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.РасчетыНалоговыхАгентовСБюджетомПоНДФЛ", Ложь);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.УплатаНДФЛНалоговымиАгентамиКРаспределению", Ложь);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.АвансовыеПлатежиИностранцевПоНДФЛ", Истина);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.РасчетыНалогоплательщиковСБюджетомПоНДФЛ", Истина);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ", Истина);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.СведенияОДоходахНДФЛ", Истина);
	ЗарплатаКадры.ДобавитьОписаниеРегистровСодержащихРегистрацииВНалоговомОргане(МассивОписаний, "РегистрНакопления.ИмущественныеВычетыНДФЛ", Истина);
...
Функция АдресВФормате9Запятых(Запись)
	...
	СтруктураАдреса = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(Запись.ЗначенияПолей);
