...
//
// Параметры:
//		ОписательВременныхТаблиц - структура полученная функцией
...
Процедура СоздатьПоВременнойТаблицеВТЛицевыеСчетаСотрудников(ОписательВременныхТаблиц, ТолькоРазрешенные, Организация, ДатаПолученияДанных = Неопределено, ЗарплатныйПроект = Неопределено) Экспорт
	...
	|					ИЛИ &ЗарплатныйПроект = ЗНАЧЕНИЕ(Справочник.ЗарплатныеПроекты.ПустаяСсылка))) КАК ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам
		...
		Если ЛСУказанПравильно Или (Не РезультатПроверки.ЗначениеЛСИзменено И ЗначениеЗаполнено(ЛицевыеСчета.Документ)) Тогда
			...
			Если Не РезультатПроверки.ЗначениеЛСИзменено И ЗначениеЗаполнено(ЛицевыеСчета.Документ) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Номер лицевого счета введен документом (см. %1)'"), ПредставлениеДокумента);
				ТекстНадписи = Новый ФорматированнаяСтрока(ТекстСообщения);
...
Процедура ГруппаПодтверждениеИзБанкаДополнитьФорму(Форма) Экспорт
	...
	ДокументПодтверждение = Неопределено;
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ДокументПодтверждение = Выборка.ДокументПодтверждение;
	КонецЕсли;
	...
	Если ТипЗнч(Форма.Параметры.Ключ) = Тип("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников") Тогда
		Типы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("ДокументСсылка.ПодтверждениеОткрытияЛицевыхСчетовСотрудников"));
	Иначе
		Типы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("ДокументСсылка.ПодтверждениеЗачисленияЗарплаты"));
	КонецЕсли;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ДокументПодтверждениеИзБанка", Новый ОписаниеТипов(Типы)));
	...
	Если ДокументПодтверждение <> Неопределено Тогда
		// Команда Открыть
		ИмяКоманды = "ОткрытьДокументПодтверждение";
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
			КомандаФормы.Заголовок = "Открыть подтверждение из банка";
		...
		КонецЕсли;
		Форма.ПодтверждениеИнфоНадпись = НСтр("ru = 'По документу получено подтверждение из банка и его редактирование невозможно'");
		Форма.ДокументПодтверждениеИзБанка = ДокументПодтверждение;
	Иначе
		// Команда Загрузить
		ИмяКоманды = "ЗагрузитьДокументПодтверждение";
		Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
			КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
			КомандаФормы.Заголовок = "Загрузить подтверждение из банка";
			КомандаФормы.Действие = "Подключаемый_" + ИмяКоманды;
		КонецЕсли;
		Если Элементы.Найти(ИмяКоманды) = Неопределено Тогда
			Элемент = Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), ГруппаПодтверждениеИзБанка);
			...
			Элемент.ИмяКоманды = ИмяКоманды;
		КонецЕсли;
		Форма.ПодтверждениеИнфоНадпись = НСтр("ru = 'По документу не получено подтверждение из банка'");
	КонецЕсли;
...
Процедура ДополнитьПлатежныйДокумент(ПлатежныйДокумент, МассивВедомостей) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ВнешниеХозяйственныеОперации.ВзаиморасчетыССотрудникамиВХО") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ВзаиморасчетыССотрудникамиВХО");
...
Процедура ПриЗаписиПлатежногоДокументаПеречисленияЗарплаты(Источник, Отказ) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ЗарегистрироватьСостояниеЗачисленияЗарплатыПоДокументу(Источник.Ссылка, Отказ);
	КонецЕсли;
...
Функция ДанныеСпискаПеречисленийПоXML(МассивОбъектов) Экспорт
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		МассивДокументов.Добавить(ОбъектПечати.СсылкаНаДокумент);
	КонецЦикла;
	
	ДанныеДляПечатиШапок = Документы.ВедомостьНаВыплатуЗарплатыВБанк.ВыборкаДляПечатиШапки(МассивДокументов);
	
	ДанныеДляПечатиСтрок = Новый ТаблицаЗначений;
	ДанныеДляПечатиСтрок.Колонки.Добавить("Ведомость");
	ДанныеДляПечатиСтрок.Колонки.Добавить("НомерЛицевогоСчета", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Фамилия", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Отчество", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Сумма", ОбщегоНазначения.ОписаниеТипаЧисло(18, 2));
	
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		
		СуммаПоДокументу = 0;
		Для каждого Сотрудник Из ОбъектПечати.ЭДПечати.ЗачислениеЗарплаты.Сотрудник Цикл
			
			СуммаПоДокументу = СуммаПоДокументу + Сотрудник.Сумма;
			
			НоваяСтрока = ДанныеДляПечатиСтрок.Добавить();
			НоваяСтрока.Ведомость = ОбъектПечати.СсылкаНаДокумент;
			НоваяСтрока.НомерЛицевогоСчета = Сотрудник.ЛицевойСчет;
			НоваяСтрока.Фамилия = Сотрудник.Фамилия;
			НоваяСтрока.Имя = Сотрудник.Имя;
			НоваяСтрока.Отчество = Сотрудник.Отчество;
			НоваяСтрока.Сумма = Сотрудник.Сумма;
			
		КонецЦикла;
		НайденнаяСтрока = ДанныеДляПечатиШапок.Найти(ОбъектПечати.СсылкаНаДокумент, "Ссылка");
		Если НайденнаяСтрока <> Неопределено Тогда
			НайденнаяСтрока.СуммаПоДокументу = СуммаПоДокументу;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат Новый Структура("ДанныеШапок, ДанныеСтрок", ДанныеДляПечатиШапок, ДанныеДляПечатиСтрок);
	
КонецФункции

Функция ДанныеСпискаНаОткрытиеЛицевыхСчетовПоXML(МассивОбъектов) Экспорт
	
	ДанныеДляПечатиШапок = Новый ТаблицаЗначений;
	ДанныеДляПечатиШапок.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников"));
	ДанныеДляПечатиШапок.Колонки.Добавить("ДатаДокумента", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ДанныеДляПечатиШапок.Колонки.Добавить("НомерДоговора", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиШапок.Колонки.Добавить("ПолноеНаименованиеОрганизации", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиШапок.Колонки.Добавить("НомерРасчетногоСчетаОрганизации", Новый ОписаниеТипов("Строка"));
	
	ДанныеДляПечатиСтрок = Новый ТаблицаЗначений;
	ДанныеДляПечатиСтрок.Колонки.Добавить("Документ", Новый ОписаниеТипов("ДокументСсылка.ЗаявкаНаОткрытиеЛицевыхСчетовСотрудников"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Фамилия", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Отчество", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ЭмбоссированныйТекст1", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ЭмбоссированныйТекст2", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ЭмбоссированныйТекст3", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ДатаРождения", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ДанныеДляПечатиСтрок.Колонки.Добавить("Пол", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ОтделениеБанка", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ФилиалОтделенияБанка", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("АдресПоПропискеПредставление", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("АдресМестаПроживанияПредставление", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("АдресМестаРаботыПредставление", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ТелефонРабочийПредставление", Новый ОписаниеТипов("Строка"));
	ДанныеДляПечатиСтрок.Колонки.Добавить("ДокументПредставление", Новый ОписаниеТипов("Строка"));
	
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		
		НоваяСтрокаШапки = ДанныеДляПечатиШапок.Добавить();
		НоваяСтрокаШапки.Документ = ОбъектПечати.СсылкаНаДокумент;
		НоваяСтрокаШапки.ДатаДокумента = ОбъектПечати.ЭДПечати.ДатаФормирования;
		НоваяСтрокаШапки.НомерДоговора = ОбъектПечати.ЭДПечати.НомерДоговора;
		НоваяСтрокаШапки.ПолноеНаименованиеОрганизации = ОбъектПечати.ЭДПечати.НаименованиеОрганизации;
		НоваяСтрокаШапки.НомерРасчетногоСчетаОрганизации = ОбъектПечати.ЭДПечати.РасчетныйСчетОрганизации;
		
		Для каждого Сотрудник Из ОбъектПечати.ЭДПечати.ОткрытиеСчетов.Сотрудник Цикл
			
			НоваяСтрока = ДанныеДляПечатиСтрок.Добавить();
			НоваяСтрока.Документ = ОбъектПечати.СсылкаНаДокумент;
			НоваяСтрока.Фамилия = Сотрудник.Фамилия;
			НоваяСтрока.Имя = Сотрудник.Имя;
			НоваяСтрока.Отчество = Сотрудник.Отчество;
			НоваяСтрока.ЭмбоссированныйТекст1 = Сотрудник.ЭмбоссированныйТекст.Поле1;
			НоваяСтрока.ЭмбоссированныйТекст2 = Сотрудник.ЭмбоссированныйТекст.Поле2;
			НоваяСтрока.ЭмбоссированныйТекст3 = Сотрудник.ЭмбоссированныйТекст.Поле3;
			НоваяСтрока.ДатаРождения = Сотрудник.ДатаРождения;
			НоваяСтрока.Пол = Сотрудник.Пол;
			НоваяСтрока.ОтделениеБанка = Сотрудник.ОтделениеБанка;
			НоваяСтрока.ФилиалОтделенияБанка = Сотрудник.ФилиалОтделенияБанка;
			
			НоваяСтрока.АдресПоПропискеПредставление = ПредставлениеАдресаИзЭДXML(
				Сотрудник.АдресПрописки, Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица);
			НоваяСтрока.АдресМестаПроживанияПредставление = ПредставлениеАдресаИзЭДXML(
				Сотрудник.АдресПроживания, Справочники.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица);
			НоваяСтрока.АдресМестаРаботыПредставление = ПредставлениеАдресаИзЭДXML(
				Сотрудник.АдресМестаРаботы, Справочники.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица);
			
			НоваяСтрока.ТелефонРабочийПредставление = Сотрудник.РабочийТелефон;
			НоваяСтрока.ДокументПредставление = ПредставлениеУдостоверенияЛичностиИзЭДXML(Сотрудник.УдостоверениеЛичности);
		КонецЦикла;
...
Процедура ЗаписатьОшибкиВЖурнал(СтруктураОшибок) Экспорт
	
	Для каждого СтруктураОшибки Из СтруктураОшибок Цикл
		ОписаниеОшибки = СтруктураОшибки.ОписаниеОшибки;
		ЗаписьЖурналаРегистрации(НСтр(СтруктураОшибки.ТекстСообщения, Метаданные.ОсновнойЯзык.КодЯзыка),
...
Функция СообщениеПроверкиНомераЛицевогоСчетаНаСоответствиеТребованиям(ФизическоеЛицо, ЗарплатныйПроект, Знач НомерЛицевогоСчета) Экспорт
	...
	Если СтрДлина(НомерЛицевогоСчета) <> 20 Тогда
	...
	УсловныйНомер = Прав(Выборка.БИК, 3);
	Если УсловныйНомер = "000" Или УсловныйНомер = "001" Или УсловныйНомер = "002" Тогда
		// Это БИК РКЦ
		УсловныйНомер = "0" + Сред(Выборка.БИК, 5, 2);
	КонецЕсли;
	ИспользованиеКлиринговойВалюты = Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(НомерЛицевогоСчета, 6, 1));
	Если ИспользованиеКлиринговойВалюты Тогда
		НомерЛицевогоСчета = Лев(НомерЛицевогоСчета,5) + СоответствиеСимволов.Получить(Сред(НомерЛицевогоСчета, 6, 1))
				+ Сред(НомерЛицевогоСчета, 7);
	КонецЕсли;
...
Процедура ВедомостьНаВыплатуЗарплатыВБанкПриЗаписи(ВедомостьОбъект, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				ЗарегистрироватьСостояниеЗачисленияЗарплатыПоДокументу(Выборка.ПлатежныйДокумент, Отказ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьНомераРеестровДокументовЗачисленияЗарплаты() Экспорт 
	...
	|		ПО СостоянияДокументовЗачисленияЗарплаты.Документ = ПрисоединенныеФайлы.ВладелецФайла
...
Процедура ВосстановитьРеквизитыПлатежныхДокументов() Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("ПлатежныйДокумент") Цикл
		...
		КонецЦикла;
		СтрокаНабора.СпособВыплатыПредставление = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ПредставленияСпособовВыплаты);
		СтрокаНабора.МесяцВыплатыПредставление = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ПредставленияМесяцевВыплаты);
		НаборЗаписей.ОбменДанными.Загрузка = Истина;
		НаборЗаписей.Записать();
	КонецЦикла;
...
Процедура ВосстановитьСостоянияДокументовЗачисленияЗарплаты() Экспорт 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьСостоянияДокументовЗачисленияЗарплаты.Документ,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияДокументовЗачисленияЗарплаты КАК СостоянияДокументовЗачисленияЗарплаты
	|		ПО УдалитьСостоянияДокументовЗачисленияЗарплаты.Документ = СостоянияДокументовЗачисленияЗарплаты.Документ
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(УдалитьСостоянияДокументовЗачисленияЗарплаты.Документ) <> ТИП(Справочник.УдалитьКомплектыВедомостейНаВыплатуЗарплатыЧерезБанк)
	|	И СостоянияДокументовЗачисленияЗарплаты.Документ ЕСТЬ NULL ";
...
Процедура СоздатьФайлыДляОбменаСБанком(СтруктураПараметровДляФормированияФайла) Экспорт
	...
	Если СтруктураПараметровДляФормированияФайла.Свойство("ПлатежныйДокумент") И ЗначениеЗаполнено(СтруктураПараметровДляФормированияФайла.ПлатежныйДокумент) Тогда
		...
		ПрисоединитьФайлОбменаСБанкамиКОбъекту(СтруктураПараметровДляФормированияФайла.ПлатежныйДокумент, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла);
	Иначе
		Для каждого ВыгружаемыйДокумент Из СтруктураПараметровДляФормированияФайла.МассивДокументов Цикл
			...
			Для каждого ДанныеШапкиДокумента Из ДанныеШапки Цикл
				...
				ПрисоединитьФайлОбменаСБанкамиКОбъекту(ВыгружаемыйДокумент, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПрисоединитьФайлОбменаСБанкамиКОбъекту(ВладелецФайла, ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла) Экспорт
	...
	ПрисоединенныйФайл = ПрисоединенныеФайлы.ДобавитьФайл(
			ВладелецФайла, Лев(ДанныеШапкиДокумента.ИмяФайла, СтрДлина(ДанныеШапкиДокумента.ИмяФайла) - 4), Прав(ДанныеШапкиДокумента.ИмяФайла, 3),,, ФайлОбменаСБанками.АдресФайла);
...
Функция ФайлОбменаСБанкамиПоЗарплатнымПроектам(ДанныеШапкиДокумента, ДанныеСтрокДокумента, СтруктураПараметровДляФормированияФайла, Ошибки)
	...
	КодировкаФайла = Строка(ДанныеШапкиДокумента.ЗарплатныйПроект.КодировкаФайла);
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку(КодировкаФайла);
	ЗаписьXML.ЗаписатьОбъявлениеXML();
...
//
// Параметры:
//		МассивИменФайлов - массив полных имен файлов.
//
// Возвращаемое значение:
//		МассивСозданныхДокументов - массив созданных документов.
//
Функция ЗагрузитьПодтвержденияИзБанка(МассивИменФайлов, ДокументОбъектПодтверждение = Неопределено) Экспорт
	
	СозданныеДокументы = Новый Соответствие;
	
	Для каждого ФайлПодтверждения Из МассивИменФайлов Цикл
		
		ИмяВременногоФайла  = ПолучитьИмяВременногоФайла("xml");
		ДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(ФайлПодтверждения.Хранение);
		ДвоичныеДанныеФайла.Записать(ИмяВременногоФайла);
		
		ДокументПодтверждения = ЗагрузитьПодтверждениеИзБанка(ИмяВременногоФайла, ДокументОбъектПодтверждение);
		СозданныеДокументы.Вставить(ДокументПодтверждения, ?(ДокументПодтверждения = Неопределено, Неопределено, ТипЗнч(ДокументПодтверждения.ПервичныйДокумент)));
		Если ДокументПодтверждения <> Неопределено Тогда
			ИмяФайла = ФайлПодтверждения.Имя;
			Поз = Найти(ИмяФайла, "\");
			Пока Поз > 0 Цикл
			...
			УстановитьПривилегированныйРежим(Истина);
			ПрисоединенныйФайл = ПрисоединенныеФайлы.ДобавитьФайл(
					ДокументПодтверждения, Лев(ИмяФайла, СтрДлина(ИмяФайла) - 4), Прав(ИмяФайла, 3),,, ФайлПодтверждения.Хранение);
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
	КонецЦикла;
...
//
Функция ЗагрузитьПодтверждениеИзБанка(ПолноеИмяФайла, ДокументОбъектПодтверждение = Неопределено)
	...
	Если ОбъектXDTO.Получить("РезультатЗачисленияЗарплаты") <> Неопределено
		...
		Возврат СоздатьДокументПодтверждения(ОбъектXDTO, Метаданные.Документы.ПодтверждениеЗачисленияЗарплаты, ДокументОбъектПодтверждение);
	ИначеЕсли ОбъектXDTO.Получить("РезультатОткрытияСчетов") <> Неопределено
		...
		Возврат СоздатьДокументПодтверждения(ОбъектXDTO, Метаданные.Документы.ПодтверждениеОткрытияЛицевыхСчетовСотрудников, ДокументОбъектПодтверждение);
	Иначе
...
//
Функция СтруктураДляЗаполненияДокументаПоПодтверждениюБанка(ВидПодтверждения, ОбъектXDTO, ПервичныйДокумент, Отказ) Экспорт
...
//
Функция СоздатьДокументПодтверждения(ОбъектXDTO, МетаданныеПодтверждения, ДокументОбъектПодтверждение = Неопределено) Экспорт
	...
	ДокументОбъект.ЗаполнитьДокументИзОбъектаXDTO(ОбъектXDTO, СсылкаНаПервичныйДокумент, Отказ);
...
Функция СостояниеОткрытияЛицевыхСчетов(ПодтверждениеОткрытияЛицевыхСчетов) Экспорт
	...
	Запрос.УстановитьПараметр("ПодтверждениеОткрытияЛицевыхСчетов", ПодтверждениеОткрытияЛицевыхСчетов);
	Запрос.УстановитьПараметр("ПервичныйДокумент", ПодтверждениеОткрытияЛицевыхСчетов.ПервичныйДокумент);
	...
	|ГДЕ
	|	ПодтверждениеОткрытияЛицевыхСчетовСотрудников.Ссылка = &ПодтверждениеОткрытияЛицевыхСчетов
	|	И ПодтверждениеОткрытияЛицевыхСчетовСотрудников.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ПодтверждениеОткрытияЛицевыхСчетовСотрудников.ПервичныйДокумент
...
Функция СостояниеЗачисленияЗарплатыДляПодтверждения(ПодтверждениеЗачисленияЗарплаты) Экспорт
	...
	Запрос.УстановитьПараметр("ПодтверждениеЗачисленияЗарплаты", ПодтверждениеЗачисленияЗарплаты);
	Запрос.УстановитьПараметр("ПервичныйДокумент", ПодтверждениеЗачисленияЗарплаты.ПервичныйДокумент);
	...
	|ГДЕ
	|	ПодтверждениеЗачисленияЗарплаты.Ссылка = &ПодтверждениеЗачисленияЗарплаты
	|	И ПодтверждениеЗачисленияЗарплаты.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ПодтверждениеЗачисленияЗарплаты.ПервичныйДокумент
	...
	ИмяОбъекта = ПодтверждениеЗачисленияЗарплаты.ПервичныйДокумент.Метаданные().ПолноеИмя();
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ДокументЗачисленияЗарплаты", ИмяОбъекта);
...
//
Функция ДанныеСотрудниковДляОткрытияЛицевыхСчетов(МенеджерВременныхТаблиц, ДатаПолученияДанных, Организация, ЗарплатныйПроект, СписокФизическихЛиц = Неопределено, ЗаявкаНаОткрытиеЛицевыхСчетов = Неопределено) Экспорт
	...
	КадровыеДанные = "Фамилия, Имя, Отчество, АдресДляИнформирования, АдресДляИнформированияПредставление,"
		...
		+ "СтатусНалогоплательщика, Пол, ДокументПредставление,"
	...
	ДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, СписокСотрудников, КадровыеДанные, ДатаПолученияДанных);
	...
	ОткрытиеЛицевыхСчетов.Сортировать("ФизическоеЛицо");
...
Процедура СоздатьВТЗакрытиеЛицевыхСчетов(МенеджерВременныхТаблиц, Ссылка, Организация, ЗарплатныйПроект, Подразделение, ДатаПолученияДанных, Сотрудник = Неопределено, ТолькоРазрешенные = Истина) Экспорт
	...
	ОписательВременныхТаблиц = ОписательВременныхТаблицДляСоздатьПоВременнойТаблицеВТЛицевыеСчетаСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТСотрудники");
	СоздатьПоВременнойТаблицеВТЛицевыеСчетаСотрудников(ОписательВременныхТаблиц, ТолькоРазрешенные, Организация, ДатаПолученияДанных, ЗарплатныйПроект);
