...
Функция ОбработкаКомандыСогласиеНаОбработкуПерсональныхДанных(ПараметрыПечати) Экспорт
	
	ОбъектыПечати = ПараметрыПечати.ОбъектыПечати;
	
	ПечатьПоСотрудникам = Ложь;
	Если ТипЗнч(ОбъектыПечати) = Тип("Массив") И ОбъектыПечати.Количество() > 0 Тогда
		ПечатьПоСотрудникам = (ТипЗнч(ОбъектыПечати[0]) = Тип("СправочникСсылка.Сотрудники"));
	Иначе
		ПечатьПоСотрудникам = (ТипЗнч(ОбъектыПечати) = Тип("СправочникСсылка.Сотрудники"));
	КонецЕсли;
	
	Если ПечатьПоСотрудникам Тогда
		ФизическиеЛицаСотрудников = СотрудникиВызовСервера.ФизическиеЛицаСотрудников(ОбъектыПечати);
		ОткрытьФорму("Обработка.СогласиеНаОбработкуПерсональныхДанных.Форма", Новый Структура("Субъекты", ФизическиеЛицаСотрудников), ПараметрыПечати.Форма);
	Иначе
		ОткрытьФорму("Обработка.СогласиеНаОбработкуПерсональныхДанных.Форма", Новый Структура("Субъекты", ОбъектыПечати), ПараметрыПечати.Форма);
	КонецЕсли; 
	...
	Если НЕ СтруктураПроверкиОднофамильцев.ФизическоеЛицоУникально Тогда
		...
		Иначе
			...
			Если СтруктураПроверкиОднофамильцев.ВозможнаПроверкаПоИНН
				...
				ОткрытьФорму("Справочник.ФизическиеЛица.Форма.ПроверкаУникальностиПоИННиСНИЛС", ПараметрыОткрытия, , , , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
