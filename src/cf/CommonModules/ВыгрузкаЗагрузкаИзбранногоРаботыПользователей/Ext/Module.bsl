...
Функция СтруктураНавигационнойСсылки(Знач НавигационнаяСсылка)
	...
	Результат = Новый Структура("Протокол, Тип, Путь, Параметры", "", "", "", "");
	
	ПодстрокиСсылки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НавигационнаяСсылка, "/");
...
Функция СсылкиВПараметрахНавигационнойСсылки(Знач СтрокаПараметров, Знач ОбъектМетаданных)
	...
	Подстроки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаПараметров, "&");
	
	Для Каждого Подстрока Из Подстроки Цикл
		
		ПозицияРазделителя = Найти(Подстрока, "=");
		...
		ИначеЕсли ОбщегоНазначенияБТС.ЭтоНаборЗаписей(ОбъектМетаданных) Тогда
			...
			ИначеЕсли КоличествоВозможныхТипов > 1 Тогда
				...
				ПозицияРазделителяТипа = Найти(ЗначениеПоля, ":");
