...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь) Экспорт
	...
	Если ТребуетсяСоздатьАдминистратора Тогда
		...
		Если РольДоступна(Метаданные.Роли.ПолныеПрава) // Не заменять на РолиДоступны.
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Параметры.Вставить("ЭтоУчебнаяПлатформа", СтандартныеПодсистемыСервер.ЭтоУчебнаяПлатформа());
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина)
		Или МодульТекущиеДелаСлужебный.ДелоОтключено("НекорректныеСведенияОПользователях") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Форма          = "Справочник.Пользователи.Форма.ПользователиИБ";
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ОписаниеПользователяИБ.Свойство("ПолноеИмя")
	 Или СоздатьНовогоПользователяИБ Тогда
