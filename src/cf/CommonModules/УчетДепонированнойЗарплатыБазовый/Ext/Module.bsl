...
Процедура ДепонированиеЗарплатыОбработкаПроведения(ДокументОбъект, Отказ) Экспорт
	...
	Если НЕ Отказ Тогда
		УчетНДФЛ.РассчитатьИЗарегистрироватьУдержанныеНалоги(ДокументОбъект.Ссылка, ДокументОбъект.Движения, Отказ, ДокументОбъект.Организация, ДокументОбъект.Дата, ДанныеДляПроведения.МенеджерТаблиц);
	КонецЕсли;
...
/// Методы доступа к документу депонирования.
 ...
 Функция ДепонированиеЗарплатыДанныеДляПроведения(ДокументОбъект) Экспорт
	...
	ТаблицаВыплат = ВзаиморасчетыССотрудниками.ДанныеВедомостейДляОплатыДокументом(ДокументОбъект.Ссылка, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ведомость), Физлица, Неопределено);
	
	// Данные для учета НДФЛ (менеджер временных таблиц с временной таблицей для передачи).
	...
	// Данные для депонирования
	
	ТаблицаДепонентов = УчетДепонированнойЗарплаты.НоваяТаблицаДепонированнойЗарплаты();
	Для Каждого СтрокаТаблицыВыплат Из ТаблицаВыплат Цикл
		СтрокаТаблицыДепонентов = ТаблицаДепонентов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыДепонентов, СтрокаТаблицыВыплат);
		СтрокаТаблицыДепонентов.Сумма = СтрокаТаблицыВыплат.СуммаКВыплате;
	КонецЦикла;	
	...
	ТаблицаДепонентов.ЗаполнитьЗначения(Ведомость, "Ведомость");
	
	// Данные для оплаты
	
	КолонкиТаблицыОплаты = "Ведомость, ФизическоеЛицо";
	ТаблицаОплат = ТаблицаВыплат.Скопировать(, КолонкиТаблицыОплаты);
	ТаблицаОплат.Свернуть(КолонкиТаблицыОплаты);
	
	ДанныеДляПроведения = Новый Структура;
	ДанныеДляПроведения.Вставить("МенеджерТаблиц",	МенеджерТаблиц);
