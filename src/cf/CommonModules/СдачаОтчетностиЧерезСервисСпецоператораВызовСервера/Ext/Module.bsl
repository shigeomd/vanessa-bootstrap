...
////////////////////////////////////////////////////////////////////////////////
// ЭКСПОРТНЫЕ МЕТОДЫ, СВЯЗАННЫЕ С РЕГЛАМЕНТНЫМИ ЗАДАНИЯМИ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ОТ ОБЪЕКТОВ МЕТАДАННЫХ
...
Процедура ОбновитьЗаписиВРегистреОтправкиОтчетностиКОбработке()
	...
	//// отражаем изменения в регистре ОтправкиОтчетностиКОбработке
	//
	//// находим последнее по дате действующее событие отправки
	//СтатусПоследнегоСобытия = Неопределено;
	//ПричиныОтказаИзПоследнегоСобытия = Неопределено;
	//ПоследнееСобытиеОтправки = ПолучитьПоследнееПроаудированноеСобытиеОтправки(Объект, СтатусПоследнегоСобытия, ПричиныОтказаИзПоследнегоСобытия);
	//
	//// в этом случае регистр очищается от записей, содержащих uid отправки
	//Если (СтатусПоследнегоСобытия = Перечисления.СтатусыОтправкиОтчетности.НеПринята И НЕ (Объект.ТребуетАудита И НЕ Объект.АудитПроведен))
	//ИЛИ СтатусПоследнегоСобытия = Перечисления.СтатусыОтправкиОтчетности.Сдана Тогда
	//	УдалитьЗаписьВРегистреСведенийОбОтчетностиКОбработке(Объект.Идентификатор);
	//	
	//// в этом случае в регистр добаляется запись, содержащая значение разделителя без uid
	//ИначеЕсли СтатусПоследнегоСобытия = Перечисления.СтатусыОтправкиОтчетности.Запланирована
	//ИЛИ СтатусПоследнегоСобытия = Перечисления.СтатусыОтправкиОтчетности.ПустаяСсылка() Тогда
	//	ДобавитьЗаписьВРегистрСведенийОтчетностиКОбработке(Объект);
	//	
	//// в остальных случаях в регистр добавляется запись с uid и
	//// анализируется необходимость очистки записи без uid
	//// (которая ранее, возможно, была порождена той же отправкой в статусе Запланирована)
	//Иначе
	//	ДобавитьЗаписьСUIDВРегистрСведенийОтчетностиКОбработкеИУдалитьБезUIDПриНеобходимости(Объект);
	//КонецЕсли;
	//
	//
	//// отражаем изменения в регистре статусов отправки
...
////////////////////////////////////////////////////////////////////////////////
// ВСПОМОГАТЕЛЬНЫЕ МЕТОДЫ РЕГЛАМЕНТНЫХ ЗАДАНИЙ
...
Функция ОтправитьГотовыйКОтправкеПакет(ОтчетностьКОтправке, Соединение, ОписаниеОшибки = "")
	...
	КаталогПакета = ПолучитьИмяВременногоФайла() + "\";
	...
	Исключение
		УдалитьВременныйФайл(КаталогПакета);
	...
	Попытка
		...
		УдалитьВременныйФайл(ПолноеИмяФайлаПакета);
	Исключение
		УдалитьВременныйФайл(КаталогПакета);
...
Функция ИмяПользователяИПарольУдаленногоДоступаКВебСервисамМС()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ИмяСлужебногоПользователяМенеджераСервиса.Значение КАК ИмяСлужебногоПользователяМенеджераСервиса,
	                      |	ПарольСлужебногоПользователяМенеджераСервиса.Значение КАК ПарольСлужебногоПользователяМенеджераСервиса
	                      |ИЗ
	                      |	Константа.ИмяСлужебногоПользователяМенеджераСервиса КАК ИмяСлужебногоПользователяМенеджераСервиса,
	                      |	Константа.ПарольСлужебногоПользователяМенеджераСервиса КАК ПарольСлужебногоПользователяМенеджераСервиса");
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Результат = Новый Структура("Имя, Пароль", Выборка.ИмяСлужебногоПользователяМенеджераСервиса, Выборка.ПарольСлужебногоПользователяМенеджераСервиса);
	Иначе
		Результат = Новый Структура("Имя, Пароль");
	КонецЕсли;
...
Функция ИзменитьРасширениеФайла(Знач КороткоеИмяФайлаВыгрузки, НовоеРасширение)
	...
	ВхождениеТочки = Найти(КороткоеИмяФайлаВыгрузки, ".");
...
Функция СформироватьФайлПакета(Отчет, КонтролирующийОрган, ТекстВыгрузки, КороткоеИмяФайлаВыгрузки, КодировкаТекстаВыгрузки = Неопределено, ДанныеФайлаДоверенности = Неопределено, КороткоеИмяФайлаДоверенности = Неопределено, ДанныеФайлаПакета, КороткоеИмяФайлаПакета) Экспорт
...
Функция ВыгрузитьСведенияОДоверенностиИСформироватьФайлПакета(Отчет, КонтролирующийОрган, ТекстВыгрузки, КороткоеИмяФайлаВыгрузки, КодировкаТекстаВыгрузки, ДвоичныеДанныеФайлаПакета, КороткоеИмяФайлаПакета) Экспорт
	...
	Возврат СформироватьФайлПакета(Отчет, КонтролирующийОрган, ТекстВыгрузки, КороткоеИмяФайлаВыгрузки, КодировкаТекстаВыгрузки, ДанныеФайлаДоверенности, КороткоеИмяФайлаДоверенности, ДвоичныеДанныеФайлаПакета, КороткоеИмяФайлаПакета);
...
Функция ПолучитьСведенияДоверенности(РегистрацияВИФНС, Организация = Неопределено)
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Организация) Тогда
		...
		СведенияОПредставляемомЛице.Вставить("ФИО", РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СведенияОргФЛ.ФИО));
	...
	Иначе
		...
		СвРукОргЮЛ = Новый Структура("ИННФЛ, ФИО", СокрЛП(СведенияОргЮЛ.ИННРук), РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СведенияОргЮЛ.ФИОРук));
...
Функция ПроверитьВозможностьВыгрузкиДоверенности(СведенияДоверенности)
	...
	Если ОшибкиПроверки.Количество() > 0 Тогда
		//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось выгрузить сведения о доверенности, выданной представителю:'"), СведенияДоверенности.Доверенность);
...
Функция ПолучитьСтрокуСостоянияОтправкиОтчетаИДополнительныеСвойства(Отчет, КонтролирующийОрган = "ФНС", ЦветТекста, ПоказыватьКнопкуОткрытияПротокола, ДатаОтправки) Экспорт
...
Функция ЭтоОтчетСтатистики(Отчет)
	...
	Если Найти(ВРЕГ(ВидОтчета.Родитель.Наименование), ВРЕГ("Статистика")) Тогда
		Возврат Истина;
	КонецЕсли; 
