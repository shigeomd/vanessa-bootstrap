...
Функция ФиксированныйОтборДинамическогоСписка(Форма, Список) Экспорт
	...
	Если Форма.Параметры.Свойство("Отбор") Тогда
		Для Каждого ЭлементОтбора Из Форма.Параметры.Отбор Цикл
			КоллекцияОтборов.Вставить(Новый ПолеКомпоновкиДанных(ЭлементОтбора.Ключ), 
				Новый Структура("ЛевоеЗначение, ВидСравнения, Использование, ПравоеЗначение, РежимОтображения, Представление",
					Новый ПолеКомпоновкиДанных(ЭлементОтбора.Ключ), ВидСравненияКомпоновкиДанных.Равно, Истина, ЭлементОтбора.Значение,
					РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, ПредставлениеЭлементаОтбораДинамическогоСписка(Форма, ЭлементОтбора.Ключ)));
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого ЭлементОтбора Из Список.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы Цикл
		Если ЭлементОтбора.Использование
			...
			Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				КоллекцияОтборов.Вставить(ЭлементОтбора.Представление, 
					Новый Структура("ЛевоеЗначение, ВидСравнения, Использование, ПравоеЗначение, РежимОтображения, Представление, ЭтоГруппа",
						ЭлементОтбора.Представление, ВидСравненияКомпоновкиДанных.Равно, ЭлементОтбора.Использование, Истина,
						РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, "", Истина));
			Иначе
				КоллекцияОтборов.Вставить(ЭлементОтбора.ЛевоеЗначение, 
					Новый Структура("ЛевоеЗначение, ВидСравнения, Использование, ПравоеЗначение, РежимОтображения, Представление",
						ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ВидСравнения, ЭлементОтбора.Использование, ЭлементОтбора.ПравоеЗначение,
						РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный, ЭлементОтбора.Представление));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Если ЭлементОтбора.Использование
			...
			Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				КоллекцияОтборов.Вставить(ЭлементОтбора.Представление, 
					Новый Структура("ЛевоеЗначение, ВидСравнения, Использование, ПравоеЗначение, РежимОтображения, Представление, ЭтоГруппа",
						ЭлементОтбора.Представление, ВидСравненияКомпоновкиДанных.Равно, ЭлементОтбора.Использование, Истина,
						ЭлементОтбора.РежимОтображения, "", Истина));
			Иначе
...
Процедура ДополнитьНадписьНастроекОтбораДинамическогоСписка(НаименованиеЭлементаОтбора, СписокНастройкиОтбораНадпись, ЭлементОтбора)
	...
	ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
		Если ТипЗнч(ПравоеЗначениеОтбора) = Тип("ФиксированныйМассив") Или ТипЗнч(ПравоеЗначениеОтбора) = Тип("Массив") Тогда
			ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в списке:'"), ТекстЭлементаОтбора);
			Для каждого ЭлементСписка Из ПравоеЗначениеОтбора Цикл
				ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2,'"), ТекстЭлементаОтбора, ЭлементСписка);
			КонецЦикла;
			СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстЭлементаОтбора, 1);
		Иначе
	...
	ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в списке по иерархии:'"), ТекстЭлементаОтбора);
		Для каждого ЭлементСписка Из ПравоеЗначениеОтбора Цикл
			ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2,'"), ТекстЭлементаОтбора, ЭлементСписка);
		КонецЦикла;
		СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстЭлементаОтбора, 1);
	...
	ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке Тогда
		ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не в списке:'"), ТекстЭлементаОтбора);
		Для каждого ЭлементСписка Из ПравоеЗначениеОтбора Цикл
			ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2,'"), ТекстЭлементаОтбора, ЭлементСписка);
		КонецЦикла;
		СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстЭлементаОтбора, 1);
	ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
		ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не в списке по иерархии:'"), ТекстЭлементаОтбора);
		Для каждого ЭлементСписка Из ПравоеЗначениеОтбора Цикл
			ТекстЭлементаОтбора = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 %2,'"), ТекстЭлементаОтбора, ЭлементСписка);
		КонецЦикла;
		СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(ТекстЭлементаОтбора, 1);
