...
Процедура УстановитьПояснениеКНомеруЛицевогоСчета(Форма, ИмяПоляДляПодсказки, СтруктураПояснения = Неопределено) Экспорт
	...
	Иначе
		...
		СообщенияПроверки = Новый ФорматированнаяСтрока(СтруктураПояснения.СообщенияПроверки, , СтруктураПояснения.ЭлементЦветТекста);
	КонецЕсли;
	
	ЗарплатаКадрыКлиентСервер.УстановитьРасширеннуюПодсказкуЭлементуФормы(Форма, ИмяПоляДляПодсказки, СообщенияПроверки);
