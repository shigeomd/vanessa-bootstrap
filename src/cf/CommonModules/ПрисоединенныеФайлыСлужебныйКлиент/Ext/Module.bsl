...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Если Контекст.ПолноеИмяФайла <> Контекст.ПолноеИмяПомещаемогоФайла Тогда
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Создание каталога не выполнено по причине:'"), Контекст.КаталогФайла));
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Изменение свойства файла ""Только просмотр"" не выполнено по причине:'"), Контекст.ПолноеИмяФайла));
		ДействияСФайлом.Добавить(Действие);
		
		Действие = Новый Структура;
		Действие.Вставить("Действие", "Удалить");
		Действие.Вставить("Файл", Контекст.ПолноеИмяФайла);
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Удаление файла не выполнено по причине:'"), Контекст.ПолноеИмяФайла));
		ДействияСФайлом.Добавить(Действие);
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Копирование файла не выполнено по причине:'"), Контекст.ПолноеИмяФайла));
		ДействияСФайлом.Добавить(Действие);
	КонецЕсли;
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение свойств файла не выполнено по причине:'"), Контекст.ПолноеИмяФайла));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки +
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Установка свойств файла не выполнено по причине:'"), Контекст.ПолноеИмяФайла));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ДанныеФайла, ИдентификаторФормы, ОбработчикЗавершения, ОбработчикПриПолученииПодписи = Неопределено) Экспорт
...
Процедура ДобавитьПодписьИзФайла(ПрисоединенныйФайл, ИдентификаторФормы, ОбработчикЗавершения, ОбработчикПриПолученииПодписи = Неопределено) Экспорт
