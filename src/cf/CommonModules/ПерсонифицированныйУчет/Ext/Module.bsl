...
Процедура ЗаполнитьПечатныеФормыРазделовРСВ1(СсылкаНаСохрРеглОтчет, СтруктураРазделов, НумероватьСтраницы, НомерПервойстраницыРаздела25, НомерПоследнейСтраницыРаздела25, НомерПервойстраницыРаздела6, ОбщееКоличествоСтраницРаздела6) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(СсылкаНаСохрРеглОтчет) Тогда
		ДанныеРСВ = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(СсылкаНаСохрРеглОтчет, "Организация, ДатаПодписи");
...
Процедура ОпределитьСвойстваОбъектовДляОтображенииВФормеОтчетность(СвойстваОбъектов) Экспорт
	...
	Если ДокументыПерсУчета.Количество() > 0 Тогда
		...
		|	ВедомостьУплатыАДВ_11.Комментарий,
		|	""-"" КАК ВариантОтчета
		|ИЗ
...
Функция СведенияОбОрганизации(Организация, Период) Экспорт
	...
	|	СведенияОбОрганизациях.КодОКВЭД,
	|	СведенияОбОрганизациях.НаименованиеОКВЭД,
...
Процедура СоздатьВТСведенияОВзносахЗаработкеСтажеЗЛ2014(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ПараметрыПолученияДанных, ПараметрыОтбора) Экспорт												
	...
	СоздатьВТСостояниеУволенности(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод);
	...
	|	ЕСТЬNULL(СостояниеУволенности.СотрудникУволен, ЛОЖЬ) КАК СотрудникУволен,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСостояниеУволенности КАК СостояниеУволенности
	|		ПО ЗастрахованныеЛицаДляПачекСЗВ.ФизическоеЛицо = СостояниеУволенности.ФизическоеЛицо
...
Процедура СоздатьВТСостояниеУволенности(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод)
	...
	Если Год(ОтчетныйПериод) < 2015 Тогда
		...
		|ПОМЕСТИТЬ ВТСостояниеУволенности
		|ИЗ
...
Процедура СоздатьВТСведенияОЗаработкеИзДанныхУчета2014(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ПараметрыПолученияЗаработка, ПараметрыОтбора, ПостфиксВТРезульт = "")
	...
	СведенияОЗаработкеПоКлассамУсловийТруда = Запрос.Выполнить().Выгрузить();
	
	Запрос.УстановитьПараметр("СведенияОЗаработкеПоКлассамУсловийТруда", СведенияОЗаработкеПоКлассамУсловийТруда);
...
Функция ТерриториальныеУсловияПФР(СтруктурнаяЕдиница, ДатаСведений = Неопределено) Экспорт
	...
	ОписаниеТипаТерриторииВыполненияРабот = Метаданные.ОпределяемыеТипы.ТерриторииВыполненияРабот.Тип;
	
	ИзмеренияДаты.Колонки.Добавить("СтруктурнаяЕдиница", ОписаниеТипаТерриторииВыполненияРабот);
	ИзмеренияДаты.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
...
Функция КатегорииЗастрахованныхЛицОрганизации(Организация, ОтчетныйПериод, ОкончаниеОтчетногоПериода = Неопределено, КорректирующиеСведения = Ложь) Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СведенияОДоходахСтраховыеВзносыОбороты.СуммаОборот, 0) <> 0
	|			ТОГДА &ТарифЕНВД
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ
	|ИЗ
	...
	|			,
	|			Организация = &Организация
	|				И ОблагаетсяЕНВД) КАК СведенияОДоходахСтраховыеВзносыОбороты
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|			,
	|			Организация = &Организация
	|				И ЯвляетсяДоходомФармацевта) КАК СведенияОДоходахСтраховыеВзносыОбороты
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|			,
	|			Организация = &Организация
	|				И ЯвляетсяДоходомЧленаЭкипажаСуднаПодФлагомРФ) КАК СведенияОДоходахСтраховыеВзносыОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
//  МенеджерВременныхТаблиц - менеджер временных таблиц, в который будет помещенна ВТ "ВТДанныеОДопВзносах"
//
Процедура СформироватьВТДанныеОДопВзносах(ПериодРегистрации, ИмяВременнойТаблицы, МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.УстановитьПараметр("ОкончаниеПериода", ПерсонифицированныйУчетКлиентСервер.ОкончаниеОтчетногоПериодаПерсУчета(ПериодРегистрации));
...
Функция СобратьСтрокуПенсииЗаВыслугуЛет(ЗаписьСтажа) Экспорт 
	...
	Если 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ИСПКЛС1.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.САМОЛЕТ.Код))	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.СПЕЦАВ.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЛЕТИСП.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ИТСИСП.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ИНСПЕКТ.Код)) 	или
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ОПЫТИСП.Код))	Тогда
		Возврат ?(ПервыйПараметр>0,""+ПервыйПараметр+" ч ","")+?(ВторойПараметр>0,""+ВторойПараметр+" мин","");
	ИначеЕсли 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.РЕАКТИВН.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.НОРМАПР.Код)) 	Тогда
		Возврат ?(ПервыйПараметр>0,""+ПервыйПараметр+" прыжков ","");
	ИначеЕсли 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.НОРМСП.Код)) 	Тогда
		Возврат ?(ПервыйПараметр>0,""+ПервыйПараметр+" сп/под ","");
	ИначеЕсли 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.УВД.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ИТС.Код))	Тогда
		Возврат ?(ПервыйПараметр>0,""+ПервыйПараметр+" м ","")+?(ВторойПараметр>0,""+ВторойПараметр+" дн ","");
	ИначеЕсли 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ХИРУРГД.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ХИРУРСМ.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП81ГД.Код)) 	или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП81СМ.Код)) 	Тогда
		Возврат ?(ТретийПараметр>0,""+ТретийПараметр+" ставок ","");
	ИначеЕсли 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП80РК.Код)) или 
		(КодОснования	=	СокрЛП(Справочники.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ЗП80ПД.Код)) Тогда
...
Функция ФайлСведенийОВзносахИСтаже(ВыборкаПоШапкеДокумента, РезультатЗапросаПоЗаписямСтажа, КоличествоДокументов) Экспорт
	...
	УзелПФР = ЗарплатаКадры.ДобавитьУзелВДеревоXML(ДеревоВыгрузки, "ФайлПФР", "", );
	...
	Если ЗначениеЗаполнено(ВыборкаПоСотрудникам.НомерОсновнойЗаписи) Тогда
		...
		Пока ВыборкаПоСотрудникам.СледующийПоЗначениюПоля("НомерОсновнойЗаписи")	Цикл	
			...
			Иначе
				...
				Если ВыборкаПоСотрудникам.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ДЕКРЕТ 
					Или ВыборкаПоСотрудникам.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ВРНЕТРУД 
					Или ВыборкаПоСотрудникам.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ДЛОТПУСК 
					Или ВыборкаПоСотрудникам.ТретийПараметрИсчисляемогоСтажа = Справочники.ПараметрыИсчисляемогоСтраховогоСтажа.ВАХТА Тогда
...
Процедура ДокументыСЗВПроверитьДополнительнойЗаписиСтажа(Ошибки, Ссылка, Запись, ОсновнаяЗапись, НомерСтрокиСотрудник, НомерСтроки, Отказ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОсновнаяЗапись.ОснованиеВыслугиЛет) Тогда
		Если Найти(",,27-СМ,27-ГД,27-СМХР,27-ГДХР,",","+КодВыслугиЛетЗаписи+",") = 0 Тогда
			...
			Пока Выборка.СледующийПоЗначениюПоля("НомерДополнительнойЗаписи") Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Для Н = 1 по 2 Цикл
						...
						ТекстПараметра = Строка(Н)+"("+ПсевдонимПараметра+")";
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.ДопустимыеСочетанияКодовГруппСтажа);
	...
	Обработчик.Опциональный = Ложь;
	Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ТерриториальныеУсловияПФР);
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОснованияДосрочногоНазначенияПенсии) Тогда
		...
		Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ОснованияДосрочногоНазначенияПенсии);
		Обработчик.Опциональный = Ложь;
		Обработчик.НачальноеЗаполнение = Истина;
	КонецЕсли;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК) Тогда
		...
		Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ВидыОбщественноПолезнойДеятельностиСЗВК);
		Обработчик.Опциональный = Ложь;
		Обработчик.НачальноеЗаполнение = Истина;
	КонецЕсли;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ПараметрыИсчисляемогоСтраховогоСтажа) Тогда
		...
		Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ПараметрыИсчисляемогоСтраховогоСтажа);
		Обработчик.Опциональный = Ложь;
		Обработчик.НачальноеЗаполнение = Истина; 
	КонецЕсли;
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОсобыеУсловияТрудаПФР) Тогда
		...
		Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.Справочники.ОсобыеУсловияТрудаПФР);
		Обработчик.Опциональный = Ложь;
		Обработчик.НачальноеЗаполнение = Истина;
	КонецЕсли;
...
//
Функция ПолучитьДанныеДокументаВФорматеПФР(ДокументВид, ДокументСерия, ДокументНомер, ДокументДатаВыдачи, ДокументКемВыдан, ДокументКодПФР) Экспорт 
	...
	ДанныеДокумента.ТипУдостоверяющегоДокумента = ДокументКодПФР;
	ДанныеДокумента.НаименованиеУдостоверяющегоДокумента = ДокументКодПФР;
	...
	ДанныеДокумента.КемВыдан = СокрЛП(ВРег(ДокументКемВыдан));
...
Процедура ЗаполнитьАдрес(НаборЗаписейАдрес, СтрокаАдрес, СписокСтран, ФорматНеструктурированныйАдрес, ФорматИностранныйАдрес, ЗаменятьБуквуЕ = Ложь) Экспорт
	АдресСписком = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(
	...
	Иначе
		...
		НаборЗаписейРоссийскийАдрес.Регион.Значение.ГеографическоеНазвание = ПолучитьИмяИАдресноеСокращение(АдресСписком.Регион, НаборЗаписейРоссийскийАдрес.Регион.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Регион.Значение.ГеографическоеНазвание, "Регион");
		КонецЕсли;	
		
		НаборЗаписейРоссийскийАдрес.Район.Значение.ГеографическоеНазвание = ПолучитьИмяИАдресноеСокращение(АдресСписком.Район, НаборЗаписейРоссийскийАдрес.Район.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Район.Значение.ГеографическоеНазвание, "Район");
		КонецЕсли;
		
		НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание = ПолучитьИмяИАдресноеСокращение(АдресСписком.Город, НаборЗаписейРоссийскийАдрес.Город.Значение.Сокращение);		
		Если ЗаменятьБуквуЕ Тогда	
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание, "Город");
		КонецЕсли;
		
		НаборЗаписейРоссийскийАдрес.НаселенныйПункт.Значение.ГеографическоеНазвание = ПолучитьИмяИАдресноеСокращение(АдресСписком.НаселенныйПункт, НаборЗаписейРоссийскийАдрес.НаселенныйПункт.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
			ЗаменитьБуквуЁ(НаборЗаписейРоссийскийАдрес.Город.Значение.ГеографическоеНазвание, "Населенный пункт");
		КонецЕсли;	
		НаборЗаписейРоссийскийАдрес.Улица.Значение.ГеографическоеНазвание = ПолучитьИмяИАдресноеСокращение(АдресСписком.Улица, НаборЗаписейРоссийскийАдрес.Улица.Значение.Сокращение);
		Если ЗаменятьБуквуЕ Тогда
...
Процедура ЗаполнитьСоставительПачки(НаборЗаписейСоставительПачки, ВыборкаПоШапкеДокумента) Экспорт	
	...
	НаборЗаписейСоставительПачки.НаименованиеОрганизации.Значение = СокрЛП(ВРЕГ(Лев(ВыборкаПоШапкеДокумента.НаименованиеПолное,НаборЗаписейСоставительПачки.НаименованиеОрганизации.Размер)));
	
	НаборЗаписейСоставительПачки.НаименованиеКраткое.Значение = СокрЛП(ВРег(Лев(ВыборкаПоШапкеДокумента.НаименованиеСокращенное, НаборЗаписейСоставительПачки.НаименованиеКраткое.Размер)));
...
Функция ПолучитьИмяИАдресноеСокращение(Знач СтрокаЭлемента, АдресноеСокращение) Экспорт 
	...
	Если ПустаяСтрока(СокрЛП(Лев(СтрокаЭлемента, ПозицияПослПробела - 1))) Тогда
		Возврат ВРег(СтрокаЭлемента);
	Иначе
		АдресноеСокращение = ВРег(Сред(СтрокаЭлемента, ПозицияПослПробела + 1));
		Возврат ВРег(Лев(СтрокаЭлемента, ПозицияПослПробела - 1));
	КонецЕсли;
...
Функция ПолучитьПредставлениеАдресаДляПФР(СтруктураАдрес) Экспорт 
	...
	Если СтруктураАдрес <> Неопределено Тогда 
		...
		Если Не ЗаПределамиРФ Тогда
			СокращениеРегион = "";
			Регион = ПолучитьИмяИАдресноеСокращение(СтруктураАдрес.Регион, СокращениеРегион);
			СокращениеРайон = "";
			Район = ПолучитьИмяИАдресноеСокращение(СтруктураАдрес.Район, СокращениеРайон);
			СокращениеГород = "";
			Город = ПолучитьИмяИАдресноеСокращение(СтруктураАдрес.Город, СокращениеГород);
			СокращениеНаселенныйПункт = "";
			НаселенныйПункт = ПолучитьИмяИАдресноеСокращение(СтруктураАдрес.НаселенныйПункт, СокращениеНаселенныйПункт);
			СокращениеУлица = "";
			Улица = ПолучитьИмяИАдресноеСокращение(СтруктураАдрес.Улица, СокращениеУлица);
			...
			Адрес = ?(ЗначениеЗаполнено(СтруктураАдрес.Индекс),", "+СтруктураАдрес.Индекс,"")+
				...
				?(ЗначениеЗаполнено(СтруктураАдрес.Квартира), ", "+СокращениеКвартира+" "+СтруктураАдрес.Квартира, "");
				Адрес = Сред(Адрес,2);//Убрали первую запятую
		Иначе
			Адрес = СтруктураАдрес.Представление;//Убрали первую запятую	
		КонецЕсли;	
	КонецЕсли;	
...
Функция ВывестиОписьАДВ6(МассивОбъектов, ОбъектыПечати,  ТипДокумента) Экспорт
	//Получим ответственные лица организации и их должности 
	ТабличныйДокумент = Новый ТабличныйДокумент;
	
	МакетОписи = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_ФормаАДВ_6_1");
...
Процедура ПроверитьДанныеОрганизации(Объект, Организация, Отказ, ДатаАктуальности = Неопределено) Экспорт 
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если ДатаАктуальности = Неопределено Тогда
			ДанныеОрганизации = СведенияОбОрганизации(Организация, Объект.ОкончаниеОтчетногоПериода);
		Иначе
			ДанныеОрганизации = СведенияОбОрганизации(Организация, ДатаАктуальности);
		КонецЕсли;	
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//СЗВ-6-3, Обр.ПодготовкаДанныхВПФР
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Сотрудник) Тогда 
			...
			Если ДанныеДокумента.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ Тогда 
				...
				Если ПроверяемыйПериод >= '20120101' Тогда
					...
					// П. 4.1 Должны отсутствовать  значения в графах «Начислено» и «Уплачено» графы «Сумма страховых взносов на накопительную часть трудовой пенсии» 
					//формы СЗВ-6-1 (2) с кодом категории застра-хованного лица ВЖИЖ, ВЖНР, ВЖСХ, ВЖЕН, ВЖМН, ВЖОИ, ВЖХО,  ВЖТЗ, ВЖИТ, ВЖСИ, ВЖЭД, ВЖЦС, ВЖСБ, 
					...
					ИначеЕсли ТарифыПФР <> Неопределено Тогда
						...
						ИначеЕсли ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаРождения) 
							...
							//тариф страховых взносов  на страховую часть  должно равняться сумме страховых взносов на накопитель-ную 
