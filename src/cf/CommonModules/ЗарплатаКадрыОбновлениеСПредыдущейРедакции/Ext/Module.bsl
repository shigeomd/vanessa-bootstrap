...
Процедура ЗарплатаКадрыОбновлениеС20()Экспорт
	...
	КадровыйУчет.ЗаполнитьГоловногоСотрудника();
	...
	УчетНДФЛ.СправкиНДФЛЗаполнитьЗначенияНовыхРеквизитов();
	
	УстановитьПериодРегистрацийВНалоговомОргане();
	
	ПерсонифицированныйУчет.ЗаполнитьРегистрационныйНомерПФР();
	
	РасчетЗарплатыДляНебольшихОрганизаций.УстановитьФОРасчетЗарплатыДляНебольшихОрганизаций();
	
	РасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьТабличнуюЧастьСотрудникиДокументовНачислениеЗарплаты();
	
	РасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьНачисленоУдержаноНачисленийЗарплаты();
	
	ОтражениеЗарплатыВБухучетеБазовый.ЗаполнитьВидОперацииПоЗарплатеВНачислениях();
	
	УчетСтраховыхВзносов.ЗаполнитьПолеДатаПолученияДоходаВРегистрахУчетаВзносов();
	УчетСтраховыхВзносов.ЗаполнитьПолеДатаРегистрацииВПериодическихРегистрах();
	ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗаполнитьПолеДатаПолученияДоходаВТаблицеВзносыДокументов();
	
	ВзаиморасчетыССотрудниками.ЗаполнитьПериодыВзаиморасчетовЗарплатыВедомостей();
	ВзаиморасчетыССотрудниками.ЗаполнитьФизическиеЛицаЗарплатыВедомостей();
...
Процедура ЗаполнитьРесурсОсновноеМестоРаботы() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
Функция КонвертироватьЭлементДоходовПоСтраховымВзносам(КонвертируемыеДанные, СоответствиеКонвертированныхОбъектов, СсылкаНаПриемник = Неопределено)
	...
	Если Элемент = Неопределено Тогда
		...
		Элемент = Справочники.УдалитьДоходыНДФЛ.НайтиПоНаименованию(КонвертируемыеДанные.Наименование);
...
Процедура ОрганизацииКонвертация()
	...
	Пока Выборка.Следующий() Цикл
		...
		ОрганизацияОбъект.РайонныйКоэффициент = Выборка.УдалитьРайонныйКоэффициентРФ;
...
Процедура УстановитьПериодРегистрацийВНалоговомОргане()
	...
	РегистрыСведений.ИсторияРегистрацийВНалоговомОргане.ЗаполнитьИсториюРегистрацийВНалоговомОргане();	
	РегистрыСведений.ИсторияРегистрацийВНалоговомОргане.ЗаполнитьИсториюРегистрацийВНалоговомОрганеНаДатуОтсчетаПериодическихСведенийСПериодомМесяц();
	...
	Если Запрос.Выполнить().Пустой() Тогда 
		...
		Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Банк") Цикл  
				Банк = КонвертироватьДанные(Выборка.Банк, СоответствиеКонвертированныхОбъектов);
...
Процедура УчетнаяПолитикаОрганизацийВзносовКонвертация()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	УчетнаяПолитикаОрганизаций.УдалитьВидТарифаСтраховыхВзносов,
	               |	УчетнаяПолитикаОрганизаций.Организация,
	               |	УчетнаяПолитикаОрганизаций.УдалитьОсобенностиИсчисленияНДФЛ
	               |ИЗ
...
Процедура СтатусыФизическихЛицКонвертация()
	...
	Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
		ЗаписьНабора.Статус = ПолучитьСоответствиеЭлементаПеречисленияСтатусыНДФЛ(ЗаписьНабора.УдалитьСтатус);
	КонецЦикла;
...
Функция СоответствиеРеквизитовКонвертируемогоДокумента(ДанныеДокумента)
	Если ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьАнкетаЗастрахованногоЛица") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовАДВ1();
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьСведенияОТрудовомСтажеИЗаработкеСЗВ4") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовСЗВ6(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьЗаявлениеОбОбменеДубликатеСтраховогоСвидетельства") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовАДВ(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьПередачаДСВвПФР") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовДСВ(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьПередачаСЗВ4вПФР") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовАДВ6(ДанныеДокумента);	
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьСведенияОНачисленномЗаработкеСЗВ63") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовСЗВ_6_3(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьСведенияОВзносахИСтраховомСтажеСПВ1") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитовСПВ(ДанныеДокумента);	
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьСправкаПоНДФЛСотруднику") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитов2_НДФЛПоСотруднику(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьСправкаПоНДФЛВНалоговыйОрган") Тогда 
		Возврат ПолучитьСоответсвиеРеквизитов2_НДФЛВНалоговыйОрган(ДанныеДокумента);
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьНачислениеЗарплатыРаботникамОрганизаций") 
		Или ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьРасчетЕСН") Тогда 
	...
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьПачкаДокументовСЗВ_6_4") Тогда
		Возврат ПолучитьСоответсвиеРеквизитовСЗВ_6_4();	
	ИначеЕсли ТипЗнч(ДанныеДокумента.Ссылка) = Тип("ДокументСсылка.УдалитьАвансовыйПлатежИностранцаПоНДФЛ") Тогда
		Возврат ПолучитьСоответсвиеРеквизитовАвансовыйПлатежИностранцаПоНДФЛ();
	Иначе	
...
Функция ПолучитьСоответсвиеРеквизитовАДВ1()Экспорт
	...
	СоответсвиеРеквизитовТЧСотрудники = Новый Структура;
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ИмяТЧ", "РаботникиОрганизации");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Сотрудник",	"ФизЛицо");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресРегистрации",	"АдресРегистрации");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресФактический",	"АдресФактический");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресРегистрацииПредставление",	"АдресРегистрации");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресФактическийПредставление",	"АдресФактический");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Телефоны",	"Телефоны");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Фамилия",	"Фамилия");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Имя",	"Имя");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Отчество",	"Отчество");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Пол",	"Пол");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДатаРождения",	"ДатаРождения");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("МестоРождения",	"МестоРождения");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("МестоРожденияПредставление",	"МестоРождения");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Гражданство",	"Гражданство");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ВидДокумента",	"ВидДокумента");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("СерияДокумента",	"СерияДокумента");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("НомерДокумента",	"НомерДокумента");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("КемВыдан",	"КемВыдан");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДатаВыдачи",	"ДатаВыдачи");

	
	СоответствиеРеквизитов.Вставить("Сотрудники",	СоответсвиеРеквизитовТЧСотрудники);
...
Функция ПолучитьСоответсвиеРеквизитовАДВ(ДанныеДокумента)Экспорт
	СоответствиеРеквизитов = Новый Структура;
	СоответствиеРеквизитов.Вставить("Дата", "Дата");
    СоответствиеРеквизитов.Вставить("Номер", "Номер");
	СоответствиеРеквизитов.Вставить("Организация", "Организация");
...
Функция ПолучитьСоответсвиеРеквизитовДСВ(ДанныеДокумента)Экспорт 
	СоответствиеРеквизитов = Новый Структура;
...
Функция ПолучитьСоответсвиеРеквизитовАДВ6(ДанныеДокумента)Экспорт 
	СоответствиеРеквизитов = Новый Структура;
...
Функция ПолучитьСоответсвиеРеквизитовСЗВ_6_3(ДанныеДокумента)Экспорт
	СоответствиеРеквизитов = Новый Структура;
...
Функция ПолучитьСоответсвиеРеквизитовСПВ(ДанныеДокумента)Экспорт
	...
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ИмяТЧ", "РаботникиОрганизации");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Сотрудник", "ФизЛицо");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("СтраховойНомерПФР", "СтраховойНомерПФР");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Фамилия", "Фамилия");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Имя", "Имя");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("Отчество", "Отчество");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("НачисленоСтраховая", "НачисленоСтраховая");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("УплаченоСтраховая", "УплаченоСтраховая");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("НачисленоНакопительная", "НачисленоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("УплаченоНакопительная", "УплаченоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДатаСоставления", "ДатаСоставления");
	...
	Если ЗначениеЗаполнено(Структура.ЗаписиОСтаже) Тогда 
		СоответсвиеРеквизитовТЧЗаписиОСтаже = Новый Структура;
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ИмяТЧ", "ЗаписиОСтаже");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("Сотрудник", "ФизЛицо");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ДатаНачалаПериода", "ДатаНачалаПериода");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ДатаОкончанияПериода", "ДатаОкончанияПериода");
		
	
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("НомерОсновнойЗаписи", "НомерОсновнойЗаписи");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("НомерДополнительнойЗаписи", "НомерДополнительнойЗаписи");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ОсобыеУсловияТруда", "ОсобыеУсловияТруда");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("КодПозицииСписка", "КодПозицииСписка");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ОснованиеИсчисляемогоСтажа", "ОснованиеИсчисляемогоСтажа");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ПервыйПараметрИсчисляемогоСтажа", "ПервыйПараметрИсчисляемогоСтажа");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ВторойПараметрИсчисляемогоСтажа", "ВторойПараметрИсчисляемогоСтажа");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ТретийПараметрИсчисляемогоСтажа", "ТретийПараметрИсчисляемогоСтажа");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ОснованиеВыслугиЛет", "ОснованиеВыслугиЛет");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ПервыйПараметрВыслугиЛет", "ПервыйПараметрВыслугиЛет");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ВторойПараметрВыслугиЛет", "ВторойПараметрВыслугиЛет");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ТретийПараметрВыслугиЛет", "ТретийПараметрВыслугиЛет");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ТерриториальныеУсловия", "ТерриториальныеУсловия");
		СоответсвиеРеквизитовТЧЗаписиОСтаже.Вставить("ПараметрТерриториальныхУсловий", "ПараметрТерриториальныхУсловий");
		
		СоответствиеРеквизитов.Вставить("ЗаписиОСтаже",	СоответсвиеРеквизитовТЧЗаписиОСтаже);
	КонецЕсли;	
...
Функция ПолучитьСоответсвиеРеквизитовСЗВ6(ДанныеДокумента)
	...
	СоответсвиеРеквизитовТЧСотрудники.Вставить("НачисленоНакопительная", "НачисленоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("УплаченоНакопительная", "УплаченоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДоначисленоСтраховая", "ДоначисленоСтраховая");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДоначисленоНакопительная", "ДоначисленоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДоУплаченоНакопительная", "ДоУплаченоНакопительная");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("ДоУплаченоСтраховая", "ДоУплаченоСтраховая");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресДляИнформирования", "АдресДляИнформирования");
	СоответсвиеРеквизитовТЧСотрудники.Вставить("АдресДляИнформированияПредставление", "АдресДляИнформирования");
...
Процедура КонвертироватьСоставКомплектовСведений(СоответствиеКонвертированныхОбъектов)
	...
	|	УдалитьСведенияПринятыеПФР.КомплектДокументов.РасчетСтраховыхВзносов КАК РСВ
...
Функция ПолучитьСоответсвиеРеквизитовСЗВ_6_4()
	СоответствиеРеквизитов = Новый Структура;
	...
	СоответсвиеРеквизитовТЧСведенияОЗаработке = Новый Структура;
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("ИмяТЧ", "СведенияОЗаработке");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("Сотрудник", "ФизЛицо");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("Месяц", "Месяц");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("Заработок", "Заработок");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("ОблагаетсяВзносамиДоПредельнойВеличины", "ОблагаетсяВзносамиДоПредельнойВеличины");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("ОблагаетсяВзносамиСвышеПредельнойВеличины", "ОблагаетсяВзносамиСвышеПредельнойВеличины");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах", "ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах");
	СоответсвиеРеквизитовТЧСведенияОЗаработке.Вставить("ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах", "ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах");	

	СоответствиеРеквизитов.Вставить("СведенияОЗаработке",	СоответсвиеРеквизитовТЧСведенияОЗаработке);
...
Функция ПолучитьСоответсвиеРеквизитов2_НДФЛПоСотруднику(ДанныеДокумента)
	СоответствиеРеквизитов = Новый Структура;
	...
	СоответствиеРеквизитов.Вставить("КодНалоговогоОрганаУведомления", "КодНалоговогоОрганаУведомления");
...
Функция ПолучитьСоответсвиеРеквизитов2_НДФЛВНалоговыйОрган(ДанныеДокумента)Экспорт
	СоответствиеРеквизитов = Новый Структура;
	...
	СоответствиеРеквизитовТЧСотрудник.Вставить("ДатаУведомления", "ДатаУведомления");
	СоответствиеРеквизитовТЧСотрудник.Вставить("НомерУведомления", "НомерУведомления");
	СоответствиеРеквизитовТЧСотрудник.Вставить("КодНалоговогоОрганаУведомления", "КодНалоговогоОрганаУведомления");
...
Функция СоответствиеРеквизитовДокументаНачислениеЗарплаты()
	...
	СоответствиеРеквизитовТЧ.Вставить("НалогКЗачетуВозврату", "НалогКЗачетуВозврату");
...
Функция ПолучитьСоответсвиеРеквизитовАвансовыйПлатежИностранцаПоНДФЛ()Экспорт
...
Функция КонвертироватьДокументыКадровоеПеремещение(СписокДокументов, СоответствиеКонвертированныхОбъектов)
	...
	|	УдалитьПриемНаРаботуВОрганизациюОсновныеНачисления.ВидРасчета КАК Начисление,
	|	ВЫБОР
	|		КОГДА УдалитьПриемНаРаботуВОрганизациюОсновныеНачисления.Действие = ЗНАЧЕНИЕ(Перечисление.УдалитьВидыДействияСНачислением.Прекратить)
	|			ТОГДА 0
	|		ИНАЧЕ УдалитьПриемНаРаботуВОрганизациюОсновныеНачисления.Размер
	|	КОНЕЦ КАК Размер
	|ИЗ
	|	Документ.УдалитьКадровоеПеремещениеОрганизаций.ОсновныеНачисления КАК УдалитьПриемНаРаботуВОрганизациюОсновныеНачисления
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	Сотрудник,
...
Функция СоответствиеРеквизитовОперацииНДФЛ()
	СоответсвиеРеквизитов = Новый Структура;
	
	СоответсвиеРеквизитов.Вставить("Дата", "Дата");
	СоответсвиеРеквизитов.Вставить("Номер", "Номер");
	СоответсвиеРеквизитов.Вставить("Проведен", "Проведен");
	СоответсвиеРеквизитов.Вставить("ДатаОперации", "ДатаОперации");
	СоответсвиеРеквизитов.Вставить("Организация", "Организация");
	СоответсвиеРеквизитов.Вставить("Сотрудник", "Сотрудник");
	СоответсвиеРеквизитов.Вставить("Комментарий", "Комментарий");
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "СведенияОДоходах");
	СоответсвиеРеквизитовТЧ.Вставить("ДатаПолученияДохода", "ДатаПолученияДохода");
	СоответсвиеРеквизитовТЧ.Вставить("КодДохода", "КодДохода");
	СоответсвиеРеквизитовТЧ.Вставить("СуммаДохода", "СуммаДохода");
	СоответсвиеРеквизитовТЧ.Вставить("ОбособленноеПодразделение", "ОбособленноеПодразделение");
	СоответсвиеРеквизитовТЧ.Вставить("КодВычета", "КодВычета");
	СоответсвиеРеквизитовТЧ.Вставить("Количество", "Количество");
	СоответсвиеРеквизитовТЧ.Вставить("СуммаВычета", "СуммаВычета");
	СоответсвиеРеквизитовТЧ.Вставить("СуммаНалогаИсчисленная", "СуммаНалогаИсчисленная");
	
	СоответсвиеРеквизитов.Вставить("СведенияОДоходах", СоответсвиеРеквизитовТЧ);
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "НДФЛИсчисленныйПоСтавке13");
	СоответсвиеРеквизитовТЧ.Вставить("МесяцНалоговогоПериода", "МесяцНалоговогоПериода");
	СоответсвиеРеквизитовТЧ.Вставить("Сумма", "Сумма");
	СоответсвиеРеквизитовТЧ.Вставить("ОбособленноеПодразделение", "ОбособленноеПодразделение");
	
	СоответсвиеРеквизитов.Вставить("НДФЛИсчисленныйПоСтавке13", СоответсвиеРеквизитовТЧ);
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "ПредоставленныеВычеты");
	СоответсвиеРеквизитовТЧ.Вставить("МесяцНалоговогоПериода", "МесяцНалоговогоПериода");
	СоответсвиеРеквизитовТЧ.Вставить("Сумма", "Сумма");
	СоответсвиеРеквизитовТЧ.Вставить("КодВычета", "КодВычета");
	СоответсвиеРеквизитовТЧ.Вставить("ОбособленноеПодразделение", "ОбособленноеПодразделение");
	
	СоответсвиеРеквизитов.Вставить("ПредоставленныеВычеты", СоответсвиеРеквизитовТЧ);

	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "НДФЛУдержанный");
	СоответсвиеРеквизитовТЧ.Вставить("МесяцНалоговогоПериода", "МесяцНалоговогоПериода");
	СоответсвиеРеквизитовТЧ.Вставить("КодДохода", "КодДохода");
	СоответсвиеРеквизитовТЧ.Вставить("ОбособленноеПодразделение", "ОбособленноеПодразделение");
	СоответсвиеРеквизитовТЧ.Вставить("Сумма", "Сумма");
	СоответсвиеРеквизитовТЧ.Вставить("Ставка", "Ставка");
	СоответсвиеРеквизитовТЧ.Вставить("СтавкаНалогообложения", "СтавкаНалогообложения");
	
	СоответсвиеРеквизитов.Вставить("НДФЛУдержанный", СоответсвиеРеквизитовТЧ);
	
	Возврат СоответсвиеРеквизитов;	
КонецФункции
...
Функция СоответствиеРеквизитовОперацииПоВзносам()
	...
	СоответсвиеРеквизитов.Вставить("Номер", "Номер");
	СоответсвиеРеквизитов.Вставить("Проведен", "Проведен");
	СоответсвиеРеквизитов.Вставить("МесяцРасчетногоПериода", "МесяцРасчетногоПериода");
	СоответсвиеРеквизитов.Вставить("Организация", "Организация");
	СоответсвиеРеквизитов.Вставить("ФизическоеЛицо", "ФизическоеЛицо");
	СоответсвиеРеквизитов.Вставить("Комментарий", "Комментарий");
	СоответсвиеРеквизитов.Вставить("ПФРНакопительная", "ПФРНакопительная");
	СоответсвиеРеквизитов.Вставить("ПФРСтраховая", "ПФРСтраховая");
	СоответсвиеРеквизитов.Вставить("ФФОМС", "ФФОМС");
	СоответсвиеРеквизитов.Вставить("ТФОМС", "ТФОМС");
	СоответсвиеРеквизитов.Вставить("ФСС", "ФСС");
	СоответсвиеРеквизитов.Вставить("ФССНесчастныеСлучаи", "ФССНесчастныеСлучаи");
	СоответсвиеРеквизитов.Вставить("ПФРНаДоплатуЛетчикам", "ПФРНаДоплатуЛетчикам");
	СоответсвиеРеквизитов.Вставить("ПФРНаДоплатуШахтерам", "ПФРНаДоплатуШахтерам");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботах", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботах");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботах", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботах");
	СоответсвиеРеквизитов.Вставить("ПФРСтраховаяЕНВД", "ПФРСтраховаяЕНВД");
	СоответсвиеРеквизитов.Вставить("ПФРНакопительнаяЕНВД", "ПФРНакопительнаяЕНВД");
	СоответсвиеРеквизитов.Вставить("ФССЕНВД", "ФССЕНВД");
	СоответсвиеРеквизитов.Вставить("ФФОМСЕНВД", "ФФОМСЕНВД");
	СоответсвиеРеквизитов.Вставить("ТФОМСЕНВД", "ТФОМСЕНВД");
	
	
	СоответсвиеРеквизитов.Вставить("ПФРПоСуммарномуТарифу", "ПФРПоСуммарномуТарифу");
	СоответсвиеРеквизитов.Вставить("ПФРПоСуммарномуТарифуЕНВД", "ПФРПоСуммарномуТарифуЕНВД");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботахОпасный", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботахОпасный");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный1", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный1");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный2", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный2");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный3", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный3");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный4", "ПФРЗаЗанятыхНаПодземныхИВредныхРаботахВредный4");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахОпасный", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахОпасный");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный1", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный1");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный2", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный2");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный3", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный3");
	СоответсвиеРеквизитов.Вставить("ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный4", "ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахВредный4");
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "СведенияОДоходах");
	СоответсвиеРеквизитовТЧ.Вставить("ВидДохода", "ВидДохода");
	СоответсвиеРеквизитовТЧ.Вставить("Результат", "Результат");
	СоответсвиеРеквизитовТЧ.Вставить("КоличествоДетей", "КоличествоДетей");
	СоответсвиеРеквизитовТЧ.Вставить("Скидка", "Скидка");
	СоответсвиеРеквизитовТЧ.Вставить("ОблагаетсяЕНВД", "ОблагаетсяЕНВД");
	СоответсвиеРеквизитовТЧ.Вставить("ОблагаетсяВзносамиНаДоплатуЛетчикам", "ОблагаетсяВзносамиНаДоплатуЛетчикам");
	СоответсвиеРеквизитовТЧ.Вставить("ОблагаетсяВзносамиНаДоплатуШахтерам", "ОблагаетсяВзносамиНаДоплатуШахтерам");
	СоответсвиеРеквизитовТЧ.Вставить("ЯвляетсяДоходомФармацевта", "ЯвляетсяДоходомФармацевта");
	СоответсвиеРеквизитовТЧ.Вставить("ЯвляетсяДоходомЧленаЭкипажаСуднаПодФлагомРФ", "ЯвляетсяДоходомЧленаЭкипажаСуднаПодФлагомРФ");
	СоответсвиеРеквизитовТЧ.Вставить("ОблагаетсяВзносамиЗаЗанятыхНаРаботахСДосрочнойПенсией", "ОблагаетсяВзносамиЗаЗанятыхНаРаботахСДосрочнойПенсией");
	СоответсвиеРеквизитовТЧ.Вставить("КлассУсловийТруда", "КлассУсловийТруда");
	
	СоответсвиеРеквизитов.Вставить("СведенияОДоходах", СоответсвиеРеквизитовТЧ);
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "ПособияПоСоциальномуСтрахованию");
	СоответсвиеРеквизитовТЧ.Вставить("ВидПособияСоциальногоСтрахования", "ВидПособияСоциальногоСтрахования");
	СоответсвиеРеквизитовТЧ.Вставить("ДатаСтраховогоСлучая", "ДатаСтраховогоСлучая");
	СоответсвиеРеквизитовТЧ.Вставить("УчитыватьКакНовыйСтраховойСлучай", "УчитыватьКакНовыйСтраховойСлучай");
	СоответсвиеРеквизитовТЧ.Вставить("ВидЗанятости", "ВидЗанятости");
	СоответсвиеРеквизитовТЧ.Вставить("ОплаченныеДни", "ОплаченныеДни");
	СоответсвиеРеквизитовТЧ.Вставить("СуммаСверхНорм", "СуммаСверхНорм");
	СоответсвиеРеквизитовТЧ.Вставить("СуммаВсего", "СуммаВсего");
	СоответсвиеРеквизитовТЧ.Вставить("ФинансированиеФедеральнымБюджетом", "ФинансированиеФедеральнымБюджетом");
	
	СоответсвиеРеквизитов.Вставить("ПособияПоСоциальномуСтрахованию", СоответсвиеРеквизитовТЧ);
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "ПособияПоУходуЗаРебенком");
	СоответсвиеРеквизитовТЧ.Вставить("НачалоОтпускаПоУходу", "НачалоОтпускаПоУходу");
	СоответсвиеРеквизитовТЧ.Вставить("ВидЗанятости", "ВидЗанятости");
	СоответсвиеРеквизитовТЧ.Вставить("ПособиеПоУходуЗаПервымРебенком", "ПособиеПоУходуЗаПервымРебенком");
	СоответсвиеРеквизитовТЧ.Вставить("ПособиеПоУходуЗаПервымРебенкомСверхНорм", "ПособиеПоУходуЗаПервымРебенкомСверхНорм");
	СоответсвиеРеквизитовТЧ.Вставить("ПособиеПоУходуЗаВторымРебенкомСверхНорм", "ПособиеПоУходуЗаВторымРебенкомСверхНорм");
	СоответсвиеРеквизитовТЧ.Вставить("ПособиеПоУходуЗаВторымРебенком", "ПособиеПоУходуЗаВторымРебенком");
	СоответсвиеРеквизитовТЧ.Вставить("ФинансированиеФедеральнымБюджетом", "ФинансированиеФедеральнымБюджетом");
		
	СоответсвиеРеквизитов.Вставить("ПособияПоУходуЗаРебенком", СоответсвиеРеквизитовТЧ);
...
Функция СоответствиеРеквизитовДокументаПерерасчетНДФЛ()
	...
	СоответсвиеРеквизитов.Вставить("Номер", "Номер");
	СоответсвиеРеквизитов.Вставить("Проведен", "Проведен");
	СоответсвиеРеквизитов.Вставить("МесяцПерерасчета", "МесяцПерерасчета");
	СоответсвиеРеквизитов.Вставить("НалоговыйПериод", "НалоговыйПериод");
	СоответсвиеРеквизитов.Вставить("Организация", "Организация");
	СоответсвиеРеквизитов.Вставить("Ответственный", "Ответственный");
	
	СоответсвиеРеквизитовТЧ = Новый Структура;
	СоответсвиеРеквизитовТЧ.Вставить("ИмяТЧ", "Сотрудники");
	СоответсвиеРеквизитовТЧ.Вставить("Сотрудник", "Сотрудник");
	СоответсвиеРеквизитовТЧ.Вставить("НалогПоСтавке09", "НалогПоСтавке09");
	СоответсвиеРеквизитовТЧ.Вставить("НалогПоСтавке13", "НалогПоСтавке13");
	СоответсвиеРеквизитовТЧ.Вставить("НалогПоСтавке35", "НалогПоСтавке35");
	СоответсвиеРеквизитовТЧ.Вставить("ПримененныйВычетЛичный", "ПримененныйВычетЛичный");
	СоответсвиеРеквизитовТЧ.Вставить("ИдентификаторСтрокиНДФЛ", "ИдентификаторСтрокиНДФЛ");
	
	СоответсвиеРеквизитов.Вставить("Сотрудники", СоответсвиеРеквизитовТЧ);
	...
	СоответсвиеРеквизитов.Вставить("ПримененныеВычетыНаДетейИИмущественные", СоответствиеРеквизитовТЧ);
	...
	Если ДополнятьБазойПоУмолчанию Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("ПериодРаспределения") Цикл
				...
				БазаРаспределенияЗаПериод = РасчетЗарплаты.ПолучитьБазуУдержанийПоУмолчанию(ФизЛица, Выборка.ПериодРаспределения, Выборка.Организация);
	...
	Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("Организация") Цикл
		Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("Период") Цикл
			Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("ПериодВзаиморасчетов") Цикл	
				...
				ТаблицаВыплат = Новый ТаблицаЗначений;
				ТаблицаВыплат.Колонки.Добавить("Сотрудник");
				ТаблицаВыплат.Колонки.Добавить("ФизическоеЛицо");
				ТаблицаВыплат.Колонки.Добавить("Подразделение");
				ТаблицаВыплат.Колонки.Добавить("КВыплате");
				    ...
				    Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("ВидДвижения") Цикл 
						...
						Пока ВыборкаПоВзаиморасчетам.Следующий() Цикл 
							Если ВыборкаПоВзаиморасчетам.ВидДвижения = ВидДвиженияНакопления.Расход Тогда
								...
								СтрокаТаблицы.КВыплате = ВыборкаПоВзаиморасчетам.Сумма;	
...
Функция ДанныеДляРегистрацииНачислений(СписокДокументов)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("РайонныйКоэффициент", РасчетЗарплаты.НачислениеРайонныйКоэффициент());
	Запрос.УстановитьПараметр("СевернаяНадбавка", РасчетЗарплаты.НачислениеСевернаяНадбавка());
	Запрос.УстановитьПараметр("СписокДокументов", СписокДокументов); 
...
Процедура ЗарегистрироватьНачисления(ВыборкаПоНачислениям, Регистратор, СоответствиеКонвертированныхОбъектов = Неопределено)
	...
	Если ТаблицаНачислений.Количество() > 0 Тогда
		УчетНачисленнойЗарплаты.ЗарегистрироватьНачисленияУдержания(Регистратор.Движения, Ложь, ВыборкаПоНачислениям.Организация, ВыборкаПоНачислениям.ПериодРегистрации, ТаблицаНачислений, Неопределено, Неопределено, Неопределено);
	КонецЕсли;
...
Функция СоответствиеВладельцевРегистрациям()
	СоответсвиеРегистраций = Новый Соответствие;
	...
	Пока Выборка.СледующийПоЗначениюПоля("Владелец") Цикл
		...
		СоответсвиеРегистрацийКодам = Новый Структура("СоотвествиеРегистрацийОКАТОКПП, СоотвествиеРегистрацийОКТМОКПП", СоотвествиеРегистрацийОКАТОКПП, СоотвествиеРегистрацийОКТМОКПП);
		
		СоответсвиеРегистраций.Вставить(Выборка.Владелец, СоответсвиеРегистрацийКодам);
		
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.КодПоОКАТО) Тогда
				ОКАТОКПП = УчетНДФЛ.СуммаОКАТОиКПП(Выборка.КодПоОКАТО, Выборка.КПП);
				СоответсвиеРегистрацийКодам.СоотвествиеРегистрацийОКАТОКПП.Вставить(ОКАТОКПП, Выборка.Ссылка);
			КонецЕсли;	
			
			Если ЗначениеЗаполнено(Выборка.КодПоОКТМО) Тогда
				ОКТМОКПП = УчетНДФЛ.СуммаОКТМОиКПП(Выборка.КодПоОКТМО, Выборка.КПП);
				СоответсвиеРегистрацийКодам.СоотвествиеРегистрацийОКТМОКПП.Вставить(ОКТМОКПП, Выборка.Ссылка);
			КонецЕсли;	
		КонецЦикла;	
		
	КонецЦикла;	
	
	Возврат СоответсвиеРегистраций;
КонецФункции	
...
Процедура СформироватьВТСДаннымиПоДоходамНДФЛ(МенеджерВременныхТаблиц, КлючиПорцииДанных, ТолькоДвиженияНепереносимыхРегистраторов = Ложь)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьНДФЛСведенияОДоходах.ПериодРегистрации КАК Период,
		...
		Пока ДанныеПоРасчетамНДФЛ.СледующийПоЗначениюПоля("Организация") Цикл
			...
			Если ТаблицаУдержанийНДФЛ.Количество() > 0 Тогда
				...
				УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛ(
					ДокументПереноса.Движения, 
				    Ложь, 
					ДанныеПоРасчетамНДФЛ.Организация, 
					НачалоМесяца(ДанныеПоРасчетамНДФЛ.Период), 
					ТаблицаУдержанийНДФЛ, 
					МенеджерВТ);															   
			КонецЕсли;	
