...
Процедура СформироватьДвиженияПриобретениеВалюты(РасшифровкаПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаПлатежа Из Параметры.РасшифровкаПлатежа Цикл
		...
		Если Реквизиты.ОтражатьРазницуВКурсе И СуммаРазницы > 0 Тогда // Доходы
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 
				"ПрочиеДоходыИРасходы", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.ОтклонениеКурсаПродажиПокупкиВалюты"));
			СвойстваСчетаКт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетКт);
			Если СвойстваСчетаКт.УчетПоПодразделениям Тогда
				Проводка.ПодразделениеКт = СтрокаПлатежа.Подразделение;
			КонецЕсли;
...
Процедура СформироватьДвиженияПродажаВалюты(РасшифровкаПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаПлатежа Из Параметры.РасшифровкаПлатежа Цикл
		...
		Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ПрочиеДоходы;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 
			"ПрочиеДоходыИРасходы", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.ДоходыРасходыПриПродажеПокупкеВалюты"));
		СвойстваСчетаКт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетКт);
		Если СвойстваСчетаКт.УчетПоПодразделениям Тогда
			Проводка.ПодразделениеКт = СтрокаПлатежа.Подразделение;
		КонецЕсли;
...
//  Отказ              - <Булево> - флаг отказа от проведения
//
Процедура СформироватьДвиженияПрочееСписание(РасшифровкаПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Параметры = ПодготовитьПараметрыПрочееСписание(РасшифровкаПлатежа, ТаблицаРеквизиты);
	...
	Если УчетСтраховыхВзносовИП.ЭтоСчетУчетаСтраховыхВзносовИП(Реквизиты.СчетУчета) Тогда
		Возврат;
	КонецЕсли;
