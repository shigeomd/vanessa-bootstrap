...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОшибки, ИмяКомпьютера);
...
Процедура ЗаполнитьСуществующиеСертификатыПользователя(СписокВыбора, ОтпечаткиСертификатовНаКлиенте,
			ОтборСертификатов, ОтборОтпечатков = Неопределено) Экспорт
...
Процедура ЗаписатьСертификатВСправочник(Форма, Программа = Неопределено, ДляШифрования = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Форма.Сертификат) Тогда
		СертификатОбъект = Форма.Сертификат.ПолучитьОбъект();
	Иначе
		...
		СертификатОбъект.Отпечаток = Форма.СертификатОтпечаток;
		
		СертификатОбъект.Добавил = ПользователиКлиентСервер.ТекущийПользователь();
	КонецЕсли;
	
	Если Не ДляШифрования Тогда
		ОбновитьЗначение(СертификатОбъект.Программа, Программа);
	КонецЕсли;
	...
	ОбновитьЗначение(СертификатОбъект.Наименование, Форма.СертификатНаименование);
	ОбновитьЗначение(СертификатОбъект.Организация,  Форма.СертификатОрганизация);
	ОбновитьЗначение(СертификатОбъект.Пользователь, Форма.СертификатПользователь);
	
	Если Не ДляШифрования Тогда
		ОбновитьЗначение(СертификатОбъект.УсиленнаяЗащитаЗакрытогоКлюча, Форма.СертификатУсиленнаяЗащитаЗакрытогоКлюча);
	КонецЕсли;
	...
	Если СертификатОбъект.ЭтоНовый() Тогда
		...
		Для каждого КлючИЗначение Из Форма.СертификатПараметрыРеквизитов Цикл
			...
			Если ПропускаемыеРеквизиты.Получить(ИмяРеквизита) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ОбновитьЗначение(СертификатОбъект[ИмяРеквизита], Свойства.ЗначениеЗаполнения, Истина);
		КонецЦикла;
	КонецЕсли;
	
	Если СертификатОбъект.Модифицированность() Тогда
		СертификатОбъект.Записать();
		Форма.Сертификат = СертификатОбъект.Ссылка;
	КонецЕсли;
...
Процедура ПеренестиНастройкиМенеджераКриптографии() Экспорт
	...
	Попытка
		...
		Если Константы.ИспользоватьЭлектронныеПодписи.Получить() = Истина
		   И Константы.ИспользоватьШифрование.Получить() = Ложь Тогда
