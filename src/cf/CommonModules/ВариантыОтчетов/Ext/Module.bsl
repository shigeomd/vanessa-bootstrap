...
//       - Истина, "ПоОтчетам" - Варианты отчета по умолчанию скрыты, а отчет (вариант с пустым ключом) включен и видим.
...
Процедура ДобавитьПодсистемыТекущегоПользователя(РодительСтрока, РодительМетаданные = Неопределено, РазделСсылка = Неопределено) Экспорт
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		Если ПодсистемаМетаданные.ВключатьВКомандныйИнтерфейс
			...
			ИначеЕсли СтрДлина(ПриоритетРодителя) > 12 Тогда
...
Функция ОтчетПодключенКХранилищу(ОтчетМетаданные, ПодключеныВсеОтчеты = Неопределено) Экспорт
	ХранилищеМетаданные = ОтчетМетаданные.ХранилищеВариантов;
	Если ХранилищеМетаданные = Неопределено Тогда
		Если ПодключеныВсеОтчеты = Неопределено Тогда
			ПодключеныВсеОтчеты = ПодключеныВсеОтчеты();
		КонецЕсли;
		ОтчетПодключен = ПодключеныВсеОтчеты;
	Иначе
...
// Запись предупреждения в журнале регистрации по варианту отчета.
Процедура ПримечаниеПоВарианту(Вариант, Сообщение, Реквизит1 = Неопределено, Реквизит2 = Неопределено, Реквизит3 = Неопределено) Экспорт
	ЗаписьЖурналаРегистрации(
		ВариантыОтчетовКлиентСервер.НаименованиеПодсистемы(Неопределено),
...
// [*] Актуализирует неразделенные данные: справочник ПредопределенныеВариантыОтчетов и константу
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ОтчетОбъект = Неопределено Тогда // Запись в кэш.
			РезультатПодключения = ПодключитьОтчетОбъект(СтрокаТаблицы.Отчет);
			Если РезультатПодключения.Подключен Тогда
				ОтчетОбъект = РезультатПодключения.Объект;
				...
				КонецЕсли;
			Иначе // Отчет не найден.
				ОшибкаПоВарианту(СтрокаТаблицы.Ссылка, РезультатПодключения.Ошибки);
				ОтчетОбъект = "";
			КонецЕсли;
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", Ошибок);
...
Процедура ОбновитьРазделенныйПредопределенный(Кэш, Шаблоны, СтрокаТаблицы)
	...
	Иначе
		...
		Иначе // Обновить (если есть изменения).
			...
			ЗаменитьКлючиПользовательскихНастроек(Шаблоны.ОбновлениеСведений, СтрокаТаблицы);
			ВариантОбъект = СтрокаТаблицы.РеквизитСсылка.ПолучитьОбъект();
		КонецЕсли;
...
Функция ДобавитьЭлементУсловногоОформления(Форма, ИнструкцияУсловногоОформления) Экспорт
	...
	Если ТипЗнч(Поля) = Тип("Строка") Тогда
		Поля = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Поля, ",", Истина, Истина);
	КонецЕсли;
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект) Экспорт
	...
	Информация = ОтчетыСервер.РасширеннаяИнформацияОНастройках(КомпоновщикНастроекКД, Неопределено, УсловияВывода);
...
Функция НайтиСсылки(ПараметрыПоиска) Экспорт
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		|	И НЕ ВариантыОтчетов.ПредопределенныйВариант В (&ОтключенныеВариантыПрограммы)
		|	И ВариантыОтчетов.ПометкаУдаления = ЛОЖЬ
		|	И НЕ ЕСТЬNULL(Предопределенные.ПометкаУдаления, ЛОЖЬ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		Иначе
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И НЕ ЕСТЬNULL(Предопределенные.ПометкаУдаления, ЛОЖЬ)", "");
...
// Выгружает текст и параметры запроса в XML.
Функция ЗапросВXML(Запрос) Экспорт
