...
//    * КодПоОКПО                - Строка - (возвращаемое значение) код по ОКПО организации.
...
//
//    * ВидВладельца - Строка - одна из строк: Директор, ГлавныйБухгалтер, Сотрудник,
//                     которая показывает кто сейчас выбран пользователем в качестве владельца сертификата.
//                     Для этого владельца требуется заполнить реквизиты ФИО, ...
//                     Для индивидуального предпринимателя всегда Сотрудник. Причем в этом случае,
//                     реквизит Сотрудник не может быть изменен пользователем даже, если не заполнен.
//
//    * Директор     - ТипВладельца - (возвращаемое значение) - это директор, который может быть выбран,
//                     как владелец сертификата.
//                   - Неопределено - начальное значение - скрыть реквизит директор.
//
//    * ГлавныйБухгалтер - ТипВладельца - (возвращаемое значение) это главный бухгалтер, который может быть выбран,
//                     как владелец сертификата.
//                   - Неопределено - начальное значение - скрыть реквизит главный бухгалтер.
//
...
//                   - Неопределено - ТипВладельца не определен - реквизит не доступен пользователю.
//
...
Процедура ПриЗаполненииРеквизитовВладельцаВЗаявленииНаСертификат(Параметры) Экспорт
	...
	Если РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Если Не ЗначениеЗаполнено(Параметры.Сотрудник) Тогда
			...
			Параметры.ВидВладельца = "Сотрудник";
		КонецЕсли;
	Иначе
...
//    * Основание     - Строка - (возвращаемое значение) - основание на котором действует должностное лицо (устав, доверенность, ...).
//
...
//                      ЭлектроннаяПодписьПереопределяемыйКлиент после создания формы
...
//    в функции ПроверитьСертификатСправочника общего модуля ЭлектроннаяПодписьКлиент.
//
Процедура ПриСозданииФормыПроверкаСертификата(Сертификат, ДополнительныеПроверки, ПараметрыДополнительныхПроверок) Экспорт
	...
	Если ТипЗнч(ПараметрыДополнительныхПроверок) = Тип("Структура")
		...
		НоваяПроверка.Представление = НСтр("ru = 'Аутентификация на сервере оператора ЭДО'");
		Подсказка = НСтр("ru = 'Проверяет возможность аутентификации на сервере оператора электронного документооборота.
			|Требуется пароль.'");
...
// Вызвается из формы ПроверкаСертификата, если при создании формы были добавлены дополнительные проверки.
//
// Параметры:
//  Сертификат           - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования - проверяемый сертификат.
...
Процедура ПриДополнительнойПроверкеСертификата(Сертификат, Проверка, МенеджерКриптографии, ОписаниеОшибки, ЭтоПредупреждение) Экспорт
	...
	ЭлектронныеДокументыСлужебный.ПриДополнительнойПроверкеСертификата(
		Сертификат, Проверка, МенеджерКриптографии, ОписаниеОшибки, ЭтоПредупреждение);
