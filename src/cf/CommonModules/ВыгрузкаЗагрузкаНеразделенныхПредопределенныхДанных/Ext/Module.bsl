			...
			Для Каждого ИмяПоля Из ПравилоКонтроля.Значение Цикл
				...
				Иначе
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Объект метаданных %1 не поддерживается!'"), ОбъектМетаданных.ПолноеИмя());
...
Процедура ВызватьИсключениеНеУдалосьОпределитьПоле(Знач ИмяПоля, Знач ИмяОбъекта)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Не удалось построить запрос получения значения поля %1 объекта мтеданных %2!'"),
		ИмяПоля, ИмяОбъекта
	);
