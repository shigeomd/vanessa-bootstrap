...
//
// Описание формата процедур-обработчико см. в описании функции
...
// Выполнение команды сервервиса ИПП на стороне сервера 1С:Предприятия.
// Параметры:
...
//		на найденый по отпечатку сертификат;
//	Неопределено - если сертификат не найден по отпечатку.
//
Функция НайтиСертификатПоОтпечатку(Отпечаток) Экспорт
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	ДвоичныеДанныеСертификата = Подключение1СТакском.ДвоичныеДанныеСертификата(СертификатЭП);
...
Процедура ПодготовитьДанныеСертификатаПоОтпечатку(КСКонтекст, КонтекстОбработчика)
	...
	Если ОтпечатокСертификата <> Неопределено Тогда
		СертификатЭП = Подключение1СТакском.НайтиСертификатПоОтпечатку(ОтпечатокСертификата);
	КонецЕсли;
