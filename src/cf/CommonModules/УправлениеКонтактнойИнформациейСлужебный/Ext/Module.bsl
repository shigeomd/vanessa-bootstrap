...
//      Структура - описание населенного пункта.
//
Функция СписокРеквизитовУлицы(Идентификатор = Неопределено, ВариантКлассификатора = "КЛАДР") Экспорт
...
//
Процедура ЗаполнитьИдентификаторыЧастейАдреса(НаселенныйПунктДетально) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		МодульАдресныйКлассификаторСлужебный.УстановитьИдентификаторыЧастейАдреса(НаселенныйПунктДетально);
	КонецЕсли;
...
//
Функция ВерсияПоставщикаДанныхКлассификатора() Экспорт
...
Функция ПроверитьАдресВXML(АдресВXML, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Иначе 
		...
		ПараметрыПроверки.ЗапрещатьВводНекорректного = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСписокАвтоподбораИзТаблицы(Результат, ТабличныеДанные, ЭтоПодборУлицы)
	...
	Для Каждого Строка Из ТабличныеДанные Цикл
		...
		Если Строка.Неактуален Тогда
			Предупреждение = СтрЗаменить(ПредупреждениеНеактуальности, "%1", Представление);
			Пометка        = Истина;
...
//
Функция ОпределитьПочтовыйИндексАдреса(XDTOАдрес, Идентификатор = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		МодульАдресныйКлассификаторСлужебный = ОбщегоНазначения.ОбщийМодуль("АдресныйКлассификаторСлужебный");
		Индекс = МодульАдресныйКлассификаторСлужебный.ИндексАдресаПоЧастямАдреса(XDTOАдресРФ, Идентификатор);
	КонецЕсли;
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации) Экспорт
	...
	Если ПустаяСтрока(Индекс) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.XPathПочтовогоИндекса(),
	...
	Если ИсключатьПроверкуДомаВАдресе(АдресРФ) Тогда
		...
		Если ЗданиеНеУказано Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, 
				УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.XPathНомераДополнительногоОбъектаАдресации("Дом"),
	...
	Если Не ПустаяСтрока(Район) Тогда
		Поле = УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.XPathРайона();
	...
	Если ЗначениеЗаполнено(ДополнительныйЭлемент) Тогда
		...
		Если ПустаяСтрока(УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(ДополнительныйЭлемент)) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru = 'Не указано сокращение у дополнительного элемента ""%1"".'"), ДополнительныйЭлемент
				), НСтр("ru = 'Улица'"));
		КонецЕсли;
		Если СтрДлина(Район) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru = 'Название дополнительного элемента ""%1"" должно быть короче 50 символов.'"), ДополнительныйЭлемент
		...
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(ДополнительныйЭлемент, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
					НСтр("ru = 'В названии дополнительного элемента ""%1"" есть не кириллические символы.'"), ДополнительныйЭлемент
				), НСтр("ru = 'ДополнительныйЭлемент'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Индекс) Тогда
		Поле = УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.XPathПочтовогоИндекса();
...
Функция СтрокаСоставаКонтактнойИнформации(Знач Текст, Знач НовоеЗначение = Неопределено) Экспорт
	...
	ОбъектXDTO= ФабрикаXDTO.ПрочитатьXML(Чтение, 
		ФабрикаXDTO.Тип(УправлениеКонтактнойИнформациейКлиентСерверПовтИсп.ПространствоИмен(), "КонтактнаяИнформация"));
...
Функция XSLT_УникальныеПоПредставлениюВСписке(Знач Текст)
	...
	Преобразователь = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.XSLT_УникальныеПоПредставлениюВСписке();
...
Функция XSLT_СтрокаКлючЗначениеВСписокЗначений(Знач Текст)
	...
	Преобразователь = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.XSLT_СтрокаКлючЗначениеВСписокЗначений();
	Возврат Преобразователь.ПреобразоватьИзСтроки(XSLT_УзелСтрокиПараметра(Текст));
...
Функция XSLT_СписокЗначенийВСтрокуКлючЗначение(Знач Текст)
	...
	Преобразователь = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.XSLT_СписокЗначенийВСтрокуКлючЗначение();
	Возврат Преобразователь.ПреобразоватьИзСтроки(Текст);
...
Функция XSLT_СтруктураВСтрокуКлючЗначение(Знач Текст)
	...
	Преобразователь = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.XSLT_СтруктураВСтрокуКлючЗначение();
	Возврат Преобразователь.ПреобразоватьИзСтроки(Текст);
...
Функция XSLT_СтруктураВСписокЗначений(Текст)
	...
	Преобразователь = УправлениеКонтактнойИнформациейСлужебныйПовтИсп.XSLT_СтруктураВСписокЗначений();
	Возврат Преобразователь.ПреобразоватьИзСтроки(Текст);
