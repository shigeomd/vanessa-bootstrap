	...
	Иначе
		...
		Иначе
			...
			БанковскийСчет.Наименование = Лев(
				...
				+ ?(БанковскийСчет.Валютный, ", " + БанковскийСчет.ВалютаДенежныхСредств , ""),
				100);
		КонецЕсли;
	КонецЕсли;
...
Функция АдминистраторНалогаОрганизации(Налог, Организация, Период = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(ВидНалоговогоОргана) Тогда
		...
		СписокПоказателей.Добавить(ВидНалоговогоОргана);
		
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Организация, Период, СписокПоказателей);
		
		СведенияОбОрганизации.Свойство(СписокПоказателей[0], КодГосударственногоОргана);
