...
//     (ФункциональнаяОпция1Отчета ИЛИ ФункциональнаяОпция2Отчета) И (ФункциональнаяОпция3Варианта ИЛИ
//     ФункциональнаяОпция4Варианта).
...
//     но надо помнить, что эти функциональные опции будут действовать только для предопределенных вариантов этого
//     отчета.
...
Процедура ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		ДополнительныеПараметры.Свойство("СтандартнаяОбработка", СтандартнаяОбработка);
		ДополнительныеПараметры.Свойство("УзелПланаОбмена",      УзелПланаОбмена);
		ДополнительныеПараметры.Свойство("НайденныеЗапреты",     НайденныеЗапреты);
	КонецЕсли;
	СтандартнаяОбработка = СтандартнаяОбработка <> Ложь;
	...
	ИначеЕсли НЕ ИсточникРегистр
	        И НЕ Источник.ЭтоНовый()
	        И НЕ Удаление Тогда
		
		Если ИзменениеИлиЗагрузкаЗапрещена(
				?(СтандартнаяОбработка <> "СтараяВерсия", Источник, Источник.Метаданные().ПолноеИмя()),
				?(СтандартнаяОбработка <> "НоваяВерсия",  Источник.Ссылка, Неопределено),
				СообщитьОЗапрете,
	...
	ИначеЕсли ИсточникРегистр И Замещение Тогда
		...
		Если ИзменениеИлиЗагрузкаЗапрещена(
				?(СтандартнаяОбработка <> "СтараяВерсия", Источник, Источник.Метаданные().ПолноеИмя()),
				?(СтандартнаяОбработка <> "НоваяВерсия",  Источник.Отбор, Неопределено),
				СообщитьОЗапрете,
				СтандартнаяОбработка <> Ложь,
				УзелПланаОбмена,
				НайденныеЗапреты) Тогда
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("УдалениеОбъекта") Тогда
		...
		Если СтандартнаяОбработка <> "НоваяВерсия"
		   И ИзменениеИлиЗагрузкаЗапрещена(
	...
	Иначе
		...
		Если СтандартнаяОбработка <> "СтараяВерсия"
		   И ИзменениеИлиЗагрузкаЗапрещена(
				Источник,
				,
				СообщитьОЗапрете,
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		ДополнительныеПараметры.Вставить("СтандартнаяОбработка", СтандартнаяОбработка <> Ложь);
		ДополнительныеПараметры.Вставить("УзелПланаОбмена",      УзелПланаОбмена);
		ДополнительныеПараметры.Вставить("НайденныеЗапреты",     НайденныеЗапреты);
	КонецЕсли;
...
Функция ПропуститьПроверкуДатЗапрета(Источник,
	...
	Если ТипЗнч(СтандартнаяОбработка) <> Тип("Булево")
	   И СтандартнаяОбработка <> "НоваяВерсия"
	   И СтандартнаяОбработка <> "СтараяВерсия" Тогда
		ВызватьИсключение
			НСтр("ru = 'Некорректное значение параметра ПроверкаЗапретаИзменения указано в процедуре
			           |ПередПроверкойЗапретаИзменения общего модуля ДатыЗапретаИзмененияПереопределяемый.'");
	КонецЕсли;
	
	Возврат СтандартнаяОбработка = Ложь // НЕ проверять запрет изменения.
...
Функция РассчитатьДатуЗапретаПоОписанию(ТекущаяДатаНаСервере, ОписаниеДатыЗапрета)
	...
	Если ЗначениеЗаполнено(ОписаниеДатыЗапрета) Тогда
		...
		Если ЗначениеЗаполнено(Строка2) Тогда
			...
			КоличествоДнейРазрешения = ОписаниеТипа.ПривестиЗначение(Строка2);
		КонецЕсли;
	КонецЕсли;
