...
// Выполнение команды сервервиса ИПП на стороне клиента 1С:Предприятия.
...
//	КонтекстОбработчика - см. опсиание функции
...
Процедура РасшифроватьКонтрольныйМаркерЭПСервера(
	...
	// Получение необходимых сессионных параметров для выпонения операции расшифрования
	ПараметрыДляРасширования = ИнтернетПоддержкаПользователейКлиентСервер.СессионныеПараметрыДляРасшифрования(
		КонтекстВзаимодействия.КСКонтекст);
	
	Если ПараметрыДляРасширования.markerED = Неопределено Тогда
	...
	Попытка
		ДвоичныеДанныеМаркера = Base64Значение(ПараметрыДляРасширования.markerED);
	Исключение
	...
	Если ЗначениеЗаполнено(ПараметрыДляРасширования.IDCertificateED_Dop) Тогда
		
		// Ссылка дополнительного сертификата
		СертификатЭП = ПараметрыДляРасширования.IDCertificateED_Dop;
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыДляРасширования.IDCertificateED) Тогда
		
		// Ссылка основного сертификата
		СертификатЭП = ПараметрыДляРасширования.IDCertificateED;
...
Процедура РасшифрованиеЗавершение(РезультатРасшифрования, ДопПараметры) Экспорт
	...
	Если РезультатРасшифрования.Успех Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗаписатьПараметрКонтекста(
			...
			"openMarkerED",
			РезультатРасшифрования.РасшифрованныеДанные,
			"sessionParameter");
