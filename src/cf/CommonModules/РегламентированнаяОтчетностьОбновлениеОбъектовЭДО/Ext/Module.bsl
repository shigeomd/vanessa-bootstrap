...
Процедура ЗаполнитьАдресЭлектроннойПочтыФСГС() Экспорт
	
	Запрос = Новый Запрос();
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СерверыДокументооборота.Ссылка,
	|	СерверыДокументооборота.АдресЭлектроннойПочтыФСГС,
	|	СерверыДокументооборота.УдалитьАдресЭлектроннойПочтыФСГС
	|ИЗ
	|	Справочник.СерверыДокументооборота КАК СерверыДокументооборота
	|ГДЕ
	|	СерверыДокументооборота.УдалитьАдресЭлектроннойПочтыФСГС <> """"
	|	И СерверыДокументооборота.АдресЭлектроннойПочтыФСГС = """"";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СерверыДокументооборотаОбъект = Выборка.Ссылка.ПолучитьОбъект();
		СерверыДокументооборотаОбъект.АдресЭлектроннойПочтыФСГС = Выборка.УдалитьАдресЭлектроннойПочтыФСГС;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(СерверыДокументооборотаОбъект);
	КонецЦикла;
...
Процедура КонвертироватьКонстанты()
	...
	ЗначениеКонстанты = Константы.УдалитьДокументооборотСКонтролирующимиОрганами_ОтпечатокСертификатаСервераОнлайнПроверки.Получить();
	Константы.ДокументооборотСКонтролирующимиОрганами_ОтпечатокСертификатаСервераОнлайнПроверки.Установить(ЗначениеКонстанты);
