...
Процедура СоздатьВТБухучетНачислений(Организация, ПериодРегистрации, ПроцентЕНВД, МенеджерВременныхТаблиц, ДополнительныеПараметры) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВидыРасчета.ВидОперацииПоЗарплате В (&РасходыПоСтрахованию)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ВЫБОР
	|		КОГДА СписокНачислений.РасходыПоСтрахованию
...
Функция ТекстЗапросаДанныеДокументаОбработкаДокументовОтражениеЗарплатыВБухучете() Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.БухучетХозрасчетныхОрганизаций") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("БухучетХозрасчетныхОрганизацийБазовый");
...
Процедура ДополнитьНачисленияДаннымиОЕНВД(ДанныеДляПроведения, ДокументСсылка, МесяцНачисления, Организация, ПроверятьЕНВД = Ложь) Экспорт
	...
	ДополнительныеПараметры = ОтражениеЗарплатыВБухучете.ДополнительныеПараметрыДляРаспределенияЕНВД();
	ДополнительныеПараметры.НачисленияПоСотрудникамСРаспределениемПоСтатьям = ДанныеДляПроведения.НачисленияПоСотрудникам;
	ДополнительныеПараметры.МенеджерВременныхТаблиц  = ДанныеДляПроведения.МенеджерВременныхТаблиц;
	ДополнительныеПараметры.ПараметрыДляОтраженияВБухучете.ДокументСсылка = ДокументСсылка;
	ДополнительныеПараметры.ПроверятьЕНВД = ПроверятьЕНВД;
	
	ОтражениеЗарплатыВБухучете.СоздатьВТНачисленияСДаннымиЕНВД(Организация, МесяцНачисления, ДокументСсылка, ДополнительныеПараметры);
...
Функция ОстаткиОтпусковДляРасчетаОценочныхОбязательств(Организация, Период) Экспорт
	...
	Возврат ОтражениеЗарплатыВБухучете.НоваяТаблицаОстаткиОтпусков();
