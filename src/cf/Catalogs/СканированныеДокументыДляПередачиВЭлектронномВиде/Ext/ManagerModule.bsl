...
Функция ЗагрузитьИзВнешнегоИсточникаПоСтруктуреРезультатаВыбора(СтруктураРезультата) Экспорт
	...
	Для каждого СтрокаТЗЗагруженныеДокументы Из ТЗЗагруженныеДокументы Цикл
		...
		Для каждого СтрокаТЗФайлыДокументов Из МассивФайловДокумента Цикл
			...
			ПрисоединенныйФайлСсылка = ПрисоединенныеФайлы.ДобавитьФайл(ОбъектСсылка, СтрокаТЗФайлыДокументов.Имя, , , , АдресВременногоХранилищаФайла);
...
Процедура ЗаполнитьКонтрагента() 
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение Контрагент - реквизита ТЧ справочника СканированныеДокументыДляПередачиВЭлектронномВиде'"), 
...
Процедура ЗаполнитьРолиУчастников() 
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Исключение
			...
			// Пишем в журнал
			ОписаниеОшибки = ИнформацияОбОшибке().Описание;
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение Роль - реквизита ТЧ справочника СканированныеДокументыДляПередачиВЭлектронномВиде'"), 
				УровеньЖурналаРегистрации.Предупреждение, , , ОписаниеОшибки);
			Продолжить;	
		КонецПопытки;
		...
		Пока ВыборкаДокумент.Следующий() Цикл
			...
			Если Направление <> Неопределено Тогда
				...
				Исключение
					...
					// Пишем в журнал
					ОписаниеОшибки = ИнформацияОбОшибке().Описание;
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение реквизита Направление справочника СканированныеДокументыДляПередачиВЭлектронномВиде'"), 
					УровеньЖурналаРегистрации.Предупреждение, , , ОписаниеОшибки);
					Продолжить;
