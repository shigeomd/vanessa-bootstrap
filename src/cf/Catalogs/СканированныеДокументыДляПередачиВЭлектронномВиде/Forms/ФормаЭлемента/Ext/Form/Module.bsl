...
&НаКлиенте
Перем НеЗадаватьВопросПередЗакрытием;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если РежимПодготовки Тогда
		...
		//Стандартной обработкой является проверка модифицированности, задание вопроса о  сохранении и сохранение объекта
		СтандартнаяОбработка = Ложь;
		
		Если НеЗадаватьВопросПередЗакрытием <> Истина Тогда
			
			Отказ = Истина;
			
			ТекстВопроса = НСтр("ru = 'Завершить подготовку документов?
...
Функция ПолучитьМассивИменФайловИзЗначенияПеретаскивания(ЗначениеПеретаскивания)
	...
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Файл") Тогда
		...
		ИЛИ ЗначениеПеретаскивания.Расширение = ".tif" Тогда
		...
		Для каждого ЭлМассива Из ЗначениеПеретаскивания Цикл
			Если ТипЗнч(ЭлМассива) = Тип("Файл") Тогда
				Если ЭлМассива.Расширение = ".jpg" 
					ИЛИ ЭлМассива.Расширение = ".tif" Тогда
...
Процедура ДобавитьВложенияПослеПолученияСвойствФайлов(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СвойстваФайла Из Результат.СвойстваФайлов Цикл
		МассивФайлов.Добавить(Новый Структура("Имя, ПолноеИмя, Расширение, Размер, АдресДанных", СвойстваФайла.Имя, СвойстваФайла.ПолноеИмя, СвойстваФайла.Расширение, СвойстваФайла.Размер));
	КонецЦикла;
...
Процедура ПолучитьСвойстваФайлаПоследовательно(ОповещениеОЗавершении, МассивСвойствФайлов, МассивПолныхИменФайлов, ПорядковыйНомерФайла)
	...
	Если ПорядковыйНомерФайла <= МассивПолныхИменФайлов.ВГраница() Тогда
		...
		ДополнительныеПараметры = Новый Структура("ОповещениеОЗавершении, МассивСвойствФайлов, МассивПолныхИменФайлов, ПорядковыйНомерФайла", ОповещениеОЗавершении, МассивСвойствФайлов, МассивПолныхИменФайлов, ПорядковыйНомерФайла);
		...
		ДокументооборотСКООперацииСФайламиКлиент.ПолучитьСвойстваФайла(ОписаниеОповещения, ТекущийФайлПолноеИмя);
		Возврат;
	...
	Для каждого СтрокаТаблицыФайлов Из ТаблицаФайлов Цикл
		...
		Если СтрокаТаблицыФайлов.АдресДанныхФайлаБазы <> СтрокаТаблицыФайлов.АдресДанныхФайлаФормы Тогда
			...
			Иначе
				...
				ПрисоединенныйФайлСсылка = ПрисоединенныеФайлы.ДобавитьФайл(Объект.Ссылка, СтрокаТаблицыФайлов.ИмяФайла, , , , СтрокаТаблицыФайлов.АдресДанныхФайлаФормы);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбновитьРазмерыДокумента()
	...
	Иначе
		...
		СодержимоеТекста = "Можно добавить еще " + СформироватьСтрокуПредставленияФайлов(ДопустимоеКоличествоФайлов) + " (" + ДопустимоеКоличествоМегабайт + " Мб).";
...
Процедура ОткрытьВложение()
	...
	ДокументооборотСКООперацииСФайламиКлиент.ОткрытьФайл(ТекДанные.АдресДанныхФайлаФормы, ТекДанные.ИмяФайла);
...
Процедура УправлениеЭУДокументИсточник()
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ВидДокументаФНС) Тогда
			...
			Элементы.ГруппаИсточник.Доступность = Истина;
		Иначе
			...
			Если МассивДоступныхВидовДокументовИсточников.Найти(Объект.ВидДокумента) = Неопределено Тогда
				...
				Элементы.ГруппаИсточник.Доступность = Ложь;
...
Процедура ЗаписатьИПерейтиКСледующему(Команда)
	...
	Если Записать() Тогда
		НеЗадаватьВопросПередЗакрытием = Истина;
		Закрыть(Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗавершитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
