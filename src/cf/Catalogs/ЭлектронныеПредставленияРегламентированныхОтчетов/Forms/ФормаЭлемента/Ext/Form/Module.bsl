	...
	Если СвойстваФайлов.Количество() > 0 Тогда
		Если Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС") Тогда
			Для Каждого СвойстваФайла Из СвойстваФайлов Цикл
				Если Найти(СвойстваФайла.ИмяФайла, "NO_NDS_") Тогда
...
Процедура ОткрытьПриложениеКБухгалтерскойОтчетности(ТипФайлаБухгалтерскойОтчетности)
	...
	ДокументооборотСКООперацииСФайламиКлиент.ОткрытьФайл(АдресДанных, ИмяФайлаПриложения);
...
&НаСервере
Функция ИмпортироватьФайлыНаСервере(МассивФайлов, ВыполнитьРазделениеФайла, ВыполнятьПроверкуКомплектаУведомлений)
	...
	Иначе
		...
		Иначе
			...
			Для Каждого ФайлОтчета Из ФайлыОтчетов Цикл
				Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиДекларацияНДС(ФайлОтчета.Имя) Тогда
	...
	Если Найти(ОсновнойФайл.Имя, "NO_NDS") Тогда
		// декларация НДС 2015
...
Функция ОпределитьТипФайлаНДС(ПолноеИмяФайла)
	...
	Если Найти(ИмяФайла, "NO_NDS_") = 1 Тогда
		Возврат Перечисления.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности;
	ИначеЕсли Найти(ИмяФайла, "NO_NDS.") = 1 Тогда
...
Функция ЭтоФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Префикс = нрег(Лев(КороткоеИмяФайла, 15));
	Возврат (Найти(Префикс, "рсч_") > 0 ИЛИ Найти(Префикс, "мсп_") > 0 ИЛИ Найти(Префикс, "2тпо_") > 0);
	...
	#Иначе
		...
		Если РазмерФайла > МаксимальноДопустимыйРазмерОтображаемогоТекста() Тогда
			Если БольшеНеПоказыватьПредупреждениеОБольшомРазмереФайла() <> Истина Тогда
				...
				ОткрытьФорму(
					КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПредупреждениеОБольшомРазмереФайла", СтруктураПараметров,,,,, ОписаниеОповещения);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	#КонецЕсли
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = 
		Форма.СвойстваФайлов.Количество() > 1 ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС");
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС")
		И Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НДС")
		И ЗагружаетсяНесколькоФайлов Тогда
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаКомлектаУведомлений; 
	Иначе	
