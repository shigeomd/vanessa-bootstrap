...
Процедура ПроверитьНаСторонеКлиентаЦиклНачало(Контекст)
	...
	Попытка
		ЭлектроннаяПодписьПереопределяемыйКлиент.ПриДополнительнойПроверкеСертификата(ПараметрыВыполнения);
	Исключение
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере, ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь)
	...
	Попытка
		Значение = ЭлементКартинка.Картинка;
	Исключение
		Форма.УстановитьЭлементЧерезСервер(НачалоЭлемента, НаСервере, ОписаниеОшибки, ЭтоОшибка);
		Возврат;
	КонецПопытки;
...
&НаСервере
Процедура УстановитьЭлементЧерезСервер(НачалоЭлемента, НаСервере, ОписаниеОшибки, ЭтоОшибка)
	
	УстановитьЭлемент(ЭтотОбъект, НачалоЭлемента, НаСервере, ОписаниеОшибки, ЭтоОшибка);
