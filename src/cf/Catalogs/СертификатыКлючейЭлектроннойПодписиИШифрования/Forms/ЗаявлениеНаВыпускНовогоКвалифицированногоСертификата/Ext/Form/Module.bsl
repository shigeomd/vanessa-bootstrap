...
Процедура ВыгрузитьЗапросНаСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьЗапросНаСертификат(, АдресЗапросаНаСертификат,
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(, АдресКорневогоСертификата,
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСКонтрагентами") Тогда
		...
		Если Не МодульРегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, БИК) Тогда
...
Процедура ПроверитьСуществованиеКонтейнераКлючейПослеПолученияИменКонтейнеров(ИменаКонтейнеровКлючей, Контекст) Экспорт
	...
	Позиция = Найти(Контекст.ПутьПослеСоздания, ":.");
	Если Позиция = 0 Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, НайтиБезУчетаРегистраИРазделителяПути(
			ИменаКонтейнеровКлючей, Контекст.ИмяДляСоздания) > 0);
		Возврат;
...
Функция НайтиБезУчетаРегистраИРазделителяПути(Строка, ПодстрокаПоиска)
	...
	Возврат Найти(НРег(СтрЗаменить(Строка, "/", "\")), НРег(СтрЗаменить(ПодстрокаПоиска, "/", "\")));
