...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		// Проверка необходимости очистки повторно используемых значений
		Если НЕ ХарактерДеятельности = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьХарактерДеятельности(Ссылка)
			ИЛИ НЕ НоменклатурнаяГруппа = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьНоменклатурнуюГруппу(Ссылка) Тогда
