...
Процедура ПоместитьОписанияФайловКомплектаВМассив(МассивОписанийФайловВыгрузки, УникальныйИдентификатор) Экспорт
	...
	ПрисоединенныеФайлы.ПолучитьПрикрепленныеФайлыКОбъекту(Ссылка, ДополнительныеФайлы);
			
	Для Каждого Файл Из ДополнительныеФайлы Цикл
		ДанныеФайлаДокумента = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Файл, УникальныйИдентификатор, Истина);
...
Процедура ПроверитьДанныеКомплекта(Ошибки, Отказ = Ложь) Экспорт
	...
	Если Год(ОтчетныйПериод) >= 2012 Тогда
		...
		ИначеЕсли Год(ОтчетныйПериод) >= 2014 Тогда 
			...
			ПроверитьСоответсвиеИсходныхИКорректируемыхСведений(Отказ);
		КонецЕсли;	
	КонецЕсли;	
	...
	ИначеЕсли ДанныеРСВ <> Неопределено И ДанныеРСВ.ФормаРеглОтчетаДействуетВТекущемОтчетномПериоде Тогда	
		...
		Если ДанныеРСВ.НачисленоРСВ1НаСтраховуюПоКатегориям[Перечисления.КатегорииЗастрахованныхЛицДляПФР.НР] <> Неопределено Тогда
			Если ДанныеРСВ.УплаченоРСВ1НаСтраховую + 1 < ДанныеАДВ62.УплаченоСтраховая Тогда
				...
				ПерсонифицированныйУчетКлиентСервер.ДобавитьРазделИтоговРСВ(РазделыИтоговСверки, ДанныеРСВ.Ссылка, "УплаченоСтраховая");
		...
		Если Разность > ДопустимоеРасхождение Тогда
			...
			Если Разность > ДопустимоеРасхождение Тогда
				...
				ПерсонифицированныйУчетКлиентСервер.ДобавитьРазделИтоговРСВ(РазделыИтоговСверки, ДанныеРСВ.Ссылка, "УплаченоНакопительная");
...
Процедура ПроверитьСоответсвиеИсходныхИКорректируемыхСведений(Отказ)
...
Функция СоответствиеКатегорий()
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	ПерсонифицированныйУчет.СформироватьВТСоответствиеТарифаИВидаЗЛКатегории(МенеджерВТ);
	...
	Для каждого СтрокаЗастрахованного Из МассивЗастрахованных Цикл
       ...
       Для каждого ЗаписьОДоходе Из СтрокиДоходаВредника Цикл
		...
		Если ДокументПачка.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ Тогда
			...
			Для Каждого СтрокаКорректироки Из СтрокиКорректировок Цикл
				НоваяСтрока = ДокументПачка.КорректирующиеСведения.Добавить();
				НоваяСтрока.Сотрудник = ФизическоеЛицо;
				
				Если ВРег(ПерсонифицированныйУчет.ЗначениеXDTOИзДерева(СтрокаКорректироки, "ТипСтроки")) = "ИТОГ" Тогда
					Продолжить;
				КонецЕсли;
				
				Год = ПерсонифицированныйУчет.ЗначениеXDTOИзДерева(СтрокаКорректироки, "Год");
				Квартал = ПерсонифицированныйУчет.ЗначениеXDTOИзДерева(СтрокаКорректироки, "Квартал");
