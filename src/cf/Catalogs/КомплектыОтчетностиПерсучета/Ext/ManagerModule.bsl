...
Функция АктивынйОтчетныйПериод(Организация, ИсходныеСведения = Истина) Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Приоритет = 1 Тогда
			АктивынйОтчетныйПериод = ТекущийОтчетныйПериод;
		Иначе
			АктивынйОтчетныйПериод = ПредыдущийОтчетныйПериод;
		КонецЕсли;
	Иначе
	...
	Возврат АктивынйОтчетныйПериод;
...
Процедура ЗаполнитьПараметрыФормированияКорректирующейФормы(СтруктураПараметов)
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", СтруктураПараметов.Организация);
	Запрос.УстановитьПараметр("ОтчетныйПериод", СтруктураПараметов.ОтчетныйПериод);
	...
	НомерПачки = РегламентированнаяОтчетность.ПолучитьНомерПачкиВыгруженныхФайловПФР(СтруктураПараметов.Организация, Год(СтруктураПараметов.ОтчетныйПериод));
	
	КорректируемыеПериоды = Новый Соответствие;
	
	Пока Выборка.Следующий() Цикл
		СкопированныйДокумент = Выборка.ПачкаДокументов.ПолучитьОбъект().Скопировать();	
		//СкопированныйДокумент.УстановитьНовыйНомер();
		...
		СтрокаТаблицы = СтруктураПараметов.СписокПачекСЗВ.Добавить();
	...
	Для Каждого Период Из КорректируемыеПериоды Цикл
		СтрокаТаблицы = СтруктураПараметов.КорректируемыеПериоды.Добавить();
	...
	СтруктураПараметов.УчитыватьИсправления = Истина;
	СтруктураПараметов.ФормироватьИсходныеСведенияВКорректирующейФорме = Истина;
КонецПроцедуры	
...
Функция ПараметрыФормированияКорректирующихСведений() Экспорт
	...
	СтруктураПараметров.Вставить("ФормироватьКорректирующуФормуРСВ", Ложь);
...
Процедура ДополнитьИсходныеСведенияЗастрахованнымиЛицамиИзКоррСведений(Организация, ОтчетныйПериод, ДокументыКомплекта)
	...
	Пока Выбока.Следующий() Цикл
		СписокФизическихЛиц.Добавить(Выбока.ФизическоеЛицо);			
	КонецЦикла;	
...
Процедура ПоместитьКоорректирующиеСведенияВАктивныйКомплектПериоды(АдресХранилища) Экспорт
	КорректирующиеСведения = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	Если ТипЗнч(КорректирующиеСведения) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеАктивногоКомплекта = ОписаниеАктивногоКомплектаПериода(
									КорректирующиеСведения.Организация, 
									КорректирующиеСведения.ОтчетныйПериод,
									Ложь);
									
	Если ОписаниеАктивногоКомплекта.СостояниеКомплекта = Перечисления.СостояниеКомплектаОтчетностиПерсучета.СведенияСформированы Тогда
		КомплетОбъект = ОписаниеАктивногоКомплекта.Ссылка.ПолучитьОбъект();
	Иначе
		КомплетОбъект = НовыйКомплектСведений(КорректирующиеСведения.Организация, КорректирующиеСведения.ОтчетныйПериод);
		КомплетОбъект.СостояниеКомплекта = Перечисления.СостояниеКомплектаОтчетностиПерсучета.СведенияСформированы;
	КонецЕсли;	
	
	КомплетОбъект.ПоместитьКорректирующиеСведения(КорректирующиеСведения);
	
	КомплетОбъект.Записать();
КонецПроцедуры	
...
Функция НулевойКомплектСведений(Организация, ОтчетныйПериод, КорректируемыеДокументы, ПериодФормирования, СписокФизическихЛиц = Неопределено)
	...
	ВыбокаНулевыхКомплектов = Запрос.Выполнить().Выбрать();
	
	Если ВыбокаНулевыхКомплектов.Следующий() Тогда
		Комплект = ВыбокаНулевыхКомплектов.Ссылка.ПолучитьОбъект();		
...
Функция СоздатьКорректирующиеПачкиСЗВЗаПериоды(Организация, ОтчетныйПериод, КорректируемыеПериоды, ДокументыКомплекта, УчитыватьИсправления = Ложь)
	...
	СоздатьВТЗарегистрированныеКорректирокиВзносов(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод);
	...
	Для Каждого ОписаниеКорректируемогоПериода Из КорректируемыеПериоды Цикл
		...
		Если ОтменяющиеДокументыПериода.Количество() > 0 Тогда
			...
			// Данные обновленных отменяющих документов мы будем учитывать при полученние данных для корректирующих документов.
		...
		// Получим алгоримты получения данных для корректирующих сведений в зависимости от варианта формирования корр.
		// седений.
		ИсточникиДанных = ПерсонифицированныйУчет.ИсточникиДанныхКвартальнойОтчетностиПоВариантуФормирования(
...
Процедура ДополнитьОписаниеКорректируемыхПериодов(МенеджерВременныхТаблиц, КорректируемыеПериоды)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗарегистрированныеКорректирокиВзносов.КорректируемыйПериод
	|ИЗ
	|	ВТЗарегистрированныеКорректирокиВзносов КАК ЗарегистрированныеКорректирокиВзносов
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура СоздатьВТЗарегистрированныеКорректирокиВзносов(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод)
	...
	|ПОМЕСТИТЬ ВТЗарегистрированныеКорректирокиВзносов
	|ИЗ
...
Функция СписокФизическихЛицСЗарегистрированнымиКорректировками(МенеджерВременныхТаблиц, КорректируемыйПериод)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗарегистрированныеКорректирокиВзносов.ФизическоеЛицо КАК ФизическоеЛицо
	|ИЗ
	|	ВТЗарегистрированныеКорректирокиВзносов КАК ЗарегистрированныеКорректирокиВзносов
	|ГДЕ
	|	ЗарегистрированныеКорректирокиВзносов.КорректируемыйПериод = &КорректируемыйПериод
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ЗаполнитьДанныеРСВ_1(ДанныеКомплекта)
	...
	Если Не ЗначениеЗаполнено(ДанныеКомплекта.РСВ_1)
		...
		Если ДанныеКомплекта.ОтчетныйПериод = '20150101' Тогда
			Если ДанныеКомплекта.СпециальныйДокумент2015года Тогда
				ИмяФормыОтчета = "ФормаОтчета2014Кв1";	
			Иначе
				ИмяФормыОтчета = "ФормаОтчета2015Кв1";	
			КонецЕсли;	
		КонецЕсли;	
...
Процедура ОтменитьИсправленияВзносовИЗаработка(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ПараметрыОбора = ПерсонифицированныйУчет.СтруктураОтбораДанныхДляКвартальнойОтчетности();
	ПараметрыОбора.ТипДоговора = СтруктураПараметров.ТипДоговора;
	ПараметрыОбора.КатегорияЗастрахованныхЛиц = СтруктураПараметров.КатегорияЗастрахованныхЛиц;
	ПараметрыОбора.СписокФизическихЛиц = СписокФизическихЛиц;
...
Процедура ОтменитьИсправленияВзносов(Комплект, Организация, ОтчетныйПериод, ТипСведенийСЗВ, ДокументыКомплекта, ПараметрыОбора)
	ВариантыИсточниковДанныхНачисленныхВзносов = ПерсонифицированныйУчет.ВариантыИсточниковДанныхНачисленныхВзносов();
	ВариантыИсточниковДанныхУплаченныхВзносов = ПерсонифицированныйУчет.ВариантыИсточниковДанныхУплаченныхВзносов();
	...
	СформироватьВТССотрудникамиКомплектаПоКатегориям(МенеджерВТ, ДокументыПериода, ПараметрыОбора.СписокФизическихЛиц);
	 
	Запрос.УстановитьПараметр("КатегорияЗастрахованныхЛиц", ПараметрыОбора.КатегорияЗастрахованныхЛиц);
	Запрос.УстановитьПараметр("ТипДоговора", ?(ПараметрыОбора.ТипДоговора = Неопределено, Перечисления.ТипыДоговоровСЗВ63.ПустаяСсылка(), ПараметрыОбора.ТипДоговора));
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура ОтменитьИсправленияЗаработка2013(Организация, ОтчетныйПериод, ТипСведенийСЗВ, ДокументыКомплекта, ПараметрыОбора)
	ВариантыИсточниковДанныхСведенийОЗаработке = ПерсонифицированныйУчет.ВариантыИсточниковДанныхСведенийОЗаработке();
...
Процедура ОтменитьИсправленияЗаработка2014(Организация, ОтчетныйПериод, ТипСведенийСЗВ, ДокументыКомплекта, ПараметрыОбора)
	ВариантыИсточниковДанныхСведенийОЗаработке = ПерсонифицированныйУчет.ВариантыИсточниковДанныхСведенийОЗаработке();
	...
	Запрос.УстановитьПараметр("СписокФизическихЛиц", ПараметрыОбора.СписокФизическихЛиц);
...
Функция ВыгрузитьФайлыВоВременноеХранилище(Ссылка, УникальныйИдентификатор = Неопределено) Экспорт  
	...
	ПолучаемыеРеквизиты = "РСВ_1, ОписьПачекСЗВ, ВедомостьУплатыАДВ11, СписокПачекСЗВ, СписокПачекСЗВ6_3, АдресФайлаВыгрузкиРСВ, ИмяФайлаВыгрузкиРСВ, НулевойКомплект, ОтчетныйПериод";
	ДанныеКомплекта = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Ссылка, ПолучаемыеРеквизиты);   
...
Функция ОбъединитьТабличныеДокументы(ПечатаемыеОбъекты) Экспорт
	...
	Для Каждого ПечатуемыйОбъект Из ПечатаемыеОбъекты Цикл
...
Процедура ДобавитьТабличныйДокументВКоллекциюПечатаемыхОбъектов(КоллекцияПечатаемыхОбъектов, ТабличныйДокумент, ОписаниеПечатуемогоОбъекта = Неопределено) Экспорт
	ПечатуемыйОбъект = РегламентированнаяОтчетность.ПустаяСтруктураДляРазделовФормыРСВ_1();
	...
	КоллекцияПечатаемыхОбъектов.Добавить(ПечатуемыйОбъект);	
КонецПроцедуры	
...
Функция ТекстОписанияДокументаРаздела6(ОписаниеПечатуемогоОбъекта)
	...
	Если ОписаниеПечатуемогоОбъекта <> Неопределено Тогда 
		Если ТипЗнч(ОписаниеПечатуемогоОбъекта.Ссылка) = Тип("ДокументСсылка.ПачкаДокументовСЗВ_6_1") Тогда
			ТекстОписания = НСтр("ru = 'Форма СЗВ-6-1'");		
		ИначеЕсли ТипЗнч(ОписаниеПечатуемогоОбъекта.Ссылка) = Тип("ДокументСсылка.РеестрСЗВ_6_2") Тогда
