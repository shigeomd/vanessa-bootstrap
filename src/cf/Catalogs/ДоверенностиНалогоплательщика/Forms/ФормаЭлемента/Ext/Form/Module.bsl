...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриИзмененииОрганизации(Ложь, Истина);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоНовый И ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервераПовтИсп.РазделениеВключено() Тогда
		
		// даем выбрать тип доверенности (для отправки через представителя через сервис или для других целей)
		СписокТиповДоверенности = Новый СписокЗначений;
		СписокТиповДоверенности.Добавить(0, "Доверенность на представителя сервиса");
		СписокТиповДоверенности.Добавить(1, "Прочая доверенность");
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОткрытииЗавершение", ЭтотОбъект);
		СписокТиповДоверенности.ПоказатьВыборЭлемента(ОписаниеОповещения, "Выберите тип доверенности");
...
&НаКлиенте
Процедура ПриОткрытииЗавершение(ТипДоверенности, ДополнительныеПараметры) Экспорт
	
	Если ТипДоверенности = Неопределено Тогда
		Закрыть();
		Возврат;
	КонецЕсли;
	
	Если ТипДоверенности.Значение = 0 Тогда
		ТекстСообщения = "";
		Если НЕ ЗаполнитьДаннымиПоУмолчанию(ТекстСообщения) Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ПредупреждениеПриОткрытииЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПредупреждениеПриОткрытииЗавершение(ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ПриИзмененииОрганизации(ГенерироватьНовыйКод = Истина, ПерерисовыватьФорму = Истина)
	
	Если ГенерироватьНовыйКод Тогда
		СпрОбъект = ДанныеФормыВЗначение(Объект, Тип("СправочникОбъект.ДоверенностиНалогоплательщика"));
		СпрОбъект.СгенерироватьНовыйКод();
		Объект.НомерДовер = СпрОбъект.НомерДовер;
...
Процедура УстановитьВыбранноеЗначениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Найти(РеквизитФормыДляУстановки, "УЛ") > 0 Тогда
...
Функция ИзменитьАдрес(СубъектДоверенности, Элемент, Оповещение = Неопределено)
	...
	Возврат ОбщегоНазначенияКлиентСервер.ОбщийМодуль("УправлениеКонтактнойИнформациейКлиент").ОткрытьФормуКонтактнойИнформации(ПараметрыФормы, , Оповещение);
...
Процедура ПолномочияПредставителяПередНачаломИзменения(Элемент, Отказ)
	...
	ДанныеТекущейСтроки = Новый Структура("ПризнакПолныеПолномочия, Признак01, Признак02, Признак03, Признак04, Признак05, Признак06," +
										...
										"Признак15, Признак16, Признак17, Признак18, Признак19, ОКАТО, КПП");
...
&НаСервере
Функция ЗаполнитьДаннымиПоУмолчанию(ТекстСообщения = "")
	
	Объект.ДатаВыдачи = ТекущаяДатаСеанса();
	Объект.ДатаОкончания = ДобавитьМесяц(ТекущаяДатаСеанса(), 36);
	СтрокаПолномочий = Объект.ПолномочияПредставителя.Добавить();
	СтрокаПолномочий.ПризнакПолныеПолномочия = Истина;
	Объект.ПризнакДоверителя = 1;
	
	// Заполняем доверенность значениями представителя по умолчанию
	СтруктураРеквизитов = СдачаОтчетностиЧерезСервисСпецоператораВызовСервера.ПолучитьСтруктуруРеквизитовДоверенности();
	Если ТипЗнч(СтруктураРеквизитов) = Тип("Структура") Тогда
		...
		Объект.ПредставительЮЛ_НаимОрг = СтруктураРеквизитов.ПредставительЮЛ_Наименование;
		СтрокаАдресаПредставительЮЛ = Объект.Адреса.Добавить();
		СтрокаАдресаПредставительЮЛ.Владелец = СубъектыДоверенностиНалогоплательщика_ПредставительЮЛ;
		СтрокаАдресаПредставительЮЛ.Индекс = СтруктураРеквизитов.ПредставительЮЛ_Индекс;
		СтрокаАдресаПредставительЮЛ.КодРегион = СтруктураРеквизитов.ПредставительЮЛ_Регион;
		СтрокаАдресаПредставительЮЛ.Район = СтруктураРеквизитов.ПредставительЮЛ_Район;
		СтрокаАдресаПредставительЮЛ.Город = СтруктураРеквизитов.ПредставительЮЛ_Город;
		СтрокаАдресаПредставительЮЛ.НаселПункт = СтруктураРеквизитов.ПредставительЮЛ_НаселенныйПункт;
		СтрокаАдресаПредставительЮЛ.Улица = СтруктураРеквизитов.ПредставительЮЛ_Улица;
		СтрокаАдресаПредставительЮЛ.Дом = СтруктураРеквизитов.ПредставительЮЛ_Дом;
		СтрокаАдресаПредставительЮЛ.Корпус = СтруктураРеквизитов.ПредставительЮЛ_Корпус;
		СтрокаАдресаПредставительЮЛ.Кварт = СтруктураРеквизитов.ПредставительЮЛ_Квартира;
		
		ФИОПредставительФЛ = Объект.ФИО.Добавить();
		ФИОПредставительФЛ.Владелец = СубъектыДоверенностиНалогоплательщика_ПредставительФЛ;
		ФИОПредставительФЛ.Фамилия = СтруктураРеквизитов.ПредставительФЛ_Фамилия;
		ФИОПредставительФЛ.Имя = СтруктураРеквизитов.ПредставительФЛ_Имя;
		ФИОПредставительФЛ.Отчество = СтруктураРеквизитов.ПредставительФЛ_Отчество;
		
		СтрокаАдресаПредставительФЛ = Объект.Адреса.Добавить();
		СтрокаАдресаПредставительФЛ.Владелец = СубъектыДоверенностиНалогоплательщика_ПредставительФЛ;
		СтрокаАдресаПредставительФЛ.Индекс = СтруктураРеквизитов.ПредставительФЛ_Индекс;
		СтрокаАдресаПредставительФЛ.КодРегион = СтруктураРеквизитов.ПредставительФЛ_Регион;
		СтрокаАдресаПредставительФЛ.Район = СтруктураРеквизитов.ПредставительФЛ_Район;
		СтрокаАдресаПредставительФЛ.Город = СтруктураРеквизитов.ПредставительФЛ_Город;
		СтрокаАдресаПредставительФЛ.НаселПункт = СтруктураРеквизитов.ПредставительФЛ_НаселенныйПункт;
		СтрокаАдресаПредставительФЛ.Улица = СтруктураРеквизитов.ПредставительФЛ_Улица;
		СтрокаАдресаПредставительФЛ.Дом = СтруктураРеквизитов.ПредставительФЛ_Дом;
		СтрокаАдресаПредставительФЛ.Корпус = СтруктураРеквизитов.ПредставительФЛ_Корпус;
		СтрокаАдресаПредставительФЛ.Кварт = СтруктураРеквизитов.ПредставительФЛ_Квартира;
		
		СтрокаУдостоверенияПредставительФЛ = Объект.УдЛичности.Добавить();
		СтрокаУдостоверенияПредставительФЛ.Владелец = СубъектыДоверенностиНалогоплательщика_ПредставительФЛ;
		//СтрокаУдостоверенияПредставительФЛ.ВидДок = Справочники.ВидыДокументовФизическихЛиц.НайтиПоНаименованию(СтруктураРеквизитов.ПредставительФЛ_ВидДок);
		СтрокаУдостоверенияПредставительФЛ.ВидДок = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ; // временно
		СтрокаУдостоверенияПредставительФЛ.СерДок = СтруктураРеквизитов.ПредставительФЛ_СерДок;
		СтрокаУдостоверенияПредставительФЛ.НомДок = СтруктураРеквизитов.ПредставительФЛ_НомДок;
		СтрокаУдостоверенияПредставительФЛ.ДатаДок = СтруктураРеквизитов.ПредставительФЛ_ДатаДок;
		СтрокаУдостоверенияПредставительФЛ.ВыдДок = СтруктураРеквизитов.ПредставительФЛ_ВыдДок;
		СтрокаУдостоверенияПредставительФЛ.КодВыдДок = СтруктураРеквизитов.ПредставительФЛ_КодВыдДок;
		
		Объект.ПредставительФЛ_Гражданство = Справочники.СтраныМира.НайтиПоКоду(СтруктураРеквизитов.ПредставительФЛ_КодСтраны);
		
		Возврат Истина;
		
	Иначе
		ТекстСообщения = НСтр("ru = 'Отсутствуют реквизиты представителя. Обратитесь в службу технической поддержки.'");
		Возврат Ложь;
	КонецЕсли;
