...
Функция ПолучитьОтчет(НаименованиеОтчета = Неопределено)
	...
	Если Сред(ТекОтчет, Найти(ТекОтчет, ".") + 1) = "РегламентированныйОтчетРСВ1" Тогда
		Возврат "Отчет." + Сред(ТекОтчет, Найти(ТекОтчет, ".") + 1) + ".Форма." + РегламентированнаяОтчетностьКлиентСерверПереопределяемый.ИмяОсновнойФормыРСВ1();
	Иначе
