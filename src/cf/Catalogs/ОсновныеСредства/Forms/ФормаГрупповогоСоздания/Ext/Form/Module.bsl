...
Процедура Создать(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТипОС) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ru = 'Тип ОС'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Тип ОС");
		ЕстьОшибкиЗаполнения = Истина;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	ТипОС = Перечисления.ТипыОС.ОбъектОС;
