...
Процедура СоздатьУстановитьПредопределенныеЭлементыВалютныйУчет()  Экспорт
	...
	Если ПредопределенныйЭлемент("СуммовыеРазницы") = Неопределено Тогда
		...
		СтатьяДоходовРасходов	= Справочники.ПрочиеДоходыИРасходы.СоздатьЭлемент();
		СтатьяДоходовРасходов.Наименование = "Суммовые разницы";
		СтатьяДоходовРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.ПрочиеВнереализационныеДоходыРасходы;
		СтатьяДоходовРасходов.ПринятиеКНалоговомуУчету    = Истина;
		СтатьяДоходовРасходов.ВидДеятельностиДляНалоговогоУчетаЗатрат = Перечисления.ВидыДеятельностиДляНалоговогоУчетаЗатрат.ОсновнаяСистемаНалогообложения;
	...
	Если ПредопределенныйЭлемент("КурсовыеРазницыПоРасчетамВУЕ") = Неопределено Тогда
		...
		СтатьяДоходовРасходов	= Справочники.ПрочиеДоходыИРасходы.СоздатьЭлемент();
		СтатьяДоходовРасходов.Наименование = "Курсовые разницы по расчетам в у.е.";
...
Процедура СоздатьУстановитьПредопределенныеЭлементыВозвратнаяТара() Экспорт 
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВозвратнуюТару") И ПредопределенныйЭлемент("РазницыЗалоговойИФактическойСтоимостиТары") = Неопределено Тогда
...
Процедура СоздатьУстановитьПредопределенныеЭлементыУСН() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользуетсяУСН") И ПредопределенныйЭлемент("СписаниеНДСНаПрочиеРасходы") = Неопределено Тогда
...
Процедура ОбработкаСправочникаДляУправленияПредопределеннымиЗначениями() Экспорт
	...
	Если ПредопределенныйЭлемент("ДоходыПоАвансамПрошлогоНалоговогоПериодаИП") = Неопределено Тогда
		...
		|
		|ВЫБРАТЬ
		|	УчетнаяПолитикаОрганизацийСрезПоследних.УдалитьСтатьяУчетаПрочихДоходовПоАвансамИП,
		|	УчетнаяПолитикаОрганизацийСрезПоследних.УдалитьСтатьяУчетаПрочихДоходовПоАвансамИП.Код
		|ИЗ
		...
		|				И НЕ СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)) КАК УчетнаяПолитикаОрганизацийСрезПоследних
		|
		|УПОРЯДОЧИТЬ ПО
		|	Код";
	...
	Если ПредопределенныйЭлемент("РезервыПоСомнительнымДолгам") = Неопределено Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	УчетнаяПолитикаОрганизацийСрезПоследних.УдалитьСтатьяУчетаПрочихДоходовИРасходовПоСомнительнымДолгам КАК Ссылка,
		|	УчетнаяПолитикаОрганизацийСрезПоследних.УдалитьСтатьяУчетаПрочихДоходовИРасходовПоСомнительнымДолгам.Код КАК Код
		|ИЗ
		|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних() КАК УчетнаяПолитикаОрганизацийСрезПоследних
		|ГДЕ
		|	УчетнаяПолитикаОрганизацийСрезПоследних.УдалитьСтатьяУчетаПрочихДоходовИРасходовПоСомнительнымДолгам <> 
...
Процедура Заполнить_СписаниеНДСНаРасходыНеНУ() Экспорт
	
	СтатьяДоходовРасходов = ПредопределенныйЭлемент("СписаниеВыделенногоНДСНаПрочиеРасходы");
...
Процедура ОбновитьПредопределенныйЭлементСписаниеНДСНаПрочиеРасходы() Экспорт
	...
	|	Справочник.ПрочиеДоходыИРасходы КАК ПрочиеДоходыИРасходы
	|ГДЕ
	|	ПрочиеДоходыИРасходы.Наименование = ""Списание НДС на прочие расходы при переходе на УСН""
	...
	|	И ПрочиеДоходыИРасходы.ИмяПредопределенныхДанных = ""СписаниеНДСНаПрочиеРасходы""";
...
Процедура ОбновитьПредопределенныеЭлементыКурсовыеИСуммовыеРазницы() Экспорт
	...
	// Курсовые разницы
	СтатьяДоходовРасходовКурсовыеРазницы = ПредопределенныйЭлемент("КурсовыеРазницыПоРасчетамВУЕ");
	Если СтатьяДоходовРасходовКурсовыеРазницы <> Неопределено Тогда
		ЭлементСправочника = СтатьяДоходовРасходовКурсовыеРазницы.ПолучитьОбъект();
		ЭлементСправочника.Наименование = "Курсовые разницы по расчетам в у.е. (по сделкам до 2015 г)";
		ЭлементСправочника.Записать();
	КонецЕсли;
	
	// Суммовые разницы
	СтатьяДоходовРасходовСуммовыеРазницы = ПредопределенныйЭлемент("СуммовыеРазницы");
	Если СтатьяДоходовРасходовСуммовыеРазницы <> Неопределено Тогда
		ЭлементСправочника = СтатьяДоходовРасходовСуммовыеРазницы.ПолучитьОбъект();
		ЭлементСправочника.Наименование = "Суммовые разницы (налоговый учет) (по сделкам до 2015 г)";
		ЭлементСправочника.Записать();
	КонецЕсли;
