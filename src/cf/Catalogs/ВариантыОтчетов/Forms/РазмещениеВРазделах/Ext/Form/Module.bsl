...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		...
		СтандартныеПодсистемыКлиентСервер.ВывестиСообщение(РезультатВыполнения, ТекстСообщения);
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоВариантов = 0 Тогда
			ТекстСообщения = НСтр("ru = 'Недостаточно прав для размещения в разделах выбранных вариантов отчетов.'");
			Элементы.ДеревоПодсистем.Доступность = Ложь;
			РезультатВыполнения.Вставить("Отказ", Истина);
		Иначе
			ТекстСообщения = НСтр("ru = 'Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", Формат(КоличествоОшибок, "ЧГ="));
			Элементы.ДеревоПодсистем.Доступность = Истина;
		КонецЕсли;
		
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(РезультатВыполнения, ТекстСообщения, ТекстОшибок);
		РезультатВыполнения = Новый ФиксированнаяСтруктура(РезультатВыполнения);
	Иначе
		Элементы.ДеревоПодсистем.Доступность = Истина;
	КонецЕсли;
