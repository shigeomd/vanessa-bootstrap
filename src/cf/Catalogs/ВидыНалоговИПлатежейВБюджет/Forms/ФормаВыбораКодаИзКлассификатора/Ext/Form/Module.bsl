...
Функция ИскатьСтрокуВТаблицеНаСервере(СтрокаПоиска, КБКИсходный = "")
	...
	Если НайденнаяОбласть = Неопределено Тогда
		...
		Если СтрДлина(СтрокаПоиска) = 17 И ТабДокумент.Область(ТекущаяОбласть.Верх, 1).Текст = СтрокаПоиска
			ИЛИ Найти(ТабДокумент.Область(ТекущаяОбласть.Верх, 2).Текст, СтрокаПоиска) > 0
			ИЛИ СтрДлина(СтрокаПоиска) < 17 И Найти(ТабДокумент.Область(ТекущаяОбласть.Верх, 1).Текст, СтрокаПоиска) > 0 Тогда
			НайденнаяОбласть = ТекущаяОбласть;
		Иначе
...
Процедура ОпределитьНомерПриказа()
	...
	Если ГодНачалаДействияПриказа = "2015" Тогда
		ИмяМакета      = "КлассификацияДоходовБюджетов2015";
		НадписьПриказа = "по приказу Минфина от 01.07.13 г. № 65н
			|(в редакции приказа Минфина от 16.12.14 г. № 150н)";
	Иначе
		ИмяМакета      = "КлассификацияДоходовБюджетов";
		НадписьПриказа = "по приказу Минфина от 01.07.13 г. № 65н
			|(в редакции приказа Минфина от 21.11.14 г. № 134н)";
	КонецЕсли;
