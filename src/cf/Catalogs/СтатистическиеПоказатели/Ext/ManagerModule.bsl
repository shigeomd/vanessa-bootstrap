...
Функция ДанныеДляЗаполненияXML(ПроверятьДатуИзменения = Истина) Экспорт
	...
	Контейнер = ФабрикаXDTO.Создать(ТипКонтейнер);
	Контейнер.ВерсияФормата = "3.0.1.0";
	
	// ОбъектыСтатистическогоНаблюдения
	ВыгрузитьОбъектыНаблюденияРекурсивно(
		Контейнер.ОбъектыСтатистическогоНаблюдения,
...
Функция ПравилаЗаполненияФормСтатистики(Знач ДанныеДляЗаполненияXML = Неопределено)
	...
	Если ДанныеДляЗаполненияXML = Неопределено Тогда
		...
		Макет = Справочники.СтатистическиеПоказатели.ПолучитьМакет("ДанныеДляЗаполнения");
		ДанныеДляЗаполненияXML = Макет.ПолучитьТекст();
	КонецЕсли;
		
	Чтение = Новый ЧтениеXML;
	Чтение.УстановитьСтроку(ДанныеДляЗаполненияXML);
	Попытка 
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Контейнер.ВерсияФормата, "3.0.2.0") >= 0 Тогда
		ТекстСообщения = НСтр("ru='Неподдерживаемая версия поставляемых данных'");
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка,,, Контейнер.ВерсияФормата);
		Возврат ПравилаЗаполненияФормСтатистики;
	КонецЕсли;	

	Для Каждого ОписаниеОбъектаНаблюдения Из Контейнер.ОбъектыСтатистическогоНаблюдения Цикл
		ПрочитатьДанныеОбъектаНаблюденияРекурсивно(ПравилаЗаполненияФормСтатистики.ОбъектыНаблюдения, ОписаниеОбъектаНаблюдения);
	КонецЦикла;
	
	Для Каждого ОписаниеФормыСтатистики Из Контейнер.ФормыСтатистики Цикл
...
Процедура ЗагрузитьОбъектыНаблюдения(ПравилаЗаполнения)
	...
	Для Каждого ПоставляемыйОбъектНаблюдения Из ПравилаЗаполнения Цикл
		...
		ПоискЗагруженногоРанееПоказателя.Вставить("ОбъектНаблюдения", ОбъектНаблюдения);
...
Процедура ЗагрузитьФормыСтатистики(ПравилаЗаполнения)
	...
	Для Каждого ПоставляемаяФорма Из ПравилаЗаполнения Цикл
		...
		ПоискЗагруженногоРанееПоляФормы.Вставить("ФормаСтатистики", ФормаСтатистики);
		...
		ПоискЗагруженногоРанееПоляФормы.Вставить("ЭтоГруппа", Истина);
