...
Процедура СформироватьЛистыКассовойКниги(ПараметрыОтчета, ДатыЛистовСОшибками, ПоказатьОповещениеОНумерации)
	...
	Если ПараметрыОтчета.ФормироватьПоПодразделению Тогда
		Если ПодразделениеВыбрано Тогда
			...
			Если ФормироватьПоПодразделению Тогда
				...
				СписокПодразделений = Справочники.ПодразделенияОрганизаций.ПодчиненныеСтруктурныеЕдиницы(
					ПараметрыОтчета.ПодразделениеОрганизации).ПодчиненныеСтруктурныеЕдиницы;
		...
		Иначе
			ПредставлениеПодразделения = "";
		КонецЕсли;
	ИначеЕсли ПолучитьФункциональнуюОпцию("ВестиУчетПоПодразделениям") Тогда
		СписокПодразделений = Справочники.ПодразделенияОрганизаций.ПодчиненныеСтруктурныеЕдиницы(
			ПараметрыОтчета.Организация).ПодчиненныеСтруктурныеЕдиницы;
		Если СписокПодразделений = Неопределено Тогда
			СписокПодразделений = Новый Массив;
		КонецЕсли;
	...
	Если ПараметрыОтчета.ПлатежныйАгент Тогда
		МассивСчетов = БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаПлатежногоАгента);
	Иначе
		МассивСчетов    = БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаОрганизации);
		МассивСчетовВал = БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаОрганизацииВал);
		Для каждого Счет Из МассивСчетовВал Цикл
	...
	|	И ПриходныйКассовыйОрдер.СчетКасса В(&МассивСчетов)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И РасходныйКассовыйОрдер.СчетКасса В(&МассивСчетов)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	|;
	...
	|	И (Хозрасчетный.СчетДт В (&МассивСчетов)
	...
	|					ТОГДА ВТ_КассовыеДокументы.СчетУчетаРасчетовСКонтрагентом
...
//		ЛистовЗаГод = 1 стр
		...
		Для каждого СтрокаТаблица Из РабочаяТаблица Цикл
			...
			КурсоваяРазница = Окр(СтрокаТаблица.ВалОстаток * Курс / Кратность, 2, 1) - СтрокаТаблица.Остаток;
		...
		Для каждого Документ Из ТаблицаДокументы Цикл
			...
			НомерДокПечатнойФормы = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.НомерДок, Истина, Ложь);
		...
		Для каждого Строка Из РабочаяТаблица Цикл
			...
			// Возможно отклонение в 1 копейку, т.к. расчеты округляются до двух знаков
			КурсоваяРазницаБезОкругления =
				(Строка.ВалОстаток + Строка.ВалПриход - Строка.ВалРасход) * Курс / Кратность - (Строка.Остаток + Строка.Приход - Строка.Расход);
			ЕстьКурсоваяРазница = ?(Макс(КурсоваяРазницаБезОкругления, - КурсоваяРазницаБезОкругления) <= 0.01, 0, 1);
			КурсоваяРазница     = ЕстьКурсоваяРазница * Окр(КурсоваяРазницаБезОкругления, 2, 1);
...
Процедура СформироватьЗавершающийЛист(ТабличныйДокумент, ПараметрыОтчета, ЛистовЗаГод, ОтветственныеЛица)
	...
	МакетОбложка = ПолучитьМакет("Обложка");
	ОбластьПослДеньГода = МакетОбложка.ПолучитьОбласть(ИмяОбласти + "|Отчет");
