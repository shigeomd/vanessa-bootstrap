...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекИндекс > 0 Цикл// перебор в обратном порядке элементов списка мПечатныеФормы
		Попытка// попытка удалить область "Ограничители1"
			...
			Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(НомСтр, 2, НомСтр, 2).Имя, "П000001000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ОткрытьФорму("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения);
