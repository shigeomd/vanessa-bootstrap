...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	Если Не ЗначениеЗаполнено(Отчет.Счет) Тогда
		...
		Сообщение.Поле = "Отчет.Счет";
		Сообщение.Текст = НСтр("ru = 'Не заполнен счет'");
		Сообщение.Сообщить();
		...
		Если Отбор.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
			...
			ИначеЕсли ТипЗнч(Отбор.ПравоеЗначение) = Тип("СписокЗначений") Тогда
				Для Каждого Значение Из Отбор.ПравоеЗначение Цикл
					СписокВыбора = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(Отбор.ПравоеЗначение.ВыгрузитьЗначения(), Неопределено);	
				КонецЦикла;
			КонецЕсли;
