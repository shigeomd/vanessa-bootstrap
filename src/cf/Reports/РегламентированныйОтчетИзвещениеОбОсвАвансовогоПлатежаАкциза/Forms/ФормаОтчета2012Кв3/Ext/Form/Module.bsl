...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2012Кв3_Извещение"));
	...
	Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы Цикл
		...
		Для Каждого Область Из ОбластьДопСтроки.Области Цикл
			...
			Если ОбластьТаблДок.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
				...
				ОбластьТаблДок.Имя = Лев(Область.Имя, Найти(Область.Имя, "_")) + Строка(ИсходноеКоличествоСтрокГруппы + ТекущееКоличествоСтрокГруппы - НомНовойСтроки);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2012Кв3");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяОбласти, "НалоговыйПериодНачало") = 1 ИЛИ Найти(ИмяОбласти, "НалоговыйПериодОкончание") = 1 Тогда
		СтандартнаяОбработка = Ложь;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		Обл = ПечатнаяФорма.Области.Найти("УдалитьСтроку" + СтруктураРеквизитовФормы.мИдГруппы1 + "01_" + Инд);
		Если Обл = Неопределено Тогда 
			Прервать;
		КонецЕсли;
		Обл.Текст = "";
...
Функция СформироватьСтруктуруПараметров()
	...
	Если ПараметрыВыгрузки.ПризОтпр = "2"  Тогда 
		Если СокрЛП(СтруктураРеквизитовФормы.НаименованиеОрганизацииПредставителя) = "" Тогда
			...
			ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СтруктураРеквизитовФормы.УполномоченноеЛицоПредставителя);
	...
	ФИОРук = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Извещение.ФИОРук);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока ПТД_Извещение.Свойство("П000100000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		
		СтрокаЗаполнена = Ложь;
		ВсеПоказателиЗаполнены = Истина;
		Для Графа = 2 По 14 Цикл
			ИмяЯчейки = "П0001000001" + Формат(Графа, "ЧЦ=2; ЧВН=") + "_" + Формат(НомерСтроки, "ЧГ=");
		...
		Если СтрокаЗаполнена Тогда
			Если НЕ ВсеПоказателиЗаполнены Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В строке номер "+НомерСтроки+" заполнены не все показатели!", "Извещение", "П000100000101_" + НомерСтроки, 0);
				СтатусПроверки = Ложь;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Извещение, "П000100000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		ЗадатьЗначениеЭлемента(Узел_СведБанк, "НаимБанк",   Извещение["П000100000106_"+Формат(НомерСтроки, "ЧГ=")]);
		ЗадатьЗначениеЭлемента(Узел_СведБанк, "ИННЮЛ_Банк", Извещение["П000100000107_"+Формат(НомерСтроки, "ЧГ=")]);
		ЗадатьЗначениеЭлемента(Узел_СведБанк, "КПП_Банк",   Извещение["П000100000108_"+Формат(НомерСтроки, "ЧГ=")]);
		ЗадатьЗначениеЭлемента(Узел_СведБанк, "БИК",        Извещение["П000100000109_"+Формат(НомерСтроки, "ЧГ=")]);
