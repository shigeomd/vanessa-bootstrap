...
Процедура ПоказатьПериод(Форма)
	...
	Форма.НадписьСрокПредставленияОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Декларацию за %1 год с указанием суммы предполагаемого дохода от осуществления предпринимательской деятельности или от занятия частной практикой необходимо представить в текущем налоговом периоде в пятидневный срок по истечении месяца со дня появления таких доходов (п.5 ст. 227 НК РФ).'"), Формат(Форма.мДатаКонцаПериодаОтчета, "ДФ=""гггг"""));
	
	КоличествоФорм = РегламентированнаяОтчетностьКлиентСервер.КоличествоФормСоответствующихВыбранномуПериоду(Форма);
	Если КоличествоФорм >= 1 И ЗначениеЗаполнено(Форма.Организация) Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СпиокФизЛиц = Новый СписокЗначений;
	
	РегламентированнаяОтчетность.ПолучитьСписокДоступныхИндивидуальныхПредпринимателей(СпиокФизЛиц);
	...
	Иначе
		...
		Если СпиокФизЛиц.НайтиПоЗначению(Организация) = Неопределено Тогда
			Организация = Неопределено;
		КонецЕсли;
		
		Для Каждого ЭлементСписка Из СпиокФизЛиц Цикл
			
			ОрганизацияСписокВыбора.Добавить(ЭлементСписка.Значение, ЭлементСписка.Представление);
	...
	Если СпиокФизЛиц.Количество() = 0 Тогда
		
		Организация = Неопределено;
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Декларацию 4-НДФЛ представляют только индивидуальные предприниматели и лица, занимающиеся частной практикой (пп. 1, 2 п. 1 ст. 227 НК РФ).
...
Процедура ВыбратьФорму(Команда)
	...
	ДопТекстОписания = НСтр("ru='Декларацию 4-НДФЛ представляют индивидуальные предприниматели и лица, занимающиеся частной практикой (пп. 1, 2 п. 1 ст. 227 НК РФ).'");
