...
Процедура ПриВыводеЗаголовка(ПараметрыОтчета, КомпоновщикНастроек, Результат) Экспорт
	...
	// Заголовок 1
	ЗаголовокОтчета =  НСтр("ru = 'Регистр налогового учета'");	
	
	// Заголовок 2
	ЗаголовокОтчета2 =  Символы.ПС + ПараметрыОтчета.НазваниеРегистра;
	Если НЕ ПараметрыОтчета.ЭтоПредопределенныйВариант Тогда
		ЗаголовокОтчета2 = ЗаголовокОтчета2 + ": " + ПараметрыОтчета.ИмяНастроекОтчета;
	КонецЕсли;
	
	ОбластьЗаголовок.Параметры.ЗаголовокОтчета = ЗаголовокОтчета + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода)
	+ ЗаголовокОтчета2;
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
		...
		Если Не ПоказателиОтчета = Неопределено Тогда
			Для каждого Показатель Из ПоказателиОтчета.Значение Цикл
				Если Не ПараметрыОтчета.Свойство(Показатель) Тогда
					...
					КоличествоПоказателей = КоличествоПоказателей + ?(Найти(Показатель,"Показатель") = 0,0,1);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.НачалоПериода) Тогда
		...
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "НачалоПериодаГраница", Новый Граница(НачалоДня(ПараметрыОтчета.НачалоПериода), ВидГраницы.Включая));
	КонецЕсли;
...
Процедура ПослеВыводаРезультата(ПараметрыОтчета, Результат) Экспорт
	...
	Для СтрокаРезультата = ЗаголовокНиз По ЗаголовокНиз + ПараметрыОтчета.ВысотаШапки Цикл
		Для КолонкаРезультата = 1 По ПараметрыОтчета.ШиринаШапки Цикл
			...
			Если Найти(Текст,"Кор. Субконто") > 0 Тогда
				ОбластьЗаголовка.Текст = СтрЗаменить(Текст,"Кор. Субконто","Основание ");
			ИначеЕсли Найти(Текст,"Субконто") > 0 Тогда
				ОбластьЗаголовка.Текст = СтрЗаменить(Текст,"Субконто","Объект учета ");
			ИначеЕсли Найти(Текст,"Регистратор") > 0 Тогда
				ОбластьЗаголовка.Текст = "Документ";
			ИначеЕсли Найти(Текст,"Счет") > 0 и ПараметрыОтчета.Регистр = "ПолуфабрикатыИПродукция" Тогда
...
Функция ПолучитьПараметрыВариантовОтчета() Экспорт
	...
	//1
	Параметры.Вставить("ВнереализационныеРасходы",								Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Внереализационные расходы"));
	Параметры.Вставить("ДоходыОтРеализацииТоваровИУслуг",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Доходы от реализации товаров, работ, услуг"));
	Параметры.Вставить("ДоходыОтРеализацииПрочегоИмущества",					Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Доходы от реализации прочего имущества, прав, ценных бумаг"));
	Параметры.Вставить("ВнереализационныеДоходы",								Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Внереализационные доходы"));
	Параметры.Вставить("КосвенныеРасходыНаПроизводствоИРеализацию",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Косвенные расходы на производство и реализацию"));
	Параметры.Вставить("ПрямыеРасходыОтносящиесяКРеализованнымТоварамУслугам",	Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Прямые расходы, относящиеся к реализованным товарам, услугам"));
	Параметры.Вставить("ПрямыеТранспортныеРасходы",								Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Транспортные расходы"));
	Параметры.Вставить("РасходыОтРеализацииПрочегоИмущества",					Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Расходы, относящиеся к реализации прочего имущества, прав, ценных бумаг"));
	Параметры.Вставить("ПрочиеКосвенныеРасходы",								Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыФормированияОтчетныхДанных","Прочие косвенные расходы"));
	
	//2
	Параметры.Вставить("ОплатаТруда",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыПромежуточныхРасчетов","Расходы на оплату труда"));
	Параметры.Вставить("Амортизация",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыПромежуточныхРасчетов","Расходы по амортизации"));
	Параметры.Вставить("МатериальныеРасходы",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыПромежуточныхРасчетов","Материальные расходы"));
	Параметры.Вставить("РасходыНаДобровольноеСтрахование",	Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыПромежуточныхРасчетов","Расходы на добровольное страхование"));
	Параметры.Вставить("ПрочиеРасходы",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыПромежуточныхРасчетов","Прочие расходы"));
	
	//3
	Параметры.Вставить("ОсновныеСредства",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость основных средств"));
	Параметры.Вставить("НематериальныеАктивы",					Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость нематериальных активов и НИОКР"));
	Параметры.Вставить("Материалы",								Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость материалов"));
	Параметры.Вставить("СпецоснасткаИСпецодеждаВЭксплуатации",	Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Специальная оснастка и специальная одежда в эксплуатации"));
	Параметры.Вставить("ПолуфабрикатыИПродукция",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость продукции и полуфабрикатов"));
	Параметры.Вставить("ТоварыНаСкладе",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость товаров на складе"));
	Параметры.Вставить("ТоварыОтгруженные",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость отгруженных товаров"));
	Параметры.Вставить("НезавершенноеПроизводство",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Стоимость незавершенного производства"));
	Параметры.Вставить("РасчетыСПоставщикамиИПокупателями",		Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Дебиторская и кредиторская задолженность"));
	Параметры.Вставить("РасчетыСБюджетом",						Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Задолженность перед бюджетом"));
	Параметры.Вставить("РасходыБудущихПериодов",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Расходы будущих периодов"));
	Параметры.Вставить("ДоходыБудущихПериодов",					Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета","Доходы будущих периодов"));
	
	//4
	Параметры.Вставить("ОперацииПриобретенияИмуществаРаботУслуг",	Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Операции приобретения товаров, прочего имущества, услуг, работ, прав, ценных бумаг"));
	Параметры.Вставить("ОперацииВыбытияИмуществаРаботУслуг",		Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Операции выбытия товаров, работ, услуг"));
	Параметры.Вставить("ОперацииВыбытияПрочегоИмущества",			Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Операции выбытия прочего имущества, прав, ценных бумаг"));
	Параметры.Вставить("ДенежныеСредстваНаличные",					Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Денежные средства в кассе"));
	Параметры.Вставить("ДенежныеСредстваБезналичные",				Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Денежные средства на банковских счетах"));
	Параметры.Вставить("ДоговораДобровольногоСтрахованияРаботников",Новый Структура("ПутьПодсистемыВарианта ,Наименование","Подсистема.УчетНалогиОтчетность82.Подсистема.РегистрыУчетаХозяйственныхОпераций","Расчеты по договорам страхования"));
...
Процедура НастроитьВариантыОтчета(Настройки, ОписаниеОтчета) Экспорт
	...
	// Интерфейс "Версия 8.2"
	
	// 1
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ВнереализационныеРасходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДоходыОтРеализацииТоваровИУслуг").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДоходыОтРеализацииПрочегоИмущества").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ВнереализационныеДоходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "КосвенныеРасходыНаПроизводствоИРеализацию").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПрямыеРасходыОтносящиесяКРеализованнымТоварамУслугам").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "РасходыОтРеализацииПрочегоИмущества").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПрочиеКосвенныеРасходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыФормированияОтчетныхДанных, "");
	
	//2
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ОплатаТруда").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыПромежуточныхРасчетов, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "Амортизация").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыПромежуточныхРасчетов, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "МатериальныеРасходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыПромежуточныхРасчетов, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "РасходыНаДобровольноеСтрахование").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыПромежуточныхРасчетов, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПрочиеРасходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыПромежуточныхРасчетов, "");
	
	//3
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ОсновныеСредства").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "НематериальныеАктивы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "Материалы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "СпецоснасткаИСпецодеждаВЭксплуатации").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПолуфабрикатыИПродукция").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ТоварыНаСкладе").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ТоварыОтгруженные").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "НезавершенноеПроизводство").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "РасчетыСПоставщикамиИПокупателями").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "РасчетыСБюджетом").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "РасходыБудущихПериодов").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДоходыБудущихПериодов").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаСостоянияЕдиницыНалоговогоУчета, "");
	
	//4
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ОперацииПриобретенияИмуществаРаботУслуг").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ОперацииВыбытияИмуществаРаботУслуг").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ОперацииВыбытияПрочегоИмущества").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДенежныеСредстваНаличные").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДенежныеСредстваБезналичные").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ДоговораДобровольногоСтрахованияРаботников").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
	ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПрямыеТранспортныеРасходы").Размещение.Вставить(Метаданные.Подсистемы.УчетНалогиОтчетность82.Подсистемы.НалогНаПрибыль.Подсистемы.РегистрыУчетаХозяйственныхОпераций, "");
