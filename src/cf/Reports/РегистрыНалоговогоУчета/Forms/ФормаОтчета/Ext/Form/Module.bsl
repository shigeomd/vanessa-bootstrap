...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Если Найти("ОперацииВыбытияИмуществаРаботУслуг, ВнереализационныеДоходы, РасходыОтРеализацииПрочегоИмущества, ПрямыеРасходыОтносящиесяКРеализованнымТоварамУслугам",Отчет.Регистр) <> 0 Тогда
	...
	Если Найти("МатериальныеРасходы",Отчет.Регистр) <> 0 Тогда
		СписокПолей.Добавить("ВидРасходов");
	КонецЕсли;	
	
	Если Не Субконто1СтатьяЗатрат.Значение или Найти("ПрямыеТранспортныеРасходы,КосвенныеРасходыНаПроизводствоИРеализацию",Отчет.Регистр) <> 0 Тогда
		СписокПолей.Добавить("КосвенныеРасходы");
	КонецЕсли;	
	
	Если Найти("ОперацииВыбытияИмуществаРаботУслуг",Отчет.Регистр) <> 0 Тогда
		СписокПолей.Добавить("Счет");
	КонецЕсли;	
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(Настройки)
	...
	ХранилищаНастроек.ХранилищеВариантовОтчетов.Сохранить("Отчет." + БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ЭтотОбъект), КлючТекущегоВарианта, Настройки, ОписаниеНастроекОтчета, "");
	...
	Иначе	
		Для Каждого Отбор Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			Если (Тип(Отбор) <> Тип("ГруппаЭлементовОтбораКомпоновкиДанных"))
				...
				И мВидСравнения <> Строка(ВидСравненияКомпоновкиДанных.ВСписке)
				И мВидСравнения <> Строка(ВидСравненияКомпоновкиДанных.НеВСписке)
