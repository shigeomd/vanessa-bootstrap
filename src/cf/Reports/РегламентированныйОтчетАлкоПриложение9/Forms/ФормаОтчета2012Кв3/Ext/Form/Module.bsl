...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.СправочникиВидыКонтактнойИнформацииЮрАдресОрганизации = Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации;
	СтруктураРеквизитовФормы.СправочникиВидыКонтактнойИнформацииЮрАдресКонтрагента = Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ЭтоАдресВМногострочнойЧасти = Найти(ИмяОбласти, "П000010000106") = 1
	                          ИЛИ Найти(ИмяОбласти, "П000010000306") = 1
	                          ИЛИ Найти(ИмяОбласти, "П000010000113") = 1
	                          ИЛИ Найти(ИмяОбласти, "П000010000313") = 1;
...
Процедура ВвестиАдресКонтрагента(ИмяОбласти, Элемент);
	...
	ДополнительныеДанныеМногострочнойЧасти = Неопределено;
	Если НЕ АктивнаяСтраница.ДополнительныеДанные[0].Значение.Свойство(ИмяГруппы, ДополнительныеДанныеМногострочнойЧасти) Тогда
		ДополнительныеДанныеМногострочнойЧасти = Новый Структура;
	КонецЕсли;
	...
	Если ДополнительныеДанныеМногострочнойЧасти.Свойство(ИмяОбласти, СохраненныйАдресПеревозчика) Тогда
...
&НаСервере
Процедура ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		АдресПолный = ДополнительныеДанныеПоАдресу(ДополнительныеДанныеЛиста, ИмяПоказателяАдреса);
		
		ЭтоИностранныйКонтрагент = (АдресПолный <> Неопределено И ЗначениеЗаполнено(АдресПолный.Страна));
		
		Если НЕ ЭтоИностранныйКонтрагент Тогда
			
			Если НЕ ПустаяСтрока(ИНН) ИЛИ НЕ ПустаяСтрока(КПП) Тогда
				Если ПустаяСтрока(ИНН) Тогда
					ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указан ИНН организации-грузоотправителя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяИНН, НомерЛиста);
				ИначеЕсли СтрДлина(ИНН) <> 10 Тогда
					ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации неверно указан ИНН организации-грузоотправителя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяИНН, НомерЛиста);
				КонецЕсли;
				
				Если ПустаяСтрока(КПП) Тогда
					ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации не указан КПП организации-грузоотправителя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяКПП, НомерЛиста);
				ИначеЕсли СтрДлина(КПП) <> 9 Тогда
					ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый + " декларации неверно указан КПП организации-грузоотправителя!";
					СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяКПП, НомерЛиста);
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСведенияПоСтранице(ДанныеВыгрузки, Узел_ОбъемПеревозки)
	...
	Для Инд = 0 По ТаблицаПеревозок.Количество() - 1 Цикл
		...
		Если ТипЗнч(ДополнительныеДанныеСтраницы) = Тип("Структура") И ДополнительныеДанныеСтраницы.Свойство("П0000100003") Тогда
			АдресГрузоотправителя = Неопределено;
			ДополнительныеДанныеСтраницы.П0000100003.Свойство("П000010000306" + Постфикс, АдресГрузоотправителя);
			...
			ДополнительныеДанныеСтраницы.П0000100003.Свойство("П000010000313" + Постфикс, АдресГрузополучателя);
