...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.ЗаголовокПредставление = Метаданные.Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ОсновнаяФорма.Синоним;
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	мПустойОбразЛистаФЛ.Очистить();
	СтруктураСтраницыФЛ = ТаблицаСтраницыФизЛица[0].Данные[0].Значение;
	Для каждого ЭлементСтруктуры Из СтруктураСтраницыФЛ Цикл
		мПустойОбразЛистаФЛ.Вставить(ЭлементСтруктуры.Ключ, ЭлементСтруктуры.Значение);
	КонецЦикла; 
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел3" Тогда
		...
		Для Каждого ЭлСтруктуры Из ЭтаФорма["СтруктураДанных" + ИмяТекРаздела] Цикл
			...
			Если (Найти(ЭлСтруктуры.Ключ, "П0") > 0) И (Прав(ЭлСтруктуры.Ключ, 1) = "0") Тогда
...
Процедура ЗагрузитьПоказателиИзМакета (РазделВМакете)
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2005Кв1");
...
Функция ВозвратитьПустойОбразЛистаФЛ(Форма)
	...
	Для Каждого Стр из Форма.мПустойОбразЛистаФЛ Цикл
...
Процедура ЗаполнитьСведенияОбФЛ(Форма, СмениласьДатаПодписания = Ложь)
	...
	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация Или СмениласьДатаПодписания) Тогда
...
Процедура ВыводДанныхОФизическомЛице_(Форма, Знач СвязьУстановлена, Знач Данные, ТипДанных, ОграниченияВыводаУчесть = Ложь, ТаблицаСтраницРазделаМ, ИндексВТабл);
	...
	Если ТипДанных = 3 Тогда
		...
		ДанныеПредставителя = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОПредставителе(Форма.СтруктураРеквизитовФормы.Организация, Форма.ДатаПодписи, Ложь, , КППТитульныйЛист);
...
Процедура ЗаполнитьДатуВЯчейках(Форма) Экспорт
	...
	//Если Мин(ПустаяСтрока(Форма.ТабличныйДокумент.Области.ОргДиректор.Значение), ПустаяСтрока(Форма.СтруктураРеквизитовФормы.мИННДиректор)) = 0 Тогда
	//	ДатаПодписиРук = Форма.ДатаПодписи; 
	//КонецЕсли;
	//
	//Если Мин(ПустаяСтрока(Форма.ТабличныйДокумент.Области.ОргБухгалтер.Значение), ПустаяСтрока(Форма.СтруктураРеквизитовФормы.мИННБухгалтер)) = 0 Тогда
	//	ДатаПодписиБух = Форма.ДатаПодписи;
	//КонецЕсли;

	//Если Мин(ПустаяСтрока(Форма.ТабличныйДокумент.Области.ОргУП.Значение), ПустаяСтрока(Форма.СтруктураРеквизитовФормы.мИННУпПред)) = 0 Тогда
	//	ДатаПодписиУпП = Форма.ДатаПодписи;
	//КонецЕсли;

	//Форма.ТабличныйДокумент.Области.ДатаПодписиРук.Значение = Формат(ДатаПодписиРук, "ДФ = ""дд.ММ.гггг 'г.'""");
	//Форма.ТабличныйДокумент.Области.ДатаПодписиБух.Значение = Формат(ДатаПодписиБух, "ДФ = ""дд.ММ.гггг 'г.'""");
	//Форма.ТабличныйДокумент.Области.ДатаПодписиУпП.Значение = Формат(ДатаПодписиУпП, "ДФ = ""дд.ММ.гггг 'г.'""");
	//Форма.СтруктураДанныхРаздел1_1.ДатаПодписи = Формат(Форма.ДатаПодписи,    "ДФ = ""дд.ММ.гггг 'г.'""");
	//Форма.СтруктураДанныхРаздел1_2.ДатаПодписи = Формат(форма.ДатаПодписи,    "ДФ = ""дд.ММ.гггг 'г.'""");
	//Форма.СтруктураДанныхРаздел1_3.ДатаПодписи = Формат(Форма.ДатаПодписи,    "ДФ = ""дд.ММ.гггг 'г.'""");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если ВосстановленИзСтаройВерсии Тогда
		Модифицированность = Истина;
	КонецЕсли;
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		ИначеЕсли ИмяПоказателя = "ИННРук1" Тогда
			...
			Если Прав(СтруктураДанныхТитульный.ИННРук, 2) = "--" Тогда
				СтруктураДанныхТитульный.ИННРук = Лев(СтруктураДанныхТитульный.ИННРук, Найти(СтруктураДанныхТитульный.ИННРук, "--") - 1);
			КонецЕсли;
		ИначеЕсли ИмяПоказателя = "ИННГБ1" Тогда
			...
			Если Прав(СтруктураДанныхТитульный.ИННГБ, 2) = "--" Тогда
				СтруктураДанныхТитульный.ИННГБ = Лев(СтруктураДанныхТитульный.ИННГБ, Найти(СтруктураДанныхТитульный.ИННГБ, "--") - 1);
			КонецЕсли;
		ИначеЕсли ИмяПоказателя = "ИННУП1" Тогда
			...
			Если Прав(СтруктураДанныхТитульный.ИННУП, 2) = "--" Тогда
				СтруктураДанныхТитульный.ИННУП = Лев(СтруктураДанныхТитульный.ИННУП, Найти(СтруктураДанныхТитульный.ИННУП, "--") - 1);
			КонецЕсли;
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "")
	...
	//Сообщение = Новый СообщениеПользователю;
	//Сообщение.Текст = "Расчет раздела """ + ИмяРаздела + """, показатель: """ + ОбластьИмя + """";
	//Сообщение.Сообщить();
	...
	ИначеЕсли ИмяРаздела = "Раздел5" Тогда
		...
		Для каждого Страница Из ТаблицаРаздела Цикл
			...
			//Если Данные.П000050017004 = 0 Тогда
			//	Данные.П000050017004 = Данные.П000050018004;
			//КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1_1" Тогда
		// Раздел 1.1
		Если (Найти(Ячейка.Имя,"КБКвФБ") > 0)
			...
			Если НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
				...
				Если Найти(Ячейка.Имя,"КБКвФБ") > 0 Тогда
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
		...
		ИначеЕсли ((Найти(Область.Имя, "П000020012002")>0) ИЛИ (Найти(Область.Имя, "П000020012003")>0)) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов)Тогда
			СтандартнаяОбработка = Ложь;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяРаздела = "Раздел1_1" Тогда
		...
		ИначеЕсли (Найти(ИмяПоказателя,"КБКвФБ") > 0)
			Или (Найти(ИмяПоказателя,"КБКвБС") > 0)
			Или (Найти(ИмяПоказателя,"КБКвМБ") > 0) Тогда
			
			Если Найти(ИмяПоказателя,"КБКвФБ") > 0 Тогда
				КодПоСтруктуре = "КБКвФБ_1_1";
			ИначеЕсли Найти(ИмяПоказателя,"КБКвБС") > 0 Тогда
				КодПоСтруктуре = "КБКвФБ_1_2";
			ИначеЕсли Найти(ИмяПоказателя,"КБКвМБ") > 0 Тогда
				КодПоСтруктуре = "КБКвФБ_1_3";
			КонецЕсли;
			
			ОбновитьПеременныеМодуля(ЭтаФорма, КодПоСтруктуре, ЗначПоказателя);
			
		ИначеЕсли (Найти(ИмяПоказателя, "Сумма") > 0) И (СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет) Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел10_Прил1" Тогда
		...
		Если Найти(ИмяПоказателя, "П000101001001_") > 0 Тогда
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Указанный ИНН %1 - не соответствует требованиям!'"), Область.Значение);
			Сообщение.Сообщить();
			
		ИначеЕсли Найти(ИмяПоказателя, "П000101001002_") > 0 Тогда
...
Процедура УниверсальнаяВыгрузкаПредписанныхПоказателей(ТекстВыгрузки, БратьИзФормы = Истина)
	...
	Для Инд = 0 По КоличествоСтрок - 1 Цикл
		...
		Если Найти(СтрокаТаблицы.ИмяПоляТаблДокумента, "Раздел1_") > 0 Тогда
			Прервать;
		КонецЕсли; 
...
Процедура УстановитьФорматВыводаНаПечать(ТабДокумент)
	...
	Для Каждого Область Из ТабДокумент.Области Цикл
		...
		Если Найти(Область.Формат, "ЧН=-") > 0 Тогда
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетПечатнойФормы, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Реквизит Из ОбщиеРеквизитыРазделов Цикл
		...
		Если Найти(Реквизит.Ключ, "ДатаПодписи") > 0 Тогда
