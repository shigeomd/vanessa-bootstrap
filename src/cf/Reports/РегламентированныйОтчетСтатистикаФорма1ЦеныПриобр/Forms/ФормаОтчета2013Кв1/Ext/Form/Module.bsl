...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(
		Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2013Кв1_ФормаОтчета"));
...
Процедура ВставитьДополнительнуюСтроку(Форма, ТекущаяСтраница)
	...
	Если ИмяГруппы = Неопределено
		...
		Или Найти(ТекущаяОбласть.Имя, "_") = 0 Тогда
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2013Кв1_ФормаОтчета"));
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока Инд < СтруктураРеквизитовФормы.мПечатныеФормы.Количество() Цикл 
		...
		Для Каждого Область Из Лист.Области Цикл 
			Если Найти(Область.Имя, "УдалитьСтрокуП0") = 1 Тогда 
				Область.Текст = "";
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей
		= Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2013Кв1");
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ИмяФайлаВыгрузки = "", ВыгружатьСОшибками = Ложь) Экспорт
	...
	// Выводим состояние для формы 1-цены приобретения (т.к. выгрузка занимает более 10 секунд).
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1. Производится выгрузка данных, подождите...'"), ЭтаФорма.Заголовок), , , );
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (из текста "Указаний ФСН по заполнению Формы 1-цены приобретения").
	
	// В разделах «Промышленные товары» и «Услуги» если в графе 1 «цена…»
	// по какой-либо позиции значение не равно нулю, то значение в графе 2
	// «количество…» тоже не должно быть равным нулю, и наоборот.
	//
	// По разделу 1.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ПараметрыВыгрузки, "ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04") Цикл
		Если Не ((ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] <> 0
			И ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"] <> 0)
			Или (ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0
			И ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"] = 0)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 1, если значение в графе 1 не равно нулю, то значение в графе 2 не равно нулю (и наоборот)!",
				ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0,
				"П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04",
				"П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"));
			СтатусПроверки = Ложь;
		КонецЕсли;
		НомСтр = НомСтр + 1;
	КонецЦикла;
	...
	Для НомГруппы = 1 По 175 Цикл
		...
		НомСтр = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			ПараметрыВыгрузки, "ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")) Цикл
			Если Не ((ПараметрыВыгрузки["ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] <> 0
				И ПараметрыВыгрузки["ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")] <> 0)
				Или (ПараметрыВыгрузки["ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] = 0
				И ПараметрыВыгрузки["ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")] = 0)) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в разделе 1, если значение в графе 1 не равно нулю, то значение в графе 2 не равно нулю (и наоборот)!",
					ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] = 0,
					"П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ="),
					"П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")));
				СтатусПроверки = Ложь;
			КонецЕсли;
			НомСтр = НомСтр + 1;
		КонецЦикла;
	...
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ПараметрыВыгрузки, "ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04") Цикл
		Если Не ((ПараметрыВыгрузки["ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] <> 0
			И ПараметрыВыгрузки["ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"] <> 0)
			Или (ПараметрыВыгрузки["ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0
			И ПараметрыВыгрузки["ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"] = 0)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 3, если значение в графе 1 не равно нулю, то значение в графе 2 не равно нулю (и наоборот)!",
				ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0,
				"П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04",
				"П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"));
			СтатусПроверки = Ложь;
		КонецЕсли;
		НомСтр = НомСтр + 1;
	КонецЦикла;
	...
	Для НомГруппы = 1 По 27 Цикл
		НомСтр = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			ПараметрыВыгрузки, "ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")) Цикл
			Если Не ((ПараметрыВыгрузки["ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] <> 0
				И ПараметрыВыгрузки["ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")] <> 0)
				Или (ПараметрыВыгрузки["ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] = 0
				И ПараметрыВыгрузки["ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")] = 0)) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в разделе 3, если значение в графе 1 не равно нулю, то значение в графе 2 не равно нулю (и наоборот)!",
					ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")] = 0,
					"П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ="),
					"П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")));
				СтатусПроверки = Ложь;
			КонецЕсли;
			НомСтр = НомСтр + 1;
		КонецЦикла;
	...
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ПараметрыВыгрузки, "ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01") Цикл
		
		// В разделе «Удобрения» если в графе 1 или 3 «цена…» по какой-либо позиции
		// значение не равно нулю, то значение в графе 2 или 4 «количество…»
		// тоже не должно быть равным нулю, и наоборот.
		//
		Если Не ((ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"] <> 0
			И ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"] <> 0)
			Или (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"] = 0
			И ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"] = 0)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 2, если значение в графе 1 не равно нулю, то значение в графе 2 не равно нулю (и наоборот)!",
				ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"] = 0,
				"П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01",
				"П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"));
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Не ((ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] <> 0
			И ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] <> 0)
			Или (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] = 0
			И ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"] = 0)) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в разделе 2, если значение в графе 3 не равно нулю, то значение в графе 4 не равно нулю (и наоборот)!",
				ИмяПоляТаблДокумента, ?(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] = 0,
				"П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03",
				"П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"));
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		// В разделе «Удобрения» цена конкретного товара за отчетный период
		// в физическом весе в графе 1 должна быть меньше цены этого же товара
		// в пересчете на 100% питательных веществ в графе 3.
		//
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"])
		   И ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]) Тогда
			Если Не (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"]
				< ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в разделе 2, значение в графе 1 должно быть меньше значения в графе 3!",
					ИмяПоляТаблДокумента, "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		
		// В разделе «Удобрения» количество конкретного товара за отчетный период
		// в физическом весе в графе 2 должно быть больше количества этого же товара
		// в пересчете на 100% питательных веществ в графе 4.
		//
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"])
		   И ЗначениеЗаполнено(ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]) Тогда
			Если Не (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"]
				> ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в разделе 2, значение в графе 2 должно быть больше значения в графе 4!",
					ИмяПоляТаблДокумента, "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы
	// (Указания ФСН по заполнению Формы 1-цены приобретения).
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ТабДокумент, "П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04") Цикл
		
		ПараметрыВыгрузки.Вставить("ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04",
			ТабДокумент["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
			
		ПараметрыВыгрузки.Вставить("ПР00001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05",
			ТабДокумент["П000001" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
	...
	Для НомГруппы = 1 По 175 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			ТабДокумент, "П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")) Цикл
			
			ПараметрыВыгрузки.Вставить("ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ="),
				ТабДокумент["П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")]);
				
			ПараметрыВыгрузки.Вставить("ПР00001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ="),
				ТабДокумент["П000001" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")]);
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ТабДокумент, "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01") Цикл
		
		ПараметрыВыгрузки.Вставить("ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01",
			ТабДокумент["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "01"]);
			
		ПараметрыВыгрузки.Вставить("ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02",
			ТабДокумент["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "02"]);
		
		ПараметрыВыгрузки.Вставить("ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03",
			ТабДокумент["П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"]);
			
		ПараметрыВыгрузки.Вставить("ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04",
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
		ТабДокумент, "П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04") Цикл
		
		ПараметрыВыгрузки.Вставить("ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04",
			ТабДокумент["П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]);
			
		ПараметрыВыгрузки.Вставить("ПР00003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05",
			ТабДокумент["П000003" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "05"]);
	...
	Для НомГруппы = 1 По 27 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(
			ТабДокумент, "П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")) Цикл
			
			ПараметрыВыгрузки.Вставить("ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ="),
				ТабДокумент["П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "04_" + Формат(НомСтр, "ЧГ=")]);
				
			ПараметрыВыгрузки.Вставить("ПР00003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ="),
				ТабДокумент["П000003" + Формат(НомГруппы, "ЧЦ=4; ЧВН=; ЧГ=") + "05_" + Формат(НомСтр, "ЧГ=")]);
...
Функция КодНаОсновеОКПД1ЦеныПриобр(НомерРаздела, НомерГруппы)
	...
	Если НомерРаздела = 1 Или НомерРаздела = 3 Тогда
		...
		Пока Найти(ОбластьКодТекст, " ") <> 0 Цикл
			ОбластьКодТекст = СтрЗаменить(ОбластьКодТекст, " ", "");
		КонецЦикла;
...
Функция РазделТекущейОбласти(Форма, ТекущаяОбласть)
	...
	Если Найти(ИмяТекущейОбласти, "УдалитьСтроку") > 0 Тогда
		ИмяТекущейОбласти = СтрЗаменить(ИмяТекущейОбласти, "УдалитьСтроку", "");
	ИначеЕсли Найти(ИмяТекущейОбласти, "ДобавитьСтроку") > 0 Тогда
