...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для ИндКол = 0 По КвоКол - 1 Цикл
		...
		Если Найти(НомГрафИскл, СтрНомКол) > 0 Тогда
		
			Продолжить;
		
		КонецЕсли;
		
		Если ТекГрафа = "" ИЛИ Найти(СтрРассчитатьГрафы, СтрНомКол) > 0 Тогда
			...
			Для ИндСтр = 0 По КвоСтр - 2 Цикл // без первой строки
				...
				Если Найти(НомСтрокИскл, СтрНомСтр) = 0 Тогда
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого Обл Из ПолеТабличногоДокументаФормаОтчета.Области Цикл
		Если Обл.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки 
			И (Обл.Имя <> "ПолеНомерКорректировки"  И Обл.Имя <> "ШапкаОбособлннныеПодразделения") Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргНазв) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано наименование оганизации!", "ФормаОтчета", "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
