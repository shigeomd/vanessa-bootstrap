...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе // Это новый документ, установим реквизиты формы по умолчанию.
			...
			//ТабличныйДокумент.Области.ВидДокументаПервичный.Значение = "V";
		КонецЕсли;
...
Процедура УдалитьДополнительнуюСтраницу(Форма, ИмяТекущейСтраницыПанели = Неопределено)
	...
	//Форма.СтруктураРеквизитовФормы.ВыводитьСтраницуВТаблДокНаСервере = Ложь;
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Иначе
			...
			ИначеЕсли ЗаполненыСведенияОбИП Тогда
				...
				Иначе
					ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ФИО);
...
Функция ПолучитьИдентификаторПеревозчика(ДанныеВыгрузки, СтрокаСведенийПеревозчика, АдресПеревозчика)
	...
	ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СтрокаСведенийПеревозчика.П000010000306);
