...
Функция ПолучитьПараметрыИсполненияОтчета() Экспорт
	...
	Возврат Новый Структура("ИспользоватьПриВыводеЗаголовка,
...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.ОрганизацияРасшифровки) Тогда		
		ЗаголовокОтчета = ЗаголовокОтчета + " " + БухгалтерскиеОтчетыВызовСервераПовтИсп.ПолучитьТекстОрганизация(ПараметрыОтчета.ОрганизацияРасшифровки, Ложь);		
	КонецЕсли;
...
Процедура СформироватьКарту(Знач ПараметрыОтчета, АдресХранилища) Экспорт	
	...
	ТекстОписаниярасшифровок = БухгалтерскиеОтчетыВызовСервераПовтИсп.ПолучитьТекстОрганизация(
...
Функция ПолучитьНаборДанныхНачислениеРеализация(Запрос)
	...
	|	СУММА(ВТПроведеноРанжированиеОшибок.Сумма) КАК Сумма,
...
Функция ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос, УчетНДС2012 = Ложь)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериодаГраница, Регистратор, Счет В (&СписокСчетовАвансовВыданных), , Организация В (&Организация), , ) КАК ХозрасчетныйОбороты
	|ГДЕ
	...
	|			И Авансы.ДоговорКонтрагента = СоставКнигиПродажВЧастиАвансов.ДоговорКонтрагента
