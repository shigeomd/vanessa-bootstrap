...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Найти(Показатель.Имя, "П000110001003") = 1
		 ИЛИ Найти(Показатель.Имя, "П000120001003") = 1
		 ИЛИ Найти(Показатель.Имя, "П000130001003") = 1 Тогда
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Модифицированность = ЭтоЛистСтарогоОбразца;
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		ИначеЕсли ИмяПоказателя = "НомерКорректировки1" Тогда
			...
			Для Ном = 1 По 3 Цикл
				...
				Если Найти("0123456789", НомерКорр) > 0 Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2013Кв4");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.Период) Тогда
		...
		СтатусПроверки = Ложь;
	ИначеЕсли Найти(ВозможныеКодыПериода, ПараметрыВыгрузки.Период) = 0 Тогда
	...
	Для Каждого Стр Из Раздел1 Цикл
		...
		Если КодОКТМОРаздел1 <> 11 И КодОКТМОРаздел1 <> 8 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1.1 на странице " + НомерСтраницы + " не" + ?(КодОКТМОРаздел1  = 0, "", "верно") + " задан код ОКТМО!", "Раздел1", "П000110001003", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Для Каждого Стр Из Раздел12 Цикл
		...
		Если КодОКТМОРаздел12 <> 11 И КодОКТМОРаздел12 <> 8 Тогда
			СообщитьВТС("В разделе 1.2 на странице " + НомерСтраницы + " не" + ?(КодОКТМОРаздел12  = 0, "", "верно") + " задан код ОКТМО!", "Раздел12", "П000120001003", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Для Каждого Стр Из Раздел13 Цикл
		...
		Если КодОКТМОРаздел13 <> 11 И КодОКТМОРаздел13 <> 8 Тогда
			СообщитьВТС("В разделе 1.3 на странице " + НомерСтраницы + " не" + ?(КодОКТМОРаздел13  = 0, "", "верно") + " задан код ОКТМО!", "Раздел13", "П000130001003", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Для Каждого Стр Из Раздел24 Цикл
		...
		Пока ДанныеСтраницы.Свойство("П000240001001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			...
			Если КодНалПериодаРаздел24МнчКод <> 2 И КодНалПериодаРаздел24МнчКод <> 0 Тогда
				СообщитьВТС("В разделе 2.4 на странице " + НомерСтраницы + " в строке " + НомерСтроки + " не верно  задан код налогового периода!", "Раздел2", "П000240001004_"+НомерСтроки , НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
	...
	Для Каждого Стр Из ДанныеПрил6 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Приложение6", УзелРодитель.Владелец()) Тогда
			...
			КонецЦикла;
				...
				ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ДанныеСтраницы["П000950001000"]);
	...
	Для Каждого Стр Из ДанныеПрил7 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Приложение7", УзелРодитель.Владелец()) Тогда
			...
			Если СтрДлина(СокрЛП(ДанныеСтраницы["П000960002000"])) = 12 Тогда //ФЛ
				...
				ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ДанныеСтраницы["П000960001000"]);
...
Процедура РасчетВСтруктуре(Форма, ИмяЛиста)
	...
	Для Каждого Страница Из ТаблицаРаздела Цикл
		...
		ИначеЕсли ИмяЛиста = "Раздел24" Тогда 
			...
			//ТаблицаБлокаР2 = Форма[Форма.мСтруктураМногостраничныхРазделов["Раздел2"]].НайтиСтроки(Новый Структура("Представление", Страница.Представление));
...
Процедура УдалитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено)
	...
	Если ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		...
		ТабличныйДокумент.Вывести(
			...
			Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел24"));
		...
		ТабличныйДокумент.Вывести(
			Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(
			Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + "Раздел25"));
...
Процедура ПереопределитьЦветЯчеекВЗависимостиОтУсловия(Форма, ИмяТекущегоРаздела)
	...
	ИначеЕсли ИмяТекущегоРаздела = "Раздел24"	тогда
		...
		//ТаблицаБлокаР2 = ТаблицаРазделаР2.НайтиСтроки(Новый Структура("Представление", ТаблицаТекущейСтраницы.Представление))[0];
		ТаблицаРазделаР2 = Форма[Форма.мСтруктураМногостраничныхРазделов["Раздел2"]];
