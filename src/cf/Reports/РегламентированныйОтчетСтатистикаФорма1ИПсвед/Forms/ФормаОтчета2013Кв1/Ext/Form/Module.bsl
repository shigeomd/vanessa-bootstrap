		...
		Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы Цикл
			...
			Для Каждого Область Из ОбластьДопСтроки.Области Цикл
				...
				Если ОбластьТаблДок.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
					...
					ИмяКолонки = Лев(Область.Имя, Найти(Область.Имя, "_")-1);
					
					ОбластьТаблДок.Имя = Лев(Область.Имя, Найти(Область.Имя, "_")) + Формат(ИсходноеКоличествоСтрокГруппы + ТекущееКоличествоСтрокГруппы - НомНовойСтроки,  "ЧГ=");
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Найти(Область.Имя,"П000004000104") > 0 Или Найти(Область.Имя,"П000004000105") > 0 Тогда
			СтандартнаяОбработка = Ложь;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого ТекОбласть Из ПолеТабличногоДокументаФормаОтчета.Области Цикл
		Если Найти(ТекОбласть.Имя, "П000004000103") > 0 Тогда
	...
	Пока ТекИндекс2 > 0 Цикл
		ТекИндекс1 = ТекИндекс2 - 1;
		Для Каждого ТекОбласть2 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Области Цикл
			Если Найти(ТекОбласть2.Имя, "П000005000101") > 0 Тогда
				Для Каждого ТекОбласть1 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс1].Значение.Области Цикл
					Если Найти(ТекОбласть1.Имя, "П000005000101") > 0 Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ТекСумма <> 100 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
			"Не выполнено условие контроля: cумма данных по заполненным строкам графы 4.1 = 100!",
