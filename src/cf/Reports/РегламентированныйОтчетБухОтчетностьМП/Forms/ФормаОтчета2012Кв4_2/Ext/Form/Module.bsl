...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.КоличествоСтрокИнформационногоМакета = СтрЧислоСтрок(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2012Кв4_2_Изменения").ПолучитьТекст());
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ОтчетностьВРосстат) ИЛИ (ЗначениеЗаполнено(ОтчетностьВРосстат) И ОтчетностьВРосстат.ПолучитьОбъект() = Неопределено) Тогда 
		ОтчетностьВРосстат = Неопределено;
		ОпределитьСсылкуНаСуществующийОтчетВРосстат();
	КонецЕсли;
	Если КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <> КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
		Элементы.ГруппаБухгалтерскаяОтчетностьВРосстат.Видимость = Ложь;
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2012Кв4_2");
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Найти(Область.Имя, "П") = 1 Тогда
...
Процедура ПечататьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.АктивныйПункт = 1 Тогда
			Если Результат.Команда = "ПечататьСразу" Тогда
				РегламентированнаяОтчетностьКлиент.ВывестиМашиночитаемуюФорму(ЭтаФорма, "ПечататьСразуСДвухмернымШтрихкодомPDF417");
			Иначе
		...
		ИначеЕсли Результат.АктивныйПункт = 2 Тогда
			Если Результат.ВключатьКодыСтрок Тогда
				Если Результат.Команда = "ПечататьСразу" Тогда
					ПечататьИзФормы("ПечататьСразу");
				Иначе 
					ПечататьИзФормы("ПоказатьБланк");
				КонецЕсли;
			Иначе
				Если Результат.Команда = "ПечататьСразу" Тогда
					ПечататьИзФормы("ПечататьСразуБезКодов");
				Иначе 
		...
		ИначеЕсли Результат.АктивныйПункт = 3 Тогда
			Если Результат.Команда = "ПечататьСразу" Тогда
				ПечататьИзФормы("ПечататьСразуКНД0710098");
			Иначе 
				ПечататьИзФормы("ПоказатьБланкКНД0710098");
			КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.ТипАдреса = Неопределено ИЛИ ПустаяСтрока(СтруктураРеквизитовФормы.ТипАдреса) ИЛИ Найти("АдрФакт, АдрЮР, АдрПочт", СтруктураРеквизитовФормы.ТипАдреса) = 0 Тогда
		СтруктураРеквизитовФормы.ТипАдреса = ТипАдресаПоУмолчанию();
	КонецЕсли;
...
Процедура ПоказатьПериод(НаимТекущегоРаздела)
	...
	Если НаимТекущегоРаздела = "Баланс" Тогда
		...
		Если НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) <> НачалоДня(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета) Тогда
			СтрПериодОтчета = Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДФ=dd.MM.yyyy") + " - "
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", Знач ОбластьИмя = "")	
	...
	// ОТЧЕТ О ЦЕЛЕВОМ ИСПОЛЬЗОВАНИИ ПОЛУЧЕННЫХ СРЕДСТВ
	//
	ИначеЕсли ИмяСтраницыПанели = "ОЦИПС" Тогда
		
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "ОЦИПС" Тогда
			
			ОЦИПС = Форма.ТабличныйДокумент.Области;
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100620004, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100620004.Значение = ОЦИПС.П000100621004.Значение
				+ ОЦИПС.П000100621504.Значение
				+ ОЦИПС.П000100622004.Значение
				+ ОЦИПС.П000100623004.Значение
				+ ОЦИПС.П000100624004.Значение
				+ ОЦИПС.П000100625004.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100631004, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100631004.Значение = ОЦИПС.П000100631104.Значение
				+ ОЦИПС.П000100631204.Значение
				+ ОЦИПС.П000100631304.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100632004, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100632004.Значение = ОЦИПС.П000100632104.Значение
				+ ОЦИПС.П000100632204.Значение
				+ ОЦИПС.П000100632304.Значение
				+ ОЦИПС.П000100632404.Значение
				+ ОЦИПС.П000100632504.Значение
				+ ОЦИПС.П000100632604.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100630004, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100630004.Значение = ОЦИПС.П000100631004.Значение
				+ ОЦИПС.П000100632004.Значение
				+ ОЦИПС.П000100633004.Значение
				+ ОЦИПС.П000100635004.Значение;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100640004, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100640004.Значение = ОЦИПС.П000100610004.Значение
				+ ОЦИПС.П000100620004.Значение
				+ ОЦИПС.П000100630004.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100620005, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100620005.Значение = ОЦИПС.П000100621005.Значение
				+ ОЦИПС.П000100621505.Значение
				+ ОЦИПС.П000100622005.Значение
				+ ОЦИПС.П000100623005.Значение
				+ ОЦИПС.П000100624005.Значение
				+ ОЦИПС.П000100625005.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100631005, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100631005.Значение = ОЦИПС.П000100631105.Значение
				+ ОЦИПС.П000100631205.Значение
				+ ОЦИПС.П000100631305.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100632005, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100632005.Значение = ОЦИПС.П000100632105.Значение
				+ ОЦИПС.П000100632205.Значение
				+ ОЦИПС.П000100632305.Значение
				+ ОЦИПС.П000100632405.Значение
				+ ОЦИПС.П000100632505.Значение
				+ ОЦИПС.П000100632605.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100630005, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100630005.Значение = ОЦИПС.П000100631005.Значение
				+ ОЦИПС.П000100632005.Значение
				+ ОЦИПС.П000100633005.Значение
				+ ОЦИПС.П000100635005.Значение;
				
			КонецЕсли;	
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100640005, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100640005.Значение = ОЦИПС.П000100610005.Значение
				+ ОЦИПС.П000100620005.Значение
				+ ОЦИПС.П000100630005.Значение;
				
			КонецЕсли;	
									
		Иначе
			
			ОЦИПС = Форма.СтруктураДанныхОЦИПС;
			
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100620004, ОбластьИмя) > 0 Тогда
				
				ОЦИПС.П000100620004 = ОЦИПС.П000100621004
				+ ОЦИПС.П000100621504
				+ ОЦИПС.П000100622004
				+ ОЦИПС.П000100623004
				+ ОЦИПС.П000100624004
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100631004, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100631004 = ОЦИПС.П000100631104
				+ ОЦИПС.П000100631204
				+ ОЦИПС.П000100631304;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100632004, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100632004 = ОЦИПС.П000100632104
				+ ОЦИПС.П000100632204
				+ ОЦИПС.П000100632304
				+ ОЦИПС.П000100632404
				+ ОЦИПС.П000100632504
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100630004, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100630004 = ОЦИПС.П000100631004
				+ ОЦИПС.П000100632004
				+ ОЦИПС.П000100633004
				+ ОЦИПС.П000100635004;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100640004, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100640004 = ОЦИПС.П000100610004
				+ ОЦИПС.П000100620004
				+ ОЦИПС.П000100630004;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100620005, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100620005 = ОЦИПС.П000100621005
				+ ОЦИПС.П000100621505
				+ ОЦИПС.П000100622005
				+ ОЦИПС.П000100623005
				+ ОЦИПС.П000100624005
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100631005, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100631005 = ОЦИПС.П000100631105
				+ ОЦИПС.П000100631205
				+ ОЦИПС.П000100631305;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100632005, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100632005 = ОЦИПС.П000100632105
				+ ОЦИПС.П000100632205
				+ ОЦИПС.П000100632305
				+ ОЦИПС.П000100632405
				+ ОЦИПС.П000100632505
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100630005, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100630005 = ОЦИПС.П000100631005
				+ ОЦИПС.П000100632005
				+ ОЦИПС.П000100633005
				+ ОЦИПС.П000100635005;
			...
			Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000100640005, ОбластьИмя) > 0 Тогда
				...
				ОЦИПС.П000100640005 = ОЦИПС.П000100610005
				+ ОЦИПС.П000100620005
				+ ОЦИПС.П000100630005;
...
Процедура УстановитьЗатенениеОбластиПоказателя(СтрокаНастройки)
	...
	Если СтрокаНастройки.ВключатьВОтчет Тогда
		//Узор = ТипУзораТабличногоДокумента.БезУзора;
		ЦветТекста = Новый Цвет(0, 0, 0);
	Иначе
		//Узор = ТипУзораТабличногоДокумента.Узор14;
	...
	Для Каждого ОбластьЗатенения Из СтрокаНастройки.ОбластиЗатенения Цикл
		...
		//ОбластьПоказателя.ЦветУзора = Новый Цвет(192, 192, 192);
		//ОбластьПоказателя.Узор = Узор;
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	ФормаНастройкиОтчета = ПолучитьФорму(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 6) + ".НастройкиОтчета", ПараметрыФормы, ЭтаФорма);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если СтрДлина(СокрЛП(мДанныеОтчета.ПолеТабличногоДокументаБаланс.П000010000702)) <> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан код строки ""Капитал и резервы"" бухгалтерского баланса", "Баланс", "П000010000702");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ПроверитьКодСтрокиОПУ(СтатусПроверки, ИмяПоказателя, НаименованиеСтроки)
	...
	Если ЗначениеЗаполнено(КодСтроки) ИЛИ ЗначениеЗаполнено(Графа03) ИЛИ ЗначениеЗаполнено(Графа04) Тогда
		Если ПустаяСтрока(КодСтроки) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		ИначеЕсли СтрДлина(КодСтроки) <> 4 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Неверно указан код строки """ + НаименованиеСтроки + """ отчета о прибылях и убытках", "Баланс", ИмяПоказателя + "02");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати) Экспорт
	...
	Если (ВидПечати = "ПечататьСразуКНД0710098" ИЛИ ВидПечати = "ПоказатьБланкКНД0710098") Тогда
		СтруктураПараметров.Очистить();
		СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма по КНД 0710098");
		СтруктураПараметров.Вставить("ОриентацияЛиста",    "Портрет");
		Структурапараметров.Вставить("ИмяОбластиТаблПоля", "");
		СтруктураПараметров.Вставить("ВидПечати",          ВидПечати);
		ВывестиНаПечатьКНД0710098(СтруктураПараметров);
		
		Если ВидПечати = "ПоказатьБланкКНД0710098" Тогда
	...
	Если Найти(ВидПечати, "ПечататьСразу") > 0 Тогда
...
&НаСервере
Процедура ВывестиНаПечатьКНД0710098(ПараметрыПечати)
	...
	ФИОБух = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ТабличныйДокумент.Области.ОргБухгалтер.Значение);
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		...
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2013Кв1_КНД0710098_1");
		...
		Если ТипПодписанта = "1" Тогда
			...
			ФИОРук = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРег(ТабличныйДокумент.Области.ОргДиректор.Значение));
		...
		Иначе
			...
			ФИОПредставителя = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРег(СведенияОПредставителе.ФИОПредставителя));
	...
	Иначе
		...
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_1");
		
		ФИОРук = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ТабличныйДокумент.Области.ОргДиректор.Значение);
	...
	ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_2");
	
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2013Кв1_КНД0710098_2");
	Иначе
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2012Кв1_КНД0710098_2");
	КонецЕсли;
...
Процедура ЗаполнитьСведенияПодписантов(ПараметрыВыгрузки)
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания >= '2013-01-01' Тогда
		...
		Если ТипПодписанта = "1" Тогда
			...
			ФИОРук = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.ОргДиректор);
		...
		Иначе
			...
			ФИОПредставителя = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СведенияОПредставителе.ФИОПредставителя);
	...
	Иначе
		...
		Если ВыгружатьСведенияБухгалтера Тогда
			ФИОБух = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.ОргБухгалтер);
...
Процедура ВывестиНаПечатьБаланс(ПараметрыПечати)
	...
	ПечатьБезКодов = (ПараметрыПечати.ВидПечати = "ПечататьСразуБезКодов" ИЛИ ПараметрыПечати.ВидПечати = "ПоказатьБланкБезКодов");
...
Процедура ВывестиНаПечатьОЦИПС(ПараметрыПечати)
	...
	СоставОтчета = ЭтаФорма[НастройкиСоставаПоказателейОтчета.ОЦИПС];
	ИсходныйТабличныйДокумент = ТабличныйДокумент;
...
Процедура ИнициализироватьНастройкиСоставаПоказателейОтчетов()
	...
	ТекстXML = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакетаНастройкиСоставаОтчетов()).ПолучитьТекст();
...
Процедура ОткрытьУведомление(ОткрытьПринудительно = Ложь) Экспорт
	...
	Если ПоказатьОписаниеИзменений Тогда
		...
		ОткрытьФорму("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УведомлениеОбИзмененияхВФормеОтчета",
...
Процедура СоздатьБухгалтерскуюОтчетностьВРосстатНажатие(Элемент)
	Если Не ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) Тогда 
		Оповещение = Новый ОписаниеОповещения("СоздатьБухгалтерскуюОтчетностьВРосстат", ЭтотОбъект);
		СохранитьНаКлиенте(, Оповещение);
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ОпределитьСсылкуНаСуществующийОтчетВРосстат()
	Если Не ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСохраненныйДок) Тогда 
		ОтчетностьВРосстат = Неопределено;
	...
	Для Каждого Стр Из Результат Цикл 
		...
		Если ДанныеОтчета.БухгалтерскаяОтчетность = СтруктураРеквизитовФормы.мСохраненныйДок Тогда
			ОтчетностьВРосстат = Стр.Ссылка;
...
&НаКлиенте
Процедура СоздатьБухгалтерскуюОтчетностьВРосстат(Отказ = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт 
	Если Не ЗначениеЗаполнено(ОтчетностьВРосстат) Тогда
		Попытка
			...
			ПолучитьФорму("Отчет.РегламентированныйОтчетСтатистикаБухОтчетность.Форма.ОсновнаяФорма", ПараметрыФормы).Открыть();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если "СозданОтчетВРосстат" = ИмяСобытия И ТипЗнч(Параметр) = Тип("Структура") Тогда 
		БухОтчетность = Неопределено;
		Параметр.Свойство("БухОтчетность", БухОтчетность);
		Если БухОтчетность = СтруктураРеквизитовФормы.мСохраненныйДок Тогда
			ОтчетностьВРосстат = Параметр.ОтчетВРосстат;
			Элементы.СоздатьБухгалтерскуюОтчетностьВРосстат.Заголовок = "Отчет в Росстат";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
