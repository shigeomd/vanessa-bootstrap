...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2012Кв1_ФормаОтчета"));
...
Функция ПолучитьПодрядИдущиеЦифрыСправаИзстроки(СтрокаФормата)
	...
	Для НомСимв = 0 По ДлинаСтрокиФормата - 1  Цикл
		Если Найти(СтрокаПодходящихСимволов, Сред(СтрокаФормата, ДлинаСтрокиФормата - НомСимв, 1)) > 0 Тогда
			ПрерватьЦиклДля = Истина;
			Пока Найти(СтрокаПодходящихСимволов, Сред(СтрокаФормата, ДлинаСтрокиФормата - НомСимв, 1)) > 0 Цикл
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	// номер строки многострочного раздела определим
	// по имени выделенной ячейки
	НомУдаляемойСтрокиСинхронизируемый = Число( Сред( ИмяЯчейкиСинхронизируемой, Найти( ИмяЯчейкиСинхронизируемой, "_") + 1));
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ЗначениеЗаполнено(НомСтр) Тогда
		...
		Если Найти("12.16.17", Сред(ОбластьИмя, 12, 2)) > 0 Тогда
		...
		ИначеЕсли Найти("01.02", Сред(ОбластьИмя, 12, 2)) > 0 Тогда
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2012Кв1_ФормаОтчета"));
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если (Область.Имя = КодПоказателя) ИЛИ (Найти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0) ИЛИ (Найти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы2) > 0) Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Найти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы1) > 0 Или Найти(Область.Имя, СтруктураРеквизитовФормы.мИдГруппы2) > 0 Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// КОНТРОЛЬ показателей формы (Указания ФСН по заполнению Формы С-2)
	// 1) если заполнена графа 1, то графа 13 > 0 по всем строкам
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000002000101_" + Формат(НомСтр, "ЧГ=")) Цикл
		Если ЗначениеЗаполнено(ПараметрыВыгрузки["П000002000101_" + Формат(НомСтр, "ЧГ=")])
			И ПараметрыВыгрузки["П000002000113_" + Формат(НомСтр, "ЧГ=")] = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если заполнена графа 1, то графа 13 > 0 по всем строкам!",
				ИмяПоляТаблДокумента, "П000002000113_" + Формат(НомСтр, "ЧГ="));
			СтатусПроверки = Ложь;
			Прервать;
		КонецЕсли;
		НомСтр = НомСтр + 1;
	КонецЦикла;
	// 2) если не заполнена графа 11, то графа 21 < 100% по всем строкам
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ПараметрыВыгрузки, "П000001000111_" + Формат(НомСтр, "ЧГ=")) Цикл
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки["П000001000111_" + Формат(НомСтр, "ЧГ=")])
			И ПараметрыВыгрузки["П000002000121_" + Формат(НомСтр, "ЧГ=")] >= 100 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: если не заполнена графа 11, то графа 21 < 100% по всем строкам!",
				ИмяПоляТаблДокумента, "П000002000121_" + Формат(НомСтр, "ЧГ="));
			СтатусПроверки = Ложь;
			Прервать;
		КонецЕсли;
		НомСтр = НомСтр + 1;
...
Функция СформироватьСтруктуруПараметров()
	...
	// Для ПОСЛЕДУЮЩЕГО (ПЕРЕД ВЫГРУЗКОЙ) контроля показателей формы (Указания ФСН по заполнению Формы С-2).
	// 1) Если заполнена графа 1, то графа 13 > 0 по всем строкам.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000002000101_" + Формат(НомСтр, "ЧГ=")) Цикл
		ПараметрыВыгрузки.Вставить("П000002000101_" + Формат(НомСтр, "ЧГ="),
			ТабДокумент["П000002000101_" + Формат(НомСтр, "ЧГ=")]);
		ПараметрыВыгрузки.Вставить("П000002000113_" + Формат(НомСтр, "ЧГ="),
			ТабДокумент["П000002000113_" + Формат(НомСтр, "ЧГ=")]);
		НомСтр = НомСтр + 1;
	КонецЦикла;
	// 2) Если не заполнена графа 11, то графа 21 < 100% по всем строкам.
	НомСтр = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000001000111_" + Формат(НомСтр, "ЧГ="))
		И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ТабДокумент, "П000002000121_" + Формат(НомСтр, "ЧГ=")) Цикл
		ПараметрыВыгрузки.Вставить("П000001000111_" + Формат(НомСтр, "ЧГ="),
			ТабДокумент["П000001000111_" + Формат(НомСтр, "ЧГ=")]);
		ПараметрыВыгрузки.Вставить("П000002000121_" + Формат(НомСтр, "ЧГ="),
			ТабДокумент["П000002000121_" + Формат(НомСтр, "ЧГ=")]);
		НомСтр = НомСтр + 1;
	КонецЦикла;
