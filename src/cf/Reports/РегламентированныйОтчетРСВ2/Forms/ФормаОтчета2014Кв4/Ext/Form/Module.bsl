...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакетаСписковВыбора());
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь, СменилсяПризнакПодписанта = Ложь)
	...
	Если СтруктураРеквизитовФормы.ИзмененаОрганизация
	 ИЛИ НЕ ЗначениеЗаполнено(ЗначениеПоказателяРаздела(ЭтаФорма, "Титульный", "ОКВЭД")) Тогда
		УстановитьПоказательРаздела(ЭтаФорма, "Титульный", "ОКВЭД", ОКВЭД);
	КонецЕсли;
	...
	Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы Цикл
		...
		Для Каждого Область Из ОбластьДопСтроки.Области Цикл
			...
			Если ОбластьТаблДок.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
				...
				ОбластьТаблДок.Имя = Лев(Область.Имя, Найти(Область.Имя, "_"))
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если НЕ Раздел.МногостраничностьВРазделе
			...
			ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(
				...
				ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + Раздел.ИмяСтраницы));
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, ТекущаяСтраница) Экспорт
	...
	// Jпределяем текущую область таблицы.
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ОбластьИмя, "ДокументПредставителя") > 0 Тогда
...
Процедура ОбработкаВыбораВПолеТабличногоДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Текст = ДобавитьРеквизит(Текст, СтруктураРеквизитовФормы.ДокументПредставителя.Серия, "");
		...
		Текст = ДобавитьРеквизит(Текст, Формат(СтруктураРеквизитовФормы.ДокументПредставителя.ДействуетС, "ДФ=dd.MM.yyyy"), " , действует с ");
		Текст = ДобавитьРеквизит(Текст, Формат(СтруктураРеквизитовФормы.ДокументПредставителя.ДействуетПо, "ДФ=dd.MM.yyyy"), " по ");
...
Процедура ПечататьСразу(Команда)
	...
	ДополнительныеПараметры.Вставить("Команда", Команда);
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПечататьСразуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	МакетПечатнойФормы = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(Сред(Лев(
		ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7), Сред(
		ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 7), ПараметрыОтчета, Контейнер);
...
Процедура Расшифровать(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьРасшифровкуОтчета(Сред(Лев(
		ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7), Сред(ЭтаФорма.ИмяФормы, Найти(
...
Функция СформироватьСтруктуруПараметров()
	...
	ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Титульный.ГлаваХозяйстваФИО));
	...
	ФИОПодписанта = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Титульный.ПодписантФИО));
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки700_2014");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеРаздела2, "П000020000101_" + Формат(Инд, "ЧГ=0")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ДанныеРаздела2["П000020000102_" + НомерСтроки]);
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеРаздела3, "П000030000101_" + Формат(Инд, "ЧГ=0")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ДанныеРаздела3["П000030000102_" + НомерСтроки]);
...
Процедура ЗаменитьБуквуЁ(РедактируемаяСтрока, НазваниеРеквизита)
	...
	Если Найти(ВРег(РедактируемаяСтрока), "Ё") > 0 Тогда
		...
		РегламентированнаяОтчетностьКлиентСервер.СообщитьОбОшибке("В реквизите """ + НазваниеРеквизита + """ (" + РедактируемаяСтрока +") буква Ё заменена на Е.");
...
Процедура ЗаполнитьСодержательнуюЧасть(ПараметрыВыгрузки, УзелРодитель)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел2, "П000020000101_" + Формат(Инд, "ЧГ=0")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			НомерПП = НомерПП + 1;
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Раздел2["П000020000102" + Постфикс]);
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел3, "П000030000101_" + Формат(Инд, "ЧГ=0")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			НомерПП = НомерПП + 1;
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Раздел3["П000030000102" + Постфикс]);
