...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь, СменилсяПризнакПодписанта = Ложь)
	...
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(РегламентированнаяОтчетностьКлиентСервер.ВернутьЗначениеПараметраСЛиста(ЭтаФорма, "Титульный", "ОКВЭД"))) Тогда
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ОбластьИмя, "СНИЛС") > 0 Тогда
	...
	ИначеЕсли Найти(ОбластьИмя, "КолХозяйств") > 0 Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Титульный.ОргНазв));
	...
	ФИОПодписанта = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Титульный.ОргПодписантФамилия));
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки700");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Пока ДанныеРаздела22.Свойство("П000220001002_" + Формат(НомСтроки, "ЧГ="), ФИОЧленаФХ) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ФИОЧленаФХ);
...
Процедура ЗаполнитьСодержательнуюЧасть(ПараметрыВыгрузки, УзелРодитель)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел22, "П000220001002_" + Формат(НомСтр, "ЧГ=")) Цикл
		...
		Если ЗначениеЗаполнено(Раздел22["П000220001002_" + Формат(НомСтр, "ЧГ=")]) Тогда
			...
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Раздел22["П000220001002_" + Формат(НомСтр, "ЧГ=")]));
