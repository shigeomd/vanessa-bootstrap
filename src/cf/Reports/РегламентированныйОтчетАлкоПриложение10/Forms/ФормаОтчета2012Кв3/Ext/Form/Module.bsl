...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ПоказательВводитсяТолькоЧерезФорму = (Найти(КодПоказателя, "П000000000105") = 1);
	...
	Если Найти(КодПоказателя, "П000000000105") = 1
	 ИЛИ Найти(КодПоказателя, "П000010000102") = 1
	 ИЛИ Найти(КодПоказателя, "П000010000103") = 1 Тогда
...
Процедура ЗаполнитьСвязанныеПоказатели(ИмяОбласти, РезультатВыбора)
	...
	ИначеЕсли Найти(ИмяОбласти, "П000010000104") = 1 Тогда
	...
	Пока ДанныеРаздела.Свойство("П000010000101" + Постфикс) Цикл
		...
		Если ЗаполненаСтрокаДекларация(ДанныеРаздела, НомерСтроки) Тогда
			...
			Если НЕ ПустаяСтрока(ВидПродукции) ИЛИ НЕ ПустаяСтрока(КодВидаПродукции) ИЛИ ЕстьЧисловыеДанные Тогда
				Если ПустаяСтрока(ВидПродукции) Тогда
					//ТекстОписанияОшибки = "В строке " + НомерСтрокиТекстовый + " раздела Декларация не указано наименование вида продукции!";
					//СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Декларация", ИмяПоказателяВидПродукции);
				КонецЕсли;
