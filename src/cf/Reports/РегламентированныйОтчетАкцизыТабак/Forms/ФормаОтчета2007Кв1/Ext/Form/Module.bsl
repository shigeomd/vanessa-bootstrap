...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		Сведения = "ИННЮЛ, КППЮЛ, НаимЮЛПол, ТелОрганизации, КодНО, НаимНО, ТипНП, ОКВЭД, ОКАТО, ОКТМО";
	КонецЕсли;
	...
	Если (НЕ ВПрограммеИзмененаДатаПодписи) И (Результат <> Неопределено) Тогда
		...
		ОКВЭД = РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(Результат.ОКВЭД, 8, " ", 1);
		ОКВЭД = Сред(ОКВЭД, 1, 2) + "." + Сред(ОКВЭД, 4, 2) + "." + Сред(ОКВЭД, 7, 2);
		СтруктураРеквизитовФормы.мКодОКВЭД = ОКВЭД;
		СтруктураДанныхРаздел1.Вставить("ОКВЭДРаздел1", ОКВЭД);
...
Процедура ЗаполнитьПечатнуюФорму(ПараметрыПечати, МакетПечатнойФормы, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Область Из ИсходныйМакет.Области Цикл
		...
		Если Найти(Область.Имя, "ДобавлениеСтроки") > 0
			Или Найти(Область.Имя, "ДобавитьСтроку") > 0
			Или Найти(Область.Имя, "УдалитьСтроку") > 0 Тогда 
			Продолжить;
		КонецЕсли;
		
		Если Найти(Область.Имя, "ДобавитьСтраницу") > 0
			Или Найти(Область.Имя, "УдалитьСтраницу") > 0 Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя)
	...
	Для Каждого Обл Из ВыбТабличноеПоле.Области Цикл 
		Если Найти(Обл.Имя, "УдалитьСтроку") = 1 Тогда 
			Обл.Текст = "";
...
//		мСохраненныйДок.Организация = Отчет.Организация;
	Иначе
...
Процедура ВыгрузитьТитульныйЛистИСведенияОФизЛицах(Текст)
	...
	Если ПрименяетсяФормат300006(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		Текст.ДобавитьСтроку("ВерФОтч:3.00006");
		Текст.ДобавитьСтроку("ПериодВерОтч:01.12.2013,");
...
Процедура ПроверкаРаздел2 (СтруктураРезультата)
	...
	Для Каждого Страница1 Из Приложение2 Цикл
		Если Страница1.Данные.П000520002000 = "460" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000520003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000520003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000520003007_" + НомерСтроки1];
				КонецЕсли;
	...
	Для Каждого Страница1 Из Приложение2 Цикл
		Если Страница1.Данные.П000520002000 = "460" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000520003008_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000520003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000520003008_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение2 Цикл
		Если Страница1.Данные.П000520002000 = "460" Тогда
			...
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000520003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000520003014_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000520003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
	...
	Для Каждого Страница1 Из Приложение3 Цикл
		Если Страница1.Данные.П000530002000 = "460" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000530003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000530003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000530003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение3 Цикл
		Если Страница1.Данные.П000530002000 = "460" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000530003008_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000530003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000530003008_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение3 Цикл
		Если Страница1.Данные.П000530002000 = "460" Тогда
			...
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000530003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000530003014_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000530003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
...
Процедура ПроверкаРаздел3 (СтруктураРезультата)
	...
	Для Каждого Страница1 Из Приложение5 Цикл
		Если Страница1.Данные.П000550002000 = "450" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000550003008_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000550003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000550003008_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение5 Цикл
		Если Страница1.Данные.П000550002000 = "450" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000550003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000550003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000550003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение6 Цикл
		Если Страница1.Данные.П000560002000 = "450" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000560003008_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000560003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000560003008_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение6 Цикл
		Если Страница1.Данные.П000560002000 = "450" Тогда
			Сумма2 = 0;
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000560003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000560003013_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000560003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
			Сумма1 = Сумма1 + Сумма2;
	...
	Для Каждого Страница1 Из Приложение5 Цикл
		Если Страница1.Данные.П000550002000 = "450" Тогда
			...
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000550003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000550003014_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000550003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
	...
	Для Каждого Страница1 Из Приложение6 Цикл
		Если Страница1.Данные.П000560002000 = "450" Тогда
			...
			НомерСтроки1 = 1;
			Пока Страница1.Данные.Свойство("П000560003007_" + НомерСтроки1) Цикл
				Если Страница1.Данные["П000560003014_" + НомерСтроки1] > 0 Тогда
					Сумма2 = Сумма2 + Страница1.Данные["П000560003007_" + НомерСтроки1];
				КонецЕсли;
				НомерСтроки1 = НомерСтроки1 + 1;
			КонецЦикла;
