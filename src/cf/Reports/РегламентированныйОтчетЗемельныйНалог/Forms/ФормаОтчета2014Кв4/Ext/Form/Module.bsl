...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2014Кв4");
		...
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020024003_Процент.Значение)
		   И ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020024003_Сумма.Значение) Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru='Строки 240а и 240б не могут заполняться одновременно. Значение в строке 240%1 очищено'"), ЛитераСтроки);
			Сообщение.Сообщить();
		КонецЕсли;
