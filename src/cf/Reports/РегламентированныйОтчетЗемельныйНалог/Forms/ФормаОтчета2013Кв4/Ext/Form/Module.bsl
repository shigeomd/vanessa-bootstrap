...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета         = Метаданные.Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].Синоним;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ЭтоПолеВводаОснованияЛьготы = (Найти(ИмяЯчейки, "_Основание") > 0);
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Найти(Показатель.Имя, "П000010002003") = 1
		 ИЛИ Найти(Показатель.Имя, "П000020002003") = 1 Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗначениеЗаполнено(ПараметрыВыгрузки.ИННРеорг) ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.КППРеорг) ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.КодРеорг) Тогда
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.КодРеорг) Тогда
			...
			СтатусПроверки = Ложь;
		ИначеЕсли Найти("0,1,2,3,5,6", ПараметрыВыгрузки.КодРеорг) = 0 Тогда
...
Процедура ПроверитьКС_1_35(СтруктураРезультата)
	...
	Льготы = ЛьготыПоКадастровымНомерам(СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка.Организация, СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка.ДатаОкончания);
...
&НаСервере
Функция ЛьготыПоКадастровымНомерам(ОрганизацияВладелец, ДатаСреза)
	
	РегистрРегистрации = Метаданные.РегистрыСведений.Найти("РегистрацияЗемельныхУчастков");
	РегистрКадастровогоУчета = Метаданные.РегистрыСведений.Найти("СведенияКадастровогоУчета");
	
	Если РегистрРегистрации <> Неопределено
	   И РегистрРегистрации.Ресурсы.Найти("КадастровыйНомер") <> Неопределено Тогда
		Запрос = Новый Запрос;
	...
	ИначеЕсли РегистрРегистрации <> Неопределено И РегистрКадастровогоУчета <> Неопределено Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
