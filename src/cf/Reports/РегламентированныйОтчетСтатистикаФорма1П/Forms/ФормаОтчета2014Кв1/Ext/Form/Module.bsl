...
Процедура ЗаполнитьЗначенияНомеровСтрок()
	...
	//////	Для Инд2 = 1 По 3 Цикл
	//////		Имя = "Дата" + Формат(Инд1, "ЧЦ=1") + "_" + Формат(Инд2, "ЧЦ=1");
	//////		Если НЕ СтруктураДанныхРаздел1.Свойство(Имя) Тогда 
	//////			СтруктураДанныхРаздел1.Вставить(Имя);
	//////		КонецЕсли;
	//////	КонецЦикла;
	//////КонецЦикла;
	...
	//////Для Ном = 19 По 30 Цикл
	//////	НомСтр = Формат(Ном, "ЧЦ=2");
	//////	КодПоказателя = "П03000" + НомСтр + "01";
	//////	СтруктураДанныхРаздел2_3.Вставить(КодПоказателя, 0);
	//////КонецЦикла;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если Найти(Область.Имя, "П000040001003_") > 0 Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	ИндексПоиска = Найти(ОбластьИмя, "_");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ИначеЕсли Найти("П010000401,П010000501,П010000601,П010000701", Область.Имя) > 0 Тогда
			СтандартнаяОбработка = Ложь;
			...
			Модифицированность = Истина;
		ИначеЕсли Найти("П010000801,П010000901,П010001001,П010001101,П010001201", Область.Имя) > 0 Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел4_6" Тогда
		Если Найти(Область.Имя, "П000040001003_") > 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000040001001_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "СправочникОКСМ");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел8" Тогда
		Если Найти(Область.Имя, "П000080001003_") > 0 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка);
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000080001001_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыОКВЭД_10");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел9" Тогда
		Если Найти(Область.Имя, "П000090001003_") > 0 Или Область.Имя = "ОргКодОКВЭД" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыОКВЭД_10");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				КодПоказателя = ПараметрВводаПоля.Показатель;
				...
				ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка);
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000090001001_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыОКВЭД_10");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел10" Тогда
		...
		Если Найти(Область.Имя, "П000000001003_") > 0 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка);
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000000001001_") > 0 Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "КодыОКВЭД_10");
			//создание динамического списка выбора
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
