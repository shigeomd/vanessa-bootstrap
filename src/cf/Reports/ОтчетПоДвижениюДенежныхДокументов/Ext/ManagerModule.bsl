...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Пока ПоПроводкам Или ПоДокументам  Цикл
		...
		Если ДатаЛиста >= ПараметрыОтчета.НачалоПериода Тогда
			...
			НаименованиеОрганизации = БухгалтерскиеОтчетыВызовСервераПовтИсп.ПолучитьТекстОрганизация(, ПараметрыОтчета.Организация);
...
//		ЛистовЗаГод = 1 стр
		...
		Если (ПоПроводкам И МЕСЯЦ(ДатаЛиста) <> МЕСЯЦ(ВыборкаИтоговПоДням.Период))
			...
			ОбластьЛистовЗаМесяцОтчет.Параметры.НадписьЛистовЗаМесяц = НСтр("ru='Количество листов за месяц:'") + " " + ЛистовЗаМесяц;
	...
	Если ВывестиПодвал Тогда
		Д = ПараметрыОтчета.КонецПериода;
		Если (НачалоДня(КонецМесяца(ДатаЛиста)) = ДатаЛиста) Или
			 (НачалоМесяца(ДатаЛиста) <> НачалоМесяца(ПараметрыОтчета.КонецПериода)) Или
		...
		Если (НачалоДня(КонецГода(ДатаЛиста)) = ДатаЛиста) Или 
			 (НачалоГода(ДатаЛиста) <> НачалоГода(ПараметрыОтчета.КонецПериода)) Или 
			 (ПоследнийЛист = 2) Тогда
			
			ОбластьЛистовЗаГодОтчет.Параметры.НадписьЛистовЗаГод = НСтр("ru='Количество листов за год:'") + " " + ЛистовЗаГод; 
