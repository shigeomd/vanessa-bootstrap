...
Функция ПечатьНаСервере(ВидПечати)
	...
	Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка СтруктураРеквизитовФормы.мПечатныеФормы
		Попытка // попытка удалить область "Ограничители"
			...
			Пока Найти(ТабДок1.Область(НомСтр, 2, НомСтр, 2).Имя, "П010102000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
			...
			Пока Найти(ТабДок2.Область(НомСтр, 2, НомСтр, 2).Имя, "П020102000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
			...
			Если ОбщегоНазначения.ПроверитьВыводТабличногоДокумента(ТабДок1, ТабДок2, Ложь) Тогда
				Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.ВысотаТаблицы,
				...
				Попытка // попытка удалить область "Ограничители"
					...
					НомСтр = 4; // номер строки "верхнего левого" показателя многострочной части раздела.
					Пока Найти(ТабДок2.Область(НомСтр, 2, НомСтр, 2).Имя, "П030102000101") > 0 Цикл
						НомСтр = НомСтр + 1;
					КонецЦикла;
					НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
	...
	Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка СтруктураРеквизитовФормы.мПечатныеФормы
		Попытка // попытка удалить область "Ограничители"
			...
			Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(НомСтр, 2, НомСтр, 2).Имя, "П010202000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2014Кв2");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	СтрПоляТаблДокумента = "ПолеТабличногоДокументаРаздел1,ПолеТабличногоДокументаРаздел2";
