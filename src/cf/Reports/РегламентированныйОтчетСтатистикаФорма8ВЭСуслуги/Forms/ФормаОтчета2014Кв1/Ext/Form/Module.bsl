...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "21/08/2013";
		...
		Если ЗначениеЗаполнено(ЗначСтруктуры.НомерСтрокиПоказателя) И ЗначениеЗаполнено(ЗначСтруктуры.НомерКолонкиПоказателя)
			...
			Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка СтруктураРеквизитовФормы.мПечатныеФормы
				Попытка // попытка удалить область "Ограничители"
					...
					Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(НомСтр, ЗначСтруктуры.НомерКолонкиПоказателя, НомСтр, ЗначСтруктуры.НомерКолонкиПоказателя).Имя, ЗначСтруктуры.ПодстрокаПоискаПоказателя) > 0 Цикл
						НомСтр = НомСтр + 1;
					КонецЦикла;
		...
		Если ЗначениеЗаполнено(ЗначСтруктуры.ПодстрокаПоискаПоказателя) И ЗначениеЗаполнено(ЗначСтруктуры.УдаляемыйЗаголовок) Тогда
			...
			Пока ТекИндекс2 > 0 Цикл
				...
				Для Каждого ТекОбласть2 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс2].Значение.Области Цикл
					Если Найти(ТекОбласть2.Имя, ЗначСтруктуры.ПодстрокаПоискаПоказателя) > 0 Тогда
						Для Каждого ТекОбласть1 Из СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс1].Значение.Области Цикл
							Если Найти(ТекОбласть1.Имя, ЗначСтруктуры.ПодстрокаПоискаПоказателя) > 0 Тогда
								Попытка
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого ТекПечФорма Из СтруктураРеквизитовФормы.мПечатныеФормы Цикл
		...
		Если Найти(ТекТабДок.Область(5, 4, 5, 4).Имя, "П000002071200") > 0 Тогда
	...
	Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка мПечатныеФормы
		Попытка // попытка удалить область "Ограничители"
			...
			Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(НомСтр, 2, НомСтр, 2).Имя, "П000101000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
	...
	Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка СтруктураРеквизитовФормы.мПечатныеФормы
		Попытка // попытка удалить область "Ограничители"
			...
			Пока Найти(ТабДок1.Область(НомСтр, 2, НомСтр, 2).Имя, "П010103000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
			...
			Пока Найти(ТабДок2.Область(НомСтр, 2, НомСтр, 2).Имя, "П020103000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
			...
			Если ОбщегоНазначения.ПроверитьВыводТабличногоДокумента(ТабДок1, ТабДок2, Ложь) Тогда
				...
				Попытка // попытка удалить область "Ограничители"
					...
					НомСтр = 4; // номер строки "верхнего левого" показателя многострочной части раздела.
					Пока Найти(ТабДок2.Область(НомСтр, 2, НомСтр, 2).Имя, "П030103000101") > 0 Цикл
						НомСтр = НомСтр + 1;
					КонецЦикла;
					НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
	...
	Пока ТекИндекс > 0 Цикл // перебор в обратном порядке элементов списка СтруктураРеквизитовФормы.мПечатныеФормы
		Попытка // попытка удалить область "Ограничители"
			...
			Пока Найти(СтруктураРеквизитовФормы.мПечатныеФормы[ТекИндекс].Значение.Область(НомСтр, 2, НомСтр, 2).Имя, "П010203000101") > 0 Цикл
				НомСтр = НомСтр + 1;
			КонецЦикла;
			НомВерхСтрДляУдаления = НомСтр; // первая из пустых строк, добавленных процедурой ВывестиНаПечатьЛистОбщая
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел2" Тогда
		...
		Если ПодстрокаВИмениПоказателя11 = "П0000020001"
		 Или ПодстрокаВИмениПоказателя11 = "П0000020002"
		 Или ПодстрокаВИмениПоказателя11 = "П0000020101"
		 Или ПодстрокаВИмениПоказателя11 = "П0000020102" Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2013Кв1");
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ПодстрокаВИмениПоказателя11 = "П0000020001"
		...
		ТабличныйДокумент.Области["П0000020002" + Сред(Область.Имя, 12)].Значение =
			?(ЗначениеЗаполнено(РезультатВыбора["Код"]), Число(РезультатВыбора["Код"]), 0);
		ТабличныйДокумент.Области["П0000020101" + Сред(Область.Имя, 12)].Значение =
			?(ЗначениеЗаполнено(РезультатВыбора["Код"]), СокрЛП(РезультатВыбора["Название"]), "");
		ТабличныйДокумент.Области["П0000020102" + Сред(Область.Имя, 12)].Значение =
			?(ЗначениеЗаполнено(РезультатВыбора["Код"]), Число(РезультатВыбора["Код"]), 0);
	// Раздел 3.
	ИначеЕсли ПодстрокаВИмениПоказателя13 = "П010103000101"
		  Или ПодстрокаВИмениПоказателя13 = "П020103000101"
	...
	Для Каждого Пок Из ПоказателиРаздела Цикл
		...
		Если Пок.Многострочность Тогда
			...
			Пока ДанныеСтраницы.Свойство(Пок.Ключ + "_" + Формат(ИндексСтроки, "ЧГ="), ЗначениеПоказателя) Цикл
				...
				Если Найти(Пок.Ключ, "ПД00002000000") > 0 Или Найти(Пок.Ключ, "ПД00002000200") > 0 Тогда
