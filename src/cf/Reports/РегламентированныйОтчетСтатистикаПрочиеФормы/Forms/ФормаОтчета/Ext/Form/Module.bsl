...
&НаСервере
Процедура ПодготовитьДанныеОтчетаПоШаблону(ДанныеШаблонаОтчета, Отказ = Ложь);
	...
	СтруктураАтрибутовОтчета.Вставить("OKUD",            ДанныеШаблонаОтчета.ОКУД);
...
Процедура ГенерироватьМакетИСтруктуруДанныхРазделаОбщаяИнформация()
	...
	ОбразцыОбластей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета");
...
Процедура ГенерироватьМакетИСтруктуруДанныхРаздела(УзелСекция, ЭлементДереваРазделыОтчета)
	...
	Для каждого УзелОписанияКолонки Из УзлыОписанияКолонок Цикл
		...
		Если Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 И УзелОписанияКолонки.УзелИдентичен(УзлыОписанияКолонок[0]) Тогда
	...
	Для каждого УзелОписанияСтроки Из УзлыОписанияСтрок Цикл
		...
		Если Найти(ВРег(ПараметрыСтроки["type"]), "M") > 0 Тогда
...
Процедура СформироватьСтрокуМакетаРаздела(МакетРаздела, МакетОбразцовОбластей, СписокПараметровКолонок, ПараметрыСтроки, ЭлементДереваРазделОтчета)
	...
	ИначеЕсли Найти(ВРег(ПараметрыСтроки["type"]), "C") > 0 Тогда
	...
	Если ДляВводаДанных Тогда
		...
		ЭлементДереваАтрибутСтроки.ЗначениеПоУмолчанию = Число("0" + ПараметрыСтроки["code"]);
	...
	Для Инд = 0 По СписокПараметровКолонок.Количество() - 1 Цикл
		...
		Если Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 И Инд = 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 И Инд = 1 Тогда
	...
	Если СписокПараметровКолонок.Количество() > 0 Тогда
		...
		Если КлючевыеЯчейки <> Неопределено Тогда
			...
			Если Лев(ИмяЦелевойОбласти, 3) = "ОДГ" Тогда
				...
				ИмяЦелевойОбласти = ЦелеваяОбласть.Имя;
			КонецЕсли;
			Если Лев(ИмяЦелевойОбласти, 2) = "ПМ" И Найти(ИмяЦелевойОбласти, "_") Тогда
				ИмяЦелевойОбласти = Лев(ИмяЦелевойОбласти, Найти(ИмяЦелевойОбласти, "_") - 1);
			КонецЕсли;
...
Процедура ПрименитьПараметрыЯчейки(Форма, ЦелеваяЯчейка, ПараметрыЯчейки, ПараметрыКолонки, ПараметрыСтроки, ЭлементДереваСтрокаРаздела)
	...
	Если Найти(ВРег(ПараметрыСтроки["type"]), "C") > 0 Тогда
		Возврат;
	ИначеЕсли Найти(ВРег(ПараметрыСтроки["type"]), "M") > 0 Тогда
		Многострочный = Истина;
	КонецЕсли;
	...
	Если Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 Тогда
	...
	Если Найти(ВРег(ПараметрыКолонки["type"]), "S") > 0 И ПараметрыЯчейки["inputType"] = 0 Тогда
	...
	Если Найти(ВРег(ПараметрыКолонки["type"]), "S") > 0 ИЛИ Найти(ВРег(ПараметрыКолонки["type"]), "Z") > 0 Тогда
		...
		Если Найти(ВРег(ПараметрыКолонки["type"]), "Z") = 0 ИЛИ ТипЗначения.СодержитТип(Тип("Число")) Тогда
...
Процедура УстановитьШиринуКолонокМакетаРаздела(МакетРаздела, СписокПараметровКолонок);
	...
	Для Инд = 0 По СписокПараметровКолонок.Количество() - 1 Цикл
		...
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 И Инд = 1 Тогда
			...
			НовСтрокаТаблицы.ШиринаФикс = ШиринаКолонки;
			НовСтрокаТаблицы.ИзменятьШирину   = Ложь;
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "B") > 0 Тогда
			НовСтрокаТаблицы.ТипКолонки = "B";
			НовСтрокаТаблицы.ШиринаФикс = ШиринаКолонки;
			НовСтрокаТаблицы.ИзменятьШирину   = Ложь;
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "S") > 0  Тогда
		...
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "V") > 0  Тогда
			НовСтрокаТаблицы.ТипКолонки  = "V";
			НовСтрокаТаблицы.ШиринаФикс  = ШиринаКолонки;
			НовСтрокаТаблицы.ИзменятьШирину   = Ложь;
		ИначеЕсли Найти(ВРег(ПараметрыКолонки["type"]), "Z") > 0  Тогда
...
Функция ОписаниеТиповИзСтрокиФормата(СтрокаФорматаЯчейки, МинШиринаЯчейкиВСимв = 0, ТипСтрокойУмолч = "Число")
	...
	ПозОткрСкобки = Найти(СтрокаФормата, "(");
	ПозЗакрСкобки = Найти(СтрокаФормата, ")");
	...
	Если Найти(ВРег(СтрокаФормата), "N") > 0 Тогда
...
&НаСервереБезКонтекста
Функция ИмяПоказателяЯчейки(КодРаздела, КодСтроки, КодКолонки, Многострочный = Ложь)
...
Функция РезультатВычисленияУсловияДляЯчейки(Форма, Выражение)
	...
	Возврат (ЗначениеЗаполнено(Выражение) И РезультатВычисленияВыраженияИзСтроки(Форма, Выражение));
...
Процедура ДобавитьКОписаниюЯчеекСсылкиНаЗависимыеЯчейки()
	...
	Для каждого ОписаниеЯчейки Из СтруктураОписанийЯчеек Цикл
		...
		Если (ПараметрыЯчейки.Свойство("inputType", ТипВвода) И ТипВвода <> 0) И ПараметрыЯчейки["vldType"] = 5 Тогда
			...
			ПозАдреса = Найти(ФильтрДляСправочника, "#");
...
Процедура ВосстановитьДанныеРаздела(ТабДокумент, РазделМногострочный)
		...
		Если ТипЗнч(ТекущаяОбласть) <> Тип("РисунокТабличногоДокумента") Тогда
			Если ТекущаяОбласть.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И ТекущаяОбласть.СодержитЗначение = Истина Тогда
				...
				Если РазделМногострочный Тогда
					Если Найти(ТекущаяОбласть.Имя, "_") > 0 Тогда
	...
	Если РазделМногострочный Тогда
		...
		Для каждого ЭлементПоказателяВГруппе Из МаксИндексыМнПоказателейВГруппе Цикл
			...
			ИмяОпцийСтроки = Лев(ЭлементПоказателяВГруппе.Ключ, Найти(ЭлементПоказателяВГруппе.Ключ, "c"));
...
&НаСервере
Процедура ДобавитьСтрокуНаСервере(ИмяПервойЯчейкиСтроки)
	
	ДобавитьДополнительнуюСтроку(ЭтаФорма, ИмяПервойЯчейкиСтроки);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСтрокуНаКлиенте(ИмяПервойЯчейкиСтроки)
...
&НаКлиентеНаСервереБезКонтекста
Процедура ДобавитьДополнительнуюСтроку(Форма, ИмяПервойЯчейкиСтроки)
	...
	Если (ПерваяЯчейкаСтроки = Неопределено) ИЛИ (Найти(ИмяПервойЯчейкиСтроки, "_") = 0) Тогда
	...
	Для НомСтрокиНовОбл = НомерВерхЯчейкиНовойОбласти По НомерНижнЯчейкиНовойОбласти Цикл
		Для НомКолонкиНовОбл = 1 По ТекТабличноеПоле.ШиринаТаблицы Цикл
			...
			Если Найти(ИмяОбласти, "_") > 0 Тогда
	...
	ИмяОпцийСтроки = Лев(ИмяПервойЯчейкиСтрокиБезИндекса, Найти(ИмяПервойЯчейкиСтрокиБезИндекса, "c"));
	СтруктураОпцийСтроки = Неопределено;
	Если СтруктураОписанийЯчеек.Свойство(ИмяОпцийСтроки, СтруктураОпцийСтроки) И ТипЗнч(СтруктураОпцийСтроки) = Тип("Структура") Тогда
		УстановитьПредставлениеКлючевыхЯчеек(Форма, СтруктураОпцийСтроки, КолСтрок + 1);
	КонецЕсли;
...
Процедура СпециальногоДобавленияСтроки(ТекТабличноеПоле, НомерВерхЯчейкиОбласти, НомерНижнЯчейкиОбласти) Экспорт
	...
	Для каждого ТекОбласть Из ВставляемаяОбластьТаблДок.Области Цикл
		...
		ИмяОбласти = ТекОбласть.Имя;
		ПозРазделителя = Найти(ИмяОбласти, "_");
...
Процедура УдалитьДополнительнуюСтроку(Форма, ИмяПервойЯчейкиСтроки) Экспорт
	...
	Для НомСтроки = НомерВерхЯчейкиОбласти По НомерНижнЯчейкиОбласти Цикл
		Для НомКолонки = 2 По ТекТабличноеПоле.ШиринаТаблицы - 1 Цикл // без первой и последней колонок
			...
			ПозРазделителя = Найти(ИмяТекОбласти, "_");
...
Процедура СпециальногоУдаленияСтроки(ТекТабличноеПоле, НомерВерхЯчейкиОбласти, НомерНижнЯчейкиОбласти, НомерУдаляемойСтроки, КолСтрокРаздела) Экспорт
	...
	Для каждого ИменованнаяОбласть Из УдаляемаяОбластьТаблДок.Области Цикл
		Если ТипЗнч(ИменованнаяОбласть) = Тип("РисунокТабличногоДокумента") Тогда
			Продолжить;
		КонецЕсли;
		МассивИменОбластей.Добавить(Лев(ИменованнаяОбласть.Имя, Найти(ИменованнаяОбласть.Имя, "_") - 1));
	КонецЦикла;
...
Функция ИмяПервойЯчейкиОбласти(Форма, ОбластьДокумента)
	...
	Если Найти(ИмяБазовойОбласти, "ДСПМs") Тогда
		ИмяПервойЯчейкиСтрокиОбласти = Сред(ИмяБазовойОбласти, 4);
	ИначеЕсли Найти(ИмяБазовойОбласти, "УСПМs") Тогда
		ИмяПервойЯчейкиСтрокиОбласти = Сред(ИмяБазовойОбласти, 4);
	ИначеЕсли Найти(ИмяБазовойОбласти, "ДЗПМs")
		  ИЛИ Найти(ИмяБазовойОбласти, "ДГПМs") Тогда
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		Если СтрокаУровня1.ИмяСтраницы = "ОбщаяИнформация" Тогда
			...
			ПечФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ТитульныйПодписи");
...
Функция СоответствиеПоказателейТитульногоИПодписей()
	...
	СоответствиеПоказателей.Вставить("КодОКУД", СтруктураАтрибутовОтчета["OKUD"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяОбласти, "ДСПМs") Тогда      // добавить строку многострочной части
		ИмяПервойЯчейкиСтроки = Сред(ИмяОбласти, 4);
		СтандартнаяОбработка = Ложь;
		ДобавитьСтрокуНаКлиенте(ИмяПервойЯчейкиСтроки);
		Возврат;
	ИначеЕсли Найти(ИмяОбласти, "ДЗПМs")
		  ИЛИ Найти(ИмяОбласти, "ДГПМs") Тогда // добавить строку многострочной части
		ИмяПервойЯчейкиСтроки = Сред(ИмяОбласти, 4) + "_1";
		СтандартнаяОбработка = Ложь;
		ДобавитьСтрокуНаКлиенте(ИмяПервойЯчейкиСтроки);
		Возврат;
	ИначеЕсли Найти(ИмяОбласти, "УСПМs") Тогда // удалить строку многострочной части
		ИмяПервойЯчейкиСтроки = Сред(ИмяОбласти, 4);
		СтандартнаяОбработка = Ложь;
		УдалитьСтрокуНаКлиенте(ИмяПервойЯчейкиСтроки);
		Возврат;
...
Процедура ТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	Если Найти(ИмяДляПроверки, "ПМs") > 0 Тогда
		
		Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку.Доступность = Истина;
		
		Если Найти(ИмяДляПроверки, "ОДЗПМs") = 0 И Найти(ИмяДляПроверки, "ОДГПМs") = 0 Тогда
...
Процедура ВвестиПоказатель(Элемент, Область, ТипВвода, ИмяСправочникаВвода, ДопПараметрВвода, СтандартнаяОбработка)
	...
	ИначеЕсли ТипВвода = 5 Тогда
		...
		ПозАдреса = Найти(ДопПараметрВвода, "#");
		Если ПозАдреса > 0 Тогда
...
Процедура ВвестиПоказательПродолжение(Элемент, Область, ТипВвода, ИмяСправочникаВвода, ДопПараметрВвода, СтандартнаяОбработка,
	...
	Иначе
		Для каждого ЭлементСправочника Из ТаблицаЭлементовСправочника Цикл
			Если ЗначениеФильтраУстановлено Тогда
				Если Найти(ЭлементСправочника.СоставнойКлюч, ЗначениеФильтра) = 0 Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			НовыйЭлементСправочника = ТаблицаВыбора.Добавить();
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьПредставлениеКлючевыхЯчеек(Форма, СтруктураОпцийСтроки, ИндексСтроки)
...
Функция ИмяПоказателяБезИндексаИСоставляющиеАдреса(ИмяПоказателя, ИндексПоказателя = 0, Многострочный = Ложь, ПолучитьСоставляющие = Ложь, НомерРаздела = 0, НомерСтроки = 0, КодКолонки = "");
	...
	Если Лев(ИмяПоказателя, 2) = "ПМ" ИЛИ Сред(ИмяПоказателя, 3, 2) = "ПМ" ИЛИ Сред(ИмяПоказателя, 4, 2) = "ПМ" Тогда
		Многострочный = Истина;
		ПозРазделителя = Найти(ИмяПоказателя, "_");
		Если ПозРазделителя > 0 Тогда
	...
	Если ПолучитьСоставляющие Тогда
		АдреснаяЧасть = "";
		Если Многострочный Тогда
			АдреснаяЧасть = Сред(ИмяПоказателяБезИндекса, Найти(ИмяПоказателяБезИндекса, "ПМ") + 2);
		ИначеЕсли Лев(ИмяПоказателяБезИндекса, 2) = "ПО" ИЛИ Сред(ИмяПоказателя, 3, 2) = "ПО" Тогда
			АдреснаяЧасть = Сред(ИмяПоказателяБезИндекса, Найти(ИмяПоказателяБезИндекса, "ПО") + 2);
		КонецЕсли;
		Если НЕ ПустаяСтрока(АдреснаяЧасть) Тогда
			Если Лев(АдреснаяЧасть, 1) = "s" Тогда
				ПозНомСтроки = Найти(АдреснаяЧасть, "r");
				Если ПозНомСтроки > 0 Тогда
					...
					ПозКодКолонки = Найти(АдреснаяЧасть, "c");
...
Функция ПоказательЗаполнен(Показатель, Значение)
	...
	Возврат ЗначениеЗаполнено(Значение) И НЕ (СокрЛП(Показатель.Формат) = "N" И Показатель.МаксРазмерность = 17 И ТипЗнч(Значение) = Тип("Число") И Окр(Значение) = 0);
...
Функция СтраницаПуста(ДанныеСтраницы, Показатели, ДеревоВыгрузки = Неопределено)
	...
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("okpo");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("name");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("leader_fio");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("responsible_post");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("responsible_fio");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("phone");
	
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("okfs");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("okved");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("org_adress");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("org_type");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("document_cretaion_date");
	ПоказателиНеанализируемыеНаЗаполненность.Добавить("stroyka");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПараметрыВыгрузки.Свойство("name", ЗначениеПараметра) И НЕ ЗначениеЗаполнено(ЗначениеПараметра) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано наименование оганизации!", ИмяРаздела, "name");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НРег(ПараметрыПроверкиОтчета["notEmpty"]) = "true" Тогда
		
		Если НЕ ПроверитьРазделыНаНаличиеИнформации(ДеревоВыгрузки, "ПолеТабличногоДокумента") Тогда
...
Функция СформироватьИмяФайла(ПараметрыВыгрузки, АтрибутыОтчета)
	...
	КодПериода        = Формат(АтрибутыОтчета["idp"], "ЧН=");
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Для каждого КонтрольноеСоотношение Из ТаблицаКонтрольныхСоотношений Цикл
		...
		СоотношениеВыполнено = РезультатПроверкиКонтрольногоСоотношения(ЭтаФорма, Формула, Условие, УсловиеПериод, РазрядностьОкругления, ДопустимоеОтклонение);
...
&НаСервереБезКонтекста
Функция РезультатПроверкиКонтрольногоСоотношения(Форма, Формула, Условие, УсловиеПериод, РазрядностьОкругления, ДопустимоеОтклонение)
	
	Форма.КвоОшибокКС = 0;
	РезУсловиеПериод = РезультатВычисленияВыраженияИзСтроки(Форма, УсловиеПериод, РазрядностьОкругления, ДопустимоеОтклонение);
	
	Если Форма.КвоОшибокКС > 0 Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если РезУсловиеПериод Тогда
		...
		Форма.КвоОшибокКС = 0;
		РезУсловие = РезультатВычисленияВыраженияИзСтроки(Форма, Условие, РазрядностьОкругления, ДопустимоеОтклонение);
		...
		Если РезУсловие Тогда
			...
			Форма.КвоОшибокКС = 0;
			РезФормула = РезультатВычисленияВыраженияИзСтроки(Форма, Формула, РазрядностьОкругления, ДопустимоеОтклонение);
...
&НаСервереБезКонтекста
Функция РезультатВычисленияВыраженияИзСтроки(Форма, ТекстовоеПредставлениеВыражения, РазрядностьОкругления = 2, ДопустимоеОтклонение = 0);
	...
	ВыделитьЭлементыИзСтрокиВыражения(Форма, ТекстовоеВыражение, ТаблицаЭлементов);
	
	Если Найти(ТекстовоеВыражение, "SUM") > 0 Тогда // есть оператор суммирования
		ОпределитьОбластьДействияОператораСуммы(ТекстовоеВыражение, ТаблицаЭлементов, Форма.КвоОшибокКС);
	КонецЕсли;
	
	ПолучитьЗначенияОбластейЭлементов(Форма, ТаблицаЭлементов, РазрядностьОкругления, Форма.КвоОшибокКС);
	...
	Если Найти(ТекстовоеВыражение, "AND") > 0 ИЛИ Найти(ТекстовоеВыражение, "OR") > 0 Тогда // есть логические предикаты
	...
	Если Найти(ТекстовоеВыражение, "&NP") > 0 Тогда // есть условие на период
		ВычислитьИЗаменитьУсловиеНаПериод(Форма, ТекстовоеВыражение);
	КонецЕсли;
	...
	Для Инд = 0 По МассивПодвыражений.ВГраница() Цикл
		СтрРезультатВычисления = ?(РезультатВычисленияЛогическогоПодвыражения(Форма, МассивПодвыражений[Инд], ТаблицаЭлементов, Инд, Форма.КвоОшибокКС), "Истина", "Ложь");
...
&НаСервереБезКонтекста
Процедура ВыделитьЭлементыИзСтрокиВыражения(Форма, ТекстовоеВыражение, ТаблицаЭлементов);
	...
	Для Инд = 1 По СтрДлина(ТекстовоеВыражение) Цикл
		...
		ИначеЕсли СтрСимв = ОгрП Тогда
			...
			Если НачалоЭлемента Тогда // добавим элемент в таблицу
				...
				СтрокаЭлемента.МассивСтрок  = МассивЗначенийИзСтрСпискаИДиапазонов(СтрокаЭлемента.Строки, , , Форма.КвоОшибокКС);
				СтрокаЭлемента.МассивГраф   = МассивЗначенийИзСтрСпискаИДиапазонов(СтрокаЭлемента.Графы , , , Форма.КвоОшибокКС);
				СтрокаЭлемента.МассивСпец1  = МассивЗначенийИзСтрСпискаИДиапазонов(СтрокаЭлемента.Специфика1, , Ложь, Форма.КвоОшибокКС);
				СтрокаЭлемента.МассивСпец2  = МассивЗначенийИзСтрСпискаИДиапазонов(СтрокаЭлемента.Специфика2, , Ложь, Форма.КвоОшибокКС);
				СтрокаЭлемента.МассивСпец3  = МассивЗначенийИзСтрСпискаИДиапазонов(СтрокаЭлемента.Специфика3, , Ложь, Форма.КвоОшибокКС);
...
&НаСервереБезКонтекста
Процедура ОпределитьОбластьДействияОператораСуммы(ТекстовоеВыражение, ТаблицаЭлементов, СчетчикОшибок = 0)
	...
	Пока Поз > 0 Цикл
		...
		Если Лев(ВремВыражение, 1) = "%" Тогда // элемент
			СтрЗаменитель = Лев(ВремВыражение, Найти(Сред(ВремВыражение, 2), "%") + 1);
			...
			Для Инд = 1 По СтрДлина(ВремВыражение) Цикл
				...
				Если ВключенСчетчикСкобок И КвоОткрытыхСкобок = 0 Тогда
					Для каждого СтрокаЭлемента Из ТаблицаЭлементов Цикл
						...
						Если Найти(ОбластьДействияОператораСуммы, СтрокаЭлемента.СтрЗаменитель) > 0 Тогда
...
&НаСервереБезКонтекста
Процедура ПолучитьЗначенияОбластейЭлементов(Форма, ТаблицаЭлементов, РазрядностьОкругления, СчетчикОшибок = 0)
	
	Для каждого СтрокаЭлемента Из ТаблицаЭлементов Цикл
		
		ИдЭлементаФормыРаздела = ИдентификаторЭлементаСпискаФормПоПредставлению(Форма, "Раздел" + СтрокаЭлемента.Раздел);
		Если ИдЭлементаФормыРаздела = Неопределено Тогда
			СчетчикОшибок = СчетчикОшибок + 1;
			Возврат;
		КонецЕсли;
		...
		ТаблДокФормыРаздела = Форма.СтруктураРеквизитовФормы.СписокФормРазделов.НайтиПоИдентификатору(ИдЭлементаФормыРаздела).Значение;
		СписокПараметровГраф = Неопределено;
		Форма.ПараметрыГрафРазделов.Свойство("Раздел" + СтрокаЭлемента.Раздел, СписокПараметровГраф);
		...
		Для каждого ЭлементСпискаПараметровГраф Из СписокПараметровГраф Цикл
			...
			Если Найти(ВРег(НаборПараметров["type"]), "Z") > 0 Тогда
				МассивГраф.Добавить(НаборПараметров["code"]);
			ИначеЕсли Найти(ВРег(НаборПараметров["type"]), "S") > 0 Тогда
		...
		Если СтрокаЭлемента.МассивСтрок[0] = "*" Тогда
			...
			Для НомСтрокиТабл = 6 По ТаблДокФормыРаздела.ВысотаТаблицы Цикл
				ИмяЯчейки = ТаблДокФормыРаздела.Область(НомСтрокиТабл, 2, НомСтрокиТабл, 2).Имя;
...
&НаСервереБезКонтекста
Функция РезультатПроверкиЗначенияСпецифики(Значение, МассивЗначенийСпецифик)
...
&НаСервереБезКонтекста
Функция СтруктураСоставляющихЭлемента(Элемент)
...
&НаСервереБезКонтекста
Функция МассивЗначенийИзСтрСпискаИДиапазонов(Стр, СтрРазделитель = ",", ИскатьДиапазон = Истина, СчетчикОшибок = 0)
	...
	Для каждого Значение Из МассивЗначений Цикл
		...
		Если ИскатьДиапазон Тогда
			ПозРаздДиапазона = Найти(Значение, "-");
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЧислоИзСтроки(Стр, СчетчикОшибок = 0)
...
&НаСервереБезКонтекста
Процедура ВыполнитьЗаменуФункцийЯзыкаКС(ТекстовоеВыражение)
...
&НаСервереБезКонтекста
Процедура ВычислитьИЗаменитьУсловиеНаПериод(Форма, ТекстовоеВыражение)
	...
	Поз = Найти(ВремВыражение, ОпрПериода);
	Пока Поз > 0 Цикл
		...
		ИначеЕсли Лев(ВремВыражение, 2) = "IN" Тогда
			...
			ЛогОператор = Лев(ВремВыражение, 2);
		Иначе
			КодНомераПериода = ПорядковыйНомерПериодаВГоду(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, Форма.СтруктураАтрибутовОтчета["idp"]);
		...
		ОбработанноеВыражение = ОбработанноеВыражение + ?(РезультатУсловияНаПериод(Форма, ЛогОператор, КодыПериодов), "Истина ", "Ложь ");
...
&НаСервереБезКонтекста
Функция РезультатУсловияНаПериод(Форма, ЛогОператор, КодыПериодов)
	...
	Если ЛогОператор = "IN" Тогда
		...
		Для каждого СтрКодПериода Из МассивКодовПериодов Цикл
			Если КодНомераПериода = НомерПериодаИзСправочника(Форма, СтрКодПериода) Тогда
				Возврат Истина;
			КонецЕсли;;
		КонецЦикла;
	Иначе
		Попытка
			Возврат Вычислить(Формат(КодНомераПериода, "ЧН=;ЧГ=") + ЛогОператор + Формат(НомерПериодаИзСправочника(Форма, КодыПериодов), "ЧН=;ЧГ="));
		Исключение
			Возврат Ложь;
		КонецПопытки
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция НомерПериодаИзСправочника(Форма, СтрКодПериода)
...
&НаСервереБезКонтекста
Функция МассивЛогическихПодвыраженийВыделитьИНазначитьИндексВТаблице(ТекстовоеВыражение, ТаблицаЭлементов)
	...
	ПозИ = Найти(ВремВыражение, РазделительИ);
	ПозИЛИ = Найти(ВремВыражение, РазделительИЛИ);
	...
	Для каждого СтрокаЭлемент Из ТаблицаЭлементов Цикл
		Для Инд = 0 По МассивПодвыражений.ВГраница() Цикл
			Если Найти(МассивПодвыражений[Инд], СтрокаЭлемент.СтрЗаменитель) > 0 Тогда
...
&НаСервереБезКонтекста
Функция ПодвыражениеИзСтроки(Стр, СтрОстаток = "", ВыделитьСлева = Истина)
...
&НаСервереБезКонтекста
Функция РезультатВычисленияЛогическогоПодвыражения(Форма, ШаблонВыражения, ТаблицаЭлементов, ИндексПодвыражения = 0, СчетчикОшибок = 0)
	...
	Для ИндСтроки = 1 По КоличествоСтрок Цикл
		Для ИндГрафы = 1 По КоличествоГраф Цикл
			...
			Для каждого ЭлементВыражения Из ЭлементыВыражения Цикл
				СтрЗначениеПоказателя = Формат(ЗначениеПоказателяИзТаблицы(ЭлементВыражения, ИндСтроки, ИндГрафы), "ЧН=; ЧГ=");
...
&НаСервереБезКонтекста
Функция ЗначениеПоказателяИзТаблицы(ЭлементВыражения, ИндСтроки, ИндГрафы);
...
&НаСервереБезКонтекста
Функция РезультатВычисленияЭлементарногоВыражения(ВычисляемоеВыражение, СчетчикОшибок)
...
&НаСервереБезКонтекста
Процедура ПреобразоватьВыражение(Выражение)
...
&НаСервереБезКонтекста
Функция ПозНайденногоИзМассива(Стр, МассивПодстрок, ДлинаНайденнойПодстроки = 0)
...
// Реализация функций, используемых в языке описания контрольных соотношений XML-шаблона

&НаСервереБезКонтекста
Функция СоответствиеФункцийРеализациям()
...
// Соответствует функции abs(p) языка контрольных соотношений
//
&НаСервереБезКонтекста
...
// Возвращает первое заполненное выражение из списка аргументов
// Соответствует функции coalesce(p1, …pn) языка контрольных соотношений
//
&НаСервереБезКонтекста
Функция ПервоеЗаполненное(В1 = Неопределено, В2 = Неопределено, В3 = Неопределено, В4 = Неопределено, В5 = Неопределено,
	В6 = Неопределено, В7 = Неопределено, В8 = Неопределено, В9 = Неопределено, В10 = Неопределено)
...
// Соответствует функции floor(p) языка контрольных соотношений
//
&НаСервереБезКонтекста
Функция НаибольшееЦелое(Выражение)
	
	Возврат Цел(Выражение - ?(Выражение - Цел(Выражение) < 0, 1, 0));
	
КонецФункции

// Заменяет значение, равное "Неопределено" указанным замещающим значением
// Соответствует функции isnull(p1, p2) языка контрольных соотношений
//
&НаСервереБезКонтекста
Функция ЕслиПусто(Выражение, ЗамещающееЗначение)
...
// Возвращает значение "Неопределено", если два указанных выражения равны
// Соответствует функции nullif(p1, p2) языка контрольных соотношений
//
&НаСервереБезКонтекста
Функция НеопределеноЕсли(Выражение1, Выражение2)
...
// Соответствует функции round(p1, p2, p3) языка контрольных соотношений
//
&НаСервереБезКонтекста
Функция ОкрЗначение(Выражение, Точность, ТипОперации = 0)
