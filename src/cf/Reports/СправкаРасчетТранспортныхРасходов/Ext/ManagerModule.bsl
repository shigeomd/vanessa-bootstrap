...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат "Справка-расчет транспортных расходов" + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(ПараметрыОтчета.СНачалаГода,НачалоГода(ПараметрыОтчета.НачалоПериода),ПараметрыОтчета.НачалоПериода),
...
Процедура ПослеВыводаРезультата(ПараметрыОтчета, Результат) Экспорт
	...
	Пока НЕ Область = Неопределено Цикл
		...
		|регламентной операции ""Закрытие счета 44"",
...
Функция ВариантыНастроек() Экспорт
	...
	Массив.Добавить(Новый Структура("Имя, Представление","ТранспортныеРасходы", "Транспортные расходы"));
