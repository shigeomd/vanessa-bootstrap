					...
					Если НайденноеЗначение[0].Свойство("Данные") Тогда
						...
						Для Каждого СтрокаТаблЗнач Из НайденноеЗначение Цикл
							...
							Если ТипЗнч(СтрокаТаблЗнач.Данные) = Тип("СписокЗначений") Тогда
								...
								СтрокаТаблЗнач.Данные[0].Значение.П000020001000 = "";
								СтрокаТаблЗнач.Данные[0].Значение.П000020004003 = 0;
								
								СтрокаТаблЗнач.Данные[0].Значение.П000020005003 = ЗначениеК1(ЭтаФорма);
								СтрокаТаблЗнач.Данные[0].Значение.П000020006003 = 1;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020010003", "П000020004003;П000020005003;П000020006003;П000020007002;П000020008002;П000020009002;П000020006201;П000020006501");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020011003", "П000020004003;П000020005003;П000020006003;П000020007002;П000020008002;П000020009002;П000020006201;П000020006501");
...
Процедура ДобавитьДополнительнуюСтрокуДаты(ИмяОбласти, МногострочнаяЧасть, ИдМСЧ)
	...
	ПозицияРазделителя = Найти(ИмяОбласти, "_");
...
Процедура ОбработатьДополнительнуюСрокуДаты(ИмяОбласти)
	...
	Если Найти(ИмяОбласти, "П000020006301") = 1 Тогда
		ИдМСЧ = СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1;
	ИначеЕсли Найти(ИмяОбласти, "П000020006601") = 1 Тогда
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета
		= Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), Найти(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].Имя;
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета
		= Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), Найти(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].ОсновнаяФорма.Синоним;
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если НЕ Раздел.МногостраничностьВРазделе
			...
			ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(
				Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + Раздел.ИмяСтраницы));
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Область.Имя, "П000020006301") = 1 И ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020006101.Значение))
	 ИЛИ (Найти(Область.Имя, "П000020006601") = 1 И ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020006401.Значение)) Тогда
	...
	Если ИмяЯчейки = "ФормаРеорганизации" ИЛИ ИмяЯчейки = "П000030000503" Тогда
		УстановитьДоступностьОбластей(ЭтаФорма, СтруктураРеквизитовФормы.НаимТекущегоРаздела);
	КонецЕсли;
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Область.Имя = КодПоказателя ИЛИ Найти(Область.Имя, "П0000200015") > 0 Тогда
...
Процедура ВвестиКодУслуги(Элемент, Область, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	ПараметрыДляЗаполненияТаблицыЗначений.Вставить("ИмяОтчета", Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7));
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "", ПриИзмененииСодержимогоОбласти = Ложь)
	...
	Если СтрЧислоВхождений(ОбластьИмя, "_") > 0 Тогда
		ОбластьИмя = Лев(ОбластьИмя, Найти(ОбластьИмя, "_") - 1);
	КонецЕсли;
...
Процедура РасчетЛистаРаздел2(Форма, МестоРазмещенияДанных)
	...
	ЗначениеСтроки070Графа4 = Окр(Строка040 * Строка050 * Строка060 * Строка070Графа2 * Строка070Графа3 / Строка070КоличествоДней, 2);
	ЗначениеСтроки080Графа4 = Окр(Строка040 * Строка050 * Строка060 * Строка080Графа2 * Строка080Графа3 / Строка080КоличествоДней, 2);
	ЗначениеСтроки090Графа4 = Окр(Строка040 * Строка050 * Строка060 * Строка090Графа2 * Строка090Графа3 / Строка090КоличествоДней, 2);
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если Найти(Область.Имя, "П000010001003") > 0 ИЛИ Область.Имя = "П000020003000" Тогда
	...
	Если Область.Имя = "ФормаРеорганизации" ИЛИ Область.Имя = "П000020001000" ИЛИ Область.Имя = "П000030000503" Тогда
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки <> Неопределено И 
		СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки Тогда
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Найти(Показатель.Имя, "П000010001003") = 1
		 ИЛИ Найти(Показатель.Имя, "П000020003000") = 1 Тогда
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Пока Раздел1.Свойство("П000010001003" + Постфикс) Цикл
		ОКТМО = Раздел1["П000010001003" + Постфикс];
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Найти(Область.Имя, "П000020006201") = 1
	 ИЛИ Найти(Область.Имя, "П000020006501") = 1 Тогда
		РассчитатьДниОсуществленияДеятельности();
	КонецЕсли;
...
Процедура ПроверитьКС_1(СтруктураРезультата)
	...
	ОписаниеКС = НСтр("ru='Сумма единого налога на вмененный доход, подлежащая уплате за налоговый период (строка 020 Раздел1) по данному ОКТМО должна быть равна произведению суммы налога, подлежащей уплате в бюджет (строка 040 Раздела 3) и результата деления суммы строк 110 всех Разделов 2 по данному ОКТМО и суммы налога, исчисленного по всем ОКТМО (строка 010 Раздела 3)'");
	...
	Для Индекс = 1 По КоличествоБлоков Цикл
		...
		КодОКАТО = СокрЛП(СтруктураРаздела1[ИмяПоказателяОКАТО]);
	...
	Для Индекс = 1 По КоличествоБлоков Цикл
		...
		КодОКАТО = ПоказательПоСтроке("КодОКАТО");
		ВставитьЗначениеМногострочногоПоказателя(СтруктураРаздела1, КодОКАТО, Индекс);
		Если ПустаяСтрока(КодОКАТО.Значение) Тогда
			Продолжить;
		...
		Для каждого СтраницаР2 Из СтруктураРеквизитовФормы.мДанныеОтчета.Раздел2 Цикл
			...
			Если ПустаяСтрока(КодОКАТОНаСтранице) ИЛИ КодОКАТО.Значение <> КодОКАТОНаСтранице Тогда 
				Продолжить;
		...
		Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р3Стр040.ЗначениеСтр + " * " + СуммаР2Стр110Стр + " / " + Р3Стр010.ЗначениеСтр + " Итог ["
			+ КС.ПраваяЧастьСтр + "]";
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	Если УсловныйКодПоказателя = "КодОКАТО" Тогда
