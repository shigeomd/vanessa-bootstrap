...
Процедура ОбработатьДополнительнуюСрокуДаты(ИмяОбласти)
	...
	Если Найти(ИмяОбласти, "П000020003301") = 1 Тогда
		ИдМСЧ = СтруктураРеквизитовФормы.ИдМСЧ_Раздел2_1;
	ИначеЕсли Найти(ИмяОбласти, "П000020003601") = 1 Тогда
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Область.Имя, "П000020003301") = 1 И ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020003101.Значение))
	 ИЛИ (Найти(Область.Имя, "П000020003601") = 1 И ЗначениеЗаполнено(ТабличныйДокумент.Области.П000020003401.Значение)) Тогда
	...
	Если Найти(ИмяЯчейки, "П0000200015") Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	ИначеЕсли ВозможныеКодыПоМесту.Найти(ПараметрыВыгрузки.ПоМесту) = Неопределено Тогда
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если Не ПустаяСтрока(ПараметрыВыгрузки.ФормаРеорганизации) И Найти("1,2,3,5,6", ПараметрыВыгрузки.ФормаРеорганизации) > 0 Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Найти(Область.Имя, "П000020003201") = 1 
	 ИЛИ Найти(Область.Имя, "П000020003501") = 1 Тогда
		РассчитатьДниОсуществленияДеятельности();
	КонецЕсли;
