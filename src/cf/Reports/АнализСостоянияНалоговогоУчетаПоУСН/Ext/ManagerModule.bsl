...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Если ПараметрыОтчета.СхемаКомпоновкиДанных.НаборыДанных.Найти("ВнешнийИсточник") <> Неопределено Тогда		
		Если ПараметрыОтчета.СхемаКомпоновкиДанных.НаборыДанных.ВнешнийИсточник.ИмяОбъекта = "ТаблицаКоэффициентов" Тогда						
			Запрос = Новый Запрос;
			...
			МетодРаспределения = УчетнаяПолитика.МетодРаспределенияРасходовУСНПоВидамДеятельности(ПараметрыОтчета.Организация, КонецДня(ПараметрыОтчета.КонецПериода));
			БазаРаспределения  = УчетнаяПолитика.БазаРаспределенияРасходовУСНПоВидамДеятельности(ПараметрыОтчета.Организация, 	КонецДня(ПараметрыОтчета.КонецПериода));
...
Процедура ПриВыводеЗаголовка(ПараметрыОтчета, КомпоновщикНастроек, Результат) Экспорт 
	...
	НаименованиеОрганизации                = БухгалтерскиеОтчетыВызовСервераПовтИсп.ПолучитьТекстОрганизация(ПараметрыОтчета.Организация, ПараметрыОтчета.ВключатьОбособленныеПодразделения);
	...
	Если Найти(ПараметрыОтчета.КоллекцияНастроек.ИмяФормируемогоМакета, "МакетРасшифровкиНоменклатурыПоДокументам") Тогда		
		Номенклатура                           = ПараметрыОтчета.КоллекцияНастроек.ЗначениеРасшифровки.ЗначениеПараметра.Наименование;
		ОбластьОтчета                          = Макет.ПолучитьОбласть("Заголовок");
...
Процедура СформироватьРасшифровку(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если Найти(ПараметрыОтчета.ИмяМакета, "МакетРасшифровкиПоНоменклатуре") Тогда		
...
Функция ПолучитьКоэффРаспределенияЕНВД(Организация, Дата, Период = "Квартал", Способ = "БУ")
	...
	Если Способ = "БУ" Тогда
		...
		Коэфф = НалоговыйУчет.КоэффициентРаспределенияРасходовПоВидамДеятельности(Организация, НачальнаяДата, Дата);
