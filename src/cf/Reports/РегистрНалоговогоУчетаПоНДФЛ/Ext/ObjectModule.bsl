...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ПоследняяГруппировка = ОтчетыКлиентСервер.ПолучитьПоследнийЭлементСтруктуры(НастройкиОтчета);
...
Процедура ПолучитьДанныеДляПостроенияОтчета(СписокФизЛицОрганизация)
	...
	|	ЕСТЬNULL(СтатусФизЛицНаКонецМесяца.Статус.КодФНС, 1) КАК Статус,
	|	ВЫБОР
	|		КОГДА ИзмеренияДатыДляВТСтатусФизЛицНаКонецМесяца.Период < &ДатаЗакона285ФЗ
	|				И НЕ ЕСТЬNULL(СтатусФизЛицНаКонецМесяца.Статус, ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент)) В (ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Нерезидент), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Беженцы))
	|			ТОГДА ИСТИНА
	...
	|				И НЕ ЕСТЬNULL(СтатусыФизЛицНаКонецНалоговогоПериода.Статус, ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент)) В (ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Нерезидент), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Беженцы))
	|			ТОГДА ИСТИНА
	...
	|	НДФЛСведенияОДоходах.КодДохода.СоответствуетОплатеТруда КАК СоответствуетОплатеТруда,
	...
	|		ИНАЧЕ НДФЛСведенияОДоходах.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года
	...
	|	АвансовыеПлатежиИностранцевПоНДФЛ.Сумма,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСписокФизЛицОрганизация КАК СписокФизЛицОрганизация
	|		ПО АвансовыеПлатежиИностранцевПоНДФЛ.ФизическоеЛицо = СписокФизЛицОрганизация.ФизическоеЛицо
	|			И (ГОД(АвансовыеПлатежиИностранцевПоНДФЛ.МесяцНалоговогоПериода) = СписокФизЛицОрганизация.Год)
	|			И АвансовыеПлатежиИностранцевПоНДФЛ.ГоловнаяОрганизация = СписокФизЛицОрганизация.ГоловнаяОрганизация
	|			И (СписокФизЛицОрганизация.НеОтбиратьПоОП
	|				ИЛИ АвансовыеПлатежиИностранцевПоНДФЛ.Организация = СписокФизЛицОрганизация.Организация)
	|ГДЕ
	|	АвансовыеПлатежиИностранцевПоНДФЛ.МесяцНалоговогоПериода МЕЖДУ &НачалоНП И &КонецНП
	|	И АвансовыеПлатежиИностранцевПоНДФЛ.МесяцНалоговогоПериода >= &ДатаИзмененияНалогообложенияДивидендов
	...
	|	КОНЕЦПЕРИОДА(СтрокиДоходов.МесяцНалоговогоПериода, МЕСЯЦ) КАК Период,
	|	СтрокиДоходов.КодДохода,
	|	СтрокиДоходов.СтавкаНалогообложенияРезидента
	...
	|	КОНЕЦПЕРИОДА(СтрокиНалогов.МесяцНалоговогоПериода, МЕСЯЦ),
	...
	|	СтрокиНалогов.СтавкаНалогообложенияРезидента
	|ИЗ
	|	ВТСтрокиАвансов КАК СтрокиНалогов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СтрокиНалогов.ФизическоеЛицо,
	|	КОНЕЦПЕРИОДА(СтрокиНалогов.МесяцНалоговогоПериода, ГОД),
	|	СтрокиНалогов.КодДохода,
	|	СтрокиНалогов.СтавкаНалогообложенияРезидента
	|ИЗ
	|	ВТСтрокиНалогов КАК СтрокиНалогов";
	...
	УчетНДФЛ.СоздатьВТСтавкаНДФЛПоСтавкеРезидента(Запрос.МенеджерВременныхТаблиц, "ВТСтрокиУчета");
	...
	|	СтандартныеВычетыПоНДФЛЗаПериод.Основание
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПрименениеВычетовВМесяцаРезидентства.ФизическоеЛицо,
	|	ПрименениеВычетовВМесяцаРезидентства.Период КАК Месяц,
	|	ПрименениеВычетовВМесяцаРезидентства.ГоловнаяОрганизация,
	...
	|	НДФЛСведенияОДоходах.КодДоходаКод,
	|	НДФЛСведенияОДоходах.КодДоходаКодДляОтчетности,
	...
	|	НДФЛСведенияОДоходах.КодВычета,
	|	НДФЛСведенияОДоходах.КодВычетаКодДляОтчетности,
	|	ВЫБОР
	...
	|			И (КОНЕЦПЕРИОДА(НДФЛСведенияОДоходах.МесяцНалоговогоПериода, МЕСЯЦ) = СтавкаНДФЛПоСтавкеРезидента.Период)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСтатусыФизЛицНаКонецНалоговогоПериода КАК СтатусыФизЛицПоГодам
	|		ПО НДФЛСведенияОДоходах.ГоловнаяОрганизация = СтатусыФизЛицПоГодам.ФизическоеЛицо
	|			И (ГОД(НДФЛСведенияОДоходах.МесяцНалоговогоПериода) = СтатусыФизЛицПоГодам.Год)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГоловнаяОрганизация,
	|	ФизическоеЛицо
	...
	|				И СтавкаНДФЛПоСтавкеРезидента.СтавкаНДФЛ = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавки.Ставка30)
	|				И СтавкаНДФЛПоСтавкеРезидентаНаКонецНалоговогоПериода.СтавкаНДФЛ = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавки.Ставка13)
	|			ТОГДА НДФЛРасчетыСБюджетом.Сумма
	|		ИНАЧЕ 0
	...
	|	НДФЛРасчетыСБюджетом.Период,
	|	СтавкаНДФЛПоСтавкеРезидента.СтавкаНДФЛ КАК Ставка,
	|	СтавкаНДФЛПоСтавкеРезидентаНаКонецНалоговогоПериода.СтавкаНДФЛ КАК СтавкаПоИтогамГода,
	|	НДФЛРасчетыСБюджетом.КодДохода,
	|	НДФЛРасчетыСБюджетом.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	...
	|			И (КОНЕЦПЕРИОДА(НДФЛРасчетыСБюджетом.МесяцНалоговогоПериода, МЕСЯЦ) = СтавкаНДФЛПоСтавкеРезидента.Период)
	...
	|			И (КОНЕЦПЕРИОДА(НДФЛРасчетыСБюджетом.МесяцНалоговогоПериода, ГОД) = СтавкаНДФЛПоСтавкеРезидентаНаКонецНалоговогоПериода.Период)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГоловнаяОрганизация,
	|	ФизическоеЛицо
	...
	|			И (КОНЕЦПЕРИОДА(Авансы.МесяцНалоговогоПериода, МЕСЯЦ) = СтавкаНДФЛПоСтавкеРезидента.Период)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСтавкаНДФЛПоСтавкеРезидента КАК СтавкаНДФЛПоСтавкеРезидентаНаКонецНалоговогоПериода
	...
	|			И (КОНЕЦПЕРИОДА(Авансы.МесяцНалоговогоПериода, ГОД) = СтавкаНДФЛПоСтавкеРезидентаНаКонецНалоговогоПериода.Период)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГоловнаяОрганизация,
	|	ФизическоеЛицо
	...
	|		ИНАЧЕ НДФЛИмущественныеВычетыФизлиц.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года
	|	КОНЕЦ КАК КодВычетаКодДляОтчетности
	...
	|		ИНАЧЕ НДФЛПредоставленныеСтандартныеВычетыФизЛиц.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года
	|	КОНЕЦ
	|ИЗ
	|	ВТСписокФизЛицОрганизация КАК ВТСписокФизЛицОрганизация
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ КАК НДФЛПредоставленныеСтандартныеВычетыФизЛиц
	...
	|	Перечислено.РегистрацияВНалоговомОргане,
	|	ВЫБОР
	...
	|	ЗачтенныеАвансы.СтавкаПоИтогамГода
	|ИЗ
	...
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СправкаПоНДФЛСотруднику.Номер),
	...
	|		ИНАЧЕ ВидыВычетовНДФЛ.КодПрименяемыйВНалоговойОтчетностиС2011Года
	|	КОНЕЦ КАК КодВычета,
	...
	|		ИНАЧЕ СоциальныеИИмущественныеВычеты.КодВычета.КодПрименяемыйВНалоговойОтчетностиС2011Года
	...
	|		ПО ДоходыПоМесяцам.Ставка = СтавкиНалогаЧислом.СтавкаНалогообложения
	...
	|	(ЕСТЬNULL(ДоходыПоМесяцам.СуммаДохода, 0) - ЕСТЬNULL(ДоходыПоМесяцам.СуммаВычета, 0)) * СтавкиНалогаЧислом.Размер,
	|	ДоходыПоМесяцам.КодДоходаКодДляОтчетности,
	|	ДоходыПоМесяцам.КодВычетаКодДляОтчетности,
	|	ДоходыПоМесяцам.Ставка,
	|	ДоходыПоМесяцам.РегистрацияВНалоговомОргане
	...
	|	РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.РеквизитыПП,
	|	РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	|ИЗ
	|	ВТРасчетыНалоговыхАгентовСБюджетомПоНДФЛ КАК РасчетыНалоговыхАгентовСБюджетомПоНДФЛ
...
Процедура ОпределитьНастройкиФормы(Форма, КлючВарианта, Настройки) Экспорт
	Настройки.СоответствиеПериодичностиПараметров.Вставить(Новый ПараметрКомпоновкиДанных("Период"), Перечисления.ДоступныеПериодыОтчета.Год);
КонецПроцедуры
...
Процедура СформироватьМакет(ДокументРезультат, ФизическоеЛицо, Год, ГоловнаяОрганизация, СтатусРезидент) Экспорт
	...
	Если ДанныеОФизЛицеОрганизации.НайтиСледующий(СтруктураОтбора) Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПолейАдреса.КодСтраны) Тогда
			Раздел2.Параметры.СтранаПроживания = СтруктураПолейАдреса.КодСтраны;
		КонецЕсли; 
	...
	Для Каждого НалогиИтоги Из ТаблицаПоНалогамИтоги Цикл
		...
		Если ЗначениеЗаполнено(СуммаАвансовыхПлатежейЗачтенная) Тогда
			ОбщиеСуммыСтроки.Параметры.СуммаАвансовыхПлатежейЗачтенная = СуммаАвансовыхПлатежейЗачтенная;
		КонецЕсли;
		СальдоПоНалогам = СуммаНалогаИсчисленная - СуммаАвансовыхПлатежейЗачтенная - СуммаНалогаУдержанная + СуммаНалогаВозвращенная - СуммаНалогаВзыскание;
