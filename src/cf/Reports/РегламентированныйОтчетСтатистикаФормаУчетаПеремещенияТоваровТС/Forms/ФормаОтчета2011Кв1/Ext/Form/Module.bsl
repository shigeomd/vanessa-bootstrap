...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрЧислоВхождений(Область.Имя, "УдалитьСтроку") = 1 И Область.Текст <> "" Тогда
		...
		Модифицированность = Истина;
		Возврат;
	ИначеЕсли Найти(Область.Имя, "ДатаЗаполнения") = 1 Тогда
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Найти(Область.Имя, "ДокСтрока") > 0 И Найти(Область.Имя, "УдалитьСтроку") = 0 Тогда
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Найти(Область.Имя, "ВалютаТовара") > 0 ИЛИ Найти(Область.Имя, "ЕдИзмТовара") > 0 Тогда
...
Процедура ВвестиПоказательПродолжение(Элемент, Область, КодПоказателя, ТаблицаВыбора, КолонкаПоиска, ИндексОбласти, Разделитель, РезультатВыбора = Неопределено)
	...
	Если Найти(Область.Имя, "СтранаОтправления") > 0 ИЛИ Найти(Область.Имя, "СтранаНазначения") > 0 Тогда
...
Функция ПолучитьСведенияОФизЛице (Знач ФизЛицо, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Попытка
		...
		Если Результат.Количество() > 0 Тогда
			СведенияФЛ.Вставить("АдрФЛ", АдресВФормате9Запятых(Результат[0]));
		КонецЕсли;
...
Функция АдресВФормате9Запятых(Запись)
	...
	Для НомСтр = 1 По СтрЧислоСтрок(Запись.ЗначенияПолей) Цикл
		
		Стр = СтрПолучитьСтроку(Запись.ЗначенияПолей, НомСтр);
		
		Если ЗначениеЗаполнено(Стр) Тогда
			СтруктураАдреса.Вставить(Лев(Стр, Найти(Стр, "=") - 1), Сред(Стр, Найти(Стр, "=") + 1));
		КонецЕсли;
	...
	Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы Цикл
		...
		Для Каждого Область Из ОбластьДопСтроки.Области Цикл
			...
			Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
				...
				ОбластьТаблДок = ТекТабличноеПоле.Область(КоординатыОбластей[Область.Имя] + (ТекущееКоличествоСтрокГруппы - НомНовойСтроки) * ВысотаСтроки, Область.Лево, КоординатыОбластей[Область.Имя] + ?(Найти(Область.Имя, "НаименованиеТовара") > 0, 4, 0) + (ТекущееКоличествоСтрокГруппы - НомНовойСтроки) * ВысотаСтроки, Область.Право);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузкиXML2011Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.мАдресвФорматеДляВыгрузки) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задан почтовый адрес оганизации!", ИмяПоляТаблДокумента, "ОргПочтовыйАдрес");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ОргФИОИсп = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ТабДокумент.ОргИсполнитель);
...
Процедура ОтрисоватьЗначкиУдаленияСтрок(Форма)
	...
	Для Каждого ОбластьТД Из ТабДок.Области Цикл
		Если СтрЧислоВхождений(ОбластьТД.Имя, "УдалитьСтроку") > 0 Тогда
			Если СтрЧислоВхождений(ОбластьТД.Имя, "_2") > 0 Тогда
				МассивОбластейСДопСтроками.Добавить(Лев(ОбластьТД.Имя, Найти(ОбластьТД.Имя, "_") - 1));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ОбластьТД Из ТабДок.Области Цикл
		Если СтрЧислоВхождений(ОбластьТД.Имя, "УдалитьСтроку") > 0 Тогда
			Если МассивОбластейСДопСтроками.Найти(Лев(ОбластьТД.Имя, Найти(ОбластьТД.Имя, "_") - 1)) = Неопределено Тогда
