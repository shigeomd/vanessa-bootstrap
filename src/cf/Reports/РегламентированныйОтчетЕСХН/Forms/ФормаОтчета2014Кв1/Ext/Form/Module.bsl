...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Cтруктура цветов раскраски автозаполняемых ячеек.
	...
	ТабличныйДокумент.Вывести(Отчеты[Сред(
		Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			...
			Если ИсходныйГод <> ОтчетныйГод Тогда
				СтруктураДанныхТитульный.ОтчетГод = ОтчетныйГод;
				ТипЧисло = Новый ОписаниеТипов("Число");
				ОтчетныйГод = ТипЧисло.ПривестиЗначение(ОтчетныйГод);
				УдалитьНеактуальныеУбыткиРаздела21(ОтчетныйГод);
			КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2014Кв1");
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000120000403", "П000120005003;П000120000203;П000120003003;П000120004003;П000120001003;П000120002003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000120000503", "П000120000203;П000120005003;П000120003003;П000120004003;П000120001003;П000120002003");
