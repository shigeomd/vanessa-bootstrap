...
Процедура ЗагрузитьПоказателиИзМакета (РазделВМакете)
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2007Кв1");
	...
	ИначеЕсли ИмяТекущейСтраницыПанели = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда			
				ИндексЯчейки = Сред(ОбластьИмя, Найти(ОбластьИмя, "_") + 1);			
			КонецЕсли;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
			...
			Если Найти(ОбластьИмя, "ОКВЭД") > 0 Тогда
			...
			ИначеЕсли Найти(ОбластьИмя, "ДатаПодписи") > 0 Тогда	
			...
			ИначеЕсли Найти(ОбластьИмя, "НомерКорректировки") > 0 Тогда		
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Если Найти(ОбластьИмя, "ОКАТО") > 0 И ЗначениеЗаполнено(Область.Значение) Тогда
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
		...
		Если Найти(ОбластьИмя, "ОКАТОРаздел2") > 0 И ЗначениеЗаполнено(Область.Значение) Тогда
			ДлинаЗначения = СтрДлина(СокрЛП(Область.Значение));
			Для Инд = ДлинаЗначения+1 По 11 Цикл
				Область.Значение = СокрЛП(Область.Значение) + "0";
			КонецЦикла;
		ИначеЕсли НЕ Найти(ОбластьИмя, "ОКАТОРаздел2") > 0 Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		// Строка 010.
		Если Найти(Ячейка,"КБК") > 0 И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел2" Тогда
		...
		Если  Найти(Область.Имя, "П000020002002") > 0 ИЛИ Найти(ОбластьИмя, "П000020002007") > 0 ИЛИ  Найти(Область.Имя, "П000020002012") > 0 Тогда
			Если НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
				
				СтандартнаяОбработка = Ложь;
				Если Найти(Область.Имя, "П000020002002") > 0 Тогда
...
Процедура УдалениеДополнительныхСтрокПриОчисткеТабличногоПоля(ПромТаблицаДопСтрокРаздела,ВыбТабличноеПоле,ВыбТабличноеПолеИмя,ИмяГруппыДопСтрок,ИсходноеКоличествоСтрокГруппы) Экспорт
	...
	//	ВыбТабличноеПоле = ЭтаФорма[ВыбТабличноеПолеИмя];
	Пока НСтр > ИсходноеКоличествоСтрокГруппы Цикл
...
Процедура ВставитьДополнительнуюСтраницуНаСервере(ИмяТекущейСтраницыПанели = Неопределено ) Экспорт
	...
	Если Найти(ИмяТекущейСтраницыПанели,"Раздел2")<>0 Тогда
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СтруктураXML2007");
...
Функция СформироватьСтруктуруПараметров()
	...
	ФИОПред = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.ОргУп);
	...
	ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.ОргНазв);
