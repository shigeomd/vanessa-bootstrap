...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(СтруктураРеквизитовФормы.ИмяМакетаССоставомПоказателей);
...
Процедура СформироватьНастройкиСоставаДополнительныхСтрок(ПеременнаяНастроекСостава)
	...
	МакетНастроекПоУмолчанию = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(СтруктураРеквизитовФормы.ИмяМакетаНастроекСоставаДопСтрок);
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяМакетаОтчета));
...
Процедура ИзвлечьИсходнуюСтруктуруНастроек(Знач МакетНастроек, ИмяСекции, ДеревоНастроек)
	...
	Если ТипЗнч(МакетНастроек) = Тип("Строка") Тогда
		МакетНастроек = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(МакетНастроек);
	КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СтруктураРеквизитовФормы.мТипАдреса = Неопределено Тогда
		СтруктураРеквизитовФормы.мТипАдреса = "ПолныйАдрПочт";
	ИначеЕсли Найти(СтруктураРеквизитовФормы.мТипАдреса, "Полный") = 0 Тогда
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки501_Кв3");
...
Процедура УдалитьНенужноеИзПечатнойФормы(ПечатнаяФорма)
	...
	Для Каждого Обл Из ПечатнаяФорма.Области Цикл
		Если Найти(Обл.Имя, "УдалитьСтроку") = 1 
...
Процедура ВывестиНаПечатьЛистБаланс(СтруктураПараметровЛиста)
	...
	МакетБланкаБаланс       = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Бланк_Баланс_2011Кв3");
	МакетБланковРасшифровки = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("БланкиРасшифровки_2011Кв3");
...
Процедура ВывестиНаПечатьЛистФормаОтчета(СтруктураПараметровЛиста)
	...
	МакетБланкаОтчета       = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Бланк_ОПУ_2011Кв3");
	МакетБланковРасшифровки = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("БланкиРасшифровки_2011Кв3");
...
Функция РазделТекущейОбласти(Форма, ТекущаяОбласть, ДлИмениОбласти = 13) Экспорт
	...
	Если Найти(ИмяТекущейОбласти, "ДобавитьСтроку_") = 1 Тогда 
