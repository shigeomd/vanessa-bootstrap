...
Процедура ВвестиСтрокуКодПодчиненностиЗавершение(Строка, ДополнительныеПараметры) Экспорт
	...
	Для НомСимвола = 1 По СтрДлина(Строка) Цикл
		Если Найти("0123456789", Сред(Строка, НомСимвола, 1)) = 0 Тогда
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузкиXML01");
