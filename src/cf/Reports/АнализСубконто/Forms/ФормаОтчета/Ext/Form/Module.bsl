...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Анализ субконто " + ТекстСубконто + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура ЗаполнитьПризнакиУчета()
	...
	Иначе
		...
		Пока ВыборкаСчета.Следующий() Цикл
			...
			ЕстьУчетПоПодразделениям = ?(ВыборкаСчета.УчетПоПодразделениям = Истина, Истина, ЕстьУчетПоПодразделениям);
