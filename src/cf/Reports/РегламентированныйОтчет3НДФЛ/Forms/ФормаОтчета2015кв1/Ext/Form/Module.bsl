...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(ОтчетОбъект().ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела));
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (ИмяЯчейки = "Индекс")
		...
		Префикс = "";
		ПозицияРазделителя = Найти(ИмяЯчейки, "_");
		...
		Если Лев(СокрЛП(СтруктураРеквизитовФормы.ВерсияБСП), 4) = "2.1." Тогда
			...
			ОбновитьАдресВТабличномДокументе(Результат, РоссийскийАдрес, Префикс);
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, УдалениеСтраницы = Ложь, ИндексУдаляемойСтраницы = Неопределено, ЗаголовокРаздела = Ложь)
	...
	ТабличныйДокумент.Вывести(ОтчетОбъект().ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяТекРаздела + ?(ЭтоПоследняяСтраница,"","_Без_Итога")));
...
Процедура РасчетПослеИзмененияСодержания(ИмяПоказателя, ИмяРаздела, Область)
	...
	Если (Лев(ИмяПоказателя,13) = "П000400001040") Тогда //Лист Б информация о дате получения дохода в валюте, нужно перечитать курс
		Курс = ПолучитьКурсВалютыНаДату(ТабличныйДокумент.Области["П000400001030" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"))].Значение, Область.Значение);
		ИмяПоказателяКурса = "П000400001050" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"));
	...
	Если (Лев(ИмяПоказателя,13) = "П000400001080") Тогда //Лист Б информация о дате уплаты налога в валюте, нужно перечитать курс
		Курс = ПолучитьКурсВалютыНаДату(ТабличныйДокумент.Области["П000400001030" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"))].Значение, Область.Значение);
		ИмяПоказателяКурса = "П000400001090" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"));
		ТабличныйДокумент.Области[ИмяПоказателяКурса].Значение = Курс;
		СохранитьЗначениеПоказателяВСтруктуре(ЭтаФорма, ИмяРаздела, ИмяПоказателяКурса, Курс);
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораКодаВалютыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Область.Значение <> ФормаВыбора.ТекущийЭлемент.ТекущиеДанные.Код Тогда
			...
			// Если заполнены даты, обновим курсы
			Если ЗначениеЗаполнено(ТабличныйДокумент.Области["П000400001040" + Сред(ИмяЯчейки,Найти(ИмяЯчейки,"_"))].Значение) Тогда
				
				//Установим курс
				Курс = ПолучитьКурсВалютыНаДату(Область.Значение, ТабличныйДокумент.Области["П000400001040" + Сред(ИмяЯчейки,Найти(ИмяЯчейки,"_"))].Значение);
				
				ИмяПоказателяКурса = "П000400001050" + Сред(ИмяЯчейки, Найти(ИмяЯчейки,"_"));
				ТабличныйДокумент.Области[ИмяПоказателяКурса].Значение = Курс;
				
			КонецЕсли; 

			Если ЗначениеЗаполнено(ТабличныйДокумент.Области["П000400001080" + Сред(ИмяЯчейки,Найти(ИмяЯчейки,"_"))].Значение) Тогда
				
				//Установим курс
				Курс = ПолучитьКурсВалютыНаДату(Область.Значение, ТабличныйДокумент.Области["П000400001080" + Сред(ИмяЯчейки,Найти(ИмяЯчейки,"_"))].Значение);
				
				ИмяПоказателяКурса = "П000400001090" + Сред(ИмяЯчейки, Найти(ИмяЯчейки,"_"));
				ТабличныйДокумент.Области[ИмяПоказателяКурса].Значение = Курс;
...
Процедура ЗаполнитьПустыеСтруктурыДанных()
	...
	Для Каждого Раздел Из мСвойстваРазделовДекларации.ПолучитьЭлементы() Цикл
		...
		Если НЕ Раздел.МногостраничностьВРазделе
			...
			ТабличныйДокумент.Очистить();
			ТабличныйДокумент.Вывести(ОтчетОбъект().ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + Раздел.ИмяСтраницы));
