...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета, УдалениеСтраницы = Ложь, ИндексУдаляемойСтраницы = Неопределено, ЗаголовокРаздела = Ложь)
	...
	ТабличныйДокумент.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяТекРаздела + ?(ЭтоПоследняяСтраница,"","_Без_Итога")));
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		// заполняем ФИО
		ДанныеФЛ = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СведенияОбОрганизации.ФИО);
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "", АвторасчетПослеАвтозаполнения = Ложь)
	...
	Если НЕ ПустаяСтрока(ИмяРаздела) 
		И Найти("ЛистА,ЛистБ,ЛистД,ЛистЖ3,", ИмяРаздела + ",") > 0 Тогда
	...
	ИначеЕсли ИмяРаздела = "ЛистГ2" Тогда
		...
		Для каждого Зап Из ТаблицаБлока Цикл
			...
			//Колонка050 = Окр( Зап.П000010201006, 2);
...
Процедура ОбновитьЗначенияСтруктурыИзСтроки(Данные, ТаблицаБлока, СтрокаТаблицыБлока)
	...
	Для Каждого ЗначениеДанных Из Данные Цикл
		Поз_ = Найти(ЗначениеДанных.Ключ,"_");
		Если  Поз_ > 0 Тогда
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Лев(ИмяЯчейки,13) = "П000080001001") Тогда //Лист Б Код ОКСМ
		...
		ДополнительныеПараметры = Новый Структура("ИмяЯчейки, Область, СтандартнаяОбработка, ФормаВыбора, Элемент", ИмяЯчейки, Область, СтандартнаяОбработка, ФормаВыбора, Элемент);
	...
	ИначеЕсли (Лев(ИмяЯчейки,13) = "П000080001004") Тогда //Лист Б информация о валюте
		...
		ДополнительныеПараметры =  Новый Структура("ИмяРаздела, ИмяЯчейки, Область, СтандартнаяОбработка, ФормаВыбора, Элемент", ИмяРаздела, ИмяЯчейки, Область, СтандартнаяОбработка, ФормаВыбора, Элемент);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СтруктураXML507");
...
Процедура ЗаполнитьМакетМЧБ(Параметры, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	// ФИО
	Если Найти(Параметры.ИмяСтраницы, "Титульный") = 0 Тогда
...
Процедура ОткрытьФормуВыбораВалютыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Область.Значение <> ФормаВыбора.ТекущийЭлемент.ТекущиеДанные.Код Тогда
			...
			//Установим курс
			Курс = ПолучитьКурсВалютыНаДату(Область.Значение,ТабличныйДокумент.Области["П000080001005" + Сред(ИмяЯчейки,Найти(ИмяЯчейки,"_"))].Значение);
			
			ИмяПоказателяКурса = "П000080001006" + Сред(ИмяЯчейки, Найти(ИмяЯчейки,"_"));
...
Процедура РасчетПослеИзмененияСодержания(ИмяПоказателя, ИмяРаздела, Область)
	...
	Если (Лев(ИмяПоказателя,13) = "П000080001005") Тогда //Лист Б информация о дате получения дохода в валюте, нужно перечитать курс				
		Курс = ПолучитьКурсВалютыНаДату(ТабличныйДокумент.Области["П000080001004" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"))].Значение, Область.Значение);	
		ИмяПоказателяКурса = "П000080001006" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"));
		ТабличныйДокумент.Области[ИмяПоказателяКурса].Значение = Курс;
		СохранитьЗначениеПоказателяВСтруктуре(ЭтаФорма, ИмяРаздела, ИмяПоказателяКурса, Курс);
	КонецЕсли;
	...
	Если (Лев(ИмяПоказателя,13) = "П000080001009") Тогда //Лист Б информация о дате уплаты налога в валюте, нужно перечитать курс				
		Курс = ПолучитьКурсВалютыНаДату(ТабличныйДокумент.Области["П000080001004" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"))].Значение, Область.Значение);	
		ИмяПоказателяКурса = "П000080001010" + Сред(ИмяПоказателя, Найти(ИмяПоказателя,"_"));
		ТабличныйДокумент.Области[ИмяПоказателяКурса].Значение = Курс;
		СохранитьЗначениеПоказателяВСтруктуре(ЭтаФорма, ИмяРаздела, ИмяПоказателяКурса, Курс);
	КонецЕсли;
