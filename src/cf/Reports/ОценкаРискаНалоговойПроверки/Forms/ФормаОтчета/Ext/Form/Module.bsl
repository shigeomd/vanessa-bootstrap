...
Процедура РезультатОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если Расшифровка.Свойство("СсылкаНаКритерий") Тогда
			...
			ПерейтиПоНавигационнойСсылке(Расшифровка.СсылкаНаКритерий);
			
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				"ОценкаРискаНалоговойПроверки." + Формат(Отчет.КонецПериода,"ДФ=yyyy-MM-dd")
				+ ".Расшифровка.СсылкаНаКритерий", "Информация", , , Истина);
	
		ИначеЕсли Расшифровка.Свойство("ПриложениеПриказа") Тогда
			
			ПерейтиПоНавигационнойСсылке(Расшифровка.ПриложениеПриказа);
			
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
				"ОценкаРискаНалоговойПроверки." + Формат(Отчет.КонецПериода,"ДФ=yyyy-MM-dd")
				+ ".Расшифровка.ПриложениеПриказа", "Информация", , , Истина);
	...
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("Строка") Тогда
		...
		Если Расшифровка = "КодОКВЭД" 
			...
			ПерейтиПоНавигационнойСсылке(АдресНавигационнойСсылки);
	...
	Иначе
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			"ОценкаРискаНалоговойПроверки." + Формат(Отчет.КонецПериода,"ДФ=yyyy-MM-dd") 
			+ ".Расшифровка.РегламентированныйОтчет", "Информация", , , Истина);
	КонецЕсли;
...
Процедура СформироватьОтчет(Команда)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		"ОценкаРискаНалоговойПроверки." + Формат(Отчет.КонецПериода,"ДФ=yyyy-MM-dd") 
		+ ".Формирование", "Информация", , , Истина);
...
Функция ПолучитьПредставлениеПериода(НачалоПериода, КонецПериода)
	...
	Если НомерКвартала = 1 Тогда
		ПредставлениеПериода = "1 квартал " + ГодОтчета+" года";
	ИначеЕсли НомерКвартала = 2 Тогда
		ПредставлениеПериода = "1 полугодие " + ГодОтчета+" года";
	ИначеЕсли НомерКвартала = 3 Тогда
		ПредставлениеПериода = "9 месяцев " + ГодОтчета+" года";
	ИначеЕсли НомерКвартала = 4 Тогда
		ПредставлениеПериода = ГодОтчета+" год";
	КонецЕсли;
...
Процедура ОткрытьФормуНовогоОтчета(Расшифровка)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		"ОценкаРискаНалоговойПроверки." + Формат(Отчет.КонецПериода,"ДФ=yyyy-MM-dd")
		+ ".СозданиеОтчета." + Расшифровка.ИсточникОтчета, "Информация", , , Истина);
