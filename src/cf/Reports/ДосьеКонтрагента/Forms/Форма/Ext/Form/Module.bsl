...
Процедура ТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Расшифровка.Действие = "Показать" 
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент()
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Информация.ВерсияПриложения, "8.3.6.1") < 0 Тогда
			ПоказатьСкрытьОбластьДокументаНаСервере(Расшифровка);
		Иначе
			ПоказатьСкрытьОбластьДокумента(ЭтотОбъект, Расшифровка);
		КонецЕсли;
...
Процедура УправлениеФормойНаСервере()
	...
	Элементы.КнопкаДобавитьВСправочник.Видимость = НЕ ЗначениеЗаполнено(Контрагент) 
...
&НаСервере
Процедура ПоказатьСкрытьОбластьДокументаНаСервере(Расшифровка)
...
Процедура ПоказатьСкрытьОбластьДокумента(Форма, Расшифровка)
	...
	СтрокиТаблицы = Форма.ОбластиРасшифровки.НайтиСтроки(Отбор);
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска)
	...
	Иначе // Поиск по наименованию
		...
		ПараметрыФормы.Вставить("Наименование",       ТекстПоиска);
		ПараметрыФормы.Вставить("РеквизитыЗаполнены", Ложь);
		ПараметрыФормы.Вставить("Заголовок",          НСтр("ru='Поиск контрагента по наименованию'"));
