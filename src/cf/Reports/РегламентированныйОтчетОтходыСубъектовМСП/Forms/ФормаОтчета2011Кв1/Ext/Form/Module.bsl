...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПечатнаяФормаРаздела = Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + Парам1.ИмяСтраницы;
			...
			Если КолвоСтрокВТаблДок > КолвоСтрокВПечатнойФорме Тогда
				...
				Для Каждого Область Из МакетПечатнойФормы.ПолучитьОбласть(НомерВерхЯчейкиОбласти + КолвоДопСтрок*ВысотаСтроки, , НомерНижнЯчейкиОбласти + КолвоДопСтрок*ВысотаСтроки).Области Цикл
					...
					Если НЕ НайденнаяОбласть = Неопределено И НайденнаяОбласть.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
						Инд = Найти(НайденнаяОбласть.Имя, "_");
						Если Инд > 0 Тогда 
...
Процедура ЗагрузитьПоказателиИзМакета(Знач РазделВМакете)	
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2011Кв1");
	...
	Если ТекОбласть <> Неопределено Тогда	
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			Иначе
				...
				Если РазделВМакете = "Вещества" Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(Область.Имя, "П100010001005_") = 1 Тогда 
		СтандартнаяОбработка = Ложь;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Найти(Область.Имя, "П100010001001_") = 1 Тогда 
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, СтрПоляТаблДокумента) Тогда
		//РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен раздел со сведениями об организациях!", "", "");
		//СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки)
	...
	Для Каждого Орг Из СоответствиеИмен Цикл
		НовыйУзел_ORG_INFO = НовыйУзелИзПрототипа(Узел_ORG_INFO);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ID_ORG", Орг.Значение.ID_ORG);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "FNAME", Орг.Значение.FNAME);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "INN", Орг.Значение.INN);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDJ_OKATO", Орг.Значение.OKATO);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDR_JUR", Орг.Значение.ORG_LOCATION);
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "FINDIVID", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDJ_INDEX", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDJ_STREET", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDF_STREET", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "EMAIL", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "ADDP_STREET", "");
		ЗаполнитьЭлемент(НовыйУзел_ORG_INFO, "REG_DATE", "");
		
		Узел_WASTE_LIC = ПолучитьПодчиненныйЭлемент(НовыйУзел_ORG_INFO, "WASTE_LIC");
		
		Лицензии = ДанныеЛицензий.НайтиСтроки(Новый Структура("КомуВыдано",  Орг.Ключ));
		Для Каждого Лицензия Из Лицензии Цикл 
			Новый_Узел_WASTE_LIC = НовыйУзелИзПрототипа(Узел_WASTE_LIC);
			ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "ID_LIC", Лицензия.LIC_ID);
			ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "NUM_DOC", Лицензия.НомерДокумента);
			ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "ISSUED_NAME", Лицензия.КтоВыдал);
			ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "DATE_DOC", Лицензия.ДатаВыдачи);
			ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "DATE_BEGIN", Лицензия.ДатаНачалаДействия);
			Если ЗначениеЗаполнено(Лицензия.ДатаОкончанияДействия) Тогда 
				ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "DATE_END", Лицензия.ДатаОкончанияДействия);
				ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "PERMANENT_FLAG", "false");
			Иначе 
				ЗаполнитьЭлемент(Новый_Узел_WASTE_LIC, "PERMANENT_FLAG", "true");
			КонецЕсли;
		КонецЦикла;
		
		РегламентированнаяОтчетность.УдалитьУзел(Узел_WASTE_LIC);
	КонецЦикла;
	...
	Для Каждого Стр Из мДанныеОтчета.ПолеТабличногоДокументаОтхСвед Цикл
		...
		ЗаполнитьЭлемент(НовыйУзел_RPT_MSP_WASTE_FACT, "WST_CODE", ОснДанные.П0302);
...
Функция СформироватьСтруктуруПараметров()
	...
	СтрокаСведений = "ОКВЭД, ОКАТО, ОКПО, ОГРН, ТипНП, ДатаРегистрации, ИННЮЛ, КППЮЛ, НаимЮЛПол, АдрЮР, АдрФакт, АдрПочт, ";
