...
Процедура ПоказатьПериод()
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц Тогда
		...
		//ПолеТабличногоДокументаТитульный.Области.НомКвартал2.Значение = Сред(Формат(Число(?(НомерМесяца="0","12",НомерМесяца)), "ЧЦ=2; ЧВН="), 2, 1);
	Иначе
		ПолеТабличногоДокументаТитульный.Области.НомКвартал.Значение = Формат(Число(НомерКвартала), "ЧЦ=2; ЧВН=");
		//ПолеТабличногоДокументаТитульный.Области.НомКвартал2.Значение = Сред(Формат(Число(НомерКвартала), "ЧЦ=2; ЧВН="), 2, 1);
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если СтруктураРеквизитовФормы.ИП Тогда
		...
		ФИОРук        = "";
		ИННРук        = "            ";
		СтруктураРеквизитовФормы.мИННДиректор  = "            ";

		ФИОБух        = "";
		ИННБух        = "            ";
		СтруктураРеквизитовФормы.мИННБухгалтер = "            ";
	...
	Признаки = ТабличныйДокумент.Области.ТипОрг1.Значение + ТабличныйДокумент.Области.ТипОрг2.Значение + ТабличныйДокумент.Области.ТипОрг3.Значение;

	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Или (НЕ ЗначениеЗаполнено(Признаки)) Тогда
		Если ТипНП = 1 Тогда
	...
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Или (ПустаяСтрока(ТабличныйДокумент.Области.КодТелНА.Значение)) Тогда
		ТабличныйДокумент.Области.КодТелНА.Значение  = ТелОрганизации;
	КонецЕсли;
	...
	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) И (ТипНП = 3) Тогда
		СтруктураРеквизитовФормы.мОргОГРН = "             ";
		ТабличныйДокумент.Области["ОГРН"].Значение = СтруктураРеквизитовФормы.мОргОГРН;
	КонецЕсли;

	Если (СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Или (ПустаяСтрока(СтруктураРеквизитовФормы.мОКАТОРаздел1)) Тогда
		Если (НЕ СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) И (НЕ СтруктураРеквизитовФормы.мДокументВосстановлен) Тогда
			...
			СтруктураРеквизитовФормы.мОКАТОРаздел1 = КодТерриториальногоОбразования;
			РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "Раздел1", "ОКАТО", 11, КодТерриториальногоОбразования);
...
&НаСервере
Процедура ДопИзменениеДатыПодписи()
...
Функция ПроверитьКорректностьЗаполненияОКТМО()
	...
	Для ТаблИндекс = 0 По ТаблЗначРаздел1.Количество() - 1 Цикл
		...
		Если НЕ (ДлинаОКАТО = 11 ИЛИ ДлинаОКАТО = 8 ИЛИ ДлинаОКАТО = 0) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru='На странице %1 раздела 1 неверно задан код по ОКТМО!'"), Формат(ТаблИндекс + 1, "ЧГ="));
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, ТекстОписанияОшибки, "Раздел1", "ОКАТО", ТаблИндекс + 1);
	...
	Для ТаблИндекс = 0 По ТаблЗначРаздел2.Количество() - 1 Цикл
		...
		Если НЕ (ДлинаОКАТО = 11 ИЛИ ДлинаОКАТО = 8 ИЛИ ДлинаОКАТО = 0) Тогда
			...
			ТекстОписанияОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru='На странице %1 раздела 2 неверно задан код по ОКТМО!'"), Формат(ТаблИндекс + 1, "ЧГ="));
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, ТекстОписанияОшибки, "Раздел2", "ОКАТО", ТаблИндекс + 1);
	...
	Для Каждого СтрПоказатели Из Форма.мТаблицаСоставПоказателей Цикл
		...
		Если (ТипДанныхПоказателя = "Е") И (ПустаяСтрока(КодПоказателяПоСтруктуре)) Тогда
			...
			Иначе
				...
				Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
					//ФорматируемаяОбласть = Форма["ПолеТабличногоДокумента" + ИмяПоляТаблДокумента].Области[ИмяПоказателя + "_" + НСтр];
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ФизЛица" Тогда
		...
		ИначеЕсли Найти(Область.Имя, "ФизЛицКодГражд") > 0 Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		Если Найти(Область.Имя, "КБК") > 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли Найти(ОбластьИмя, "КодСтраны") > 0
			  ИЛИ Найти(ОбластьИмя, "П000010003000") > 0 Тогда
	...
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел2" Тогда
		...
		Если (Найти(ОбластьИмя, "П000020005004") > 0) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли (Найти(ОбластьИмя, "П000020005004") > 0) И (СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли Найти(ОбластьИмя, "П000020004000") > 0
			  ИЛИ Найти(ОбластьИмя, "П000020003000") > 0 Тогда
		...
		ИначеЕсли Найти(Область.Имя, "П000020005011") > 0 Тогда
            Если СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда // отключен выбор
				//можно добавить форму ввода
				Возврат;
			КонецЕсли;
	...
	Если (ИмяТекущейСтраницыПанели = "Раздел2") Или (ИмяТекущейСтраницыПанели = "Раздел1") Тогда
		...
		Если Группа <> Неопределено Тогда
			...
			РегламентированнаяОтчетностьКлиентСервер.СохранитьЗначениеВМногостраничномБлоке(
				ЭтаФорма, Группа, НомерСтрокиМнгЧ, Сред(ОбластьИмя, 1, 13), Элемент.ТекущаяОбласть.Значение);
		КонецЕсли;
