...
Процедура ВыводДанныхОФизическомЛице(Знач СвязьУстановлена, Знач Данные, ТипДанных, ОграниченияВыводаУчесть = Ложь, ТаблицаСтраницРазделаМ, ИндексВТабл);
	...
	Если ТипДанных = 3 Тогда
		...
		ДанныеПредставителя = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОПредставителе(ЭтаФорма.СтруктураРеквизитовФормы.Организация, ЭтаФорма.ДатаПодписи, Ложь, , КПП);
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.РазделМногостраничный Тогда
		...
		Если (Найти(Элемент.ТекущиеДанные.КолонкаРазделыОтчета, "Источник") > 0 И Найти(Элемент.ТекущиеДанные.КолонкаРазделыОтчета, "дохода") > 0)
		...
		ИначеЕсли Найти(Элемент.ТекущиеДанные.КолонкаРазделыОтчета, "Сведения") > 0 И Найти(Элемент.ТекущиеДанные.КолонкаРазделыОтчета, "о доходах") > 0 Тогда
...
Процедура ОчиститьТабличноеПоле(Форма, ВыбТабличноеПолеИмя)
	...
	Если (ВыбТабличноеПолеИмя = "Раздел3") ИЛИ 
		...
		// Cодержат два многострочных блока.
...
Процедура ПостроениеПервоначальнойСтруктуры(Форма, Первый = Истина, НаимРаздела)
	...
	Если Первый Тогда
		...
		//ОчиститьТабличноеПоле(Форма, "ПолеТабличногоДокументаРаздел1_2");
		//СтруктураДанныхТекущейТаблицыРаздел1_2 = СобратьДанныеТекущегоТаблПоля(Форма, "ПолеТабличногоДокументаРаздел1_2");
...
Процедура ИнициализацияСоставаПоказателей()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СоставПоказателей2007Кв1");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
		...
		// Выбор КодВЭД
		Если Найти(Ячейка,"КодВЭД") > 0 Тогда
			СтандартнаяОбработка = Ложь;
			...
			ОткрытьФорму("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли (Найти(Ячейка,"П000020010002") > 0) Или (Найти(Ячейка, "П000020010003") > 0) Тогда
			...
			КолонкаПоиска = "Код";
			Если Найти(Ячейка, "П000020010002") > 0 Тогда
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел3" Тогда
		...
		Если (Найти(Ячейка, "П000030009004_") > 0
			Или  Найти(Ячейка, "П000030013004_") > 0)
			И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
		
		Если ((Найти(Ячейка,СтруктураРеквизитовФормы.мИдГруппы3_Стр2_1) > 0)
			Или (Найти(Ячейка, СтруктураРеквизитовФормы.мИдГруппы3_Стр2_2) > 0)
			Или (Найти(Ячейка, СтруктураРеквизитовФормы.мИдГруппы3_Стр3_1) > 0)
			Или (Найти(Ячейка, СтруктураРеквизитовФормы.мИдГруппы3_Стр3_2) > 0))
			И (Найти(Ячейка, "1_") = 0)
			И (Найти(Ячейка, "5_") = 0)
			И (Найти(Ячейка, "7_") = 0)
			И (Найти(Ячейка, "8_") = 0)
			И (Найти(Ячейка, "6_") = 0) Тогда
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел4" Тогда
		...
		Если (Найти(ОбластьИмя, "НомСтр") = 0) И (ОбластьИмя <> "П000040003001") И (ОбластьИмя <> "П000040008001") И (ОбластьИмя <> "П000040013001") И (ОбластьИмя <> "П000040016001") Тогда
			СтандартнаяОбработка = Ложь;
			Если Область.Значение = "V" Тогда
				Область.Значение = "";
			Иначе
	...
	Если Результат <> Неопределено Тогда
		...
		Если НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
			...
			Если Значение2 = "Пустое значение" Тогда
				Значение2 = "";
			КонецЕсли;
			Если Найти(Ячейка, "П000010007003") > 0 Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Найти(ОбластьИмя, "П000030009002_") > 0 Тогда
		Если НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
	...
	Если Найти(ОбластьИмя, "П000030013002_") > 0 Тогда
		Если НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
