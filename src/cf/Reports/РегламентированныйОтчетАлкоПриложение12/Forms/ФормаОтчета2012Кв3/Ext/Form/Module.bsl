...
Процедура СформироватьСпискиВыбора()
	...
	ИначеЕсли СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20140901' Тогда
		ИмяМакета = "Списки2012Кв3_20131101";
	Иначе
		ИмяМакета = "Списки2014Кв4";
	КонецЕсли;
	
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Сведения = "ИННЮЛ, КППЮЛ, НаимЮЛПол, ТелОрганизации, АдресЭлектроннойПочтыОрганизации, ФИОРук, ФИОБух" + ?(ЭтоПБОЮЛ, ", АдрПрописки", ", АдрЮР");
	...
	Если Результат <> Неопределено Тогда
		...
		Если ЭтоПБОЮЛ Тогда
			...
			СтруктураРеквизитовФормы.ИндивидуальныйПредприниматель = СтруктураРеквизитовФормы.Организация.ИндивидуальныйПредприниматель.Наименование;
		Иначе
	...
	Для НомНовойСтроки = ИсходноеКоличествоСтрокГруппы По ТекущееКоличествоСтрокГруппы Цикл
		...
		Для Каждого Область Из ОбластьДопСтроки.Области Цикл
			...
			Если ОбластьТаблДок.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Тогда
				...
				ОбластьТаблДок.Имя = Лев(Область.Имя, Найти(Область.Имя, "_")) + Формат(ТекущееКоличествоСтрокГруппы - НомНовойСтроки + 1, "ЧГ=");
...
Процедура ПроверитьЗаполнениеТитульный(ПараметрыВыгрузки, СтатусПроверки)
	...
	Если ПараметрыВыгрузки.Свойство("ИННЮЛ") Тогда
	...
	Если ПараметрыВыгрузки.ЭтоПБОЮЛ Тогда
		УказаныФИО = НЕ ПустаяСтрока(ПараметрыВыгрузки.ИПФамилия) И НЕ ПустаяСтрока(ПараметрыВыгрузки.ИПИмя);
...
Процедура ПроверитьЗаполнениеРаздел1(ПараметрыВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ПустаяСтрока(ДанныеЛиста.ОбособленноеПодразделение) Тогда
			ТекстОписанияОшибки = "На листе " + НомерЛистаСтроковый + " раздела ""Раздел 1"" не указаны реквизиты обособленного подразделения!";
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания > '20140901' Тогда
		...
		Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
			...
			Если НЕ Найти(ДанныеОП, ", ИНН") > 0 ИЛИ НЕ Найти(ДанныеОП, ", КПП") > 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			КППОП = Сред(ДанныеОП, Найти(ДанныеОП, ", КПП") + 6, 9);
			...
			Если СтрокаТЗ.Колво > 1 Тогда
				...
				Если НайденныеОП.Количество() > 0 Тогда
					...
					Для Каждого ЭлНайденныеОП Из НайденныеОП Цикл
						...
						ПерваяИтерация = Ложь;
					КонецЦикла;
					ТекстОписанияОшибки = "Страницы №№ " + ЛистыПовторы + " раздела ""Раздел 1"" заполнены по одному и тому же предприятию " + ЭлНайденныеОП.НаимОП + ", (КПП " + ЭлНайденныеОП.КППОП + ")";
...
Процедура ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			//ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			//+ " декларации не задан вид продукции!";
		...
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не задан код вида продукции!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателя, НомерЛиста);
		ИначеЕсли СтрДлина(ЗначениеГрафы) <> 3 Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации неверно задан код вида продукции!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указано наименование производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указан ИНН производителя/импортера!";
		...
		ИначеЕсли ЭтоРоссийскаяОрганизация И СтрДлина(ИНН) <> 10 Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если ЭтоРоссийскаяОрганизация Тогда
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
				+ " декларации не указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
				+ " декларации неверно указан КПП производителя/импортера!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел1", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеРаздел2(ПараметрыВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ПустаяСтрока(ДанныеЛиста.ОбособленноеПодразделение) Тогда
			ТекстОписанияОшибки = "На листе " + НомерЛистаСтроковый + " раздела ""Раздел 2"" не указаны реквизиты обособленного подразделения!";
			...
			Если СтрокаТЗ.Колво > 1 Тогда
				...
				Если НайденныеОП.Количество() > 0 Тогда
					...
					Для Каждого ЭлНайденныеОП Из НайденныеОП Цикл
						...
						ПерваяИтерация = Ложь;
					КонецЦикла;
					ТекстОписанияОшибки = "Страницы №№ " + ЛистыПовторы + " раздела ""Раздел 2"" заполнены по одному и тому же предприятию " + ЭлНайденныеОП.НаимОП + ", (КПП " + ЭлНайденныеОП.КППОП + ")";
...
Процедура ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	Для Каждого СтрокаДекларации Из ДанныеДопСтрок Цикл
		...
		ИначеЕсли СтрДлина(ИНН) <> ДлинаИНН И СтрДлина(ИНН) <> 11 Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации неверно указан ИНН производителя/импортера!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указано наименование поставщика!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если ПустаяСтрока(ИНН) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указан ИНН поставщика!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		ИначеЕсли СтрДлина(ИНН) <> 10 И СтрДлина(ИНН) <> 11 И СтрДлина(ИНН) <> 12 Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации неверно указан ИНН поставщика!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяИНН, НомерЛиста);
		КонецЕсли;
		...
		Если СтрДлина(ИНН) = 10 Тогда
			Если ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
				+ " декларации не указан КПП поставщика!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			ИначеЕсли СтрДлина(КПП) <> 9 Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
				+ " декларации неверно указан КПП поставщика!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		ИначеЕсли СтрДлина(ИНН) = 12 ИЛИ СтрДлина(ИНН) = 11 Тогда
			Если НЕ ПустаяСтрока(КПП) Тогда
				ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
				+ " декларации указан КПП у поставщика, не являющегося юридическим лицом, зарегистрированным на территории Российской федерации!";
				СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателяКПП, НомерЛиста);
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указана дата закупки!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если ПустаяСтрока(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указан номер товарно-транспортной накладной!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ЗначениеГрафы) Тогда
			ТекстОписанияОшибки = "В строке " + НомерСтрокиСтроковый + " таблицы "+ ИмяТаблицы + " на листе " + НомерЛистаСтроковый
			+ " декларации не указан объем закупленной продукции!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Раздел2", ИмяПоказателя, НомерЛиста);
		КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	ФИОРуководителя = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ПоказателиТитульный.ОргДиректор);
	...
	ФИОГлавбуха = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ПоказателиТитульный.ОргБухгалтер);
	...
	Если ЭтоПБОЮЛ Тогда
		...
		ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Орг.ИндивидуальныйПредприниматель.Наименование);
...
Функция ИзвлечьСтруктуруXML()
	...
	КонецЕсли;
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
...
Процедура ВыгрузитьПакетПослеПолученияКонтекста(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.ВыгрузитьПакетДляОтправкиВФСРАР(ЭтаФорма, Истина, ОписаниеОповещения);
...
Функция ПолучитьМакетФормыОтчета(ИмяРаздела)
	...
	КонецЕсли;
	Возврат Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)]
	    .ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела + ИмяСуффиксаРаздела);
КонецФункции
...
Функция ПолучитьМакетПечатнойФормы(ИмяРаздела)
	...
	КонецЕсли;
	Возврат Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)]
	    .ПолучитьМакет("Печатная" + Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела + ИмяСуффиксаРаздела);
КонецФункции
