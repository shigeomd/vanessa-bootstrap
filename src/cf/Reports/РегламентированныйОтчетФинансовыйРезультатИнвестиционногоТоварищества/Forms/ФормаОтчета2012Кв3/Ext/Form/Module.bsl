...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2012Кв3");
...
Процедура РасчетРаздел2(Форма, ОбластьИмя = "", МестоРазмещенияДанных)
	...
	Если НЕ ЗначениеЗаполнено(ОбластьИмя)
		...
		//Если ЭлементФормулыД <> 0 Тогда
		//	ЗначениеСтроки050 = Раздел3Строка010Графа4 * (Раздел2Строка040 / ЭлементФормулыД);
		//Иначе
		//	ЗначениеСтроки050 = 0;
		//КонецЕсли;
		//
		//ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, "П000020005003", ЗначениеСтроки050);
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяЯчейки, "НалоговыйОрган") = 1 Тогда
		СтандартнаяОбработка = Ложь;
		ОбработкаВыбораИФНС();
		Возврат;
	КонецЕсли;
	...
	Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений.Количество() > 0 Тогда
		...
		ТребРезультат = 0;
		Если Найти(КодПоказателя, "НалоговыйПериод") = 1 Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	ФИОПодписанта = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.ФИОПодписанта);
...
Процедура ДополнитьДанныеМЧБ_Титульный(Показатели)
	...
	Иначе
		ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО("");
	КонецЕсли;
