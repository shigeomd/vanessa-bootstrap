...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Найти(Область.Имя, "П00000200") > 0 Тогда
			КолонкаПоиска = "Код";
		Иначе
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(
...
Процедура ВвестиПоказатель(Элемент, Область, КодПоказателя, Знач ЭтоСумма, Знач ТекстВыбора, ТаблицаВыбора, СтандартнаяОбработка, КолонкаПоиска = "Код") Экспорт
	...
	Если Найти(Область.Имя, "П00000200") > 0 И Найти(Область.Имя, "04") = 0 Тогда
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	// раздел 1
	// колонки 1, 3
	Если Найти(Область.Имя, "П00000100") > 0 И Прав(Область.Имя, 2) = "01" Тогда
	...
	КонецЕсли;
	Если Найти(Область.Имя, "П00000100") > 0 И Прав(Область.Имя, 2) = "03" Тогда
	...
	КонецЕсли;
	// колонка 4
	Если Найти(Область.Имя, "П000001000104") > 0 Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение = Строка(РезультатВыбора["Код"]);
	КонецЕсли;
	// колонка 5
	Если Найти(Область.Имя, "П00000100") > 0 И Прав(Область.Имя, 2) = "05" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение = Строка(РезультатВыбора["Код"]);
	КонецЕсли;
	
	// раздел 2
	// колонки 1, 3, 5
	Если Найти(Область.Имя, "П00000200") > 0 И Прав(Область.Имя, 2) = "01" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение
			= ?(ЗначениеЗаполнено(РезультатВыбора["Код"]), Строка(РезультатВыбора["Название"]), "");
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Лев(Область.Имя, 11) + "03"].Значение
			= Строка(РезультатВыбора["Код"]);
	...
	КонецЕсли;
	Если Найти(Область.Имя, "П00000200") > 0 И Прав(Область.Имя, 2) = "03" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение = Строка(РезультатВыбора["Код"]);
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Лев(Область.Имя, 11) + "01"].Значение
			= ?(ЗначениеЗаполнено(РезультатВыбора["Код"]), Строка(РезультатВыбора["Название"]), "");
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Лев(Область.Имя, 11) + "05"].Значение
			= Строка(РезультатВыбора["Кратко"]);
	КонецЕсли;
	Если Найти(Область.Имя, "П00000200") > 0 И Прав(Область.Имя, 2) = "05" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение
	...
	КонецЕсли;
	// колонка 4
	Если Найти(Область.Имя, "П00000200") > 0 И Прав(Область.Имя, 2) = "04" Тогда
		ЭтаФорма[ТекущийЭлемент.Имя].Области[Область.Имя].Значение = Строка(РезультатВыбора["Код"]);
	КонецЕсли;
