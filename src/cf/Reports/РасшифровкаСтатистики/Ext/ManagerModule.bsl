...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	// Очищаем структуру отчета, чтобы сформировать ее изходя полученных настроек 
	...
	//Если про обходе выбранных полей обнружатся поля не ресурсы и не группировки то нужно будет включить детальную группировку
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	Если Данные_Расшифровки.Свойство("ОбъектНаблюдения") Тогда //Откроем редактирование настроек для этого объекта
		...
		// Если доступна и Аналитаки то откроем настройки для конкретной аналитики
	...
	ИначеЕсли Данные_Расшифровки.Количество() > 0 Тогда
		...
		// 2 комплекта нужно потому что отчеты по счету в качастве указателя на субконто используют 
		// номер субконто как он задан у конкретного счета, напимер: Договоры это Субконто2 счета 60
		...
		Если Данные_Расшифровки.Свойство("Счет") И ЗначениеЗаполнено(Данные_Расшифровки.Счет) Тогда
			...
			СписокПунктовМеню.Добавить("ОборотноСальдоваяВедомостьПоСчету", "ОСВ по счету " + Счет);
			НастройкиРасшифровки.Вставить("ОборотноСальдоваяВедомостьПоСчету", ПользовательскиеНастройкиДляСчета);
			СписокПунктовМеню.Добавить("КарточкаСчета", "Карточка счета " + Счет);
		...
		Для Каждого Поле Из Данные_Расшифровки Цикл
			...
			// Перебираме поля расшифровки в поисках субконто счета
			Если Поле.Ключ <> "Счет" И ЕстьСчет Тогда
				Для НомерСубконто = 1 По СвойстваСчета.КоличествоСубконто Цикл
					...
					Если ВидСубконто.ТипЗначения.СодержитТип(ТипЗнч(Поле.Значение)) Тогда
						...
						МассивГруппировокДляСчета.Добавить(Новый Структура("Поле, Представление, Использование, ТипГруппировки", "Субконто" + НомерСубконто, ВидСубконто.Наименование, Истина, 0));
						...
						// Добавляем отбор и группировку по счету и по этуму субконто
						...
						МассивГруппировокДляСубконто.Добавить(Новый Структура("Поле, Представление, Использование, ТипГруппировки", "Субконто" + НомерСубконтоПоПорядку, ВидСубконто.Наименование, Истина, 0));
		...
		Если МассивГруппировокДляСчета.Количество() > 0 Тогда
			...
			ПредставлениеВидовСубконто = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(СписокВидовСубконто.ВыгрузитьЗначения(), ", ");  //Неявное преобразование
			
			// Добавляем пункты меню для отчетов по субконто
			СписокПунктовМеню.Добавить("АнализСубконто", "Анализ субконто " + ПредставлениеВидовСубконто);
			НастройкиРасшифровки.Вставить("АнализСубконто", ПользовательскиеНастройкиДляСубконто);
			СписокПунктовМеню.Добавить("КарточкаСубконто", "Карточка субконто " + ПредставлениеВидовСубконто);
...
Процедура ПослеВыводаРезультата(ПараметрыОтчета, Результат) Экспорт
	...
	// Общая обработка установит парметры печати и колонтитулы
...
Функция ПолучитьДанныеРасшифровки(ДанныеРасшифровки, СтруктураПолей, Расшифровка)
	...
	Если ДанныеРасшифровки <> Неопределено Тогда
		...
		Для каждого ЭлементДанных Из СтруктураПолей Цикл
			// Получаем элемент расшифровки, в ктором нужно искать поля
