...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2010Кв3");
...
Функция ИзвлечьСтруктуруXML()
	...
	Если ПрименяетсяФормат502(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
		Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки502");
	Иначе
		Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки501");
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого ЛистРаздела2 Из Раздел2 Цикл
		...
		НомерСтроки = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистаРаздела2, "П000200004001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			
			П000200004001 = СокрЛП(ДанныеЛистаРаздела2["П000200004001_" + Формат(НомерСтроки, "ЧГ=")]);
			П000200004002 = СокрЛП(ДанныеЛистаРаздела2["П000200004002_" + Формат(НомерСтроки, "ЧГ=")]);
			П000200004003 = ДанныеЛистаРаздела2["П000200004003_" + Формат(НомерСтроки, "ЧГ=")];
			...
			Если ЗначениеЗаполнено(П000200004001) ИЛИ ЗначениеЗаполнено(П000200004002) ИЛИ ЗначениеЗаполнено(П000200004003) Тогда
				...
				Если ДлинаВидПАТов <> 3 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 2 на странице " + НомерЛиста + " в строке " + НомерСтроки + " не " + ?(ДлинаВидПАТов = 0, "", "верно") + " задан код вида подакцизного товара!", "Раздел2", "П000200004001_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Если ДлинаОКЕИ <> 3 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 2 на странице " + НомерЛиста + " в строке " + НомерСтроки + " не " + ?(ДлинаОКЕИ = 0, "", "верно") + " задан код единицы измерения по ОКЕИ!", "Раздел2", "П000200004002_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				// проверяем налоговую базу
				Если НЕ ЗначениеЗаполнено(П000200004003) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 2 на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задана налоговая база!", "Раздел2", "П000200004003_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого ЛистПриложения Из Раздел2_Прил1 Цикл
		...
		// перебираем все строки табличной части
		НомерСтроки = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистаПриложения, "П000210004001_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			
			П000210004001 = ДанныеЛистаПриложения["П000210004001_" + Формат(НомерСтроки, "ЧГ=")];
			П000210004002 = ДанныеЛистаПриложения["П000210004002_" + Формат(НомерСтроки, "ЧГ=")];
			П000210004003 = ДанныеЛистаПриложения["П000210004003_" + Формат(НомерСтроки, "ЧГ=")];
			...
			Если ЗначениеЗаполнено(П000210004001) ИЛИ ЗначениеЗаполнено(П000210004002) ИЛИ ЗначениеЗаполнено(П000210004003) Тогда
				Если НЕ ЗначениеЗаполнено(П000210004001) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В приложении на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задано содержание э.с./мощность двигателя!", "Раздел2_Прил1", "П000210004001_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(П000210004002) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В приложении на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задан объем подакцизной продукции!", "Раздел2_Прил1", "П000210004002_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(П000210004003) Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В приложении на странице " + НомерЛиста + " в строке " + НомерСтроки + " не задана налоговая база!", "Раздел2_Прил1", "П000210004003_" + Формат(НомерСтроки, "ЧГ="), НомерЛиста);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для Каждого ЛистРаздела2 Из Раздел2 Цикл
		...
		Инд = 1;
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛистРаздела2, "П000200004001_" + Формат(Инд, "ЧГ=")) Цикл
			...
			ВидПАТов 	= СокрЛП(ДанныеЛистРаздела2["П000200004001_" + Формат(Инд,"ЧГ=")]);
