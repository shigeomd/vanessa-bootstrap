...
Функция ПодготовитьПараметрыОтчета()
	...
	ИначеЕсли Отчет.НаборПоказателейОтчета = 3 Тогда
		...
		ПараметрыОтчета.Вставить("НазваниеНабораПоказателейОтчета"  , "бухгалтерский учет с постоянными" + Символы.ПС + "и временными разницами");
	КонецЕсли;	
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Справка-расчет калькуляция себестоимости продукции" + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура ИнициализацияКомпоновщикаНастроек()
	...
	БухгалтерскиеОтчетыВызовСервера.ИнициализацияКомпоновщикаНастроек(ЭтаФорма, ОрганизацияИзменилась, "КалькуляцияСебестоимости");
