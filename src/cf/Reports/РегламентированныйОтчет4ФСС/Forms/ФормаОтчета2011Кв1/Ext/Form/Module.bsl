...
Процедура НачальнаяПодготовкаФормы() Экспорт 
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		Иначе
			...
			//РегламентированнаяОтчетностьКлиентСервер.УстановитьФорматВывода(ЭтаФорма);
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, СменилсяПризнакПодписанта = Ложь)
	...
	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(Форма.ТабличныйДокумент.Области.ОКВЭД.Значение)) Тогда
	...
	//Если Форма.СтруктураРеквизитовФормы.ЕстьРеквизитДополнительныйКодФСС Тогда
	//	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(Форма.ТабличныйДокумент.Области.ДополнительныйКод.Значение)) Тогда
	//		ДополнительныйКодФСС = Форма.СтруктураРеквизитовФормы.Организация.ДополнительныйКодФСС;
	//		Если ЗначениеЗаполнено(ДополнительныйКодФСС) Тогда
	//			Форма.ТабличныйДокумент.Области.ДополнительныйКод.Значение = ДополнительныйКодФСС;
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЕсли;
	//
	//Если Форма.СтруктураРеквизитовФормы.ЕстьРеквизитКодПодчиненностиФСС Тогда
	//	Если (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) ИЛИ (НЕ ЗначениеЗаполнено(Форма.ТабличныйДокумент.Области.КодПодчиненности.Значение)) Тогда
	//		КодПодчиненностиФСС = Форма.СтруктураРеквизитовФормы.Организация.КодПодчиненностиФСС;
...
//
// Параметры:
//  ИмяТекущегоРаздела - строка, имя страницы основной панели формы.
//
&НаКлиентеНаСервереБезКонтекста
...
// Выполняет расчет вычисляемых показателей в структуре раздела.
// Выводит расчитанные показатели в табличный документ активного раздела.
//
// Параметры:
//  ИмяТекущегоРаздела - строка, имя страницы основной панели формы.
//
...
Процедура РасчетВСтруктуре(Форма, ИмяТекущегоРаздела)
	...
	ИначеЕсли ИмяТекущегоРаздела = "Раздел1_4" Тогда
		...
		Для Ном = 1 По ТаблицаБлока.Количество() Цикл
			...
			//СтруктураДанныхРаздела["П000310001005_" + Ном] = ТаблицаБлока[Ном-1]["П000310001005"];
			//СтруктураДанныхРаздела["П000310001006_" + Ном] = ТаблицаБлока[Ном-1]["П000310001006"];
			//СтруктураДанныхРаздела["П000310001007_" + Ном] = ТаблицаБлока[Ном-1]["П000310001007"];
			//СтруктураДанныхРаздела["П000310001008_" + Ном] = ТаблицаБлока[Ном-1]["П000310001008"];
...
// ОчиститьПоказателиОтчетаЗаполняемыеИзИБНаСервере()
//
&НаСервере
...
Процедура ЗаполнитьПечатнуюФорму(ПараметрыПечати, МакетПечатнойФормы, ОбщиеРеквизитыРазделов)
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник И Область.СодержитЗначение Тогда
			...
			Если ПараметрыПечати.ИмяСтраницы = "Титульный" Тогда
				...
				ИначеЕсли СтрЧислоВхождений(Область.Имя, "КодОрганаФСС") > 0 Тогда
					...
					//МакетПечатнойФормы.Рисунки.D1.Значение = Сред(СтрКодОрганаФСС, 1, 1);
					//МакетПечатнойФормы.Рисунки.D2.Значение = Сред(СтрКодОрганаФСС, 2, 1);
					//МакетПечатнойФормы.Рисунки.D3.Значение = Сред(СтрКодОрганаФСС, 3, 1);
					//МакетПечатнойФормы.Рисунки.D4.Значение = Сред(СтрКодОрганаФСС, 4, 1);
	...
	//			ИначеЕсли СтрЧислоВхождений(Область.Имя, "СоставленаНа") > 0 Тогда
	//				
	//				СтрСоставленаНа = Область.Значение;
	//				СтрСоставленаНа = Прав("000" + СтрСоставленаНа, 3);

	//				Для Ном = 1 По СтрДлина(СтрСоставленаНа) Цикл
	//					
	//					МакетПечатнойФормы.Области[Область.Имя + Ном].Значение = Сред(СтрСоставленаНа, Ном, 1);
	//					
	//				КонецЦикла;
	//				
	//				Продолжить;
...
Процедура ЗаполнитьСодержательнуюЧасть(ПараметрыВыгрузки, УзелРодитель)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел1_4, "П000310001001_" + Формат(НомСтр, "ЧГ=")) Цикл
		...
		Если ЗначениеЗаполнено(Раздел1_4["П000310001002_" + Формат(НомСтр, "ЧГ=")]) Тогда
			...
			ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ВРЕГ(Раздел1_4["П000310001002_" + Формат(НомСтр, "ЧГ=")]));
