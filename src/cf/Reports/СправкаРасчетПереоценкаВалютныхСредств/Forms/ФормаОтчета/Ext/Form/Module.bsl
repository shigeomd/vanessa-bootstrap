...
Функция ПодготовитьПараметрыОтчета()
	...
	ИначеЕсли Отчет.НаборПоказателейОтчета = 3 Тогда
		...
		ПараметрыОтчета.Вставить("НазваниеНабораПоказателейОтчета"  , "бухгалтерский, налоговый	учет с постоянными и временными разницами");
	КонецЕсли;	
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Справка-расчет переоценки валютных средств" + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(Отчет.СНачалаГода,НачалоГода(Отчет.НачалоПериода),Отчет.НачалоПериода), Отчет.КонецПериода);
