...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2013Кв4");
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Область)
	...
	Если ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел3" Тогда
		...
		ПозицияПодстроки = Найти(Область.Имя, "_");
...
Процедура СформироватьРаздел1()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКТМО) И НЕ ПустаяСтрока(ВидИмущества) И Найти("1234567", ВидИмущества) <> 0 Тогда	
...
Процедура ПроверитьЗаполнение_Раздел2(СочетаниеРазделов, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодВидаИмущества) ИЛИ Найти("1234567", КодВидаИмущества) = 0 Тогда
