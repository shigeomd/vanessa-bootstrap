...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если Год(ПараметрыОтчета.НачалоПериода) <> Год(ПараметрыОтчета.КонецПериода) Тогда
		ПараметрыОтчета.НачалоПериода = НачалоГода(ПараметрыОтчета.КонецПериода);
	КонецЕсли;
	
	ПараметрыОтчета.НачалоПериода = НачалоМесяца(ПараметрыОтчета.НачалоПериода);
	ПараметрыОтчета.КонецПериода  = КонецМесяца(ПараметрыОтчета.КонецПериода);
	
	ПараметрыОтчета.Вставить("НачалоГода",  НачалоГода(ПараметрыОтчета.НачалоПериода));
	ПараметрыОтчета.Вставить("КонецГода",   КонецГода(ПараметрыОтчета.КонецПериода));
	
	ПараметрыОтчета.Вставить("СНачалаГода", (ПараметрыОтчета.НачалоПериода = ПараметрыОтчета.НачалоГода));
	
	Если Не УчетнаяПолитика.ВестиУчетПоВидамДеятельностиИП(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода) Тогда
...
Процедура ПодготовитьДанныеОтчета(ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("ДатаНач1",        ПараметрыОтчета.НачалоПериода-1);
	Запрос.УстановитьПараметр("ДатаНачГода",     ПараметрыОтчета.НачалоГода);
	Запрос.УстановитьПараметр("ДатаНачГода1",    ПараметрыОтчета.НачалоГода-1);
	Запрос.УстановитьПараметр("ДатаКон",         ПараметрыОтчета.КонецПериода);
	Запрос.УстановитьПараметр("ДатаНачОстатков", Новый Граница(ПараметрыОтчета.НачалоПериода, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ДатаКонОстатков", Новый Граница(ПараметрыОтчета.КонецПериода, ВидГраницы.Включая));
	...
	Запрос.УстановитьПараметр("Субсчета69", БухгалтерскийУчетВызовСервераПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РасчетыПоСоциальномуСтрахованию));
	...
	ВсеХарактерыДеятельности = УчетДоходовИРасходовПредпринимателя.ХарактерыДеятельностиОСН();
	Для Каждого ХарактерДеятельности Из ХарактерыДеятельностиЕНВД Цикл
		ВсеХарактерыДеятельности.Добавить(ХарактерДеятельности);
	КонецЦикла;
	Для Каждого ХарактерДеятельности Из ХарактерыДеятельностиУСНПатент Цикл
		ВсеХарактерыДеятельности.Добавить(ХарактерДеятельности);
	КонецЦикла;
	...
	ВыводитьНачОстатки = (ПараметрыОтчета.НачалоПериода = ПараметрыОтчета.НачалоГода);
	ВыводитьКонОстатки = (ПараметрыОтчета.КонецПериода = ПараметрыОтчета.КонецГода);
...
Функция ТекстЗапросаОстаткиМПЗ(ПараметрыОтчета, НомераТаблиц, ИмяТаблицы, УловиеПоВидуМПЗ, ОстаткиНачальные = Истина)
	...
	Если НЕ ОстаткиНачальные Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДатаНачОстатков", "&ДатаКонОстатков");
	КонецЕсли;
...
Функция ТекстЗапросаОборотыМПЗ(ПараметрыОтчета, НомераТаблиц, ИмяТаблицы, УловиеПоВидуМПЗ)
	...
	|ГДЕ
	|	ИПМПЗ.Активность
	|	И ИПМПЗ.Период МЕЖДУ &ДатаНач И &ДатаКон
	...
	Если НЕ ПараметрыОтчета.СНачалаГода Тогда
...
Функция ТекстЗапросаОборотыИМР(ПараметрыОтчета, НомераТаблиц)
	...
	|ГДЕ
	|	ИПИМР.Активность
	|	И ИПИМР.Период МЕЖДУ &ДатаНач И &ДатаКон
...
Функция ТекстЗапросаВТДоходыИРасходы(ПараметрыОтчета, НомераТаблиц, ДляПризнанияРасходовТребуетсяПолучениеДоходаИП)
	...
	Если НЕ ДляПризнанияРасходовТребуетсяПолучениеДоходаИП Тогда
		...
		|ГДЕ
		|	ИПДоходы.Активность
		|	И ИПДоходы.Период МЕЖДУ &ДатаНач И &ДатаКон
...
Функция ТекстЗапросаОС(ПараметрыОтчета, НомераТаблиц)
	...
	|	МЕСЯЦ(&ДатаКон) - ВЫБОР
	|		КОГДА ГОД(ДОБАВИТЬКДАТЕ(СостоянияПринятоКУчетуОС.ДатаСостояния, МЕСЯЦ, 1)) < ГОД(&ДатаКон)
	|			ТОГДА 0
	...
	|ГДЕ
	|	НачислениеАмортизацииОС.Период < &ДатаНачГода
...
Функция ТекстЗапросаНМА(ПараметрыОтчета, НомераТаблиц)
	...
	|	МЕСЯЦ(&ДатаКон) - ВЫБОР
	|		КОГДА ГОД(ДОБАВИТЬКДАТЕ(СостоянияПринятоКУчетуНМА.Период, МЕСЯЦ, 1)) < ГОД(&ДатаКон)
	|			ТОГДА 0
...
Функция ТекстЗапросаРасходыНаОплатуТруда(ПараметрыОтчета, НомераТаблиц)
	...
	НомераТаблиц.Вставить("ВТДокументыФизическихЛиц",			НомераТаблиц.Количество());
	...
	НомераТаблиц.Вставить("ВТДокументыФизическихЛицУничтожить",	НомераТаблиц.Количество());
	...
	|				И Физлицо В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ВТРасходыНаОплатуТруда.ФизЛицо КАК Физлицо
	|					ИЗ
	|						ВТРасходыНаОплатуТруда)) КАК ДокументыФизическихЛицСрезПоследних
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Физлицо
	|;
	...
	|	ДокументыФизическихЛиц.ВидДокумента КАК Вид,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДокументыФизическихЛиц КАК ДокументыФизическихЛиц
	|		ПО РасходыНаОплатуТруда.ФизЛицо = ДокументыФизическихЛиц.Физлицо
...
Функция ТекстЗапросаПрочиеРасходы(ПараметрыОтчета, НомераТаблиц)
	...
	|	И ИППрочиеРасходы.Период МЕЖДУ &ДатаНач И &ДатаКон
...
Функция ТекстЗапросаРасходыБудущихПериодов(ПараметрыОтчета, НомераТаблиц)
	...
	|ГДЕ
	|	ИПРБП.Активность
	|	И ИПРБП.Период МЕЖДУ &ДатаНач И &ДатаКон
...
Функция СформироватьТитульныйЛист(ПараметрыОтчета, КодТаблицы)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КнигаУчетаДоходовИРасходовПредпринимателя_ТитульныйЛист";
...
Функция СформироватьСодержание(ПараметрыОтчета, КодТаблицы)
	...
	ТабличныйДокумент.ТолькоПросмотр      = Истина;
	ТабличныйДокумент.ОриентацияСтраницы  = ОриентацияСтраницы.Ландшафт;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КнигаУчетаДоходовИРасходовПредпринимателя_Содержание";
...
Функция СформироватьСведения(ПараметрыОтчета, КодТаблицы)
	...
	ТабличныйДокумент.ЧерноБелаяПечать    = Истина;
	ТабличныйДокумент.ОриентацияСтраницы  = ОриентацияСтраницы.Ландшафт;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КнигаУчетаДоходовИРасходовПредпринимателя_Сведения";
...
Функция СформироватьТаблицу11_12_13(ПараметрыОтчета, КодТаблицы)
	...
	ТабличныйДокумент.ТолькоПросмотр      = Истина;
	ТабличныйДокумент.ОриентацияСтраницы  = ОриентацияСтраницы.Ландшафт;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КнигаУчетаДоходовИРасходовПредпринимателя_Таблица" + Таблица.КодТаблицы;
...
Функция СформироватьТаблицу16_17(ПараметрыОтчета, КодТаблицы)
	...
	ВыводитьНачОстатки = (ПараметрыОтчета.НачалоПериода = НачалоГода(ПараметрыОтчета.НачалоПериода));
	ВыводитьКонОстатки = (ПараметрыОтчета.КонецПериода = КонецГода(ПараметрыОтчета.КонецПериода));
	
	НачОстаткиМПЗ = ?(ВыводитьНачОстатки, ПараметрыОтчета.НачОстаткиМПЗ, Неопределено);
	КонОстаткиМПЗ = ?(ВыводитьКонОстатки, ПараметрыОтчета.КонОстаткиМПЗ, Неопределено);
	НачОстаткиМПЗОтгруженные = ?(ВыводитьНачОстатки, ПараметрыОтчета.НачОстаткиМПЗОтгруженные, Неопределено);
	КонОстаткиМПЗОтгруженные = ?(ВыводитьКонОстатки, ПараметрыОтчета.КонОстаткиМПЗОтгруженные, Неопределено);
		...
		Если ВыборкаОбщие.Следующий() Тогда
			...
			Если ВыборкаПериодОтчета.НайтиСледующий(1, "ПериодОтчета") Тогда
				...
				Пока ВыборкаМесяц.Следующий() Цикл
					...
					ВыборкаФизЛицо = ВыборкаМесяц.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "ФизЛицо");
					Пока ВыборкаФизЛицо.Следующий() Цикл
						...
						УдЛичности = ?(НЕ ЗначениеЗаполнено(ВыборкаФизЛицо.Вид),"","" + ВыборкаФизЛицо.Вид + ", ") 
							+ ?(НЕ ЗначениеЗаполнено(ВыборкаФизЛицо.Серия),"","" + "Серия: "   + СокрЛП(ВыборкаФизЛицо.Серия))
							+ ?(НЕ ЗначениеЗаполнено(ВыборкаФизЛицо.Номер),"","" + ", № " + СокрЛП(ВыборкаФизЛицо.Номер))
							+ ?(НЕ ЗначениеЗаполнено(ВыборкаФизЛицо.ДатаВыдачи),"","" + ", выдан " + Формат(ВыборкаФизЛицо.ДатаВыдачи, "ДФ=dd.MM.yyyy"));
						...
						Если ВыборкаУдержание.НайтиСледующий(Ложь, "ЭтоУдержание") Тогда
							...
							Пока ВыборкаВидНачисления.Следующий() Цикл
								...
								Пока Выборка.Следующий() Цикл
									Если ЗначениеЗаполнено(Выборка.ДокументОплаты) И РасшифровкаДокументаОплаты.НайтиПоЗначению(Выборка.ДокументОплаты) = Неопределено Тогда
										ДатаОплаты = ДатаОплаты + ?(ПустаяСтрока(ДатаОплаты), "", "; ") + Формат(Выборка.ДатаДокументаОплаты, "ДЛФ=D");
										РасшифровкаДокументаОплаты.Добавить(Выборка.ДокументОплаты);
									КонецЕсли;
								КонецЦикла;
						...
						ОбластьРаботникКВыплате.Параметры.ДатаОплаты = ДатаОплаты;
...
Функция СформироватьТаблицу61(ПараметрыОтчета, КодТаблицы)
	...
	Область.Параметры.ДатаКон      = ПараметрыОтчета.КонецПериода;
...
Функция ОписаниеПериода(ПараметрыОтчета, ПечататьНалоговыйПериод = Ложь, СНГ = Ложь)
	
	Если ПараметрыОтчета.НачалоПериода = ПараметрыОтчета.НачалоГода
		И ПараметрыОтчета.КонецПериода = ПараметрыОтчета.КонецГода Тогда
		
		ТекстНалоговыйПериод = ?(ПечататьНалоговыйПериод, НСтр("ru = ' налоговый период'"), "");
		...
		ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'за%1 %2'"), ТекстНалоговыйПериод, ТекстПериода);
		
	Иначе
		
		ТекстПериода = НРег(ПредставлениеПериода(
			?(СНГ, НачалоГода(ПараметрыОтчета.НачалоПериода), ПараметрыОтчета.НачалоПериода),
			ПараметрыОтчета.КонецПериода,
			"ФП = Истина"));
		
		ОписаниеПериода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='за %1'"), ТекстПериода);
