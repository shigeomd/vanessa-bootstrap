			...
			Если КодКварталаРаздел1_2 = "21" Тогда
				...
				// есть Лист 02 с Признаком Налогоплательщика = 3 или Приложение 5 к Листу 02 с Признаком Налогоплательщика = 3
					...
					ИначеЕсли Есть_Прил5 И флЕстьОбособ Тогда
						...
						РазделяемаяСумма = Макс(СуммаПоказателяПоВсемСтраницам(Форма, "Лист02_Прил5", "П002050012103", Истина, услПрПл_3_Лист02_Прил5), 0);
						//делим сумму на 3 части без остатка
						Данные.П000120022003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, РазделяемаяСумма/3);
						Данные.П000120023003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, РазделяемаяСумма/3);
						Данные.П000120024003 = РазделяемаяСумма - Данные.П000120022003 - Данные.П000120023003;
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	//
	ИначеЕсли Найти(ИмяПоказателя, "ПоМесту") > 0 Тогда
		флЕстьОбособ = (СтруктураРеквизитовФормы.мПоМесту = "220" ИЛИ СтруктураРеквизитовФормы.мПоМесту = "223" ИЛИ СтруктураРеквизитовФормы.мПоМесту = "224");
		...
		Если ЗначПоказателя = "" Тогда
			...
			Если НЕ Есть_Прил5 И НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
				Для каждого СтраницаТаблицыРаздела Из ТаблицаРаздела Цикл
					...
					ДанныеСтраницы.П002000023003 = 0;
					//ТабличныйДокумент.Области.П002000023003.Значение = 0;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров()
	...
	//?ПараметрыВыгрузки.Вставить("ДеревоСтраниц", мСохраненныйДок.ДеревоНастройкиСтраниц.Получить());
	...
	Для Каждого Стр Из ДанныеЛист02Прил1 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Лист02_Прил1", ДеревоВыгрузки) Тогда
			...
			Иначе
				...
				//	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерСтраницы + " раздела ""Приложение 1 к листу 02"" указано значение признака налогоплательщика, которое отсутствует в соответствующем показателе раздела ""Лист 02""!", "Лист02_Прил1", "П002010000100", НомерСтраницы);
				//	СтатусПроверки = Ложь;
				//КонецЕсли;
	...
	Для Каждого Стр Из ДанныеЛист02Прил2 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Лист02_Прил2", ДеревоВыгрузки) Тогда
			...
			Иначе
				...
				//	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерСтраницы + " раздела ""Приложение 2 к листу 02"" указано значение признака налогоплательщика, которое отсутствует в соответствующем показателе раздела ""Лист 02""!", "Лист02_Прил2", "П002020000100", НомерСтраницы);
				//	СтатусПроверки = Ложь;
				//КонецЕсли;
				...
				//		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерСтраницы + " раздела ""Приложение 2 к листу 02"" не указан код метода начисления амортизации!", "Лист02_Прил2", "П002020013303", НомерСтраницы);
				//		СтатусПроверки = Ложь;
				//	КонецЕсли;
				//КонецЕсли;
	...
	Для Каждого Стр Из ДанныеЛист02Прил3 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Лист02_Прил3", ДеревоВыгрузки) Тогда
			...
			Иначе
				...
				//	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерСтраницы + " раздела ""Приложение 3 к листу 02"" указано значение признака налогоплательщика, которое отсутствует в соответствующем показателе раздела ""Лист 02""!", "Лист02_Прил3", "П002030000100", НомерСтраницы);
				//	СтатусПроверки = Ложь;
				//КонецЕсли;
	...
	Для Каждого Стр Из ДанныеЛист02Прил4 Цикл
		...
		Если НЕ СтраницаПуста(ДанныеСтраницы, "Лист02_Прил4", ДеревоВыгрузки) Тогда
			...
			Иначе
				...
				//	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерСтраницы + " раздела ""Приложение 4 к листу 02"" указано значение признака налогоплательщика, которое отсутствует в соответствующем показателе раздела ""Лист 02""!", "Лист02_Прил4", "П002040000100", НомерСтраницы);
				//	СтатусПроверки = Ложь;
				//КонецЕсли;
