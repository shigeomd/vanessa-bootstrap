...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.КоличествоСтрокИнформационногоМакета = СтрЧислоСтрок(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2012Кв4_Изменения").ПолучитьТекст());
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСписковВыбора = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2012Кв4");
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	//ИначеЕсли Область.Имя = "П000100621002" Тогда
	//	
	...
	//	СписокВыбора.Добавить("6210");
	//	СписокВыбора.Добавить("6215");
	//	СписокВыбора.Добавить("6220");
	//	СписокВыбора.Добавить("6230");
	//			
	//	ТекстВыбора = "Выбор кода строки";
	//	
	//	ИсходноеЗначение = СокрЛП(Область.Значение);
	//	ИсходнаяСтрока = СписокВыбора.НайтиПоЗначению(ИсходноеЗначение);
	//	
	//	ДополнительныеПараметры = Новый Структура;
	//	ДополнительныеПараметры.Вставить("Область", Область);
	//	ОписаниеОповещения = Новый ОписаниеОповещения("ВыборКодаЭлементаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	//	Если ИсходнаяСтрока = Неопределено Тогда
	//		ПоказатьВыборИзСписка(ОписаниеОповещения, СписокВыбора);
	//	Иначе
	//		ПоказатьВыборИзСписка(ОписаниеОповещения, СписокВыбора, ,ИсходнаяСтрока);
	//	КонецЕсли;
...
Процедура ПечататьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ИначеЕсли Результат.АктивныйПункт = 3 Тогда
			Если Результат.Команда = "ПечататьСразу" Тогда
				ПечататьИзФормы("ПечататьСразуКНД0710097");
			Иначе 
...
Функция ПечатьНаСервере(ВидПечати) Экспорт
	...
	Если (ВидПечати = "ПечататьСразуКНД0710097" ИЛИ ВидПечати = "ПоказатьБланкКНД0710097") Тогда
...
Процедура ВывестиНаПечатьКНД0710097(ПараметрыПечати)
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		...
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2013Кв1_КНД0710097_1");
	...
	Иначе
		...
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2012Кв1_КНД0710097_1");
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2013-01-01' Тогда
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2013Кв1_КНД0710097_2");
	Иначе
		ПечатнаяФорма = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаПечати2012Кв1_КНД0710097_2");
	КонецЕсли;
