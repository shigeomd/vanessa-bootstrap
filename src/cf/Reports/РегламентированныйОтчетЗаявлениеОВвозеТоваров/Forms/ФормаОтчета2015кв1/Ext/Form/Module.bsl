...
Процедура ПриОткрытии(Отказ)
	...
	Если Найти(ЭтаФорма.Заголовок, ОргСтр) = 0 Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2015Кв1");
...
Процедура УдалитьВсеСтраницы(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьВсеСтраницыЗавершение", ЭтотОбъект);
	РегламентированнаяОтчетностьКлиент.МеханизмыОчисткиРегламентированныхОтчетов(ЭтаФорма, "ОчиститьТекущийРаздел" + ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура УдалитьВсеСтраницыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИдГруппы <> Неопределено Тогда
		...
		Если КолвоСтрокВТаблДок > КолвоСтрокВПечатнойФорме Тогда
			...
			Для Каждого Обл Из МакетПечатнойФормы.Области Цикл
				Если Обл.Лево = 0 И Обл.Право = 0 И Найти(Обл.Имя, "ОсновнаяЧасть") = 0 И Найти(Обл.Имя, "Ограничители") = 0 И Найти(Обл.Имя, "ПустаяСтрока") = 0 Тогда
...
Процедура ВывестиНаПечатьЛистРаздел1(СтруктураПараметров)
	...
	МакетПечатиРаздела = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ПечатнаяФормаОтчета2015Кв1_Раздел1");
	
	МакетОбразцаПечатиРаздела = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ПечатнаяФормаОтчета2015Кв1_Раздел1");
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ИмяВременногоФайла = КаталогВременныхФайлов() + ПараметрыВыгрузки.ИдФайл + ".xml";
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки507");
...
Функция ВыгрузкаВозможна(Параметры)
	...
	ПроверитьВозможностьВыгрузки_Раздел3(СтатусПроверки);
	
	ПроверитьВозможностьВыгрузки_Приложение(СтатусПроверки);
...
Процедура ПроверитьВозможностьВыгрузки_ПараметрыВыгрузки(СтатусПроверки)
	...
	ИначеЕсли СтрДлина(СокрЛП(КодНО))<> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Неверно заполнен код налогового органа. Требуется заполнить 4 символами в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(НаимОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнено наименование организации отправителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИННЮЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен ИНН организации отправителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(КППОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен КПП организации отправителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(ФамилияФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнена фамилия отправителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИмяФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнено имя отправителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ФамилияПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнена фамилия подписанта. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ИмяПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнено имя подписанта. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПрПодп = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнено наименование документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НомерДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнен номер документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ДатаДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнена дата документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Параметры выгрузки"", вызываемой из командного меню отчета.", "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьВозможностьВыгрузки_Раздел3(СтатусПроверки)
...
&НаСервере
Процедура ПроверитьВозможностьВыгрузки_Приложение(СтатусПроверки)
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Раздел1" Тогда
		...
		ИначеЕсли ИмяОбласти = "КомиссионерБайконур" Тогда
			...
			ОтобразитьКодСтраны(ПараметрыОбработки, ДополнительныеДанныеРазделов);   
			СоздатьДинамическийСписокВыбора(Область, СтандартнаяОбработка, Элемент);
		ИначеЕсли Найти(ИмяОбласти, "П000010000109") = 1 ИЛИ Найти(ИмяОбласти, "П000010000110") = 1 Тогда	
		...
		ИначеЕсли Найти(Область.Имя, "ПрПриобретенияУФЛ") > 0 Тогда
		...
		ИначеЕсли Найти(Область.Имя, "П000010000107") > 0 Тогда
			...
			ДополнительныеПараметры = Новый Структура("Область, ПолеТабличногоДокументаРаздел1, СтандартнаяОбработка, ФормаВыбора, Элемент", Область, ПолеТабличногоДокументаРаздел1, СтандартнаяОбработка, ФормаВыбора, Элемент);
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяТекущейСтраницыПанели = "Раздел1" Тогда
		...
		Если Найти(ОбластьИмя, "П000010000104") > 0  И СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		ИначеЕсли Найти(ОбластьИмя, "П000010000123") = 1 И СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
			НомерСтроки = Сред(ОбластьИмя, 15);
...
Функция ОбновитьДанныеРеквизитовВыгрузки()
	...
	Если ДанныеПредставителя.ТипПредставителя = "1" Тогда
		Если НЕ ЭтоПБОЮЛ Тогда
			...
			ПрПодп = 2;
			ФИОПодп = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ФИОПодпТитул);
		...
		Иначе
			...
			ФИОотправителя = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ФИОПодпТитул);
