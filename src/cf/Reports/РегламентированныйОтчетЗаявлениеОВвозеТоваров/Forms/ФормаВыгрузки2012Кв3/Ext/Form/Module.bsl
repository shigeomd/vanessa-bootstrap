...
Процедура УправлениеЭУ(Форма)
	...
	//Если Форма.СтруктураРеквизитов.ЭтоПБОЮЛ Тогда
	//	Форма.Элементы.ПрПодп2.Доступность 	 = Ложь;
	//	Форма.Элементы.ПрКурьер2.Доступность = Ложь;
	//Иначе
	//	Форма.Элементы.ПрПодп1.Доступность 	 = Ложь;
	//	Форма.Элементы.ПрКурьер1.Доступность = Ложь;
	//КонецЕсли;
...
Процедура ПрОтпрПриИзменении(Элемент)
	...
	Если ПрОтпр = 1 Тогда
		...
		ФИОФЛОтпр = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СтруктураРеквизитов.НалогоплательщикФИО);
