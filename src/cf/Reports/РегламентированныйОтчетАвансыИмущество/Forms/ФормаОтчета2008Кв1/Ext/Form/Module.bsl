	...
	Если ИмяСтраницыПанели = "Раздел2" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя) ИЛИ СтрЧислоВхождений(Форма.СтруктураЗависимыхЯчеекДляАвторасчета.П000020018003, ОбластьИмя) > 0 Тогда
			...
			Если КодВидаИмущества = "3" Тогда
				...
				* (ПолеТабличногоДокументаРаздел2.Области.П000020017003.Значение / 100);
			...
			ИначеЕсли КодВидаИмущества = "1" ИЛИ КодВидаИмущества = "2" Тогда
				Коэффициент150 = ?(ПолеТабличногоДокументаРаздел2.Области.П000020015003_Знам.Значение = 0, 0, ПолеТабличногоДокументаРаздел2.Области.П000020015003_Числ.Значение / ПолеТабличногоДокументаРаздел2.Области.П000020015003_Знам.Значение);
				Делимое = СтоимостьОблагаемогоНалогомИмущества
				* (ПолеТабличногоДокументаРаздел2.Области.П000020017003.Значение / 100)
				* Коэффициент150;
				ПолеТабличногоДокументаРаздел2.Области.П000020018003.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Делимое / 4);
...
Процедура СформироватьРаздел1()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКАТО) И НЕ ПустаяСтрока(ВидИмущества) И Найти("123", ВидИмущества) <> Неопределено Тогда
