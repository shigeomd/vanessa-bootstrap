...
Процедура СформироватьРаздел1()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКТМО) И НЕ ПустаяСтрока(ВидИмущества) И Найти("12356", ВидИмущества) <> 0 Тогда	
...
Процедура ПроверитьЗаполнение_Раздел2(СочетаниеРазделов, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодВидаИмущества) ИЛИ Найти("123456", КодВидаИмущества) = 0 Тогда
