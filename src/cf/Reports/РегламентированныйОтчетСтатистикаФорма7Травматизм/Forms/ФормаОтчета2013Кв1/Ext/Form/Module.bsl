...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	МакетПечатнойФормы = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ПечатнаяФормаРаздела);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не ПараметрыВыгрузки.ВыгрузкаСПриложением Тогда
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ОргНазв) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не задано наименование оганизации! (Форма 7-травматизм)",
	...
	Иначе
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ОргНазв) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не задано наименование оганизации! (Прил. к форме 7-травматизм)",
	...
	Если Не ПараметрыВыгрузки.ВыгрузкаСПриложением Тогда
		...
		Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, "ПолеТабличногоДокументаФорма7Травматизм") Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не заполнен раздел ""Форма 7-травматизм""!",
				"Форма7Травматизм", "");
			СтатусПроверки = Ложь;
	...
	Иначе
		...
		Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, "ПолеТабличногоДокументаПриложение") Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не заполнен раздел ""Приложение к форме 7-травматизм""!",
				"Приложение", "");
			СтатусПроверки = Ложь;
			Возврат СтатусПроверки;
		КонецЕсли;
	...
	Если Не ПараметрыВыгрузки.ВыгрузкаСПриложением Тогда
		// КОНТРОЛЬ показателей формы (Указания по заполнению Формы 7-травматизм).
		// Строка 01 ≥ строке 02.
		Если Не (ПараметрыВыгрузки["ПР00001000104"] >= ПараметрыВыгрузки["ПР00001000204"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 01 должно быть больше или равно значению в стр. 02!",
				"Форма7Травматизм", "П000001000204");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 01 ≥ строке 03.
		Если Не (ПараметрыВыгрузки["ПР00001000104"] >= ПараметрыВыгрузки["ПР00001000304"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 01 должно быть больше или равно значению в стр. 03!",
				"Форма7Травматизм", "П000001000304");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 01 ≥ строке 04.
		Если Не (ПараметрыВыгрузки["ПР00001000104"] >= ПараметрыВыгрузки["ПР00001000404"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 01 должно быть больше или равно значению в стр. 04!",
				"Форма7Травматизм", "П000001000404");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 01 ≥ строке 05.
		Если Не (ПараметрыВыгрузки["ПР00001000104"] >= ПараметрыВыгрузки["ПР00001000504"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 01 должно быть больше или равно значению в стр. 05!",
				"Форма7Травматизм", "П000001000504");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 02 ≥ строке 06.
		Если Не (ПараметрыВыгрузки["ПР00001000204"] >= ПараметрыВыгрузки["ПР00001000604"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 02 должно быть больше или равно значению в стр. 06!",
				"Форма7Травматизм", "П000001000604");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 03 ≥ строке 07.
		Если Не (ПараметрыВыгрузки["ПР00001000304"] >= ПараметрыВыгрузки["ПР00001000704"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 03 должно быть больше или равно значению в стр. 07!",
				"Форма7Травматизм", "П000001000704");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 04 ≥ строке 08.
		Если Не (ПараметрыВыгрузки["ПР00001000404"] >= ПараметрыВыгрузки["ПР00001000804"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 04 должно быть больше или равно значению в стр. 08!",
				"Форма7Травматизм", "П000001000804");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Согл. КС в шаблоне выгрузки:
		Если Не (ПараметрыВыгрузки["ПР00001001400"] > 0) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 14 должно быть больше нуля!",
				"Форма7Травматизм", "П000001001400");
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если ПараметрыВыгрузки["ПР00001000104"] <> 0 Тогда
			Если Не (ПараметрыВыгрузки["ПР00001000904"] <> 0) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в ""Форме 7-травматизм"" значение в стр. 09 должно быть не равно нулю, если значение в стр. 01 не равно нулю!",
					"Форма7Травматизм", "П000001000904");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
		
	Иначе
		// КОНТРОЛЬ показателей формы (Указания по заполнению Приложения к форме 7-травматизм).
		// Стр. 01 гр.3 = сумме стр. 02-15 гр. 3. (Реализовано путем авторасчета.)
		// Стр. 01 гр.3 = сумме стр. 16-30 гр. 3.
		ТекСумма = 0;
		Для НомСтр = 16 По 30 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"];
		КонецЦикла;
		Если Не (ПараметрыВыгрузки["ПР00002000103"] = ТекСумма) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Прил. к форме 7-травматизм"" по гр. 3 значение в стр. 01 должно быть равно сумме значений в строках 16-30!",
				"Приложение", "П000002000103");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Стр. 01 гр.4 = сумме стр. 02-15 гр. 4. (Реализовано путем авторасчета.)
		// Стр. 01 гр.4 = сумме стр. 16-30 гр. 4.
		ТекСумма = 0;
		Для НомСтр = 16 По 30 Цикл
			ТекСумма = ТекСумма + ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"];
		КонецЦикла;
		Если Не (ПараметрыВыгрузки["ПР00002000104"] = ТекСумма) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: в ""Прил. к форме 7-травматизм"" по гр. 4 значение в стр. 01 должно быть равно сумме значений в строках 16-30!",
				"Приложение", "П000002000104");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Гр. 3 >= гр. 4 по всем строкам.
		Для НомСтр = 1 По 30 Цикл
			Если Не (ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "03"] >=
				ПараметрыВыгрузки["ПР00002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04"]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
					"Не выполнено условие контроля: в ""Прил. к форме 7-травматизм"" в стр. " + Формат(НомСтр, "ЧЦ=2; ЧВН=; ЧГ=")
					+ " значение по гр. 3 должно быть больше или равно значению по гр. 4!",
					"Приложение", "П000002" + Формат(НомСтр, "ЧЦ=4; ЧВН=; ЧГ=") + "04");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЦикла;
		// Строка 01 графа 4 (Форма № 7-травматизм) = строка 01 графа 3 (Приложение к форме № 7-травматизм).
		Если Не (ПараметрыВыгрузки["ПР00001000104"] = ПараметрыВыгрузки["ПР00002000103"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: значение в стр. 01 по гр. 4 (""Форма 7-травматизм"") должно быть равно значению в стр. 01 по гр. 3 (""Прил. к форме 7-травматизм"")!",
				"Приложение", "П000002000103");
			СтатусПроверки = Ложь;
		КонецЕсли;
		// Строка 05 графа 4 (Форма № 7-травматизм) = строка 01 графа 4 (Приложение к форме № 7-травматизм).
		Если Не (ПараметрыВыгрузки["ПР00001000504"] = ПараметрыВыгрузки["ПР00002000104"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма,
				"Не выполнено условие контроля: значение в стр. 05 по гр. 4 (""Форма 7-травматизм"") должно быть равно значению в стр. 01 по гр. 4 (""Прил. к форме 7-травматизм"")!",
				"Приложение", "П000002000104");
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Функция СформироватьСтруктуруДопАтрибутов()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2013Кв1");
	ВыгрузкаСПриложением = Неопределено;
	мДанныеОтчета.ПолеТабличногоДокументаПриложение.Свойство("ВыгрузкаСПриложением", ВыгрузкаСПриложением);
	ВыгрузкаСПриложением = ?(ВыгрузкаСПриложением = Неопределено, Ложь, ВыгрузкаСПриложением);
	ИмяОбласти = ?(ВыгрузкаСПриложением, "АтрПриложение", "АтрФорма");
	МакетВидФормы = Макет.ПолучитьОбласть(ИмяОбласти);
	Для НомСтр = 1 По МакетВидФормы.ВысотаТаблицы Цикл
		АтрКлюч = СокрЛП(МакетВидФормы.Область(НомСтр, 1, НомСтр, 1).Текст);
		Если НЕ ПустаяСтрока(АтрКлюч) Тогда
			АтрКлюч = СтроковыеФункцииКлиентСервер.ЗаменитьОдниСимволыДругими(".- ", АтрКлюч, "___");
			ДопАтрибуты.Вставить(АтрКлюч, СокрЛП(МакетВидФормы.Область(НомСтр, 2, НомСтр, 2).Текст));
		КонецЕсли;
	КонецЦикла;
