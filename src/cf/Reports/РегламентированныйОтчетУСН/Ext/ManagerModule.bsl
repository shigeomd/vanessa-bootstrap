...
Функция ТаблицаФормОтчета() Экспорт
	...
	НоваяФорма.РедакцияФормы      = "от 22.06.2009 г. №58н.";
	...
	НоваяФорма.РедакцияФормы      = "от 20.04.2011 г. №48н.";
	НоваяФорма.ДатаНачалоДействия = '20110101';
	НоваяФорма.ДатаКонецДействия  = '20131231';
	
	НоваяФорма = ТаблицаФормОтчета.Добавить();
	НоваяФорма.ФормаОтчета        = "ФормаОтчета2014Кв1";
	НоваяФорма.ОписаниеОтчета     = "Приложение № 1 к приказу ФНС России от 04.07.2014 г. №ММВ-7-3/352@.";
	НоваяФорма.РедакцияФормы      = "от 04.07.2014 г. №ММВ-7-3/352@.";
...
Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	...
	Если ЭкземплярРеглОтчета.ВыбраннаяФорма = "ФормаОтчета2014Кв1" Тогда
		...
		Если ДанныеРеглОтчета.ПоказателиОтчета.Свойство("ПолеТабличногоДокументаРаздел11") Тогда
			...
			// 100 - сумма налога, подлежащая доплате.
			КодСтрокиСуммаПодлежащаяДоплате11 = "П000110010003";
			
			Если ЗначениеЗаполнено(НалогКУплате11[КодСтрокиСуммаПодлежащаяДоплате11]) Тогда
				Сумма = ТаблицаДанныхРеглОтчета.Добавить();
				Сумма.Период = Период;
				Сумма.ОКАТО  = ОКТМО11;
				Сумма.КБК    = "18210501011011000110";// в декларации отсутствует
		...
		Если ДанныеРеглОтчета.ПоказателиОтчета.Свойство("ПолеТабличногоДокументаРаздел12") Тогда
			...
			// 100 - сумма налога, подлежащая доплате.
			КодСтрокиСуммаПодлежащаяДоплате12 = "П000120010003";
			
			Если ЗначениеЗаполнено(НалогКУплате12[КодСтрокиСуммаПодлежащаяДоплате12]) Тогда
				Сумма = ТаблицаДанныхРеглОтчета.Добавить();
				Сумма.Период = Период;
				Сумма.ОКАТО  = ОКТМО12;
				Сумма.КБК    = "18210501021011000110";// в декларации отсутствует
				Сумма.Сумма  = НалогКУплате12[КодСтрокиСуммаПодлежащаяДоплате12];
			КонецЕсли;
			
			// 120 - сумма минимального налога, подлежащая уплате.
			КодСтрокиСуммаМинимальногоНалога12 = "П000120012003";
			
			Если ЗначениеЗаполнено(НалогКУплате12[КодСтрокиСуммаМинимальногоНалога12]) Тогда
				Сумма = ТаблицаДанныхРеглОтчета.Добавить();
				Сумма.Период = Период;
				Сумма.ОКАТО  = ОКТМО12;
				Сумма.КБК    = "18210501050010000110";// в декларации отсутствует
				Сумма.Сумма  = НалогКУплате12[КодСтрокиСуммаМинимальногоНалога12];
			КонецЕсли;
