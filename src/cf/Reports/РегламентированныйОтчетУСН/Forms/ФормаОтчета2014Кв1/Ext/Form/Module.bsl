...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.НаименованиеОтчета = Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7);
	СтруктураРеквизитовФормы.НаименованиеФормыОтчета = Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета"));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	СтруктураРеквизитовФормы.ИПИспользуетТрудНаемныхРаботников = РегламентированнаяОтчетностьПереопределяемый.ИПИспользуетТрудНаемныхРаботников(СтруктураРеквизитовФормы.Организация);
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		УстановитьНадписьМестаПостановкиНаУчет();
	КонецЕсли;
...
&НаСервере
Процедура УстановитьНадписьМестаПостановкиНаУчет()
...
Процедура ЗаполнитьСведенияОбОрганизации(ИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ИзмененаДатаПодписи Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТабличныйДокумент.Области.ОКВЭД.Значение)
	...
	СтруктураДанныхРаздел22.Вставить("П000220026003", СтавкаНалогаУСН(ЭтаФорма,
		КонецДня(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -9))));
	СтруктураДанныхРаздел22.Вставить("П000220026103", СтавкаНалогаУСН(ЭтаФорма,
		КонецДня(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -6))));
	СтруктураДанныхРаздел22.Вставить("П000220026203", СтавкаНалогаУСН(ЭтаФорма,
		КонецДня(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -3))));
	СтруктураДанныхРаздел22.Вставить("П000220026303", СтавкаНалогаУСН(ЭтаФорма,
...
&НаКлиентеНаСервереБезКонтекста
Функция СтавкаНалогаУСН(Форма, ДатаОкончанияПериода)
	...
	ОргСведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Форма.СтруктураРеквизитовФормы.Организация,ДатаОкончанияПериода, Сведения);
...
Процедура ВопросПерерасчетВсехРассчитываемыхПоказателейЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ТекущийРазделОтчетаСокрНаим = "Раздел21" Тогда
		ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(ТабличныйДокумент, ТабличныйДокумент.Области["П000210010203"]);
	Иначе
		...
		Если ЗначениеЗаполнено(ПарамКодыНалога) Тогда
			...
			ДополнительныеПараметры = Новый Структура(
				...
				"", ТекущийРазделОтчетаСокрНаим, МассивНомСтр, Коэффициент);
			ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	// Раздел 1.1.
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110002003", "П000210011003;П000210014003;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110004003", "П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110005003", "П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110007003", "П000210011203;П000210014203;П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110008003", "П000210011203;П000210014203;П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110010003", "П000210011303;П000210014303;П000210011203;П000210014203;П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000110011003", "П000210011303;П000210014303;П000210011203;П000210014203;П000210011003;П000210014003;П000210011103;П000210014103;П000210010203");
	...
	// Раздел 2.1.
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000210013003", "П000210011003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000210013103", "П000210011103");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000210013203", "П000210011203");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000210013303", "П000210011303");
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000220028003", "П000220021303");
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "")
	...
	ИначеЕсли ИмяРаздела = "Раздел21" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел21" Тогда
				...
				Форма.ТабличныйДокумент.Области["П000210013003"].Значение = Окр_П000210011003 * 6 / 100;
			Иначе
				Окр_П000210011003 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
					Форма, Форма.СтруктураДанныхРаздел21.П000210011003);
				Форма.СтруктураДанныхРаздел21.П000210013003 = Окр_П000210011003 * 6 / 100;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел21" Тогда
				...
				Форма.ТабличныйДокумент.Области["П000210013103"].Значение = Окр_П000210011103 * 6 / 100;
			Иначе
				Окр_П000210011103 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
					Форма, Форма.СтруктураДанныхРаздел21.П000210011103);
				Форма.СтруктураДанныхРаздел21.П000210013103 = Окр_П000210011103 * 6 / 100;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел21" Тогда
				...
				Форма.ТабличныйДокумент.Области["П000210013203"].Значение = Окр_П000210011203 * 6 / 100;
			Иначе
				Окр_П000210011203 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
					Форма, Форма.СтруктураДанныхРаздел21.П000210011203);
				Форма.СтруктураДанныхРаздел21.П000210013203 = Окр_П000210011203 * 6 / 100;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
			...
			Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел21" Тогда
				...
				Форма.ТабличныйДокумент.Области["П000210013303"].Значение = Окр_П000210011303 * 6 / 100;
			Иначе
				Окр_П000210011303 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(
					Форма, Форма.СтруктураДанныхРаздел21.П000210011303);
				Форма.СтруктураДанныхРаздел21.П000210013303 = Окр_П000210011303 * 6 / 100;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел22" Тогда
		...
		// Стр. 280 = стр. 213 х 1 / 100 (ставка налога 1%).
		Если НЕ ЗначениеЗаполнено(ОбластьИмя)
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(ИмяЯчейки, "ИзменениеМестаПостановкиНаУчет") = 1 Тогда
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если Найти(ИмяПоказателя, "П000210011") > 0 Тогда
		РасчетНаКлиенте(ИмяРаздела, ИмяПоказателя);
	КонецЕсли;
	
	// Обработка в разделе 2.1 значений в строках 140-143,
	// в зависимости от значений в строке 102 и строках 130-133.
	Если Найти(ИмяПоказателя, "П000210011") > 0
		ИЛИ Найти(ИмяПоказателя, "П000210013") > 0 // случай отключенного авторасчета
		ИЛИ Найти(ИмяПоказателя, "П000210014") > 0
		ИЛИ ИмяПоказателя = "П000210010203" Тогда
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	СоответствиеРазделовМакетам.Вставить("Раздел11",  "ПечатныйБланк2014Кв1_Раздел11");
	...
	СоответствиеРазделовМакетам.Вставить("Раздел22",  "ПечатныйБланк2014Кв1_Раздел22");
...
Функция ПараметрыВыгрузкиОтчета()
	...
	Если ЭтоПБОЮЛ Тогда
		...
		ФИО = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(Титульный.НаимОрг);
	...
	Для Каждого Показатель Из ПоказателиТаблПоле Цикл
		...
		Если ИмяПоказателя = "ИНН1_1" Тогда
			...
			Если Прав(СтруктураДанныхТитульный.ИНН, 2) = "--" Тогда
				СтруктураДанныхТитульный.ИНН = Лев(СтруктураДанныхТитульный.ИНН, Найти(СтруктураДанныхТитульный.ИНН, "--") - 1);
			КонецЕсли;
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(
		...
		Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 7), ПараметрыОтчета, Контейнер);
