...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "НачалоГода",    НачалоГода(ПараметрыОтчета.НачалоПериода));
	
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "Период",        Новый Граница(КонецДня(ПараметрыОтчета.КонецПериода), 
		ВидГраницы.Включая));
...
Процедура СформироватьРасчетНалогаНаПрибыль(ПараметрыОтчета, Документ, КэшПараметровОтчета)
	...
	РасчетСтавкиНалогаНаПрибыль = НалоговыйУчетОбособленныхПодразделений.ВыполнитьЗапросРасчетСтавкиНалогаНаПрибыльЗаМесяц(КонецДня(ПараметрыОтчета.КонецПериода));
	...
	Если СтрокаСтавки = Неопределено Тогда
		СтавкаНалогаНаПрибыль = Окр(НалоговыйУчет.ПолучитьСтавкуНалогаНаПрибыль(Новый Структура("Организация, Дата", 
		ПараметрыОтчета.ОрганизацияРасшифровки, КонецДня(ПараметрыОтчета.КонецПериода))) * 100,2);
	Иначе
	...
	ЭлементыСхемы.НазваниеОрганизации.Наименование = ПолучитьЗаголовокОтчета(ПараметрыОтчета);
...
Процедура СформироватьТаблицуОрганизаций(ПараметрыОтчета, ДокументРезультат, Результат, НазваниеПараметра, НомерСхемы, НазваниеОтчета, КэшПараметровОтчета)
	...
	ЗаголовокОтчета.Параметры.НазваниеОрганизации = ПолучитьЗаголовокОтчета(ПараметрыОтчета);
...
Функция ПолучитьЗаголовокОтчета(ПараметрыОтчета)
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.ОрганизацияРасшифровки) Тогда		
		ЗаголовокОтчета = ЗаголовокОтчета + " " + 
			БухгалтерскиеОтчетыВызовСервераПовтИсп.ПолучитьТекстОрганизация(ПараметрыОтчета.ОрганизацияРасшифровки, Ложь);	
	КонецЕсли;
