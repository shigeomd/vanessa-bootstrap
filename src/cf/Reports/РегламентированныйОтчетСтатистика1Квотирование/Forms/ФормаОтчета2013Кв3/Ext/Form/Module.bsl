...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2013Кв3_ФормаОтчета"));
...
Процедура Расчет(Форма)
	...
	//мФормаОтчета.Области.П000010023004.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(мФормаОтчета.Области.П000010023003.Значение * Ставка);
	//мФормаОтчета.Области.П000010024004.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(мФормаОтчета.Области.П000010024003.Значение * Ставка);
	//мФормаОтчета.Области.П000010025004.Значение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(мФормаОтчета.Области.П000010025003.Значение * Ставка);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Найти(Область.Имя, "СоцГарант") > 0 Тогда
		Если Найти(Область.Имя, "СоцГарантДДУ") > 0 Тогда
			Установить = Истина;
			Линия = Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1);
		...
		КонецЕсли;
	ИначеЕсли Найти(Область.Имя, "ОргФС") > 0 Тогда
		УстановитьСнятьНижнююГраницуОбласти(Область);
	ИначеЕсли Найти(Область.Имя, "ВывестиПриложениеНаПечать") > 0 Тогда
