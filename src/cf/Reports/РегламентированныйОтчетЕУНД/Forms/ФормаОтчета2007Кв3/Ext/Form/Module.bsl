...
Процедура ЗаполнитьСведенияОбОрганизации(ИзмененаДатаПодписи = Ложь)	
	...
	Если НЕ ИзмененаДатаПодписи Тогда
		...
		СтрокаСведений = "ИННЮЛ, КППЮЛ, НаимЮЛПол, КодНО, НаимНО, ТипНП, ТелОрганизации, ОКВЭДКвадратикиТочкиРазделения, ОКВЭД, ФИОРук, ФИО, ОКАТО, ОКТМО";
...
Процедура ПолеТабличногоДокументаТитульныйВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ((Найти(ОбластьИмя, "НаимНалог") > 0) Или (Найти(ОбластьИмя, "НомерГлавыНК") > 0)) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		СписокНалогов.Добавить("21", "Налог на добавленную стоимость", ?(Найти(Налоги, "Налог на добавленную стоимостьглава21глава/") > 0, Истина, Ложь));
		
		Если ОтчетностьДо2012Года Тогда
			СписокНалогов.Добавить("24", "Единый социальный налог", ?(Найти(Налоги, "Единый социальный налогглава24глава/") > 0, Истина, Ложь));
		КонецЕсли;
		          		
		СписокНалогов.Добавить("25",   "Налог на прибыль организаций", ?(Найти(Налоги, "Налог на прибыль организацийглава25глава/") > 0, Истина, Ложь));
		
		Если ОтчетностьДо2012Года Тогда
			СписокНалогов.Добавить("30", "Налог на имущество организаций", ?(Найти(Налоги, "Налог на имущество организацийглава30глава/") > 0, Истина, Ложь));
		КонецЕсли;
		          		
		СписокНалогов.Добавить("26.2", "Упрощенная система налогообложения", ?(Найти(Налоги, "Упрощенная система налогообложенияглава26.2глава/") > 0, Истина, Ложь));
	...
	ИначеЕсли (Найти(ОбластьИмя, "НалоговыйПериод") > 0) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли (Найти(ОбластьИмя, "НомерКвартала") > 0) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
...
Функция СформироватьСтруктуруПараметров()
	...
	Если ПустаяСтрока(СведенияОбОрганизации.ФИОБух) Тогда
		...
		Иначе
			ФИОРуководителя = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СведенияОбОрганизации.ФИОРук);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если (НЕ ПрименяетсяФормат402(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) И ДлинаОКАТО <> 11)
	 ИЛИ (ПрименяетсяФормат402(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) И ДлинаОКАТО <> 8 И ДлинаОКАТО <> 11) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На титульном листе не " + ?(ДлинаОКАТО = 0, "", " верно") + " задан ОКАТО!", "Титульный", "ОКАТО1");
		СтатусПроверки = Ложь;
	КонецЕсли;
