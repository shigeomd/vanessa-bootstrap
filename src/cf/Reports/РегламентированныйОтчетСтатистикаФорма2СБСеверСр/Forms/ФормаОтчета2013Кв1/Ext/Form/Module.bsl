...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если Найти(ИмяОбласти, "П000004000103_") = 1 Тогда
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 09.08.2013 № 317";
	
	Для Стр = 1 По 28 Цикл 
		Верх = ПолеТабличногоДокументаФормаОтчета.Области["П0105_"+Стр].Верх;
		Текст = ПолеТабличногоДокументаФормаОтчета.Область(Верх,3,Верх,3).Текст;
		
		ТекстФормулы = "для строки """ + Текст + """ гр.5 >= гр.6";
		Описание     = "для строки """ + Текст + """ гр.5 < гр.6";
		ЛеваяЧасть  = Показатели["П0105_"+Стр];
		ПраваяЧасть = Показатели["П0106_"+Стр];
	...
	Для Гр = 5 По 7 Цикл
		ТекстФормулы = "по гр." + Гр + " 23.20 >= 23.20.11.201* + 23.20.14.110 + 23.20.15.203* + 23.20.17.310";
		Описание     = "по гр." + Гр + " 23.20 < 23.20.11.201* + 23.20.14.110 + 23.20.15.203* + 23.20.17.310";
		ЛеваяЧасть  = Показатели["П010"+Гр+"_17"];
		ПраваяЧасть1 = Показатели["П010"+Гр+"_18"];
		ПраваяЧасть2 = Показатели["П010"+Гр+"_19"];
		ПраваяЧасть3 = Показатели["П010"+Гр+"_20"];
		ПраваяЧасть4 = Показатели["П010"+Гр+"_24"];
