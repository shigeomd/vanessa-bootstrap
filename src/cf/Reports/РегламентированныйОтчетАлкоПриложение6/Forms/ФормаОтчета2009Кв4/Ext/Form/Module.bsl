...
Процедура ЗагрузитьПоказателиИзМакета(РазделВМакете)
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '2012-07-01' Тогда
		МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2012Кв3_20120726");
	Иначе
		МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2006Кв1");
	КонецЕсли;
			...
			Для Ном = 1 По ?(НомерГрафы = 9, 4, ?(НомерГрафы = 11, 2, 1)) Цикл
				...
				Если СтруктураГруппы[0].Свойство(КолонкаТаблицыИмя) Тогда
					...
					Если (КолонкаТаблицыИмя = "П000000000008"
						...
						Если НЕ ((Найти(ЗначениеЯчейки, "643,") = 1) 
							Или (ЗначениеЯчейки = ",,,,,,,,,")
...
Функция СобратьДанныеТекущегоТаблПоля(Форма, ИмяТабличногоПоля)
	...
	Для Инд = 0 По ТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация"
		   И (Найти(ИмяПоказателя, "П000000000008") > 0 Или Найти(ИмяПоказателя, "П000010003008") > 0) Тогда
		   
			НомерСтроки = Сред(ИмяПоказателя, 15);
			
			Если Найти(ИмяПоказателя, "П000010003008") > 0 Тогда
		...
		ИначеЕсли (Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Возвраты")
		        И (Найти(ИмяПоказателя, "П000010000008") > 0 Или Найти(ИмяПоказателя, "П000010001008") > 0) Тогда
				
			НомерСтроки = Сред(ИмяПоказателя, 15);
			
			Если Найти(ИмяПоказателя, "П000010000008") > 0 Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный" Тогда
			Если Форма[Форма.мСтруктураМногостраничныхРазделов.Титульный].Индекс(НомерАктивнойСтраницы) > 0 Тогда
				Если НЕ ((ИмяПоказателя = "СерЛиц") Или (ИмяПоказателя = "НомерЛиц")
				 ...
				 Или (Найти(ИмяПоказателя, "ИНН1") > 0)
				 Или (Найти(ИмяПоказателя, "КПП1") > 0)) Тогда
					ЗначениеПоказателя = "";
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ВыводДанныхОФизическомЛице(Знач СвязьУстановлена, Знач Данные, ТипДанных, ОграниченияВыводаУчесть = Ложь, ТаблицаСтраницРазделаМ, ИндексВТабл);
	...
	Если ТипДанных = 3 Тогда
		...
		ДанныеПредставителя = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОПредставителе(СтруктураРеквизитовФормы.Организация, ДатаПодписи, Ложь, , КППТитульныйЛист);
...
Процедура УстановитьПримечаниеНаТитульныхЛистахГруппыАлкогольныхДеклараций()
	...
	Если ЗначениеЗаполнено(мТитульный.Области.ТипОрг1.Значение) Тогда
		...
		//Элементы.Представляется.Видимость = Истина;
	...
	ИначеЕсли ЗначениеЗаполнено(мТитульный.Области.ТипОрг2.Значение)
	      Или НЕ ЗначениеЗаполнено(мТитульный.Области.ТипОрг1.Значение) Тогда
			...
			//Элементы.Представляется.Видимость = Ложь;
...
Процедура УдалитьВсеСтраницыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Декларация" Тогда
		ТабличныйДокумент.Области.НомерПП.Значение = ЭтаФорма[мСтруктураМногостраничныхРазделов.Декларация].Количество();
	ИначеЕсли Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Возвраты" Тогда
		ТабличныйДокумент.Области.НомерПП.Значение = ЭтаФорма[мСтруктураМногостраничныхРазделов.Возвраты].Количество();	
	КонецЕсли;
...
Процедура ОбработкаАдресаПриЛистании()
	...
	//Если ЗначениеЗаполнено(ТабличныйДокумент.Области.ВидДок.Значение)
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли НЕ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Титульный"
		...
		Если (Найти(Элемент.ТекущаяОбласть.Имя, "П000000000016_") > 0) Или (Найти(Элемент.ТекущаяОбласть.Имя, "П000010000016_") > 0) Тогда
		...
		ИначеЕсли (Найти(Элемент.ТекущаяОбласть.Имя, "П000000000008_") > 0
			Или Найти(Элемент.ТекущаяОбласть.Имя, "П000010003008_") > 0
			Или Найти(Элемент.ТекущаяОбласть.Имя, "П000010000008_") > 0
			Или Найти(Элемент.ТекущаяОбласть.Имя, "П000010001008_") > 0) И (СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			
			НомерСтроки = Сред(Элемент.ТекущаяОбласть.Имя, 15);
			
			СтруктураГруппы1 = Неопределено;
			
			Если Найти(Элемент.ТекущаяОбласть.Имя, "П000010003008") > 0 Тогда
				Группа = СтруктураРеквизитовФормы.мИдГруппы2;
				Показатель = "П000010003008";
			ИначеЕсли Найти(Элемент.ТекущаяОбласть.Имя, "П000000000008") > 0 Тогда
				Группа = СтруктураРеквизитовФормы.мИдГруппы1;
				Показатель = "П000000000008";
			ИначеЕсли Найти(Элемент.ТекущаяОбласть.Имя, "П000010000008") > 0 Тогда
				Группа = СтруктураРеквизитовФормы.мИдГруппы3;
				Показатель = "П000010000008";
			ИначеЕсли Найти(Элемент.ТекущаяОбласть.Имя, "П000010001008") > 0 Тогда
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Декларация" Тогда
		...
		ИначеЕсли ((Найти(Область.Имя, "П000000000005") > 0) Или (Найти(Область.Имя, "П000010003005") > 0))
			И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли (Найти(Область.Имя, "П000000000008") > 0
			Или Найти(Область.Имя, "П000010003008") > 0) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			СтандартнаяОбработка = Ложь;
			...
			Если Найти(Область.Имя, "П000010003008") > 0 Тогда
				Группа = СтруктураРеквизитовФормы.мИдГруппы2;
				Показатель = "П000010003008";
			Иначе
				Группа = СтруктураРеквизитовФормы.мИдГруппы1;
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Возвраты" Тогда	
		...
		Если ((Найти(Область.Имя, "П000010000002") > 0)
			Или  (Найти(Область.Имя, "П000010000003") > 0))
			И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли ((Найти(Область.Имя, "П000010000005") > 0) Или (Найти(Область.Имя, "П000010001005") > 0))
			И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
		...
		ИначеЕсли ((Найти(Область.Имя, "П000010000008") > 0)
			Или (Найти(Область.Имя, "П000010001008") > 0)) И (НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов) Тогда
			СтандартнаяОбработка = Ложь;
			...
			Если Найти(Область.Имя, "П000010001008") > 0 Тогда
				Группа = СтруктураРеквизитовФормы.мИдГруппы4;
				Показатель = "П000010001008";
			Иначе
				Группа = СтруктураРеквизитовФормы.мИдГруппы3;
					...
					Если ТипПодраздела = "/" Тогда//многострочный подраздел
						...
						Пока Истина Цикл
							Для Каждого СтрФормата Из СтрФорматаПодраздела Цикл
								...
								ИндексЗнакаВИмениЯчейки = Найти(ИмяЯчейки, "_");
...
Функция СформироватьСтруктуруПараметров()
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(СтруктураРеквизитовФормы.Организация) И ПустаяСтрока(СведенияОбОрганизации.ФИОБух) Тогда
		...
		ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ОргНазв);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СхемаВыгрузки401");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Если НЕ ПустаяСтрока(ОргДиректор) Тогда
		...
		Иначе
			ФИОФЛ = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ОргДиректор);
	...
	Если НЕ ПустаяСтрока(ОргБухгалтер) Тогда
		...
		Иначе
			ФИОФЛ = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ОргБухгалтер);
			ПараметрыВыгрузки.Вставить("Фамилия", 	ФИОФЛ.Фамилия);
			ПараметрыВыгрузки.Вставить("Имя", 		ФИОФЛ.Имя);
			ПараметрыВыгрузки.Вставить("Отчество", 	ФИОФЛ.Отчество);
	...
	Если НЕ ПустаяСтрока(ОргУП) Тогда
		...
		Иначе
			ФИОФЛ = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(ОргУП);
			ПараметрыВыгрузки.Вставить("Фамилия", 	ФИОФЛ.Фамилия);
			ПараметрыВыгрузки.Вставить("Имя", 		ФИОФЛ.Имя);
			ПараметрыВыгрузки.Вставить("Отчество", 	ФИОФЛ.Отчество);
		...
		Если ЗначениеЗаполнено(РазделДекларация["П000000000003_" + стрНомСтроки]) Тогда
			...
			Если СтруктураАдреса.Страна = "643" Тогда
				...
				Иначе
					...
					ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(РазделДекларация["П000000000004_" + стрНомСтроки]);
		...
		Если ЗначениеЗаполнено(РазделВозвраты["П000010000003_" + стрНомСтроки]) Тогда
			...
			Если СтруктураАдреса.Страна = "643" Тогда
				...
				Иначе
					...
					ФИОИП = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(РазделВозвраты["П000010000004_" + стрНомСтроки]);
					ПараметрыВыгрузки.Вставить("Фамилия",	ФИОИП.Фамилия);
					ПараметрыВыгрузки.Вставить("Имя",		ФИОИП.Имя);
					ПараметрыВыгрузки.Вставить("Отчество",	ФИОИП.Отчество);
...
Функция ПолучитьСтруктуруДанныхФЛ(СтраницаФЛ)
	...
	ФИОФЛ = РегламентированнаяОтчетностьКлиентСервер.РазложитьФИО(СтраницаФЛ.ФизЛицоФИО);
	ПараметрыВыгрузки.Вставить("Фамилия", 	ФИОФЛ.Фамилия);
	ПараметрыВыгрузки.Вставить("Имя", 		ФИОФЛ.Имя);
	ПараметрыВыгрузки.Вставить("Отчество", 	ФИОФЛ.Отчество);
...
Процедура ЗаполнитьНаОснованииИсточника(Знач ИдГруппаИсточника)
	...
	Для Каждого Стр Из ТаблицаРаздела Цикл
		...
		Если НЕ ((Найти(Данные.Адрес, "643,") = 1) 
			Или (Данные.Адрес = ",,,,,,,,,")
...
Процедура ОчиститьТребуемуюГруппу(Форма, Группа, ВыбТабличноеПоле)
	...
	Для Инд = 0 По ВыбТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если (ИмяГруппыДопСтрок = Форма.СтруктураРеквизитовФормы.мИдГруппы1 И Найти(ТекущаяОбласть.Имя, "П000010003008") > 0)
		 Или (ИмяГруппыДопСтрок = Форма.СтруктураРеквизитовФормы.мИдГруппы2 И Найти(ТекущаяОбласть.Имя, "П000000000008") > 0)
		 Или (ИмяГруппыДопСтрок = Форма.СтруктураРеквизитовФормы.мИдГруппы3 И Найти(ТекущаяОбласть.Имя, "П000010000008") > 0)
		 Или (ИмяГруппыДопСтрок = Форма.СтруктураРеквизитовФормы.мИдГруппы4 И Найти(ТекущаяОбласть.Имя, "П000010001008") > 0) Тогда
			Продолжить;
		КонецЕсли;
