...
Процедура УстановитьДоступностьПолейРаздела1(Форма)
	...
	Если РазрешитьВвод Тогда
		...
		Пока Форма.ТабличныйДокумент.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
			...
			КонецЕсли;
			Если Форма.ТаблицаВариантыЗаполненияРаздел1[0].ТаблицаВариантовЗаполнения.НайтиСтроки(Новый Структура("КодПоказателя", "П000010000121")).Количество() = 0 Тогда
				УстановитьЦветФонаЯчейки(Форма, "П000010000121_" + Постфикс, 0, "Раздел1");
			КонецЕсли;
	...
	Иначе
		...
		Пока Форма.ТабличныйДокумент.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
			...
			УстановитьЦветФонаЯчейки(Форма, "П000010000121_" + Постфикс, 5, "Раздел1");
		...
		УстановитьЦветФонаЯчейки(Форма, "П000010000221", 5, "Раздел1");
...
Процедура ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ИдГруппы, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста)
	...
	ТаблицаДубликатов.Свернуть(ИмяКодаВидаПродукции + ", " + ИмяИННПроизводителя + ", " + ИмяКПППроизводителя, "Счетчик");
	...
	Для Каждого СтрокаКодаВидаПродукции Из КодыВидовПродукции Цикл
		...
		Для Каждого СтрокаСведенийОПроизводителеИмпортере Из СведенияОПроизводителеИмпортере Цикл
			...
			Для Каждого СтрокаДвижения Из СтрокиСведений Цикл
				...
				УстановитьЗначениеЭлемента(НовыйУзелДвижение, "П100000000021", СтрокаДвижения.П000010000121);
...
Функция ПолучитьМакетФормыОтчета(ИмяРаздела)
	...
	Возврат Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)]
	    .ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела);
...
Функция ПолучитьМакетПечатнойФормы(ИмяРаздела)
	...
	Возврат Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)]
	    .ПолучитьМакет("Печатная" + Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяРаздела);
