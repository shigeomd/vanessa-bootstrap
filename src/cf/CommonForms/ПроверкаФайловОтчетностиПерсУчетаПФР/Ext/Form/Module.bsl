...
&НаСервере
Процедура ОпределитьНаличиеПроверочныхПрограммНаСерврере()
	Если ПроверятьПрограммойCheckUFA Тогда	
...
Процедура УстановитьИнфоНадписьНаличиеПрограммНаСервере()
	...
	ОпределитьНаличиеПроверочныхПрограммНаСерврере();
...
&НаКлиенте
Процедура ПослеПроверкиНаличияПрограммНаКлиенте() Экспорт 
	ОбнаруженыВсеПрограммы = (ФайлПрограммыCheckUFAОбнаружен Или Не ПроверятьПрограммойCheckUFA) 
								И (ФайлПрограммыCheckXMLОбнаружен Или Не ПроверятьПрограммойCheckXML);
...
Процедура СохранитьФайлыНаСервере()
	...
	КаталогФайлов = КаталогВременныхФайлов() + "\" + Строка(Новый УникальныйИдентификатор);	
...
Процедура ПослеИнициализацииУдаляемогоКаталога(Файл, ДополнительныеПараметры) Экспорт
	Оповещение = Новый ОписаниеОповещения("ПослеПроверкиСушествованияВременногоКаталога", ЭтотОбъект);
	
	Файл.НачатьПроверкуСуществования(Оповещение);
КонецПроцедуры	

&НаКлиенте
Процедура ПослеПроверкиСушествованияВременногоКаталога(Существует, ДополнительныеПараметы) Экспорт 
