...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ВыборФорматаВложений")
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			ДополнитьПараметрыНовогоПисьмаНаСервере(ПараметрыНовогоПисьма);
			
			КлючеваяОперация = "КлассическаяОтправкаДокументаПоЭлектроннойПочте";
...
Процедура ВычислитьСумму(Команда)
	...
	Если НастройкаПечатнойФормы <> Неопределено Тогда
		...
		ВыделенныеОбласти = СтандартныеПодсистемыКлиент.ВыделенныеОбласти(ТабличныйДокумент);
		СуммаВыделенныхЯчеек = ВычислитьСуммуСервер(ТабличныйДокумент, ВыделенныеОбласти);
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	Элементы.ГруппаСумма.Видимость = ЕстьДанныеДляПечати;
...
Процедура УстановитьТекущуюСтраницу()
	...
	Элементы.ГруппаСумма.Доступность = ПечатнаяФормаДоступна;
	РассчитатьСуммуЯчеек();
...
Процедура Подключаемый_РассчитатьСуммуЯчеек()
	...
	ТабличныйДокумент = ЭтотОбъект[НастройкаПечатнойФормы.ИмяРеквизита];
	СуммаВыделенныхЯчеек = СтандартныеПодсистемыКлиентСервер.СуммаЯчеек(ТабличныйДокумент, Неопределено);
	Элементы.ВычислитьСумму.Доступность = (СуммаВыделенныхЯчеек = "<");
КонецПроцедуры

&НаСервереБезКонтекста
Функция ВычислитьСуммуСервер(Знач ТабличныйДокумент, Знач ВыделенныеОбласти)
	Возврат СтандартныеПодсистемыКлиентСервер.СуммаЯчеек(ТабличныйДокумент, ВыделенныеОбласти);
КонецФункции

#КонецОбласти

&НаСервере
Процедура ДополнитьПараметрыНовогоПисьмаНаСервере(ПараметрыНовогоПисьма)
	
	ТаблицаПечатныхФорм = НастройкиПечатныхФорм.Выгрузить();
	ТаблицаПечатныхФорм.Колонки.Добавить("ТабличныйДокумент");
	
	Для Каждого СтрокаТаблицыПечатныхФорм Из ТаблицаПечатныхФорм Цикл
		СтрокаТаблицыПечатныхФорм.ТабличныйДокумент = ЭтаФорма[СтрокаТаблицыПечатныхФорм.ИмяРеквизита];
	КонецЦикла;
	
	УправлениеПечатьюПереопределяемый.ДополнитьПараметрыНовогоПисьма(ПараметрыНовогоПисьма,
		ОбъектыПечати,
		ПараметрыВывода,
		ТаблицаПечатныхФорм);
