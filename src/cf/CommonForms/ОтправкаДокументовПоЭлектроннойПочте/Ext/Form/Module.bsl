...
Процедура Выбрать(Команда)
	...
	Если ТекущаяСтраница = "СтраницаВыборФормата" Тогда
...
Процедура Отмена(Команда)
	...
	Если ТекущаяСтраница = "СтраницаВыборФормата" И НЕ СценарийОтправкиОднойПечатнойФормы Тогда
		ТекущаяСтраница = "СтраницаВыборПечатнойФормы";
		УправлениеФормой(ЭтотОбъект);
	Иначе
		Закрыть();
...
Процедура ОтправитьПоЭлектроннойПочтеНастройкаУчетнойЗаписиПредложена(УчетнаяЗаписьНастроена, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеКоманды.Количество() > 0 Тогда
		...
		ФорматыСохраненияПечатныхФорм = ОбщегоНазначенияКлиентСервер.ПолучитьМассивОтмеченныхЭлементовСписка(
...
Процедура УправлениеФормой(Форма)
	...
	Если Элементы.Страницы.ТекущаяСтраница.Имя <> Форма.ТекущаяСтраница Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы[Форма.ТекущаяСтраница];
	КонецЕсли;
