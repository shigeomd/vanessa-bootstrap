...
Процедура ВариантНажатие(Элемент)
	...
	Соседи = ДобавленныеВарианты.НайтиСтроки(Новый Структура("Подсистема", Вариант.Подсистема));
	Если Соседи.Количество() > 1 Тогда
...
Процедура ЗаголовокРазделаНажатие(Элемент)
	...
	Подстроки = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ГруппаРазделаИмя, "_");
	...
	ФормаПараметры.Вставить("ПутьКПодсистеме",      СтрЗаменить(Раздел.ПолноеИмя, "Подсистема.", ""));
...
Процедура ЗагрузитьВсеНастройки()
	...
	// СПоказатьОповещениеОПодсказках, ПоказыватьПодсказки, ИскатьВоВсехРазделах".
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ОбщиеНастройки);
