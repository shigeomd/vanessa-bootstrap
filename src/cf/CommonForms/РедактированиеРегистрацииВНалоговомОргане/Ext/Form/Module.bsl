...
&НаКлиенте
Процедура ИсторияРегистрацийВНалоговомОрганеПериодАвтоПодбор(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ИсторияРегистрацийВНалоговомОрганеПериодОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ПрочитатьРегистрациюВНалоговомОргане()
	...
	Если НЕ ЗначениеЗаполнено(ИсторияРегистрацийВНалоговомОргане.РегистрацияВНалоговомОргане) Тогда
...
&НаСервере
Процедура ПоместитьДанныеРегистрацииВФорму(РегистрацияВНалоговомОрганеСсылка)
	...
	Если РегистрацияВНалоговомОрганеСсылка.Пустая() Тогда
		...
		Если ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.Организации") Тогда
			ВладелецРегистрацииВНалоговомОргане = СтруктурнаяЕдиница;
		Иначе
			ВладелецРегистрацииВНалоговомОргане = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница, "Владелец");
		КонецЕсли;
...
Процедура ЗаписатьРегистрациюВНалоговомОрганеНаСервере(Отказ)
	...
	Если ИсторияРегистрацийВНалоговомОрганеНоваяЗапись Тогда
		
		СуществующаяЗапись = РегламентированнаяОтчетность.ПолучитьПоКодамРегистрациюВИФНС(РегистрацияВНалоговомОргане.Владелец, РегистрацияВНалоговомОргане.Код, РегистрацияВНалоговомОргане.КПП);
...
Функция КлючевыеРеквизитыРегистрацииВНалоговомОргане()
	...
	Возврат "КПП,НаименованиеИФНС,КодПоОКАТО,НаименованиеОбособленногоПодразделения,Представитель,УполномоченноеЛицоПредставителя,ДокументПредставителя,Доверенность,Код,Наименование";
...
Процедура ОбновитьИсторияРегистрацийВНалоговомОрганеПериод(Форма, ДатаСеанса)
	...
	Иначе
		Если Форма.Модифицированность
			И Форма.ИсторияРегистрацийВНалоговомОргане.Период = НачалоМесяца(ЗарплатаКадрыКлиентСервер.ДатаОтсчетаПериодическихСведенийСПериодомМесяц()) Тогда
