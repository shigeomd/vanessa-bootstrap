...
#Область ОбработчикиСобытийТаблицыФормыСписокфайлов
...
Процедура ПрочитатьФайлыИзКаталогаПослеПоискаФайлов(НайденныеФайлы, ПараметрыЗагрузки) Экспорт
	...
	Для Каждого ОписаниеФайла Из НайденныеФайлы Цикл
		Если Найти(ОписаниеФайла.Имя, МаскаИмениФайлов) > 0 Тогда
	...
	КаталогСФайламиXML = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(КаталогСФайлами);
...
&НаКлиенте
Процедура ПрочитатьФайлыИзКаталогаЗавершение(ПомещенныеФайлы) Экспорт
	
	// Действия, выполняемые по завершению чтения файлов из каталога.
	
	КоличествоНайденныхФайлов = ОбновитьСписокФайловНаСервере(ПомещенныеФайлы, Истина);
...
Функция ЭтоXMLФайлВФорматеФНС(ДеревоXML)
	...
	Если ДеревоXML.Свойства().Получить("ВерсФорм") <> Неопределено И (ДеревоXML.ВерсФорм = "5.02" Или ДеревоXML.ВерсФорм = "5.03") Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ДобавитьВСписокФайлСправокОДоходах(ТипФайлаСведений, ПомещенныйФайл, ПомещенныеФайлы, ДеревоXML)
	...
	ИНН = ДеревоXML.СвРекв.СвЮЛ.ИННЮЛ;
	КПП = ДеревоXML.СвРекв.СвЮЛ.КПП;
	НоваяЗапись.Период					= Дата(ДеревоXML.СвРекв.ОтчетГод, 1, 1);
	НоваяЗапись.ПериодСтрокой			= ДеревоXML.СвРекв.ОтчетГод + НСтр("ru=' год'");
	...
	КонецЕсли;
	СписокСправок2НДФЛ = СписокСправок2НДФЛ(ДеревоXML);
	Если СписокСправок2НДФЛ.Количество() > 0 Тогда
		ПерваяСправка = СписокСправок2НДФЛ[0];
		ДатаЗаполненияГод = Прав(ПерваяСправка.ДатаДок, 4);
		ДатаЗаполненияМесяц = Сред(ПерваяСправка.ДатаДок, 4, 2);
		ДатаЗаполненияДень = Лев(ПерваяСправка.ДатаДок, 2);
...
&НаСервере
Функция СписокСправок2НДФЛ(ДеревоXML)
...
Функция ЭтоДокументРСВ(ДеревоXML)
	...
	Если ДеревоXML.Свойства().Получить("РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014") <> Неопределено Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ДобавитьВСписокФайлРСВ(ТипФайлаСведений, ПомещенныйФайл, ПомещенныеФайлы, ДеревоXML)
	...
	Если ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.РегистрационныйНомерПФР = "0" Тогда
		Возврат;
	КонецЕсли; 
	...
	// Определим страхователя
	НоваяЗапись.РегистрационныйНомер = ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.РегистрационныйНомерПФР;
	НоваяЗапись.НаименованиеКраткое = ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.НаименованиеОрганизации; 
	НоваяЗапись.ЭтоРСВ_1 = Истина;
	
	НомерКорректировкиСтрокой = ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.НомерКорректировки;
	...
	КонецЕсли;
	// Определим период отчетности.
	КодПериодаСтрокой = ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.КодОтчетногоПериода;
	ГодСтрокой = ПачкаДокументов.РАСЧЕТ_ПО_СТРАХОВЫМ_ВЗНОСАМ_НА_ОПС_И_ОМС_ПЛАТЕЛЬЩИКАМИ_ПРОИЗВОДЯЩИМИ_ВЫПЛАТЫ_ФЛ_2014.КалендарныйГод;
...
Функция ЗаполнитьСписокФайловПачек(ЗаписьСпискаФайлов, ПачкаДокументов, ПомещенныеФайлы)
	...
	Если СписокИменФайловВПачке.Количество() > 0 Тогда
		...
		Иначе
			...
			ПозицияПредупреждения = Найти(ЗаписьСпискаФайлов.Комментарий, НСтр("ru='Отсутствуют необходимые файлы пачек:'"));
