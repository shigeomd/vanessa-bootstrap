...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СуществуетСправка Тогда
		...
		Для Каждого Справка Из СписокСправок Цикл 
			...
			Если Не КнопкаСправки = Неопределено тогда
	...
	НастройкиПредупреждений = ОбщегоНазначенияБП.НастройкиПредупрежденийОбИзменениях(
		"ПоказатьПредупреждение_ОкруглениеНалогов");
	ПоказатьПредупрежденийОбОкругленииПрибыли = Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаПрибыль
	И Объект.Дата >= Дата('20140101')
		И УчетнаяПолитика.СистемаНалогообложения(Объект.Организация, Объект.Дата) = Перечисления.СистемыНалогообложения.Общая
		И Объект.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
