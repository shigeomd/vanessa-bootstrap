...
Процедура ДобавитьКолонкуСодержаниеСписаниеСпецодеждыСпецоснастки(ТаблицаЗначений) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		СтрокаТаблицы.Содержание = "Списание " + БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьНазваниеОбъекта(СтрокаТаблицы.СчетПередачи);
	КонецЦикла;
...
Функция ПечатьМБ8(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СписаниеМатериаловИзЭксплуатации_МБ8";
	...
	Пока ДокументыДляПечати.СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		Пока ДокументыДляПечати.Следующий() Цикл
			...
			Если НЕ ОбщегоНазначенияБПВызовСервера.ПроверитьВыводТабличногоДокумента(ТабличныйДокумент, СтрокаТаблицы) Тогда
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыМБ8()
	...
	ТекстЗапроса = "
		...
		|ВЫБРАТЬ
		|	ДокументСписаниеМатериаловИзЭксплуатацииСпецодежда.Ссылка КАК Регистратор,
		...
		|	ПРАВОЕ СОЕДИНЕНИЕ Документ.СписаниеМатериаловИзЭксплуатации КАК ДокументСписаниеМатериаловИзЭксплуатацииСпецодежда
		|	ПО СписаниеМатериаловИзЭксплуатацииСпецодежда.Ссылка = ДокументСписаниеМатериаловИзЭксплуатацииСпецодежда.Ссылка
		|ГДЕ
		|	ДокументСписаниеМатериаловИзЭксплуатацииСпецодежда.Ссылка В (&МассивОбъектов)
		|СГРУППИРОВАТЬ ПО
		|	ДокументСписаниеМатериаловИзЭксплуатацииСпецодежда.Ссылка,
		...
		|	ЕСТЬNULL(ВТ_ВсеТЧ_ВсеСтоимости.ПартияМатериаловВЭксплуатации.Дата, НЕОПРЕДЕЛЕНО) КАК ДатаВводаВЭксплуатацию,
		...
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Номер                           КАК Номер,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Дата                            КАК ДатаСоставления,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Дата                            КАК ДатаДокумента,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Организация                     КАК Организация,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Организация                     КАК ЮрФизЛицо,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.Организация                     КАК Руководители,
		|	ВТ_ВсеДанныеДляПечати.Регистратор.ПодразделениеОрганизации        КАК Подразделение,
		|	ВЫБОР
		|		КОГДА ВТ_ВсеДанныеДляПечати.Регистратор.СпособСписанияРасходов = ЗНАЧЕНИЕ(Перечисление.СпособыСписанияРасходов.ВДебетСчетаУказанногоВДокументе)
		|			ТОГДА ВТ_ВсеДанныеДляПечати.Регистратор.СчетДт.Код
		|		ИНАЧЕ """"
		|	КОНЕЦ                                                             КАК Счет,
