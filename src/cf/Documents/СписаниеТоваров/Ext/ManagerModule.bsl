...
Функция ПечатьСписаниеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Если ШапкаИтоги.Следующий() Тогда
		...
		Пока Шапка.Следующий() Цикл
			...
			ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(Шапка, "Списание товаров");
			...
			Пока ВыборкаСтрокТовары.Следующий() Цикл
				...
				Если УчетПоПродажнойСтоимости 
					...
					Сумма        = ЦенаСписания * ВыборкаСтрокТовары.Количество;
				...
				ОбластьДанных.Параметры.Цена  = ЦенаСписания;
			...
			Пока ВыборкаСтрокТара.Следующий() Цикл
				...
				Иначе
					...
					Иначе
						...
						ЦенаСписания = ?(СтрокаСуммСписания.Количество = 0, 0, СтрокаСуммСписания.Сумма / СтрокаСуммСписания.Количество);
						Сумма        = ЦенаСписания * ВыборкаСтрокТара.Количество;
					КонецЕсли;
			...
			НайденныеСтроки = ТаблицаСуммСписанияПоДокументам.НайтиСтроки(Новый Структура("Регистратор", Шапка.Ссылка));
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				СуммаСписанияВсего = СуммаСписанияВсего + НайденнаяСтрока.Сумма;
			КонецЦикла;
			
			ОбластьДанных.Параметры.Всего = ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаСписанияВсего);
			ТабличныйДокумент.Присоединить(ОбластьДанных);
			...
			ОбластьМакета.Параметры.ИтоговаяСтрока ="Всего наименований " + (ВыборкаСтрокТовары.Количество() + ВыборкаСтрокТара.Количество())
			                                         + ", на сумму " + ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаСписанияВсего, Шапка.ВалютаДокумента);

			ОбластьМакета.Параметры.СуммаПрописью  = ОбщегоНазначенияБПВызовСервера.СформироватьСуммуПрописью(СуммаСписанияВсего, Шапка.ВалютаДокумента);
...
Функция ПолучитьДополнительныеРеквизитыДляРеестра() Экспорт
	...
	Результат = Новый Структура("Информация, ВалютаДокумента, СуммаДокумента", "Склад", "Неопределено", "");
