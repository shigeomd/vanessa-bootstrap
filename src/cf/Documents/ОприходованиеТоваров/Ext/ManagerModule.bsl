...
Функция ТекстЗапросаКнигаУчетаДоходовИРасходовУСН(НомераТаблиц)
	...
	|	""Оприходованы излишки ТМЦ"" КАК Содержание,
	|	Реквизиты.ИнвентаризацияТоваровНаСкладе КАК ИнвентаризацияТоваровНаСкладе
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
...
Процедура ДобавитьКолонкуСодержаниеУСН(ТаблицаЗначений) Экспорт
	...
	Если ТаблицаЗначений.Колонки.Найти("СодержаниеУСН") = Неопределено Тогда
		ТаблицаЗначений.Колонки.Добавить("СодержаниеУСН", ОбщегоНазначения.ОписаниеТипаСтрока(150));
	КонецЕсли;

	Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
		
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ИнвентаризацияТоваровНаСкладе) Тогда
			СтрокаТаблицы.СодержаниеУСН = "Оприходованы излишки ТМЦ.";
		Иначе
			Содержание = "Оприходованы излишки, выявленные при проведении инвентаризации %1.";
			Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Содержание,
				НалоговыйУчетУСН.РеквизитыПервичногоДокумента(СтрокаТаблицы.ИнвентаризацияТоваровНаСкладе));
			СтрокаТаблицы.СодержаниеУСН = Содержание;
		КонецЕсли;
