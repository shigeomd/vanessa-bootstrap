...
Функция ПечатьИНВ1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|ПОМЕСТИТЬ ОСИСведенияПредварительная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОСИСведенияПредварительная.Ссылка КАК Ссылка,
	|	ОСИСведенияПредварительная.Дата,
	|	ОСИСведенияПредварительная.НомерСтроки,
	|	ОСИСведенияПредварительная.ОсновноеСредство,
	|	ОСИСведенияПредварительная.Организация,
	|	ОСИСведенияПредварительная.СтоимостьПоДаннымУчета,
	|	ОСИСведенияПредварительная.СтоимостьФактическая,
	|	ОСИСведенияПредварительная.НаличиеПоДаннымУчета,
	|	ОСИСведенияПредварительная.НаличиеФактическое,
	|	ОСИСведенияПредварительная.МаксПериодМеста,
	|	ОСИСведенияПредварительная.МаксПериодБУ,
	|	ПервоначальныеСведенияОСБухгалтерскийУчет.ИнвентарныйНомер КАК ИнвентарныйНомер,
	|	ОСИСведенияПредварительная.МОЛ
	|ПОМЕСТИТЬ ОСИСведения
	|ИЗ
	|	ОСИСведенияПредварительная КАК ОСИСведенияПредварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет КАК ПервоначальныеСведенияОСБухгалтерскийУчет
	|		ПО ОСИСведенияПредварительная.ОсновноеСредство = ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство
	|			И ОСИСведенияПредварительная.Организация = ПервоначальныеСведенияОСБухгалтерскийУчет.Организация
	|			И ОСИСведенияПредварительная.МаксПериодБУ = ПервоначальныеСведенияОСБухгалтерскийУчет.Период
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ОСИСведенияПредварительная
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	КоличествоОСДокументе.КоличествоОС КАК КоличествоОС,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_КоличествоОСВДокументеИнвентаризации";
	...
	Пока Док.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Шапка.Параметры.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Док.Номер, Истина, Ложь);
		...
		Если ПредседательКомиссии <> Неопределено И ПредседательКомиссии.Председатель Тогда
			...
			ДанныеПредседателя = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Док.Организация, ПредседательКомиссии.ФизЛицо, Док.Дата);
		...
		Для каждого ЧленКомиссии Из ТаблицаИнвентаризационнаяКомиссия Цикл
			...
			ДанныеЧленаКомиссии = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Док.Организация, ЧленКомиссии.ФизЛицо, Док.Дата);
