...
Процедура ПроверитьЗаполнениеПериодовРаботы(Отказ) 
	...
	Для каждого ПериодРаботы Из ПериодыРаботы Цикл
		...
		Для каждого ПовторяющаясяСтрока Из ПериодыРаботы Цикл
			...
			Если  ПериодРаботы.НомерСтроки >=  ПовторяющаясяСтрока.НомерСтроки Или  
				((ПериодРаботы.ПериодРаботыС > ПовторяющаясяСтрока.ПериодРаботыПо) Или
				(ПериодРаботы.ПериодРаботыПо < ПовторяющаясяСтрока.ПериодРаботыС)) Тогда
...
Процедура ПроверитьЗаполнениеДнейОтсутствия(Отказ) 
	...
	Для каждого ПериодОтсутствия Из ДниБолезниУходаЗаДетьми Цикл
		...
		Для каждого ПовторяющаясяСтрока Из ДниБолезниУходаЗаДетьми Цикл
			Если ПериодОтсутствия.НомерСтроки >=  ПовторяющаясяСтрока.НомерСтроки Или 
				((ПериодОтсутствия.ПериодС > ПовторяющаясяСтрока.ПериодПо) Или 
				(ПериодОтсутствия.ПериодПо < ПовторяющаясяСтрока.ПериодС)) Тогда 
				// Периоды не пересекаются
				Продолжить;
			КонецЕсли;	
