...
Процедура ЗаполнитьДаннымиВсего(Параметры, ДеревоВыгрузки)
	...
	УзелСвКнПок = ПолучитьПодчиненныйЭлемент(УзелДокумент, "СвКнПок");
	УзелВсего = ПолучитьПодчиненныйЭлемент(УзелСвКнПок, "Всего");
	...
	Если СуммаБезНДС18 <> 0 ИЛИ НДС18 <> 0 Тогда
		НовыйУзелВтЧисле = ПродублироватьУзелОбразец(ОбразецУзлаВтЧисле);
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(Параметры, ДеревоВыгрузки)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		НовыйУзелСвПокупка = ПродублироватьУзелОбразец(ОбразецУзелСвПокупка);
		...
		УстановитьЗначениеЭлемента(НовыйУзелСвПокупка, "НомерСчФ", СокрЛП(РеквизитыСчетаФактуры[1]));
		...
		Для Каждого СтрокаДатыОплаты Из ДатыОплаты Цикл
			Если ЗначениеЗаполнено(СтрокаДатыОплаты) Тогда
				НовыйУзелДатаОплСчФПрод = ПродублироватьУзелОбразец(ОбразецУзлаДатаОплСчФПрод);
		...
		Если СтрокаТабличнойЧасти.СуммаБезНДС18 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС18 <> 0 Тогда
			НовыйУзелВтчСтоимПок = ПродублироватьУзелОбразец(ОбразецУзлаВтчСтоимПок);
		...
		ОбразецУзлаНомерТД = ПолучитьПодчиненныйЭлемент(НовыйУзелСвПокупка, "НомерТД");
		...
		Если РеквизитыПроисхожденияТовара.Количество() >= 2 Тогда
			УстановитьЗначениеЭлемента(НовыйУзелСвПокупка, "КодПроисх", Лев(СокрЛП(РеквизитыПроисхожденияТовара[0]), 3));
			Для Инд = 1 По РеквизитыПроисхожденияТовара.Количество() - 1 Цикл
				НовыйУзелНомерТД = ПродублироватьУзелОбразец(ОбразецУзлаНомерТД);
...
Функция ПродублироватьУзелОбразец(УзелОбразец);
