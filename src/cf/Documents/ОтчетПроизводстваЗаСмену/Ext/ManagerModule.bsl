...
Процедура ЗаполнитьМатериалыПоПродукцииУслугам(Материалы, Продукция, Услуги, Организация, Склад) Экспорт
	...
	ПредопределеннаяСтатьяЗатрат = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтатьиЗатрат.СписаниеМатериалов");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаМатериалов Цикл
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.СтатьяЗатрат) Тогда
			НоваяСтрока.СтатьяЗатрат = ПредопределеннаяСтатьяЗатрат;
		КонецЕсли;		
...
Функция ТекстЗапросаПлановаяСтоимостьУслуг(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаУслуги.СуммаПлановая КАК СуммаПлановаяНУ,
...
Функция ТекстЗапросаВыпускУслуг(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаУслуги.СуммаПлановая <> 0
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПрямыеРасходыРаспределятьПоКоличеству
	|ИЗ
	|	ТаблицаУслуги КАК ТаблицаУслуги
	|		ЛЕВОЕ СОЕДИНЕНИЕ Реквизиты КАК Реквизиты
	|		ПО (ИСТИНА)
...
Функция ПодготовитьТаблицыМатериаловПродукцииИП(ТаблицаСписанныеМПЗ, ВыпускПродукцииТаблица, ВыпускУслугТаблица, ВыпускОтходовТаблица, ТаблицаРеквизиты) Экспорт
	...
	Если ВыпускУслугТаблица <> Неопределено Тогда
		Для Каждого Услуга Из ВыпускУслугТаблица Цикл
			...
			Если Услуга.ПлановаяСтоимость = 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			НоваяСтрока.ПлановаяСтоимость		= Услуга.ПлановаяСтоимость;
	...
	Если ВыпускОтходовТаблица <> Неопределено Тогда
		Для Каждого Отход Из ВыпускОтходовТаблица Цикл
			...
			НоваяСтрока.ПлановаяСтоимость		= Отход.Сумма;
	...
	ОтборПродукции	= Новый Структура("НоменклатурнаяГруппа");
	...
	Для Каждого Материал Из ВремТаблицаСписанныеМПЗ Цикл
		...
		Если ЗначениеЗаполнено(Материал.НоменклатурнаяГруппа) Тогда
			...
			ЗаполнитьЗначенияСвойств(ОтборПродукции, Материал);
			
			СтрокиПродукции = ПолученоПродукции.НайтиСтроки(ОтборПродукции);
			Для каждого Продукция Из СтрокиПродукции Цикл
				МассивКоэффициентов.Добавить(Продукция.ПлановаяСтоимость);
			КонецЦикла;
...
Функция ПечатьМ11(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Объект ИЗ МассивОбъектов Цикл
		...
		Область.Параметры.Заголовок                = "ТРЕБОВАНИЕ-НАКЛАДНАЯ № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Объект.Номер, Истина, Ложь);
		...
		Если ЗначениеЗаполнено(Объект.Склад) Тогда 
			...
			Область.Параметры.КладовщикДолжность = ДанныеФизЛица.Должность;
			Область.Параметры.КладовщикФИО = ДанныеФизЛица.Представление;
		КонецЕсли;
		ТабДокумент.Вывести(Область);
...
Функция ПечатьУслугиСобственнымПодразделениям(МассивОбъектов, ОбъектыПечати)
	...
	|	ДанныеДокументов.Номенклатура.Представление КАК НоменклатураПредставление,
	...
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.АвтоМасштаб = Истина;
	ТабДокумент.ИмяПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_ОтчетПроизводстваЗаСмену_УслугиСобственнымПодразделениям";
	ПервыйДокумент = Истина;
