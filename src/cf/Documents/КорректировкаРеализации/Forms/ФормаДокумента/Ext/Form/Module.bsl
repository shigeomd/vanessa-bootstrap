...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|НомерГТД, СтранаПроисхождения, СчетДоходовВедетсяУчетПоНоменклатурнымГруппам,
...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументРеализации) ИЛИ Объект.ДокументРеализации = Объект.ИсправляемыйДокументРеализации Тогда
		Элементы.ГруппаИсправляемыйДокументРеализации.ТекущаяСтраница = Элементы.ГруппаТотЖеИсправляемыйДокументРеализации;
	Иначе
		Элементы.ГруппаИсправляемыйДокументРеализации.ТекущаяСтраница = Элементы.ГруппаДругойИсправляемыйДокументРеализации;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ДокументРеализации) 
		...
		Элементы.ГруппаОтраженияДоходовИРасходовПоПериодам.ТекущаяСтраница = Элементы.ГруппаПрошлыйГод;
	Иначе
		Элементы.ГруппаОтраженияДоходовИРасходовПоПериодам.ТекущаяСтраница = Элементы.ГруппаТекущийГод;
	КонецЕсли;
...
Функция ДокументРеализацииОбработкаВыбораСервер(Знач Документ, Знач ВыбранноеЗначение, Знач ВидОперации, Знач ВалютаРегламентированногоУчета)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") И
		...
		Если НЕ Документы.КорректировкаРеализации.ОтчетКомиссионераЗаполненПоСтарому(ВыбранноеЗначение) Тогда
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		...
		Если ЭтоОтчетКомиссионера Тогда
			...
			ОтчетКомиссионераЗаполненПоНовому = ЗначениеЗаполнено(ДокументОснование) И ЭтоОтчетКомиссионера
				И НЕ Документы.КорректировкаРеализации.ОтчетКомиссионераЗаполненПоСтарому(ДокументОснование);
...
Процедура АгентскиеУслугиКонтрагентПриИзмененииСервер(СтрокаТабличнойЧасти, Знач ДанныеОбъекта, Знач СчетаУчетаКЗаполнению)
	...
	БухгалтерскийУчетПереопределяемый.УстановитьДоговорКонтрагента(СтрокаТабличнойЧасти.ДоговорКонтрагента,
