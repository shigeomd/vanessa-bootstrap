...
Функция ПолучитьПервичныйДокументРеализации(ТекущийДокументРеализации)
	...
	Пока ТипЗнч(ПервичныйДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Цикл
...
Функция ПолучитьПредставлениеДокумента(ДокументСсылка) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументСсылка) Тогда
		...
		РеквизитыДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДокументСсылка, "ПредставлениеНомера, Дата");
	...
	Если НЕ ЗначениеЗаполнено(ВидОперации)  
		...
		Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда 
			...
			СтруктураПараметров.Вставить("НомерИсходногоДокумента", 			
				ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыИсправляемогоДокумента.Номер, Истина, Ложь));
		...
		Иначе
			...
			СтруктураПараметров.Вставить("НомерИсходногоДокумента", 	
				ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыДокументаРеализации.Номер, Истина, Ложь));
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Если Реквизиты.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку
		...
		Если ТипЗнч(РеквизитыСФ.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах")
			...
			И РеквизитыСФ.Продавец <> РеквизитыСФ.Контрагент Тогда
	...
	Если Реквизиты.РасчетыВВалюте или Реквизиты.РасчетыВУсловныхЕдиницах Тогда
		...
		Иначе
			...
			Если Найти(ИмяДокументаРеализации, "СчетФактура") = 0 или (Найти(ИмяДокументаРеализации, "ДокументРасчетовСКонтрагентом")) Тогда
	...
	ДеятельностьНаПатенте = ТипЗнч(ДокументРеализацииСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализацииСсылка, "ДеятельностьНаПатенте");
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	Реквизиты.ИсправляемыйДокументРеализации,
	|	Реквизиты.СуммаВключаетНДС,
...
КонецФункции // ТекстЗапросаКорректировкаРеализацииУСН()
...
Функция ПодготовитьТаблицуВыделениеАвансов(ТаблицаВзаиморасчетыВозврат, ТаблицаРеквизиты, ТаблицаПоДокументуУменьшение, ТаблицаПоДокументуУвеличение) Экспорт
...
Функция ПодготовитьТаблицуСписанныеТоварыПоПериодам(ТаблицаСписанныеТовары, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если Параметры.ТаблицаСписанныеТовары.Количество() = 0 Тогда
...
Функция ПодготовитьСтруктуруВыручкиПоВидамДеятельности(ТаблицаСобственныеТоварыУслуги, ТаблицаТоварыУслугиКомитентов, ТаблицаРеквизиты) Экспорт
	...
	СтруктураВыручки = Новый Структура("Всего, ЕНВД, Комиссия, Патент", ВыручкаВсего, ВыручкаЕНВД, ВыручкаКомиссии, ВыручкаПатент);
	
	Параметры = ПодготовитьПараметрыВыручкаПоВидамДеятельности(ТаблицаСобственныеТоварыУслуги, ТаблицаТоварыУслугиКомитентов, ТаблицаРеквизиты);
	
	Если Параметры.Реквизиты.Количество() = 0
		Или (Параметры.ТаблицаСобственныеТоварыУслуги.Количество() = 0 И Параметры.ТаблицаТоварыУслугиКомитентов.Количество() = 0) Тогда
		Возврат СтруктураВыручки;
	КонецЕсли; 
	
	Реквизиты = Параметры.Реквизиты[0];
	
	// Собственные товары и услуги - разделим на ЕНВД и основной вид деятельности
	Для каждого СтрокаСобственные Из ТаблицаСобственныеТоварыУслуги Цикл		
		Если БухгалтерскийУчетВызовСервераПовтИсп.СчетОтноситсяКДеятельностиЕНВД(СтрокаСобственные.СчетДоходов) Тогда
			ВыручкаЕНВД = ВыручкаЕНВД + СтрокаСобственные.СуммаВзаиморасчетов;		
		КонецЕсли; 
	КонецЦикла;
	ВыручкаВсего = ВыручкаВсего + ТаблицаСобственныеТоварыУслуги.Итог("СуммаВзаиморасчетов");
	
	// Товары, услуги комитента
	ВыручкаКомиссии = ТаблицаТоварыУслугиКомитентов.Итог("СуммаВзаиморасчетов");
	ВыручкаВсего    = ВыручкаВсего + ВыручкаКомиссии;
	...
	СтруктураВыручки.ЕНВД     = ВыручкаЕНВД;
	СтруктураВыручки.Комиссия = ВыручкаКомиссии;
	СтруктураВыручки.Патент   = ВыручкаПатент;
	
	Возврат СтруктураВыручки;

КонецФункции // ВыручкаПоВидамДеятельности()

Функция ПодготовитьПараметрыВыручкаПоВидамДеятельности(ТаблицаСобственныеТоварыУслуги, ТаблицаТоварыУслугиКомитентов, ТаблицаРеквизиты)

	Параметры = Новый Структура;
	
	// Подготовка таблицы шапки документа
	СписокОбязательныхКолонок = ""
	+ "ДеятельностьНаПатенте"; // <Булево> - признак отнесения корректируемого документа реализации к деятельности на патенте
	...
	// Подготовка таблицы реализованных товаров и услуг комитента:
	СписокОбязательныхКолонок = ""
	+ "СуммаВзаиморасчетов"
	;	
...
КонецФункции // ПодготовитьПараметрыВыручкаПоВидамДеятельности()
...
Функция ПодготовитьТаблицуВозвращенныеТоварыУСН(ТаблицаТоварыВозврат, ТаблицаРеквизиты) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		Или Параметры.ТаблицаТоварыВозврат.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли; 
	
	Возврат Параметры.ТаблицаТоварыВозврат;

КонецФункции // ПодготовитьТаблицуВозвращенныеТоварыУСН()
...
КонецФункции // ПодготовитьПараметрыТаблицыВозвращенныеТоварыУСН()
...
КонецФункции // ПодготовитьТаблицуРасходыУСННаВозвратПоСделке()
...
КонецФункции // ПодготовитьПараметрыРасходыУСННаВозвратПоСделке()
...
Функция ПодготовитьТаблицуДляПроведенияПоРублевымСуммам(Реквизиты ,ТаблицаСобственныеТоварыУслуги, ТаблицаТоварыУслугиКомитентов, ТаблицаСторноСобственныеТоварыУслуги, ТаблицаСторноТоварыУслугиКомитентов, РублевыеСуммыДокументовВВалюте) Экспорт
...
Процедура ЗаполнитьПоДокументу(Объект) Экспорт
	...
	Если ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		ИЛИ ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		
		ЗаполнитьПоРеализации(Объект);
		
	ИначеЕсли ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.АктОбОказанииПроизводственныхУслуг") Тогда
		
		ЗаполнитьПоАктуОказанияПроизводственныхУслуг(Объект);
		
	ИначеЕсли ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		
		Если ЗначениеЗаполнено(Объект.ДокументРеализации) 
			И ОтчетКомиссионераЗаполненПоСтарому(Объект.ДокументРеализации) Тогда
			
			ЗаполнитьПоОтчетуКомиссионера(Объект);
...
Процедура ЗаполнитьПоРеализации(Объект)
	...
	ИмяВидаДокумента = Объект.ДокументРеализации.Метаданные().Имя;
...
Процедура ЗаполнитьПоАктуОказанияПроизводственныхУслуг(Объект)
	Объект.Товары.Очистить();
	Объект.Услуги.Очистить();
	Объект.АгентскиеУслуги.Очистить();
...
Процедура ЗаполнитьПоОтчетуКомиссионера(Объект)
	Объект.Товары.Очистить();
	Объект.Услуги.Очистить();
	Объект.АгентскиеУслуги.Очистить();
...
Функция ОтчетКомиссионераЗаполненПоСтарому(ОтчетКомиссионера) Экспорт
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Акт") Тогда
		...
		СтруктураПараметровПечати.Вставить("ИмяПараметровПечати", "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаРеализации_Акт");
		СтруктураПараметровПечати.Вставить("ИмяМакетаПечати",     "Документ.КорректировкаРеализации.ПФ_MXL_Акт");
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТОРГ12") Тогда
		...
		ИмяПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаРеализации_ТОРГ12";
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СоглашениеОбИзмененииСтоимости") Тогда
		...
		ИмяПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаРеализации_СоглашениеОбИзмененииСтоимости";
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийТОРГ12()
	...
	|	КорректировкаРеализации.ЗаРуководителяПоПриказу КАК ЗаРуководителяПоПриказу,
	|	КорректировкаРеализации.ЗаГлавногоБухгалтераПоПриказу КАК ЗаГлавногоБухгалтераПоПриказу,
