		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			...
			Если ЗначениеЗаполнено(ТекСтрокаТовары.СчетУчета)
				И (ТекСтрокаТовары.СчетУчета = Счет10_09 
					ИЛИ БухгалтерскийУчетВызовСервераПовтИсп.СчетВИерархии(ТекСтрокаТовары.СчетУчета, Счет10_09)) Тогда
			...
			ИначеЕсли СчетаУчета<> Неопределено
				...
				И БухгалтерскийУчетВызовСервераПовтИсп.СчетВИерархии(СчетаУчета.СчетПередачи, Счет10_11_2) Тогда
