...
Функция ПечатьМ11(МассивОбъектов, ОбъектыПечати)
	...
	ТабДокумент.ИмяПараметровПечати	= "ПараметрыПечатиПередачаМатериаловВЭксплуатацию_М11";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл
			...
			Область.Параметры.Заголовок = "ТРЕБОВАНИЕ-НАКЛАДНАЯ № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаПоДокументам.Номер, Истина, Ложь);
...
Функция ПечатьМБ7(МассивОбъектов, ОбъектыПечати)
	...
	|	ТабСотрудники.ФизическоеЛицо,
	|	ТабСотрудники.СотрудникКод,
	...
	|	Документ.ПередачаМатериаловВЭксплуатацию КАК ДокИсх
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.ПередачаМатериаловВЭксплуатацию.Спецодежда КАК Док
	|	ПО ДокИсх.Ссылка = Док.Ссылка
