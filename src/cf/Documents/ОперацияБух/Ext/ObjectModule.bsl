...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИначеЕсли Найти(СпособЗаполнения, "Сторно") > 0 Тогда
		ТиповаяОперация = Неопределено;
		...
		Если ЗначениеЗаполнено(СторнируемыйДокумент) И (НачалоМесяца(СторнируемыйДокумент.Дата) <> НачалоМесяца(Дата)) Тогда
			...
			МассивСубсчетов = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(МассивСчетов);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Найти(СпособЗаполнения, "Сторно") = 0 Тогда
		МассивНепроверяемыхРеквизитов.Добавить("СторнируемыйДокумент");
	КонецЕсли;
	Если Найти(СпособЗаполнения, "ТиповаяОперация") = 0 Тогда
