...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ТаблицаТовары.Содержание КАК Товар,
...
Процедура ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабличнойЧасти, СведенияОНоменклатуре) Экспорт
	...
	ИначеЕсли ИмяТабличнойЧасти = "МатериалыЗаказчика" Тогда
		...
		Если ЗначениеЗаполнено(СчетаУчета.СчетПередачиЗаб) Тогда
			СтрокаТабличнойЧасти.СчетУчета = БухгалтерскийУчетВызовСервераПовтИсп.СчетУчетаМатериалыПринятыеВПереработкуВПроизводстве(СчетаУчета.СчетПередачиЗаб);
		КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.Текст =
		...
		+ ТекстЗапросаПлановаяСебестоимость(НомераТаблиц)
...
Функция ТекстЗапросаРеализация(Номератаблиц)
	...
	|	ТаблицыДокументаРеализации.Ссылка.СчетУчетаРасчетовСКонтрагентом КАК КорСчет,
	|	ТаблицыДокументаРеализации.Ссылка.Контрагент КАК КорСубконто1,
	|	ТаблицыДокументаРеализации.Ссылка.ДоговорКонтрагента КАК КорСубконто2,
	|	ТаблицыДокументаРеализации.Ссылка КАК КорСубконто3,
	|	ТаблицыДокументаРеализации.Ссылка.Контрагент КАК Контрагент,
	|	ТаблицыДокументаРеализации.Ссылка.Контрагент КАК Покупатель,
	|	ТаблицыДокументаРеализации.Ссылка.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ТаблицыДокументаРеализации.Ссылка.ПодразделениеОрганизации КАК Подразделение,
	|	ЛОЖЬ КАК ЭтоКомиссия,
...
Функция ТекстЗапросаПлановаяСебестоимость(НомераТаблиц)
...
Функция ТекстЗапросаСписаниеМатериалов(НомераТаблиц)
	...
	|	ТаблицаТовары.Ссылка.Контрагент                КАК Комитент,
	|	НЕОПРЕДЕЛЕНО                                   КАК ДоговорКомиссии,
...
Функция ТекстЗапросаЗачетАвансов(Номератаблиц)
	...
	|ПОМЕСТИТЬ
	|	ВТ_ТаблицыДокументаЗачетАванса
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицыДокументаЗачетАванса.Ссылка                                             КАК ДокументРасчетов,
	|	ТаблицыДокументаЗачетАванса.Ссылка.СчетУчетаРасчетовСКонтрагентом              КАК СчетРасчетов,
	|	ТаблицыДокументаЗачетАванса.Ссылка.СчетУчетаРасчетовПоАвансам                  КАК СчетАвансов,
	|	ТаблицыДокументаЗачетАванса.Ссылка.Контрагент                                  КАК Контрагент,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента                          КАК ДоговорКонтрагента,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВидДоговора              КАК ВидДоговора,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВалютаВзаиморасчетов     КАК ВалютаВзаиморасчетов,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах КАК РасчетыВУсловныхЕдиницах,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.УчетАгентскогоНДС        КАК УчетАгентскогоНДС,
	|	ВЫБОР
	|		КОГДА ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВалютаВзаиморасчетов = &ВалютаРеглУчета
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                                                                          КАК РасчетыВВалюте,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ПодразделениеОрганизации                    КАК Подразделение,
	...
	|ГДЕ
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВидДоговора <> ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицыДокументаЗачетАванса.Ссылка,
	|	ТаблицыДокументаЗачетАванса.Ссылка.СчетУчетаРасчетовСКонтрагентом,
	|	ТаблицыДокументаЗачетАванса.Ссылка.СчетУчетаРасчетовПоАвансам,
	|	ТаблицыДокументаЗачетАванса.Ссылка.Контрагент,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВидДоговора,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.ВалютаВзаиморасчетов,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ДоговорКонтрагента.УчетАгентскогоНДС,
	|	ТаблицыДокументаЗачетАванса.Ссылка.ПодразделениеОрганизации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеализацияУслугПоПереработке") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "РеализацияУслугПоПереработке", "Реализация услуг по переработке",
			ПечатьАктаОбОказанииУслуг(МассивОбъектов, ОбъектыПечати), , "Документ.РеализацияУслугПоПереработке.ПФ_MXL_Акт");
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОПродукции") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ОтчетОПродукции", "Отчет о продукции", 
			ПечатьОтчета("Услуги", МассивОбъектов, ОбъектыПечати), , "Документ.РеализацияУслугПоПереработке.ПФ_MXL_Акт");
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОМатериалах") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ОтчетОМатериалах", "Отчет о материалах",
			ПечатьОтчета("МатериалыЗаказчика", МассивОбъектов, ОбъектыПечати), , "Документ.РеализацияУслугПоПереработке.ПФ_MXL_Акт");
...
Функция ПечатьАктаОбОказанииУслуг(МассивОбъектов, ОбъектыПечати)
	...
	|	РеализацияУслугПоПереработке.ИсполнительПоПриказу,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Исполнитель) Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(Выборка.ИсполнительПоПриказу) И ЗначениеЗаполнено(ЗаместительПоПриказу) Тогда 
					ЗаместительПоПриказу = ЗаместительПоПриказу + ", " + Выборка.ИсполнительПоПриказу;
				КонецЕсли;
...
//
Функция ПечатьОтчета(ТабЧасть, МассивОбъектов, ОбъектыПечати)
