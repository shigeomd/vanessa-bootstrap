...
Функция ЗаполнитьДокументНаСервере()
	...
	СтруктураПараметров = Новый Структура("НалоговыйПериод, Организация", 
		 Объект.НалоговыйПериод, Объект.Организация); 
		 
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		
		АдресХранилища	= ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
		Документы.ЖурналУчетаСчетовФактурДляПередачиВЭлектронномВиде.ПодготовитьДанныеДляЗаполнения(СтруктураПараметров, АдресХранилища);
