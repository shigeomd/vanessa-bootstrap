...
Функция ТабличныеДокументыФормыСЗВ_6_4(МассивОбъектов, ОбъектыПечати = Неопределено, ВыводитьОпись = Ложь, ДатаПодписи = Неопределено) Экспорт
	...
	Пока ВыборкаДокументов.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Если ВыборкаЗаписейСтажа.НайтиСледующий(СтруктураПоиска) Тогда
			...
			ОбластьШапка.Параметры.НаименованиеОрганизации = ПерсонифицированныйУчет.СтрокаВОтчет(ВыборкаДокументов.НаименованиеОрганиазции);
...
Процедура ЗаполнитьСведенияОЗаработкеВМакет(ОбастьЗаработок, ВыборкаПоСтрокамЗаработка, Ссылка, НомерСтрокиСотрудник)
	...
	ОчиститьОбластьЗаработок(ОбастьЗаработок);
	...
	Если ВыборкаПоСтрокамЗаработка.НайтиСледующий(СтруктураПоиска) Тогда
		...
		Пока ВыборкаПоСтрокамЗаработка.Следующий() Цикл
			...
			ОбастьЗаработок.Параметры["ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах" + ВыборкаПоСтрокамЗаработка.Месяц] 
				= ВыборкаПоСтрокамЗаработка.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах;
...
Процедура ОчиститьОбластьЗаработок(ОбастьЗаработок)
	
	ОбастьЗаработок.Параметры["ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботахИтог"] = 0;
	ОбастьЗаработок.Параметры["ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботахИтог"] = 0;
	ОбастьЗаработок.Параметры["ЗаработокИтог"] = 0;
	ОбастьЗаработок.Параметры["ОблагаетсяВзносамиДоПредельнойВеличиныИтог"] = 0;
	ОбастьЗаработок.Параметры["ОблагаетсяВзносамиСвышеПредельнойВеличиныИтог"] = 0;
	
	Для НомерМесяца = 1 По 3 Цикл
		ОбастьЗаработок.Параметры["ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах" + НомерМесяца] = 0; 
		ОбастьЗаработок.Параметры["ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах" + НомерМесяца] = 0;  
		ОбастьЗаработок.Параметры["Заработок" + НомерМесяца] = 0; 
		ОбастьЗаработок.Параметры["ОблагаетсяВзносамиДоПредельнойВеличины" + НомерМесяца] = 0;
		ОбастьЗаработок.Параметры["ОблагаетсяВзносамиСвышеПредельнойВеличины" + НомерМесяца] = 0;
	КонецЦикла;
...
Функция СформироватьПечатнуюФормуСписокЗастрахованныхЛиц(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока ВыборкаДокументов.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		ОбластьШапка.Параметры.НаименованиеСокращенное = ПерсонифицированныйУчет.СтрокаВОтчет(ВыборкаДокументов.НаименованиеОрганиазции);
		...
		ТабличныйДокумент.Вывести(ОбластьШапка);
		//ТабличныйДокумент.Вывести(ОбластьПовторятьПриПечати);
