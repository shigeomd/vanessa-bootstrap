...
Процедура СчетУчетаЗатратПриИзменении(Элемент)
	...
	УстановитьЗаголовкиИДоступностьСубконто(ЭтаФорма, Объект.СчетУчетаЗатрат, "СчетУчетаЗатрат");
	...
	БухгалтерскийУчетКлиентСервер.ПриИзмененииСчета(Объект.СчетУчетаЗатрат, Объект, ПоляОбъекта, , ЗначенияСубконто);
...
Процедура СубконтоСчетУчетаЗатрат1ПриИзменении(Элемент)
	...
	СубконтоПриИзмененииНаКлиенте(Элемент);
...
Процедура ПокупателиТоварыПередУдалением(Элемент, Отказ)
	...
	ТекДанные = Элементы.Покупатели.ТекущиеДанные;
...
Процедура ПокупателиТоварыПокупательПриИзменении(Элемент)
	...
	ЗапуститьПроверкуКонтрагентов(Элементы.Покупатели);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеОбъекта = Новый Структура(
		...
		|Реализация, ПрименяютсяСтавки4и2");
...
Процедура ВозвратыПередУдалением(Элемент, Отказ)
	ТекДанные = Элементы.Возвраты.ТекущиеДанные;
	Если ТекДанные = Неопределено Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ВозвратыПокупательПриИзменении(Элемент)
	ЗапуститьПроверкуКонтрагентов(Элементы.Возвраты);
КонецПроцедуры
...
Процедура ТоварыВозвращенныеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ДанныеСтрокиВозвраты.Покупатель) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
			"Колонка",, "Покупатель", ДанныеСтрокиВозвраты.НомерСтроки, "Покупатели");
		ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
			"Покупатели", ДанныеСтрокиВозвраты.НомерСтроки, "Покупатель");
...
Процедура ТоварыВозвращенныеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если НЕ Копирование Тогда
			ДанныеСтрокиТовары.СтавкаНДС = УчетНДСКлиент.ПолучитьСтавкуНДСПоУмолчанию(ПлательщикНДС);
		КонецЕсли; 
...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ЗначениеЗаполнено(Форма.СчетФактураНаВознаграждение) И Форма.ТребуетсяСчетФактура Тогда
		Элементы.ГруппаСчетФактураНаВознаграждениеСтраницы.ТекущаяСтраница = Элементы.ГруппаЗарегистрироватьСчетФактуруНаВознаграждение;
	Иначе
		Элементы.ГруппаСчетФактураНаВознаграждениеСтраницы.ТекущаяСтраница = Элементы.ГруппаСчетФактураНаВознаграждениеСсылка;
	КонецЕсли;
	Элементы.НадписьСчетФактураНаВознаграждение.Гиперссылка = Форма.ТребуетсяСчетФактура;
	...
	Если Объект.ДокументБезНДС Тогда
		...
		Элементы.ГруппаРеализация.ТекущаяСтраница 	= Элементы.РеализацияБезНДС;
		Элементы.ГруппаВозвраты.ТекущаяСтраница 	= Элементы.ВозвратыБезНДС;
	Иначе
		Элементы.ТоварыИтогСуммаНДС.Видимость	  	= Истина;
		Элементы.ИтогНДСВозвраты.Видимость	  		= Истина;
		Элементы.ГруппаРеализация.ТекущаяСтраница 	= Элементы.РеализацияСНДС;
		Элементы.ГруппаВозвраты.ТекущаяСтраница 	= Элементы.ВозвратыСНДС;
	КонецЕсли;
...
Функция ПолучитьСписокПараметров(Форма, Объект, ШаблонИмяПоляОбъекта)
	...
	СписокПараметров.Вставить("СчетУчета"  , Форма.Объект.СчетУчетаЗатрат);
...
Процедура ИзменитьПараметрыВыбораПолейСубконто(Форма)
	...
	ПараметрыДокумента = ПолучитьСписокПараметров(Форма, Форма.Объект, "Субконто%Индекс%");
	БухгалтерскийУчетКлиентСервер.ИзменитьПараметрыВыбораПолейСубконто(
		Форма, Форма.Объект, "Субконто%Индекс%", "СубконтоСчетУчетаЗатрат%Индекс%", ПараметрыДокумента);

КонецПроцедуры

&НаКлиенте
Процедура СубконтоПриИзмененииНаКлиенте(Элемент)
...
Процедура ДоговорКонтрагентаОбработатьИзменение()
	...
	РеквизитыДоговорКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		...
		"ПроцентКомиссионногоВознаграждения,СпособРасчетаКомиссионногоВознаграждения,ВалютаВзаиморасчетов,ТипЦен, РасчетыВУсловныхЕдиницах, ПредъявляетНДС, НДСПоСтавкам4и2");
		
	РасчетыВУЕ 			 = РеквизитыДоговорКонтрагента.РасчетыВУсловныхЕдиницах; 	
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		Если Не РеквизитыДоговорКонтрагента.ПредъявляетНДС 
		...
		ИначеЕсли РеквизитыДоговорКонтрагента.ПредъявляетНДС 
			И Объект.СтавкаНДСВознаграждения = Перечисления.СтавкиНДС.БезНДС Тогда 
			Объект.СтавкаНДСВознаграждения = Перечисления.СтавкиНДС.НДС18;	
			СтавкаНДСИзменилась = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура НадписьСчетФактураНаВознаграждениеНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	БухгалтерскийУчетКлиентПереопределяемый.ОткрытьСчетФактуру(ЭтаФорма, СчетФактураНаВознаграждение, "СчетФактураПолученный");
...
&НаСервере
Процедура ПодставитьСтатьюЗатратВШапку()
