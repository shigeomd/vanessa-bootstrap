...
Процедура ЗаполнитьДляСпискаОССервер()
	...
	ТЗ_ОС = Запрос.Выполнить().Выгрузить();
	ТЗ_ОС.Индексы.Добавить("ОсновноеСредство");

	Для каждого Строка Из Объект.ОС Цикл
		// В соответствующие поля строки запишем данные из запроса
		СтрокаТЗ = ТЗ_ОС.Найти(Строка.ОсновноеСредство, "ОсновноеСредство");

		Если СтрокаТЗ = Неопределено Тогда
			Строка.СпециальныйКоэффициент = 0;
		Иначе
			Строка.СпециальныйКоэффициент = СтрокаТЗ.СпециальныйКоэффициент;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ВопросСуществующиеДанныеБудутПересчитаныЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьДляСпискаОС(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ВопросСуществующиеДанныеБудутПересчитаныЗавершение", ЭтотОбъект);
