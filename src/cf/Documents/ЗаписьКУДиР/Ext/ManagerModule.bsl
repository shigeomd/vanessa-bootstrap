...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.Текст =
		...
		+ ТекстЗапросаТаблицаУменьшениеНалога(НомераТаблиц);
...
Функция ТекстЗапросаТаблицаУменьшениеНалога(НомераТаблиц)
...
Процедура ЗарегистрироватьДанныеПервичныхДокументов() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяЗапись.Номер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.Номер, Истина, Ложь);
		
		Попытка
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(Набор);
		Исключение
			ТекстСообщения	= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось зарегистрировать данные документа %1'"),
				Выборка.Документ);
...
Процедура ДобавитьДвиженияПоРасходамУменьшающимНалогУСН() Экспорт
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Исключение
			ТекстСообщения = НСтр("ru = 'Не удалось сформировать движения документа ""Запись книги доходов и расходов УСН"" по регистру ""Книга учета доходов и расходов (раздел IV)""'");
