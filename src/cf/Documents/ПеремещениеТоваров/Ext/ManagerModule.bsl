...
Функция ПодготовитьТаблицуСписанныеМПЗИП(ТаблицаСписанныеМПЗ, ТаблицаРеквизиты) Экспорт
	...
	Если НЕ СпособОценкиТоваровВРознице = Перечисления.СпособыОценкиТоваровВРознице.ПоПродажнойСтоимости
		ИЛИ НЕ Реквизиты.ТипОтправителя = Перечисления.ТипыСкладов.ОптовыйСклад
		ИЛИ НЕ Реквизиты.ТипСклада = Перечисления.ТипыСкладов.НеавтоматизированнаяТорговаяТочка Тогда
		
		// МПЗ списываются, только при перемещении ОПТ -> НТТ
...
Функция ПечатьПеремещениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			ОбластьДанных.Параметры.Товар = ?(ЗначениеЗаполнено(ВыборкаСтрокТовары.Товар), СокрЛП(ВыборкаСтрокТовары.Товар), СокрЛП(ВыборкаСтрокТовары.Номенлатура));
			ТабличныйДокумент.Присоединить(ОбластьДанных);
