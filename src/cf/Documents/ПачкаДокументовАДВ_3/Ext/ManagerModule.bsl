...
Функция СформироватьЗапросПоДаннымФизЛицДокумента(ДокументСсылка, Дата, МенеджерВременныхТаблиц) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ДанныеОТрудовомСтажеИЗаработкеСЗВ_6.Дата) КАК Ссылка,
	...
	|				И (РеестрСЗВ_6_2Сотрудники.Ссылка.ДокументПринятВПФР)) КАК ДанныеОТрудовомСтажеИЗаработкеСЗВ_6
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	КОНЕЦ КАК Телефоны,
	|	ВЫБОР
	|		КОГДА ДанныеОСЗВ6.Ссылка ЕСТЬ НЕ NULL 
...
Функция СформироватьВыходнойФайл(ВыборкаПоШапкеДокумента, ВыборкаСотрудники)
	...
	Пока ВыборкаСотрудники.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаСотрудники.МестоРожденияВСвидетельстве) Тогда 
			...
			НаборЗаписейМестоРожденияИзСтрахового.ГородРождения = ПерсонифицированныйУчет.ПолучитьИмяИАдресноеСокращение(МестоРожденияИзСтрахового.НаселенныйПункт,Сокращение);
			НаборЗаписейМестоРожденияИзСтрахового.РайонРождения = ПерсонифицированныйУчет.ПолучитьИмяИАдресноеСокращение(МестоРожденияИзСтрахового.Район,Сокращение);
		...
		Если ВыборкаСотрудники.ПризнакОтменыОтчества  Тогда
			НаборЗаписейАДВ3.ПризнакОтменыОтчества.Значение = "ОТМН";
		Иначе
		...
		Если ВыборкаСотрудники.ПризнакОтменыОтчества  Тогда
			НаборЗаписейАДВ3.ПризнакОтменыОтчества.Значение = "ОТМЕНЕНО";
		Иначе
			НаборЗаписейАДВ3.Удалить("ПризнакОтменыОтчества");
		КонецЕсли;
		
		// Место рождения может быть отменено.
		Если ВыборкаСотрудники.ПризнакОтменыМестаРождения  Тогда
			НаборЗаписейАДВ3.ПризнакОтменыМестаРождения.Значение = "ОТМЕНЕНО";
		Иначе
			НаборЗаписейАДВ3.Удалить("ПризнакОтменыМестаРождения");
		КонецЕсли;
...
Функция СформироватьПечатнуюФормуАДВ3(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АнкетаЗастрахованногоЛица_ФормаАДВ_3";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Пока ВыборкаСотрудники.Следующий() Цикл
			...
			МестоРожденияВСвидетельстве.НаселенныйПункт = ПерсонифицированныйУчет.ПолучитьИмяИАдресноеСокращение(МестоРожденияВСвидетельстве.НаселенныйПункт,Сокращение);
			...
			МестоРожденияВСвидетельстве.Район = ПерсонифицированныйУчет.ПолучитьИмяИАдресноеСокращение(МестоРожденияВСвидетельстве.Район, Сокращение);
			...
			Если ВРег(ВыборкаСотрудники.ОтметкаОПредставленииСведений) <> "ПРЕДСТАВЛЕНЫ РАНЕЕ" Тогда
...
Функция СформироватьПечатнуюФормуАДВ6(МассивОбъектов, ОбъектыПечати)
	Возврат ПерсонифицированныйУчет.ВывестиОписьАДВ6(МассивОбъектов, ОбъектыПечати, "ДУБЛ");
КонецФункции
