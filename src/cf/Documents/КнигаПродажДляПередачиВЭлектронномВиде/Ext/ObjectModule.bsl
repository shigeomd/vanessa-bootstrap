...
Процедура ЗаполнитьДаннымиВсего(Параметры, ДеревоВыгрузки)
	...
	УзелСвКнПрод = ПолучитьПодчиненныйЭлемент(УзелДокумент, "СвКнПрод");
	УстановитьЗначениеЭлемента(УзелСвКнПрод, "ВсегоВсПродВклНДС", ВсегоПродаж);
	
	УзелВсего = ПолучитьПодчиненныйЭлемент(УзелСвКнПрод, "Всего");
