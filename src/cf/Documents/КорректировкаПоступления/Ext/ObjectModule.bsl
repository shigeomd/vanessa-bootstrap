	...
	Если НЕ (ДополнительныеСвойства.Свойство("ПропуститьОбновлениеРеквизитовВСвязанныхДокументах") 
		...
		Если ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
			Если ЗначениеЗаполнено(ИсправляемыйДокументПоступления) Тогда
				НомерПредставления 	= ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ДокументПоступления) Тогда
		...
		Если ТипЗнч(ДокументПоступленияСсылка) <> Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") Тогда
			Если НЕ ЗначениеЗаполнено(ДокументПоступленияСсылка) Тогда
				...
				Возврат;
			КонецЕсли;
			Если ОбщегоНазначения.СсылкаСуществует(ДокументПоступленияСсылка) Тогда
				ДокументПроведен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПоступленияСсылка, "Проведен");
				Если НЕ ДокументПроведен Тогда
					ТекстСообщения = НСтр("ru = 'Не проведен документ, отражающий корректируемое поступление!'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ДокументПоступления", "Объект", Отказ);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	КонтрагентРезидентТаможенногоСоюза = УчетНДС.КонтрагентРезидентТаможенногоСоюза(Контрагент);
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Если УчетТоваров.НеУказанаСтранаПроисхождения(СтрокаТаблицы.НомерГТД, СтрокаТаблицы.СтранаПроисхождения) 
			И НЕ КонтрагентРезидентТаможенногоСоюза Тогда
		...
		// Проверка комитента по закупке для товаров, учитываемых за балансом.
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаТаблицы.СчетУчета);
		Если НЕ ЭтоКомиссияПоПродаже И КорректироватьБУиНУ Тогда
			Если СвойстваСчета.Забалансовый Тогда
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.Контрагент) Тогда
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка",, НСтр("ru = 'Комитент'"),
					СтрокаТаблицы.НомерСтроки, ИмяСписка);
				...
				Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СчетРасчетов) Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УчетТоваров.СформироватьДвиженияКорректировкаПоступленияПоЗакупленнымТоварамКомитента(
		ПараметрыПроведения.ЗакупленныеТоварыКомитента, ТаблицаОстаткиТоварыКомитентов,
	...
	УчетНДС.СформироватьДвиженияНДСКорректировкиПоступлениеТоваровУслуг(
		ПараметрыПроведения.НДСРеквизиты, ПараметрыПроведения.НДСТаблицыДокумента, Движения, Отказ);
