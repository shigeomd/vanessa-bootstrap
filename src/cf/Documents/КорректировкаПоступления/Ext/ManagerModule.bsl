...
Функция ПолучитьПервичныйДокументПоступления(ТекущийДокументПоступления)
	...
	Пока ТипЗнч(ПервичныйДокументПоступления) = Тип("ДокументСсылка.КорректировкаПоступления") Цикл
...
Процедура ОбновитьРеквизитыДокументовКорректировкиПоступления(РеквизитыСсылки, РеквизитыОбъекта, Отказ)
	...
	Пока ВыборкаСвязанныхДокументов.Следующий() Цикл
		...
		Если ВыборкаСвязанныхДокументов.ИзменятьПредставлениеНомера Тогда
			НомерПредставления 	= ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РеквизитыОбъекта.Номер, Истина, Ложь); 
	...
	Для Каждого СтрокаДокументаКорректировки ИЗ ТаблицаДокументовКорректировки Цикл
		...
		Если СтрокаДокументаКорректировки.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
			СтрокаДокументаКорректировки.ПредставлениеНомера = 
				ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаДокументаКорректировки.НомерКорректировки, Истина, Ложь);   
		ИначеЕсли СтрокаДокументаКорректировки.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
			СтрокаДокументаКорректировки.ПредставлениеНомера = 
				ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаДокументаКорректировки.НомерИсправляемогоДокументаОснования, Истина, Ложь) + 
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Если Реквизиты.РасчетыВВалюте или Реквизиты.РасчетыВУсловныхЕдиницах Тогда
		...
		Иначе
			...
			Если Найти(ИмяДокументаПоступления, "СчетФактура") = 0 или (Найти(ИмяДокументаПоступления, "ДокументРасчетовСКонтрагентом")) Тогда
		...
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
			...
			Реквизиты.ВалютаВзаиморасчетов, ДокументПоступленияСсылка.Дата);
	...
	ВестиСуммовойУчетПоСкладам = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(
	...
	Запрос.УстановитьПараметр("ИсправленныйСчетФактураДляДвиженияНДС",
	...
	Запрос.УстановитьПараметр("СчетФактураДата", 
		...
		Реквизиты.Период));
	Запрос.УстановитьПараметр("СчетФактураДляДвиженияНДС", 
		?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки,
		УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументПоступления(Реквизиты.ДокументПоступления), 
		ДокументСсылка));
	...
	Запрос.Текст = ТекстЗапросаКорректировкаПоступления(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаРеквизитыНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	Реквизиты.ПодразделениеОрганизации,
	|	Реквизиты.ДокументПоступления,
	|	Реквизиты.ДокументПоступления.Дата КАК ДокументПоступлениеДата,
	|	Реквизиты.Склад,
	...
	|	Реквизиты.ДоговорКонтрагента,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Реквизиты.ДоговорКонтрагента.ВалютаВзаиморасчетов, ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	...
	|	Реквизиты.ДокументПоступлениеДата,
	|	Реквизиты.Склад,
	|	Реквизиты.ТипСклада,
	|	Реквизиты.ВалютаДокумента,
	|	Реквизиты.Контрагент,
...
Процедура ПодготовитьТаблицуДокументаРасчетыВВалюте(ТаблицаДляОбработки, Реквизиты)
	...
	РублеваяСуммаДляПересчетаСуммДоКорректировки = СуммаВзаиморасчетовДоКорректировки*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсДвиженийПервичногоДокумента/Реквизиты.КратностьВзаиморасчетов);
	...
	Если НЕ Реквизиты.РасчетыВУсловныхЕдиницах Тогда
		РублеваяСуммаДляПересчетаСуммДоКорректировки = СуммаВзаиморасчетовДоКорректировки*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсВзаиморасчетов/Реквизиты.КратностьВзаиморасчетов);
	КонецЕсли;	
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДляОбработки Цикл
		...
		СтрокаТаблицы.СуммаНДСРубДоКорректировки = СтрокаТаблицы.СуммаБезНДСРубДоКорректировки*ПроцентНДСДоКорректировки/(100 + ПроцентНДСДоКорректировки);			
		...
		Если СтрокаТаблицы.СуммаВзаиморасчетовДоИзменения = СтрокаТаблицы.СуммаВзаиморасчетовДоКорректировки Тогда
			...
			Если ПроцентНДСДоКорректировки <> ПроцентНДСДоИзменения Тогда
				СтрокаТаблицы.СуммаНДСРубДоИзменения = СтрокаТаблицы.СуммаБезНДСРубДоИзменения*ПроцентНДСДоИзменения/(100 + ПроцентНДСДоИзменения);
			Иначе
			...
			Если СтрокаТаблицы.СуммаВзаиморасчетов = СтрокаТаблицы.СуммаВзаиморасчетовДоКорректировки Тогда
				...
				Если ПроцентНДСДоКорректировки <> ПроцентНДС Тогда
					СтрокаТаблицы.СуммаНДСРуб = СтрокаТаблицы.СуммаБезНДСРубДоИзменения*ПроцентНДС/(100 + ПроцентНДС);
				Иначе
	...
	РублеваяСуммаПодлежащаяРаспределению = СуммаВзаиморасчетовДоИзмененияПодлежащаяПересчетуПоКурсуДвиженийДокумента*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсДвиженийПоследнейКорректировки/Реквизиты.КратностьВзаиморасчетов);
	
	ОбщегоНазначенияБПВызовСервера.РаспределитьСуммуПоКолонкеТаблицы(
	РублеваяСуммаПодлежащаяРаспределению, ТаблицаДоИзменения, "СуммаРубДоИзменения", "СуммаВзаиморасчетовДоИзменения");
	
	Если НЕ Реквизиты.РасчетыВУсловныхЕдиницах Тогда
		РублеваяСуммаПодлежащаяРаспределению = СуммаВзаиморасчетовДоИзмененияПодлежащаяПересчетуПоКурсуДвиженийДокумента*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсВзаиморасчетов/Реквизиты.КратностьВзаиморасчетов);
	КонецЕсли;	
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДоИзменения Цикл
		...
		СтрокаТаблицы.СуммаНДСРубДоИзменения = СтрокаТаблицы.СуммаБезНДСРубДоИзменения*ПроцентНДС/(100 + ПроцентНДС);
	...
	Если СуммаВзаиморасчетовДоИзмененияПоТекущейКорректировки < СуммаВзаиморасчетовПоТекущейКорректировке Тогда
		...
		РублеваяСуммаПодлежащаяРаспределению = УвеличениеСуммаВзаиморасчетовПодлежащаяРаспределению*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсВзаиморасчетов/Реквизиты.КратностьВзаиморасчетов);
		...
		Для Каждого СтрокаТаблицы Из ТаблицаПоТекущейКорректировке Цикл
			...
			СтрокаТаблицыДляРаспределенияУвеличенияДоИзменения.СуммаНДСРубУвеличения    = СтрокаТаблицыДляРаспределенияУвеличенияДоИзменения.СуммаБезНДСРубУвеличения*ПроцентНДСУвеличения/(100+ПроцентНДСУвеличения);
	...
	РублеваяСуммаДляРаспределения = СуммаВзаиморасчетовДляРаспределения*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсДвиженийПоследнейКорректировки/Реквизиты.КратностьВзаиморасчетов);
	
	ОбщегоНазначенияБПВызовСервера.РаспределитьСуммуПоКолонкеТаблицы(
	РублеваяСуммаДляРаспределения, ТаблицаОкончательныхПреобразований, "СуммаРуб", "СуммаВзаиморасчетов");
	
	Если НЕ Реквизиты.РасчетыВУсловныхЕдиницах Тогда
		РублеваяСуммаДляРаспределения = СуммаВзаиморасчетовДляРаспределения*?(Реквизиты.КратностьВзаиморасчетов = 0, 0, Реквизиты.КурсВзаиморасчетов/Реквизиты.КратностьВзаиморасчетов);
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДляОбработки Цикл
		...
		Иначе
			...
			СтрокаТаблицыОкончательныхПреобразований.СуммаНДСРуб = СтрокаТаблицыОкончательныхПреобразований.СуммаБезНДСРуб*ПроцентНДС/(100+ПроцентНДС);
...
Функция ТекстЗапросаРеквизитыНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
...
Функция ТекстЗапросаТаблицСторнирующихНДСЗаписей(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	НомераТаблиц.Вставить("ДатыСобытий",      НомераТаблиц.Количество());
	...
	|				И ИсправленныйСчетФактура = НЕОПРЕДЕЛЕНО) КАК ЗаписиКнигиПокупок
	|ГДЕ
	|	ЗаписиКнигиПокупок.НДСОборот >= 0
	|	И ЗаписиКнигиПокупок.СуммаБезНДСОборот >= 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА КОНЕЦПЕРИОДА(&Период, КВАРТАЛ) = КОНЕЦПЕРИОДА(ДатыСобытий.ДатаСобытия, КВАРТАЛ)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		КОГДА КОНЕЦПЕРИОДА(&Период, КВАРТАЛ) = КОНЕЦПЕРИОДА(ДатыСобытий.ДатаСобытия, КВАРТАЛ)
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(ДатыСобытий.ДатаСобытия, КВАРТАЛ)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыСобытий КАК ДатыСобытий
	|		ПО ЗаписиКнигиПокупок.СчетФактура = ДатыСобытий.СчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаТаблицВычетНДСНаОснованииДокумента(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|	&ИсправленныйСчетФактураДляДвиженияНДС КАК ИсправленныйСчетФактура,
		|	ЛОЖЬ КАК Услуга,
		|	ТаблицаТовары.Номенклатура,
		|	ТаблицаТовары.СчетУчета,
		|	ТаблицаТовары.СчетУчетаНДС,
...
Функция ТекстЗапросаТаблицКорректировочныеДвиженияНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	&СчетФактураДляДвиженияНДС КАК СчетФактура,
		|	&ИсправленныйСчетФактураДляДвиженияНДС КАК ИсправленныйСчетФактура,
...
Функция ТекстЗапросаКорректировкаПоступленияУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	//ИменаКолонокУСН = "Номенклатура,ОтражениеВУСН,НомерСтроки,Количество,НДС,НДСВал,Сумма,СуммаБезНДС,СуммаБезНДСВал,СуммаВал,СчетУчета";
	...
	Иначе
		...
		Если УчестьРасходыУСНТовары Тогда
			...
			|	ТаблицаТовары.Количество - ТаблицаТовары.КоличествоДоИзменения КАК Количество,
			|	ТаблицаТовары.СуммаВзаиморасчетов - ТаблицаТовары.СуммаВзаиморасчетовДоИзменения КАК Сумма,
			|	ТаблицаТовары.СуммаНДСВзаиморасчетов - ТаблицаТовары.СуммаНДСВзаиморасчетовДоИзменения КАК НДС,
			|	ВЫБОР
		...
		Если УчестьРасходыУСНУслуги Тогда
			...
			|	0 КАК Количество,
			|	ТаблицаУслуги.СуммаВзаиморасчетов - ТаблицаУслуги.СуммаВзаиморасчетовДоИзменения КАК Сумма,
			|	ТаблицаУслуги.СуммаНДСВзаиморасчетов - ТаблицаУслуги.СуммаНДСВзаиморасчетовДоИзменения КАК НДС,
			|	ВЫБОР
			|		КОГДА &ПрименяетсяУСНДоходы
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.НеПринимаются)
			...
			|	ТаблицаУслуги КАК ТаблицаУслуги
			|ГДЕ
			|	(ТаблицаУслуги.СуммаВзаиморасчетов - ТаблицаУслуги.СуммаВзаиморасчетовДоИзменения > 0
			|			ИЛИ ТаблицаУслуги.СуммаВзаиморасчетов - ТаблицаУслуги.СуммаВзаиморасчетовДоИзменения = 0
			|				И ТаблицаУслуги.СуммаНДСВзаиморасчетов - ТаблицаУслуги.СуммаНДСВзаиморасчетовДоИзменения > 0)";
		КонецЕсли;
		...
		Если УчестьРасходыУСНТовары Тогда
			...
			|	ТаблицаТовары.СуммаВзаиморасчетов - ТаблицаТовары.СуммаВзаиморасчетовДоИзменения КАК Сумма,
			|	ТаблицаТовары.СуммаНДСВзаиморасчетов - ТаблицаТовары.СуммаНДСВзаиморасчетовДоИзменения КАК НДС,
			|	ТаблицаТовары.КоличествоДоИзменения КАК КоличествоПоПоступлению,
			|	ТаблицаТовары.СуммаВзаиморасчетовДоИзменения КАК СуммаПоПоступлению,
			|	ТаблицаТовары.СуммаНДСВзаиморасчетовДоИзменения КАК НДСПоПоступлению,
			|	ВЫБОР
			|		КОГДА &ПрименяетсяУСНДоходы
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.НеПринимаются)
			|		ИНАЧЕ ТаблицаТовары.ОтражениеВУСН
			...
			|				И (ТаблицаТовары.СуммаВзаиморасчетов - ТаблицаТовары.СуммаВзаиморасчетовДоИзменения <> 0
			|					ИЛИ ТаблицаТовары.СуммаНДСВзаиморасчетов - ТаблицаТовары.СуммаНДСВзаиморасчетовДоИзменения <> 0))";
		КонецЕсли;
		Если УчестьРасходыУСНТовары
			И УчестьРасходыУСНУслуги Тогда
			ТекстЗапроса = ТекстЗапроса + "
		...
		Если УчестьРасходыУСНУслуги Тогда
			...
			|	ТаблицаУслуги.СуммаВзаиморасчетовДоИзменения КАК СуммаПоПоступлению,
			|	ТаблицаУслуги.СуммаНДСВзаиморасчетовДоИзменения КАК НДСПоПоступлению,
			|	ВЫБОР
			|		КОГДА &ПрименяетсяУСНДоходы
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.НеПринимаются)
			|		ИНАЧЕ ТаблицаУслуги.ОтражениеВУСН
...
КонецФункции // ТекстЗапросаКорректировкаПоступленияУСН()
...
Функция ПодготовитьТаблицыАвансовОплат(ТаблицаВзаиморасчетов, ТаблицаРеквизиты) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		Или Параметры.ТаблицаВзаиморасчетов.Количество() = 0 Тогда		
			Возврат Неопределено;
	КонецЕсли; 
...
Функция ПодготовитьТаблицуВыделениеАвансов(ТаблицаВзаиморасчетыВозврат, ТаблицаРеквизиты, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам) Экспорт
	...
	Если Параметры.РеквизитыВыделенияАвансов.Количество() = 0 или Параметры.ТаблицаВыделенияАвансов.Количество() = 0 Тогда
	...
	Если Реквизиты.ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить() или СтрокаВыделяемогоАванса = Неопределено Тогда
...
Функция ПодготовитьТаблицыТоваровУслуг(РеквизитыДокумента, ТаблицаДокумента, ЭтоУслуги = Ложь) Экспорт
	...
	Если ЭтоУслуги И ТипЗнч(Реквизиты.ДокументПоступленияСсылка) = Тип("ДокументСсылка.ПоступлениеДопРасходов") Тогда
	...
	Если НЕ ЭтоУслуги И Реквизиты.УчетВПродажныхЦенах И Реквизиты.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин Тогда
...
Функция ДополнитьОстаткамиБУТаблицуКорректировкиТоваров(РеквизитыДокумента, ТаблицаПоТоварам) Экспорт
	...
	Если РеквизитыДокумента = Неопределено
		ИЛИ ТаблицаПоТоварам = Неопределено Тогда
	...
	Если НЕ (НачалоМесяца(Реквизиты.ДокументПоступленияДата) <> НачалоМесяца(Реквизиты.Период)) Тогда
		Возврат ТаблицаПоТоварам;
	КонецЕсли;
...
Процедура ДополнитьОстаткамиБУЗабалансовыеТаблицуКорректировкиТоваров(Реквизиты, ТаблицаПоТоварам)
	...
	Для Каждого СтрокаТовары Из ТаблицаПоТоварамЗабалансовые Цикл
		ЕстьКонтрагентНаСчете = БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(СтрокаТовары.СчетУчета);
...
Функция ПолучитьТаблицуОстатковБУЗабалансовые(Реквизиты, ТаблицаПоТоварамЗабалансовые)
	...
	Для Каждого СчетУчета Из МассивСчетаУчета Цикл
		ЕстьСкладНаСчете = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(СчетУчета);
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ИспользоватьУправляемыеБлокировки() Тогда
		...
		Для каждого СчетУчета Из МассивСчетаУчета Цикл
			...
			СуммовойУчетПоСкладам 		= БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяСуммовойУчетПоСкладам(СчетУчета);
			ВедетсяУчетПоКонтрагентам 	= БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоКонтрагентам(СчетУчета);
...
Функция ПодготовитьТаблицуДокументаПоКурсуАвансов(ТаблицаРеквизиты, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам, ТаблицаАвансов) Экспорт
	...
	Если Параметры.РеквизитыЗачетАвансов.Количество() = 0 или Параметры.ТаблицаАвансов.Количество() = 0 Тогда
		Возврат Новый Структура("ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам", ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам);
	КонецЕсли; 
	...
	Если Реквизиты.ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить() или ТаблицаИстинныхАвансов.Количество() = 0 Тогда
		Возврат Новый Структура("ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам", ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам);
	КонецЕсли;
	
	ЕдинаяТаблица = ПодготовитьЕдинуюТаблицуПоПереданным(ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам);
	
	Если ЕдинаяТаблица.Количество() = 0 Тогда
		Возврат Новый Структура("ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам", ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоАгентскимУслугам);
	КонецЕсли;	
...
Процедура СформироватьДвиженияВыделениеАванса(ТаблицаВзаиморасчеты, ТаблицаРеквизиты, Движения, Отказ) Экспорт
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СоглашениеОбИзмененииСтоимости") Тогда
		...
		ИмяПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаПоступления_СоглашениеОбИзмененииСтоимости";
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "СоглашениеОбИзмененииСтоимости", "Соглашение об изменении стоимости", 
			ПечатьТорговыхДокументов.ПечатьСоглашенияОбИзмененииСтоимости(ТаблицаСведенийСоглашенияОбИзмененииСтоимости, ОбъектыПечати, ИмяПараметровПечати),,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Акт") Тогда
		...
		СтруктураПараметровПечати.Вставить("ВидДокументаПечати",  "КорректировкаПоступления");
		СтруктураПараметровПечати.Вставить("ИмяПараметровПечати", "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаПоступления_Акт");
		СтруктураПараметровПечати.Вставить("ИмяМакетаПечати",     "Документ.КорректировкаПоступления.ПФ_MXL_Акт");
...
Функция ПолучитьТаблицуСведенийТОРГ12Покупатель(Знач СписокДокументов, Знач ВключатьУслуги) Экспорт
...
Функция ПолучитьТаблицуСведенийСоглашенияОбИзмененииСтоимости(Знач СписокДокументов) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СведенияОДокументе = ТаблицаСведений.Добавить();
		ЗаполнитьЗначенияСвойств(СведенияОДокументе, Выборка);
		СведенияОДокументе.НомерДокумента	  = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерДокумента, Истина, Ложь);
