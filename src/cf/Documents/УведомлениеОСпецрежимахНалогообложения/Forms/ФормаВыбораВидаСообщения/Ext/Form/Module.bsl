...
Процедура ОбработкаВыбораВидаУведомления(Тип, Наименование)
	...
	Если ТипЗнч(Тип) = Тип("ПеречислениеСсылка.ВидыУведомленийОСпецрежимахНалогообложения") Тогда
		...
		Если СозданиеСообщения Тогда
			...
			ОткрытьФорму("Документ.УведомлениеОСпецрежимахНалогообложения.ФормаОбъекта", Новый Структура("Организация,ВидУведомления,UIDФорма1СОтчетность", Организация, ВидУведомления, UIDФорма1СОтчетность), ЭтаФорма.ВладелецФормы);
