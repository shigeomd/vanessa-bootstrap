...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.Текст = ТекстЗапросаТаблицаОС(НомераТаблиц)
		...
		+ ТекстЗапросаВосстановлениеАмортизационнойПремии(НомераТаблиц)
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаОС.Регистратор КАК ДокументРасчетов,
	|	ТаблицаОС.Регистратор.СчетУчетаРасчетовСКонтрагентом КАК СчетРасчетов,
	|	ТаблицаОС.Регистратор.СчетУчетаРасчетовПоАвансам КАК СчетАвансов,
	|	ТаблицаОС.Регистратор.Контрагент КАК Контрагент,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	|	ТаблицаОС.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах КАК РасчетыВУсловныхЕдиницах,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.УчетАгентскогоНДС КАК УчетАгентскогоНДС,
	|	ВЫБОР
	|		КОГДА ТаблицаОС.ВалютаВзаиморасчетов = &ВалютаРеглУчета
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК РасчетыВВалюте,
	|	ТаблицаОС.Регистратор.ПодразделениеОрганизации КАК Подразделение,
	...
	|ГДЕ
	|	ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации = ЛОЖЬ
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаОС.Регистратор,
	|	ТаблицаОС.Регистратор.СчетУчетаРасчетовСКонтрагентом,
	|	ТаблицаОС.Регистратор.СчетУчетаРасчетовПоАвансам,
	|	ТаблицаОС.Регистратор.Контрагент,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.ВидДоговора,
	|	ТаблицаОС.ВалютаВзаиморасчетов,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента.УчетАгентскогоНДС,
	|	ТаблицаОС.Регистратор.ПодразделениеОрганизации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"ТаблицаОС.Регистратор.ДоговорКонтрагента.ВидДоговора",
								БухгалтерскийУчетПереопределяемый.ТекстЗапросаВидДоговора("ТаблицаОС.Регистратор.ДоговорКонтрагента"));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"ТаблицаОС.Регистратор.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах",
								БухгалтерскийУчетПереопределяемый.ТекстЗапросаРасчетыВУЕ("ТаблицаОС.Регистратор.ДоговорКонтрагента"));
...
Функция ТекстЗапросаСписаниеОстаточнойСтоимости(НомераТаблиц)
	...
	|	ТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ВЫБОР
	|		КОГДА ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости)
	|		ИНАЧЕ ТаблицаОС.СчетРасходов
	|	КОНЕЦ КАК СчетСписания,
	|	ВЫБОР
	|		КОГДА ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации
	|			ТОГДА ТаблицаОС.Регистратор.Контрагент
	|		ИНАЧЕ ТаблицаОС.Субконто
	|	КОНЕЦ КАК Субконто,
	|	ВЫБОР
	|		КОГДА НЕ ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации
	|				ИЛИ ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации
	|					И &C2013Года
...
Функция ТекстЗапросаРеализация(НомераТаблиц)
	...
	|	ТаблицаОС.Регистратор.СчетУчетаРасчетовСКонтрагентом КАК КорСчет,
	|	ТаблицаОС.Регистратор.Контрагент КАК КорСубконто1,
	|	ТаблицаОС.Регистратор.ДоговорКонтрагента КАК КорСубконто2,
	|	ТаблицаОС.Регистратор КАК КорСубконто3,
	...
	|	ТаблицаОС.Регистратор.Контрагент КАК Контрагент,
	|	ТаблицаОС.Регистратор.Контрагент КАК Покупатель,
	|	ТаблицаОС.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ТаблицаОС.Регистратор.ПодразделениеОрганизации КАК Подразделение,
	...
	|ГДЕ
	|	(ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации = ЛОЖЬ
	|			ИЛИ ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации = ИСТИНА
	|				И &C2013года
	|				И &ОтражатьВНалоговомУчете)
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаРеализацияНДС(НомераТаблиц)
	...
	|	ТаблицаОС КАК ТаблицаОС
	|ГДЕ
	|	ТаблицаОС.Регистратор.ПравоСобственностиПереходитПослеГосРегистрации = ИСТИНА
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаВосстановлениеАмортизационнойПремии(НомераТаблиц)
...
Функция ПечатьОС1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	|	ПервоначальныеСведенияОбОсновныхСредствахСрезПоследних.СпособНачисленияАмортизации КАК СпособАмортизации,
