...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВидУслуги = Перечисления.ВидыУслугПриИОН.ПредставлениеСправкиОбИсполненииОбязанностейПоУплате Тогда
		ПолучательПредставление = НСтр("ru = 'ФНС'") + ?(ЗначениеЗаполнено(Объект.Организация) И РеквизитыОрганизации.КрупнейшийНалогоплательщик = Истина, " " + Строка(РеквизитыОрганизации.КодНалоговогоОрганаПолучателя), "");
		Если Параметры.Ключ.Пустая() Тогда
			Объект.ФорматОтвета = Перечисления.ФорматОтветаНаЗапросИОН.XML;
		КонецЕсли;
	КонецЕсли;
	Элементы.ФорматОтвета.Доступность = Объект.ВидУслуги <> Перечисления.ВидыУслугПриИОН.ПредставлениеСправкиОбИсполненииОбязанностейПоУплате;
...
Процедура ВидУслугиПриИзменении(Элемент)
	...
	КонецЕсли;
	Элементы.ФорматОтвета.Доступность = Объект.ВидУслуги <> ПредопределенноеЗначение("Перечисление.ВидыУслугПриИОН.ПредставлениеСправкиОбИсполненииОбязанностейПоУплате");
...
&НаКлиенте
Процедура ЗапрашиваемыеНалогиКБКАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
...
Функция РеквизитыОрганизацииНаСервере(Организация)
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация, "КрупнейшийНалогоплательщик, КодНалоговогоОрганаПолучателя, РегистрацияВНалоговомОргане.КодПоОКАТО, РегистрацияВНалоговомОргане.КодПоОКТМО");
