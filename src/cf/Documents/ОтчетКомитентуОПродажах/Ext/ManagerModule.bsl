...
Функция ПолучитьЗакупленные(Реквизиты) Экспорт
	...
	Если Реквизиты.СпособЗаполнения = "ЗаполнитьЗакупленнымиПоДоговору" Тогда
		...
		|	ЗакупленныеТоварыКомитентовОстатки.Номенклатура.ЕдиницаИзмерения,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеАгентскиеУслуги.Ссылка КАК Партия,
	|	ПоступлениеАгентскиеУслуги.Номенклатура КАК Номенклатура,
	|	СУММА(ПоступлениеАгентскиеУслуги.Сумма) КАК СуммаПоДокументу
	|ПОМЕСТИТЬ ВТ_АгентскиеУслуги
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.АгентскиеУслуги КАК ПоступлениеАгентскиеУслуги
	|ГДЕ
	|	ПоступлениеАгентскиеУслуги.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТ_ЗакупленныеТоварыКомитентовОстатки.Партия
	|			ИЗ
	|				ВТ_ЗакупленныеТоварыКомитентовОстатки)
	|	И ПоступлениеАгентскиеУслуги.Контрагент = &Контрагент
	...
	|	"""" КАК Содержание,
	|	МАКСИМУМ(ПоступлениеТовары.СтавкаНДС) КАК СтавкаНДС,
	|	МИНИМУМ(ПоступлениеТовары.НомерСтроки) КАК НомерСтроки,
	|	1 КАК Сумма,
	|	1 КАК СуммаПоДокументу
	|ПОМЕСТИТЬ ВТ_ПорядокСтрок
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТовары
	|ГДЕ
	|	ПоступлениеТовары.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТ_ЗакупленныеТоварыКомитентовОстатки.Партия
	|			ИЗ
	|				ВТ_ЗакупленныеТоварыКомитентовОстатки)
	|	И ПоступлениеТовары.Контрагент = &Контрагент
	...
	|	МАКСИМУМ(ПоступлениеАгентскиеУслуги.СтавкаНДС),
	|	МИНИМУМ(ПоступлениеАгентскиеУслуги.НомерСтроки),
	|	ПоступлениеАгентскиеУслуги.Сумма,
	|	ЕСТЬNULL(ВТ_АгентскиеУслуги.СуммаПоДокументу, 0)
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.АгентскиеУслуги КАК ПоступлениеАгентскиеУслуги
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_АгентскиеУслуги КАК ВТ_АгентскиеУслуги
	|		ПО ПоступлениеАгентскиеУслуги.Ссылка = ВТ_АгентскиеУслуги.Партия
	|			И ПоступлениеАгентскиеУслуги.Номенклатура = ВТ_АгентскиеУслуги.Номенклатура
	|ГДЕ
	|	ПоступлениеАгентскиеУслуги.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТ_ЗакупленныеТоварыКомитентовОстатки.Партия
	...
	|	ПоступлениеАгентскиеУслуги.Содержание,
	|	ПоступлениеАгентскиеУслуги.Сумма,
	|	ВТ_АгентскиеУслуги.СуммаПоДокументу
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Партия,
	|	Номенклатура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ЗакупленныеТоварыКомитентовОстатки.Номенклатура,
	|	ВТ_ПорядокСтрок.Содержание,
	|	ВТ_ЗакупленныеТоварыКомитентовОстатки.ЕдиницаИзмерения,
	|	ВТ_ЗакупленныеТоварыКомитентовОстатки.Партия КАК Партия,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТ_ПорядокСтрок.СуммаПоДокументу, 0) = 0
	|			ТОГДА ВТ_ЗакупленныеТоварыКомитентовОстатки.КоличествоОстаток
	|		ИНАЧЕ ВТ_ЗакупленныеТоварыКомитентовОстатки.КоличествоОстаток * ВТ_ПорядокСтрок.Сумма / ВТ_ПорядокСтрок.СуммаПоДокументу
	|	КОНЕЦ КАК КоличествоОстаток,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВТ_ПорядокСтрок.СуммаПоДокументу, 0) = 0
	|			ТОГДА ВТ_ЗакупленныеТоварыКомитентовОстатки.СуммаОстаток
	|		ИНАЧЕ ВТ_ЗакупленныеТоварыКомитентовОстатки.СуммаОстаток * ВТ_ПорядокСтрок.Сумма / ВТ_ПорядокСтрок.СуммаПоДокументу
	|	КОНЕЦ КАК СуммаОстаток,
	|	ЕСТЬNULL(ВТ_ПорядокСтрок.НомерТабЧасти, 0) КАК НомерТабЧасти,
	|	ЕСТЬNULL(ВТ_ПорядокСтрок.НомерСтроки, 0) КАК НомерСтрокиДокумента,
	|	ЕСТЬNULL(ВТ_ПорядокСтрок.СтавкаНДС, 0) КАК СтавкаНДС,
	|	ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПорядокСтрок КАК ВТ_ПорядокСтрок
	|		ПО ВТ_ЗакупленныеТоварыКомитентовОстатки.Партия = ВТ_ПорядокСтрок.Партия
	|			И ВТ_ЗакупленныеТоварыКомитентовОстатки.Номенклатура = ВТ_ПорядокСтрок.Номенклатура
	|
	|УПОРЯДОЧИТЬ ПО
	|	Поставщик,
	|	ДатаСФ,
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|	ТаблицаТовары.Номенклатура.Услуга КАК Услуга,
...
Процедура СформироватьДвиженияСписаниеРеализованныхТоваровКомитента(ТаблицаТоварыКомитента, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Параметры.Реквизиты.Количество() = 0
		ИЛИ Параметры.ТоварыКомитента.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ТаблицаТоваров.Свернуть("Номенклатура, Услуга, Покупатель, ДатаРеализации", "Количество, Выручка, СуммаПоступления");
...
Функция ПолучитьОстаткиРеализованныеТоварыКомитентов(СтруктураПараметров)
	...
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ИспользоватьУправляемыеБлокировки() Тогда
		...
		ЗначенияБлокировки.Вставить("Контрагент", 			Реквизиты.Контрагент);
		ЗначенияБлокировки.Вставить("ДоговорКонтрагента", 	Реквизиты.ДоговорКонтрагента);

		СтруктураПараметровБлокировки = Новый Структура();
		СтруктураПараметровБлокировки.Вставить("ТипТаблицы",     "РегистрНакопления");
		СтруктураПараметровБлокировки.Вставить("ИмяТаблицы",     "РеализованныеТоварыКомитентов");
		СтруктураПараметровБлокировки.Вставить("ИсточникДанных", РеализованныеТоварыКомитентов);
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ВТ_НомерСтроки_ГТД.Ссылка,
	|	ВТ_НомерСтроки_ГТД.Номенклатура,
	...
	|ВЫБРАТЬ
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаТовары.Номенклатура.Услуга
	|				ТОГДА 0
	|			ИНАЧЕ 1
	...
	|	ВТВознаграждение.ТоварКод,
	|	ВТВознаграждение.ТоварАртикул,
			...
			Если НЕ ЭтоКомиссияПоПродаже Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаКонтрагентов.ДатаВыданногоСчетаФактуры)
					...
					ТекстКонтрагент = ТекстКонтрагент + НСтр("ru = 'Выдан счет-фактура комитенту: № '") 
						+ ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаКонтрагентов.НомерВыданногоСчетаФактуры, Истина, Ложь) 
