...
Функция ТекстЗапросаПервоначальныеСведенияОСИП(НомераТаблиц)
	...
	|	Реквизиты.СчетУчета КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств)
	|			ТОГДА ИСТИНА
...
Функция ПечатьОС1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	|	ДокументПринятиеКУчетуОС.КоэффициентУскоренияБУ,
	|	ДокументПринятиеКУчетуОС.МОЛБУ,
	|	ДокументПринятиеКУчетуОС.ГодоваяНормаАмортизацииБУ КАК НормаАморт,
	|	ДокументПринятиеКУчетуОС.Ответственный,
	...
	|	ДокументПринятиеКУчетуОС.Склад,
	|	ДокументПринятиеКУчетуОС.СпособПоступления,
	|	ДокументПринятиеКУчетуОС.СрокПолезногоИспользованияБУ КАК СрокПолезнИспПриПост,
	|	ДокументПринятиеКУчетуОС.СтоимостьБУ,
