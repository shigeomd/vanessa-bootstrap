...
Процедура ПроверкаЗаполненияКолонокТабличныхЧастейПоСубконто(
	...
	Для Каждого ТекущаяСтрока Из ЭтотОбъект[ИмяТабличнойЧасти] Цикл
			...
			Если (СоответствующаяВидуСубконтоКолонка <> Неопределено) И (НЕ ПустаяСтрока(СоответствующаяВидуСубконтоКолонка)) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Значение) Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						?(Найти(СоответствующаяВидуСубконтоКолонка, "Субконто") = 1, ТекущийВидСубконто.Наименование, ПредставлениеКолонки));
		...
		МассивСчетовУчетаПартий.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств);
		...
		Если ТипЗнч(ДругиеОбязательныеКолонки) = Тип("Структура") Тогда
			Для Каждого ТекущаяКолонка Из ДругиеОбязательныеКолонки Цикл
				...
				Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения И ТекущаяКолонка.Ключ = "Партия" Тогда
...
Процедура ОбновитьСчетаФактуры(Отказ)
	...
	|		ИНАЧЕ &ВалютаРегламентированногоУчета
	|	КОНЕЦ КАК ВалютаДокумента,
	|	ЕСТЬNULL(Док.ДокументОплаты.ДатаВходящегоДокумента, ЕСТЬNULL(Док.ДокументОплаты.Дата, НЕОПРЕДЕЛЕНО)) КАК ДатаДокументаОплаты,
	|	ЕСТЬNULL(Док.ДокументОплаты.НомерВходящегоДокумента, ЕСТЬNULL(Док.ДокументОплаты.Номер, НЕОПРЕДЕЛЕНО)) КАК НомерДокументаОплаты,
	|	Док.ДатаСФ,
	|	Док.НомерСФ,
	|	Док.КорректировочныйСчетФактура,
	|	Док.Исправление,
	...
	|	Док.Продавец,
	|	Док.КодВидаОперации,
	|	ЕСТЬNULL(Док.ДокументОплаты.ДатаВходящегоДокумента, ЕСТЬNULL(Док.ДокументОплаты.Дата, НЕОПРЕДЕЛЕНО)),
	|	ЕСТЬNULL(Док.ДокументОплаты.НомерВходящегоДокумента, ЕСТЬNULL(Док.ДокументОплаты.Номер, НЕОПРЕДЕЛЕНО)),
	|	ВЫБОР
	|		КОГДА &РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСНачисленный)
	|				И Док.ДоговорКонтрагента.ВалютаВзаиморасчетов <> &ВалютаРегламентированногоУчета
	|				И НЕ Док.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ВедетсяУчетПоСкладам = БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетПоСкладам(ПланыСчетов.Хозрасчетный.СырьеИМатериалы);
	...
	Иначе
		...
		СоответствиеСубконтоИКолонки.Вставить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыСтраховыхВзносовИП, "ВидСтраховогоВзносаИП");
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСНачисленный
		...
		Иначе //Авансы
			...
			СтруктураОбязательныхРеквизитов = Новый Структура("СчетУчета, СтавкаНДС, ВидЦенности, Контрагент, ДоговорКонтрагента, СчетФактура, СуммаБезНДС",
				"Счет учета", "Ставка НДС", "Вид ценности", "Контрагент", "Договор контрагента", "Расчетный документ", "Сумма без НДС");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_РасчетыПоНалогамИСборам(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_ТоварыОтгруженные(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_Номенклатура(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_РасчетыПоЗаработнойПлате(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_ОС(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);

	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_НМА(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);

	УчетДоходовИРасходовПредпринимателя.СформироватьДвижения_ПрочиеНепризнанныеРасходы(ПараметрыПроведения, 
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
