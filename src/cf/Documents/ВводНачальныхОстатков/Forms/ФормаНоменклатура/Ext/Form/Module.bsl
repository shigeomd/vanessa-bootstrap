...
Процедура УстановитьПараметрыВыбораНазначенияИспользования(СтрокаТаблицы)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета)
	   И БухгалтерскийУчетВызовСервераПовтИсп.СчетВИерархии(СтрокаТаблицы.СчетУчета,
			ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.СпецодеждаВЭксплуатации")) Тогда
