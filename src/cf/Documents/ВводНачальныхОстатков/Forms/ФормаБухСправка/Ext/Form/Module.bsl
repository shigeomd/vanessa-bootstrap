...
Процедура УправлениеФормойСервер()
	...
	ПорядокПризнанияМатериальныхРасходов	= УчетнаяПолитика.ПорядокПризнанияМатериальныхРасходов(Объект.Организация, Объект.Дата);
...
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицы_БухСправка(СтрокаТаблицы, ПараметрыДокумента)
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета) Тогда
			...
			СтрокаТаблицы.СуммаВРТолькоПросмотр		= НЕ БухгалтерскийУчетВызовСервераПовтИсп.СчетВИерархии(СтрокаТаблицы.СчетУчета,
