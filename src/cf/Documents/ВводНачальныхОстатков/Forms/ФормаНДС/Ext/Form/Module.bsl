...
Процедура ПодготовитьФормуНаСервере()
	...
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСПоАвансамПолученным Тогда 
		...
		Элементы.ДанныеПоНДССтавкаНДС.ПараметрыВыбора = ПараметрыВыбора;
	...
	СчетаНДС = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(МассивСчетов);
	...
	Для Каждого Колонка ИЗ КолонкиТабличнойЧасти Цикл
		Если Найти(Колонка.Имя,"СуммаБезНДС") Тогда
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	МассивДоговоров = ОбщегоНазначения.ВыгрузитьКолонку(Объект.ДанныеПоНДС, "ДоговорКонтрагента", Истина);
...
Процедура ПересчитатьНДС(ИзменяемаяКолонка, КолонкаСуммыБезНДС, СтрокаТаблицы)
	...
	Если Найти(ИзменяемаяКолонка,"СуммаБезНДС")>0 Тогда
...
Процедура ПриИзмененииКонтрагентаСервер(ПараметрыСтроки, Знач ПараметрыДокумента)
	...
	БухгалтерскийУчетПереопределяемый.УстановитьДоговорКонтрагента(ПараметрыСтроки.ДоговорКонтрагента,
...
Процедура ОткрытьФормуРеквизитовСчетаФактуры(ВыбранноеЗначение)
	...
	ОткрытьФорму("Документ.ВводНачальныхОстатков.Форма.ФормаРеквизитыСчетаФактуры", ПараметрыФормы, ЭтаФорма,,,,ОповещениеФормы,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьРеквизитыСчетаФактуры(ДанныеСтроки)
	...
	Иначе	
		РеквизитыСчетаФактурыТекст 	= "<Ввести реквизиты>";
