...
Функция ПодготовитьДанныеФормы()
	...
	Для Каждого ПакетСтраниц Из ПакетыСтраниц Цикл
		...
		Если Лев(ПакетСтраниц.Раздел, 1) = "_" Тогда
			...
			Иначе
				...
				Если ТекущееПодменю = Неопределено Тогда
					ВызватьИсключение СтрЗаменить(НСтр("ru = 'Не найдена группа ""%1""'"), "%1", ИмяПодменю);
				КонецЕсли;
	...
	Если ФайлыПакета = Неопределено Тогда
		ФайлыПакета = ИнформацияПриЗапуске.ИзвлечьФайлыПакета(ПакетСтраниц.ИмяМакета);
	КонецЕсли;
...
Процедура РазместитьСтраницыПакета(ПакетСтраниц, ФайлыПакета)
	...
	Для Каждого ВебСтраница Из ФайлыПакета.ВебСтраницы Цикл
		...
		Для Каждого Картинка Из ФайлыПакета.Картинки Цикл
			...
			Если Длина > 0 И Лев(ПутьККартинке, Длина) = ВебСтраница.ОтносительныйКаталог Тогда
