	...
	Если НастройкиАвторизации.Используется Тогда
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ru = 'ЗаписьПользователя'")),
...
Процедура ПроверитьСоставРолиOData() Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшбки = Символы.НПП + НСтр("ru = 'Следующие права избыточно включены в состав роли:'") + Символы.ПС + Символы.ВК + 
	...
	Если НедостающиеПрава.Количество() > 0 Тогда
		ТекстОшбки = Символы.НПП + НСтр("ru = 'Следующие права должны быть включены в состав роли:'") + Символы.ПС + Символы.ВК + 
...
Функция ПредставлениеИзбыточныхИлиНедоставющихПрав(Знач ОписанияПрав, Знач Отступ)
	...
	Для Каждого КлючИЗначение Из ОписанияПрав Цикл
		...
		Строка = Строка + ОбъектМетаданных.ПолноеИмя() + ": " + СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Права, ", ");
