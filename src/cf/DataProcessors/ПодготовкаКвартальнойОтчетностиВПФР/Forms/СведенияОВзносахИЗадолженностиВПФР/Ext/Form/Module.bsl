...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ТекстСообщенияНеЗаполнеСотрудник = НСтр("ru = 'В строке №%1 списка сотрудников не заполнено поле ""Сотрудник""'");
	...
	Для Каждого СтрокаСотрудник Из СведенияОВзносах Цикл
		Если Не ЗначениеЗаполнено(СтрокаСотрудник.ФизическоеЛицо) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								ТекстСообщенияНеЗаполнеСотрудник,
								ИндексСтроки + 1);
...
#Область ОбработчикиСобытийТаблицыФормыСведенияовзносах
...
Процедура СведенияОВзносахПередУдалением(Элемент, Отказ)
	Отказ = Не Элементы.СведенияОВзносах.ТекущиеДанные.ФиксСтрока;
КонецПроцедуры
...
Процедура ДанныеCSVФайлаВДанныеФормы(ДанныеФайла)	
	...
	Для НомерСтроки = 3 По КоличествоСтрок Цикл
		...
		ДобавитьСтрокуТаблицуЗадолженнсти(СтрокаДанных, СоответствиеСНИЛСФизическимЛицам, СтрокиПоСотрудникам);
	КонецЦикла;
КонецПроцедуры	
...
Процедура ПроверитьВозможностьЗагрузкиДанныхФайла(ДанныеФайла)
	...
	РегНомерИзФалйаФорматированный = Лев(РегНомерПФРИзФайла, 3) + "-" + Сред(РегНомерПФРИзФайла, 4, 3) + "-" + Сред(РегНомерПФРИзФайла, 7);
	...
	Если РегНомерОрганизации <> РегНомерИзФалйаФорматированный Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, РегНомерИзФалйаФорматированный, РегНомерОрганизации);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
...
&НаСервере
Процедура ДобавитьСтрокуТаблицуЗадолженнсти(СтрокаДанных, СоответствиеСНИЛСФизическимЛицам, СтрокиПоСотрудникам)
	ЗначенияКолонокВСтроке = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаДанных, ";");
	...
	Попытка
		...
		Если ФизическоеЛицо = Неопределено Тогда
			...
			Отчетсво = ЗначенияКолонокВСтроке[6];
			
			ФИО = Фамиля + " " + Имя + ?(ПустаяСтрока(Отчетсво), "", " ") + Отчетсво;
