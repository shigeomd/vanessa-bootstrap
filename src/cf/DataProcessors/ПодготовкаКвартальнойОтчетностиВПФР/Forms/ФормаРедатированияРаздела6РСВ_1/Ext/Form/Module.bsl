...
#Область ОбработчикиСобытийТаблицыФормыСведенияозаработкенавредныхитяжелыхработах
...
#Область ОбработчикиСобытийТаблицыФормыДанныераздела66

&НаКлиенте
Процедура ДанныеРаздела66КорректируемыйПериодСтрокойНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ДанныеТекущйеСтроки = Элементы.ДанныеРаздела66.ТекущиеДанные;
	Оповещение = Новый ОписаниеОповещения("КорректируемыйПериодСтрокойЗавершениеВыбора", ЭтотОбъект);
	ПерсонифицированныйУчетКлиент.ОтчетныйПериодНачалоВыбора(ЭтаФорма, ДанныеТекущйеСтроки, "КорректируемыйПериод", "КорректируемыйПериодСтрокой", '20100101',, Оповещение);	
КонецПроцедуры
...
&НаСервере
Функция  СоответсвиеДанныхОбъектаДаннымФормы()
...
Процедура ДанныеИзВременногоХранилищаВДанныеФормы(АдресВоВременномХранилище)
	...
	Если Ошибки <> Неопределено Тогда 
		ЗарплатаКадрыОтображениеОшибок.ПреобразоватьПутиКДаннымВСообщенияхПользователю(
			...
			ЭтотОбъект,
			Ошибки,
			СоответсвиеДанныхОбъектаДаннымФормы());
...
Функция ПредставлениеПериодаСведенийОЗаработке(НомерМесяца, ОтчетныйПериод)
	...
	Иначе	
		МесяцПредставление = Формат(Дата(Год(ТекущаяДата()), НомерМесяца, 1), "ДФ=ММММ");
		Возврат НСтр("ru = 'в т.ч. '") + НРег(Лев(МесяцПредставление, 1)) + Сред(МесяцПредставление, 2);
	КонецЕсли;	
КонецФункции	
...
Функция ДанныеСотрудникаВоВременноеХранилище()	
	...
	ПорядокЗаписей = ПорялокЗаписейСтажа();
...
Функция СтруктураПолейЗаписиОСтаже()
	СтруктураПолей = Новый Структура;
	СтруктураПолей.Вставить("НомерОсновнойЗаписия");
