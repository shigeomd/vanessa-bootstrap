...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	Если Параметры.Комплект.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(Объект.Организация) 
			...
			УстановитьАкивныйКомплектПериода();
		Иначе
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеСостоянияКомлектаОтчетностиПФР" 
...
&НаКлиенте
Процедура СписокДокументовПредаставлениеКорректируемогоПериодаРегулирование(Элемент, Направление, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СписокДокументовПредаставлениеКорректируемогоПериодаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
Процедура ОбработатьАктивизациюСтрокиДокументов()
	...
	Если ОписаниеТекущегоДокумента <> Неопределено 
		...
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПриАктивизацииСтрокиВФормеКвартальнаяОтчетностьВПФР");
...
Процедура КомплектОбъектСостояниеКомплектаПриИзмененииНаСервере()
	...
	СохранитьИзмененияВКомплектеНаСеврере();
КонецПроцедуры
...
Процедура ОткрытьФормуРедактированияКарточкиДокумента()
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда	
		...
		Если ТипЗнч(Объект.СсылкаНаРедактируемыйДокумент) = Тип("ДокументСсылка.ПачкаРазделов6РасчетаРСВ_1") Тогда
			ОткрытьФорму("Обработка.ПодготовкаКвартальнойОтчетностиВПФР.Форма.ФормаРедатированияРаздела6РСВ_1", ПараметрыОткрытияФормы, ЭтаФорма);	
		Иначе	
...
Процедура ЗаписьНаДискПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.КомплектОбъект.РСВ_1) Тогда
		...
		МассивСохранмяемыхОтчетов.Добавить(Объект.КомплектОбъект.РСВ_1);
		
		ОшибкиоОтчета = Новый Массив;
		
		ФайлВыгрузкиВоВременномХранилище = Новый Структура;
		РегламентированнаяОтчетностьКлиент.ВыгрузитьОтчеты(МассивСохранмяемыхОтчетов, , ОшибкиоОтчета, , Истина, ФайлВыгрузкиВоВременномХранилище);
...
Функция ОписаниеФайловКомплекта()
	...
	ПрисоединенныеФайлы.ПолучитьПрикрепленныеФайлыКОбъекту(РедактируемыйКомплектСсылка, ДополнительныеФайлы);
	
	Для Каждого Файл Из ДополнительныеФайлы Цикл  
		Если Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Файл, "ПометкаУдаления") Тогда
			ОписаниеФайлов.Добавить(ПрисоединенныеФайлы.ПолучитьДанныеФайла(Файл, УникальныйИдентификатор, Истина));
		КонецЕсли;	
	КонецЦикла;	
...
Функция РезультатФормированияКорректирующихДокументовВДлительнойОперации(ПараметрыФормированияСведений)
	...
	Если ПараметрыФормированияСведений.ВариантФормирования <> Перечисления.ВариантыФормированияКорректирующихСведенийВПФР.КорректировкаСтажа
		...
		СтруктураПараметров.ФормироватьКорректирующуФормуРСВ = Истина;
	КонецЕсли;	
...
Процедура РезультатПерераспределенияВзносовВДанныеФормы()
	...
	ОбновляемыДокументы = Новый Массив;
	
	Для Каждого СтрокаТаблицы Из Объект.ДокументыКомплекта Цикл
		ОбновляемыДокументы.Добавить(СтрокаТаблицы.Ссылка);
	КонецЦикла;
	
	ОбновитьДанныеШапкиДокументов(ОбновляемыДокументы);
КонецПроцедуры	
...
&НаСервере
Процедура УстановитьАкивныйКомплектПериода()
	ОбработкаОбъект = ДанныеФормыВОбъект();
...
Процедура ПриЧтенииДанныхКомплекта(ТекущийОбъект, ОчищатьПротоколПроверки = Истина)
	...
	Иначе 
		СсылкиНаОбъектыСОшибками = ПерсонифицированныйУчетФормы.СсылкиНаОбъектыСОшибками(ЭтаФорма);
	
		Для Каждого Ссылка Из СсылкиНаОбъектыСОшибками Цикл
			СтрокаСпискаДокументов = СтрокаСпискаДокументовПоСсылке(ЭтаФорма, Ссылка);
			
			Если СтрокаСпискаДокументов <> Неопределено Тогда
				СтрокаСпискаДокументов.ЕстьОшибки = Истина;
			КонецЕсли;	
		КонецЦикла;	
...
Процедура УстановитьСвойстваЭлементовОтображенияКомплектаПоНаличиюФормСЗВ6()
	...
	Элементы.СписокДокументовПредаставлениеКорректируемогоПериода.Видимость = ЕстьФормыСЗВ6 Или ЕстьКорректирующиеСведенияСведения();
...
Процедура УстановитьСвойстваЭлементовОтображенияКомплектаПоПериоду()
	Если ОтчетныйПериод >= '20140101' Тогда
		...
		Элементы.ДанныеРСВИтогиЗаработокРСВ1ИтогиДоаВзносыЗаголовок.Видимость = Ложь;
	...
	Иначе
		Если ОтчетныйПериод >= '20130101' Тогда 
			...
			Элементы.ИтогиПоКатегориямОблагаетсяВзносамиДоПредельнойВелечины.Видимость = Истина;
			Элементы.ИтогиПоКатегориямОблагаетсяВзносамиСвышеПредельнойВелечины.Видимость = Истина;
		...
		Иначе
			...
			Элементы.ИтогиПоКатегориямОблагаетсяВзносамиДоПредельнойВелечины.Видимость = Ложь;
			Элементы.ИтогиПоКатегориямОблагаетсяВзносамиСвышеПредельнойВелечины.Видимость = Ложь;
		КонецЕсли;
		...
		Элементы.ДанныеРСВИтогиЗаработокРСВ1ИтогиДоаВзносыЗаголовок.Видимость = Истина;
...
//	ПерсонифицированныйУчетФормы.ИзменитьВидимостьЭлементовИндикацииОшибок(ЭтотОбъект);
...
Процедура ПриЧтенииДанныхДокумента(ТекущийОбъект)
	...
	УстановитьЗаголовкиВТаблицачФормы();
...
&НаСервере
Процедура УстановитьЗаголовкиВТаблицачФормы()
	Если Элементы.КарточкаДокументаСтраницы.ТекущаяСтраница = Элементы.КарточкаСЗВ Тогда 	
		Если Объект.Сотрудники.Количество() > 0 Тогда
			ОписаниеЗаголовковТаблицы = ОписаниеКолонокЗаголовковТаблицы("Сотрудники");
...
Процедура УстановитьЗаголовокТекущегоДокумента()
	...
	ИначеЕсли ТипЗнч(Объект.СсылкаНаРедактируемыйДокумент) = Тип("ДокументСсылка.ПачкаРазделов6РасчетаРСВ_1") Тогда
		ЗаголовокТекущегоДокумента = НСтр("ru = 'Пачка разделов 6 РСВ-1'");	
...
Процедура УстановитьСвойстваЭлементовТаблицыСотрудники()
	...
	Если ТипЗнч(Объект.СсылкаНаРедактируемыйДокумент) = Тип("ДокументСсылка.ПачкаДокументовСЗВ_6_1") 
		...
		Элементы.СотрудникиОблагаетсяВзносамиДоПредельнойВелечины.Видимость = Ложь;
		Элементы.СотрудникиОблагаетсяСвшеПредельнойВелечины.Видимость = Ложь;
	...
	ИначеЕсли ТипЗнч(Объект.СсылкаНаРедактируемыйДокумент) = Тип("ДокументСсылка.ПачкаДокументовСЗВ_6_4") Тогда	
		...
		Элементы.СотрудникиЗаработок.Видимость = Истина;
		Элементы.СотрудникиОблагаетсяВзносами.Видимость = Ложь;
		Элементы.СотрудникиОблагаетсяВзносамиДоПредельнойВелечины.Видимость = Истина;
		Элементы.СотрудникиОблагаетсяСвшеПредельнойВелечины.Видимость = Истина;
		Элементы.СотрудникиНачисленоСтраховаяВсего.Видимость = Истина;
		Элементы.СотрудникиНачисленоСтраховаяПоКатегорииЧЭСВсего.Видимость = Истина;
		Элементы.СотрудникиУплаченоСтраховаяВсего.Видимость = Истина;
		Элементы.СотрудникиКоэффициентУплатыСтраховая.Видимость = Истина;
...
&НаСервере
Процедура СохранитьИзмененияВКомплектеНаСеврере()
	СохранитьИзмененияВРедактируемойПачке();
...
Функция ОписаниеКорректирующихДокументовКомплекта()
	...
	Для Каждого ОписаниеДокумента Из Объект.ДокументыКомплекта Цикл
		Если ПерсонифицированныйУчетКлиентСервер.ЭтоДокументСведенийОВзносахИСтаже(ОписаниеДокумента.Ссылка)
			И ОписаниеДокумента.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ Тогда
			СтрукутраОписанияДокумента = Новый Структура("Ссылка, ПредставлениеДокумента, КатегорияЗастрахованныхЛиц, ТипДоговора, ТипСведенийСЗВ, КорректируемыйПериод, НомерПачки, РегистрационныйНомерПФРвКорректируемыйПериод");
			
			ЗаполнитьЗначенияСвойств(СтрукутраОписанияДокумента, ОписаниеДокумента);
			
			КорректирующиеДокументы.Добавить(СтрукутраОписанияДокумента);
		КонецЕсли;
	КонецЦикла;	
...
Процедура ДанныеТекущегоДокументаПоСотрудникуВДанныеФормы(АдресВоВременномХранилище)
	...
	СоответсвиеПутиКДаннымТаблиц.Вставить("ЗаписиОСтаже", "Объект.ЗаписиОСтаже");
	СоответсвиеПутиКДаннымТаблиц.Вставить("СведенияОЗаработке", "Объект.СведенияОЗаработке");
	СоответсвиеПутиКДаннымТаблиц.Вставить("СведенияОЗаработкеНаВредныхИТяжелыхРаботах", "Объект.СведенияОЗаработкеНаВредныхИТяжелыхРаботах");
	СоответсвиеПутиКДаннымТаблиц.Вставить("КорректирующиеСведения", "Объект.КорректирующиеСведения");
...
Функция ОписаниеКолонокЗаголовковТаблицыСотрудники()
	...
	ОписаниеЗаголовка = ПерсонифицированныйУчетКлиентСервер.СтруктураОписанияКолонокЗаголовков();
	ОписаниеЗаголовка.ПолеТаблицы = "СотрудникиОблагаетсяВзносамиДоПредельнойВелечины";
	...
	ОписаниеЗаголовка = ПерсонифицированныйУчетКлиентСервер.СтруктураОписанияКолонокЗаголовков();
	ОписаниеЗаголовка.ПолеТаблицы = "СотрудникиОблагаетсяСвшеПредельнойВелечины";
...
Функция ОписаниеКолонокЗаголовковТаблицыИтогиПоКатегориям()
	...
	ОписаниеЗаголовка = ПерсонифицированныйУчетКлиентСервер.СтруктураОписанияКолонокЗаголовков();
	ОписаниеЗаголовка.ПолеТаблицы = "ИтогиПоКатегориямОблагаетсяВзносамиДоПредельнойВелечины";
	ОписаниеЗаголовка.Заголовок = "";
	
	ОписаниеЗаголовковКолонок.Добавить(ОписаниеЗаголовка);
	
	ОписаниеЗаголовка = ПерсонифицированныйУчетКлиентСервер.СтруктураОписанияКолонокЗаголовков();
	ОписаниеЗаголовка.ПолеТаблицы = "ИтогиПоКатегориямОблагаетсяВзносамиСвышеПредельнойВелечины";
	ОписаниеЗаголовка.Заголовок = "";
