...
#Область ОбработчикиСобытийТаблицыФормыСотрудник
...
#Область ОбработчикиСобытийТаблицыФормыКорректирующиедокументы
...
Процедура ОповеститьОФормированииСведений()
	Если ВариантФормирования = ПредопределенноеЗначение("Перечисление.ВариантыФормированияКорректирующихСведенийВПФР.КорректировкаКатегорииПоЗастрахованномуЛицу")
		...
		Если ЕстьКомплектыНеопределенногоСтатуса(Организация, КорректируемыйПериод) Тогда
			Текстпредупреждения = НСтр("ru = 'В корретируемом периоде ни одному из комплектов не установлено состояние ""Сведения отправлены"". 
...
Процедура ОповеститьОРучномРедактированииСведений()
	...
	Для Каждого ОписаниеДокумента Из КорректирующиеДокументы Цикл
		СтрукутраОписанияДокумента = Новый Структура("Ссылка, ПредставлениеДокумента, КатегорияЗастрахованныхЛиц, ТипДоговора, ТипСведенийСЗВ, НомерПачки, РегистрационныйНомерПФРвКорректируемыйПериод");
			
		ЗаполнитьЗначенияСвойств(СтрукутраОписанияДокумента, ОписаниеДокумента);
			
		ОписаниеДокументов.Добавить(СтрукутраОписанияДокумента);
	КонецЦикла;	
