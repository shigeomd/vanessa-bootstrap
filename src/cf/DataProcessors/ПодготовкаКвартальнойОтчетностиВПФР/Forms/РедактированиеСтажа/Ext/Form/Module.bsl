...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПерсонифицированныйУчетФормы.ДополнитьФорму(ЭтаФорма);
...
Процедура ЗаписиОСтажеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ДанныеТекущейСтроки <> Неопределено
		...
		ОткрытьФормуРедатированияЗаписиСтажа(ДанныеТекущейСтроки);
	КонецЕсли;	
КонецПроцедуры
...
Процедура ЗаписиОСтажеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ДанныеТекущейСтроки = Неопределено Тогда
		ТекстПредупреждения = НСтр("ru = 'Для добавления новой записи, спозиционируйтесь на строке сотрудника, по которому хотите добавить запись. Добавление новых сотрудников в комплект осуществляется через форму редактирования комплекта.'");
	...
	ОткрытьФормуРедатированияЗаписиСтажа(Родитель);
...
Процедура ЗаписиОСтажеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	ЗавершитьРедатированиеСтрокиСтажа(ВыбранноеЗначение);
КонецПроцедуры
...
Функция ИменаЭлементовПолейСтажа()
	...
	ОформляемыеПоляСтажа.Добавить("ЗаписиОСтажеНомерДополнителнойЗаписи");
...
Процедура УстановитьДоступностьКомандОтменыИсправлений(Форма)	
	...
	Если ДанныеТекущейСтроки <> Неопределено 
		И ДанныеТекущейСтроки.ФиксСтаж Тогда
...
Процедура ПроверитьЗаписиОСтаже()
	...
	ПерсонифицированныйУчетФормы.ПоместитьОписаниеОшибокВХранилище(ЭтаФорма, Отказ);
	
	Ссылки = Новый Массив;
	Ссылки.Добавить(Справочники.КомплектыОтчетностиПерсучета.ПустаяСсылка());
	ПерсонифицированныйУчетФормы.ПоместитьОшибкиВДанныеФормы(ЭтаФорма, Ссылки);
...
Процедура ОтменитьИсправленияСтажа(Сотрудник = Неопределено)
	...
	Для Каждого Родитель Из СтрокиРодителиПоСотрудникам Цикл
		...
		Если Выборка.НайтиСледующий(СтруктураПоиска) Тогда
			...
			Пока Выборка.Следующий() Цикл 
				СтрокаДетальнойЗаписи = ДобавитьСтрокуДетальнойЗписиПоСтажу(ЗаписиОСтаже, Родитель, Выборка, ИндексДобавляемойСтроки);
...
&НаКлиенте
Процедура ОткрытьФормуРедатированияЗаписиСтажа(ДанныеТекущейСтроки)
	Если ДанныеТекущейСтроки = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗавершитьРедатированиеСтрокиСтажа(РедатируемыеДанные)
	Если РедатируемыеДанные.Отмена Тогда
		Возврат;
	КонецЕсли;
	
	Если РедатируемыеДанные.НоваяСтрока Тогда
		Родитель = ЗаписиОСтаже[РедатируемыеДанные.ИндексСтроки];
	Иначе 
		Родитель = РодительСтроки(ЗаписиОСтаже, ЗаписиОСтаже[РедатируемыеДанные.ИндексСтроки]);
	КонецЕсли;	
	...
	ФиксироватьДанныеСтажаВЦеломПоСотруднику = РедатируемыеДанные.ФиксСтаж И Не Родитель.фиксСтаж;
	
	Если РедатируемыеДанные.Модифицированность Тогда
		Родитель.Модифицированность = Истина;
	...
	Если ЗначениеЗаполнено(РедатируемыеДанные.НомерДополнительнойЗаписи) Тогда
		НовыйИндексСтроки = ИндексСтрокиПоНомеруЗаписи(
								ЗаписиОСтаже, 
								Родитель, 
								ПодчиненныеСтроки,
								РедатируемыеДанные.НомерОсновнойЗаписи, 
								РедатируемыеДанные.НомерДополнительнойЗаписи);	
	Иначе 
		Если ТекущийПериод >= '20140101' Тогда
			ТретийПараметрСтажа = РедатируемыеДанные.ТретийПараметрИсчисляемогоСтажа;
		Иначе
		...
		НовыйИндексСтроки = ИндексСтрокиПоДате(
								...
								Родитель, 
								ПодчиненныеСтроки,
								РедатируемыеДанные.ДатаНачалаПериода,
								ТретийПараметрСтажа);								
	КонецЕсли;	
	
	Если РедатируемыеДанные.НоваяСтрока Тогда
		СтрокаДетальнойЗаписи = ДобавитьСтрокуДетальнойЗписиПоСтажу(ЗаписиОСтаже, Родитель, РедатируемыеДанные, НовыйИндексСтроки);
		ПерсонифицированныйУчетКлиентСервер.УстановитьЗаголовкиВСтрокеТаблицы(ЭтотОбъект, СтрокаДетальнойЗаписи, ОписаниеКолонокЗаголовковТаблицыЗаписиОСтаже());
		...
		КонецЕсли;		
	Иначе 
		СтрокаДетальнойЗаписи = ЗаписиОСтаже[РедатируемыеДанные.ИндексСтроки];
		НомерОсновнойЗаписи = СтрокаДетальнойЗаписи.НомерОсновнойЗаписи;
		ЗаполнитьСтрокуТаблицы(СтрокаДетальнойЗаписи, РедатируемыеДанные);
	КонецЕсли;		
	
	СдвивнутьОсновнуюСтрокуСДополнительными(СтрокаДетальнойЗаписи, НовыйИндексСтроки, ПодчиненныеСтроки, НомерОсновнойЗаписи);
	...
	Модифицированность = РедатируемыеДанные.Модифицированность;
	
	УстановитьДоступностьКомандОтменыИсправлений(ЭтаФорма);
КонецПроцедуры	

&НаКлиенте
Процедура СдвивнутьОсновнуюСтрокуСДополнительными(СдвигаемаяСтрока, НовыйИндексСтроки, ПодчиненныеСтроки, НомерОсновнойЗаписи = Неопределено)
...
Процедура ЗаполнитьСведенияоСтаже()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЕстьДетальныеЗаписи Тогда
			ДобавитьСтрокуДетальнойЗписиПоСтажу(ЗаписиОСтаже, Родитель, Выборка);
		КонецЕсли;				
	КонецЦикла;	
...
&НаКлиентеНаСервереБезКонтекста
Функция СоответствиПолейСтрокТаблицПолямЗаписейСтажа(ТипСтроки)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПодчиненныеСтроки(ЗаписиОСтаже, Родиетль)
	ПодчиненныеСтроки = Новый Массив;
	
	ИндексТекущейСтроки = ЗаписиОСтаже.Индекс(Родиетль);
	...
	Для Индекс = ИндексТекущейСтроки + 1 По КоличествоСтрок - 1 Цикл
		...
		Если ОбрабатываемаяСтрока.ИдентификаторСтрокиРодителя <> Родиетль.ИдентификаторСтрокиСтажа Тогда
			Прервать;
		КонецЕсли;	
...
&НаКлиентеНаСервереБезКонтекста
Функция ДобавитьСтрокуДетальнойЗписиПоСтажу(ЗаписиОСтаже, Родитель, ДанныеСтажа, ИндексСтроки = Неопределено)
...
Процедура ЗаполнитьСтрокуТаблицы(СтрокаТаблицы, ДанныеСтажа)
	СоответствиПолейСтрокТаблицПолямЗаписейСтажа = СоответствиПолейСтрокТаблицПолямЗаписейСтажа(СтрокаТаблицы.ТипСтроки);	
	
	Для Каждого КлючЗначение Из СоответствиПолейСтрокТаблицПолямЗаписейСтажа Цикл
...
Функция ДанныеЗаписиСтажаИзСтрокТаблицы(ЗаписиОСтаже, Родитель, СтрокаДетальныхЗаписей = Неопределено)
	...
	СоответствиПолейСтрокТаблицПолямЗаписейСтажа = СоответствиПолейСтрокТаблицПолямЗаписейСтажа(Родитель.ТипСтроки);
	...
	СоответствиПолейСтрокТаблицПолямЗаписейСтажа = СоответствиПолейСтрокТаблицПолямЗаписейСтажа(СтрокаДетальныхЗаписей.ТипСтроки);
...
Функция ОписаниеЭлементовСИндикациейОшибок() Экспорт
	...
	ОписаниеЭлемента = ПерсонифицированныйУчетФормы.ДобавитьОписаниеЭлементаИндикацииОшибок(
							ОписаниеЭлементовИндикацииОшибок, 
							"ЗаписиОСтаже");
	
	ОписаниеЭлемента.ПутьКХранилищуОшибок = "ЗаписиОСтаже";
	ОписаниеЭлемента.ИмяЭлементаФормы = "ЗаписиОСтаже";
	ОписаниеЭлемента.ВыводитьГиперссылку = Ложь;
				
	Возврат ОписаниеЭлементовИндикацииОшибок;
КонецФункции	

&НаСервере
Функция СоответствиеДанныхОбъектовДаннымФормы(Ссылка) Экспорт
	
	СоответствиеДанныхОбъектаДаннымФормы = Новый Соответствие;                                      
		
	ПерсонифицированныйУчетФормы.ДобавитьЭлементСоответствияДанныхОбъектаДаннымФормы(
		СоответствиеДанныхОбъектаДаннымФормы, 
		Тип("СправочникСсылка.КомплектыОтчетностиПерсучета"), 
		"ЗаписиОСтаже",
		"ЗаписиОСтаже[&ИндексСтрокиЗаписиОСтаже]");
			
	Возврат СоответствиеДанныхОбъектаДаннымФормы;	
КонецФункции	

#КонецОбласти

#Область ОтображниеКолонокзаголовков
