...
&НаКлиенте
Процедура ПоказатьРезультатВыполнения(Результат, ПараметрыВыполнения) Экспорт
	СтандартныеПодсистемыКлиент.ПоказатьРезультатВыполнения(ЭтотОбъект, ПараметрыВыполнения);
КонецПроцедуры
...
Процедура ПолучитьОптимальныеАгрегатКлиент()
	...
	Если Результат.МожноПолучить Тогда
		...
		#Иначе
			...
			Расширение = НРег(Сред(Результат.ИмяФайла, Найти(Результат.ИмяФайла, ".") + 1));
	...
	Для НомерСтроки = 1 По Всего Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			...
			ПодробныйТекстОшибок = ПодробныйТекстОшибок
				...
				+ СтрЗаменить(ЗаголовокОшибки, "%1", ИмяРегистраНакопления)
				+ Символы.ПС
