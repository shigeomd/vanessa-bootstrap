...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкиПредупреждений = ОбщегоНазначенияБП.НастройкиПредупрежденийОбИзменениях("НачинаемРаботатьВ30");
...
Процедура ПриЗакрытии()
	...
	Если НЕ ПоказыватьПриОткрытииПрограммы Тогда
		
		ОбщегоНазначенияБПКлиент.ПоказатьПредупреждениеОбИзменениях("НачинаемРаботатьВ30_82", , НастройкиПредупреждений);
...
Процедура ПолеHTMLДокументаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Найти(ИмяОткрываемойСтраницы, "ShowMaket=") > 0 Тогда
