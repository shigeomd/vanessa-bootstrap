...
Процедура ПКО_ПослеЗагрузки_АвансовыйОтчет(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого ИмяТЧ из МассивТЧ Цикл
		Для каждого СтрокаТЧ из Объект[ИмяТЧ] Цикл
			...
			ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СведенияОНоменклатуре);
		КонецЦикла;
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ВозвратТоваровОтПокупателя(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если КомиссияПоСтрокамТабличнойЧасти<> Неопределено Тогда
		Для каждого Значение из КомиссияПоСтрокамТабличнойЧасти Цикл
			...
			СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
			Документы.ВозвратТоваровОтПокупателя.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ВозвратТоваровОтПокупателя.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета);
...
Процедура ПКО_ПослеЗагрузки_ВозвратТоваровПоставщику(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		...
		КонецЕсли;
	КонецЕсли;	
	ДанныеОбъекта.Вставить("ЭтоКомиссия", ЭтоКомиссия);
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ВозвратТоваровПоставщику.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета);
	КонецЦикла;
	
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ВозвратТоваровПоставщику.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
...
Процедура ПКО_ПослеЗагрузки_ИнвентаризацияТоваровНаСкладе(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ИнвентаризацияТоваровНаСкладе.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ОприходованиеТоваров(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ОприходованиеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
...
Процедура ПКО_ПослеЗагрузки_ОтчетКомиссионераОПродажах(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		...
		Документы.ОтчетКомиссионераОПродажах.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ПеремещениеТоваров(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		...
		Документы.ПеремещениеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета, НовыеСчетаУчета);
	...
	Для каждого СтрокаТЧ из Объект.ТоварыНаКомиссии Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.СкладОтправитель);
		НовыеСчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.СкладПолучатель);
		Документы.ПеремещениеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ТоварыНаКомиссии", СчетаУчета, НовыеСчетаУчета);
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.СкладОтправитель);
		НовыеСчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.СкладПолучатель);
		Документы.ПеремещениеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета, НовыеСчетаУчета);
...
Процедура ПКО_ПослеЗагрузки_ПоступлениеДопРасходов(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		...
		Документы.ПоступлениеДопРасходов.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ПоступлениеТоваровУслуг(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ПоступлениеТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ПоступлениеТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета);
	КонецЦикла;
	
	Для каждого СтрокаТЧ из Объект.Услуги Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.ПоступлениеТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Услуги", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_РеализацияТоваровУслуг(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если КомиссияПоСтрокамТабличнойЧасти<> Неопределено Тогда
		Для каждого Значение из КомиссияПоСтрокамТабличнойЧасти Цикл
			СтрокаТЧ = Объект.Товары.Получить(Значение.Ключ - 1);
			СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура);
			Документы.РеализацияТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
			Если Значение.Значение Тогда
				Если ЗначениеЗаполнено(СчетаУчета.СчетУчета) Тогда
					СтрокаТЧ.СчетУчета = СчетаУчета.СчетУчета;
				КонецЕсли;
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.РеализацияТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета);
	КонецЦикла;
	
	Для каждого СтрокаТЧ из Объект.Услуги Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.РеализацияТоваровУслуг.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Услуги", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_СписаниеТоваров(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.ВозвратнаяТара Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.СписаниеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "ВозвратнаяТара", СчетаУчета);
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.СчетУчета) Тогда
			СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
			Документы.СписаниеТоваров.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ГТДИмпорт(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Товары Цикл
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура);
		Документы.ГТДИмпорт.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Товары", СчетаУчета);
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_КомплектацияНоменклатуры(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого СтрокаТЧ из Объект.Комплектующие Цикл
		...
		СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Объект.Организация, СтрокаТЧ.Номенклатура, Объект.Склад);
		Документы.КомплектацияНоменклатуры.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТЧ, "Комплектующие", СчетаУчета);
