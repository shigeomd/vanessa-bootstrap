...
Функция ЗаполнитьТаблицыДляОстатков(СтруктураПараметров)
	...
	//// Отладка и контроль
	//ТаблицаВзаиморасчетовКредиторка = УчетЗатрат.ВыгрузитьВременнуюТаблицу(Запрос.МенеджерВременныхТаблиц, "ВзаиморасчетыКЗ");
	//ТаблицаОплатаКредиторки = УчетЗатрат.ВыгрузитьВременнуюТаблицу(Запрос.МенеджерВременныхТаблиц, "ОплатаДокументовКЗ_Коэффициенты");
	//ТаблицаПартийМПЗ = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ПартииМПЗиНДС");
	//ТаблицаОстатковОС = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ОстаткиОС");
	//ТаблицаОстатковНМА = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ОстаткиНМА");
	//ТаблицаОстатковНалоговСборов = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ОстаткиНалогиИСборы");
	//ТаблицаРасчетовСПерсоналом = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ОстаткиРасчетовСПерсоналом");
	//ТаблицаРасчетовСПодотчетниками = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ОстаткиПоПодотчетникам");
	//ТаблицаНачДанныхРБП = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "НачальныеДанныеРБП");
	//ОборотыПоНоменклатуреДтКт = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ТабОборотыПоНоменклатуреДтКт");
	//ВзаиморасчетыЗаМПЗ = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ВзаиморасчетыЗаМПЗ");
	//ВзаиморасчетыЗаРБП = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ВзаиморасчетыЗаРБП");
	//// конец отладки
	...
	Для каждого НомерТаблицы Из НомераТаблиц Цикл
		...
		Если Найти(НомерТаблицы.Ключ, "Уничтожение") > 0 Тогда			
			Продолжить;
		КонецЕсли; 
		
		ПозРазделителя = Найти(НомерТаблицы.Ключ, "__");
...
Функция ТекстЗапросаПоОС(СтруктураПараметров, ДополнительныеПараметрыЗапроса, ВременныеТаблицыДляУничтожения)
	...
	Если СтруктураПараметров.ЭтоИндивидуальныйПредприниматель Тогда
		...
		ТекстПолейРеквизитыУСН = "ДанныеОСиОстаточнаяСтоимость.ПервоначальнаяСтоимостьБУ КАК ПервоначальнаяСтоимостьУСН,
		|	ВЫБОР
	...
	Иначе
		...
		ТекстПолейРеквизитыУСН = "ДанныеОСиОстаточнаяСтоимость.ПервоначальнаяСтоимостьНУ КАК ПервоначальнаяСтоимостьУСН,
		|	ВЫБОР
...
Процедура РаспределитьОстаткиМПЗПоДокументамРасчетов(СтруктураПараметров, МенеджерВТ)
	...
	Если НЕ УчетнаяПолитикаДоПерехода.СложныйУчетНДС И УчетнаяПолитикаДоПерехода.СпособОценкиМПЗ = Перечисления.СпособыОценки.ПоСредней Тогда
		...
		Для каждого СтрокаПоНоменклатуре Из ЗапасыНаСкладах Цикл				
			...
			Для каждого СтрокаВзаиморасчетов Из СтрокиВзаиморасчетов Цикл
				...
				СтрокаПартии.Партия           = СтрокаВзаиморасчетов.ДокументРасчетов;
			...
			// Добавляем партию без документа, документ будет создан при заполнении "Ввода остатков"
			Если КоличествоРаспределить + СуммаРаспределить + СуммаНДСРаспределить > 0 Тогда		
	...
	//// отладка и контроль
	//ТаблицаПартийМПЗ = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ПартииМПЗиНДС");
	//ТабВзаиморасчетовПоНоменклатуре = УчетЗатрат.ВыгрузитьВременнуюТаблицу(МенеджерВТ, "ВзаиморасчетыЗаМПЗ");
	////конец отладки
...
Процедура СоздатьДокументыВводаОстатков(ТаблицыДляДокументов, СтруктураПараметров, Отказ)
	...
	Попытка
		...
		РазделыУчета =  ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(ТаблицыДляДокументов.ВыгрузитьКолонку("РазделУчетаДляВводаОстатков"));
...
Функция СоздатьСтруктуруОбщихРеквизитовВводаОстатков(СтруктураПараметров)
	...
	ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = '#Создан автоматически обработкой ""Заполнение начальных остатков для УСН"", дата создания - %1'"), ТекущаяДата());
...
Функция СоздатьСлужебныйДокументРасчетов(СтруктураПараметров)
	...
	ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = '#Создан автоматически обработкой ""Заполнение начальных остатков"", дата создания - %1'"), ТекущаяДата());
...
Функция СоздатьСлужебныйДокументПартии(СтруктураПараметров)
	...
	ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Нстр("ru = '#Служебная партия, для ввода остатков. Создана автоматически обработкой ""Заполнение начальных остатков для УСН"", дата создания - %1'"), ТекущаяДата());
...
Функция СоздатьСтруктуруОбщихПараметровЗапроса(СтруктураПараметров)
	...
	СчетаВсехРасходовУСН = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетаВсехРасходовУСН);
	...
	ПараметрыЗапроса.Вставить("СчетаУчетаМатериалов", БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетУчетаМатериалов, , , , СчетаУчетаМПЗ_МатериалыВЭксплуатации));
	...
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ТоварыВРозничнойТорговлеВПродажныхЦенахАТТ);
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ТоварыВРозничнойТорговлеВПродажныхЦенахНТТ);
	ПараметрыЗапроса.Вставить("СчетаУчетаТоваровНаСкладе", БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетУчетаТоваровНаСкладе, , , , СчетаИсключения));
	...
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости);
	ПараметрыЗапроса.Вставить("СчетаУчетаТоваровОтгруженных", БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетУчетаТоваровОтгруженных, , , , СчетаИсключения));
	
	// Счета приобретения ОС
	СчетаПриобретенияОС = Новый Массив;
	СчетаПриобретенияОС.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств); // 08.04
	...
	СчетаУчетаНалогов = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетаГруппыУчетаНалогов, Ложь, , , СчетаИсключения);
