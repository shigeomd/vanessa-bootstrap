...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрПодтверждение.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ПодтверждениеОператораФСГС Тогда
		...
		КонецЕсли;
		ИдентификаторСпецоператора = Лев(ИдентификаторОтправителя, Найти(ИдентификаторОтправителя, ".") - 1);
		ОрганизацияСтр = "Спецоператор связи" + ?(ПустаяСтрока(ИдентификаторСпецоператора), "", ": " + ИдентификаторСпецоператора);
