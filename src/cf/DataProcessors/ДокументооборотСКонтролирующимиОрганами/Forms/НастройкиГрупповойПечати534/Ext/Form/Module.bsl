...
Функция СформироватьРезультат()
	...
	Возврат Новый Структура("ПечататьДокумент, ПечататьПодтверждениеДатыОтправки, ПечататьРезультатПриема, ПечататьРезультатОбработки, ПечататьСообщениеОбОтзыве, ПечататьСообщениеОНесоответствиях",
							 ПечататьДокумент, ПечататьПодтверждениеДатыОтправки, ПечататьРезультатПриема, ПечататьРезультатОбработки, ПечататьСообщениеОбОтзыве, ПечататьСообщениеОНесоответствиях);
...
Процедура СохранитьНастройки()
	...
	Если Элементы.ПечататьДокумент.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьДокумент", , ПечататьДокумент);
	КонецЕсли;
	
	Если Элементы.ПечататьПодтверждениеДатыОтправки.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьПодтверждениеДатыОтправки", , ПечататьПодтверждениеДатыОтправки);
	КонецЕсли;
	
	Если Элементы.ПечататьРезультатПриема.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьРезультатПриема", , ПечататьРезультатПриема);
	КонецЕсли;
	
	Если Элементы.ПечататьРезультатОбработки.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьРезультатОбработки", , ПечататьРезультатОбработки);
	КонецЕсли;
	
	Если Элементы.ПечататьРезультатОбработки.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьСообщениеОбОтзыве", , ПечататьСообщениеОбОтзыве);
	КонецЕсли;
	
	Если Элементы.ПечататьРезультатОбработки.Доступность Тогда
		ХранилищеОбщихНастроек.Сохранить("НастройкиГрупповойПечатиЦикловОбмена_ПечататьСообщениеОНесоответствиях", , ПечататьСообщениеОНесоответствиях);
	КонецЕсли;
