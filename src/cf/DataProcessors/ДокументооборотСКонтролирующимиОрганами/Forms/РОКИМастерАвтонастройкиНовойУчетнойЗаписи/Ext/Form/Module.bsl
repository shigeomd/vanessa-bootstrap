...
Процедура ПорядковыйНомерПереходаПриИзмененииПродолжение(ЭтоПереходДалее)
	...
	// СтраницыДекорации пока не используются.
...
////////////////////////////////////////////////////////////////////////////////
// ПЕРЕОПРЕДЕЛЯЕМАЯ ЧАСТЬ ПОМОЩНИКА
////////////////////////////////////////////////////////////////////////////////
...
Процедура СертификатШифрованиеФайлОткрытие(Элемент, СтандартнаяОбработка)
	...
	ДокументооборотСКООперацииСФайламиКлиент.ПолучитьСвойстваФайла(СертификатШифрованиеФайл);
...
Процедура ОткрытьСертификатИзСтроки64(СертификатСтрока64)
	...
	ДокументооборотСКООперацииСФайламиКлиент.Base64ВФайл(ОписаниеОповещения, СертификатСтрока64, ".cer");
	Возврат;	
...
Процедура КонтейнерНачалоВыбора(Элемент)
	...
	ДокументооборотСКОКриптографияКлиент.ПолучитьКонтейнерыКлючей(Оповещение);
...
Процедура ПрименитьДанныеСтраницы2(ВыполняемоеОповещение)
	...
	Если НЕ ФайлНастроекРасшифрован Тогда
		...
		ДокументооборотСКОКриптографияКлиент.РасшифроватьPKCS7(ОписаниеОповещения, ФайлНастроекАдресДанных);
...
Процедура ПрименитьДанныеСтраницы2ФайлНастроекРасшифрован(ВыполняемоеОповещение, АдресДанныхРасшифрованногоФайлаСНастройками) 
	...
	ДополнительныеПараметры = Новый Структура("ВыполняемоеОповещение", ВыполняемоеОповещение);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрименитьДанныеСтраницы2Завершение", ЭтотОбъект, ДополнительныеПараметры);
	
	// формируем таблицу исправлений на основе расшифрованного файла настроек
	КонтекстЭДОКлиент.СформироватьТаблицуИсправленийНаОсновеФайлаНастроек(, АдресДанныхРасшифрованногоФайлаСНастройками, Истина, ОписаниеОповещения);
...
Функция ПрименитьДанныеСтраницы6(ВыполняемоеОповещение)
	...
	КонтекстЭДОКлиент.ПрименитьИзмененныеНастройки(НоваяУчетнаяЗапись, ТаблицаИсправлений, Ложь, Истина, ОписаниеОповещения);
...
Процедура ЗарегистрироватьСертификатСПривязкойКЗакрытомуКлючу(ВыполняемоеОповещение, Сертификат, СертификатКонтейнер)
	...
	Если ТипЗнч(Сертификат) <> Тип("Строка") Тогда
		...
		ДокументооборотСКООперацииСФайламиКлиент.Base64ВФайл(ОписаниеОповещения, Сертификат.Сертификат);
		Возврат;
...
Процедура ЗарегистрироватьСертификатСПривязкойКЗакрытомуКлючуПоИмениФайла(ВыполняемоеОповещение, СертификатИмяФайла, СертификатКонтейнер)
	...
	ДокументооборотСКОКриптографияКлиент.ИмпортироватьСертификат(ОписаниеОповещения, СертификатИмяФайла, "MY");
...
Процедура ЗарегистрироватьСертификатСПривязкойКЗакрытомуКлючуПоИмениФайлаПослеИмпортированияСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	ДокументооборотСКОКриптографияКлиент.СвязатьСертификатСКонтейнеромКлючей(ОписаниеОповещения, Результат.СвойстваСертификата, СертификатКонтейнер);
