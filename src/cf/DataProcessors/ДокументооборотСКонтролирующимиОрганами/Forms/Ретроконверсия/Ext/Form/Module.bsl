...
Процедура ОткрытьФайлСведений()
	...
	Если НРег(Вложение.РасширениеФайла) = "txt" Тогда
...
Функция ПолучитьСтрокуВложенияНаСервере(Основание)
	...
	//Если Результат.Свойство("ТипСодержимогоФайла") Тогда 
	//	Результат.Вставить("ЭтоXML", Результат.ТипСодержимогоФайла = Перечисления.ТипыСодержимогоФайлов.xml);
	//КонецЕсли;
	
	КонтекстЭДОСервер.УдалитьВременныйФайл(ИмяВременногоФайла);
