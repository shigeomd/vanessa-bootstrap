...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоФорматXML Тогда
		...
		Если УзелПротокол <> Неопределено Тогда
			...
			Если УзелТитульный <> Неопределено Тогда
				...
				ДатаВремяОтправки = Формат(ДатаВремяИзСтроки(ЗначенияЭлементов["dt_send"]), "ДЛФ=DDT; ДП=-");
				...
				ИЛИ (НРег(СтатусПротокола) = "warnings"
				   И  (Найти(НРег(СообщениеПротокола), "ошибок не обнаружено") > 0
				   ИЛИ Найти(НРег(СообщениеПротокола), "отчет принят") > 0)) Тогда
				...
				Если ЗначенияЭлементов["dt_load"] <> Неопределено Тогда
					ТекстПротокола = ТекстПротокола + "Дата при~ма отч~та: " + Формат(ДатаВремяИзСтроки(ЗначенияЭлементов["dt_load"]), "ДЛФ=DT; ДП=-") + Символы.ПС;
				КонецЕсли;
				...
				Если ЗначенияЭлементов["dt_send"] <> Неопределено Тогда
					ТекстПротокола = ТекстПротокола + "Дата передачи файла: " + Формат(ДатаВремяИзСтроки(ЗначенияЭлементов["dt_send"]), "ДЛФ=DT; ДП=-") + Символы.ПС;
				КонецЕсли;
	...
	Если НЕ ЭтоФорматXML Тогда
		...
		// получаем признак того, что протокол положительный
		Если Найти(ВРег(ТекстПротокола), ВРег("Отчёт не принят")) > 0 ИЛИ Найти(ВРег(ТекстПротокола), ВРег("Отчет не принят")) > 0 Тогда
			ПротоколЯвляетсяПоложительным = "Нет";
			Элементы.НадписьПротоколЯвляетсяПоложительным.ЦветТекста = Новый Цвет(255, 0, 0);
		Иначе
			ПротоколЯвляетсяПоложительным = "Да";
