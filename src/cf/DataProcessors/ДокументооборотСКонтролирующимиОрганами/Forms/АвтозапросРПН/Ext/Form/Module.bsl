...
Процедура ПриОткрытии(Отказ)
	...
	Оповестить("Запись_ОтправкиРПН", ПараметрыОповещения, ОтправкаОбъект.Ссылка);
	...
	Если ЭтаФорма.ВладелецФормы <> Неопределено 
		...
		РегламентированнаяОтчетностьКлиент.ОбновитьПанельСостоянияОтправкиВРегламентированномОтчете(ЭтаФорма.ВладелецФормы , "РПН");
	КонецЕсли;
...
Процедура ОбновитьПанельЗапроса()
	...
	Если ЧислоПопыток = 3 Тогда
		Если ОтправкаОбъект.СтатусПроверкиНаПортале = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.Сдан") ИЛИ
			ПротоколЗаполнен(ОтправкаОбъект.Ссылка) Тогда
			НадписьОсталосьСекунд = НСтр("ru = 'Обработка отчета порталом не завершена. " +
				"Попробуйте через некоторое время использовать в отчете кнопку ""Обновить состояние отправки"".'");
		Иначе
			НадписьОсталосьСекунд = НСтр("ru = 'Результатов отправки отчета получить не удалось. " +
				"Возможно, сервер перегружен. Попробуйте через некоторое время использовать в отчете кнопку ""Обновить состояние отправки"".'");
		КонецЕсли;
	Иначе
...
Процедура ДействияПослеОбновленияРезультата()
	...
	Если ОтправкаОбъект.СтатусОтправки = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.Отправлен") Тогда
		...
		Если ЧислоПопыток = 3 Тогда
			...
			Если ОтправкаОбъект.СтатусПроверкиНаПортале = ПредопределенноеЗначение("Перечисление.СтатусыОтправки.Сдан") ИЛИ
				ПротоколЗаполнен(ОтправкаОбъект.Ссылка) Тогда
				КонтекстЭДОКлиент.ПоказатьПротоколОбработкиПоСсылкеИсточникаДляРПН(ОтправкаОбъект.Ссылка);
			КонецЕсли;
		КонецЕсли;
...
Процедура ОжиданиеЗапроса()
	...
	Если ЧислоСекунд < 1 Тогда
		...
		КонтекстЭДОКлиент.ОбновитьРезультатОтправленногоОтчетаРПН(ЭтаФорма, ЗначениеCookie, , EMail, Пароль, ОписаниеОповещения);
		Возврат;
...
Процедура ОжиданиеЗапросаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ПараметрыОповещения.Вставить("Ссылка", ОтправкаОбъект.ОтчетСсылка);
		ПараметрыОповещения.Вставить("Организация", ОтправкаОбъект.Организация);
		Оповестить("Редактирование_ОтправкиРПН", ПараметрыОповещения, ОтправкаОбъект.Ссылка);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ПараметрыОповещения.Вставить("Ссылка", ОтправкаОбъект.ОтчетСсылка);
	ПараметрыОповещения.Вставить("Организация", ОтправкаОбъект.Организация);
	Оповестить("Завершение автообмена с сервером РПН", ПараметрыОповещения, ОтправкаОбъект.Ссылка);
