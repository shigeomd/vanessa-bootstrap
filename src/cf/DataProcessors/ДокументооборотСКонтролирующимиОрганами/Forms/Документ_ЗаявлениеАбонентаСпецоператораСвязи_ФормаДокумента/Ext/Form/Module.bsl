...
Процедура ПолучателиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ ТолькоПросмотр Тогда
		...
		Если КонтекстЭДОКлиент.ТекущаяСтрокаВыбрана(Объект.Получатели,ТекущаяСтрока,"редактирования") Тогда
			ЭлементКоллекции	 = Объект.Получатели.НайтиПоИдентификатору(ТекущаяСтрока);
			НаправлениеДействие	 = "РедактироватьР";
...
Процедура СкопироватьНаправление(Команда)
	...
	Если КонтекстЭДОКлиент.ТекущаяСтрокаВыбрана(Объект.Получатели,ТекущаяСтрока,"копирования") Тогда
		ЭлементКоллекции = Объект.Получатели.НайтиПоИдентификатору(ТекущаяСтрока);
		Если ЭлементКоллекции.ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФНС") Тогда
			...
			Модифицированность = Истина;
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбранное действие допустимо только для направления ФНС.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура УдалитьНаправление(Команда)
	...
	Если КонтекстЭДОКлиент.ТекущаяСтрокаВыбрана(Объект.Получатели,ТекущаяСтрока,"удаления") Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удалить направление %1?'"),Элементы.Получатели.ТекущиеДанные.ТипПолучателя), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
	КонецЕсли;
...
Процедура ВосстановитьНаправления(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Восстановить направления?'");
...
Процедура ЗаполнитьСписокУЦ()
	...
	Элементы.СпецоператорИУЦ.СписокВыбора.ЗагрузитьЗначения(КонтекстЭДОКлиент.ПолучитьСпецоператорыСвязи(СтруктураДанныхСпецоператорыСвязи.СпецоператорыСвязи,СтруктураДанныхСпецоператорыСвязи.Макет));
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.СпецоператорСвязи) И НЕ СтруктураДанныхСпецоператорыСвязи.ЗначениеЗаполненияСпецоператораСвязи = Неопределено Тогда
...
Процедура ПриОткрытииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДанныхСпецоператорыСвязи = Новый Структура("СпецоператорыСвязи,Макет,ТекстМакетаСоглашение,ЗначениеЗаполненияСпецоператораСвязи",
		ДанныеЗаполнения.СпецоператорыСвязи,ДанныеЗаполнения.МакетПараметрыСпецоператоровСвязи,ДанныеЗаполнения.ТекстМакетаСоглашение,ДанныеЗаполнения.ЗначениеЗаполненияСпецоператораСвязи);
...
Процедура ПроверитьЗаполнениеДокумента(ВыполняемоеОповещение)
	...
	Если НЕ КонтекстЭДОКлиент.ПроверитьСНИЛС(Объект.ВладелецЭЦПСНИЛС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Некорректно указан СНИЛС сотрудника. Не соответствует маске 999-999-999 99'"), ,"ВладелецЭЦПСНИЛС");
		ИтогПроверки = Ложь;
	...
	Если ПустаяСтрока(Объект.ВладелецЭЦПДатаВыдачиДокумента) ИЛИ Объект.ВладелецЭЦПДатаВыдачиДокумента = '00010101' Тогда
	...
	Если Объект.ВладелецЭЦПДатаВыдачиДокумента > КонтекстЭДОКлиент.ТекущаяДатаНаСервере() ИЛИ Год(Объект.ВладелецЭЦПДатаВыдачиДокумента) < 1900 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Указана недопустимая дата выдачи документа удостоверяющего личность.'"), ,"ВладелецЭЦПДатаВыдачиДокумента");
		ИтогПроверки = Ложь;
	КонецЕсли;
...
Функция ПреобразоватьПредставлениеАдреса(ПредставлениеАдреса)
	...
	Для ПозСим = 1 По СтрДлина(СтрПолеАдреса) Цикл
		Если Найти("0123456789", Сред(СтрПолеАдреса, ПозСим, 1)) > 0 Тогда
...
Процедура ОткрытьФормуНаправления(ТекущаяСтрока = Неопределено)
	...
	КонтекстЭДОКлиент.ОткрытьФормуНаправления(ЭтаФорма,ТекущаяСтрока,КонтекстЭДОКлиент,ДанныеОрганизации);
...
Процедура УдалитьНаправлениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ЭлементКоллекции		 = Объект.Получатели.НайтиПоИдентификатору(ТекущаяСтрока);
		ИндексЭлементаКоллекции	 = Объект.Получатели.Индекс(ЭлементКоллекции);
		Объект.Получатели.Удалить(ИндексЭлементаКоллекции);
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ОтправитьЗаявлениеНаСерверЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.СформироватьИОтправитьЗаявление(Объект, ИдентификаторАбонента,, ОписаниеОповещения, ФормироватьЗакрытыйКлючИЗапросНаСертификат);
...
Процедура ПолучитьЗарегистрироватьОтвет()
	...
	КонтекстЭДОКлиент.ОбновитьСостояниеЗаявления(ОписаниеОповещения, ЭтаФорма, Объект);
...
Процедура ПреобразоватьАдресКодРегиона(Адрес,ТипАдреса,ИтогПроверки)
	...
	Иначе
		...
		Если ПустаяСтрока(КодРегиона) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1, не верно указан регион.'"),?(ТипАдреса = 0," Юридический адрес"," Фактический адрес")), ,?(ТипАдреса = 0,"ЮрАдрес","ФактАдрес"));
			ИтогПроверки = Ложь;
		Иначе
...
Процедура ОчиститьРеквизитыФормы() 
	...
	Объект.РегНомерПФР                         = "";
	Объект.РегНомерФСС                         = "";
	Объект.ПризнакКрупнейшегоНалогоплательщика = Ложь;
...
Процедура ЗаполнитьДанныеВладельцаЭЦП()
	...
	ДанныеСотрудника = ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ПолучитьДанныеСотрудника(ТипЗаполнения, ДанныеОрганизации, Объект.ВладелецЭЦП);
...
Процедура ЗаполнитьДанныеПоОрганизации(ДанныеЗаполнения)
	...
	Если ТипОрганизации Тогда
		...
		Объект.ПризнакКрупнейшегоНалогоплательщика	= ДанныеОрганизации.КрупнейшийНалогоплательщик;
	...
	КонтекстЭДОКлиент.ПоказатьДопКодФСС(ДанныеОрганизации.РеквизитДопКодФСС,Элементы.ДополнительныйКодФСС,Объект.ПризнакОбособленногоПодразделения,Объект.РегНомерФСС);
...
Процедура ОбновитьЭлементыСтраницыОтчетностьВРПН(Форма)
	...
	ДоступностьЭлементов = 
		ДоступностьПодачиЗаявкиНаСертификатДляРПН 
		И Объект.ПодатьЗаявкуНаПодключениеРПН;
...
Процедура ДобавитьЖелтыеСтроки()
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления("Изменяемые настройки подключения",,,,,0); // заголовок
	...
	Для сч = 0 По КоличествоСтрок - 1 Цикл
		...
		Если СтрокаТаблицаДанных.ЭтотПараметрИзменился Тогда
			ДобавитьСтрокуВТаблицуДанныхЗаявления(СтрокаТаблицаДанных.НазваниеРеквизита, СтрокаТаблицаДанных.ЗначениеРеквизита, СтрокаТаблицаДанных.ЭтотПараметрИзменился, Ложь, Истина);
		КонецЕсли;
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления("",,,,,КоличествоИзмененныхСтрок + 1); // заголовок
...
&НаСервере
Процедура ДобавитьСтрокуВТаблицуДанныхЗаявления(
		ИзмененныйРеквизит,  
		ЗначениеРеквизита = "",
...
Процедура СформироватьТаблицуИзмененныхДанных()
	...
	// Общие сведения
	ДобавитьСтрокуВТаблицуДанныхЗаявления("Общие сведения", , , Истина); // заголовок 
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.Криптопровайдер, ОтображаемоеНазваниеКриптопровайдера);
	
	// Регистрационный номер программы
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.НомерОсновнойПоставки1С, Объект.НомерОсновнойПоставки1с);
	
	ПереизданиеСертификата = ЭтотПараметрИзменился(Перечисления.ПараметрыПодключенияК1СОтчетности.ПереизданиеСертификата);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ПереизданиеСертификата, ПереизданиеСертификата);
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ПродлениеАбонентскогоСертификата, ПродлитьАбонентскийСертификат, ПродлитьАбонентскийСертификат);
	
	ПродлитьЛицензию = ЭтотПараметрИзменился(Перечисления.ПараметрыПодключенияК1СОтчетности.ПродлениеЛицензии);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ПродлениеЛицензии, ПродлитьЛицензию, ПродлитьЛицензию);
	
	// Сведения об организации
	ДобавитьСтрокуВТаблицуДанныхЗаявления("Сведения об организации", , , Истина); // заголовок 
	
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КраткоеНаименование, Объект.КраткоеНаименование);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ПолноеНаименование, Объект.ПолноеНаименование);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ИНН, Объект.ИНН, Ложь); // ИНН запрещено менять
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КПП, Объект.КПП);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ОГРН, Объект.ОГРН);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.РегНомерПФР, Объект.РегНомерПФР);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.РегНомерФСС, Объект.РегНомерФСС);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.АдресЮридический, 
		РегламентированнаяОтчетностьКлиентСервер.ПредставлениеАдресаВФормате9Запятых(Объект.АдресЮридический, Истина));
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодРегионаФСРАР, Объект.КодРегионаФСРАР);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ТелефонОсновной, Объект.ТелефонОсновной);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ТелефонДополнительный, Объект.ТелефонДополнительный);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ЭлектроннаяПочта, Объект.ЭлектроннаяПочта); 
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦП, ФИОВладельцаЭЦП);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДолжность, Объект.ВладелецЭЦПДолжность);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦППодразделение, Объект.ВладелецЭЦППодразделение);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСНИЛС, Объект.ВладелецЭЦПСНИЛС);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПВидДокумента, Объект.ВладелецЭЦПВидДокумента, Ложь);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПСерияДокумента, Строка(Объект.ВладелецЭЦПСерияДокумента), Ложь);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПНомерДокумента, Строка(Объект.ВладелецЭЦПНомерДокумента), Ложь);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПДатаВыдачиДокумента, Объект.ВладелецЭЦПДатаВыдачиДокумента, Ложь);
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ВладелецЭЦПКемВыданДокумент, Объект.ВладелецЭЦПКемВыданДокумент, Ложь);
	...
	Если ОператорПоддерживаетСМСУведомление Тогда
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.ТелефонМобильный, Объект.ТелефонМобильный);
	КонецЕсли;
	
	// Сведения о контролирующих органах
	ДобавитьСтрокуВТаблицуДанныхЗаявления("Сведения о контролирующих органах", , , Истина); // заголовок
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВФНС, СдаватьВФНС);
	Если СдаватьВФНС Тогда
		КодыФНСПрописью = КодыФНСПрописьюКлиент(СтрокиСФНС);
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодыФНС, КодыФНСПрописью);
	Иначе
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодыФНС, "", Ложь);
	КонецЕсли;
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВПФР, СдаватьВПФР);
	Если СдаватьВПФР Тогда
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодПФР, СтрокиСПФР[0].КодПолучателя);
	Иначе
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодПФР, "", Ложь);
	КонецЕсли;
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВФСС, СдаватьВФСС);
	...
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВРосстат, СдаватьВРосстат);
	Если СдаватьВРосстат Тогда
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодРосстата, СтрокиСРосстат[0].КодПолучателя);
	Иначе
		ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.КодРосстата, "");
	КонецЕсли;
	
	// Росалкогольрегулирование
	СдаватьВФСРАР = Объект.ПодатьЗаявкуНаСертификатДляФСРАР;
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВФСРАР, СдаватьВФСРАР);
	
	// Росприроднадзор
	СдаватьВРПН = Объект.ПодатьЗаявкуНаПодключениеРПН;
	ДобавитьСтрокуВТаблицуДанныхЗаявления(Перечисления.ПараметрыПодключенияК1СОтчетности.СдаватьВРПН, СдаватьВРПН);
