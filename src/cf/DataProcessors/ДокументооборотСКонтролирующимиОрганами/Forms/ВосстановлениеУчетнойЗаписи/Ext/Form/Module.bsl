...
Процедура КоманднаяПанельМастерДалее(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаИнформацияОбУчетнойЗаписи Тогда
		...
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВыборКлюча Тогда
			...
			Если МастерДалее Тогда
				// По идентификатору документооборота и спецоператору 
		...
		ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаВыборСпецоператораСвязи Тогда
			Если Не ЗначениеЗаполнено(Спецоператор) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru = 'Укажите спецоператора связи'"), ,"Спецоператор");
				МастерДалее = Ложь;
			КонецЕсли;
...
Функция ПолучитьКлючиЭлектроннойПодписиВМоделиСервисаНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		"ЭлектроннаяПодписьВМоделиСервиса.ПолучитьДействующиеКлючи",
		СтруктураПараметров,
		НаименованиеЗадания);
...
Процедура ЗагрузитьПодготовленныеДанныеНаСервере()
	...
	ИначеЕсли ЕстьКлючиСДругимИдентификатором Тогда
		...
		Если Лев(СокрЛП(СтандартныеПодсистемыСервер.ВерсияБиблиотеки()) + ".", 4) = "2.1." Тогда
			НайденныеИдентификаторыПриложений = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СтроковыеФункцииКлиентСервер").ПолучитьСтрокуИзМассиваПодстрок(МассивИдентификаторовПриложений);
		Иначе
			НайденныеИдентификаторыПриложений = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("СтроковыеФункцииКлиентСервер").СтрокаИзМассиваПодстрок(МассивИдентификаторовПриложений);
		КонецЕсли;
...
Функция ПолучитьИдентификаторАбонента(КлючЭлектроннойПодписи, СпецоператорСвязи)
	...
	Иначе
		...
		КонтекстЭДОКлиент.РегистрацияПолучитьОтветНаЗаявлениеСервер(ДокументЗаявление, ИдентификаторАбонента, Неопределено, Истина);
	КонецЕсли;
