...
Функция ВыбранныеУчетныеЗаписи(Форма, СтраницаФормы = Неопределено)
	...
	Если СтраницаФормы = Неопределено Тогда
		//СтраницаФормы = Элементы.ПанельФормы.ТекущаяСтраница;
	КонецЕсли;
...
Процедура УстановитьПараметрыПроизвольныхЗапросов(сохрТекущаяСтраницаПанелиФормы, сохрТекущаяСтраницаПанелиФНС, сохрТекущаяСтраницаПанелиПФР, сохрТекущаяСтраницаПанелиФСГС, ВыбранныеУчетныеЗаписи = Неопределено)
	...
	Если сохрТекущаяСтраницаПанелиФормы = "ГруппаФНС" Тогда 
		...
		ИначеЕсли сохрТекущаяСтраницаПанелиФНС = "ГруппаПредставления" Тогда
			...
			ЗаполнитьЗначенияСвойств(Представления, ПолучитьНастройкиДляДинамическогоСписка("Представления"));
		...
		ИначеЕсли сохрТекущаяСтраницаПанелиФНС = "ГруппаУведомления" Тогда
			...
			//ЗаполнитьЗначенияСвойств(Уведомления, ПолучитьНастройкиДляДинамическогоСписка("Уведомления"));
	...
	ИначеЕсли сохрТекущаяСтраницаПанелиФормы = "ГруппаПрочее" Тогда
		...
		//ТранспортноеСообщение.Параметры.УстановитьЗначениеПараметра("Организация", ВыбранныеУчетныеЗаписи);
		...
		Иначе
			ТранспортноеСообщение.Отбор.Элементы.Очистить();
			ЭлементОтбора = ТранспортноеСообщение.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		КонецЕсли;
...
Процедура УправлениеЗакладкойУведомлений(Вид = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Видимость = Ложь; // документ Уведомление о спецрежимах налогообложения
	КонецЕсли;	
	...
	Если Вид = Неопределено Тогда
		Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
			Вид = "Документы";
		ИначеЕсли ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
			Вид = "ПрочиеУведомления";
		ИначеЕсли НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) И НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
			Вид = "Файлы";
		КонецЕсли;
	КонецЕсли;
	...
	//
	Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Пометка 							= Ложь;
	Элементы.ТаблицаУведомленияКомандаОткрытьФайлы.Пометка 								= Ложь;
	Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Пометка 					= Ложь;
	Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Пометка 	= Ложь;
	...
	Если Вид = "Все" ИЛИ Вид = "Файлы" Тогда
		ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                              ...
		                              |	И ЭлектронныеПредставленияРегламентированныхОтчетов.ВидОтчета.ТипДокумента В (ЗНАЧЕНИЕ(Перечисление.ТипыОтправляемыхДокументов.УведомлениеОКонтролируемыхСделках), ЗНАЧЕНИЕ(Перечисление.ТипыОтправляемыхДокументов.ИсходящееУведомлениеФНС), ЗНАЧЕНИЕ(Перечисление.ТипыОтправляемыхДокументов.УведомлениеОРозничныхЦенахНаТабак))";
	...
	// тогда добавляям в запрос для динамического списка запрос по документу Уведомление о контролируемых сделках.
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
		...
		Если Вид = "Все" ИЛИ Вид = "Документы" Тогда
	...
	Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
		
		Если Вид = "Все" Тогда
			ТекстЗапроса = ТекстЗапроса + " ОБЪЕДИНИТЬ ВСЕ ";
		КонецЕсли;
		
		Если Вид = "Все" ИЛИ Вид = "ПрочиеУведомления" Тогда
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ
										  |	ВЫБОР
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "УведомлениеОСпецрежимахНалогообложения", ИмяДокументаУведомлениеОСпецрежимахНалогообложения);
	...
	Если СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак() Тогда
		...
		Если Вид = "Все" ИЛИ Вид = "УведомленияОРозничныхЦенахНаТабак" Тогда
			ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ
										  |	ВЫБОР
										  ...
										  |			КОНЕЦ
										  |	КОНЕЦ КАК СтатусКартинка,
										  |	РегламентированныйОтчет.ДатаНачала КАК ДатаСоздания,
										  ...
										  |	ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УведомлениеОРозничныхЦенахНаТабачныеИзделия) КАК ВидУведомления
										  |ИЗ
										  ...
										  |	И РегламентированныйОтчет.ИсточникОтчета  = ""РегламентированныйОтчетРозничныеЦеныНаТабак""";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяДокументаУведомлениеОСпецрежимахНалогообложения = 
...
Процедура ЗаполнитьУчетныеЗаписи()
	...
	Иначе
		...
		Для ОбратныйИндекс = 1 По КоличествоУчетныхЗаписей Цикл
			...
			И ДопустимыеУчетныеЗаписиРПН.Найти(ТекЗапись) = Неопределено Тогда
...
Процедура ОсновнаяКоманднаяПанельОбменятьсяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	//Если ПолученныеТранспортныеСообщения = Неопределено ИЛИ ТипЗнч(ПолученныеТранспортныеСообщения) <> Тип("Массив") Тогда
	//	МассивПолученныхТС = Новый Массив;
	//	ПолученныеТранспортныеСообщения = МассивПолученныхТС;
	//КонецЕсли;
...
Процедура ОсуществитьОбменПоВыбраннойУчетнойЗаписи(ВыбранныеУчетныеЗаписи, СписокПолученныхТС, НомерОбъекта)
	...
	Иначе
		...
		ДополнительныеПараметры = Новый Структура("ВыбранныеУчетныеЗаписи, СписокПолученныхТС, НомерОбъекта", ВыбранныеУчетныеЗаписи, СписокПолученныхТС, НомерОбъекта);
		ОписаниеОповещения = Новый ОписаниеОповещения("ОсуществитьОбменПоВыбраннойУчетнойЗаписиЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		КонтекстЭДОКлиент.ОсуществитьОбменПоУчетнойЗаписи(ВыбраннаяУчетнаяЗапись, , СписокПолученныхТС, ОписаниеОповещения);
...
Процедура ТаблицаИсходящиеДокументыПФРПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.Видимость Тогда
		Если ЦиклыОбменаИсходящиеПФР.Параметры.Элементы.Найти(Новый ПараметрКомпоновкиДанных("СсылкаДляОтбора")) <> Неопределено Тогда
			ЦиклыОбменаИсходящиеПФР.Параметры.УстановитьЗначениеПараметра("СсылкаДляОтбора", Элементы.ТаблицаИсходящиеДокументыПФР.ТекущаяСтрока);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьЗначенияЗаполнения(ТипПереписки)
	...
	Если ТипПереписки = "СФНС" Тогда 
		Возврат Новый Структура("ЗначенияЗаполнения", Новый Структура("Тип", Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСФНС));
	ИначеЕсли ТипПереписки = "СПФР" Тогда 
		Возврат Новый Структура("ЗначенияЗаполнения", Новый Структура("Тип", Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСПФР));
	ИначеЕсли ТипПереписки = "СФСГС" Тогда
		Возврат Новый Структура("ЗначенияЗаполнения", Новый Структура("Тип", Перечисления.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСФСГС));
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// Получаем имя документа "ИсходящееУведомлениеФНС"
	ИмяСобытияЗаписиДокументаУведомлениеОСпецрежимахНалогообложения = 
		?(НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения), "", "Запись_" + ИмяДокументаУведомлениеОСпецрежимахНалогообложения);
	...
	ИначеЕсли ИмяСобытия = "Запись_ЭлектронныеПредставленияРегламентированныхОтчетов" 
		...
		ИЛИ ИмяСобытия = ИмяСобытияЗаписиДокументаУведомлениеОСпецрежимахНалогообложения Тогда
	...
	ИначеЕсли ИмяСобытия = "Запись_ОписиИсходящихДокументовВНалоговыеОрганы" И ТипЗнч(Параметр) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы") Тогда
	...
	ИначеЕсли ИмяСобытия = "Показать циклы обмена уведомления" 
		...
		Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) Тогда
			Если Тип("ДокументСсылка." + ИмяДокументаУведомлениеОКонтролируемыхСделках) = ТипЗначенияПараметра Тогда 
				СменитьЗакладкуПриНеобходимости("Документы", Ложь);
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
			Если Тип("ДокументСсылка." + ИмяДокументаУведомлениеОСпецрежимахНалогообложения) = ТипЗначенияПараметра Тогда 
				СменитьЗакладкуПриНеобходимости("ПрочиеУведомления", Ложь);
			КонецЕсли;
		КонецЕсли;
		
		Если Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") = ТипЗначенияПараметра Тогда 
			СменитьЗакладкуПриНеобходимости("Файлы", Ложь);
		КонецЕсли;
		ПриСменеСтраницы(Неопределено, Неопределено);
	...
	ИначеЕсли ИмяСобытия = "Запись_ОтправкиФСС" ИЛИ ИмяСобытия = "Редактирование_ОтправкиФСС" 
		...
		ИЛИ ИмяСобытия = "Запись_ОтправкиРПН" ИЛИ ИмяСобытия = "Редактирование_ОтправкиРПН" Тогда
...
////////////////////////////////////////////////////////////////////////////////
// Список "Требования и уведомления"
...
////////////////////////////////////////////////////////////////////////////////
// Список "Заявления о ввозе товаров"
...
////////////////////////////////////////////////////////////////////////////////
// Список "Документы по требованиям"

&НаКлиенте
Процедура КомандаПредставленияСоздать(Команда)
	
	ОткрытьФорму("Справочник.ОписиИсходящихДокументовВНалоговыеОрганы.Форма.ФормаЭлемента");
...
Процедура ТаблицаТребованияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТаблицаТребованияОтветы" Тогда
		...
		ИначеЕсли Элемент.ТекущиеДанные.КоличествоОтветовОформлениеЯчейки.Текст = "Создать" Тогда
			...
			КонтекстЭДОКлиент.СоздатьИсходящуюОписьПоОснованию(ВходящаяОпись);
		Иначе
...
Процедура ПоказатьОтветыНаВходящуюопись(ВходящаяОпись)
	...
	МассивОписейИсходящихДокументов = ПолучитьМассивОписейИсходящихДокументовПоОписиВходящихДокументов(ВходящаяОпись);
	...
	Если МассивОписейИсходящихДокументов.Количество() > 1 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьОтветыНаВходящуюописьЗавершение", ЭтотОбъект);
		СписокОписейИсходящихДокументов.ПоказатьВыборЭлемента(ОписаниеОповещения, "Выберите ответ на требование");
...
&НаКлиенте
Процедура ПоказатьОтветыНаВходящуюописьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
...
&НаСервереБезКонтекста
Функция ПолучитьМассивОписейИсходящихДокументовПоОписиВходящихДокументов(ОбъектСсылка)
	
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	Возврат КонтекстЭДОСервер.ПолучитьМассивОписейИсходящихДокументовПоОписиВходящихДокументов(ОбъектСсылка);	
...
////////////////////////////////////////////////////////////////////////////////
// Страница "ФСС" - Список "Отчетность"
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	ДокументооборотСКООперацииСФайламиКлиент.СохранитьФайлы(МассивОписанийПолучаемыхФайлов);
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) И НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОСпецрежимахНалогообложения) Тогда
		// предупреждаем пользователя о необходимости загрузить файлы
		ОписаниеОповещения = Новый ОписаниеОповещения("КомандаУведомленияСоздатьПродолжитьЗагрузкуФайла", ЭтотОбъект);
		ПродолжитьЗагрузкуФайла(ОписаниеОповещения);
	Иначе
...
Процедура КомандаУведомленияСоздатьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		Если РезультатВыбора.Значение = "Документы" Тогда
			СменитьЗакладкуПриНеобходимости("Документы");
		...
		ИначеЕсли РезультатВыбора.Значение = "УведомленияОРозничныхЦенахНаТабак" Тогда
			СменитьЗакладкуПриНеобходимости("УведомленияОРозничныхЦенахНаТабак");
		...
		ИначеЕсли РезультатВыбора.Значение = "ПрочиеУведомления" Тогда
			СменитьЗакладкуПриНеобходимости("ПрочиеУведомления");
		...
		ИначеЕсли РезультатВыбора.Значение = "Файлы" Тогда				
			СменитьЗакладкуПриНеобходимости("Файлы");
			КонтекстЭДОКлиент.ПолучениеФайловДляИмпортаНачало(УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПродолжитьЗагрузкуФайла(ВыполняемоеОповещение)
	
	КлючОбъектаДляСохранения = "УправлениеОбменом";
	КлючНастроекДляСохранения = "БольшеНеПоказыватьПредупреждениеОЗагрузкеФайла";
	
	БольшеНеПоказыватьПредупреждениеОЗагрузкеФайла = БольшеНеПоказыватьПредупреждениеОЗагрузкеФайла(КлючОбъектаДляСохранения, КлючНастроекДляСохранения); 
	
	Если БольшеНеПоказыватьПредупреждениеОЗагрузкеФайла = Истина Тогда
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, Истина);
	Иначе
		ТекстВопроса = 	НСтр("ru='Для создания документа необходимо выбрать загружаемые файлы. Продолжить?'");
		СтруктураПараметров = Новый Структура();
		СтруктураПараметров.Вставить("ТекстПредупреждения", 		ТекстВопроса);
		СтруктураПараметров.Вставить("ЗаголовокФормы", 				НСтр("ru = 'Необходимо выбрать файлы'"));
		СтруктураПараметров.Вставить("КлючОбъектаДляСохранения", 	КлючОбъектаДляСохранения);
		СтруктураПараметров.Вставить("КлючНастроекДляСохранения", 	КлючНастроекДляСохранения);
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПродолжитьЗагрузкуФайлаЗавершение", ЭтотОбъект, ВыполняемоеОповещение);
		ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПредупреждениеСФлагомБольшеНеПоказывать", СтруктураПараметров,,,,,ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПродолжитьЗагрузкуФайлаЗавершение(ПродолжитьОткрытиеФайла, ВыполняемоеОповещение) Экспорт
	
	Если ПродолжитьОткрытиеФайла = Истина Тогда
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, Истина);
	Иначе
...
&НаСервереБезКонтекста
Функция БольшеНеПоказыватьПредупреждениеОЗагрузкеФайла(КлючОбъектаДляСохранения, КлючНастроекДляСохранения)
	
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		КлючОбъектаДляСохранения,
		КлючНастроекДляСохранения,
		,,
		);
...
////////////////////////////////////////////////////////////////////////////////
// Страница "РАР" - Список "Отчетность"
...
////////////////////////////////////////////////////////////////////////////////
// Страница "РПН" - Список "Отчетность"
...
Процедура ВыгрузитьСодержимоеОтправкиРПН(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не выбраны отправки в Росприродназдора!'"));
		Возврат;
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// Страница "Доп. сервисы" - Список "Запросы на выписку сведений из ЕГРЮЛ/ЕГРИП"
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ Сертификаты
...
&НаКлиенте
Процедура СменитьЗакладкуПриНеобходимости(ИмяСобытия, ВыводитьПредупреждение = Истина)

	НужноВыполнитьСменуЗакладки = Ложь;
	Вид = Неопределено;
	ПредыдущийРежим = "";
	
	Если Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Пометка Тогда
		ПредыдущийРежим = Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Заголовок;
	ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Пометка Тогда
		ПредыдущийРежим = Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Заголовок;
	ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Пометка Тогда
		ПредыдущийРежим = Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Заголовок;
	ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьФайлы.Пометка Тогда
		ПредыдущийРежим = Элементы.ТаблицаУведомленияКомандаОткрытьФайлы.Заголовок;
	КонецЕсли;
	...
	ИначеЕсли ИмяСобытия = "УведомленияОРозничныхЦенахНаТабак" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Пометка Тогда
		Вид = "УведомленияОРозничныхЦенахНаТабак";
		НужноВыполнитьСменуЗакладки = Истина;
	...
	Если НужноВыполнитьСменуЗакладки Тогда
		...
		НовыйРежим = "";
		Если Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Пометка Тогда
			НовыйРежим = Элементы.ТаблицаУведомленияКомандаОткрытьДокументы.Заголовок;
		ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Пометка Тогда
			НовыйРежим = Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОРозничныхЦенахНаТабак.Заголовок;
		ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Пометка Тогда
			НовыйРежим = Элементы.ТаблицаУведомленияКомандаОткрытьПрочиеУведомления.Заголовок;
		ИначеЕсли Элементы.ТаблицаУведомленияКомандаОткрытьФайлы.Пометка Тогда
			НовыйРежим = Элементы.ТаблицаУведомленияКомандаОткрытьФайлы.Заголовок;
		КонецЕсли;
...
Процедура НачалоВыбораПредмета(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Форма = ПолучитьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ВыборПредметаЦиклаОбмена", ПараметрыОткрытия);
	
	// Запоминаем предыдущее значение отбора
	ПредыдущееЗначениеПредмета = Предмет;
	// Получаем новое значение отбора
	ДополнительныеПараметры = Новый Структура("ИмяПредмета, ПредыдущееЗначениеПредмета", ИмяПредмета, ПредыдущееЗначениеПредмета);
	ОписаниеОповещения = Новый ОписаниеОповещения("НачалоВыбораПредметаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Функция ПолучитьНастройкиДляДинамическогоСписка(ИмяДинамическогоСписка)
	...
	ИначеЕсли ИмяДинамическогоСписка = "Представления" Тогда
		Настройки.ОсновнаяТаблица = "Справочник.ОписиИсходящихДокументовВНалоговыеОрганы";
		Настройки.ДинамическоеСчитываниеДанных = Истина;
		Настройки.ТекстЗапроса = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	ВЫБОР
		...
		|			КОНЕЦ
		|	КОНЕЦ КАК СтатусКартинка,
		|	ОписиИсходящихДокументов.ДатаСоздания КАК ДатаСоздания,
		|	ОписиИсходящихДокументов.НалоговыйОрган КАК Получатель,
		|	ОписиИсходящихДокументов.Организация,
		|	ОписиИсходящихДокументов.Требование КАК Требование,
		|	ОписиИсходящихДокументов.Ссылка
		|ИЗ
		...
		|ГДЕ
		|	ОписиИсходящихДокументов.Организация В(&Организация)";
	...
	ИначеЕсли ИмяДинамическогоСписка = "Требования" Тогда
		...
		|			ОписиИсходящихДокументовВНалоговыеОрганы.ОписьВходящихДокументов КАК ОписьВходящихДокументов
		|		ИЗ
		|			Справочник.ОписиИсходящихДокументовВНалоговыеОрганы КАК ОписиИсходящихДокументовВНалоговыеОрганы
		|		
		|		СГРУППИРОВАТЬ ПО
		|			ОписиИсходящихДокументовВНалоговыеОрганы.ОписьВходящихДокументов) КАК ОтветыПоВходящимОписям
		|		ПО ВходящиеОписи.Ссылка = ОтветыПоВходящимОписям.ОписьВходящихДокументов
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		...
		|		ГДЕ
		|			ОписиВходящихДокументовИзНалоговыхОргановВходящиеДокументы.СсылкаНаОбъект.ВидДокумента = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговыхДокументов.ТребованиеОПредставленииДокументов)
		|		
		|		СГРУППИРОВАТЬ ПО
	...
	ИначеЕсли ИмяДинамическогоСписка = "Уведомления" Тогда
		...
		//Настройки.ТекстЗапроса = 
		//"ВЫБРАТЬ
		//|	1 КАК СтатусКартинка,
		//|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ДатаСоздания,
		//|	ВЫРАЗИТЬ(ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Справочник.Организации) КАК Организация,
		//|	ДАТАВРЕМЯ(1, 1, 1) КАК ОтчетныйГод,
		//|	ОписиИсходящихДокументов.Ссылка,
		//|	ВЫРАЗИТЬ(""0000"" КАК СТРОКА(4)) КАК НалоговыйОрган,
		//|	НЕОПРЕДЕЛЕНО КАК ВидУведомления
		//|ИЗ
		//|	Справочник.ОписиИсходящихДокументовВНалоговыеОрганы КАК ОписиИсходящихДокументов
		//|ГДЕ
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаВходящиеФСГС" Тогда
		...
		|					РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|				ГДЕ
		|					СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение.Тип В (ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоВходящееФСГС))
		|				
		|				СГРУППИРОВАТЬ ПО
		|					СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение) КАК СодержимоеТранспортногоКонтейнера
		|				ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|		ГДЕ
		|			ТранспортноеСообщение.Тип В (ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоВходящееФСГС))) КАК ЗапросКолонка1
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка1.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ТранспортноеСообщение.Ссылка КАК Ссылка,
		|			ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена,
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаОтчетностьПФР" Тогда
		...
		|			ТОГДА ЦиклыОбмена.ВидОтчета.Наименование
		|		ИНАЧЕ ЦиклыОбмена.Предмет.Представление
		|	КОНЕЦ КАК Отчет,
		|	ЦиклыОбмена.Период КАК Период,
		|	ЦиклыОбмена.ПредставлениеПериода КАК ПредставлениеПериода,
...
Функция ПолучитьСоответствиеСписокСтраница()
	...
	//СоответствиеСписокСтраница.Вставить("Уведомления", "ГруппаУведомления");
	...
	//// Прочее
	//СоответствиеСписокСтраница.Вставить("ТранспортноеСообщение", "ГруппаПрочее");
...
Функция ПолучитьСоответствиеСтраницаНадпись()
	...
	//// Прочее
	//СоответствиеСтраницаНадпись.Вставить("ТранспортноеСообщение", "ГруппаПрочее");
