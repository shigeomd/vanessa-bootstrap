...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоЭлектроннаяПодписьВМоделиСервиса = ЭлектроннаяПодписьВМоделиСервисаВызовСервера.ЭтоЭлектроннаяПодписьВМоделиСервиса(ЦиклОбмена);
...
Процедура ОформитьСтрокиПодписей()
	...
	ДокументооборотСКОКриптографияКлиент.НайтиСертификаты(ОписаниеОповещения, СертификатыДляПоиска);
...
Процедура КомандаПоказатьСертификат(Команда)
	...
	ДокументооборотСКОКриптографияКлиент.НайтиСертификат(ОписаниеОповещения, Сертификат);
...
Процедура ПоказатьСертификатПослеОпределенияСуществованияСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено И Результат.СертификатНайден Тогда
		...
		ДокументооборотСКОКриптографияКлиент.ПоказатьСертификат(ВходящийКонтекст);
	...
	Иначе
		...
		Иначе
			...
			ДокументооборотСКОКриптографияКлиент.ПоказатьСертификат(Новый Структура("Адрес", АдресСертификата));
		КонецЕсли;
...
Процедура КомандаПодписиПроверить(Команда)
	...
	Если СвойстваСертификата = Неопределено Тогда
		ПоказатьПредупреждение(, "Не удалось проверить электронно-цифровую подпись!");
	Иначе
		...
		ДокументооборотСКОКриптографияКлиент.ПроверитьПодписьPKCS7(ОписаниеОповещения, АдресФайлаЭЦП, АдресФайлаДИВ, Ложь);
	КонецЕсли;
...
Процедура КомандаПодписиПроверитьПослеПроверки(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если РезультатПроверки Тогда
			ПоказатьПредупреждение(, "Электронно-цифровая подпись валидна.");
		Иначе
			ПоказатьПредупреждение(, "Электронно-цифровая подпись не валидна");
		КонецЕсли;
		...
		ПоказатьВопрос(ОписаниеОповещения, "Сохранить результат проверки электронно-цифровой подписи?", РежимДиалогаВопрос.ДаНет);
