...
&НаСервереБезКонтекста
Функция МетодыМенеджераОбъектаДляБлокированияРеквизитов(ИмяОбъекта)
	
	МодульИнтеграцияСтандартныхПодсистем = ОбщийМодуль("ИнтеграцияСтандартныхПодсистем");
	МодульЗапретРедактированияРеквизитовОбъектовПереопределяемый = ОбщийМодуль("ЗапретРедактированияРеквизитовОбъектовПереопределяемый");
	Если МодульИнтеграцияСтандартныхПодсистем = Неопределено Или МодульЗапретРедактированияРеквизитовОбъектовПереопределяемый = Неопределено Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	ОбъектыСЗаблокированнымиРеквизитами = Новый Соответствие;
	МодульИнтеграцияСтандартныхПодсистем.ПриОпределенииОбъектовСЗаблокированнымиРеквизитами(ОбъектыСЗаблокированнымиРеквизитами);
	МодульЗапретРедактированияРеквизитовОбъектовПереопределяемый.ПриОпределенииОбъектовСЗаблокированнымиРеквизитами(ОбъектыСЗаблокированнымиРеквизитами);
...
Функция ЗаблокированныеРеквизиты()
	...
	Если ВерсияБСПСоответствуетТребованиям() Тогда
		...
		ДоступныеМетоды = МетодыМенеджераОбъектаДляБлокированияРеквизитов(ВидИзменяемыхОбъектов);
		Если ТипЗнч(ДоступныеМетоды) = Тип("Массив") И (ДоступныеМетоды.Количество() = 0 
			ИЛИ ДоступныеМетоды.Найти("ПолучитьБлокируемыеРеквизитыОбъекта") <> Неопределено) Тогда 
...
Процедура УстановитьОграниченияВыбираемыхТиповИПараметрыВыбораЗначения(ТабличноеПоле)
	...
	Если Не ПустаяСтрока(ТабличноеПоле.ТекущиеДанные.СвязиПараметровВыбора) Тогда
		Для Индекс = 1 По СтрЧислоСтрок(ТабличноеПоле.ТекущиеДанные.СвязиПараметровВыбора) Цикл
			...
			ЧастиИмениРеквизита = РазложитьСтрокуВМассивПодстрок(ИмяРеквизита, ".", Истина);
			ИмяТабличнойЧасти = "";
...
Процедура ДобавитьСвойствоВТаблицуРеквизитов(ТаблицаДопСвойств, ЭтоДопРеквизит = Истина)
	...
	Если ТаблицаДопСвойств = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
//
&НаСервере
Процедура ДобавитьРеквизитыВНабор(Реквизиты, НеРедактируемые, ФильтруемыеРеквизиты, БлокируемыеРеквизиты, ОбъектМетаданных)
	...
	Для Каждого ОписаниеРеквизита Из Реквизиты Цикл
		...
		НоваяОперация.Представление = ОписаниеРеквизита.Представление();
