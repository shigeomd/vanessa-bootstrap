...
//
// Параметры:
//  КлючеваяОперация - СправочникСсылка.КлючевыеОперации, ключевая операция, для которой следует рассчитать целевое
//                     время.
...
Процедура ОбработатьРеквизитыОбъекта(КолонкиТаблицаКлючевыхОпераций)
	...
	Иначе
		...
		Для Каждого Элемент Из Элементы.Производительность.ПодчиненныеЭлементы Цикл
			Сч = Сч + 1;
			// Пропускаем первые 3 элемента, чтобы не менять заголовки у колонок "Ключевая операция", "Приоритет" и "Целевое
			// время".
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	Для Каждого КолонкаТаблицаКлючевыхОпераций Из ТаблицаКлючевыхОпераций.Колонки Цикл
		...
		// Для отображения только часов если шаг Час.
		Точка.Текст = ?(Объект.Шаг = "Час", Лев(КолонкаТаблицаКлючевыхОпераций.Заголовок, 2), КолонкаТаблицаКлючевыхОпераций.Заголовок);
		Строка = 0;
...
Процедура ЗаполнитьОписание(ИмяФайлаОписания, Начало, Конец, МассивКлючевыхОпераций)
	...
	Для ИндексОперации = 0 По МассивКлючевыхОпераций.Количество() - 1 Цикл
		ЗаписьXML.ЗаписатьНачалоЭлемента("KeyOperation");
		ЗаписьXML.ЗаписатьАтрибут("name", Строка(МассивКлючевыхОпераций[ИндексОперации]));
...
Процедура ОткрытьФормуВыбора()
	...
	Отбор = Новый Массив;
	Для ТекИндекс = 0 По ТЧ.Количество() - 1 Цикл
		Отбор.Добавить(ТЧ[ТекИндекс].КлючеваяОперация);
	КонецЦикла;
...
Функция СоответствиеЦветовУровнюАпдекса()
	...
	Значения.От = 0.001; // О означает что операция не выполнялась вообще.
	Значения.До = 0.5;
