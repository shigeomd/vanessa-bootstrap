...
Процедура Конвертация_ПослеЗагрузкиОбъекта(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ИначеЕсли ИмяТипаОбъекта = "Документ" Тогда
		...
		Если Константы.ВыполнятьСинхронизациюV77.Получить() Тогда
			...
			СинхронизацияДанныхV77Сервер.ДобавитьСтрокуВТаблицуСоответствий(Параметры.ТаблицаСоответствийДокументов, Объект);
		Иначе
			Если ТипЗнч(Объект) = Тип("ДокументОбъект.ВводНачальныхОстатков") Тогда
...
Процедура Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ) Экспорт
	...
	НаборКонстант = Константы.СоздатьНабор("ВедетсяПроизводственнаяДеятельность, ВедетсяРозничнаяТорговля, ИспользуетсяОСНО, 
		|ОсуществляетсяРеализацияТоваровУслугКомитентов, ОсуществляетсяРеализацияТоваровУслугЧерезКомиссионеров, 
		|ИспользоватьНесколькоСкладов, ИспользоватьТипыЦенНоменклатуры");
	НаборКонстант.Прочитать();
	НаборКонстант.ВедетсяПроизводственнаяДеятельность = Истина;
	НаборКонстант.ВедетсяРозничнаяТорговля = Истина;
	НаборКонстант.ИспользуетсяОСНО = Истина;
...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	Попытка
		...
		ИспользуетсяЕНВД = УчетнаяПолитика.ПлательщикЕНВДЗаПериод(Параметры.Организация, ДатаНачала, ДатаОкончания);
		Константы.ИспользуетсяЕНВД.Установить(ИспользуетсяЕНВД);
	Исключение
		Возврат;
	КонецПопытки;
	...
	Для Каждого СтрокаТаблицы ИЗ Результат Цикл
		...
		Если СтрокаТаблицы.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСПоПриобретеннымЦенностям
			ИЛИ СтрокаТаблицы.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСПоАвансамПолученным
			ИЛИ СтрокаТаблицы.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСНачисленный Тогда
		...
		ИначеЕсли (Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения) 
			...
			Иначе
				...
				Если Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы Тогда
					КоллекцияОбъектов = Объект.МатериалыПереданныеВПереработку;		
					...
					КоллекцияОбъектов = Объект.СпецодеждаИСпецоснасткаВЭксплуатации;		
...
Процедура ПКО_ПослеЗагрузки_УслугиСтороннихОрганизаций_ПТУ(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_ОказаниеУслуг_Комитент(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_ЗачетАвансаПокупателя(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_ЗачетАвансаПоставщику(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_ВыплатаЗПБанк(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ОтказЗаписи = Ложь;
	ВзаиморасчетыССотрудниками.ВедомостьПередЗаписью(Объект, ОтказЗаписи, РежимЗаписиДокумента.Запись);
...
Процедура ПКО_ПослеЗагрузки_ВводВЭксплуатациюОС(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Объект.СтоимостьВР = Объект.СтоимостьБУ - Объект.СтоимостьНУ - Объект.СтоимостьПР;
...
Процедура ПКО_ПослеЗагрузки_ТребованиеНакладная_Производство(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_КорректировочныйСчетФактураПолученный(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_ИсправлениеСчетаФактурыПолученного(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
...
Процедура ПКО_ПослеЗагрузки_КорректировочныйСчетФактураПолученный_СчетФактура(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт

	Если Не Объект.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаАванс И Не Объект.СформированПриВводеНачальныхОстатковНДС Тогда
		Объект.ОпределениеПараметровСчетаФактуры();
	КонецЕсли;

КонецПроцедуры

Процедура ПКО_ПослеЗагрузки_ИсправлениеСчетаФактурыПолученного_СчетФактура(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
