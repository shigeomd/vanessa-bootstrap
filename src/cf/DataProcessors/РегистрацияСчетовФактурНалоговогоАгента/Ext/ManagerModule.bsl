...
Процедура ЗаполнитьСтрокиДокумента(СтруктураПараметров, ТаблицаРезультатов)
	...
	Запрос.УстановитьПараметр("ЗначениеСтавкиНДС", УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(Перечисления.СтавкиНДС.НДС18) / 100);
	...
	|	СУММА(ХозрасчетныйОборотыДтКт.СуммаОборот + ХозрасчетныйОборотыДтКт.СуммаОборот * &ЗначениеСтавкиНДС) КАК Сумма,
	|	СУММА(ХозрасчетныйОборотыДтКт.СуммаОборот * &ЗначениеСтавкиНДС) КАК СуммаНДС,
	...
	|			И (СчетФактураВыданный.Ссылка.Исправление = ЛОЖЬ)
	|			И (СчетФактураВыданный.Ссылка.Исправление = ЛОЖЬ)
...
Процедура СформироватьСчетаФактуры(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Если Не ЕстьОшибки Тогда
		РегистрыСведений.ВыполнениеРегламентныхОперацийНДС.ЗафиксироватьФактВыполненияРегламентнойОперации(
					...
					СтруктураПараметров.Организация, Неопределено, Перечисления.РегламентныеОперации.РегистрацияСчетовФактурНалоговогоАгента, НЕ ЕстьОшибки);	
	КонецЕсли;
...
Функция ВыполнитьФормированиеСчетовФактурНалоговогоАгента(СтруктураПараметров, Список)
	...
	МассивДокументов
		= ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(Список.ВыгрузитьКолонку("ДокументОснование"));
	...
	Для Каждого Документ Из МассивДокументов Цикл
		...
		Для Каждого Договор Из МассивДоговоров Цикл
			...
			Для Каждого СтрокаОтбораДоговор Из СтрокиПоДоговору Цикл
				...
				Иначе
					СчФ.НомерПлатежноРасчетногоДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаОтбораДоговор.ДокументОснование);
