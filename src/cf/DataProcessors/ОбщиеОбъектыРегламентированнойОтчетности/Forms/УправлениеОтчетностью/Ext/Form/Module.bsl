...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтображатьСписокЦО = ХранилищеНастроекДанныхФорм.Загрузить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_ОтображатьСписокЦикловОбмена");
	Если ОтображатьСписокЦО = Неопределено Тогда 
		ОтображатьСписокЦО = Истина
	Иначе 
		Элементы.ОтображатьСписокЦикловОбмена.Пометка = ХранилищеНастроекДанныхФорм.Загрузить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_ОтображатьСписокЦикловОбмена");
	КонецЕсли;
	...
	Если ЭлектронныйДокументооборотАктивен ИЛИ ЭлектронныйДокументооборотСФССАктивен 
			ИЛИ ЭлектронныйДокументооборотСФСРАРАктивен ИЛИ ЭлектронныйДокументооборотСРПНАктивен Тогда
		УстановитьОтборыЦО();
	КонецЕсли;
...
Процедура ВосстановитьПараметрыОтбора()
	...
	Если ТипЗнч(ПараметрыОткрытия) <> Тип("Структура") Тогда
		...
		КодИФНС       = ХранилищеНастроекДанныхФорм.Загрузить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_КодИФНС");
		ОтборКодИФНС  = ХранилищеНастроекДанныхФорм.Загрузить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_ОтборКодИФНС");
...
Процедура СохранитьНастройки()
	...
	ХранилищеНастроекДанныхФорм.Сохранить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_КодИФНС", КодИФНС);
	ХранилищеНастроекДанныхФорм.Сохранить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_ОтборКодИФНС", ОтборКодИФНС);
	...
	ХранилищеНастроекДанныхФорм.Сохранить("Обработка.ОбщиеОбъектыРегламентированнойОтчетности.Форма.УправлениеОтчетностью", "ФормаРеглИФинОтчетности_ОтображатьСписокЦикловОбмена", Элементы.ОтображатьСписокЦикловОбмена.Пометка);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Заполнение списка полученных сообщений" И ТипЗнч(Параметр) = Тип("Массив") Тогда
		...
		Если НЕ ЭлектронныйДокументооборотАктивен И НЕ ЭлектронныйДокументооборотСФССАктивен 
			И НЕ ЭлектронныйДокументооборотСФСРАРАктивен И НЕ ЭлектронныйДокументооборотСРПНАктивен Тогда
			Возврат;
		КонецЕсли;
...
Процедура УстановитьОтборы() Экспорт
	...
	Иначе
		...
		Иначе
			ОтборДинамическогоСписка = ЖурналВыгрузки.Отбор;
		КонецЕсли;
