...
Процедура Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ) Экспорт
	...
	Значение_ВестиУчетПоРаботникам = одАтрибут(ФайлОбмена, одТипЧисло, "УчетЗарплатыВоВнешнейПрограмме");
	Значение_УчетНоменклатурыПоСкладам = одАтрибут(ФайлОбмена, одТипЧисло, "УчетНоменклатурыПоСкладам");
	...
	Если ПараметрыУчета.СкладскойУчет<> 2
		...
		ПараметрыУчета.СкладскойУчет = ?(Значение_УчетНоменклатурыПоСкладам = 1, 2, ПараметрыУчета.СкладскойУчет);
		ПараметрыУчета.ВестиУчетПоРаботникам = Значение_ВестиУчетПоРаботникам;
	...
	НаборКонстант = Константы.СоздатьНабор("ВедетсяПроизводственнаяДеятельность, ВедетсяРозничнаяТорговля, ИспользуетсяНДФЛИП, 
		|ИспользуетсяУСН, ИспользуетсяУСНПатент, ОсуществляетсяРеализацияТоваровУслугКомитентов, 
		|ОсуществляетсяРеализацияТоваровУслугЧерезКомиссионеров, ИспользуетсяЕНВД, ИспользоватьНесколькоСкладов, ИспользоватьТипыЦенНоменклатуры");
	НаборКонстант.Прочитать();
	НаборКонстант.ВедетсяПроизводственнаяДеятельность = Истина;
	НаборКонстант.ВедетсяРозничнаяТорговля = Истина;
	НаборКонстант.ИспользуетсяНДФЛИП = Истина;
	НаборКонстант.ИспользуетсяУСН = Истина;
	НаборКонстант.ИспользуетсяУСНПатент = Истина;
	НаборКонстант.ИспользуетсяЕНВД = Истина;
...
Процедура ПКО_ПослеЗагрузки_ИндивидуальныйПредприниматель(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
