...
Процедура ВыполнитьЗакрытиеМесяца(Организация, ПериодРегистрации, ГраницаВыгрузкиОборотов)
	...
	//	// Результат.Операции содержит таблицу с колонками
	//	// Организация, ВидОперации, Состояние, Ссылка.
	//	Если Не ЗначениеЗаполнено(СтрокаТаблицы.Ссылка) Тогда
	//		// Регламентную операцию не удалось записать
	//	Иначе СтрокаТаблицы.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.ВыполненоСОшибками Тогда
	//		// При выполнении произошла ошибка, перечень ошибок содержится в самой регламентной операции
	//	ИначеЕсли СтрокаТаблицы.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.НеВыполнено Тогда
	//		// Не удалось выполнить операцию - что-то пошло совсем не так.
	//		// Детальная информация - в журнале регистрации
	//	КонецЕсли;
	//КонецЕсли;
...
Процедура СформироватьЗаписиКнигиПокупокИКнигиПродаж(Организация, ПериодРегистрации)
	...
	СтруктураПараметров = Новый Структура("Организация, Дата, ПредъявленНДСКВычету0, ИмяТабЧасти",
			Организация, ПериодРегистрации, Ложь, Неопределено);
