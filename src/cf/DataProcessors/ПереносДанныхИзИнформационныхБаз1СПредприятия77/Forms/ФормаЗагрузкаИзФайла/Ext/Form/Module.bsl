...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ВозможностьВыбораФайлов Тогда
		...
		|1С:Предприятия 7.7 с помощью ""Помощника перехода на 1С:Бухгалтерию 8""";
	Иначе
...
Процедура ОбработчикОжиданияПроверкаДанных()
	...
	Если ОповеститьОбИзмененииЗадачБухгалтера Тогда
...
Процедура ПроверитьДанныеНаСервере(ОповеститьОбИзмененииЗадачБухгалтера)
 ...
 ПараметрыПроверки = Новый Структура("ИдентификаторКонфигурации, Организация, НачалоПериодаВыгрузки, ОкончаниеПериодаВыгрузки, 
	...
	РезультатВыполнения = КалендарьБухгалтера.ЗапуститьЗаполнениеВФоне(УникальныйИдентификатор, ЭтаФорма.Организация);
	
	ОповеститьОбИзмененииЗадачБухгалтера = РезультатВыполнения.ЗаданиеВыполнено;
