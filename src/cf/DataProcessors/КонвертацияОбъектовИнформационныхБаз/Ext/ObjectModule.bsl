...
Процедура ОтменитьПроведениеОбъектаВИБ(Объект, Тип, ЗаписатьОбъект = Ложь) Экспорт
	...
	Если ОбменДаннымиСобытия.ЗагрузкаЗапрещена(Объект, УзелОбменаЗагрузкаДанных) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьПометкуУдаленияУОбъекта(Объект, ПометкаУдаления, ИмяТипаОбъекта) Экспорт
	...
	Если (ПометкаУдаления = Неопределено И Объект.ПометкаУдаления <> Истина)
		Или ОбменДаннымиСобытия.ЗагрузкаЗапрещена(Объект, УзелОбменаЗагрузкаДанных) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если Объект <> Неопределено Тогда
		...
		Если ПриоритетОбъектаОбмена <> Перечисления.ПриоритетыОбъектовОбмена.ПриоритетПоступившегоПриОбменеОбъектаВыше Тогда
			СтандартныеПодсистемыСервер.ПриПолученииДанныхОтПодчиненного(Объект, ПолучениеЭлемента, ОтправкаНазад, УзелОбменаЗагрузкаДанных);
		Иначе
			СтандартныеПодсистемыСервер.ПриПолученииДанныхОтГлавного(Объект, ПолучениеЭлемента, ОтправкаНазад, УзелОбменаЗагрузкаДанных);
		КонецЕсли;
...
Функция ЭтоОбъектКлассификатор(ТипОбъектаСтрокой, ПКО)
	...
	Если    ВидОбъекта = "СправочникСсылка"
		...
		Или ВидОбъекта = "ПланВидовРасчетаСсылка"
	Тогда
...
////////////////////////////////////////////////////////////////////////////////
// ПРЕДОПРЕДЕЛЕННЫЕ ПРАВИЛА КОНВЕРТАЦИИ
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			// Нет процедуры отключения обработки.
		КонецПопытки;
