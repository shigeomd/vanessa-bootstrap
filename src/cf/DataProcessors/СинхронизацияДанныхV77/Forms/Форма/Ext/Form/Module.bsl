...
Процедура ЗагрузитьДанныеСинхронногоУчета(АдресФайла) Экспорт
	...
	СинхронизацияДанныхV77Сервер.ЗаписатьСообщениеВЖР(ТекстСообщения);
...
Функция ВыполнитьЗагрузкуДанныхНаСервере(АдресФайла)
	...
	ВозрастРелизаПравил8  = Число(Лев(СокрЛП(СтрЗаменить(НомерРелиза8, ".", "")), 8));
	...
	ВозрастРелизаКонфигурации8  = Число(Лев(СокрЛП(СтрЗаменить(Метаданные.Версия, ".", "")), 8));
	...
	СтрОтветственный = СокрП(СинхронизацияДанныхV77Сервер.ПолучитьПараметрСинхронизацииV77("ОтветственныйПользователь"));
...
Процедура ПриОткрытии(Отказ)
	...
	Объект.ИмяФайлаДанных = СинхронизацияДанныхV77Сервер.ПолучитьПараметрСинхронизацииV77("ИмяФайлаДанных");
	НомерЗагрузки         = СинхронизацияДанныхV77Сервер.ПолучитьПараметрСинхронизацииV77("НомерПоследнейЗагрузки");
...
Функция ЗагрузитьРезультат()
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда	
		...
		КонецЕсли;
		СинхронизацияДанныхV77Сервер.УстановитьПараметрСинхронизацииV77("НомерПоследнейЗагрузки", НомерВыгрузки);
	КонецЕсли;
...
Процедура ИмяФайлаДанныхНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если СтароеИмяФайла <> Объект.ИмяФайлаДанных Тогда
		СинхронизацияДанныхV77Сервер.УстановитьПараметрСинхронизацииV77("ИмяФайлаДанных", Объект.ИмяФайлаДанных);
	КонецЕсли;
...
Процедура ПриЗакрытии()
	...
	СтароеИмяФайла = СинхронизацияДанныхV77Сервер.ПолучитьПараметрСинхронизацииV77("ИмяФайлаДанных");
