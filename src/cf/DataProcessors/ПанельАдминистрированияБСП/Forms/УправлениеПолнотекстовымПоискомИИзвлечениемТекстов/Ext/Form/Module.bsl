...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость = РежимРаботы.ЭтоАдминистраторСистемы;
...
Процедура РегламентныеЗаданияГиперссылкаНажатие(ИмяПредопределенного)
	Сведения = СведенияОРегламентномЗадании(ЭтотОбъект, ИмяПредопределенного);
...
Процедура РегламентныеЗаданияСохранить(ИмяПредопределенного, Изменения, УстановитьВидимостьДоступность)
	...
	Если Сведения.Идентификатор = Неопределено Тогда
		Возврат;
	КонецЕсли;
	РегламентноеЗадание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(Сведения.Идентификатор);
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = ""
		...
		Если НаборКонстант.ИзвлекатьТекстыФайловНаСервере Тогда
			Сведения = СведенияОРегламентномЗадании(ЭтотОбъект, "ИзвлечениеТекста");
...
Процедура ОбновитьРегламентныеЗадания()
	...
	РегламентныеЗаданияСохранить("ОбновлениеИндексаППД", Изменения, Ложь);
	РегламентныеЗаданияСохранить("СлияниеИндексаППД", Изменения, Ложь);
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ФайловыеФункции") Тогда
		Если Не НаборКонстант.ИзвлекатьТекстыФайловНаСервере Тогда
			Изменения.Использование = Ложь;
		КонецЕсли;
		РегламентныеЗаданияСохранить("ИзвлечениеТекста", Изменения, Ложь);
	КонецЕсли;
КонецПроцедуры
...
Функция СведенияОРегламентномЗадании(Форма, ИмяПредопределенного)
	Сведения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.СведенияОРегламентныхЗаданиях, ИмяПредопределенного, 0);
	Если Сведения = 0 Тогда
		Сведения = Новый Структура("Идентификатор, Использование, Расписание");
		#Если Сервер Тогда
			Форма.СведенияОРегламентныхЗаданиях.Вставить(ИмяПредопределенного, Сведения);
			...
			Если МетаданныеПредопределенного = Неопределено Тогда
				Возврат Сведения;
			КонецЕсли;
			Задание = РегламентныеЗадания.НайтиПредопределенное(МетаданныеПредопределенного);
