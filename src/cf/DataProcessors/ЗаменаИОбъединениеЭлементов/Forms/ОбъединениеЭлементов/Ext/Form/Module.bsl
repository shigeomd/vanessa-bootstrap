...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция = ПоискИУдалениеДублей.ИнструкцияУсловногоОформления();
	Условие = Новый Структура("Вид, Значение", "НеРавно", Новый ПолеКомпоновкиДанных("ОсновнойЭлемент"));
	...
	ПоискИУдалениеДублей.ДобавитьЭлементУсловногоОформления(ЭтотОбъект, Инструкция);
...
Процедура ШагПоискМестИспользованияПриАктивации(Знач Страница, Знач ДополнительныеПараметры) Экспорт
	ФоновоеЗаданиеЗапустить("ОпределитьМестаИспользования");
КонецПроцедуры
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ОткрытьФорму("Обработка.ЗаменаИОбъединениеЭлементов.Форма.МногострочноеПредупреждение", ПараметрыПредупреждения, ЭтотОбъект);
...
Процедура ШагОбъединениеПриАктивации(Знач Страница, Знач ДополнительныеПараметры) Экспорт
	ФоновоеЗаданиеЗапустить("ЗаменитьСсылки");
КонецПроцедуры
...
Функция НомерШагаМастераПоИдентификатору(Знач ИдентификаторШага)
	...
	ВызватьИсключение СтрЗаменить(НСтр("ru = 'Не найден шаг ""%1"".'"), "%1", ИмяПоиска);
КонецФункции
...
&НаСервере
Функция ФоновоеЗаданиеЗапустить(Знач ИмяПроцедуры)
	// Отмена предыдущего задания.
	ФоновоеЗаданиеОтменить();
	
	// Определение параметров запуска.
	ПолноеИмяМетода = "ПоискИУдалениеДублей." + ИмяПроцедуры;
	
	Если ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
	...
	ИначеЕсли ИмяПроцедуры = "ЗаменитьСсылки" Тогда
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		НастройкиПошаговогоМастера.ИмяПроцедуры                   = Неопределено;
		Возврат Ложь;
	КонецЕсли;
	...
	НастройкиПошаговогоМастера.ИмяПроцедуры                   = ИмяПроцедуры;
