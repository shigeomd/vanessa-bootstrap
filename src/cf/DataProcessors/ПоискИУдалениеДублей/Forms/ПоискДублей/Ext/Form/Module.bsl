...
Процедура ИзменитьОсновнойЭлементИерархически(Знач ДанныеСтроки, Знач Родитель)
	...
	Родитель.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 (%2)'"), 
		ДанныеСтроки.Наименование, Родитель.Количество);
КонецПроцедуры
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач Данные)
	...
	Иначе
		ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ОбновитьОписаниеПравилПоиска()
	...
	Иначе
		ТекстПравил = СокрЛП(Сред(ТекстПравил, СтрДлина(Союз)));
	КонецЕсли;
...
Процедура ШагВыполнениеПоискаПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ФоновоеЗаданиеЗапустить("ФоновыйПоискДублей");
...
Процедура ШагВыполнениеУдаленияПриАктивации(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	Элементы.Шапка.Доступность = Ложь;
	ФоновоеЗаданиеЗапустить("ФоновоеУдалениеДублей");
КонецПроцедуры
...
Функция ФоновоеЗаданиеЗапустить(Знач ИмяПроцедуры)
	...
	ПараметрыМетода.Вставить("ИмяПроцедуры", ИмяПроцедуры);
	
	Если ИмяПроцедуры = "ФоновыйПоискДублей" Тогда
	...
	ИначеЕсли ИмяПроцедуры = "ФоновоеУдалениеДублей" Тогда
