...
Процедура ОбластиПоискаДублей(Список, Знач АнализироватьПрикладныеПравила = Ложь) Экспорт
	...
	КонецЕсли;
	ГруппаОбластейПоискаДублей(Список, ОбъектыСПоискомДублей, АнализироватьПрикладныеПравила, Справочники,             "Справочник");
	ГруппаОбластейПоискаДублей(Список, ОбъектыСПоискомДублей, АнализироватьПрикладныеПравила, ПланыВидовХарактеристик, "ПланВидовХарактеристик");
	ГруппаОбластейПоискаДублей(Список, ОбъектыСПоискомДублей, АнализироватьПрикладныеПравила, ПланыСчетов,             "ПланСчетов");
	ГруппаОбластейПоискаДублей(Список, ОбъектыСПоискомДублей, АнализироватьПрикладныеПравила, ПланыВидовРасчета,       "ПланВидовРасчета");
...
Процедура ФоновоеУдалениеДублей(Знач Параметры, Знач АдресРезультата) Экспорт
	...
	ПараметрыЗамены.Вставить("ВключатьБизнесЛогику", Параметры.УчитыватьПрикладныеПравила);
...
Функция ЕстьПрикладныеПравилаОбластиПоискаДублей(Знач ИмяОбъекта) Экспорт
	...
	Возврат СведенияОбОбъекте <> Неопределено И (СведенияОбОбъекте = "" Или Найти(СведенияОбОбъекте, "ПараметрыПоискаДублей") > 0);
...
Функция СтрокиПодобны(Знач Строка1, Знач Строка2)
	...
	Если Макс(СтрДлина(Строка1), СтрДлина(Строка2)) > 10 Тогда
		Возврат ПроцентПодобияСтрок(Строка1, Строка2) >= 90;
	Иначе
		Возврат ПроцентПодобияСтрок(Строка1, Строка2) >= 80;
	КонецЕсли;
...
Процедура ГруппаОбластейПоискаДублей(Результат, ОбъектыСПоискомДублей, Знач АнализироватьПрикладныеПравила, Знач МенеджерГруппы, Знач Пиктограмма)
	...
	Для Каждого Элемент Из МенеджерГруппы Цикл
		...
		Если АнализироватьПрикладныеПравила Тогда
			...
			ЕстьПрикладныеПравила = СведенияОбОбъекте <> Неопределено И 
				(СведенияОбОбъекте = "" Или Найти(СведенияОбОбъекте, "ПараметрыПоискаДублей") > 0);
		Иначе
