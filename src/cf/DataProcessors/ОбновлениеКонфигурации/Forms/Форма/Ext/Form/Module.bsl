...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнформацияОНаличииСоединений = СоединенияИБ.ИнформацияОСоединениях();
	ЕстьАктивныеСеансы = Не ИнформацияОНаличииСоединений.НаличиеАктивныхСоединений;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.ВыполнитьОбновление Тогда
		Если ЕстьАктивныеСеансы Тогда
			ПерейтиКВыборуРежимаОбновления();
			Возврат;
		ИначеЕсли ПроверитьДоступКИБ() Тогда
...
&НаКлиенте
Процедура ДекорацияПорядокОбновленияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
Функция ВставитьПараметрыСкрипта(Знач Текст, Знач СтруктураПараметров)
	...
	Результат = СтрЗаменить(Результат, "[ВыполнятьСжатиеТаблицИБ]"				, ?(ВыполнятьСжатиеТаблицИБ(), "true", "false"));
...
Процедура ВыполнитьЗагрузкуСпискаОбновлений(Знач АдресФайлаЗагрузки, ТаблицаОбновлений = Неопределено)
	...
	// Основные колонки
	ТаблицаОбновлений.Колонки.Добавить("Конфигурация"			, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("Поставщик"				, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("Версия"					, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("ВерсияДляОбновления"	, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("ПутьКФайлуОбновления"	, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("НомерДискаИТС"			, ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаОбновлений.Колонки.Добавить("ВерсияПлатформы"		, ОбщегоНазначения.ОписаниеТипаСтрока(0));
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	ИначеЕсли НоваяТекущаяСтраница = Страницы.УспешноеОбновление Тогда
		...
		ОзнакомитьсяСДополнительнымиИнструкциями = Не ПустаяСтрока(ИмяФайлаПорядокОбновления);
		ПоказатьНовоеВВерсии = Не ОзнакомитьсяСДополнительнымиИнструкциями;
		Элементы.ОзнакомитьсяСДополнительнымиИнструкциями.Видимость = ОзнакомитьсяСДополнительнымиИнструкциями;
		ФункцияКнопкиДалее = Ложь;
		ДоступностьКнопкиНазад = Ложь;
		ДоступностьКнопкиЗакрыть = Ложь;
...
Функция ОбработатьСтраницуУспешноеОбновление(ВыдаватьСообщения = Истина)
	...
	Если ПоказатьНовоеВВерсии Тогда
		ОткрытьФорму("ОбщаяФорма.ОписаниеИзмененийСистемы", Новый Структура("ПоказыватьТолькоИзменения", Истина));
	КонецЕсли;
	
	Если ОзнакомитьсяСДополнительнымиИнструкциями Тогда
...
Функция ПроверитьДоступКИБ()
	...
	ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента();
	Если ПараметрыРаботыКлиента.ЭтоБазоваяВерсияКонфигурации Или ПараметрыРаботыКлиента.ЭтоУчебнаяПлатформа Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если Результат И Не ПараметрыРаботыКлиента.ИнформационнаяБазаФайловая Тогда
		...
		Исключение
			Результат = Ложь;
			ТекстСообщения = НСтр("ru = 'Не удалось выполнить подключение к кластеру серверов:'") + Символы.ПС;
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытияЖурнала,
				"Ошибка", ТекстСообщения + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбнаруженнаяОшибкаПодключения = ТекстСообщения + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
...
Функция ЗапланироватьОбновлениеКонфигурации()
	...
	ПутьЗапускаемогоСкрипта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 %2 //nologo ""%3"" /p1:""%4"" /p2:""%5""",
		...
		ПараметрыАдминистрирования.ПарольАдминистратораИнформационнойБазы, ПараметрыАдминистрирования.ПарольАдминистратораКластера);
...
Функция СкопироватьФайл(ИмяФайлаИсточник, ИмяФайлаНазначение, ВыдаватьСообщения = Ложь)
	Попытка
		СоздатьКаталог(ОбновлениеКонфигурацииКлиент.ПолучитьКаталогФайла(ИмяФайлаНазначение));
...
Процедура ВыполнитьОбновлениеКонфигурации()
	...
	ЗаписатьСтатусОбновления(ИмяПользователя(), Истина, Ложь, Ложь, ПараметрыПриложения["СтандартныеПодсистемы.СообщенияДляЖурналаРегистрации"]);
...
&НаСервере
Функция ВыполнятьСжатиеТаблицИБ()
	
	Если Объект.ДоступныеОбновления.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДоступнаяВерсия = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(Объект.ДоступныеОбновления[Объект.ДоступныеОбновления.Количество()-1].Версия);
	
	Если ПустаяСтрока(ДоступнаяВерсия) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТекущаяВерсия = ОбщегоНазначенияКлиентСервер.ВерсияКонфигурацииБезНомераСборки(Метаданные.Версия);
	
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсииБезНомераСборки(ДоступнаяВерсия, ТекущаяВерсия) > 0;
