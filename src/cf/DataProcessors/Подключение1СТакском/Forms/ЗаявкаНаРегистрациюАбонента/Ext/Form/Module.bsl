...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Создание стурктуры с реквизитами обработки.
	...
	ИначеЕсли СтатусФормы = "show" Тогда
		...
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			НомерЗаявки,
			Формат(ДатаЗаявки, "ДФ=""дд ММ гггг 'г.' ЧЧ:мм:сс"""));
	...
	Если ТекущийВариантИнтерфейсаКлиентскогоПриложения() = ВариантИнтерфейсаКлиентскогоПриложения.Такси Тогда
		...
		Элементы.ГруппаКотактнойИнформации.Отображение = ОтображениеОбычнойГруппы.СлабоеВыделение;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПрограммноеЗакрытие Тогда
		...
		Если СтатусФормы <> "show" И НЕ НажатаКнопкаОтправить Тогда
			
			ПараметрыЗапроса = Новый Массив;
			ПараметрыЗапроса.Добавить(Новый Структура("Имя, Значение", "endForm", "close"));
			// Это признак нажатия кнопки "Закрыть"
			
			// Отправить параметры на сервер
			ИнтернетПоддержкаПользователейКлиент.ОбработкаКомандСервиса(КонтекстВзаимодействия,
				ЭтотОбъект,
				ПараметрыЗапроса);
...
// Процедура настроки доступности элементов формы
