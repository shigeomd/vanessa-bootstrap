...
&НаКлиенте
Процедура СохранитьФайлыДляОбменаСБанком(ПолучаемыеФайлы, КаталогФайлов, ЗакрытьФормуДлительнойОперации = Неопределено, ОповеститьТип = Неопределено) Экспорт
...
Функция ПараметрыЗаполненияВедомостиНезачисленнымиСтроками(Ведомость)
	...
	ЗначенияЗаполнения.ЭтоДанныеЗаполненияНезачисленнымиСтроками = Истина;
...
Процедура ОткрытиеЛицевыхСчетовЗаполнить()
	...
	ДанныеСотрудников = ОбменСБанкамиПоЗарплатнымПроектам.ДанныеСотрудниковДляОткрытияЛицевыхСчетов(
			Запрос.МенеджерВременныхТаблиц, Объект.ДатаФормирования, Объект.Организация, Объект.ЗарплатныйПроект);
...
Процедура ПрочитатьСводныеСведения(МенеджерВременныхТаблиц)
	...
	Иначе
		...
		Иначе
			// КоличествоЧеловекБезЛицевыхСчетов > КоличествоЧеловекОткрытиеЛицевогоСчетаОтложено
...
Функция ОткрытиеЛицевыхСчетовЗаписатьНаСервере(РежимЗаписи, ИдентификаторСтроки = Неопределено, ДокументКопирования = Неопределено)
	...
	// 2 - Подобран документ с подразделением и физическим лицом, указаными в строке табличной части
	// 3 - Подобран документ с подразделением, указаным в отборе обработки и физическим лицом, указаным в строке табличной части
	// 4 - Подобран документ из других строк табличной части, без физического лица, указаного в строке табличной части
	// 5 - Подобран документ, соответсвующий отбору обработки и без физического лица, указаного в строке табличной части
