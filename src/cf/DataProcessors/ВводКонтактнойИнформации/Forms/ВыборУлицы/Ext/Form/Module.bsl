...
&НаКлиенте
Процедура НайтиАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
...
Функция ЕстьПодчиненныеЭлементы(Идентификатор)
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Свойство("ФорматАдреса", "ФИАС");
	ПараметрыПоиска.Свойство("СкрыватьНеактуальные", Истина);

	ДанныеКлассификатора = УправлениеКонтактнойИнформациейСлужебный.АдресаДляИнтерактивногоВыбора(Идентификатор, 90, ПараметрыПоиска);
	Если ДанныеКлассификатора.Данные.Количество() > 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура УстановитьДанныеПоПредставлениюУлицы(Представление, ИдентификаторРодителя)
	...
	Если АнализКлассификатора <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ЧастиУлицы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Представление, ",", Истина, Истина);
		Если ЧастиУлицы.Количество() = 3 Тогда
			Отбор = Новый Структура("Представление", ЧастиУлицы[0]);
			Варианты = ВариантыАдреса.НайтиСтроки(Отбор);
			Если Варианты.Количество() > 0 Тогда 
				Элементы.ВариантыАдреса.ТекущаяСтрока = Варианты[0].ПолучитьИдентификатор();
			КонецЕсли;
			ДополнительныйЭлемент =  ЧастиУлицы[1];
			ПодчиненныйЭлемент = ЧастиУлицы[2];
