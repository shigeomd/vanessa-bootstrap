...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеУчетнойПолитики" ИЛИ ИмяСобытия = "ОбновитьРегламентныеОперации" Тогда
...
// При изменении состава операций на форме, функцию необходимо переписать в первую очередь
&НаСервере
...
КонецПроцедуры // ПервоначальноеЗаполнениеТаблицыОпераций()
...
Процедура ЗаполнитьДанныеОпераций()
	...
	Если ПереходНаУСНЗапущен Тогда
		...
		ПерваяГруппаЗакрытияМесяцаВыполнена = ЗакрытиеМесяца.ВыполненыОперацииПредыдущихГрупп(ДатаПроверяемогоПериода, Объект.Организация, ВидОперацииДляПроверки, Неопределено);
...
КонецПроцедуры // ОбновитьСтатусыОпераций()
...
// Управление внешним видом формы, картинками элементов и доступностью в зависимости от состояния операций и реквизитов формы
&НаСервере
...
Процедура ОбновитьПараметрыПерехода()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		ТекстДатаПереходаНаУСН = НСтр("ru = 'Для начала перехода требуется установить применение УСН и указать дату перехода на УСН в учетной политике'");
		...
		Если ПрименяетсяУСН Тогда
			...
			|	УчетнаяПолитикаОрганизаций.ДатаПереходаНаУСН,
			|	УчетнаяПолитикаОрганизаций.ПрименяетсяУСНДоходыМинусРасходы
			|ИЗ
			|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(
			|			&ТекПериод,
			|			Организация = &Организация
			|				И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Общая)) КАК УчетнаяПолитикаОрганизацийСрезПоследних
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
			|		ПО УчетнаяПолитикаОрганизацийСрезПоследних.Период < УчетнаяПолитикаОрганизаций.Период
			|ГДЕ
			|	УчетнаяПолитикаОрганизаций.Период < &ТекПериод
		...
		Иначе
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	УчетнаяПолитикаОрганизацийСрезПервых.Период,
			|	УчетнаяПолитикаОрганизацийСрезПервых.Организация,
			|	УчетнаяПолитикаОрганизацийСрезПервых.ДатаПереходаНаУСН,
			|	УчетнаяПолитикаОрганизацийСрезПервых.ПрименяетсяУСНДоходыМинусРасходы
			|ИЗ
			|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПервых(
			|			&ТекПериод,
			|			Организация = &Организация
			|				И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная)) КАК УчетнаяПолитикаОрганизацийСрезПервых"
			;
		КонецЕсли;
...
КонецПроцедуры // ЗаполнитьЗависимостьОперацийПерехода()
...
КонецПроцедуры // ОпределитьНеобходимостьОперацииПерехода()
...
КонецФункции // ПолучитьДокументыПоОперации()
...
КонецФункции // СписокКомандДляОперации()
		...
		Если СтрокиПроверяемыхОпераций.Количество() > 0 Тогда					
			...
			Если Не СтрокаПроверяемойОперации.Состояние = ПредопределенноеЗначение("Перечисление.ВидыСостоянийРегламентныхОпераций.Выполнено")
				 ИЛИ Не СтрокаПроверяемойОперации.Актуальность Тогда
					...
					ТекстПредупреждения            = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстШаблон, ПредставлениеОперации, ПредставлениеТребуемойОперации);
...
КонецФункции // МожноВыполненитьОперацию(СтрокаОперации, ПредставлениеОперации)()
...
КонецПроцедуры // ОбработкаНажатияКнопкиМеню()
...
КонецПроцедуры // ЗафиксироватьВыполнение ОперацииПерехода()
...
КонецПроцедуры // ОтметитьОперациюВыполненнойИПропустить()
...
Процедура ОтменитьВыполнениеОперацииПерехода(СтрокаОперации, ФиксироватьУстаревшие = Ложь, Отказ = Ложь)
	...
	ПараметрыОперации.Вставить("СпособОбработкиДокументовОперации", СпособыОбработкиДокументов.Получить(СтрокаОперации.СпособОбработкиДокументовПриОтмене));
	
	УчетУСН.ОтменитьВыполнениеОперацииПереходаНаУСН(ПараметрыОперации, ФиксироватьУстаревшие, Отказ);

КонецПроцедуры // ОтменитьВыполнениеОперацииПерехода() 
...
Процедура ОткрытьСпискиДокументовПоОперации(СтрокаОперации)
	...
	ИначеЕсли СтрокаОперации.ИмяОперации = "ВозвратНДСПокупателям" Тогда
		...
		ПараметрыФормы = Новый Структура("Отбор, РежимОтбораПоДате", ПараметрыОтбора, "ДоДаты");
	...
	ИначеЕсли СтрокаОперации.ИмяОперации = "ПризнаниеАвансовДоходами" Тогда
		...
		ПараметрыФормы = Новый Структура("Отбор, РежимОтбораПоДате", ПараметрыОтбора, "СДаты");
...
КонецПроцедуры // ОткрытьСпискиДокументовПоОперации()
...
КонецПроцедуры // ОткрытьДокументПоОперации()
...
КонецПроцедуры // ОткрытьСформироватьОтчет()
 
&НаКлиенте
Процедура ОткрытьВнешнююФорму(СтрокаОперации)

	Если СтрокаОперации.ИмяОперации = "ПерейтиВЗакрытиеМесяца" Тогда
		ПараметрыФормы = Новый Структура("Организация, ПериодРегистрации, ПодготовкаКВосстановлениюНДСПриПереходеНаУСН", Объект.Организация, Объект.ДатаПерехода - 1, Истина);
		ОткрытьФорму("Обработка.ЗакрытиеМесяца.Форма", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор);
	КонецЕсли; 		

КонецПроцедуры // ОткрытьВнешнююФорму()
...
КонецПроцедуры // РаспечататьДокументПоОперации()
...
Процедура ОткрытьЗаписьРегистраСведений(СтрокаОперации)
	...
	Если СтрокаОперации.ИмяОперации = "ПроверкаУчетнойПолитики" Тогда
		...
		Если ПереходНаУСНЗапущен Тогда
			...
			КлючЗаписи = Новый(Тип("РегистрСведенийКлючЗаписи.УчетнаяПолитикаОрганизаций"), ПараметрыКлюча);
			
			Если Не КлючЗаписи.Пустой() Тогда
				ОткрытьФорму("РегистрСведений.УчетнаяПолитикаОрганизаций.ФормаЗаписи", Новый Структура("Ключ", КлючЗаписи), ЭтаФорма, УникальныйИдентификатор);		
			КонецЕсли;
			
		Иначе
			ОткрытьФорму("РегистрСведений.УчетнаяПолитикаОрганизаций.ФормаСписка", Новый Структура("Отбор", ОтборУчетнойПолитики), ЭтаФорма, УникальныйИдентификатор)
		КонецЕсли;  
...
КонецПроцедуры // ВыполнитьОперациюОбработкаДанныхНаКлиенте()
...
КонецФункции // ВыполнитьОперациюОбработкаДанныхНаСервере()
