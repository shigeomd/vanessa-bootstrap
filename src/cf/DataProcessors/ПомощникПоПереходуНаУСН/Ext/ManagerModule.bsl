...
Процедура ВыполнитьВосстановлениеНДС(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ТаблицаДанныхВосстановлениеНДС = Документы.ВосстановлениеНДС.СформироватьДанныеДляВосстановленияНДСПоОстаткам(ПараметрыДокумента);
...
Процедура ВключитьАвансыВДоходыУСН(СтруктураПараметров, АдресХранилища) Экспорт
	...
	СчетаАвансов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВал); // 62.02
	СчетаАвансов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымУЕ); // 62.02
	// получим субсчета (в случае, если добавлены пользователем)
	МассивСчетовАвансов = БухгалтерскийУчет.ПолучитьМассивСчетовССубсчетами(СчетаАвансов);
...
Процедура ОчиститьОстаткиРегистровНУ(СтруктураПараметров, АдресХранилища) Экспорт
	...
	СформироватьДвиженияПоРегиструНДСпоОСиНМА(СтруктураПараметров);
...
Процедура СформироватьДвиженияПоРегиструХозрасчетный(СтруктураПараметров)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ВыборкаДетальныеЗаписи.СуммаНУОстатокДт <> 0 или ВыборкаДетальныеЗаписи.СуммаПРОстатокДт <> 0 или ВыборкаДетальныеЗаписи.СуммаВРОстатокДт <> 0 Тогда
...
Процедура СформироватьДвиженияПоРегиструНДСпоОСиНМА(СтруктураПараметров)
	...
	Запрос.Текст = 
		...
		|	&Период
		|ИЗ
		|	РегистрНакопления.НДСпоОСиНМА.Остатки(&Граница, Организация = &Организация) КАК НДСпоОСиНМАОстатки
		|ГДЕ
		|	(НДСпоОСиНМАОстатки.СуммаБезНДСОстаток <> 0
		|			ИЛИ НДСпоОСиНМАОстатки.НДСОстаток <> 0)";
	...
	Если Не Результат.Пустой() Тогда
		НачатьТранзакцию();
		СтруктураПараметров.Вставить("РезультатЗапроса", Результат);
		СтруктураПараметров.Вставить("ИмяРегистра", "НДСпоОСиНМА");
		СформироватьДокументОперацияБух(СтруктураПараметров);
		ЗафиксироватьТранзакцию();
