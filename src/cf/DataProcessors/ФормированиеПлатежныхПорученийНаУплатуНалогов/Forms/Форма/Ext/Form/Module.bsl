...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаОстатков) Тогда
		Объект.ДатаОстатков = ОбщегоНазначенияБП.ПолучитьРабочуюДату();
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухОстаткиСубконто2");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухОстатки.Субконто2Доступность", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухОстаткиСубконто3");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухОстатки.Субконто3Доступность", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухОстаткиСчетУчета");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухОстатки.СчетУчета", ВидСравненияКомпоновкиДанных.Заполнено);

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.БухОстатки.СчетПредставление"));
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СтраницыСпособОплаты.Доступность  = ЗначениеЗаполнено(Объект.Организация);
	Элементы.ВыбранныйСпособОплаты.Доступность = Элементы.ВыбранныйСпособОплаты.СписокВыбора.Количество() > 1;
	
	Элементы.СтраницыСпособОплаты.ТекущаяСтраница = ?(Форма.ЭтоЮрЛицо, 
		Элементы.СтраницаОплатаСБанковскогоСчета, Элементы.СтраницаВыборСпособаОплаты);
...
Процедура БухОстаткиЗаполнитьНаСервере()
	...
	ТаблицаОстатков = Обработки.ФормированиеПлатежныхПорученийНаУплатуНалогов.ОстаткиНаСчетах68и69(
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	ПоляСубконто = Новый Структура("Субконто1, Субконто2, Субконто3", "Субконто1", "Субконто2", "Субконто3");
	
	Для Каждого СтрокаТаблицы Из Объект.БухОстатки Цикл
		
		БухгалтерскийУчетКлиентСервер.УстановитьДоступностьСубконто(СтрокаТаблицы.СчетУчета, СтрокаТаблицы, ПоляСубконто);
		
		СвойствоСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаТаблицы.СчетУчета);
		СтрокаТаблицы.СчетПредставление        = СокрЛП(СвойствоСчета.Код) + " " + СокрЛП(СвойствоСчета.Наименование);
		СтрокаТаблицы.СчетУчетПоПодразделениям = СвойствоСчета.УчетПоПодразделениям;
...
Функция СоздатьПлатежныеДокументыНаСервере(АктуализированыРеквизитыПлатежаВБюджет = Неопределено)
	...
	СтруктураПараметров.Вставить("Платежи",         ПоместитьВоВременноеХранилище(Объект.БухОстатки.Выгрузить(Отбор), УникальныйИдентификатор));
	
	Возврат Обработки.ФормированиеПлатежныхПорученийНаУплатуНалогов.СоздатьПлатежныеДокументы(СтруктураПараметров, АктуализированыРеквизитыПлатежаВБюджет);
