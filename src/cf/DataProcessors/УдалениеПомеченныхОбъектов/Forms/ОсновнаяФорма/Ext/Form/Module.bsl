...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СвязиНеУдаленныхПредставление.Имя);
...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		ТекущаяСтраница = Элементы.СтраницаВыберитеНеУдаленныйОбъект;
	...
	Иначе
		...
		Иначе
			...
			НеУдаленныеПодсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Места использования объекта ""%1"" (%2):'"),
...
Функция ИзменитьПометкуУдаленияОбъектов(МассивСсылок, ПометкаУдаления)
	...
	СтандартныеПодсистемыКлиентСервер.ОповеститьДинамическиеСписки(Результат, МассивСсылок);
...
Процедура ФоновоеЗаданиеЗагрузитьРезультат(Результат)
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		СтандартныеПодсистемыКлиентСервер.ОповеститьДинамическиеСписки(Результат, РезультатВыполненияВФоне.Удаленные);
