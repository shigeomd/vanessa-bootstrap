...
Процедура ВыполнитьСохранениеНумерацииВФоне(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Помощник перехода с 20.Восстановление нумерации'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ПараметрыПомощника = ОбновлениеСПредыдущейРедакции.ПараметрыПереходаСРедакции20();
	Параметры.Вставить("ПоказатьПомощникПереходаСРедакции20", ПараметрыПомощника.ЗапуститьПомощникПереходаСРедакции20);
