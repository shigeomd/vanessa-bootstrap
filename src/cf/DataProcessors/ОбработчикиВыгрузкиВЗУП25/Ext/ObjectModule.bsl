...
Процедура ПКС_ПередВыгрузкой_УчетнаяПолитикаНалоговыйУчет_ВидТарифаСтраховыхВзносов_Ресурс8(ФайлОбмена, Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, 
                                           ПКС, ПКО, ОбъектКоллекции, Отказ, Значение, ТипПриемника, ИмяПКО, 
                                           ИмяПКОВидСубконто, Пусто, Выражение, УзелКоллекцииСвойств, 
                                           НеЗамещать) Экспорт
	...
	Запрос.УстановитьПараметр("Период",Источник.Период);
	...
	Иначе
		...
		Значение = Справочники.ВидыТарифовСтраховыхВзносов.ПолучитьИмяПредопределенного(Выборка.ВидТарифа);
