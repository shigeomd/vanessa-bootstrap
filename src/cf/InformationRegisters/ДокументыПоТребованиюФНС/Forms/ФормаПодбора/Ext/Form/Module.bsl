...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Найти(ИмяСобытия, "Запись_") > 0 Тогда
		...
		Если Найти(ИмяСобытия, "Запись_СканированныеДокументыДляПередачиВЭлектронномВиде") > 0 Тогда
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НужноЗакрыватьФорму <> Истина Тогда
		
		Если МассивОтобранныхЗаписей.Количество() > 0 Тогда
			Отказ = Истина;
			Оповещение = Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект);
			ПоказатьВопрос(Оповещение, "Есть отобранные документы. Вы действительно хотите закрыть форму?", РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
&НаКлиенте
Процедура ОтборКонтрагентПредставлениеОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
...
Процедура ОтборКонтрагентПредставлениеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ОтборКонтрагентИНН) Тогда
		//введен вручную ИНН
		//ОтборИНН = СтрЗаменить(ОтборКонтрагентПредставление, " (ИНН)", "");
		ОтбработатьИзменениеИННКонтрагента(ОтборКонтрагентИНН);
...
&НаКлиенте
Процедура НадписьОтмеченоДокументовОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
...
Процедура ПодготовитьОтборПоКонтрагенту(ОтборСписка)
	...
	//добавим группу
	ГруппаОтбораИЛИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ОтборСписка.Элементы, , ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
...
Процедура ПодготовитьОтборПоПериоду(ОтборСписка)
	...
	//добавим группу
	ГруппаОтбораИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(ОтборСписка.Элементы, , ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
...
Процедура НадписьСкрытьИнформациюНажатие(Элемент)
	...
	|Для её открытия нажмите кнопку ""F1"" на клавиатуре или кнопку ""?"", расположенную в правом верхнем углу формы.");
