...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ФормаСтатистики) Тогда
		...
		Если ТипЗнч(Параметры.НастраиваемыеСтрокиОтчета) = Тип("Массив") И Параметры.НастраиваемыеСтрокиОтчета.Количество() > 0 Тогда
			...
			СтрокиОтчета = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Параметры.НастраиваемыеСтрокиОтчета);
	...
	Иначе
		...
		СписокВыбора.Добавить(
			...
			НСтр("ru = 'Все'")); // См. ЗаполнениеФормСтатистики.НастраиваемыеФормыСтатистикиПредоставляемыеОрганизацией()
...
Процедура ОбновитьСписокНастроек(ТекущаяСтрока = Неопределено)
	...
	|	ОбъектНаблюдения,
	|	Детализировать";
	// Список номеров строк формируется с помощью СтрокаИзМассиваПодстрок(), 
	// а она умеет бесследно пропускать пустые строки. 
	// Поэтому в последнем запросе пакета в поле НомерСтрокиОтчета безопасно возвращать пустую строку.
...
Функция СписокНомеровСтрок(ВыборкаИсходныхДанных, ТолькоСводные = Ложь)
	...
	Пока ВыборкаСтрок.Следующий() Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(НомераСтрок);
...
Функция ФормыДляЗаполненияСпискаНастроек()
	...
	Иначе
		...
		СписокФорм = ЗаполнениеФормСтатистики.НастраиваемыеФормыСтатистикиПредоставляемыеОрганизацией(Организация);
