...
Функция ОчиститьСписокНоменклатуныхГрупп(ВидДеятельности, ИсключаяХарактерДеятельности) Экспорт
	...
	ПараметрыБлокировки	= Новый Структура("ТипТаблицы, ИмяТаблицы, ИсточникДанных",
		"РегистрСведений", "ВидыДеятельностиПредпринимателей", ТаблицаБлокировки);
	ОписаниеИсточника	= Новый Структура("НоменклатурнаяГруппа, ХарактерДеятельности",
		"НоменклатурнаяГруппа", "ХарактерДеятельности");
	ОбщегоНазначенияБПВызовСервера.УстановитьУправляемуюБлокировку(ПараметрыБлокировки, , ОписаниеИсточника);
...
Функция ВидДеятельностиПоУмолчанию(Организация) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УчетнаяПолитикаОрганизацийСрезПоследних.ОсновнойВидДеятельности КАК ВидДеятельности
		|ИЗ
