...
Процедура ОбновитьГраницуАктуальности(Организация, Период, ВидОперации) Экспорт
	...
	Если УжеБылаУстаревшей Тогда
		СдвинутьГраницуАктуальностиВперед(Организация, Период, НомерГруппы, ВидОперации);
	Иначе
...
Процедура СдвинутьГраницуАктуальностиНазад(Организация, Период, НомерГруппы, ВидОперации) Экспорт
	...
	Запрос.УстановитьПараметр("СледующийМесяц",      НачалоМесяца(ДобавитьМесяц(Период, 1)));
	...
	|	Документ.РегламентнаяОперация КАК РегламентнаяОперация
	|ГДЕ
	|	РегламентнаяОперация.Дата >= &СледующийМесяц
	|	И РегламентнаяОперация.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийРегламентныхОпераций.Выполнено)
...
Процедура СдвинутьГраницуАктуальностиВперед(Организация, Период, НомерГруппы, ВидОперации)
	
	Если ЗакрытиеМесяца.ПредыдущиеОперацииВыполнены(Период, Организация, ВидОперации) Тогда
